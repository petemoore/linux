...
static bool tegra_pcie_port_check_link(struct tegra_pcie_port *port)
{
	...
	value = pete_readl("drivers/pci/controller/pci-tegra.c:2251", port->base + RP_PRIV_MISC);
	value &= ~RP_PRIV_MISC_PRSNT_MAP_EP_ABSNT;
	value |= RP_PRIV_MISC_PRSNT_MAP_EP_PRSNT;
	pete_writel("drivers/pci/controller/pci-tegra.c:2254", value, port->base + RP_PRIV_MISC);

	do {
		unsigned int timeout = TEGRA_PCIE_LINKUP_TIMEOUT;

		do {
			value = pete_readl("drivers/pci/controller/pci-tegra.c:2260", port->base + RP_VEND_XP);
		...
		do {
			value = pete_readl("drivers/pci/controller/pci-tegra.c:2276", port->base + RP_LINK_CONTROL_STATUS);
...
static void tegra_pcie_change_link_speed(struct tegra_pcie *pcie)
{
	...
	list_for_each_entry(port, &pcie->ports, list) {
		 ...
		 */
		value = pete_readl("drivers/pci/controller/pci-tegra.c:2305", port->base + RP_LINK_CONTROL_STATUS_2);
		value &= ~PCI_EXP_LNKSTA_CLS;
		value |= PCI_EXP_LNKSTA_CLS_5_0GB;
		pete_writel("drivers/pci/controller/pci-tegra.c:2308", value, port->base + RP_LINK_CONTROL_STATUS_2);
		...
		while (ktime_before(ktime_get(), deadline)) {
			value = pete_readl("drivers/pci/controller/pci-tegra.c:2317", port->base + RP_LINK_CONTROL_STATUS);
		...
		/* Retrain the link */
		value = pete_readl("drivers/pci/controller/pci-tegra.c:2329", port->base + RP_LINK_CONTROL_STATUS);
		value |= PCI_EXP_LNKCTL_RL;
		pete_writel("drivers/pci/controller/pci-tegra.c:2331", value, port->base + RP_LINK_CONTROL_STATUS);

		deadline = ktime_add_us(ktime_get(), LINK_RETRAIN_TIMEOUT);

		while (ktime_before(ktime_get(), deadline)) {
			value = pete_readl("drivers/pci/controller/pci-tegra.c:2336", port->base + RP_LINK_CONTROL_STATUS);
			if ((value & PCI_EXP_LNKSTA_LT) == 0)
				break;
...
static int tegra_pcie_ports_seq_show(struct seq_file *s, void *v)
{
	...
	value = pete_readl("drivers/pci/controller/pci-tegra.c:2569", port->base + RP_VEND_XP);

	if (value & RP_VEND_XP_DL_UP)
		up = true;

	value = pete_readl("drivers/pci/controller/pci-tegra.c:2574", port->base + RP_LINK_CONTROL_STATUS);
