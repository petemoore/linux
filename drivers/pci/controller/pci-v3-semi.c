				...
				unsigned int devfn, int offset)
{
	 ...
	 */
	pete_writel("drivers/pci/controller/pci-v3-semi.c:365", v3_addr_to_lb_base(v3->non_pre_mem) |
	 ...
	 */
	pete_writel("drivers/pci/controller/pci-v3-semi.c:373", v3_addr_to_lb_base(v3->config_mem) |
	       V3_LB_BASE_ADR_SIZE_16MB | V3_LB_BASE_ENABLE,
	       v3->base + V3_LB_BASE1);
	pete_writew("drivers/pci/controller/pci-v3-semi.c:376", mapaddress, v3->base + V3_LB_MAP1);
...
static void v3_unmap_bus(struct v3_pci *v3)
{
	 ...
	 */
	pete_writel("drivers/pci/controller/pci-v3-semi.c:386", v3_addr_to_lb_base(v3->pre_mem) |
	       V3_LB_BASE_ADR_SIZE_256MB | V3_LB_BASE_PREFETCH |
	       V3_LB_BASE_ENABLE,
	       v3->base + V3_LB_BASE1);
	pete_writew("drivers/pci/controller/pci-v3-semi.c:390", v3_addr_to_lb_map(v3->pre_bus_addr) |
	 ...
	 */
	pete_writel("drivers/pci/controller/pci-v3-semi.c:397", v3_addr_to_lb_base(v3->non_pre_mem) |
...
static irqreturn_t v3_irq(int irq, void *data)
{
	...
	status = pete_readw("drivers/pci/controller/pci-v3-semi.c:442", v3->base + V3_PCI_STAT);
	...
	pete_writew("drivers/pci/controller/pci-v3-semi.c:451", status, v3->base + V3_PCI_STAT);

	status = pete_readb("drivers/pci/controller/pci-v3-semi.c:453", v3->base + V3_LB_ISTAT);
	...
	pete_writeb("drivers/pci/controller/pci-v3-semi.c:471", 0, v3->base + V3_LB_ISTAT);
	if (v3->map)
...
static int v3_integrator_init(struct v3_pci *v3)
{
	...
	if (!(val & INTEGRATOR_SC_PCI_ENABLE)) {
		...
		pete_writel("drivers/pci/controller/pci-v3-semi.c:502", 0x6200, v3->base + V3_LB_IO_BASE);

		/* Wait for the mailbox to settle after reset */
		do {
			pete_writeb("drivers/pci/controller/pci-v3-semi.c:506", 0xaa, v3->base + V3_MAIL_DATA);
			pete_writeb("drivers/pci/controller/pci-v3-semi.c:507", 0x55, v3->base + V3_MAIL_DATA + 4);
		} while (pete_readb("drivers/pci/controller/pci-v3-semi.c:508", v3->base + V3_MAIL_DATA) != 0xaa &&
			 pete_readb("drivers/pci/controller/pci-v3-semi.c:509", v3->base + V3_MAIL_DATA) != 0x55);
	}
				 ...
				 struct resource_entry *win)
{
	...
	case IORESOURCE_IO:
		...
		pete_writel("drivers/pci/controller/pci-v3-semi.c:530", v3_addr_to_lb_base2(pci_pio_to_address(io->start)) |
		       V3_LB_BASE2_ENABLE,
		       v3->base + V3_LB_BASE2);
		pete_writew("drivers/pci/controller/pci-v3-semi.c:533", v3_addr_to_lb_map2(io->start - win->offset),
		       v3->base + V3_LB_MAP2);
		break;
	case IORESOURCE_MEM:
		mem = win->res;
		if (mem->flags & IORESOURCE_PREFETCH) {
			...
			pete_writel("drivers/pci/controller/pci-v3-semi.c:555", v3_addr_to_lb_base(v3->pre_mem) |
			       V3_LB_BASE_ADR_SIZE_256MB |
			       V3_LB_BASE_PREFETCH |
			       V3_LB_BASE_ENABLE,
			       v3->base + V3_LB_BASE1);
			pete_writew("drivers/pci/controller/pci-v3-semi.c:560", v3_addr_to_lb_map(v3->pre_bus_addr) |
			       V3_LB_MAP_TYPE_MEM, /* Was V3_LB_MAP_TYPE_MEM_MULTIPLE */
			       v3->base + V3_LB_MAP1);
		} else {
			...
			pete_writel("drivers/pci/controller/pci-v3-semi.c:575", v3_addr_to_lb_base(v3->non_pre_mem) |
			       V3_LB_BASE_ADR_SIZE_256MB |
			       V3_LB_BASE_ENABLE,
			       v3->base + V3_LB_BASE0);
			pete_writew("drivers/pci/controller/pci-v3-semi.c:579", v3_addr_to_lb_map(v3->non_pre_bus_addr) |
			       V3_LB_MAP_TYPE_MEM,
			       v3->base + V3_LB_MAP0);
		}
		break;
				       ...
				       struct device_node *np)
{
	...
	resource_list_for_each_entry(entry, &bridge->dma_ranges) {
		...
		if (i == 0) {
			pete_writel("drivers/pci/controller/pci-v3-semi.c:690", pci_base, v3->base + V3_PCI_BASE0);
			pete_writel("drivers/pci/controller/pci-v3-semi.c:691", pci_map, v3->base + V3_PCI_MAP0);
		} else if (i == 1) {
			pete_writel("drivers/pci/controller/pci-v3-semi.c:693", pci_base, v3->base + V3_PCI_BASE1);
			pete_writel("drivers/pci/controller/pci-v3-semi.c:694", pci_map, v3->base + V3_PCI_MAP1);
		} else {
...
static int v3_pci_probe(struct platform_device *pdev)
{
	 ...
	 */
	if (pete_readl("drivers/pci/controller/pci-v3-semi.c:746", v3->base + V3_LB_IO_BASE) != (regs->start >> 16))
		dev_err(dev, "V3_LB_IO_BASE = %08x but device is @%pR\n",
			pete_readl("drivers/pci/controller/pci-v3-semi.c:748", v3->base + V3_LB_IO_BASE), regs);
	 ...
	 */
	if (pete_readw("drivers/pci/controller/pci-v3-semi.c:778", v3->base + V3_SYSTEM) & V3_SYSTEM_M_LOCK)
		pete_writew("drivers/pci/controller/pci-v3-semi.c:779", V3_SYSTEM_UNLOCK, v3->base + V3_SYSTEM);

	/* Disable all slave access while we set up the windows */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:782", v3->base + V3_PCI_CMD);
	val &= ~(PCI_COMMAND_IO | PCI_COMMAND_MEMORY | PCI_COMMAND_MASTER);
	pete_writew("drivers/pci/controller/pci-v3-semi.c:784", val, v3->base + V3_PCI_CMD);

	/* Put the PCI bus into reset */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:787", v3->base + V3_SYSTEM);
	val &= ~V3_SYSTEM_M_RST_OUT;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:789", val, v3->base + V3_SYSTEM);

	/* Retry until we're ready */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:792", v3->base + V3_PCI_CFG);
	val |= V3_PCI_CFG_M_RETRY_EN;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:794", val, v3->base + V3_PCI_CFG);

	/* Set up the local bus protocol */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:797", v3->base + V3_LB_CFG);
	...
	pete_writew("drivers/pci/controller/pci-v3-semi.c:802", val, v3->base + V3_LB_CFG);

	/* Enable the PCI bus master */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:805", v3->base + V3_PCI_CMD);
	val |= PCI_COMMAND_MASTER;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:807", val, v3->base + V3_PCI_CMD);
	 ...
	 */
	pete_writel("drivers/pci/controller/pci-v3-semi.c:826", 0x00000000, v3->base + V3_PCI_IO_BASE);
	...
	pete_writew("drivers/pci/controller/pci-v3-semi.c:834", val, v3->base + V3_PCI_CFG);
	 ...
	 */
	pete_writew("drivers/pci/controller/pci-v3-semi.c:841", V3_FIFO_PRIO_LB_RD1_FLUSH_AP1 |
	 ...
	 * interrupts
	 */
	pete_writeb("drivers/pci/controller/pci-v3-semi.c:852", 0, v3->base + V3_LB_ISTAT);
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:853", v3->base + V3_LB_CFG);
	val |= V3_LB_CFG_LB_LB_INT;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:855", val, v3->base + V3_LB_CFG);
	pete_writeb("drivers/pci/controller/pci-v3-semi.c:856", V3_LB_ISTAT_PCI_WR | V3_LB_ISTAT_PCI_PERR,
	       v3->base + V3_LB_IMASK);
	...
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:867", v3->base + V3_PCI_CMD);
	val |= PCI_COMMAND_MEMORY | PCI_COMMAND_INVALIDATE;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:869", val, v3->base + V3_PCI_CMD);

	/* Clear pending interrupts */
	pete_writeb("drivers/pci/controller/pci-v3-semi.c:872", 0, v3->base + V3_LB_ISTAT);
	/* Read or write errors and parity errors cause interrupts */
	pete_writeb("drivers/pci/controller/pci-v3-semi.c:874", V3_LB_ISTAT_PCI_RD | V3_LB_ISTAT_PCI_WR | V3_LB_ISTAT_PCI_PERR,
	       v3->base + V3_LB_IMASK);

	/* Take the PCI bus out of reset so devices can initialize */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:878", v3->base + V3_SYSTEM);
	val |= V3_SYSTEM_M_RST_OUT;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:880", val, v3->base + V3_SYSTEM);

	/*
	 * Re-lock the system register.
	 */
	val = pete_readw("drivers/pci/controller/pci-v3-semi.c:885", v3->base + V3_SYSTEM);
	val |= V3_SYSTEM_M_LOCK;
	pete_writew("drivers/pci/controller/pci-v3-semi.c:887", val, v3->base + V3_SYSTEM);
