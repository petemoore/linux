				      ...
				      enum iproc_pcie_reg reg)
{
	...
	return pete_readl("drivers/pci/controller/pcie-iproc.c:423", pcie->base + offset);
}
					...
					enum iproc_pcie_reg reg, u32 val)
{
	...
	pete_writel("drivers/pci/controller/pcie-iproc.c:434", val, pcie->base + offset);
}
					 ...
					 void __iomem *cfg_data_p)
{
	 ...
	 */
	data = pete_readl("drivers/pci/controller/pcie-iproc.c:504", cfg_data_p);
	while (data == CFG_RETRY_STATUS && timeout--) {
		...
		udelay(1);
		data = pete_readl("drivers/pci/controller/pcie-iproc.c:516", cfg_data_p);
	}
				       ...
				       int size, u32 *val)
{
	...
	*val = pete_readl("drivers/pci/controller/pcie-iproc.c:664", addr);
					...
					int size, u32 val)
{
	...
	if (size == 4) {
		pete_writel("drivers/pci/controller/pcie-iproc.c:684", val, addr);
		return PCIBIOS_SUCCESSFUL;
	}

	mask = ~(((1 << (size * 8)) - 1) << ((where & 0x3) * 8));
	tmp = pete_readl("drivers/pci/controller/pcie-iproc.c:689", addr) & mask;
	tmp |= val << ((where & 0x3) * 8);
	pete_writel("drivers/pci/controller/pcie-iproc.c:691", tmp, addr);
				      ...
				      int size_idx, u64 axi_addr, u64 pci_addr)
{
	 ...
	 */
	pete_writel("drivers/pci/controller/pcie-iproc.c:874", lower_32_bits(axi_addr) | (size_idx << OARR_SIZE_CFG_SHIFT) |
	       OARR_VALID, pcie->base + oarr_offset);
	pete_writel("drivers/pci/controller/pcie-iproc.c:876", upper_32_bits(axi_addr), pcie->base + oarr_offset + 4);

	/* now program the OMAP registers */
	pete_writel("drivers/pci/controller/pcie-iproc.c:879", lower_32_bits(pci_addr), pcie->base + omap_offset);
	pete_writel("drivers/pci/controller/pcie-iproc.c:880", upper_32_bits(pci_addr), pcie->base + omap_offset + 4);

	dev_dbg(dev, "ob window [%d]: offset 0x%x axi %pap pci %pap\n",
		window_idx, oarr_offset, &axi_addr, &pci_addr);
	dev_dbg(dev, "oarr lo 0x%x oarr hi 0x%x\n",
		pete_readl("drivers/pci/controller/pcie-iproc.c:885", pcie->base + oarr_offset),
		pete_readl("drivers/pci/controller/pcie-iproc.c:886", pcie->base + oarr_offset + 4));
	dev_dbg(dev, "omap lo 0x%x omap hi 0x%x\n",
		pete_readl("drivers/pci/controller/pcie-iproc.c:888", pcie->base + omap_offset),
		pete_readl("drivers/pci/controller/pcie-iproc.c:889", pcie->base + omap_offset + 4));
			       ...
			       u64 pci_addr, resource_size_t size)
{
	 ...
	 */
	pete_writel("drivers/pci/controller/pcie-iproc.c:1080", lower_32_bits(pci_addr) | BIT(size_idx),
	       pcie->base + iarr_offset);
	pete_writel("drivers/pci/controller/pcie-iproc.c:1082", upper_32_bits(pci_addr), pcie->base + iarr_offset + 4);

	dev_dbg(dev, "iarr lo 0x%x iarr hi 0x%x\n",
		pete_readl("drivers/pci/controller/pcie-iproc.c:1085", pcie->base + iarr_offset),
		pete_readl("drivers/pci/controller/pcie-iproc.c:1086", pcie->base + iarr_offset + 4));
	...
	for (window_idx = 0; window_idx < nr_windows; window_idx++) {
		val = pete_readl("drivers/pci/controller/pcie-iproc.c:1094", pcie->base + imap_offset);
		val |= lower_32_bits(axi_addr) | IMAP_VALID;
		pete_writel("drivers/pci/controller/pcie-iproc.c:1096", val, pcie->base + imap_offset);
		pete_writel("drivers/pci/controller/pcie-iproc.c:1097", upper_32_bits(axi_addr),
		       pcie->base + imap_offset + ib_map->imap_addr_offset);

		dev_dbg(dev, "imap window [%d] lo 0x%x hi 0x%x\n",
			window_idx, pete_readl("drivers/pci/controller/pcie-iproc.c:1101", pcie->base + imap_offset),
			pete_readl("drivers/pci/controller/pcie-iproc.c:1102", pcie->base + imap_offset +
