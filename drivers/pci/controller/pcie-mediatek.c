...
static void mtk_msi_ack_irq(struct irq_data *data)
{
	...
	pete_writel("drivers/pci/controller/pcie-mediatek.c:421", 1 << hwirq, port->base + PCIE_IMSI_STATUS);
}
...
static void mtk_pcie_enable_msi(struct mtk_pcie_port *port)
{
	...
	pete_writel("drivers/pci/controller/pcie-mediatek.c:525", val, port->base + PCIE_IMSI_ADDR);

	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:527", port->base + PCIE_INT_MASK);
	val &= ~MSI_MASK;
	pete_writel("drivers/pci/controller/pcie-mediatek.c:529", val, port->base + PCIE_INT_MASK);
}
...
static void mtk_pcie_intr_handler(struct irq_desc *desc)
{
	...
	status = pete_readl("drivers/pci/controller/pcie-mediatek.c:606", port->base + PCIE_INT_STATUS);
	if (status & INTX_MASK) {
		for_each_set_bit_from(bit, &status, PCI_NUM_INTX + INTX_SHIFT) {
			/* Clear the INTx */
			pete_writel("drivers/pci/controller/pcie-mediatek.c:610", 1 << bit, port->base + PCIE_INT_STATUS);
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		if (status & MSI_STATUS){
			 ...
			 */
			pete_writel("drivers/pci/controller/pcie-mediatek.c:627", MSI_STATUS, port->base + PCIE_INT_STATUS);
			while ((imsi_status = pete_readl("drivers/pci/controller/pcie-mediatek.c:628", port->base + PCIE_IMSI_STATUS))) {
...
static int mtk_pcie_startup_port_v2(struct mtk_pcie_port *port)
{
	...
	if (pcie->base) {
		val = pete_readl("drivers/pci/controller/pcie-mediatek.c:684", pcie->base + PCIE_SYS_CFG_V2);
		val |= PCIE_CSR_LTSSM_EN(port->slot) |
		       PCIE_CSR_ASPM_L1_EN(port->slot);
		pete_writel("drivers/pci/controller/pcie-mediatek.c:687", val, pcie->base + PCIE_SYS_CFG_V2);
	} else if (pcie->cfg) {
		val = PCIE_CSR_LTSSM_EN(port->slot) |
		      PCIE_CSR_ASPM_L1_EN(port->slot);
	...
	pete_writel("drivers/pci/controller/pcie-mediatek.c:695", 0, port->base + PCIE_RST_CTRL);
	 ...
	 * space.
	 */
	pete_writel("drivers/pci/controller/pcie-mediatek.c:702", PCIE_LINKDOWN_RST_EN, port->base + PCIE_RST_CTRL);
	...
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:712", port->base + PCIE_RST_CTRL);
	val |= PCIE_PHY_RSTB | PCIE_PERSTB | PCIE_PIPE_SRSTB |
	       PCIE_MAC_SRSTB | PCIE_CRSTB;
	pete_writel("drivers/pci/controller/pcie-mediatek.c:715", val, port->base + PCIE_RST_CTRL);

	/* Set up vendor ID and class code */
	if (soc->need_fix_class_id) {
		val = PCI_VENDOR_ID_MEDIATEK;
		pete_writew("drivers/pci/controller/pcie-mediatek.c:720", val, port->base + PCIE_CONF_VEND_ID);

		val = PCI_CLASS_BRIDGE_PCI;
		pete_writew("drivers/pci/controller/pcie-mediatek.c:723", val, port->base + PCIE_CONF_CLASS_ID);
	}

	if (soc->need_fix_device_id)
		pete_writew("drivers/pci/controller/pcie-mediatek.c:727", soc->device_id, port->base + PCIE_CONF_DEVICE_ID);
	...
	/* Set INTx mask */
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:737", port->base + PCIE_INT_MASK);
	val &= ~INTX_MASK;
	pete_writel("drivers/pci/controller/pcie-mediatek.c:739", val, port->base + PCIE_INT_MASK);
	...
	pete_writel("drivers/pci/controller/pcie-mediatek.c:747", val, port->base + PCIE_AHB_TRANS_BASE0_L);

	val = upper_32_bits(mem->start);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:750", val, port->base + PCIE_AHB_TRANS_BASE0_H);

	/* Set PCIe to AXI translation memory space.*/
	val = PCIE2AHB_SIZE | WIN_ENABLE;
	pete_writel("drivers/pci/controller/pcie-mediatek.c:754", val, port->base + PCIE_AXI_WINDOW0);
				      ...
				      unsigned int devfn, int where)
{
	...
	pete_writel("drivers/pci/controller/pcie-mediatek.c:764", PCIE_CONF_ADDR(where, PCI_FUNC(devfn), PCI_SLOT(devfn),
...
static int mtk_pcie_startup_port(struct mtk_pcie_port *port)
{
	...
	/* assert port PERST_N */
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:785", pcie->base + PCIE_SYS_CFG);
	val |= PCIE_PORT_PERST(port->slot);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:787", val, pcie->base + PCIE_SYS_CFG);

	/* de-assert port PERST_N */
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:790", pcie->base + PCIE_SYS_CFG);
	val &= ~PCIE_PORT_PERST(port->slot);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:792", val, pcie->base + PCIE_SYS_CFG);
	...
	/* enable interrupt */
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:802", pcie->base + PCIE_INT_ENABLE);
	val |= PCIE_PORT_INT_EN(port->slot);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:804", val, pcie->base + PCIE_INT_ENABLE);

	/* map to all DDR region. We need to set it before cfg operation. */
	pete_writel("drivers/pci/controller/pcie-mediatek.c:807", PCIE_BAR_MAP_MAX | PCIE_BAR_ENABLE,
	       port->base + PCIE_BAR0_SETUP);

	/* configure class code and revision ID */
	pete_writel("drivers/pci/controller/pcie-mediatek.c:811", PCIE_CLASS_CODE | PCIE_REVISION_ID, port->base + PCIE_CLASS);

	/* configure FC credit */
	pete_writel("drivers/pci/controller/pcie-mediatek.c:814", PCIE_CONF_ADDR(PCIE_FC_CREDIT, func, slot, 0),
	       pcie->base + PCIE_CFG_ADDR);
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:816", pcie->base + PCIE_CFG_DATA);
	val &= ~PCIE_FC_CREDIT_MASK;
	val |= PCIE_FC_CREDIT_VAL(0x806c);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:819", PCIE_CONF_ADDR(PCIE_FC_CREDIT, func, slot, 0),
	       pcie->base + PCIE_CFG_ADDR);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:821", val, pcie->base + PCIE_CFG_DATA);

	/* configure RC FTS number to 250 when it leaves L0s */
	pete_writel("drivers/pci/controller/pcie-mediatek.c:824", PCIE_CONF_ADDR(PCIE_FTS_NUM, func, slot, 0),
	       pcie->base + PCIE_CFG_ADDR);
	val = pete_readl("drivers/pci/controller/pcie-mediatek.c:826", pcie->base + PCIE_CFG_DATA);
	val &= ~PCIE_FTS_NUM_MASK;
	val |= PCIE_FTS_NUM_L0(0x50);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:829", PCIE_CONF_ADDR(PCIE_FTS_NUM, func, slot, 0),
	       pcie->base + PCIE_CFG_ADDR);
	pete_writel("drivers/pci/controller/pcie-mediatek.c:831", val, pcie->base + PCIE_CFG_DATA);
