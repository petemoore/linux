...
static int keembay_pcie_pll_init(struct keembay_pcie *pcie)
{
	...
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:216", val, pcie->apb_base + PCIE_REGS_LJPLL_CNTRL_2);

	val = FIELD_PREP(LJPLL_POST_DIV3A, 0x2) |
		FIELD_PREP(LJPLL_POST_DIV2A, 0x2);
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:220", val, pcie->apb_base + PCIE_REGS_LJPLL_CNTRL_3);

	val = FIELD_PREP(LJPLL_EN, 0x1) | FIELD_PREP(LJPLL_FOUT_EN, 0xc);
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:223", val, pcie->apb_base + PCIE_REGS_LJPLL_CNTRL_0);
...
static void keembay_pcie_msi_irq_handler(struct irq_desc *desc)
{
	...
	pp = &pcie->pci.pp;
	val = pete_readl("drivers/pci/controller/dwc/pcie-keembay.c:253", pcie->apb_base + PCIE_REGS_INTERRUPT_STATUS);
	mask = pete_readl("drivers/pci/controller/dwc/pcie-keembay.c:254", pcie->apb_base + PCIE_REGS_INTERRUPT_ENABLE);

	status = val & mask;

	if (status & MSI_CTRL_INT) {
		dw_handle_msi_irq(pp);
		pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:260", status, pcie->apb_base + PCIE_REGS_INTERRUPT_STATUS);
	}
...
static void keembay_pcie_ep_init(struct dw_pcie_ep *ep)
{
	...
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:288", EDMA_INT_EN, pcie->apb_base + PCIE_REGS_INTERRUPT_ENABLE);
}
				      ...
				      struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/pci/controller/dwc/pcie-keembay.c:360", pcie->apb_base + PCIE_REGS_PCIE_PHY_CNTL);
	val |= PHY0_SRAM_BYPASS;
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:362", val, pcie->apb_base + PCIE_REGS_PCIE_PHY_CNTL);

	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:364", PCIE_DEVICE_TYPE, pcie->apb_base + PCIE_REGS_PCIE_CFG);

	ret = keembay_pcie_pll_init(pcie);
	if (ret)
		return ret;

	val = pete_readl("drivers/pci/controller/dwc/pcie-keembay.c:370", pcie->apb_base + PCIE_REGS_PCIE_CFG);
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:371", val | PCIE_RSTN, pcie->apb_base + PCIE_REGS_PCIE_CFG);
	...
	val = pete_readl("drivers/pci/controller/dwc/pcie-keembay.c:381", pcie->apb_base + PCIE_REGS_INTERRUPT_ENABLE);
	if (IS_ENABLED(CONFIG_PCI_MSI))
		val |= MSI_CTRL_INT_EN;
	pete_writel("drivers/pci/controller/dwc/pcie-keembay.c:384", val, pcie->apb_base + PCIE_REGS_INTERRUPT_ENABLE);
