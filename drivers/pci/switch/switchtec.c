...
{
	pete_writeq("drivers/pci/switch/switchtec.c:1245", stdev->dma_mrpc_dma_addr, &stdev->mmio_mrpc->dma_addr);
	flush_wc_buf(stdev);
...
static void stdev_release(struct device *dev)
{
	...
	if (stdev->dma_mrpc) {
		...
		flush_wc_buf(stdev);
		pete_writeq("drivers/pci/switch/switchtec.c:1257", 0, &stdev->mmio_mrpc->dma_addr);
