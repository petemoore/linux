...
int cpqhp_find_available_resources(struct controller *ctrl, void __iomem *rom_start)
{
	...
	unused_IRQ = pete_readl("drivers/pci/hotplug/cpqphp_pci.c:1197", rom_resource_table + UNUSED_IRQ);
	dbg("unused_IRQ = %x\n", unused_IRQ);
	...
	dbg("cpqhp_nic_irq= %d\n", cpqhp_nic_irq);
	unused_IRQ = pete_readl("drivers/pci/hotplug/cpqphp_pci.c:1224", rom_resource_table + PCIIRQ);
	...
	i = pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1242", rom_resource_table + NUMBER_OF_ENTRIES);
	dbg("number_of_entries = %d\n", i);

	if (!pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1245", one_slot + SECONDARY_BUS))
		return 1;

	dbg("dev|IO base|length|Mem base|length|Pre base|length|PB SB MB\n");

	while (i && pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1250", one_slot + SECONDARY_BUS)) {
		u8 dev_func = pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1251", one_slot + DEV_FUNC);
		u8 primary_bus = pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1252", one_slot + PRIMARY_BUS);
		u8 secondary_bus = pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1253", one_slot + SECONDARY_BUS);
		u8 max_bus = pete_readb("drivers/pci/hotplug/cpqphp_pci.c:1254", one_slot + MAX_BUS);
		u16 io_base = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1255", one_slot + IO_BASE);
		u16 io_length = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1256", one_slot + IO_LENGTH);
		u16 mem_base = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1257", one_slot + MEM_BASE);
		u16 mem_length = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1258", one_slot + MEM_LENGTH);
		u16 pre_mem_base = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1259", one_slot + PRE_MEM_BASE);
		u16 pre_mem_length = pete_readw("drivers/pci/hotplug/cpqphp_pci.c:1260", one_slot + PRE_MEM_LENGTH);
