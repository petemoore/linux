...
static irqreturn_t mtk_disp_rdma_irq_handler(int irq, void *dev_id)
{
	...
	pete_writel("drivers/gpu/drm/mediatek/mtk_disp_rdma.c:96", 0x0, priv->regs + DISP_REG_RDMA_INT_STATUS);
			     ...
			     unsigned int mask, unsigned int val)
{
	...
	unsigned int tmp = pete_readl("drivers/gpu/drm/mediatek/mtk_disp_rdma.c:110", rdma->regs + reg);

	tmp = (tmp & ~mask) | (val & mask);
	pete_writel("drivers/gpu/drm/mediatek/mtk_disp_rdma.c:113", tmp, rdma->regs + reg);
}
...
static int mtk_disp_rdma_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/gpu/drm/mediatek/mtk_disp_rdma.c:357", 0x0, priv->regs + DISP_REG_RDMA_INT_ENABLE);
	pete_writel("drivers/gpu/drm/mediatek/mtk_disp_rdma.c:358", 0x0, priv->regs + DISP_REG_RDMA_INT_STATUS);
