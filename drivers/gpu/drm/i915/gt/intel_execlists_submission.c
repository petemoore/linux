...
static void write_desc(struct intel_engine_execlists *execlists, u64 desc, u32 port)
{
	if (execlists->ctrl_reg) {
		pete_writel("drivers/gpu/drm/i915/gt/intel_execlists_submission.c:729", lower_32_bits(desc), execlists->submit_reg + port * 2);
		pete_writel("drivers/gpu/drm/i915/gt/intel_execlists_submission.c:730", upper_32_bits(desc), execlists->submit_reg + port * 2 + 1);
	} else {
		pete_writel("drivers/gpu/drm/i915/gt/intel_execlists_submission.c:732", upper_32_bits(desc), execlists->submit_reg);
		pete_writel("drivers/gpu/drm/i915/gt/intel_execlists_submission.c:733", lower_32_bits(desc), execlists->submit_reg);
	}
}
...
static void execlists_submit_ports(struct intel_engine_cs *engine)
{
	...
	if (execlists->ctrl_reg)
		pete_writel("drivers/gpu/drm/i915/gt/intel_execlists_submission.c:941", EL_CTRL_LOAD, execlists->ctrl_reg);
}
