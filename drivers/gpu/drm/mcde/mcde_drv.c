...
static int mcde_probe(struct platform_device *pdev)
{
	 ...
	 * need them.
	 */
	pid = pete_readl("drivers/gpu/drm/mcde/mcde_drv.c:365", mcde->regs + MCDE_PID);
	...
	pete_writel("drivers/gpu/drm/mcde/mcde_drv.c:383", 0, mcde->regs + MCDE_IMSCERR);
	pete_writel("drivers/gpu/drm/mcde/mcde_drv.c:384", 0xFFFFFFFF, mcde->regs + MCDE_RISERR);
