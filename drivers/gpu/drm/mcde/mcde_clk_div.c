...
static int mcde_clk_div_enable(struct clk_hw *hw)
{
	...
	val = pete_readl("drivers/gpu/drm/mcde/mcde_clk_div.c:24", mcde->regs + cdiv->cr);
	/*
	...
	pete_writel("drivers/gpu/drm/mcde/mcde_clk_div.c:38", val, mcde->regs + cdiv->cr);
					       ...
					       unsigned long prate)
{
	...
	cr = pete_readl("drivers/gpu/drm/mcde/mcde_clk_div.c:98", mcde->regs + cdiv->cr);
	if (cr & MCDE_CRX1_BCD)
		return prate;
