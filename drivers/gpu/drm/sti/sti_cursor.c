...
#define DBGFS_DUMP(reg) seq_printf(s, "\n  %-25s 0x%08X", #reg, \
				   pete_readl("drivers/gpu/drm/sti/sti_cursor.c:81", cursor->regs + reg))
...
static int cursor_dbg_show(struct seq_file *s, void *data)
{
	...
	DBGFS_DUMP(CUR_CTL);
	DBGFS_DUMP(CUR_VPO);
	cursor_dbg_vpo(s, pete_readl("drivers/gpu/drm/sti/sti_cursor.c:117", cursor->regs + CUR_VPO));
	DBGFS_DUMP(CUR_PML);
	cursor_dbg_pml(s, cursor, pete_readl("drivers/gpu/drm/sti/sti_cursor.c:119", cursor->regs + CUR_PML));
	DBGFS_DUMP(CUR_PMP);
	DBGFS_DUMP(CUR_SIZE);
	cursor_dbg_size(s, pete_readl("drivers/gpu/drm/sti/sti_cursor.c:122", cursor->regs + CUR_SIZE));
	DBGFS_DUMP(CUR_CML);
	cursor_dbg_cml(s, cursor, pete_readl("drivers/gpu/drm/sti/sti_cursor.c:124", cursor->regs + CUR_CML));
	DBGFS_DUMP(CUR_AWS);
	DBGFS_DUMP(CUR_AWE);
	seq_putc(s, '\n');
	return 0;
				     ...
				     struct drm_atomic_state *state)
{
	...
	val = y << 16 | x;
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:293", val, cursor->regs + CUR_AWS);
	y = sti_vtg_get_line_number(*mode, mode->vdisplay - 1);
	x = sti_vtg_get_pixel_number(*mode, mode->hdisplay - 1);
	val = y << 16 | x;
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:297", val, cursor->regs + CUR_AWE);

	/* Set memory location, size, and position */
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:300", cursor->pixmap.paddr, cursor->regs + CUR_PML);
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:301", cursor->width, cursor->regs + CUR_PMP);
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:302", cursor->height << 16 | cursor->width, cursor->regs + CUR_SIZE);

	y = sti_vtg_get_line_number(*mode, dst_y);
	x = sti_vtg_get_pixel_number(*mode, dst_x);
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:306", (y << 16) | x, cursor->regs + CUR_VPO);

	/* Set and fetch CLUT */
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:309", cursor->clut_paddr, cursor->regs + CUR_CML);
	pete_writel("drivers/gpu/drm/sti/sti_cursor.c:310", CUR_CTL_CLUT_UPDATE, cursor->regs + CUR_CTL);
