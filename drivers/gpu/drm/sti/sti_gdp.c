...
#define DBGFS_DUMP(reg) seq_printf(s, "\n  %-25s 0x%08X", #reg, \
				   pete_readl("drivers/gpu/drm/sti/sti_gdp.c:149", gdp->regs + reg ## _OFFSET))
...
static int gdp_dbg_show(struct seq_file *s, void *data)
{
	...
	gdp_dbg_ctl(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:230", gdp->regs + GAM_GDP_CTL_OFFSET));
	DBGFS_DUMP(GAM_GDP_AGC);
	DBGFS_DUMP(GAM_GDP_VPO);
	gdp_dbg_vpo(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:233", gdp->regs + GAM_GDP_VPO_OFFSET));
	DBGFS_DUMP(GAM_GDP_VPS);
	gdp_dbg_vps(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:235", gdp->regs + GAM_GDP_VPS_OFFSET));
	DBGFS_DUMP(GAM_GDP_PML);
	DBGFS_DUMP(GAM_GDP_PMP);
	DBGFS_DUMP(GAM_GDP_SIZE);
	gdp_dbg_size(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:239", gdp->regs + GAM_GDP_SIZE_OFFSET));
	DBGFS_DUMP(GAM_GDP_NVN);
	gdp_dbg_nvn(s, gdp, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:241", gdp->regs + GAM_GDP_NVN_OFFSET));
	DBGFS_DUMP(GAM_GDP_KEY1);
	DBGFS_DUMP(GAM_GDP_KEY2);
	DBGFS_DUMP(GAM_GDP_PPT);
	gdp_dbg_ppt(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:245", gdp->regs + GAM_GDP_PPT_OFFSET));
	DBGFS_DUMP(GAM_GDP_CML);
	DBGFS_DUMP(GAM_GDP_MST);
	gdp_dbg_mst(s, pete_readl("drivers/gpu/drm/sti/sti_gdp.c:248", gdp->regs + GAM_GDP_MST_OFFSET));
...
static struct sti_gdp_node_list *sti_gdp_get_free_nodes(struct sti_gdp *gdp)
{
	...
	hw_nvn = pete_readl("drivers/gpu/drm/sti/sti_gdp.c:402", gdp->regs + GAM_GDP_NVN_OFFSET);
	if (!hw_nvn)
		goto end;
...
struct sti_gdp_node_list *sti_gdp_get_current_nodes(struct sti_gdp *gdp)
{
	...
	hw_nvn = pete_readl("drivers/gpu/drm/sti/sti_gdp.c:434", gdp->regs + GAM_GDP_NVN_OFFSET);
	if (!hw_nvn)
		goto end;
				  ...
				  struct drm_atomic_state *state)
{
	...
	dev_dbg(gdp->dev, "Current NVN:0x%X\n",
		pete_readl("drivers/gpu/drm/sti/sti_gdp.c:835", gdp->regs + GAM_GDP_NVN_OFFSET));
	dev_dbg(gdp->dev, "Posted buff: %lx current buff: %x\n",
		(unsigned long) dma_obj->dma_addr,
		pete_readl("drivers/gpu/drm/sti/sti_gdp.c:838", gdp->regs + GAM_GDP_PML_OFFSET));
	...
	if (!curr_list) {
		...
		pete_writel("drivers/gpu/drm/sti/sti_gdp.c:846", gdp->is_curr_top ?
	...
	if (mode->flags & DRM_MODE_FLAG_INTERLACE) {
		if (gdp->is_curr_top) {
			...
			pete_writel("drivers/gpu/drm/sti/sti_gdp.c:860", dma_updated_top,
			       gdp->regs + GAM_GDP_NVN_OFFSET);
		}
	} else {
		/* Direct update for progressive to avoid one frame delay */
		pete_writel("drivers/gpu/drm/sti/sti_gdp.c:865", dma_updated_top, gdp->regs + GAM_GDP_NVN_OFFSET);
	}
