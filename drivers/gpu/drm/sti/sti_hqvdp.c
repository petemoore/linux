...
static int sti_hqvdp_get_free_cmd(struct sti_hqvdp *hqvdp)
{
	...
	curr_cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:384", hqvdp->regs + HQVDP_MBX_CURRENT_CMD);
	next_cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:385", hqvdp->regs + HQVDP_MBX_NEXT_CMD);
...
static int sti_hqvdp_get_curr_cmd(struct sti_hqvdp *hqvdp)
{
	...
	curr_cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:412", hqvdp->regs + HQVDP_MBX_CURRENT_CMD);
...
static int sti_hqvdp_get_next_cmd(struct sti_hqvdp *hqvdp)
{
	...
	next_cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:440", hqvdp->regs + HQVDP_MBX_NEXT_CMD);
...
#define DBGFS_DUMP(reg) seq_printf(s, "\n  %-25s 0x%08X", #reg, \
				   pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:453", hqvdp->regs + reg))
...
static int hqvdp_dbg_show(struct seq_file *s, void *data)
{
	...
	infoxp70 = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:579", hqvdp->regs + HQVDP_MBX_INFO_XP70);
	...
	if (pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:592", hqvdp->regs + HQVDP_MBX_STARTUP_CTRL1)
					& STARTUP_CTRL1_RST_DONE)
		seq_puts(s, "\tReset is done");
	else
		seq_puts(s, "\tReset is NOT done");
	DBGFS_DUMP(HQVDP_MBX_STARTUP_CTRL2);
	if (pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:598", hqvdp->regs + HQVDP_MBX_STARTUP_CTRL2)
	...
	if (!(pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:607", hqvdp->regs + HQVDP_MBX_SOFT_VSYNC) & 3))
		seq_puts(s, "\tHW Vsync");
	else
		seq_puts(s, "\tSW Vsync ?!?!");

	/* Last command */
	cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:613", hqvdp->regs + HQVDP_MBX_CURRENT_CMD);
	...
	/* Next command */
	cmd = pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:625", hqvdp->regs + HQVDP_MBX_NEXT_CMD);
...
static void sti_hqvdp_disable(struct sti_hqvdp *hqvdp)
{
	...
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:767", 0, hqvdp->regs + HQVDP_MBX_NEXT_CMD);

	for (i = 0; i < POLL_MAX_ATTEMPT; i++) {
		if (pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:770", hqvdp->regs + HQVDP_MBX_INFO_XP70)
				& INFO_XP70_FW_READY)
			break;
...
static int sti_hqvdp_vtg_cb(struct notifier_block *nb, unsigned long evt, void *data)
{
	...
	if (hqvdp->btm_field_pending) {
		...
		pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:837", hqvdp->hqvdp_cmd_paddr + btm_cmd_offset,
...
static void sti_hqvdp_init_plugs(struct sti_hqvdp *hqvdp)
{
	...
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:875", PLUG_PAGE_SIZE_256, hqvdp->regs + HQVDP_RD_PLUG_PAGE_SIZE);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:876", PLUG_MIN_OPC_8, hqvdp->regs + HQVDP_RD_PLUG_MIN_OPC);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:877", PLUG_MAX_OPC_64, hqvdp->regs + HQVDP_RD_PLUG_MAX_OPC);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:878", PLUG_MAX_CHK_2X, hqvdp->regs + HQVDP_RD_PLUG_MAX_CHK);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:879", PLUG_MAX_MSG_1X, hqvdp->regs + HQVDP_RD_PLUG_MAX_MSG);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:880", PLUG_MIN_SPACE_1, hqvdp->regs + HQVDP_RD_PLUG_MIN_SPACE);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:881", PLUG_CONTROL_ENABLE, hqvdp->regs + HQVDP_RD_PLUG_CONTROL);

	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:883", PLUG_PAGE_SIZE_256, hqvdp->regs + HQVDP_WR_PLUG_PAGE_SIZE);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:884", PLUG_MIN_OPC_8, hqvdp->regs + HQVDP_WR_PLUG_MIN_OPC);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:885", PLUG_MAX_OPC_64, hqvdp->regs + HQVDP_WR_PLUG_MAX_OPC);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:886", PLUG_MAX_CHK_2X, hqvdp->regs + HQVDP_WR_PLUG_MAX_CHK);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:887", PLUG_MAX_MSG_1X, hqvdp->regs + HQVDP_WR_PLUG_MAX_MSG);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:888", PLUG_MIN_SPACE_1, hqvdp->regs + HQVDP_WR_PLUG_MIN_SPACE);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:889", PLUG_CONTROL_ENABLE, hqvdp->regs + HQVDP_WR_PLUG_CONTROL);
}
...
static void sti_hqvdp_start_xp70(struct sti_hqvdp *hqvdp)
{
	...
	/* Reset */
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:959", SW_RESET_CTRL_FULL, hqvdp->regs + HQVDP_MBX_SW_RESET_CTRL);

	for (i = 0; i < POLL_MAX_ATTEMPT; i++) {
		if (pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:962", hqvdp->regs + HQVDP_MBX_STARTUP_CTRL1)
				& STARTUP_CTRL1_RST_DONE)
			break;
		msleep(POLL_DELAY_MS);
	}
	...
	for (i = 0; i < header->rd_size / 4; i++)
		pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:975", fw_rd_plug[i], hqvdp->regs + HQVDP_RD_PLUG + i * 4);
	for (i = 0; i < header->wr_size / 4; i++)
		pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:977", fw_wr_plug[i], hqvdp->regs + HQVDP_WR_PLUG + i * 4);

	sti_hqvdp_init_plugs(hqvdp);

	/* Authorize Idle Mode */
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:982", STARTUP_CTRL1_AUTH_IDLE, hqvdp->regs + HQVDP_MBX_STARTUP_CTRL1);

	/* Prevent VTG interruption during the boot */
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:985", SOFT_VSYNC_SW_CTRL_IRQ, hqvdp->regs + HQVDP_MBX_SOFT_VSYNC);
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:986", 0, hqvdp->regs + HQVDP_MBX_NEXT_CMD);

	/* Download PMEM & DMEM */
	for (i = 0; i < header->pmem_size / 4; i++)
		pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:990", fw_pmem[i], hqvdp->regs + HQVDP_PMEM + i * 4);
	for (i = 0; i < header->dmem_size / 4; i++)
		pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:992", fw_dmem[i], hqvdp->regs + HQVDP_DMEM + i * 4);

	/* Enable fetch */
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:995", STARTUP_CTRL2_FETCH_EN, hqvdp->regs + HQVDP_MBX_STARTUP_CTRL2);

	/* Wait end of boot */
	for (i = 0; i < POLL_MAX_ATTEMPT; i++) {
		if (pete_readl("drivers/gpu/drm/sti/sti_hqvdp.c:999", hqvdp->regs + HQVDP_MBX_INFO_XP70)
				& INFO_XP70_FW_READY)
			break;
		msleep(POLL_DELAY_MS);
	}
	...
	/* Launch Vsync */
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:1011", SOFT_VSYNC_HW, hqvdp->regs + HQVDP_MBX_SOFT_VSYNC);
				    ...
				    struct drm_atomic_state *state)
{
	...
	pete_writel("drivers/gpu/drm/sti/sti_hqvdp.c:1235", hqvdp->hqvdp_cmd_paddr + cmd_offset,
	       hqvdp->regs + HQVDP_MBX_NEXT_CMD);
