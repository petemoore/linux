				     ...
				     union hdmi_infoframe *frame)
{
	...
	for (i = 0; i < len; i += 7) {
		pete_writel("drivers/gpu/drm/vc4/vc4_hdmi.c:907", buffer[i + 0] << 0 |
		       buffer[i + 1] << 8 |
		       buffer[i + 2] << 16,
		       base + packet_reg);
		packet_reg += 4;

		pete_writel("drivers/gpu/drm/vc4/vc4_hdmi.c:913", buffer[i + 3] << 0 |
		       buffer[i + 4] << 8 |
		       buffer[i + 5] << 16 |
		       buffer[i + 6] << 24,
		       base + packet_reg);
	...
	for (; packet_reg < packet_reg_next; packet_reg += 4)
		pete_writel("drivers/gpu/drm/vc4/vc4_hdmi.c:926", 0, base + packet_reg);
