...
static irqreturn_t vc4_crtc_irq_handler(int irq, void *data)
{
	...
	int i;
	u32 stat = pete_readl("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1228", crtc_list[0]->regs + SMICS);
	irqreturn_t ret = IRQ_NONE;
	u32 chan;

	if (stat & SMICS_INTERRUPTS) {
		pete_writel("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1233", 0, crtc_list[0]->regs + SMICS);

		chan = pete_readl("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1235", crtc_list[0]->regs + SMIDSW0);
		...
		if ((chan & 0xFFFF0000) != SMI_NEW) {
			...
			if (chan & 1) {
				pete_writel("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1248", SMI_NEW, crtc_list[0]->regs + SMIDSW0);
			...
			if (crtc_list[1]) {
				...
				chan = pete_readl("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1256", crtc_list[0]->regs + SMIDSW1);

				if (chan & 1) {
					pete_writel("drivers/gpu/drm/vc4/vc4_firmware_kms.c:1259", SMI_NEW, crtc_list[0]->regs + SMIDSW1);
...
static int vc4_fkms_bind(struct device *dev, struct device *master, void *data)
{
	...
	if (num_displays > 0) {
		...
		if (fkms->revision >= BCM2712) {
			...
			pete_writel("drivers/gpu/drm/vc4/vc4_firmware_kms.c:2020", 0, crtc_list[0]->regs + SMICS);
			ret = devm_request_irq(dev, platform_get_irq(pdev, 0),
