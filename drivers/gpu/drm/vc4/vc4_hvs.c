...
void vc4_hvs_dump_state(struct vc4_hvs *hvs)
{
	...
	for (i = 0; i < 64; i += 4) {
		DRM_INFO("0x%08x (%s): 0x%08x 0x%08x 0x%08x 0x%08x\n",
			 ...
			 pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:218", (u32 __iomem *)hvs->dlist + i + 0),
			 pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:219", (u32 __iomem *)hvs->dlist + i + 1),
			 pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:220", (u32 __iomem *)hvs->dlist + i + 2),
			 pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:221", (u32 __iomem *)hvs->dlist + i + 3));
	}
...
static int vc4_hvs_debugfs_dlist(struct seq_file *m, void *data)
{
	...
	for (i = 0; i < SCALER_CHANNELS_COUNT; i++) {
		...
		for (j = HVS_READ(SCALER_DISPLISTX(i)); j < dlist_mem_size; j++) {
			dlist_word = pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:264", (u32 __iomem *)vc4->hvs->dlist + j);
...
static int vc6_hvs_debugfs_dlist(struct seq_file *m, void *data)
{
	...
	for (i = 0; i < SCALER_CHANNELS_COUNT; i++) {
		...
		for (j = active_dlist; j < dlist_mem_size; j++) {
			...
			dlist_word = pete_readl("drivers/gpu/drm/vc4/vc4_hvs.c:313", (u32 __iomem *)vc4->hvs->dlist + j);
			drm_printf(&p, "dlist: %02d: 0x%08x\n", j,
				   dlist_word);
			if (!next_entry_start ||
			    next_entry_start == j) {
					...
					const u32 *kernel)
{
	...
	for (i = 0; i < VC4_KERNEL_DWORDS; i++) {
		if (i < VC4_LINEAR_PHASE_KERNEL_DWORDS)
			pete_writel("drivers/gpu/drm/vc4/vc4_hvs.c:541", kernel[i], &dst_kernel[i]);
		else {
			pete_writel("drivers/gpu/drm/vc4/vc4_hvs.c:543", kernel[VC4_KERNEL_DWORDS - i - 1],
			       &dst_kernel[i]);
		}
	}
			  ...
			  struct drm_atomic_state *state)
{
	...
	pete_writel("drivers/gpu/drm/vc4/vc4_hvs.c:1502", SCALER_CTL0_END, dlist_next);
	dlist_next++;
