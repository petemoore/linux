...
static inline void fimd_set_bits(struct fimd_context *ctx, u32 reg, u32 mask,
				 u32 val)
{
	val = (val & mask) | (pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:257", ctx->regs + reg) & ~mask);
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:258", val, ctx->regs + reg);
}
...
static int fimd_enable_vblank(struct exynos_drm_crtc *crtc)
{
	...
	if (!test_and_set_bit(0, &ctx->irq_flags)) {
		val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:270", ctx->regs + VIDINTCON0);
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:287", val, ctx->regs + VIDINTCON0);
	}
...
static void fimd_disable_vblank(struct exynos_drm_crtc *crtc)
{
	...
	if (test_and_clear_bit(0, &ctx->irq_flags)) {
		val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:302", ctx->regs + VIDINTCON0);
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:313", val, ctx->regs + VIDINTCON0);
	}
}
...
static void fimd_enable_video_output(struct fimd_context *ctx, unsigned int win,
					bool enable)
{
	u32 val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:339", ctx->regs + WINCON(win));
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:346", val, ctx->regs + WINCON(win));
}
...
static void fimd_enable_shadow_channel_path(struct fimd_context *ctx,
						...
						bool enable)
{
	u32 val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:353", ctx->regs + SHADOWCON);
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:360", val, ctx->regs + SHADOWCON);
}
...
static int fimd_clear_channels(struct exynos_drm_crtc *crtc)
{
	...
	for (win = 0; win < WINDOWS_NR; win++) {
		u32 val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:381", ctx->regs + WINCON(win));
...
static void fimd_setup_trigger(struct fimd_context *ctx)
{
	...
	u32 val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:462", timing_base + TRIGCON);
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:475", val, timing_base + TRIGCON);
}
...
static void fimd_commit(struct exynos_drm_crtc *crtc)
{
	...
	if (ctx->i80_if) {
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:495", val, timing_base + I80IFCONFAx(0));

		/* disable auto frame rate */
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:498", 0, timing_base + I80IFCONFBx(0));
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:520", vidcon1, ctx->regs + driver_data->timing_base + VIDCON1);
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:530", val, ctx->regs + driver_data->timing_base + VIDTCON0);
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:540", val, ctx->regs + driver_data->timing_base + VIDTCON1);
	}

	if (driver_data->has_vidoutcon)
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:544", ctx->vidout_con, timing_base + VIDOUT_CON);
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:574", val, ctx->regs + driver_data->timing_base + VIDTCON2);
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:591", val, ctx->regs + VIDCON0);
}
				...
				unsigned int alpha, unsigned int pixel_alpha)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:647", val, ctx->regs + VIDOSD_C(win));

	val = VIDW_ALPHA_R(win_alpha_l) | VIDW_ALPHA_G(win_alpha_l) |
		VIDW_ALPHA_B(win_alpha_l);
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:651", val, ctx->regs + VIDWnALPHA0(win));

	val = VIDW_ALPHA_R(0x0) | VIDW_ALPHA_G(0x0) |
		VIDW_ALPHA_B(0x0);
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:655", val, ctx->regs + VIDWnALPHA1(win));
				...
				struct drm_framebuffer *fb, int width)
{
	...
	case DRM_FORMAT_ABGR8888:
	case DRM_FORMAT_BGR565:
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:724", WIN_RGB_ORDER_REVERSE, ctx->regs + WIN_RGB_ORDER(win));
		break;
	default:
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:727", WIN_RGB_ORDER_FORWARD, ctx->regs + WIN_RGB_ORDER(win));
		break;
	}
...
static void fimd_win_set_colkey(struct fimd_context *ctx, unsigned int win)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:761", keycon0, ctx->regs + WKEYCON0_BASE(win));
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:762", keycon1, ctx->regs + WKEYCON1_BASE(win));
}
				    ...
				    unsigned int win, bool protect)
{
	...
	val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:795", ctx->regs + reg);
	if (protect)
		val |= bits;
	else
		val &= ~bits;
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:800", val, ctx->regs + reg);
}
			      ...
			      struct exynos_drm_plane *plane)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:852", val, ctx->regs + VIDWx_BUF_START(win, 0));

	/* buffer end address */
	size = pitch * state->crtc.h;
	val = (unsigned long)(dma_addr + size);
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:857", val, ctx->regs + VIDWx_BUF_END(win, 0));
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:872", val, ctx->regs + VIDWx_BUF_SIZE(win, 0));
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:879", val, ctx->regs + VIDOSD_A(win));
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:891", val, ctx->regs + VIDOSD_B(win));
	...
	if (win != 3 && win != 4) {
		...
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:903", val, ctx->regs + offset);
...
static void fimd_atomic_disable(struct exynos_drm_crtc *crtc)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:980", 0, ctx->regs + VIDCON0);
...
static void fimd_trigger(struct device *dev)
{
	...
	reg = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:1003", timing_base + TRIGCON);
	reg |= (TRGMODE_ENABLE | SWTRGCMD_ENABLE);
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:1005", reg, timing_base + TRIGCON);
...
static void fimd_dp_clock_enable(struct exynos_drm_clk *clk, bool enable)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:1050", val, ctx->regs + DP_MIE_CLKCON);
}
...
static irqreturn_t fimd_irq_handler(int irq, void *dev_id)
{
	...
	val = pete_readl("drivers/gpu/drm/exynos/exynos_drm_fimd.c:1071", ctx->regs + VIDINTCON1);

	clear_bit = ctx->i80_if ? VIDINTCON1_INT_I80 : VIDINTCON1_INT_FRAME;
	if (val & clear_bit)
		pete_writel("drivers/gpu/drm/exynos/exynos_drm_fimd.c:1075", clear_bit, ctx->regs + VIDINTCON1);
