...
static inline void decon_set_bits(struct decon_context *ctx, u32 reg, u32 mask,
				  u32 val)
{
	val = (val & mask) | (pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:99", ctx->addr + reg) & ~mask);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:100", val, ctx->addr + reg);
}
...
static int decon_enable_vblank(struct exynos_drm_crtc *crtc)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:114", val, ctx->addr + DECON_VIDINTCON0);
...
static void decon_disable_vblank(struct exynos_drm_crtc *crtc)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:131", 0, ctx->addr + DECON_VIDINTCON0);
}
...
static u32 decon_get_frame_count(struct decon_context *ctx, bool end)
{
	 ...
	 */
	frm = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:143", ctx->addr + DECON_CRFMID);
	do {
		status = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:145", ctx->addr + DECON_VIDCON1);
		pfrm = frm;
		frm = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:147", ctx->addr + DECON_CRFMID);
	} while (frm != pfrm && --cnt);
...
static void decon_setup_trigger(struct decon_context *ctx)
{
	...
	if (!(ctx->out_type & I80_HW_TRG)) {
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:182", TRIGCON_TRIGEN_PER_F | TRIGCON_TRIGEN_F |
		       TRIGCON_TE_AUTO_MASK | TRIGCON_SWTRIGEN,
		       ctx->addr + DECON_TRIGCON);
		return;
	}

	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:188", TRIGCON_TRIGEN_PER_F | TRIGCON_TRIGEN_F | TRIGCON_HWTRIGMASK
...
static void decon_commit(struct exynos_drm_crtc *crtc)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:224", val, ctx->addr + DECON_VIDOUTCON0);
	...
	if (interlaced)
		...
		val = VIDTCON2_LINEVAL(m->vdisplay - 1) |
			VIDTCON2_HOZVAL(m->hdisplay - 1);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:232", val, ctx->addr + DECON_VIDTCON2);
	...
	if (!crtc->i80_mode) {
		...
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:241", val, ctx->addr + DECON_VIDTCON00);

		val = VIDTCON01_VSPW_F(
				m->crtc_vsync_end - m->crtc_vsync_start - 1);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:245", val, ctx->addr + DECON_VIDTCON01);

		val = VIDTCON10_HBPD_F(
				m->crtc_htotal - m->crtc_hsync_end - 1) |
			VIDTCON10_HFPD_F(
				m->crtc_hsync_start - m->crtc_hdisplay - 1);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:251", val, ctx->addr + DECON_VIDTCON10);

		val = VIDTCON11_HSPW_F(
				m->crtc_hsync_end - m->crtc_hsync_start - 1);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:255", val, ctx->addr + DECON_VIDTCON11);
	}
				 ...
				 struct drm_framebuffer *fb)
{
	...
	val = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:334", ctx->addr + DECON_WINCONx(win));
	val &= WINCONx_ENWIN_F;
			       ...
			       struct exynos_drm_plane *plane)
{
	...
	if (crtc->base.mode.flags & DRM_MODE_FLAG_INTERLACE) {
		...
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:416", val, ctx->addr + DECON_VIDOSDxA(win));

		val = COORDINATE_X(state->crtc.x + state->crtc.w - 1) |
			COORDINATE_Y((state->crtc.y + state->crtc.h) / 2 - 1);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:420", val, ctx->addr + DECON_VIDOSDxB(win));
	} else {
		val = COORDINATE_X(state->crtc.x) | COORDINATE_Y(state->crtc.y);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:423", val, ctx->addr + DECON_VIDOSDxA(win));

		val = COORDINATE_X(state->crtc.x + state->crtc.w - 1) |
				COORDINATE_Y(state->crtc.y + state->crtc.h - 1);
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:427", val, ctx->addr + DECON_VIDOSDxB(win));
	}

	val = VIDOSD_Wx_ALPHA_R_F(0xff) | VIDOSD_Wx_ALPHA_G_F(0xff) |
		VIDOSD_Wx_ALPHA_B_F(0xff);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:432", val, ctx->addr + DECON_VIDOSDxC(win));

	val = VIDOSD_Wx_ALPHA_R_F(0x0) | VIDOSD_Wx_ALPHA_G_F(0x0) |
		VIDOSD_Wx_ALPHA_B_F(0x0);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:436", val, ctx->addr + DECON_VIDOSDxD(win));

	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:438", dma_addr, ctx->addr + DECON_VIDW0xADD0B0(win));

	val = dma_addr + pitch * state->src.h;
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:441", val, ctx->addr + DECON_VIDW0xADD1B0(win));
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:449", val, ctx->addr + DECON_VIDW0xADD2(win));
...
static void decon_swreset(struct decon_context *ctx)
{
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:490", 0, ctx->addr + DECON_VIDCON0);
	readl_poll_timeout(ctx->addr + DECON_VIDCON0, val,
			   ~val & VIDCON0_STOP_STATUS, 12, 20000);

	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:494", VIDCON0_SWRESET, ctx->addr + DECON_VIDCON0);
	...
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:507", VIDCON0_CLKVALUP | VIDCON0_VLCKFREE, ctx->addr + DECON_VIDCON0);
	decon_set_bits(ctx, DECON_CMU,
		       CMU_CLKGAGE_MODE_SFR_F | CMU_CLKGAGE_MODE_MEM_F, ~0);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:510", VIDCON1_VCLK_RUN_VDEN_DISABLE, ctx->addr + DECON_VIDCON1);
	pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:511", CRCCTRL_CRCEN | CRCCTRL_CRCSTART_F | CRCCTRL_CRCCLKEN,
...
static irqreturn_t decon_irq_handler(int irq, void *dev_id)
{
	...
	val = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:695", ctx->addr + DECON_VIDINTCON1);
	val &= VIDINTCON1_INTFRMDONEPEND | VIDINTCON1_INTFRMPEND;

	if (val) {
		pete_writel("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:699", val, ctx->addr + DECON_VIDINTCON1);
		if (ctx->out_type & IFTYPE_HDMI) {
			val = pete_readl("drivers/gpu/drm/exynos/exynos5433_drm_decon.c:701", ctx->addr + DECON_VIDOUTCON0);
