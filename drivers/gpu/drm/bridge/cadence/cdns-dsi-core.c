...
static void cdns_dsi_bridge_disable(struct drm_bridge *bridge)
{
	...
	val = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:664", dsi->regs + MCTL_MAIN_DATA_CTL);
	val &= ~(IF_VID_SELECT_MASK | IF_VID_MODE | VID_EN | HOST_EOT_GEN |
		 DISP_EOT_GEN);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:667", val, dsi->regs + MCTL_MAIN_DATA_CTL);

	val = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:669", dsi->regs + MCTL_MAIN_EN) & ~IF_EN(input->id);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:670", val, dsi->regs + MCTL_MAIN_EN);
...
static void cdns_dsi_hs_init(struct cdns_dsi *dsi)
{
	 ...
	 */
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:697", DPHY_CMN_PSO | DPHY_PLL_PSO | DPHY_ALL_D_PDN | DPHY_C_PDN |
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:707", PLL_LOCKED, dsi->regs + MCTL_MAIN_STS_CLR);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:708", DPHY_CMN_PSO | DPHY_ALL_D_PDN | DPHY_C_PDN | DPHY_CMN_PDN,
	       dsi->regs + MCTL_DPHY_CFG0);
	WARN_ON_ONCE(readl_poll_timeout(dsi->regs + MCTL_MAIN_STS, status,
					status & PLL_LOCKED, 100, 100));
	/* De-assert data and clock reset lines. */
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:713", DPHY_CMN_PSO | DPHY_ALL_D_PDN | DPHY_C_PDN | DPHY_CMN_PDN |
...
static void cdns_dsi_init_link(struct cdns_dsi *dsi)
{
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:736", val, dsi->regs + MCTL_MAIN_PHY_CTL);
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:741", CLK_LANE_ULPOUT_TIME(ulpout) | DATA_LANE_ULPOUT_TIME(ulpout),
	       dsi->regs + MCTL_ULPOUT_TIME);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:744", LINK_EN, dsi->regs + MCTL_MAIN_DATA_CTL);

	val = CLK_LANE_EN | PLL_START;
	for (i = 0; i < output->dev->lanes; i++)
		val |= DATA_LANE_START(i);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:750", val, dsi->regs + MCTL_MAIN_EN);
...
static void cdns_dsi_bridge_enable(struct drm_bridge *bridge)
{
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:781", HBP_LEN(dsi_cfg.hbp) | HSA_LEN(dsi_cfg.hsa),
	       dsi->regs + VID_HSIZE1);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:783", HFP_LEN(dsi_cfg.hfp) | HACT_LEN(dsi_cfg.hact),
	       dsi->regs + VID_HSIZE2);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:786", VBP_LEN(mode->crtc_vtotal - mode->crtc_vsync_end - 1) |
	       VFP_LEN(mode->crtc_vsync_start - mode->crtc_vdisplay) |
	       VSA_LEN(mode->crtc_vsync_end - mode->crtc_vsync_start + 1),
	       dsi->regs + VID_VSIZE1);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:790", mode->crtc_vdisplay, dsi->regs + VID_VSIZE2);

	tmp = dsi_cfg.htotal -
	      (dsi_cfg.hsa + DSI_BLANKING_FRAME_OVERHEAD +
	       DSI_HSA_FRAME_OVERHEAD);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:795", BLK_LINE_PULSE_PKT_LEN(tmp), dsi->regs + VID_BLKSIZE2);
	if (output->dev->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE)
		pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:797", MAX_LINE_LIMIT(tmp - DSI_NULL_FRAME_OVERHEAD),
		       dsi->regs + VID_VCA_SETTING2);

	tmp = dsi_cfg.htotal -
	      (DSI_HSS_VSS_VSE_FRAME_OVERHEAD + DSI_BLANKING_FRAME_OVERHEAD);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:802", BLK_LINE_EVENT_PKT_LEN(tmp), dsi->regs + VID_BLKSIZE1);
	if (!(output->dev->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE))
		pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:804", MAX_LINE_LIMIT(tmp - DSI_NULL_FRAME_OVERHEAD),
		       dsi->regs + VID_VCA_SETTING2);
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:816", REG_WAKEUP_TIME(reg_wakeup) | REG_LINE_DURATION(tmp),
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:837", CLK_DIV(div) | HSTX_TIMEOUT(tmp),
	       dsi->regs + MCTL_DPHY_TIMEOUT1);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:840", LPRX_TIMEOUT(tmp), dsi->regs + MCTL_DPHY_TIMEOUT2);
	...
	if (output->dev->mode_flags & MIPI_DSI_MODE_VIDEO) {
		...
		pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:877", tmp, dsi->regs + VID_MAIN_CTL);
	}

	tmp = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:880", dsi->regs + MCTL_MAIN_DATA_CTL);
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:889", tmp, dsi->regs + MCTL_MAIN_DATA_CTL);

	tmp = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:891", dsi->regs + MCTL_MAIN_EN) | IF_EN(input->id);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:892", tmp, dsi->regs + MCTL_MAIN_EN);
}
...
static irqreturn_t cdns_dsi_interrupt(int irq, void *data)
{
	...
	flag = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1003", dsi->regs + DIRECT_CMD_STS_FLAG);
	if (flag) {
		ctl = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1005", dsi->regs + DIRECT_CMD_STS_CTL);
		ctl &= ~flag;
		pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1007", ctl, dsi->regs + DIRECT_CMD_STS_CTL);
				 ...
				 const struct mipi_dsi_msg *msg)
{
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1073", pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1073", dsi->regs + MCTL_MAIN_DATA_CTL) | ctl,
	       dsi->regs + MCTL_MAIN_DATA_CTL);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1076", cmd, dsi->regs + DIRECT_CMD_MAIN_SETTINGS);
	...
	for (i = 0; i < tx_len; i += 4) {
		...
		pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1086", val, dsi->regs + DIRECT_CMD_WRDATA);
	}

	/* Clear status flags before sending the command. */
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1090", wait, dsi->regs + DIRECT_CMD_STS_CLR);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1091", wait, dsi->regs + DIRECT_CMD_STS_CTL);
	reinit_completion(&dsi->direct_cmd_comp);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1093", 0, dsi->regs + DIRECT_CMD_SEND);

	wait_for_completion_timeout(&dsi->direct_cmd_comp,
				    msecs_to_jiffies(1000));

	sts = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1098", dsi->regs + DIRECT_CMD_STS);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1099", wait, dsi->regs + DIRECT_CMD_STS_CLR);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1100", 0, dsi->regs + DIRECT_CMD_STS_CTL);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1102", pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1102", dsi->regs + MCTL_MAIN_DATA_CTL) & ~ctl,
	       dsi->regs + MCTL_MAIN_DATA_CTL);
	...
	for (i = 0; i < rx_len; i += 4) {
		...
		val = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1121", dsi->regs + DIRECT_CMD_RDDATA);
...
static int cdns_dsi_drm_probe(struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1206", dsi->regs + ID_REG);
	...
	val = pete_readl("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1215", dsi->regs + IP_CONF);
	dsi->direct_cmd_fifo_depth = 1 << (DIRCMD_FIFO_DEPTH(val) + 2);
	dsi->rx_fifo_depth = RX_FIFO_DEPTH(val);
	init_completion(&dsi->direct_cmd_comp);

	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1220", 0, dsi->regs + MCTL_MAIN_DATA_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1221", 0, dsi->regs + MCTL_MAIN_EN);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1222", 0, dsi->regs + MCTL_MAIN_PHY_CTL);
	...
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1233", 0, dsi->regs + MCTL_MAIN_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1234", 0, dsi->regs + MCTL_DPHY_ERR_CTL1);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1235", 0, dsi->regs + CMD_MODE_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1236", 0, dsi->regs + DIRECT_CMD_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1237", 0, dsi->regs + DIRECT_CMD_RD_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1238", 0, dsi->regs + VID_MODE_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1239", 0, dsi->regs + TVG_STS_CTL);
	pete_writel("drivers/gpu/drm/bridge/cadence/cdns-dsi-core.c:1240", 0, dsi->regs + DPI_IRQ_EN);
