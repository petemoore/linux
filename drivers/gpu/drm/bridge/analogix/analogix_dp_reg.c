			     ...
			     struct drm_dp_aux_msg *msg)
{
	...
	reg = BUF_CLR;
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1038", reg, dp->reg_base + ANALOGIX_DP_BUFFER_DATA_CTL);
	...
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1066", reg, dp->reg_base + ANALOGIX_DP_AUX_CH_CTL_1);

	/* Select DPCD device address */
	reg = AUX_ADDR_7_0(msg->address);
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1070", reg, dp->reg_base + ANALOGIX_DP_AUX_ADDR_7_0);
	reg = AUX_ADDR_15_8(msg->address);
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1072", reg, dp->reg_base + ANALOGIX_DP_AUX_ADDR_15_8);
	reg = AUX_ADDR_19_16(msg->address);
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1074", reg, dp->reg_base + ANALOGIX_DP_AUX_ADDR_19_16);

	if (!(msg->request & DP_AUX_I2C_READ)) {
		for (i = 0; i < msg->size; i++) {
			reg = buffer[i];
			pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1079", reg, dp->reg_base + ANALOGIX_DP_BUF_DATA_0 +
			       4 * i);
		}
	}
	...
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1091", reg, dp->reg_base + ANALOGIX_DP_AUX_CH_CTL_2);
	...
	pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1110", RPLY_RECEIV, dp->reg_base + ANALOGIX_DP_INT_STA);

	/* Clear interrupt source for AUX CH access error */
	reg = pete_readl("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1113", dp->reg_base + ANALOGIX_DP_INT_STA);
	status_reg = pete_readl("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1114", dp->reg_base + ANALOGIX_DP_AUX_CH_STA);
	if ((reg & AUX_ERR) || (status_reg & AUX_STATUS_MASK)) {
		pete_writel("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1116", AUX_ERR, dp->reg_base + ANALOGIX_DP_INT_STA);
	...
	if (msg->request & DP_AUX_I2C_READ) {
		for (i = 0; i < msg->size; i++) {
			reg = pete_readl("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1125", dp->reg_base + ANALOGIX_DP_BUF_DATA_0 +
				    4 * i);
	...
	reg = pete_readl("drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c:1132", dp->reg_base + ANALOGIX_DP_AUX_RX_COMM);
