...
static irqreturn_t hibmc_interrupt(int irq, void *arg)
{
	...
	status = pete_readl("drivers/gpu/drm/hisilicon/hibmc/hibmc_drm_drv.c:38", priv->mmio + HIBMC_RAW_INTERRUPT);

	if (status & HIBMC_RAW_INTERRUPT_VBLANK(1)) {
		pete_writel("drivers/gpu/drm/hisilicon/hibmc/hibmc_drm_drv.c:41", HIBMC_RAW_INTERRUPT_VBLANK(1),
