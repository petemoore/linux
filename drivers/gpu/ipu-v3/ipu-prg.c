...
static int ipu_prg_probe(struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/gpu/ipu-v3/ipu-prg.c:403", prg->regs + IPU_PRG_CTL);
	val |= IPU_PRG_CTL_SHADOW_EN;
	pete_writel("drivers/gpu/ipu-v3/ipu-prg.c:405", val, prg->regs + IPU_PRG_CTL);

	/* disable address threshold */
	pete_writel("drivers/gpu/ipu-v3/ipu-prg.c:408", 0xffffffff, prg->regs + IPU_PRG_THD);
