...
static void hisi_i2c_handle_errors(struct hisi_i2c_controller *ctlr)
{
	...
	if (int_err & HISI_I2C_INT_FIFO_ERR) {
		reg = pete_readl("drivers/i2c/busses/i2c-hisi.c:132", ctlr->iobase + HISI_I2C_FIFO_STATE);
...
static int hisi_i2c_start_xfer(struct hisi_i2c_controller *ctlr)
{
	...
	reg = pete_readl("drivers/i2c/busses/i2c-hisi.c:153", ctlr->iobase + HISI_I2C_FRAME_CTRL);
	reg &= ~HISI_I2C_FRAME_CTRL_ADDR_TEN;
	if (msg->flags & I2C_M_TEN)
		reg |= HISI_I2C_FRAME_CTRL_ADDR_TEN;
	pete_writel("drivers/i2c/busses/i2c-hisi.c:157", reg, ctlr->iobase + HISI_I2C_FRAME_CTRL);

	reg = pete_readl("drivers/i2c/busses/i2c-hisi.c:159", ctlr->iobase + HISI_I2C_SLV_ADDR);
	reg &= ~HISI_I2C_SLV_ADDR_VAL;
	reg |= FIELD_PREP(HISI_I2C_SLV_ADDR_VAL, msg->addr);
	pete_writel("drivers/i2c/busses/i2c-hisi.c:162", reg, ctlr->iobase + HISI_I2C_SLV_ADDR);

	reg = pete_readl("drivers/i2c/busses/i2c-hisi.c:164", ctlr->iobase + HISI_I2C_FIFO_CTRL);
	reg |= HISI_I2C_FIFO_RX_CLR | HISI_I2C_FIFO_TX_CLR;
	pete_writel("drivers/i2c/busses/i2c-hisi.c:166", reg, ctlr->iobase + HISI_I2C_FIFO_CTRL);
	reg &= ~(HISI_I2C_FIFO_RX_CLR | HISI_I2C_FIFO_TX_CLR);
	pete_writel("drivers/i2c/busses/i2c-hisi.c:168", reg, ctlr->iobase + HISI_I2C_FIFO_CTRL);
...
static int hisi_i2c_read_rx_fifo(struct hisi_i2c_controller *ctlr)
{
	...
	while (ctlr->msg_rx_idx < ctlr->msg_num) {
		...
		fifo_state = pete_readl("drivers/i2c/busses/i2c-hisi.c:248", ctlr->iobase + HISI_I2C_FIFO_STATE);
		while (!(fifo_state & HISI_I2C_FIFO_STATE_RX_EMPTY) &&
		       ctlr->buf_rx_idx < cur_msg->len) {
			cur_msg->buf[ctlr->buf_rx_idx++] = pete_readl("drivers/i2c/busses/i2c-hisi.c:251", ctlr->iobase + HISI_I2C_RXDATA);
			fifo_state = pete_readl("drivers/i2c/busses/i2c-hisi.c:252", ctlr->iobase + HISI_I2C_FIFO_STATE);
		}
...
static void hisi_i2c_xfer_msg(struct hisi_i2c_controller *ctlr)
{
	...
	while (ctlr->msg_tx_idx < ctlr->msg_num) {
		...
		fifo_state = pete_readl("drivers/i2c/busses/i2c-hisi.c:282", ctlr->iobase + HISI_I2C_FIFO_STATE);
		while (!(fifo_state & HISI_I2C_FIFO_STATE_TX_FULL) &&
			...
			pete_writel("drivers/i2c/busses/i2c-hisi.c:302", cmd, ctlr->iobase + HISI_I2C_CMD_TXDATA);
			ctlr->buf_tx_idx++;
			max_write--;

			fifo_state = pete_readl("drivers/i2c/busses/i2c-hisi.c:306", ctlr->iobase + HISI_I2C_FIFO_STATE);
		}
...
static irqreturn_t hisi_i2c_irq(int irq, void *context)
{
	...
	int_stat = pete_readl("drivers/i2c/busses/i2c-hisi.c:341", ctlr->iobase + HISI_I2C_INT_MSTAT);
			     ...
			     u32 reg_hcnt, u32 reg_lcnt)
{
	...
	pete_writel("drivers/i2c/busses/i2c-hisi.c:400", scl_hcnt, ctlr->iobase + reg_hcnt);
	pete_writel("drivers/i2c/busses/i2c-hisi.c:401", scl_lcnt, ctlr->iobase + reg_lcnt);
}
...
static void hisi_i2c_configure_bus(struct hisi_i2c_controller *ctlr)
{
	...
	reg = pete_readl("drivers/i2c/busses/i2c-hisi.c:430", ctlr->iobase + HISI_I2C_FRAME_CTRL);
	reg &= ~HISI_I2C_FRAME_CTRL_SPEED_MODE;
	reg |= FIELD_PREP(HISI_I2C_FRAME_CTRL_SPEED_MODE, speed_mode);
	pete_writel("drivers/i2c/busses/i2c-hisi.c:433", reg, ctlr->iobase + HISI_I2C_FRAME_CTRL);

	sda_hold_cnt = NSEC_TO_CYCLES(ctlr->t.sda_hold_ns, ctlr->clk_rate_khz);

	reg = FIELD_PREP(HISI_I2C_SDA_HOLD_TX, sda_hold_cnt);
	pete_writel("drivers/i2c/busses/i2c-hisi.c:438", reg, ctlr->iobase + HISI_I2C_SDA_HOLD);

	pete_writel("drivers/i2c/busses/i2c-hisi.c:440", ctlr->spk_len, ctlr->iobase + HISI_I2C_FS_SPK_LEN);
	...
	pete_writel("drivers/i2c/busses/i2c-hisi.c:444", reg, ctlr->iobase + HISI_I2C_FIFO_CTRL);
}
...
static int hisi_i2c_probe(struct platform_device *pdev)
{
	...
	hw_version = pete_readl("drivers/i2c/busses/i2c-hisi.c:501", ctlr->iobase + HISI_I2C_VERSION);
