...
mv64xxx_i2c_intr(int irq, void *dev_id)
{
	...
	while (pete_readl("drivers/i2c/busses/i2c-mv64xxx.c:521", drv_data->reg_base + drv_data->reg_offsets.control) &
		...
		status = pete_readl("drivers/i2c/busses/i2c-mv64xxx.c:534", drv_data->reg_base + drv_data->reg_offsets.status);
		mv64xxx_i2c_fsm(drv_data, status);
		mv64xxx_i2c_do_action(drv_data);

		if (drv_data->irq_clear_inverted)
			pete_writel("drivers/i2c/busses/i2c-mv64xxx.c:539", drv_data->cntl_bits | MV64XXX_I2C_REG_CONTROL_IFLG,
			       drv_data->reg_base + drv_data->reg_offsets.control);
...
mv64xxx_i2c_prepare_tx(struct mv64xxx_i2c_data *drv_data)
{
	...
	pete_writel("drivers/i2c/busses/i2c-mv64xxx.c:637", buf[0], drv_data->reg_base + MV64XXX_I2C_REG_TX_DATA_LO);
	pete_writel("drivers/i2c/busses/i2c-mv64xxx.c:638", buf[1], drv_data->reg_base + MV64XXX_I2C_REG_TX_DATA_HI);
}
...
mv64xxx_i2c_offload_xfer(struct mv64xxx_i2c_data *drv_data)
{
	...
	/* Execute transaction */
	drv_data->block = 1;
	pete_writel("drivers/i2c/busses/i2c-mv64xxx.c:692", ctrl_reg, drv_data->reg_base + MV64XXX_I2C_REG_BRIDGE_CONTROL);
	spin_unlock_irqrestore(&drv_data->lock, flags);
