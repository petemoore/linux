			    ...
			    bool eq_zero, u32  timeout)
{
	...
	do {
		bits = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:512", io + addr) & mask;
...
static void mlxbf_i2c_smbus_master_unlock(struct mlxbf_i2c_priv *priv)
{
	...
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:554", 0, priv->mst->io + MLXBF_I2C_SMBUS_MASTER_GW);
}
...
static int mlxbf_i2c_smbus_check_status(struct mlxbf_i2c_priv *priv)
{
	...
	cause_status_bits = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:599", priv->mst_cause->io +
	...
	master_status_bits = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:607", priv->mst->io +
				  ...
				  u8 len, u8 block_en, u8 pec_en, bool read)
{
	...
	/* Clear status bits. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:714", 0x0, priv->mst->io + MLXBF_I2C_SMBUS_MASTER_STATUS);
	/* Set the cause data. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:716", ~0x0, priv->mst_cause->io + MLXBF_I2C_CAUSE_OR_CLEAR);
	/* Zero PEC byte. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:718", 0x0, priv->mst->io + MLXBF_I2C_SMBUS_MASTER_PEC);
	/* Zero byte count. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:720", 0x0, priv->mst->io + priv->chip->smbus_master_rs_bytes_off);

	/* GW activation. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:723", command, priv->mst->io + MLXBF_I2C_SMBUS_MASTER_GW);
				  ...
				  struct mlxbf_i2c_smbus_request *request)
{
	...
	if (read_en) {
		 ...
		 */
		pete_writel("drivers/i2c/busses/i2c-mlxbf.c:853", MLXBF_I2C_SMBUS_MASTER_FSM_PS_STATE_MASK,
				  ...
				  const struct mlxbf_i2c_timings *timings)
{
	...
	timer |= mlxbf_i2c_set_timer(priv, timings->scl_low,
				     false, MLXBF_I2C_MASK_16,
				     MLXBF_I2C_SHIFT_16);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1140", timer, priv->timer->io +
	...
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1151", timer, priv->timer->io +
	...
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1158", timer, priv->timer->io + MLXBF_I2C_SMBUS_TIMER_THOLD);

	timer = mlxbf_i2c_set_timer(priv, timings->setup_start, true,
				    MLXBF_I2C_MASK_16, MLXBF_I2C_SHIFT_0);
	timer |= mlxbf_i2c_set_timer(priv, timings->setup_stop, true,
				     MLXBF_I2C_MASK_16, MLXBF_I2C_SHIFT_16);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1164", timer, priv->timer->io +
		MLXBF_I2C_SMBUS_TIMER_TSETUP_START_STOP);

	timer = mlxbf_i2c_set_timer(priv, timings->setup_data, true,
				    MLXBF_I2C_MASK_16, MLXBF_I2C_SHIFT_0);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1169", timer, priv->timer->io + MLXBF_I2C_SMBUS_TIMER_TSETUP_DATA);

	timer = mlxbf_i2c_set_timer(priv, timings->buf, false,
				    MLXBF_I2C_MASK_16, MLXBF_I2C_SHIFT_0);
	timer |= mlxbf_i2c_set_timer(priv, timings->thigh_max, false,
				     MLXBF_I2C_MASK_16, MLXBF_I2C_SHIFT_16);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1175", timer, priv->timer->io + MLXBF_I2C_SMBUS_THIGH_MAX_TBUF);

	timer = timings->timeout;
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1178", timer, priv->timer->io + MLXBF_I2C_SMBUS_SCL_LOW_TIMEOUT);
}
				 ...
				 struct mlxbf_i2c_priv *priv)
{
	 ...
	 * functionality.
	 */
	config_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1450", gpio_res->io + MLXBF_I2C_GPIO_0_FUNC_EN_0);
	config_reg = MLXBF_I2C_GPIO_SMBUS_GW_ASSERT_PINS(priv->bus,
							 config_reg);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1453", config_reg, gpio_res->io + MLXBF_I2C_GPIO_0_FUNC_EN_0);

	config_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1455", gpio_res->io + MLXBF_I2C_GPIO_0_FORCE_OE_EN);
	config_reg = MLXBF_I2C_GPIO_SMBUS_GW_RESET_PINS(priv->bus,
							config_reg);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1458", config_reg, gpio_res->io + MLXBF_I2C_GPIO_0_FORCE_OE_EN);
...
static u64 mlxbf_i2c_calculate_freq_from_tyu(struct mlxbf_i2c_resource *corepll_res)
{
	...
	corepll_val = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1472", corepll_res->io + MLXBF_I2C_CORE_PLL_REG1);
...
static u64 mlxbf_i2c_calculate_freq_from_yu(struct mlxbf_i2c_resource *corepll_res)
{
	...
	corepll_reg1_val = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1502", corepll_res->io + MLXBF_I2C_CORE_PLL_REG1);
	corepll_reg2_val = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1503", corepll_res->io + MLXBF_I2C_CORE_PLL_REG2);
			      ...
			      struct i2c_client *slave)
{
	...
	for (reg = 0; reg < reg_cnt; reg++) {
		slave_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1590", priv->slv->io +
		...
		for (byte = 0; byte < 4; byte++) {
			...
			if (!(addr_tmp & MLXBF_I2C_SMBUS_SLAVE_ADDR_EN_BIT)) {
				...
				pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1612", slave_reg, priv->slv->io +
...
static int mlxbf_i2c_slave_disable(struct mlxbf_i2c_priv *priv, u8 addr)
{
	 ...
	 */
	for (reg = 0; reg < reg_cnt; reg++) {
		slave_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1647", priv->slv->io +
				MLXBF_I2C_SMBUS_SLAVE_ADDR_CFG + reg * 0x4);
		...
		for (byte = 0; byte < 4; byte++) {
			...
			if (addr_tmp == addr) {
				...
				pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1668", slave_reg, priv->slv->io +
					MLXBF_I2C_SMBUS_SLAVE_ADDR_CFG +
					(reg * 0x4));
				...
				struct mlxbf_i2c_priv *priv)
{
	...
	/* Reset FSM. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1772", 0, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_FSM);
	 ...
	 * interrupts first.
	 */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1781", ~0, priv->slv_cause->io + MLXBF_I2C_CAUSE_OR_CLEAR);
	int_reg = MLXBF_I2C_CAUSE_READ_WAIT_FW_RESPONSE;
	int_reg |= MLXBF_I2C_CAUSE_WRITE_SUCCESS;
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1784", int_reg, priv->slv_cause->io + MLXBF_I2C_CAUSE_OR_EVTEN0);

	/* Finally, set the 'ready' bit to start handling transactions. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1787", 0x1, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_READY);
				   ...
				   bool *write)
{
	...
	*read = false;
	...
	coalesce0_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1813", priv->coalesce->io + MLXBF_I2C_CAUSE_COALESCE_0);
	...
	cause_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:1820", priv->slv_cause->io + MLXBF_I2C_CAUSE_ARBITER);
	...
	/* Clear cause bits. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1827", ~0x0, priv->slv_cause->io + MLXBF_I2C_CAUSE_OR_CLEAR);
...
static int mlxbf_i2c_irq_send(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
{
	...
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1940", control32, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_GW);
...
clear_csr:
	...
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1950", 0x0, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_RS_MASTER_BYTES);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1951", 0x0, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_PEC);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:1952", 0x1, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_READY);
...
clear_csr:
	/* Release the Slave GW. */
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:2006", 0x0, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_RS_MASTER_BYTES);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:2007", 0x0, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_PEC);
	pete_writel("drivers/i2c/busses/i2c-mlxbf.c:2008", 0x1, priv->slv->io + MLXBF_I2C_SMBUS_SLAVE_READY);
...
static irqreturn_t mlxbf_i2c_irq(int irq, void *ptr)
{
	 ...
	 * from the master.
	 */
	rw_bytes_reg = pete_readl("drivers/i2c/busses/i2c-mlxbf.c:2043", priv->slv->io +
