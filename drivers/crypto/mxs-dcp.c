...
static int mxs_dcp_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/crypto/mxs-dcp.c:1037", MXS_DCP_CTRL_GATHER_RESIDUAL_WRITES |
	       MXS_DCP_CTRL_ENABLE_CONTEXT_CACHING | 0xf,
	       sdcp->base + MXS_DCP_CTRL);

	/* Enable all DCP DMA channels. */
	pete_writel("drivers/crypto/mxs-dcp.c:1042", MXS_DCP_CHANNELCTRL_ENABLE_CHANNEL_MASK,
	 ...
	 * address will do.
	 */
	pete_writel("drivers/crypto/mxs-dcp.c:1052", 0xffff0000, sdcp->base + MXS_DCP_CONTEXT);
	for (i = 0; i < DCP_MAX_CHANS; i++)
		pete_writel("drivers/crypto/mxs-dcp.c:1054", 0xffffffff, sdcp->base + MXS_DCP_CH_N_STAT_CLR(i));
	pete_writel("drivers/crypto/mxs-dcp.c:1055", 0xffffffff, sdcp->base + MXS_DCP_STAT_CLR);
	...
	sdcp->caps = pete_readl("drivers/crypto/mxs-dcp.c:1085", sdcp->base + MXS_DCP_CAPABILITY1);
