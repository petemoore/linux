...
static void ocs_hcu_done_irq_en(struct ocs_hcu_dev *hcu_dev)
{
	...
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:182", 0xFFFFFFFF, hcu_dev->io_base + OCS_HCU_ISR);
	hcu_dev->irq_err = false;
	/* Enable error and HCU done interrupts. */
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:185", HCU_IRQ_HASH_DONE | HCU_IRQ_HASH_ERR_MASK,
...
{
	/* Clear any pending interrupts. */
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:192", 0xFFFFFFFF, hcu_dev->io_base + OCS_HCU_DMA_MSI_ISR);
	hcu_dev->irq_err = false;
	/* Only operating on DMA source completion and error interrupts. */
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:195", HCU_DMA_IRQ_ERR_MASK | HCU_DMA_IRQ_SRC_DONE,
	       hcu_dev->io_base + OCS_HCU_DMA_MSI_IER);
	/* Unmask */
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:198", HCU_DMA_MSI_UNMASK, hcu_dev->io_base + OCS_HCU_DMA_MSI_MASK);
}

static void ocs_hcu_irq_dis(struct ocs_hcu_dev *hcu_dev)
{
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:203", HCU_IRQ_DISABLE, hcu_dev->io_base + OCS_HCU_IER);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:204", HCU_DMA_MSI_DISABLE, hcu_dev->io_base + OCS_HCU_DMA_MSI_IER);
}
					 ...
					 enum ocs_hcu_algo algo)
{
	 ...
	 */
	for (i = 0; i < n; i++)
		chain[i] = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:270", hcu_dev->io_base + OCS_HCU_CHAIN);

	data->msg_len_lo = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:272", hcu_dev->io_base + OCS_HCU_MSG_LEN_LO);
	data->msg_len_hi = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:273", hcu_dev->io_base + OCS_HCU_MSG_LEN_HI);
					  ...
					  enum ocs_hcu_algo algo)
{
	 ...
	 */
	for (i = 0; i < n; i++)
		pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:301", chain[i], hcu_dev->io_base + OCS_HCU_CHAIN);

	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:303", data->msg_len_lo, hcu_dev->io_base + OCS_HCU_MSG_LEN_LO);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:304", data->msg_len_hi, hcu_dev->io_base + OCS_HCU_MSG_LEN_HI);
}
			      ...
			      enum ocs_hcu_algo algo, u8 *dgst, size_t dgst_len)
{
	...
	for (i = 0; i < dgst_len / sizeof(u32); i++)
		chain[i] = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:328", hcu_dev->io_base + OCS_HCU_CHAIN);
			  ...
			  bool use_hmac)
{
	...
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:365", cfg, hcu_dev->io_base + OCS_HCU_MODE);
...
static void ocs_hcu_clear_key(struct ocs_hcu_dev *hcu_dev)
{
	...
	for (reg_off = 0; reg_off < OCS_HCU_HW_KEY_LEN; reg_off += sizeof(u32))
		pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:380", 0, hcu_dev->io_base + OCS_HCU_KEY_0 + reg_off);
}
...
static int ocs_hcu_write_key(struct ocs_hcu_dev *hcu_dev, const u8 *key, size_t len)
{
	 ...
	 * 3 <---> 0, 2 <---> 1.
	 */
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:418", HCU_BYTE_ORDER_SWAP,
	...
	for (i = 0; i < OCS_HCU_HW_KEY_LEN_U32; i++)
		pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:425", key_u32[OCS_HCU_HW_KEY_LEN_U32 - 1 - i],
				...
				bool finalize)
{
	...
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:471", dma_list->dma_addr, hcu_dev->io_base + OCS_HCU_DMA_NEXT_SRC_DESCR);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:472", 0, hcu_dev->io_base + OCS_HCU_DMA_SRC_SIZE);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:473", 0, hcu_dev->io_base + OCS_HCU_DMA_DST_SIZE);

	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:475", OCS_HCU_START, hcu_dev->io_base + OCS_HCU_OPERATION);

	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:477", cfg, hcu_dev->io_base + OCS_HCU_DMA_DMA_MODE);

	if (finalize)
		pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:480", OCS_HCU_TERMINATE, hcu_dev->io_base + OCS_HCU_OPERATION);
		       ...
		       size_t dgst_len)
{
	...
	ocs_hcu_done_irq_en(hcu_dev);
	reinit_completion(&hcu_dev->irq_done);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:705", OCS_HCU_TERMINATE, hcu_dev->io_base + OCS_HCU_OPERATION);
		   ...
		   void *data, size_t data_len, u8 *dgst, size_t dgst_len)
{
	...
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:749", dma_handle, hcu_dev->io_base + OCS_HCU_DMA_SRC_ADDR);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:750", data_len, hcu_dev->io_base + OCS_HCU_DMA_SRC_SIZE);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:751", OCS_HCU_START, hcu_dev->io_base + OCS_HCU_OPERATION);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:752", reg, hcu_dev->io_base + OCS_HCU_DMA_DMA_MODE);

	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:754", OCS_HCU_TERMINATE, hcu_dev->io_base + OCS_HCU_OPERATION);
...
irqreturn_t ocs_hcu_irq_handler(int irq, void *dev_id)
{
	...
	hcu_irq = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:815", hcu_dev->io_base + OCS_HCU_ISR);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:816", hcu_irq, hcu_dev->io_base + OCS_HCU_ISR);

	/* Read and clear the HCU DMA interrupt. */
	dma_irq = pete_readl("drivers/crypto/intel/keembay/ocs-hcu.c:819", hcu_dev->io_base + OCS_HCU_DMA_MSI_ISR);
	pete_writel("drivers/crypto/intel/keembay/ocs-hcu.c:820", dma_irq, hcu_dev->io_base + OCS_HCU_DMA_MSI_ISR);
