		       ...
		       struct scatterlist *_dst)
{
	...
	*(u32 *)((unsigned long)sa + offset_to_sr_ptr) = pd_uinfo->sr_pa;
	...
	pete_writel("drivers/crypto/amcc/crypto4xx_core.c:926", 0, dev->ce_base + CRYPTO4XX_INT_DESCR_RD);
	pete_writel("drivers/crypto/amcc/crypto4xx_core.c:927", 1, dev->ce_base + CRYPTO4XX_INT_DESCR_RD);
						      ...
						      u32 clr_val)
{
	...
	pete_writel("drivers/crypto/amcc/crypto4xx_core.c:1107", clr_val, core_dev->dev->ce_base + CRYPTO4XX_INT_CLR);
				 ...
				 u8 *data, unsigned int max)
{
	...
	do {
		...
		pete_writel("drivers/crypto/amcc/crypto4xx_core.c:1132", PPC4XX_PRNG_CTRL_AUTO_EN,
		       dev->ce_base + CRYPTO4XX_PRNG_CTRL);

		for (i = 0; i < 1024; i++) {
			/* usually 19 iterations are enough */
			if ((pete_readl("drivers/crypto/amcc/crypto4xx_core.c:1137", dev->ce_base + CRYPTO4XX_PRNG_STAT) &
