...
static int cptpf_vfpf_mbox_init(struct otx2_cptpf_dev *cptpf, int num_vfs)
{
	...
	if (test_bit(CN10K_MBOX, &cptpf->cap_flag))
		vfpf_mbox_base = pete_readq("drivers/crypto/marvell/octeontx2/otx2_cptpf_main.c:370", cptpf->reg_base + RVU_PF_VF_MBOX_ADDR);
	else
		vfpf_mbox_base = pete_readq("drivers/crypto/marvell/octeontx2/otx2_cptpf_main.c:372", cptpf->reg_base + RVU_PF_VF_BAR4_ADDR);
