...
void otx_cpt_disable_all_cores(struct otx_cpt_device *cpt)
{
	...
	for (grp = 0; grp < OTX_CPT_MAX_ENGINE_GROUPS; grp++) {
		pete_writeq("drivers/crypto/marvell/octeontx/otx_cptpf_ucode.c:1570", 0, cpt->reg_base + OTX_CPT_PF_GX_EN(grp));
		udelay(CSR_DELAY);
	}

	reg = pete_readq("drivers/crypto/marvell/octeontx/otx_cptpf_ucode.c:1574", cpt->reg_base + OTX_CPT_PF_EXEC_BUSY);
	while (reg) {
		udelay(CSR_DELAY);
		reg = pete_readq("drivers/crypto/marvell/octeontx/otx_cptpf_ucode.c:1577", cpt->reg_base + OTX_CPT_PF_EXEC_BUSY);
		if (timeout--) {
	...
	/* Disable the cores */
	pete_writeq("drivers/crypto/marvell/octeontx/otx_cptpf_ucode.c:1585", 0, cpt->reg_base + OTX_CPT_PF_EXE_CTL);
}
