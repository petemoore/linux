				  ...
				  const struct firmware *fw)
{
	...
	for (i = 0; i < fw->size / sizeof(u32); i++) {
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:329", val,
		       priv->base + EIP197_CLASSIFICATION_RAMS +
		       i * sizeof(val));
	}
				  ...
				  int ipuesz, int ifppsz, int minifw)
{
	...
	for (pe = 0; pe < priv->config.pes; pe++) {
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:377", 0, EIP197_PE(priv) + EIP197_PE_ICE_RAM_CTRL(pe));
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:386", val, EIP197_PE(priv) + EIP197_PE_ICE_FPP_CTRL(pe));
		...
		if (minifw)
			...
			val = EIP197_PE_ICE_UENG_START_OFFSET((ipuesz - 1) &
					EIP197_PE_ICE_UENG_INIT_ALIGN_MASK) |
				EIP197_PE_ICE_UENG_DEBUG_RESET;
		pete_writel("drivers/crypto/inside-secure/safexcel.c:395", val, EIP197_PE(priv) + EIP197_PE_ICE_PUE_CTRL(pe));
	}
...
retry_fw:
	...
	for (pe = 0; pe < priv->config.pes; pe++)
		pete_writel("drivers/crypto/inside-secure/safexcel.c:453", EIP197_PE_ICE_RAM_CTRL_PUE_PROG_EN,
		       EIP197_PE(priv) + EIP197_PE_ICE_RAM_CTRL(pe));
...
static int safexcel_hw_setup_cdesc_rings(struct safexcel_crypto_priv *priv)
{
	...
	for (i = 0; i < priv->config.rings; i++) {
		/* ring base address */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:512", lower_32_bits(priv->ring[i].cdr.base_dma),
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_LO);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:514", upper_32_bits(priv->ring[i].cdr.base_dma),
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_HI);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:517", EIP197_xDR_DESC_MODE_64BIT | EIP197_CDR_DESC_MODE_ADCP |
		       (priv->config.cd_offset << 14) | priv->config.cd_size,
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_DESC_SIZE);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:520", ((cd_fetch_cnt *
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:528", val, EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_DMA_CFG);

		/* clear any pending interrupt */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:531", GENMASK(5, 0),
...
static int safexcel_hw_setup_rdesc_rings(struct safexcel_crypto_priv *priv)
{
	...
	for (i = 0; i < priv->config.rings; i++) {
		/* ring base address */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:560", lower_32_bits(priv->ring[i].rdr.base_dma),
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_LO);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:562", upper_32_bits(priv->ring[i].rdr.base_dma),
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_HI);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:565", EIP197_xDR_DESC_MODE_64BIT | (priv->config.rd_offset << 14) |
		       priv->config.rd_size,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_DESC_SIZE);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:569", ((rd_fetch_cnt *
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:578", val,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_DMA_CFG);

		/* clear any pending interrupt */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:582", GENMASK(7, 0),
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_STAT);

		/* enable ring interrupt */
		val = pete_readl("drivers/crypto/inside-secure/safexcel.c:586", EIP197_HIA_AIC_R(priv) + EIP197_HIA_AIC_R_ENABLE_CTRL(i));
		val |= EIP197_RDR_IRQ(i);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:588", val, EIP197_HIA_AIC_R(priv) + EIP197_HIA_AIC_R_ENABLE_CTRL(i));
	}
...
static int safexcel_hw_init(struct safexcel_crypto_priv *priv)
{
	 ...
	 */
	if (priv->flags & SAFEXCEL_HW_EIP197) {
		val = pete_readl("drivers/crypto/inside-secure/safexcel.c:607", EIP197_HIA_AIC(priv) + EIP197_HIA_MST_CTRL);
		val |= EIP197_MST_CTRL_TX_MAX_CMD(5);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:609", val, EIP197_HIA_AIC(priv) + EIP197_HIA_MST_CTRL);
	}

	/* Configure wr/rd cache values */
	pete_writel("drivers/crypto/inside-secure/safexcel.c:613", EIP197_MST_CTRL_RD_CACHE(RD_CACHE_4BITS) |
	...
	pete_writel("drivers/crypto/inside-secure/safexcel.c:620", 0, EIP197_HIA_AIC_G(priv) + EIP197_HIA_AIC_G_ENABLE_CTRL);

	/* Clear any pending interrupt */
	pete_writel("drivers/crypto/inside-secure/safexcel.c:623", GENMASK(31, 0), EIP197_HIA_AIC_G(priv) + EIP197_HIA_AIC_G_ACK);
	...
	for (pe = 0; pe < priv->config.pes; pe++) {
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:630", EIP197_DxE_THR_CTRL_RESET_PE,
		       EIP197_HIA_DFE_THR(priv) + EIP197_HIA_DFE_THR_CTRL(pe));

		if (priv->flags & EIP197_PE_ARB)
			/* Reset HIA input interface arbiter (if present) */
			pete_writel("drivers/crypto/inside-secure/safexcel.c:635", EIP197_HIA_RA_PE_CTRL_RESET,
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:646", val, EIP197_HIA_DFE(priv) + EIP197_HIA_DFE_CFG(pe));

		/* Leave the DFE threads reset state */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:649", 0, EIP197_HIA_DFE_THR(priv) + EIP197_HIA_DFE_THR_CTRL(pe));

		/* Configure the processing engine thresholds */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:652", EIP197_PE_IN_xBUF_THRES_MIN(6) |
		       EIP197_PE_IN_xBUF_THRES_MAX(9),
		       EIP197_PE(priv) + EIP197_PE_IN_DBUF_THRES(pe));
		pete_writel("drivers/crypto/inside-secure/safexcel.c:655", EIP197_PE_IN_xBUF_THRES_MIN(6) |
		       EIP197_PE_IN_xBUF_THRES_MAX(7),
		       EIP197_PE(priv) + EIP197_PE_IN_TBUF_THRES(pe));

		if (priv->flags & SAFEXCEL_HW_EIP197)
			/* enable HIA input interface arbiter and rings */
			pete_writel("drivers/crypto/inside-secure/safexcel.c:661", EIP197_HIA_RA_PE_CTRL_EN |
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:668", EIP197_DxE_THR_CTRL_RESET_PE,
		       EIP197_HIA_DSE_THR(priv) + EIP197_HIA_DSE_THR_CTRL(pe));

		/* Wait for all DSE threads to complete */
		while ((pete_readl("drivers/crypto/inside-secure/safexcel.c:672", EIP197_HIA_DSE_THR(priv) + EIP197_HIA_DSE_THR_STAT(pe)) &
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:694", val, EIP197_HIA_DSE(priv) + EIP197_HIA_DSE_CFG(pe));

		/* Leave the DSE threads reset state */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:697", 0, EIP197_HIA_DSE_THR(priv) + EIP197_HIA_DSE_THR_CTRL(pe));

		/* Configure the processing engine thresholds */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:700", EIP197_PE_OUT_DBUF_THRES_MIN(opbuflo) |
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:710", val, EIP197_PE(priv) + EIP197_PE_EIP96_TOKEN_CTRL(pe));

		/* H/W capabilities selection: just enable everything */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:713", EIP197_FUNCTION_ALL,
		       EIP197_PE(priv) + EIP197_PE_EIP96_FUNCTION_EN(pe));
		pete_writel("drivers/crypto/inside-secure/safexcel.c:715", EIP197_FUNCTION_ALL,
	...
	for (i = 0; i < priv->config.rings; i++) {
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:722", GENMASK(31, 0),
		       EIP197_HIA_AIC_R(priv) + EIP197_HIA_AIC_R_ENABLE_CLR(i));

		/* Disable external triggering */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:726", 0, EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_CFG);

		/* Clear the pending prepared counter */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:729", EIP197_xDR_PREP_CLR_COUNT,
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_PREP_COUNT);

		/* Clear the pending processed counter */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:733", EIP197_xDR_PROC_CLR_COUNT,
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_PROC_COUNT);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:736", 0,
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_PREP_PNTR);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:738", 0,
		       EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_PROC_PNTR);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:741", (EIP197_DEFAULT_RING_SIZE * priv->config.cd_offset),
	...
	for (i = 0; i < priv->config.rings; i++) {
		/* Disable external triggering*/
		pete_writel("drivers/crypto/inside-secure/safexcel.c:748", 0, EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_CFG);

		/* Clear the pending prepared counter */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:751", EIP197_xDR_PREP_CLR_COUNT,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_PREP_COUNT);

		/* Clear the pending processed counter */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:755", EIP197_xDR_PROC_CLR_COUNT,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_PROC_COUNT);

		pete_writel("drivers/crypto/inside-secure/safexcel.c:758", 0,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_PREP_PNTR);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:760", 0,
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_PROC_PNTR);

		/* Ring size */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:764", (EIP197_DEFAULT_RING_SIZE * priv->config.rd_offset),
		       EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_RING_SIZE);
	}

	for (pe = 0; pe < priv->config.pes; pe++) {
		/* Enable command descriptor rings */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:770", EIP197_DxE_THR_CTRL_EN | GENMASK(priv->config.rings - 1, 0),
		       EIP197_HIA_DFE_THR(priv) + EIP197_HIA_DFE_THR_CTRL(pe));

		/* Enable result descriptor rings */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:774", EIP197_DxE_THR_CTRL_EN | GENMASK(priv->config.rings - 1, 0),
		       EIP197_HIA_DSE_THR(priv) + EIP197_HIA_DSE_THR_CTRL(pe));
	}

	/* Clear any HIA interrupt */
	pete_writel("drivers/crypto/inside-secure/safexcel.c:779", GENMASK(30, 20), EIP197_HIA_AIC_G(priv) + EIP197_HIA_AIC_G_ACK);

	if (priv->flags & EIP197_SIMPLE_TRC) {
		pete_writel("drivers/crypto/inside-secure/safexcel.c:782", EIP197_STRC_CONFIG_INIT |
		...
		pete_writel("drivers/crypto/inside-secure/safexcel.c:786", EIP197_PE_EIP96_TOKEN_CTRL2_CTX_DONE,
				       ...
				       int ring)
{
	...
	pete_writel("drivers/crypto/inside-secure/safexcel.c:815", EIP197_HIA_RDR_THRESH_PKT_MODE |
...
finalize:
	...
	pete_writel("drivers/crypto/inside-secure/safexcel.c:890", (rdesc * priv->config.rd_offset),
	       EIP197_HIA_RDR(priv, ring) + EIP197_HIA_xDR_PREP_COUNT);

	/* let the CDR know we have pending descriptors */
	pete_writel("drivers/crypto/inside-secure/safexcel.c:894", (cdesc * priv->config.cd_offset),
...
handle_results:
	...
	nreq = pete_readl("drivers/crypto/inside-secure/safexcel.c:1022", EIP197_HIA_RDR(priv, ring) + EIP197_HIA_xDR_PROC_COUNT);
...
acknowledge:
	if (i)
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1052", EIP197_xDR_PROC_xD_PKT(i) |
...
static irqreturn_t safexcel_irq_ring(int irq, void *data)
{
	...
	status = pete_readl("drivers/crypto/inside-secure/safexcel.c:1094", EIP197_HIA_AIC_R(priv) + EIP197_HIA_AIC_R_ENABLED_STAT(ring));
	if (!status)
		return rc;

	/* RDR interrupts */
	if (status & EIP197_RDR_IRQ(ring)) {
		stat = pete_readl("drivers/crypto/inside-secure/safexcel.c:1100", EIP197_HIA_RDR(priv, ring) + EIP197_HIA_xDR_STAT);
		...
		/* ACK the interrupts */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1114", stat & 0xff,
		       EIP197_HIA_RDR(priv, ring) + EIP197_HIA_xDR_STAT);
	}

	/* ACK the interrupts */
	pete_writel("drivers/crypto/inside-secure/safexcel.c:1119", status, EIP197_HIA_AIC_R(priv) + EIP197_HIA_AIC_R_ACK(ring));
				  ...
				  int is_pci_dev)
{
	 ...
	 * values
	 */
	version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1413", priv->base + EIP97_HIA_AIC_BASE + EIP197_HIA_VERSION);
	...
	if (EIP197_REG_LO16(version) == EIP197_HIA_VERSION_LE) {
		...
		version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1424", priv->base + EIP197_HIA_AIC_BASE +
				EIP197_HIA_VERSION);
		if (EIP197_REG_LO16(version) == EIP197_HIA_VERSION_LE) {
			priv->hwconfig.hiaver = EIP197_VERSION_MASK(version);
	 ...
	 */
	if (mask) {
		val = pete_readl("drivers/crypto/inside-secure/safexcel.c:1449", EIP197_HIA_AIC(priv) + EIP197_HIA_MST_CTRL);
		val = val ^ (mask >> 24); /* toggle byte swap bits */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1451", val, EIP197_HIA_AIC(priv) + EIP197_HIA_MST_CTRL);
	}
	 ...
	 */
	version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1459", EIP197_GLOBAL(priv) + EIP197_VERSION);
	if (((priv->flags & SAFEXCEL_HW_EIP197) &&
	...
	version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1479", EIP197_PE(priv) + + EIP197_PE_VERSION(0));
	...
	version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1487", EIP197_PE(priv) + EIP197_PE_EIP96_VERSION(0));
	...
	hwopt = pete_readl("drivers/crypto/inside-secure/safexcel.c:1494", EIP197_GLOBAL(priv) + EIP197_OPTIONS);
	hiaopt = pete_readl("drivers/crypto/inside-secure/safexcel.c:1495", EIP197_HIA_AIC(priv) + EIP197_HIA_OPTIONS);
	...
	if (priv->flags & SAFEXCEL_HW_EIP197) {
		/* EIP197 */
		peopt = pete_readl("drivers/crypto/inside-secure/safexcel.c:1502", EIP197_PE(priv) + EIP197_PE_OPTIONS(0));
		...
		if (EIP206_OPT_ICE_TYPE(peopt) == 1) {
			...
			version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1521", EIP197_PE(priv) +
		...
		if (EIP206_OPT_OCE_TYPE(peopt) == 1) {
			...
			version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1533", EIP197_PE(priv) + EIP197_PE_PSE_VERSION(0));
			if (EIP197_REG_LO16(version) != EIP96_VERSION_LE) {
			...
			version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1540", EIP197_PE(priv) +
				  EIP197_PE_ICE_VERSION(0));
			if (EIP197_REG_LO16(version) != EIP207_VERSION_LE) {
	...
	for (i = 0; i < EIP197_MAX_RING_AIC; i++) {
		version = pete_readl("drivers/crypto/inside-secure/safexcel.c:1569", EIP197_HIA_AIC_R(priv) +
	...
	priv->hwconfig.algo_flags = pete_readl("drivers/crypto/inside-secure/safexcel.c:1582", EIP197_PE(priv) +
...
static void safexcel_hw_reset_rings(struct safexcel_crypto_priv *priv)
{
	...
	for (i = 0; i < priv->config.rings; i++) {
		/* clear any pending interrupt */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1722", GENMASK(5, 0), EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_STAT);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1723", GENMASK(7, 0), EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_STAT);

		/* Reset the CDR base address */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1726", 0, EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_LO);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1727", 0, EIP197_HIA_CDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_HI);

		/* Reset the RDR base address */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1730", 0, EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_LO);
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1731", 0, EIP197_HIA_RDR(priv, i) + EIP197_HIA_xDR_RING_BASE_ADDR_HI);
	}
}
			       ...
			       const struct pci_device_id *ent)
{
	...
	if (priv->data->version == EIP197_DEVBRD) {
		...
		val = pete_readl("drivers/crypto/inside-secure/safexcel.c:1933", pciebase + EIP197_XLX_IRQ_BLOCK_ID_ADDR);
		if ((val >> 16) == EIP197_XLX_IRQ_BLOCK_ID_VALUE) {
			...
			pete_writel("drivers/crypto/inside-secure/safexcel.c:1939", EIP197_XLX_USER_VECT_LUT0_IDENT,
			       pciebase + EIP197_XLX_USER_VECT_LUT0_ADDR);
			pete_writel("drivers/crypto/inside-secure/safexcel.c:1941", EIP197_XLX_USER_VECT_LUT1_IDENT,
			       pciebase + EIP197_XLX_USER_VECT_LUT1_ADDR);
			pete_writel("drivers/crypto/inside-secure/safexcel.c:1943", EIP197_XLX_USER_VECT_LUT2_IDENT,
			       pciebase + EIP197_XLX_USER_VECT_LUT2_ADDR);
			pete_writel("drivers/crypto/inside-secure/safexcel.c:1945", EIP197_XLX_USER_VECT_LUT3_IDENT,
			       pciebase + EIP197_XLX_USER_VECT_LUT3_ADDR);

			/* Enable all device interrupts */
			pete_writel("drivers/crypto/inside-secure/safexcel.c:1949", GENMASK(31, 0),
		...
		/* assert reset */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1959", 1, priv->base + EIP197_XLX_GPIO_BASE);
		wmb(); /* maintain strict ordering for accesses here */
		/* deassert reset */
		pete_writel("drivers/crypto/inside-secure/safexcel.c:1962", 0, priv->base + EIP197_XLX_GPIO_BASE);
		wmb(); /* maintain strict ordering for accesses here */
	}
