		      ...
		      const char *name)
{
	...
	for (i = 0; i < MAX_SG; i++) {
		...
		mutex_lock(&ss->mlock);
		pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:108", rctx->p_key, ss->base + SS_KEY_ADR_REG);

		if (ivlen) {
			if (rctx->op_dir == SS_ENCRYPTION) {
				if (i == 0)
					pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:113", rctx->p_iv[0], ss->base + SS_IV_ADR_REG);
				else
					pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:115", rctx->t_dst[i - 1].addr + rctx->t_dst[i - 1].len * 4 - ivlen, ss->base + SS_IV_ADR_REG);
			} else {
				pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:117", rctx->p_iv[i], ss->base + SS_IV_ADR_REG);
			}
		}
		...
		pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:128", rctx->t_src[i].addr, ss->base + SS_SRC_ADR_REG);
		pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:129", rctx->t_dst[i].addr, ss->base + SS_DST_ADR_REG);
		pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:130", rctx->t_src[i].len, ss->base + SS_LEN_ADR_REG);

		reinit_completion(&ss->flows[flow].complete);
		ss->flows[flow].status = 0;
		wmb();

		pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:136", v, ss->base + SS_CTL_REG);
...
static irqreturn_t ss_irq_handler(int irq, void *data)
{
	...
	p = pete_readl("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:155", ss->base + SS_INT_STA_REG);
	for (flow = 0; flow < MAXFLOW; flow++) {
		if (p & (BIT(flow))) {
			pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:158", BIT(flow), ss->base + SS_INT_STA_REG);
...
static int sun8i_ss_pm_resume(struct device *dev)
{
	...
	pete_writel("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:648", BIT(0) | BIT(1), ss->base + SS_INT_CTL_REG);
...
static int sun8i_ss_probe(struct platform_device *pdev)
{
	...
	v = pete_readl("drivers/crypto/allwinner/sun8i-ss/sun8i-ss-core.c:878", ss->base + SS_CTL_REG);
	v >>= SS_DIE_ID_SHIFT;
	v &= SS_DIE_ID_MASK;
