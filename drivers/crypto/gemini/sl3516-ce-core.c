...
static void start_dma_tx(struct sl3516_ce_dev *ce)
{
	...
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:84", v, ce->base + IPSEC_TXDMA_CTRL);
}
...
static void start_dma_rx(struct sl3516_ce_dev *ce)
{
	...
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:96", v, ce->base + IPSEC_RXDMA_CTRL);
}
		       ...
		       const char *name)
{
	...
	v = pete_readl("drivers/crypto/gemini/sl3516-ce-core.c:178", ce->base + IPSEC_STATUS_REG);
	if (v & 0xFFF) {
...
static irqreturn_t ce_irq_handler(int irq, void *data)
{
	...
	v = pete_readl("drivers/crypto/gemini/sl3516-ce-core.c:194", ce->base + IPSEC_DMA_STATUS);
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:195", v, ce->base + IPSEC_DMA_STATUS);
...
static void sl3516_ce_start(struct sl3516_ce_dev *ce)
{
	ce->ctx = 0;
	ce->crx = 0;
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:330", ce->dtx, ce->base + IPSEC_TXDMA_CURR_DESC);
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:331", ce->drx, ce->base + IPSEC_RXDMA_CURR_DESC);
	pete_writel("drivers/crypto/gemini/sl3516-ce-core.c:332", 0, ce->base + IPSEC_DMA_STATUS);
}
...
static int sl3516_ce_probe(struct platform_device *pdev)
{
	...
	v = pete_readl("drivers/crypto/gemini/sl3516-ce-core.c:468", ce->base + IPSEC_ID);
	dev_info(ce->dev, "SL3516 dev %lx rev %lx\n",
		 v & GENMASK(31, 4),
		 v & GENMASK(3, 0));
	v = pete_readl("drivers/crypto/gemini/sl3516-ce-core.c:472", ce->base + IPSEC_DMA_DEVICE_ID);
