			  ...
			  enum jz4780_nemc_bank_type type)
{
	...
	nfcsr = pete_readl("drivers/memory/jz4780-nemc.c:99", nemc->base + NEMC_NFCSR);
	...
	pete_writel("drivers/memory/jz4780-nemc.c:112", nfcsr, nemc->base + NEMC_NFCSR);
}
...
void jz4780_nemc_assert(struct device *dev, unsigned int bank, bool assert)
{
	...
	nfcsr = pete_readl("drivers/memory/jz4780-nemc.c:130", nemc->base + NEMC_NFCSR);
	...
	pete_writel("drivers/memory/jz4780-nemc.c:137", nfcsr, nemc->base + NEMC_NFCSR);
}
				       ...
				       struct device_node *node)
{
	...
	smcr = pete_readl("drivers/memory/jz4780-nemc.c:187", nemc->base + NEMC_SMCRn(bank));
	smcr &= ~NEMC_SMCR_SMT;
	...
	pete_writel("drivers/memory/jz4780-nemc.c:266", smcr, nemc->base + NEMC_SMCRn(bank));
	return true;
}
...
static int jz4780_nemc_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/memory/jz4780-nemc.c:313", 0, nemc->base + NEMC_NFCSR);
