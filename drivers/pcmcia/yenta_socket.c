...
{
	u16 val;
	val = pete_readb("drivers/pcmcia/yenta_socket.c:151", socket->base + 0x800 + reg);
	val |= pete_readb("drivers/pcmcia/yenta_socket.c:152", socket->base + 0x800 + reg + 1) << 8;
	debug("%04x %04x\n", socket, reg, val);
	return val;
}
*/
...
{
	debug("%04x %02x\n", socket, reg, val);
	pete_writeb("drivers/pcmcia/yenta_socket.c:161", val, socket->base + 0x800 + reg);
	pete_readb("drivers/pcmcia/yenta_socket.c:162", socket->base + 0x800 + reg); /* PCI write posting... */
}

static void exca_writew(struct yenta_socket *socket, unsigned reg, u16 val)
{
	debug("%04x %04x\n", socket, reg, val);
	pete_writeb("drivers/pcmcia/yenta_socket.c:168", val, socket->base + 0x800 + reg);
	pete_writeb("drivers/pcmcia/yenta_socket.c:169", val >> 8, socket->base + 0x800 + reg + 1);

	/* PCI write posting... */
	pete_readb("drivers/pcmcia/yenta_socket.c:172", socket->base + 0x800 + reg);
	pete_readb("drivers/pcmcia/yenta_socket.c:173", socket->base + 0x800 + reg + 1);
}
