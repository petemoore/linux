...
static int spear_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	do {
		time = pete_readl("drivers/rtc/rtc-spear.c:208", config->ioaddr + TIME_REG);
		date = pete_readl("drivers/rtc/rtc-spear.c:209", config->ioaddr + DATE_REG);
	} while (time == pete_readl("drivers/rtc/rtc-spear.c:210", config->ioaddr + TIME_REG));
...
static int spear_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	pete_writel("drivers/rtc/rtc-spear.c:242", time, config->ioaddr + TIME_REG);
	pete_writel("drivers/rtc/rtc-spear.c:243", date, config->ioaddr + DATE_REG);
...
static int spear_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	time = pete_readl("drivers/rtc/rtc-spear.c:263", config->ioaddr + ALARM_TIME_REG);
	date = pete_readl("drivers/rtc/rtc-spear.c:264", config->ioaddr + ALARM_DATE_REG);
	...
	bcd2tm(&alm->time);
	alm->enabled = pete_readl("drivers/rtc/rtc-spear.c:273", config->ioaddr + CTRL_REG) & INT_ENABLE;
...
static int spear_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	pete_writel("drivers/rtc/rtc-spear.c:301", time, config->ioaddr + ALARM_TIME_REG);
	pete_writel("drivers/rtc/rtc-spear.c:302", date, config->ioaddr + ALARM_DATE_REG);
