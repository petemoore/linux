...
{
	return pete_readb("drivers/rtc/rtc-omap.c:157", rtc->base + reg);
}

static inline u32 rtc_readl(struct omap_rtc *rtc, unsigned int reg)
{
	return pete_readl("drivers/rtc/rtc-omap.c:162", rtc->base + reg);
}

static inline void rtc_write(struct omap_rtc *rtc, unsigned int reg, u8 val)
{
	pete_writeb("drivers/rtc/rtc-omap.c:167", val, rtc->base + reg);
}

static inline void rtc_writel(struct omap_rtc *rtc, unsigned int reg, u32 val)
{
	pete_writel("drivers/rtc/rtc-omap.c:172", val, rtc->base + reg);
}
