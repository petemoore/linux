				 ...
				 struct device_node *parent)
{
	...
	for (n = 0, base = 0; n < num_chips; n++, base += ORION_IRQS_PER_CHIP) {
		...
		/* mask all interrupts */
		pete_writel("drivers/irqchip/irq-orion.c:94", 0, gc->reg_base + ORION_IRQ_MASK);
	}
					...
					struct device_node *parent)
{
	...
	pete_writel("drivers/irqchip/irq-orion.c:196", 0, gc->reg_base + ORION_BRIDGE_IRQ_MASK);
	pete_writel("drivers/irqchip/irq-orion.c:197", 0, gc->reg_base + ORION_BRIDGE_IRQ_CAUSE);
