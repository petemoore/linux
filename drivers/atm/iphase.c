...
static void ia_hack_tcq(IADEV *dev) {
  ...
  tcq_wr = pete_readl("drivers/atm/iphase.c:148", dev->seg_reg+TCQ_WR_PTR) & 0xffff;
...
static void clear_lockup (struct atm_vcc *vcc, IADEV *dev) {
  ...
  if (vcc->qos.txtp.traffic_class == ATM_ABR) {
	...
	pete_writew("drivers/atm/iphase.c:273", 0xFFFD, dev->seg_reg+MODE_REG_0);
        /* Wait for 10 Micro sec */
        udelay(10); 
	...
	if (i < dev->num_vc)
        ...
        pete_writew("drivers/atm/iphase.c:284", T_ONLINE, dev->seg_reg+MODE_REG_0);
        pete_writew("drivers/atm/iphase.c:285", ~(TRANSMIT_DONE|TCQ_NOT_EMPTY), dev->seg_reg+SEG_MASK_REG);
        pete_writew("drivers/atm/iphase.c:286", TRANSMIT_DONE, dev->seg_reg+SEG_INTR_STATUS_REG);       
	vcstatus->cnt = 0;
     } /* foundLockUp */
...
static int ia_cbr_setup (IADEV *dev, struct atm_vcc *vcc) {
   ...
   if (dev->NumEnabledCBR == 1) {
       pete_writew("drivers/atm/iphase.c:570", (CBR_EN | UBR_EN | ABR_EN | (0x23 << 2)), dev->seg_reg+STPARMS);
...
static void ia_cbrVc_close (struct atm_vcc *vcc) {
   ...
   if (iadev->NumEnabledCBR == 0) {
      pete_writew("drivers/atm/iphase.c:584", (UBR_EN | ABR_EN | (0x23 << 2)), iadev->seg_reg+STPARMS);
...
static void ia_eeprom_put (IADEV *iadev, u32 addr, u_short val)
{
	...
	NVRAM_CLR_CE;
	CFG_OR(NVCE);
	t = pete_readl("drivers/atm/iphase.c:722", iadev->reg+IPHASE5575_EEPROM_ACCESS); 
	while (!(t & NVDO))
		t = pete_readl("drivers/atm/iphase.c:724", iadev->reg+IPHASE5575_EEPROM_ACCESS); 
...
{
	return pete_readl("drivers/atm/iphase.c:823", ia->phy + (reg >> 2));
}

static void ia_phy_write32(struct iadev_priv *ia, unsigned int reg, u32 val)
{
	pete_writel("drivers/atm/iphase.c:828", val, ia->phy + (reg >> 2));
}
...
static void desc_dbg(IADEV *iadev) {
  ...
  u32 i;
  void __iomem *tmp;
  // regval = pete_readl("drivers/atm/iphase.c:1031", (u32)ia_cmds->maddr);
  tcq_wr_ptr =  pete_readw("drivers/atm/iphase.c:1032", iadev->seg_reg+TCQ_WR_PTR);
  printk("B_tcq_wr = 0x%x desc = %d last desc = %d\n",
                     tcq_wr_ptr, pete_readw("drivers/atm/iphase.c:1034", iadev->seg_ram+tcq_wr_ptr),
                     pete_readw("drivers/atm/iphase.c:1035", iadev->seg_ram+tcq_wr_ptr-2));
  printk(" host_tcq_wr = 0x%x  host_tcq_rd = 0x%x \n",  iadev->host_tcq_wr, 
                   iadev->ffL.tcq_rd);
  tcq_st_ptr =  pete_readw("drivers/atm/iphase.c:1038", iadev->seg_reg+TCQ_ST_ADR);
  tcq_ed_ptr =  pete_readw("drivers/atm/iphase.c:1039", iadev->seg_reg+TCQ_ED_ADR);
  printk("tcq_st_ptr = 0x%x    tcq_ed_ptr = 0x%x \n", tcq_st_ptr, tcq_ed_ptr);
  i = 0;
  while (tcq_st_ptr != tcq_ed_ptr) {
      tmp = iadev->seg_ram+tcq_st_ptr;
      printk("TCQ slot %d desc = %d  Addr = %p\n", i++, pete_readw("drivers/atm/iphase.c:1044", tmp), tmp);
      tcq_st_ptr += 2;
  }
...
{  
  ...
  state = pete_readl("drivers/atm/iphase.c:1063", iadev->reass_reg + STATE_REG) & 0xffff;  
  while((state & EXCPQ_EMPTY) != EXCPQ_EMPTY)  
  { printk("state = %x \n", state); 
        excpq_rd_ptr = pete_readw("drivers/atm/iphase.c:1066", iadev->reass_reg + EXCP_Q_RD_PTR) & 0xffff;  
 printk("state = %x excpq_rd_ptr = %x \n", state, excpq_rd_ptr); 
	...
	vci = pete_readw("drivers/atm/iphase.c:1071", iadev->reass_ram+excpq_rd_ptr);  
	error = pete_readw("drivers/atm/iphase.c:1072", iadev->reass_ram+excpq_rd_ptr+2) & 0x0007;  
        // pwang_test
	excpq_rd_ptr += 4;  
	if (excpq_rd_ptr > (pete_readw("drivers/atm/iphase.c:1075", iadev->reass_reg + EXCP_Q_ED_ADR)& 0xffff))  
 	    excpq_rd_ptr = pete_readw("drivers/atm/iphase.c:1076", iadev->reass_reg + EXCP_Q_ST_ADR)& 0xffff;
	pete_writew("drivers/atm/iphase.c:1077",  excpq_rd_ptr, iadev->reass_reg + EXCP_Q_RD_PTR);  
        state = pete_readl("drivers/atm/iphase.c:1078", iadev->reass_reg + STATE_REG) & 0xffff;  
  }  
#endif
}  
...
{  
	IADEV *iadev;  
	iadev = INPH_IA_DEV(dev);  
        pete_writew("drivers/atm/iphase.c:1087", desc, iadev->reass_ram+iadev->rfL.fdq_wr); 
	iadev->rfL.fdq_wr +=2;
	if (iadev->rfL.fdq_wr > iadev->rfL.fdq_ed)
		iadev->rfL.fdq_wr =  iadev->rfL.fdq_st;  
	pete_writew("drivers/atm/iphase.c:1091", iadev->rfL.fdq_wr, iadev->reass_reg+FREEQ_WR_PTR);  
}  
...
{  
	...
	iadev = INPH_IA_DEV(dev);  
	if (iadev->rfL.pcq_rd == (pete_readw("drivers/atm/iphase.c:1108", iadev->reass_reg+PCQ_WR_PTR)&0xffff)) 
	{  
   	    printk(KERN_ERR DEV_LABEL "(itf %d) Receive queue empty\n", dev->number);  
	    return -EINVAL;  
	}  
	/* mask 1st 3 bits to get the actual descno. */  
	desc = pete_readw("drivers/atm/iphase.c:1114", iadev->reass_ram+iadev->rfL.pcq_rd) & 0x1fff;  
        IF_RX(printk("reass_ram = %p iadev->rfL.pcq_rd = 0x%x desc = %d\n", 
                                    iadev->reass_ram, iadev->rfL.pcq_rd, desc);
              printk(" pcq_wr_ptr = 0x%x\n",
                               pete_readw("drivers/atm/iphase.c:1118", iadev->reass_reg+PCQ_WR_PTR)&0xffff);)
	...
	if ( iadev->rfL.pcq_rd== iadev->rfL.pcq_ed) 
		...
		iadev->rfL.pcq_rd += 2;
	pete_writew("drivers/atm/iphase.c:1124", iadev->rfL.pcq_rd, iadev->reass_reg+PCQ_RD_PTR);  
	...
	*/  
	...
	pete_writel("drivers/atm/iphase.c:1203", 1, iadev->dma+IPHASE5575_RX_COUNTER);   
out:	return 0;  
out_free_desc:
        free_desc(dev, desc);
        goto out;
...
{  
  ...
  iadev = INPH_IA_DEV(dev);  
  status = pete_readl("drivers/atm/iphase.c:1217", iadev->reass_reg+REASS_INTR_STATUS_REG) & 0xffff;  
  ...
  {  
	...
	*/  
	state = pete_readl("drivers/atm/iphase.c:1227", iadev->reass_reg + STATE_REG) & 0xffff;  
        IF_EVENT(printk("Rx intr status: RX_PKT_RCVD %08x\n", status);) 
	while(!(state & PCQ_EMPTY))  
	{  
             rx_pkt(dev);  
	     state = pete_readl("drivers/atm/iphase.c:1232", iadev->reass_reg + STATE_REG) & 0xffff;  
	}  
        iadev->rxing = 1;
  }  
...
printk("Test logic RUN!!!!\n");
        pete_writew("drivers/atm/iphase.c:1248",  ~(RX_FREEQ_EMPT|RX_EXCP_RCVD),iadev->reass_reg+REASS_MASK_REG);
        iadev->rxing = 1;
     }
...
{  
  ...
  dle_lp = pete_readl("drivers/atm/iphase.c:1290", iadev->dma+IPHASE5575_RX_LIST_ADDR) & (sizeof(struct dle)*DLE_ENTRIES - 1);  
...
INCR_DLE:
  ...
  /* if the interrupts are masked because there were no free desc available,  
		unmask them now. */ 
  if (!iadev->rxing) {
     state = pete_readl("drivers/atm/iphase.c:1364", iadev->reass_reg + STATE_REG) & 0xffff;
     if (!(state & FREEQ_EMPTY)) {
        state = pete_readl("drivers/atm/iphase.c:1366", iadev->reass_reg + REASS_MASK_REG) & 0xffff;
        pete_writel("drivers/atm/iphase.c:1367", state & ~(RX_FREEQ_EMPT |/* RX_EXCP_RCVD |*/ RX_PKT_RCVD),
...
{  
  ...
  //    spin_lock_init(&iadev->rx_lock); 
	...
	pete_writel("drivers/atm/iphase.c:1451", iadev->rx_dle_dma & 0xfffff000,
	       iadev->dma + IPHASE5575_RX_LIST_ADDR);  
	IF_INIT(printk("Tx Dle list addr: 0x%p value: 0x%0x\n",
                      iadev->dma+IPHASE5575_TX_LIST_ADDR,
                      pete_readl("drivers/atm/iphase.c:1455", iadev->dma + IPHASE5575_TX_LIST_ADDR));
	printk("Rx Dle list addr: 0x%p value: 0x%0x\n",
                      iadev->dma+IPHASE5575_RX_LIST_ADDR,
                      pete_readl("drivers/atm/iphase.c:1458", iadev->dma + IPHASE5575_RX_LIST_ADDR));)
  
	pete_writew("drivers/atm/iphase.c:1460", 0xffff, iadev->reass_reg+REASS_MASK_REG);  
	pete_writew("drivers/atm/iphase.c:1461", 0, iadev->reass_reg+MODE_REG);  
	pete_writew("drivers/atm/iphase.c:1462", RESET_REASS, iadev->reass_reg+REASS_COMMAND_REG);  
	...
	*/  
	...
	pete_writew("drivers/atm/iphase.c:1478", RX_DESC_BASE >> 16, iadev->reass_reg+REASS_DESC_BASE);  
	/* Set the buffer size register */  
	pete_writew("drivers/atm/iphase.c:1480", iadev->rx_buf_sz, iadev->reass_reg+BUF_SIZE);  
	...
	pete_writew("drivers/atm/iphase.c:1498", i >> 16,  iadev->reass_reg+REASS_QUEUE_BASE); 
        pete_writew("drivers/atm/iphase.c:1499", i, iadev->reass_reg+FREEQ_ST_ADR);
        pete_writew("drivers/atm/iphase.c:1500", i+iadev->num_rx_desc*sizeof(u_short), 
                                         iadev->reass_reg+FREEQ_ED_ADR);
        pete_writew("drivers/atm/iphase.c:1502", i, iadev->reass_reg+FREEQ_RD_PTR);
        pete_writew("drivers/atm/iphase.c:1503", i+iadev->num_rx_desc*sizeof(u_short), 
                                        iadev->reass_reg+FREEQ_WR_PTR);    
	/* Fill the FREEQ with all the free descriptors. */  
	freeq_st_adr = pete_readw("drivers/atm/iphase.c:1506", iadev->reass_reg+FREEQ_ST_ADR);  
	...
	IF_INIT(printk("freeq_start: 0x%p\n", freeq_start);)
        ...
        pete_writew("drivers/atm/iphase.c:1516", i, iadev->reass_reg+PCQ_ST_ADR);
        pete_writew("drivers/atm/iphase.c:1517", i+iadev->num_vc*sizeof(u_short), iadev->reass_reg+PCQ_ED_ADR);
        pete_writew("drivers/atm/iphase.c:1518", i, iadev->reass_reg+PCQ_RD_PTR);
        pete_writew("drivers/atm/iphase.c:1519", i, iadev->reass_reg+PCQ_WR_PTR);

        /* Exception Queue */
        i = (EXCEPTION_Q * iadev->memSize) & 0xffff;
        pete_writew("drivers/atm/iphase.c:1523", i, iadev->reass_reg+EXCP_Q_ST_ADR);
        pete_writew("drivers/atm/iphase.c:1524", i + NUM_RX_EXCP * sizeof(RX_ERROR_Q), 
                                             iadev->reass_reg+EXCP_Q_ED_ADR);
        pete_writew("drivers/atm/iphase.c:1526", i, iadev->reass_reg+EXCP_Q_RD_PTR);
        pete_writew("drivers/atm/iphase.c:1527", i, iadev->reass_reg+EXCP_Q_WR_PTR); 
 
    	/* Load local copy of FREEQ and PCQ ptrs */
        iadev->rfL.fdq_st = pete_readw("drivers/atm/iphase.c:1530", iadev->reass_reg+FREEQ_ST_ADR) & 0xffff;
       	iadev->rfL.fdq_ed = pete_readw("drivers/atm/iphase.c:1531", iadev->reass_reg+FREEQ_ED_ADR) & 0xffff ;
	iadev->rfL.fdq_rd = pete_readw("drivers/atm/iphase.c:1532", iadev->reass_reg+FREEQ_RD_PTR) & 0xffff;
	iadev->rfL.fdq_wr = pete_readw("drivers/atm/iphase.c:1533", iadev->reass_reg+FREEQ_WR_PTR) & 0xffff;
        iadev->rfL.pcq_st = pete_readw("drivers/atm/iphase.c:1534", iadev->reass_reg+PCQ_ST_ADR) & 0xffff;
	iadev->rfL.pcq_ed = pete_readw("drivers/atm/iphase.c:1535", iadev->reass_reg+PCQ_ED_ADR) & 0xffff;
	iadev->rfL.pcq_rd = pete_readw("drivers/atm/iphase.c:1536", iadev->reass_reg+PCQ_RD_PTR) & 0xffff;
	iadev->rfL.pcq_wr = pete_readw("drivers/atm/iphase.c:1537", iadev->reass_reg+PCQ_WR_PTR) & 0xffff;
	...
	/* VP Table */  
	/* pete_writew("drivers/atm/iphase.c:1544", 0x0b80, iadev->reass_reg+VP_LKUP_BASE); */  
	...
	*/  
	...
	pete_writew("drivers/atm/iphase.c:1552", (i >> 3), iadev->reass_reg+REASS_TABLE_BASE);   
	...
	for(i=0; i < j; i++)  
       ...
       pete_writew("drivers/atm/iphase.c:1565", ((i>>3) & 0xfff8) | vcsize_sel, iadev->reass_reg+VC_LKUP_BASE);
	...
	{  
		...
		*vc_table = (i << 6) | 15;	/* for invalid VCI */  
        ...
        pete_writew("drivers/atm/iphase.c:1580", i >> 3, iadev->reass_reg+ABR_LKUP_BASE);
	...
	pete_writew("drivers/atm/iphase.c:1595", 0xff00, iadev->reass_reg+VP_FILTER);  
        pete_writew("drivers/atm/iphase.c:1596", 0, iadev->reass_reg+XTRA_RM_OFFSET);
	pete_writew("drivers/atm/iphase.c:1597", 0x1,  iadev->reass_reg+PROTOCOL_ID);
 	...
 	*/  
        pete_writew("drivers/atm/iphase.c:1603", 0xF6F8, iadev->reass_reg+PKT_TM_CNT );

        i = (j >> 6) & 0xFF;
        j += 2 * (j - 1);
        i |= ((j << 2) & 0xFF00);
        pete_writew("drivers/atm/iphase.c:1608", i, iadev->reass_reg+TMOUT_RANGE);
	...
	pete_readw("drivers/atm/iphase.c:1615", iadev->reass_reg+REASS_INTR_STATUS_REG);   
  
	/* Mask Register - clear it */  
	pete_writew("drivers/atm/iphase.c:1618", ~(RX_FREEQ_EMPT|RX_PKT_RCVD), iadev->reass_reg+REASS_MASK_REG);  
	...
	if (!iadev->rx_open) {
        ...
        iadev->rx_pkt_cnt = 0;
	/* Mode Register */  
	pete_writew("drivers/atm/iphase.c:1633", R_ONLINE, iadev->reass_reg+MODE_REG);  
	return 0;  
...
{  
	...
	status = pete_readl("drivers/atm/iphase.c:1667", iadev->seg_reg+SEG_INTR_STATUS_REG);  
        if (status & TRANSMIT_DONE){
           ...
           pete_writew("drivers/atm/iphase.c:1674", TRANSMIT_DONE, iadev->seg_reg+SEG_INTR_STATUS_REG);
...
static void tx_dle_intr(struct atm_dev *dev)
{
        ...
        dle_lp = pete_readl("drivers/atm/iphase.c:1697", iadev->dma+IPHASE5575_TX_LIST_ADDR) & 
...
{  
	...
	IF_INIT(printk("Tx MASK REG: 0x%0x\n", 
                                pete_readw("drivers/atm/iphase.c:1920", iadev->seg_reg+SEG_MASK_REG));)  
	...
	pete_writel("drivers/atm/iphase.c:1935", iadev->tx_dle_dma & 0xfffff000,
	       iadev->dma + IPHASE5575_TX_LIST_ADDR);  
	pete_writew("drivers/atm/iphase.c:1937", 0xffff, iadev->seg_reg+SEG_MASK_REG);  
	pete_writew("drivers/atm/iphase.c:1938", 0, iadev->seg_reg+MODE_REG_0);  
	pete_writew("drivers/atm/iphase.c:1939", RESET_SEG, iadev->seg_reg+SEG_COMMAND_REG);  
	...
	*/  
	...
	pete_writew("drivers/atm/iphase.c:1964", TX_DESC_BASE, iadev->seg_reg+SEG_DESC_BASE);  
	...
	pete_writew("drivers/atm/iphase.c:2012", i >> 16, iadev->seg_reg+SEG_QUEUE_BASE);  
  
	/* Transmit Complete Queue */  
	pete_writew("drivers/atm/iphase.c:2015", i, iadev->seg_reg+TCQ_ST_ADR);  
	pete_writew("drivers/atm/iphase.c:2016", i, iadev->seg_reg+TCQ_RD_PTR);  
	pete_writew("drivers/atm/iphase.c:2017", i+iadev->num_tx_desc*sizeof(u_short),iadev->seg_reg+TCQ_WR_PTR); 
	iadev->host_tcq_wr = i + iadev->num_tx_desc*sizeof(u_short);
        pete_writew("drivers/atm/iphase.c:2019", i+2 * iadev->num_tx_desc * sizeof(u_short), 
                                              iadev->seg_reg+TCQ_ED_ADR); 
	/* Fill the TCQ with all the free descriptors. */  
	tcq_st_adr = pete_readw("drivers/atm/iphase.c:2022", iadev->seg_reg+TCQ_ST_ADR);  
	...
	pete_writew("drivers/atm/iphase.c:2032", i, iadev->seg_reg+PRQ_ST_ADR);  
	pete_writew("drivers/atm/iphase.c:2033", i+2 * iadev->num_tx_desc * sizeof(u_short), 
                                              iadev->seg_reg+PRQ_ED_ADR);
	pete_writew("drivers/atm/iphase.c:2035", i, iadev->seg_reg+PRQ_RD_PTR);  
	pete_writew("drivers/atm/iphase.c:2036", i, iadev->seg_reg+PRQ_WR_PTR);  
	 
        /* Load local copy of PRQ and TCQ ptrs */
        iadev->ffL.prq_st = pete_readw("drivers/atm/iphase.c:2039", iadev->seg_reg+PRQ_ST_ADR) & 0xffff;
	iadev->ffL.prq_ed = pete_readw("drivers/atm/iphase.c:2040", iadev->seg_reg+PRQ_ED_ADR) & 0xffff;
 	iadev->ffL.prq_wr = pete_readw("drivers/atm/iphase.c:2041", iadev->seg_reg+PRQ_WR_PTR) & 0xffff;

	iadev->ffL.tcq_st = pete_readw("drivers/atm/iphase.c:2043", iadev->seg_reg+TCQ_ST_ADR) & 0xffff;
	iadev->ffL.tcq_ed = pete_readw("drivers/atm/iphase.c:2044", iadev->seg_reg+TCQ_ED_ADR) & 0xffff;
	iadev->ffL.tcq_rd = pete_readw("drivers/atm/iphase.c:2045", iadev->seg_reg+TCQ_RD_PTR) & 0xffff;

	/* Just for safety initializing the queue to have desc 1 always */  
	/* Fill the PRQ with all the free descriptors. */  
	prq_st_adr = pete_readw("drivers/atm/iphase.c:2049", iadev->seg_reg+PRQ_ST_ADR);  
	...
	/* CBR Table */  
        ...
        pete_writew("drivers/atm/iphase.c:2059", 0,iadev->seg_reg+CBR_PTR_BASE);
#else /* Charlie's logic is wrong ? */
        tmp16 = (iadev->seg_ram+CBR_SCHED_TABLE*iadev->memSize)>>17;
        IF_INIT(printk("cbr_ptr_base = 0x%x ", tmp16);)
        pete_writew("drivers/atm/iphase.c:2063", tmp16,iadev->seg_reg+CBR_PTR_BASE);
#endif

        IF_INIT(printk("value in register = 0x%x\n",
                                   pete_readw("drivers/atm/iphase.c:2067", iadev->seg_reg+CBR_PTR_BASE));)
        tmp16 = (CBR_SCHED_TABLE*iadev->memSize) >> 1;
        pete_writew("drivers/atm/iphase.c:2069", tmp16, iadev->seg_reg+CBR_TAB_BEG);
        IF_INIT(printk("cbr_tab_beg = 0x%x in reg = 0x%x \n", tmp16,
                                        pete_readw("drivers/atm/iphase.c:2071", iadev->seg_reg+CBR_TAB_BEG));)
        pete_writew("drivers/atm/iphase.c:2072", tmp16, iadev->seg_reg+CBR_TAB_END+1); // CBR_PTR;
        tmp16 = (CBR_SCHED_TABLE*iadev->memSize + iadev->num_vc*6 - 2) >> 1;
        pete_writew("drivers/atm/iphase.c:2074", tmp16, iadev->seg_reg+CBR_TAB_END);
        IF_INIT(printk("iadev->seg_reg = 0x%p CBR_PTR_BASE = 0x%x\n",
               iadev->seg_reg, pete_readw("drivers/atm/iphase.c:2076", iadev->seg_reg+CBR_PTR_BASE));)
        IF_INIT(printk("CBR_TAB_BEG = 0x%x, CBR_TAB_END = 0x%x, CBR_PTR = 0x%x\n",
          pete_readw("drivers/atm/iphase.c:2078", iadev->seg_reg+CBR_TAB_BEG), pete_readw("drivers/atm/iphase.c:2078", iadev->seg_reg+CBR_TAB_END),
          pete_readw("drivers/atm/iphase.c:2079", iadev->seg_reg+CBR_TAB_END+1));)
	...
	*/  
        ...
        pete_writew("drivers/atm/iphase.c:2106", vcsize_sel | ((i >> 8) & 0xfff8),iadev->seg_reg+VCT_BASE);
        i =  EXT_VC_TABLE * iadev->memSize;
        pete_writew("drivers/atm/iphase.c:2108", (i >> 8) & 0xfffe, iadev->seg_reg+VCTE_BASE);
        i = UBR_SCHED_TABLE * iadev->memSize;
        pete_writew("drivers/atm/iphase.c:2110", (i & 0xffff) >> 11,  iadev->seg_reg+UBR_SBPTR_BASE);
        i = UBR_WAIT_Q * iadev->memSize; 
        pete_writew("drivers/atm/iphase.c:2112", (i >> 7) & 0xffff,  iadev->seg_reg+UBRWQ_BASE);
	...
	*/  
        ...
        pete_writew("drivers/atm/iphase.c:2124", (i >> 11) & 0xffff, iadev->seg_reg+ABR_SBPTR_BASE);
        i = ABR_WAIT_Q * iadev->memSize;
        pete_writew("drivers/atm/iphase.c:2126", (i >> 7) & 0xffff, iadev->seg_reg+ABRWQ_BASE);
	...
	/* Max Rate Register */  
        if (iadev->phy_type & FE_25MBIT_PHY) {
	   pete_writew("drivers/atm/iphase.c:2158", RATE25, iadev->seg_reg+MAXRATE);  
	   pete_writew("drivers/atm/iphase.c:2159", (UBR_EN | (0x23 << 2)), iadev->seg_reg+STPARMS);  
        }
        else {
	   pete_writew("drivers/atm/iphase.c:2162", cellrate_to_float(iadev->LineRate),iadev->seg_reg+MAXRATE);
	   pete_writew("drivers/atm/iphase.c:2163", (UBR_EN | ABR_EN | (0x23 << 2)), iadev->seg_reg+STPARMS);  
        }
	/* Set Idle Header Reigisters to be sure */  
	pete_writew("drivers/atm/iphase.c:2166", 0, iadev->seg_reg+IDLEHEADHI);  
	pete_writew("drivers/atm/iphase.c:2167", 0, iadev->seg_reg+IDLEHEADLO);  
  
	/* Program ABR UBR Priority Register  as  PRI_ABR_UBR_EQUAL */
        pete_writew("drivers/atm/iphase.c:2170", 0xaa00, iadev->seg_reg+ABRUBR_ARB); 
	...
	pete_writew("drivers/atm/iphase.c:2179", RM_TYPE_4_0, iadev->seg_reg+RM_TYPE);  
        skb_queue_head_init (&iadev->tx_backlog);
  
	/* Mode Register 1 */  
	pete_writew("drivers/atm/iphase.c:2183", MODE_REG_1_VAL, iadev->seg_reg+MODE_REG_1);  
  
	/* Mode Register 0 */  
	pete_writew("drivers/atm/iphase.c:2186", T_ONLINE, iadev->seg_reg+MODE_REG_0);  
  
	/* Interrupt Status Register - read to clear */  
	pete_readw("drivers/atm/iphase.c:2189", iadev->seg_reg+SEG_INTR_STATUS_REG);  
  
	/* Interrupt Mask Reg- don't mask TCQ_NOT_EMPTY interrupt generation */  
        pete_writew("drivers/atm/iphase.c:2192", ~(TRANSMIT_DONE | TCQ_NOT_EMPTY), iadev->seg_reg+SEG_MASK_REG);
        pete_writew("drivers/atm/iphase.c:2193", TRANSMIT_DONE, iadev->seg_reg+SEG_INTR_STATUS_REG);  
        iadev->tx_pkt_cnt = 0;
...
{  
   ...
   dev = dev_id;  
   iadev = INPH_IA_DEV(dev);  
   while( (status = pete_readl("drivers/atm/iphase.c:2232", iadev->reg+IPHASE5575_BUS_STATUS_REG) & 0x7f))  
   { 
	handled = 1;
	...
	{  
	   ...
	   pete_writel("drivers/atm/iphase.c:2245", STAT_DLERINT, iadev->reg + IPHASE5575_BUS_STATUS_REG);
	   rx_dle_intr(dev);  
	}  
	...
	{  
	   pete_writel("drivers/atm/iphase.c:2256", STAT_DLETINT, iadev->reg + IPHASE5575_BUS_STATUS_REG);
	   tx_dle_intr(dev);  
	}  
...
{  
	...
	iadev = INPH_IA_DEV(dev);  
	mac1 = cpu_to_be32(le32_to_cpu(pete_readl("drivers/atm/iphase.c:2279",   
				iadev->reg+IPHASE5575_MAC1)));  
	mac2 = cpu_to_be16(le16_to_cpu(pete_readl("drivers/atm/iphase.c:2281", iadev->reg+IPHASE5575_MAC2)));  
...
{  
	...
	pete_writel("drivers/atm/iphase.c:2303", 0, iadev->reg+IPHASE5575_EXT_RESET);  
	for (i = 0; i < 64; i++) {
 	...
 	unsigned long flags;
  	static u_char blinking[8] = {0, 0, 0, 0, 0, 0, 0, 0};
        ...
        for (i = 0; i < iadev_count; i++) {
           if (ia_dev[i]) {
	      ctrl_reg = pete_readl("drivers/atm/iphase.c:2446", ia_dev[i]->reg+IPHASE5575_BUS_CONTROL_REG);
	      if (blinking[i] == 0) {
		 blinking[i]++;
                 ctrl_reg &= (~CTRL_LED);
                 pete_writel("drivers/atm/iphase.c:2450", ctrl_reg, ia_dev[i]->reg+IPHASE5575_BUS_CONTROL_REG);
                 ia_update_stats(ia_dev[i]);
              }
              else {
		 blinking[i] = 0;
		 ctrl_reg |= CTRL_LED;
                 pete_writel("drivers/atm/iphase.c:2456", ctrl_reg, ia_dev[i]->reg+IPHASE5575_BUS_CONTROL_REG);
...
static void ia_phy_put(struct atm_dev *dev, unsigned char value,   
	unsigned long addr)  
{  
	pete_writel("drivers/atm/iphase.c:2472", value, INPH_IA_DEV(dev)->phy+addr);  
}  
  
static unsigned char ia_phy_get(struct atm_dev *dev, unsigned long addr)  
{  
	return pete_readl("drivers/atm/iphase.c:2477", INPH_IA_DEV(dev)->phy+addr);  
}  
...
{  
	...
	IF_INIT(printk("Bus ctrl reg: %08x\n", 
                            pete_readl("drivers/atm/iphase.c:2538", iadev->reg+IPHASE5575_BUS_CONTROL_REG));)  
	ctrl_reg = pete_readl("drivers/atm/iphase.c:2539", iadev->reg+IPHASE5575_BUS_CONTROL_REG);  
	ctrl_reg = (ctrl_reg & (CTRL_LED | CTRL_FE_RST))  
       ...
       pete_writel("drivers/atm/iphase.c:2555", ctrl_reg, iadev->reg+IPHASE5575_BUS_CONTROL_REG);   
  
	IF_INIT(printk("Bus ctrl reg after initializing: %08x\n", 
                           pete_readl("drivers/atm/iphase.c:2558", iadev->reg+IPHASE5575_BUS_CONTROL_REG));  
	   printk("Bus status reg after init: %08x\n", 
                            pete_readl("drivers/atm/iphase.c:2560", iadev->reg+IPHASE5575_BUS_STATUS_REG));)  
	...
	ctrl_reg = pete_readl("drivers/atm/iphase.c:2570", iadev->reg+IPHASE5575_BUS_CONTROL_REG);  
       	pete_writel("drivers/atm/iphase.c:2571", ctrl_reg | CTRL_FE_RST, iadev->reg+IPHASE5575_BUS_CONTROL_REG);   
	IF_INIT(printk("Bus ctrl reg after initializing: %08x\n", 
                               pete_readl("drivers/atm/iphase.c:2573", iadev->reg+IPHASE5575_BUS_CONTROL_REG));)  
	...
	switch (ia_cmds.sub_cmd) {
	     ...
	     if (!capable(CAP_NET_ADMIN)) return -EPERM;
             ...
             for(i=0; i<0x80; i+=2, tmps++)
                if(put_user((u16)(pete_readl("drivers/atm/iphase.c:2781", iadev->seg_reg+i) & 0xffff), tmps)) return -EFAULT;
             ia_cmds.status = 0;
             ia_cmds.len = 0x80;
             break;
	     ...
	     if (!capable(CAP_NET_ADMIN)) return -EPERM;
             tmps = (u16 __user *)ia_cmds.buf;
             for(i=0; i<0x80; i+=2, tmps++)
                if(put_user((u16)(pete_readl("drivers/atm/iphase.c:2789", iadev->reass_reg+i) & 0xffff), tmps)) return -EFAULT;
             ia_cmds.status = 0;
             ia_cmds.len = 0x80;
             break;
          case MEMDUMP_FFL:
	     ...
	     rfL = &regs_local->rfredn;
 	     ...
 	     for (i=0; i<(sizeof (rfredn_t))/4; i++)
                ((u_int *)rfL)[i] = pete_readl("drivers/atm/iphase.c:2806", iadev->reass_reg + i) & 0xffff;
             	/* Copy real ffred registers into the local copy */
	     for (i=0; i<(sizeof (ffredn_t))/4; i++)
                ((u_int *)ffL)[i] = pete_readl("drivers/atm/iphase.c:2809", iadev->seg_reg + i) & 0xffff;
...
static int ia_pkt_tx (struct atm_vcc *vcc, struct sk_buff *skb) {
	...
	*(u16*)(iadev->seg_ram+iadev->ffL.prq_wr) = desc; 
	...
	iadev->tx_dle_q.write = wr_ptr;  
        ...
        iadev->tx_pkt_cnt++;
	/* Increment transaction counter */  
	pete_writel("drivers/atm/iphase.c:3045", 2, iadev->dma+IPHASE5575_TX_COUNTER);  
