...
static void push_rxbufs(ns_dev * card, struct sk_buff *skb)
{
	...
	stat = pete_readl("drivers/atm/nicstar.c:965", card->membase + STAT);
	...
	if (addr2) {
		...
		spin_lock_irqsave(&card->res_lock, flags);
		while (CMD_BUSY(card)) ;
		pete_writel("drivers/atm/nicstar.c:1029", addr2, card->membase + DR3);
		pete_writel("drivers/atm/nicstar.c:1030", id2, card->membase + DR2);
		pete_writel("drivers/atm/nicstar.c:1031", addr1, card->membase + DR1);
		pete_writel("drivers/atm/nicstar.c:1032", id1, card->membase + DR0);
		pete_writel("drivers/atm/nicstar.c:1033", NS_CMD_WRITE_FREEBUFQ | NS_PRV_BUFTYPE(skb),
		       card->membase + CMD);
		spin_unlock_irqrestore(&card->res_lock, flags);
	...
	if (!card->efbie && card->sbfqc >= card->sbnr.min &&
		...
		card->efbie = 1;
		pete_writel("drivers/atm/nicstar.c:1046", (pete_readl("drivers/atm/nicstar.c:1046", card->membase + CFG) | NS_CFG_EFBIE),
		       card->membase + CFG);
	}
...
static irqreturn_t ns_irq_handler(int irq, void *dev_id)
{
	...
	stat_r = pete_readl("drivers/atm/nicstar.c:1069", card->membase + STAT);

	/* Transmit Status Indicator has been written to T. S. Queue */
	if (stat_r & NS_STAT_TSIF) {
		TXPRINTK("nicstar%d: TSI interrupt\n", card->index);
		process_tsq(card);
		pete_writel("drivers/atm/nicstar.c:1075", NS_STAT_TSIF, card->membase + STAT);
	}

	/* Incomplete CS-PDU has been transmitted */
	if (stat_r & NS_STAT_TXICP) {
		pete_writel("drivers/atm/nicstar.c:1080", NS_STAT_TXICP, card->membase + STAT);
	...
	if (stat_r & NS_STAT_TSQF) {
		pete_writel("drivers/atm/nicstar.c:1087", NS_STAT_TSQF, card->membase + STAT);
	...
	if (stat_r & NS_STAT_TMROF) {
		pete_writel("drivers/atm/nicstar.c:1094", NS_STAT_TMROF, card->membase + STAT);
		PRINTK("nicstar%d: Timer overflow.\n", card->index);
	}

	/* PHY device interrupt signal active */
	if (stat_r & NS_STAT_PHYI) {
		pete_writel("drivers/atm/nicstar.c:1100", NS_STAT_PHYI, card->membase + STAT);
	...
	if (stat_r & NS_STAT_SFBQF) {
		pete_writel("drivers/atm/nicstar.c:1109", NS_STAT_SFBQF, card->membase + STAT);
	...
	if (stat_r & NS_STAT_LFBQF) {
		pete_writel("drivers/atm/nicstar.c:1116", NS_STAT_LFBQF, card->membase + STAT);
	...
	if (stat_r & NS_STAT_RSQF) {
		pete_writel("drivers/atm/nicstar.c:1123", NS_STAT_RSQF, card->membase + STAT);
	...
	if (stat_r & NS_STAT_EOPDU) {
		...
		process_rsq(card);
		pete_writel("drivers/atm/nicstar.c:1132", NS_STAT_EOPDU, card->membase + STAT);
	}

	/* Raw cell received */
	if (stat_r & NS_STAT_RAWCF) {
		pete_writel("drivers/atm/nicstar.c:1137", NS_STAT_RAWCF, card->membase + STAT);
#ifndef RCQ_SUPPORT
		...
		while (pete_readl("drivers/atm/nicstar.c:1145", card->membase + RAWCT) != card->rawch) {
	...
	if (stat_r & NS_STAT_SFBQE) {
		...
		pete_writel("drivers/atm/nicstar.c:1169", NS_STAT_SFBQE, card->membase + STAT);
		printk("nicstar%d: Small free buffer queue empty.\n",
		       card->index);
		for (i = 0; i < card->sbnr.min; i++) {
			sb = dev_alloc_skb(NS_SMSKBSIZE);
			if (sb == NULL) {
				pete_writel("drivers/atm/nicstar.c:1175", pete_readl("drivers/atm/nicstar.c:1175", card->membase + CFG) &
	...
	if (stat_r & NS_STAT_LFBQE) {
		...
		pete_writel("drivers/atm/nicstar.c:1194", NS_STAT_LFBQE, card->membase + STAT);
		printk("nicstar%d: Large free buffer queue empty.\n",
		       card->index);
		for (i = 0; i < card->lbnr.min; i++) {
			lb = dev_alloc_skb(NS_LGSKBSIZE);
			if (lb == NULL) {
				pete_writel("drivers/atm/nicstar.c:1200", pete_readl("drivers/atm/nicstar.c:1200", card->membase + CFG) &
				       ~NS_CFG_EFBIE, card->membase + CFG);
				card->efbie = 0;
				break;
			}
	...
	if (stat_r & NS_STAT_RSQAF) {
		pete_writel("drivers/atm/nicstar.c:1216", NS_STAT_RSQAF, card->membase + STAT);
...
static void ns_close(struct atm_vcc *vcc)
{
	...
	if (vcc->qos.rxtp.traffic_class != ATM_NONE) {
		...
		spin_lock_irqsave(&card->res_lock, flags);
		while (CMD_BUSY(card)) ;
		pete_writel("drivers/atm/nicstar.c:1431", NS_CMD_CLOSE_CONNECTION | addr << 2,
		       card->membase + CMD);
		spin_unlock_irqrestore(&card->res_lock, flags);
		...
		if (vc->rx_iov != NULL) {
			...
			stat = pete_readl("drivers/atm/nicstar.c:1440", card->membase + STAT);
			card->sbfqc = ns_stat_sfbqc_get(stat);
			card->lbfqc = ns_stat_lfbqc_get(stat);
	...
	{
		u32 stat, cfg;
		stat = pete_readl("drivers/atm/nicstar.c:1549", card->membase + STAT);
		cfg = pete_readl("drivers/atm/nicstar.c:1550", card->membase + CFG);
		printk("STAT = 0x%08X  CFG = 0x%08X  \n", stat, cfg);
		printk
		    ("TSQ: base = 0x%p  next = 0x%p  last = 0x%p  TSQT = 0x%08X \n",
		     card->tsq.base, card->tsq.next,
		     card->tsq.last, pete_readl("drivers/atm/nicstar.c:1555", card->membase + TSQT));
		printk
		    ("RSQ: base = 0x%p  next = 0x%p  last = 0x%p  RSQT = 0x%08X \n",
		     card->rsq.base, card->rsq.next,
		     card->rsq.last, pete_readl("drivers/atm/nicstar.c:1559", card->membase + RSQT));
...
static void process_tsq(ns_dev * card)
{
	...
	if (serviced_entries)
		pete_writel("drivers/atm/nicstar.c:1910", PTR_DIFF(previous, card->tsq.base),
		       card->membase + TSQH);
}
...
static void process_rsq(ns_dev * card)
{
	...
	pete_writel("drivers/atm/nicstar.c:1971", PTR_DIFF(previous, card->rsq.base), card->membase + RSQH);
}
...
static void dequeue_rx(ns_dev * card, ns_rsqe * rsqe)
{
	...
	stat = pete_readl("drivers/atm/nicstar.c:1987", card->membase + STAT);
	card->sbfqc = ns_stat_sfbqc_get(stat);
	card->lbfqc = ns_stat_lfbqc_get(stat);
...
static int ns_proc_read(struct atm_dev *dev, loff_t * pos, char *page)
{
	...
	stat = pete_readl("drivers/atm/nicstar.c:2413", card->membase + STAT);
	if (!left--)
	...
	if (card->max_pcr == ATM_25_PCR && !left--) {
		...
		for (i = 0; i < 4; i++) {
			while (CMD_BUSY(card)) ;
			pete_writel("drivers/atm/nicstar.c:2449", NS_CMD_READ_UTILITY | 0x00000200 | i,
			       card->membase + CMD);
			while (CMD_BUSY(card)) ;
			phy_regs[i] = pete_readl("drivers/atm/nicstar.c:2452", card->membase + DR0) & 0x000000FF;
		}
...
static int ns_ioctl(struct atm_dev *dev, unsigned int cmd, void __user * arg)
{
	...
	case NS_GETPSTAT:
		...
		case NS_BUFTYPE_SMALL:
			pl.count =
			    ns_stat_sfbqc_get(pete_readl("drivers/atm/nicstar.c:2490", card->membase + STAT));
		...
		case NS_BUFTYPE_LARGE:
			pl.count =
			    ns_stat_lfbqc_get(pete_readl("drivers/atm/nicstar.c:2498", card->membase + STAT));
...
static void ns_poll(struct timer_list *unused)
{
	...
	for (i = 0; i < num_cards; i++) {
		...
		stat_w = 0;
		stat_r = pete_readl("drivers/atm/nicstar.c:2709", card->membase + STAT);
		if (stat_r & NS_STAT_TSIF)
			stat_w |= NS_STAT_TSIF;
		if (stat_r & NS_STAT_EOPDU)
		...
		pete_writel("drivers/atm/nicstar.c:2718", stat_w, card->membase + STAT);
		spin_unlock_irqrestore(&card->int_lock, flags);
	}
		       ...
		       unsigned long addr)
{
	...
	spin_lock_irqsave(&card->res_lock, flags);
	while (CMD_BUSY(card)) ;
	pete_writel("drivers/atm/nicstar.c:2734", (u32) value, card->membase + DR0);
	pete_writel("drivers/atm/nicstar.c:2735", NS_CMD_WRITE_UTILITY | 0x00000200 | (addr & 0x000000FF),
	       card->membase + CMD);
	spin_unlock_irqrestore(&card->res_lock, flags);
}
...
static unsigned char ns_phy_get(struct atm_dev *dev, unsigned long addr)
{
	...
	spin_lock_irqsave(&card->res_lock, flags);
	while (CMD_BUSY(card)) ;
	pete_writel("drivers/atm/nicstar.c:2749", NS_CMD_READ_UTILITY | 0x00000200 | (addr & 0x000000FF),
	       card->membase + CMD);
	while (CMD_BUSY(card)) ;
	data = pete_readl("drivers/atm/nicstar.c:2752", card->membase + DR0) & 0x000000FF;
	spin_unlock_irqrestore(&card->res_lock, flags);
