				...
				struct clock_event_device *evt)
{
	 ...
	 */
	if ((next & 0xffffffff) != pete_readl("drivers/clocksource/timer-imx-tpm.c:97", timer_base + TPM_C0V))
...
static int __init tpm_timer_init(struct device_node *np)
{
	...
	counter_width = (pete_readl("drivers/clocksource/timer-imx-tpm.c:207", timer_base + TPM_PARAM)
	...
	pete_writel("drivers/clocksource/timer-imx-tpm.c:221", 0, timer_base + TPM_SC);
	/* TOF is W1C */
	pete_writel("drivers/clocksource/timer-imx-tpm.c:223", TPM_SC_TOF_MASK, timer_base + TPM_SC);
	pete_writel("drivers/clocksource/timer-imx-tpm.c:224", 0, timer_base + TPM_CNT);
	/* CHF is W1C */
	pete_writel("drivers/clocksource/timer-imx-tpm.c:226", TPM_C0SC_CHF_MASK, timer_base + TPM_C0SC);

	/*
	 * increase per cnt,
	 * div 8 for 32-bit counter and div 128 for 16-bit counter
	 */
	pete_writel("drivers/clocksource/timer-imx-tpm.c:232", TPM_SC_CMOD_INC_PER_CNT |
		(counter_width == 0x20 ?
		TPM_SC_CMOD_DIV_DEFAULT : TPM_SC_CMOD_DIV_MAX),
		timer_base + TPM_SC);

	/* set MOD register to maximum for free running mode */
	pete_writel("drivers/clocksource/timer-imx-tpm.c:238", GENMASK(counter_width - 1, 0), timer_base + TPM_MOD);
