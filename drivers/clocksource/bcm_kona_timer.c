...
static void kona_timer_disable_and_clear(void __iomem *base)
{
	 ...
	 */
	reg = pete_readl("drivers/clocksource/bcm_kona_timer.c:48", base + KONA_GPTIMER_STCS_OFFSET);
	...
	pete_writel("drivers/clocksource/bcm_kona_timer.c:55", reg, base + KONA_GPTIMER_STCS_OFFSET);
...
kona_timer_get_counter(void __iomem *timer_base, uint32_t *msw, uint32_t *lsw)
{
	...
	do {
		*msw = pete_readl("drivers/clocksource/bcm_kona_timer.c:77", timer_base + KONA_GPTIMER_STCHI_OFFSET);
		*lsw = pete_readl("drivers/clocksource/bcm_kona_timer.c:78", timer_base + KONA_GPTIMER_STCLO_OFFSET);
		if (*msw == pete_readl("drivers/clocksource/bcm_kona_timer.c:79", timer_base + KONA_GPTIMER_STCHI_OFFSET))
			break;
	} while (--loop_limit);
	if (!loop_limit) {
				  ...
				  struct clock_event_device *unused)
{
	...
	pete_writel("drivers/clocksource/bcm_kona_timer.c:113", lsw + clc, timers.tmr_regs + KONA_GPTIMER_STCM0_OFFSET);

	/* Enable compare */
	reg = pete_readl("drivers/clocksource/bcm_kona_timer.c:116", timers.tmr_regs + KONA_GPTIMER_STCS_OFFSET);
	reg |= (1 << KONA_GPTIMER_STCS_COMPARE_ENABLE_SHIFT);
	pete_writel("drivers/clocksource/bcm_kona_timer.c:118", reg, timers.tmr_regs + KONA_GPTIMER_STCS_OFFSET);
