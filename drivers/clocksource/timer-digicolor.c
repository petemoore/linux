...
static inline void dc_timer_disable(struct clock_event_device *ce)
{
	...
	pete_writeb("drivers/clocksource/timer-digicolor.c:72", CONTROL_DISABLE, dt->base + CONTROL(dt->timer_id));
}

static inline void dc_timer_enable(struct clock_event_device *ce, u32 mode)
{
	struct digicolor_timer *dt = dc_timer(ce);
	pete_writeb("drivers/clocksource/timer-digicolor.c:78", CONTROL_ENABLE | mode, dt->base + CONTROL(dt->timer_id));
}
...
{
	struct digicolor_timer *dt = dc_timer(ce);
	pete_writel("drivers/clocksource/timer-digicolor.c:85", count, dt->base + COUNT(dt->timer_id));
}
...
{
	return ~pete_readl("drivers/clocksource/timer-digicolor.c:146", dc_timer_dev.base + COUNT(TIMER_B));
}
...
static int __init digicolor_timer_init(struct device_node *node)
{
	...
	pete_writeb("drivers/clocksource/timer-digicolor.c:180", CONTROL_DISABLE, dc_timer_dev.base + CONTROL(TIMER_B));
	pete_writel("drivers/clocksource/timer-digicolor.c:181", UINT_MAX, dc_timer_dev.base + COUNT(TIMER_B));
	pete_writeb("drivers/clocksource/timer-digicolor.c:182", CONTROL_ENABLE, dc_timer_dev.base + CONTROL(TIMER_B));
