			...
			struct sabi_data *out)
{
	...
	int ret = 0;
	u16 port = pete_readw("drivers/platform/x86/samsung-laptop.c:390", samsung->sabi + config->header_offsets.port);
	...
	outb(pete_readb("drivers/platform/x86/samsung-laptop.c:405", samsung->sabi + config->header_offsets.en_mem), port);

	/* write out the command */
	pete_writew("drivers/platform/x86/samsung-laptop.c:408", config->main_function, samsung->sabi_iface + SABI_IFACE_MAIN);
	pete_writew("drivers/platform/x86/samsung-laptop.c:409", command, samsung->sabi_iface + SABI_IFACE_SUB);
	pete_writeb("drivers/platform/x86/samsung-laptop.c:410", 0, samsung->sabi_iface + SABI_IFACE_COMPLETE);
	if (in) {
		pete_writel("drivers/platform/x86/samsung-laptop.c:412", in->d0, samsung->sabi_iface + SABI_IFACE_DATA);
		pete_writel("drivers/platform/x86/samsung-laptop.c:413", in->d1, samsung->sabi_iface + SABI_IFACE_DATA + 4);
		pete_writew("drivers/platform/x86/samsung-laptop.c:414", in->d2, samsung->sabi_iface + SABI_IFACE_DATA + 8);
		pete_writeb("drivers/platform/x86/samsung-laptop.c:415", in->d3, samsung->sabi_iface + SABI_IFACE_DATA + 10);
	}
	outb(pete_readb("drivers/platform/x86/samsung-laptop.c:417", samsung->sabi + config->header_offsets.iface_func), port);

	/* write protect memory to make it safe */
	outb(pete_readb("drivers/platform/x86/samsung-laptop.c:420", samsung->sabi + config->header_offsets.re_mem), port);

	/* see if the command actually succeeded */
	complete = pete_readb("drivers/platform/x86/samsung-laptop.c:423", samsung->sabi_iface + SABI_IFACE_COMPLETE);
	iface_data = pete_readb("drivers/platform/x86/samsung-laptop.c:424", samsung->sabi_iface + SABI_IFACE_DATA);
	...
	if (out) {
		out->d0 = pete_readl("drivers/platform/x86/samsung-laptop.c:441", samsung->sabi_iface + SABI_IFACE_DATA);
		out->d1 = pete_readl("drivers/platform/x86/samsung-laptop.c:442", samsung->sabi_iface + SABI_IFACE_DATA + 4);
		out->d2 = pete_readw("drivers/platform/x86/samsung-laptop.c:443", samsung->sabi_iface + SABI_IFACE_DATA + 2);
		out->d3 = pete_readb("drivers/platform/x86/samsung-laptop.c:444", samsung->sabi_iface + SABI_IFACE_DATA + 1);
	}
...
static int find_signature(void __iomem *memcheck, const char *testStr)
{
	...
	for (loca = 0; loca < 0xffff; loca++) {
		char temp = pete_readb("drivers/platform/x86/samsung-laptop.c:918", memcheck + loca);
				      ...
				      unsigned int ifaceP)
{
	...
	printk(KERN_DEBUG " SMI Port Number = 0x%04x\n",
	       pete_readw("drivers/platform/x86/samsung-laptop.c:1325", samsung->sabi + config->header_offsets.port));
	printk(KERN_DEBUG " SMI Interface Function = 0x%02x\n",
	       pete_readb("drivers/platform/x86/samsung-laptop.c:1327", samsung->sabi + config->header_offsets.iface_func));
	printk(KERN_DEBUG " SMI enable memory buffer = 0x%02x\n",
	       pete_readb("drivers/platform/x86/samsung-laptop.c:1329", samsung->sabi + config->header_offsets.en_mem));
	printk(KERN_DEBUG " SMI restore memory buffer = 0x%02x\n",
	       pete_readb("drivers/platform/x86/samsung-laptop.c:1331", samsung->sabi + config->header_offsets.re_mem));
	printk(KERN_DEBUG " SABI data offset = 0x%04x\n",
	       pete_readw("drivers/platform/x86/samsung-laptop.c:1333", samsung->sabi + config->header_offsets.data_offset));
	printk(KERN_DEBUG " SABI data segment = 0x%04x\n",
	       pete_readw("drivers/platform/x86/samsung-laptop.c:1335", samsung->sabi + config->header_offsets.data_segment));
...
static void __init samsung_sabi_diag(struct samsung_laptop *samsung)
{
	...
	for (i = 0; loca < 0xffff && i < sizeof(samsung->sdiag) - 1; loca++) {
		char temp = pete_readb("drivers/platform/x86/samsung-laptop.c:1356", samsung->f0000_segment + loca);
...
static int __init samsung_sabi_init(struct samsung_laptop *samsung)
{
	...
	ifaceP = (pete_readw("drivers/platform/x86/samsung-laptop.c:1411", samsung->sabi + config->header_offsets.data_segment) & 0x0ffff) << 4;
	ifaceP += pete_readw("drivers/platform/x86/samsung-laptop.c:1412", samsung->sabi + config->header_offsets.data_offset) & 0x0ffff;
