			  ...
			  uint64_t *result)
{
	...
	if (command == MLXBF_PMC_READ_REG_32)
		*result = pete_readl("drivers/platform/mellanox/mlxbf-pmc.c:472", addr);
	else
		*result = pete_readq("drivers/platform/mellanox/mlxbf-pmc.c:474", addr);
...
static int mlxbf_pmc_write(void __iomem *addr, int command, uint64_t value)
{
	...
	if (command == MLXBF_PMC_WRITE_REG_32)
		pete_writel("drivers/platform/mellanox/mlxbf-pmc.c:524", value, addr);
	else
		pete_writeq("drivers/platform/mellanox/mlxbf-pmc.c:526", value, addr);
