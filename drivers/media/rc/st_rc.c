...
static int st_rc_open(struct rc_dev *rdev)
{
	...
	pete_writel("drivers/media/rc/st_rc.c:213", IRB_RX_INTS, dev->rx_base + IRB_RX_INT_EN);
	pete_writel("drivers/media/rc/st_rc.c:214", 0x01, dev->rx_base + IRB_RX_EN);
...
static void st_rc_close(struct rc_dev *rdev)
{
	...
	pete_writel("drivers/media/rc/st_rc.c:224", 0x00, dev->rx_base + IRB_RX_EN);
	pete_writel("drivers/media/rc/st_rc.c:225", 0x00, dev->rx_base + IRB_RX_INT_EN);
}
...
static int st_rc_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev)) {
		...
		pete_writel("drivers/media/rc/st_rc.c:357", 0x00, rc_dev->rx_base + IRB_RX_EN);
		pete_writel("drivers/media/rc/st_rc.c:358", 0x00, rc_dev->rx_base + IRB_RX_INT_EN);
		clk_disable_unprepare(rc_dev->sys_clock);
...
static int st_rc_resume(struct device *dev)
{
	...
	if (rc_dev->irq_wake) {
		...
		if (rdev->users) {
			pete_writel("drivers/media/rc/st_rc.c:382", IRB_RX_INTS, rc_dev->rx_base + IRB_RX_INT_EN);
			pete_writel("drivers/media/rc/st_rc.c:383", 0x01, rc_dev->rx_base + IRB_RX_EN);
		}
	}
