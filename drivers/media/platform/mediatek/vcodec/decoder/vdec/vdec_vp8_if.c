...
static void write_hw_segmentation_data(struct vdec_vp8_inst *inst)
{
	...
	seg_id_addr = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:185", inst->reg_base.top + VP8_SEGID_DRAM_ADDR) >> 4;

	for (i = 0; i < ARRAY_SIZE(vsi->segment_buf); i++) {
		for (j = ARRAY_SIZE(vsi->segment_buf[i]) - 1; j >= 0; j--) {
			val = (1 << 16) + ((seg_id_addr + i) << 2) + j;
			pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:190", val, cm + VP8_HW_VLD_ADDR);

			val = vsi->segment_buf[i][j];
			pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:193", val, cm + VP8_HW_VLD_VALUE);
		}
	}
}
...
static void read_hw_segmentation_data(struct vdec_vp8_inst *inst)
{
	...
	seg_id_addr = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:206", inst->reg_base.top + VP8_SEGID_DRAM_ADDR) >> 4;

	for (i = 0; i < ARRAY_SIZE(vsi->segment_buf); i++) {
		for (j = ARRAY_SIZE(vsi->segment_buf[i]) - 1; j >= 0; j--) {
			val = ((seg_id_addr + i) << 2) + j;
			pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:211", val, cm + VP8_HW_VLD_ADDR);

			val = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:213", cm + VP8_HW_VLD_VALUE);
...
static void enable_hw_rw_function(struct vdec_vp8_inst *inst)
{
	...
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:229", 0x101, ld + VP8_WO_VLD_SRST);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:230", 0x101, hwb + VP8_WO_VLD_SRST);

	mtk_vcodec_write_vdecsys(inst->ctx, 0, 0x1);
	val = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:233", misc + VP8_RW_MISC_SRST);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:234", (val & 0xFFFFFFFE), misc + VP8_RW_MISC_SRST);

	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:236", 0x1, misc + VP8_RW_MISC_SYS_SEL);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:237", 0x17F, misc + VP8_RW_MISC_SPEC_CON);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:238", 0x71201100, misc + VP8_RW_MISC_FUNC_CON);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:239", 0x0, ld + VP8_WO_VLD_SRST);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:240", 0x0, hwb + VP8_WO_VLD_SRST);
	mtk_vcodec_write_vdecsys(inst->ctx, VP8_RW_DCM_CON, 0x1);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:242", 0x1, misc + VP8_RW_MISC_DCM_CON);
	pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:243", 0x1, hwd + VP8_RW_VP8_CTRL);
}
...
static void store_dec_table(struct vdec_vp8_inst *inst)
{
	...
	for (i = 0; i < VP8_DEC_TABLE_PROC_LOOP; i++) {
		pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:254", addr, hwd + VP8_BSASET);
		for (j = 0; j < VP8_DEC_TABLE_UNIT ; j++) {
			val = *p++;
			pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:257", val, hwd + VP8_BSDSET);
		}
...
static void load_dec_table(struct vdec_vp8_inst *inst)
{
	...
	for (i = 0; i < VP8_DEC_TABLE_PROC_LOOP; i++) {
		pete_writel("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:271", addr, hwd + VP8_BSASET);
		/* read total 11 bytes */
		*p++ = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:273", hwd + VP8_BSDSET);
		*p++ = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:274", hwd + VP8_BSDSET);
		*p++ = pete_readl("drivers/media/platform/mediatek/vcodec/decoder/vdec/vdec_vp8_if.c:275", hwd + VP8_BSDSET) & 0xFFFFFF;
		addr += VP8_DEC_TABLE_RW_UNIT;
	}
}
