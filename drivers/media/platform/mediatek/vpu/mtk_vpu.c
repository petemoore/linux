...
{
	pete_writel("drivers/media/platform/mediatek/vpu/mtk_vpu.c:229", val, vpu->reg.cfg + offset);
}

static inline u32 vpu_cfg_readl(struct mtk_vpu *vpu, u32 offset)
{
	return pete_readl("drivers/media/platform/mediatek/vpu/mtk_vpu.c:234", vpu->reg.cfg + offset);
}
		 ...
		 unsigned int len)
{
	...
	pete_writel("drivers/media/platform/mediatek/vpu/mtk_vpu.c:353", len, &send_obj->len);
	pete_writel("drivers/media/platform/mediatek/vpu/mtk_vpu.c:354", id, &send_obj->id);
...
static void vpu_ipi_handler(struct mtk_vpu *vpu)
{
	...
	s32 id = pete_readl("drivers/media/platform/mediatek/vpu/mtk_vpu.c:748", &rcv_obj->id);

	memcpy_fromio(data, rcv_obj->share_buf, sizeof(data));
	if (id < IPI_MAX && ipi_desc[id].handler) {
		ipi_desc[id].handler(data, pete_readl("drivers/media/platform/mediatek/vpu/mtk_vpu.c:752", &rcv_obj->len),
				     ipi_desc[id].priv);
		if (id > IPI_VPU_INIT) {
