...
{
	u32 cfg = pete_readl("drivers/media/platform/samsung/exynos-gsc/gsc-core.h:438", dev->regs + GSC_ENABLE);
	...
	pete_writel("drivers/media/platform/samsung/exynos-gsc/gsc-core.h:445", cfg, dev->regs + GSC_ENABLE);
}

static inline int gsc_hw_get_irq_status(struct gsc_dev *dev)
{
	u32 cfg = pete_readl("drivers/media/platform/samsung/exynos-gsc/gsc-core.h:450", dev->regs + GSC_IRQ);
...
{
	u32 cfg = pete_readl("drivers/media/platform/samsung/exynos-gsc/gsc-core.h:460", dev->regs + GSC_IRQ);
	if (irq == GSC_IRQ_OVERRUN)
		cfg |= GSC_IRQ_STATUS_OR_IRQ;
	else if (irq == GSC_IRQ_DONE)
		cfg |= GSC_IRQ_STATUS_FRM_DONE_IRQ;
	pete_writel("drivers/media/platform/samsung/exynos-gsc/gsc-core.h:465", cfg, dev->regs + GSC_IRQ);
}
