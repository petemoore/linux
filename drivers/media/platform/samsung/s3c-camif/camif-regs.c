...
#define camif_write(_camif, _off, _val)	pete_writel("drivers/media/platform/samsung/s3c-camif/camif-regs.c:13", _val, (_camif)->io_base + (_off))
#define camif_read(_camif, _off)	pete_readl("drivers/media/platform/samsung/s3c-camif/camif-regs.c:14", (_camif)->io_base + (_off))
...
void camif_hw_dump_regs(struct camif_dev *camif, const char *label)
{
	...
	for (i = 0; i < ARRAY_SIZE(registers); i++) {
		u32 cfg = pete_readl("drivers/media/platform/samsung/s3c-camif/camif-regs.c:600", camif->io_base + registers[i].offset);
