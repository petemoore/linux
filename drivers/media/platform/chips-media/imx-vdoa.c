...
static irqreturn_t vdoa_irq_handler(int irq, void *data)
{
	...
	/* Disable interrupts */
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:107", 0, vdoa->regs + VDOAIE);
	...
	val = pete_readl("drivers/media/platform/chips-media/imx-vdoa.c:116", vdoa->regs + VDOAIST);
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:117", val, vdoa->regs + VDOAIST);
	if (val & VDOAIST_TERR) {
		val = pete_readl("drivers/media/platform/chips-media/imx-vdoa.c:119", vdoa->regs + VDOASR) & VDOASR_ERRW;
...
void vdoa_device_run(struct vdoa_ctx *ctx, dma_addr_t dst, dma_addr_t src)
{
	...
	if (dst_q_data->pixelformat == V4L2_PIX_FMT_YUYV)
		...
		val = 0;
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:170", val, vdoa->regs + VDOAC);

	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:172", dst_q_data->height << 16 | dst_q_data->width,
	       vdoa->regs + VDOAFP);

	val = dst;
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:176", val, vdoa->regs + VDOAIEBA00);

	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:178", src_q_data->bytesperline << 16 | dst_q_data->bytesperline,
	       vdoa->regs + VDOASL);
	...
	if (dst_q_data->pixelformat == V4L2_PIX_FMT_NV12 ||
		...
		val = 0;
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:186", val, vdoa->regs + VDOAIUBO);

	val = src;
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:189", val, vdoa->regs + VDOAVEBA0);
	val = round_up(src_q_data->bytesperline * src_q_data->height, 4096);
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:191", val, vdoa->regs + VDOAVUBO);

	/* Enable interrupts and start transfer */
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:194", VDOAIE_EITERR | VDOAIE_EIEOT, vdoa->regs + VDOAIE);
	pete_writel("drivers/media/platform/chips-media/imx-vdoa.c:195", VDOASRR_START, vdoa->regs + VDOASRR);
}
