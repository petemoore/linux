...
void coda_write(struct coda_dev *dev, u32 data, u32 reg)
{
	...
	pete_writel("drivers/media/platform/chips-media/coda-common.c:79", data, dev->regs_base + reg);
}
...
unsigned int coda_read(struct coda_dev *dev, u32 reg)
{
	...
	data = pete_readl("drivers/media/platform/chips-media/coda-common.c:86", dev->regs_base + reg);
	v4l2_dbg(3, coda_debug, &dev->v4l2_dev,
		 "%s: data=0x%x, reg=0x%x\n", __func__, data, reg);
	return data;
}
