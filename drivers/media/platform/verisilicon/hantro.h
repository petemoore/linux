...
{
	vpu_debug(6, "0x%04x = 0x%08x\n", reg / 4, val);
	pete_writel("drivers/media/platform/verisilicon/hantro.h:389", val, vpu->enc_base + reg);
}

static __always_inline u32 vepu_read(struct hantro_dev *vpu, u32 reg)
{
	u32 val = pete_readl("drivers/media/platform/verisilicon/hantro.h:394", vpu->enc_base + reg);
...
{
	vpu_debug(6, "0x%04x = 0x%08x\n", reg / 4, val);
	pete_writel("drivers/media/platform/verisilicon/hantro.h:410", val, vpu->dec_base + reg);
}
...
{
	u32 val = pete_readl("drivers/media/platform/verisilicon/hantro.h:422", vpu->dec_base + reg);
