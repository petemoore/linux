...
static void c8sectpfe_timer_interrupt(struct timer_list *t)
{
	...
	for (chan_num = 0; chan_num < fei->tsin_count; chan_num++) {
		...
		if (channel->irec && pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:75", channel->irec + DMA_PRDS_TPENABLE))
...
static void channel_swdemux_tsklet(struct tasklet_struct *t)
{
	...
	wp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:96", channel->irec + DMA_PRDS_BUSWP_TP(0));
	rp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:97", channel->irec + DMA_PRDS_BUSRP_TP(0));
	...
	if (wp == (channel->back_buffer_busaddr + FEI_BUFFER_SIZE))
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:131", channel->back_buffer_busaddr, channel->irec +
			DMA_PRDS_BUSRP_TP(0));
	else
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:134", wp, channel->irec + DMA_PRDS_BUSRP_TP(0));
}
...
static int c8sectpfe_start_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	if (dvbdmxfeed->pid == 8192) {
		tmp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:187", fei->io + C8SECTPFE_IB_PID_SET(channel->tsin_id));
		tmp &= ~C8SECTPFE_PID_ENABLE;
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:189", tmp, fei->io + C8SECTPFE_IB_PID_SET(channel->tsin_id));
	...
	if (stdemux->running_feed_count == 0) {
		...
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:217", channel->fifo,
			fei->io + C8SECTPFE_IB_BUFF_STRT(channel->tsin_id));
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:219", channel->fifo + FIFO_LEN - 1,
			fei->io + C8SECTPFE_IB_BUFF_END(channel->tsin_id));

		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:222", channel->fifo,
			fei->io + C8SECTPFE_IB_READ_PNT(channel->tsin_id));
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:224", channel->fifo,
			fei->io + C8SECTPFE_IB_WRT_PNT(channel->tsin_id));


		/* reset read / write memdma ptrs for this channel */
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:229", channel->back_buffer_busaddr, channel->irec +
			DMA_PRDS_BUSBASE_TP(0));

		tmp = channel->back_buffer_busaddr + FEI_BUFFER_SIZE - 1;
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:233", tmp, channel->irec + DMA_PRDS_BUSTOP_TP(0));

		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:235", channel->back_buffer_busaddr, channel->irec +
			DMA_PRDS_BUSWP_TP(0));

		/* Issue a reset and enable InputBlock */
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:239", C8SECTPFE_SYS_ENABLE | C8SECTPFE_SYS_RESET
			, fei->io + C8SECTPFE_IB_SYS(channel->tsin_id));

		/* and enable the tp */
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:243", 0x1, channel->irec + DMA_PRDS_TPENABLE);
...
static int c8sectpfe_stop_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	if (dvbdmxfeed->pid == 8192) {
		tmp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:282", fei->io + C8SECTPFE_IB_PID_SET(channel->tsin_id));
		tmp |= C8SECTPFE_PID_ENABLE;
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:284", tmp, fei->io + C8SECTPFE_IB_PID_SET(channel->tsin_id));
	} else {
	...
	if (--stdemux->running_feed_count == 0) {
		...
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:302", 0, fei->io + C8SECTPFE_IB_SYS(channel->tsin_id));

		/* disable this channels descriptor */
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:305", 0,  channel->irec + DMA_PRDS_TPENABLE);

		tasklet_disable(&channel->tsklet);

		/* now request memdma channel goes idle */
		idlereq = (1 << channel->tsin_id) | IDLEREQ;
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:311", idlereq, fei->io + DMA_IDLE_REQ);
		...
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:326", channel->back_buffer_busaddr,
			channel->irec + DMA_PRDS_BUSBASE_TP(0));

		tmp = channel->back_buffer_busaddr + FEI_BUFFER_SIZE - 1;
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:330", tmp, channel->irec + DMA_PRDS_BUSTOP_TP(0));

		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:332", channel->back_buffer_busaddr,
...
static void c8sectpfe_getconfig(struct c8sectpfei *fei)
{
	...
	hw->num_ib = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:382", fei->io + SYS_CFG_NUM_IB);
	hw->num_mib = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:383", fei->io + SYS_CFG_NUM_MIB);
	hw->num_swts = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:384", fei->io + SYS_CFG_NUM_SWTS);
	hw->num_tsout = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:385", fei->io + SYS_CFG_NUM_TSOUT);
	hw->num_ccsc = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:386", fei->io + SYS_CFG_NUM_CCSC);
	hw->num_ram = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:387", fei->io + SYS_CFG_NUM_RAM);
	hw->num_tp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:388", fei->io + SYS_CFG_NUM_TP);
...
static irqreturn_t c8sectpfe_idle_irq_handler(int irq, void *priv)
{
	...
	int bit;
	unsigned long tmp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:407", fei->io + DMA_IDLE_REQ);
	...
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:421", 0, fei->io + DMA_IDLE_REQ);
				...
				struct channel_info *tsin)
{
	...
	tmp = pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:545", fei->io + SYS_INPUT_CLKEN);
	tmp |= BIT(tsin->tsin_id);
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:547", tmp, fei->io + SYS_INPUT_CLKEN);
	...
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:560", tmp, fei->io + C8SECTPFE_IB_IP_FMT_CFG(tsin->tsin_id));

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:562", C8SECTPFE_SYNC(0x9) |
		C8SECTPFE_DROP(0x9) |
		C8SECTPFE_TOKEN(0x47),
		fei->io + C8SECTPFE_IB_SYNCLCKDRP_CFG(tsin->tsin_id));

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:567", TS_PKT_SIZE, fei->io + C8SECTPFE_IB_PKT_LEN(tsin->tsin_id));

	/* Place the FIFO's at the end of the irec descriptors */

	tsin->fifo = (tsin->tsin_id * FIFO_LEN);

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:573", tsin->fifo, fei->io + C8SECTPFE_IB_BUFF_STRT(tsin->tsin_id));
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:574", tsin->fifo + FIFO_LEN - 1,
		fei->io + C8SECTPFE_IB_BUFF_END(tsin->tsin_id));

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:577", tsin->fifo, fei->io + C8SECTPFE_IB_READ_PNT(tsin->tsin_id));
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:578", tsin->fifo, fei->io + C8SECTPFE_IB_WRT_PNT(tsin->tsin_id));

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:580", tsin->pid_buffer_busaddr,
		fei->io + PIDF_BASE(tsin->tsin_id));

	dev_dbg(fei->dev, "chan=%d PIDF_BASE=0x%x pid_bus_addr=%pad\n",
		tsin->tsin_id, pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:584", fei->io + PIDF_BASE(tsin->tsin_id)),
		&tsin->pid_buffer_busaddr);
	...
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:598", tmp, fei->io + C8SECTPFE_IB_PID_SET(tsin->tsin_id));

	dev_dbg(fei->dev, "chan=%d setting wp: %d, rp: %d, buf: %d-%d\n",
		tsin->tsin_id,
		pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:602", fei->io + C8SECTPFE_IB_WRT_PNT(tsin->tsin_id)),
		pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:603", fei->io + C8SECTPFE_IB_READ_PNT(tsin->tsin_id)),
		pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:604", fei->io + C8SECTPFE_IB_BUFF_STRT(tsin->tsin_id)),
		pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:605", fei->io + C8SECTPFE_IB_BUFF_END(tsin->tsin_id)));

	/* Get base addpress of pointer record block from DMEM */
	tsin->irec = fei->io + DMA_MEMDMA_OFFSET + DMA_DMEM_OFFSET +
			pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:609", fei->io + DMA_PTRREC_BASE);
	...
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:616", tsin->fifo, tsin->irec + DMA_PRDS_MEMBASE);

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:618", tsin->fifo + FIFO_LEN - 1, tsin->irec + DMA_PRDS_MEMTOP);

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:620", (188 + 7)&~7, tsin->irec + DMA_PRDS_PKTSIZE);

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:622", 0x1, tsin->irec + DMA_PRDS_TPENABLE);

	/* read/write pointers with physical bus address */

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:626", tsin->back_buffer_busaddr, tsin->irec + DMA_PRDS_BUSBASE_TP(0));

	tmp = tsin->back_buffer_busaddr + FEI_BUFFER_SIZE - 1;
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:629", tmp, tsin->irec + DMA_PRDS_BUSTOP_TP(0));

	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:631", tsin->back_buffer_busaddr, tsin->irec + DMA_PRDS_BUSWP_TP(0));
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:632", tsin->back_buffer_busaddr, tsin->irec + DMA_PRDS_BUSRP_TP(0));
...
static int c8sectpfe_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:711", 0, fei->io + SYS_INPUT_CLKEN);

	/* Enable memdma clock */
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:714", MEMDMAENABLE, fei->io + SYS_OTHER_CLKEN);
...
static void c8sectpfe_remove(struct platform_device *pdev)
{
	...
	if (pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:897", fei->io + DMA_CPU_RUN))
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:898", 0x0,  fei->io + DMA_CPU_RUN);

	/* unclock all internal IP's */
	if (pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:901", fei->io + SYS_INPUT_CLKEN))
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:902", 0, fei->io + SYS_INPUT_CLKEN);

	if (pete_readl("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:904", fei->io + SYS_OTHER_CLKEN))
		pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:905", 0, fei->io + SYS_OTHER_CLKEN);
			...
			int seg_num)
{
	...
	for (i = 0; i < phdr->p_filesz; i++) {
		...
		pete_writeb("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:1023", pete_readb("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:1023", (void __iomem *)imem_src), (void __iomem *)dest);

		/* Every 3 bytes, add an additional
		 * padding zero in destination */
		if (i % 3 == 2) {
			dest++;
			pete_writeb("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:1029", 0x00, (void __iomem *)dest);
		}
...
static int load_c8sectpfe_fw(struct c8sectpfei *fei)
{
	 ...
	 */
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:1152", 0x1, fei->io + DMA_PER_STBUS_SYNC);

	dev_info(fei->dev, "Boot the memdma SLIM core\n");
	pete_writel("drivers/media/platform/st/sti/c8sectpfe/c8sectpfe-core.c:1155", 0x1,  fei->io + DMA_CPU_RUN);
