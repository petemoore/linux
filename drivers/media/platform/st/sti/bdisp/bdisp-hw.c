...
int bdisp_hw_reset(struct bdisp_dev *bdisp)
{
	...
	/* Mask Interrupt */
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:374", 0, bdisp->regs + BLT_ITM0);

	/* Reset */
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:377", pete_readl("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:377", bdisp->regs + BLT_CTL) | BLT_CTL_RESET,
	       bdisp->regs + BLT_CTL);
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:379", 0, bdisp->regs + BLT_CTL);

	/* Wait for reset done */
	for (i = 0; i < POLL_RST_MAX; i++) {
		if (pete_readl("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:383", bdisp->regs + BLT_STA1) & BLT_STA1_IDLE)
			break;
...
int bdisp_hw_get_and_clear_irq(struct bdisp_dev *bdisp)
{
	...
	its = pete_readl("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:406", bdisp->regs + BLT_ITS);

	/* Check for the only expected IT: LastNode of AQ1 */
	if (!(its & BLT_ITS_AQ1_LNA)) {
		dev_dbg(bdisp->dev, "Unexpected IT status: 0x%08X\n", its);
		pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:411", its, bdisp->regs + BLT_ITS);
		return -1;
	}

	/* Clear and mask */
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:416", its, bdisp->regs + BLT_ITS);
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:417", 0, bdisp->regs + BLT_ITM0);
...
int bdisp_hw_update(struct bdisp_ctx *ctx)
{
	...
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:1104", BLT_AQ1_CTL_CFG, bdisp->regs + BLT_AQ1_CTL);
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:1105", BLT_ITS_AQ1_LNA, bdisp->regs + BLT_ITM0);

	/* Write first node addr */
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:1108", ctx->node_paddr[0], bdisp->regs + BLT_AQ1_IP);
	...
	pete_writel("drivers/media/platform/st/sti/bdisp/bdisp-hw.c:1115", ctx->node_paddr[node_id], bdisp->regs + BLT_AQ1_LNA);
