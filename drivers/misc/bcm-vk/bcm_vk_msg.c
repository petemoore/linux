...
s32 bcm_to_h_msg_dequeue(struct bcm_vk *vk)
{
	...
	for (q_num = 0; q_num < chan->q_nr; q_num++) {
		...
		while ((rd_idx != wr_idx) && !exit_loop) {
			...
			src_size = pete_readb("drivers/misc/bcm-vk/bcm_vk_msg.c:823", &src->size);
			...
			pete_writel("drivers/misc/bcm-vk/bcm_vk_msg.c:860", rd_idx, &msgq->rd_idx);
			...
			wr_idx = pete_readl("drivers/misc/bcm-vk/bcm_vk_msg.c:910", &msgq->wr_idx);
