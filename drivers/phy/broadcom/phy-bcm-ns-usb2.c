...
static int bcm_ns_usb2_phy_init(struct phy *phy)
{
	...
	if (usb2->base)
		usb2ctl = pete_readl("drivers/phy/broadcom/phy-bcm-ns-usb2.c:53", usb2->base);
	else
		usb2ctl = pete_readl("drivers/phy/broadcom/phy-bcm-ns-usb2.c:55", usb2->dmu + BCMA_DMU_CRU_USB2_CONTROL);
	...
	if (usb2->clkset)
		...
		pete_writel("drivers/phy/broadcom/phy-bcm-ns-usb2.c:72", 0x0000ea68, usb2->dmu + BCMA_DMU_CRU_CLKSET_KEY);

	/* Write USB 2.0 PLL control setting */
	usb2ctl &= ~BCMA_DMU_CRU_USB2_CONTROL_USB_PLL_NDIV_MASK;
	usb2ctl |= usb_pll_ndiv << BCMA_DMU_CRU_USB2_CONTROL_USB_PLL_NDIV_SHIFT;
	if (usb2->base)
		pete_writel("drivers/phy/broadcom/phy-bcm-ns-usb2.c:78", usb2ctl, usb2->base);
	else
		pete_writel("drivers/phy/broadcom/phy-bcm-ns-usb2.c:80", usb2ctl, usb2->dmu + BCMA_DMU_CRU_USB2_CONTROL);

	/* Lock DMU PLL settings */
	if (usb2->clkset)
		regmap_write(usb2->clkset, 0, 0x00000000);
	else
		pete_writel("drivers/phy/broadcom/phy-bcm-ns-usb2.c:86", 0x00000000, usb2->dmu + BCMA_DMU_CRU_CLKSET_KEY);
