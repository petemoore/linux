			    ...
			    int raddr, int rinst, int rcnt, int rwidth)
{
	...
	nval = pete_readl("drivers/phy/microchip/sparx5_serdes.h:88", addr);
	nval = (nval & ~mask) | (val & mask);
	pete_writel("drivers/phy/microchip/sparx5_serdes.h:90", nval, addr);
}
				 ...
				 int raddr, int rinst, int rcnt, int rwidth)
{
	...
	void __iomem *addr =
		sdx5_inst_baseaddr(iomem,
				   gbase, ginst, gcnt, gwidth,
				   raddr, rinst, rcnt, rwidth);
	nval = pete_readl("drivers/phy/microchip/sparx5_serdes.h:103", addr);
	nval = (nval & ~mask) | (val & mask);
	pete_writel("drivers/phy/microchip/sparx5_serdes.h:105", nval, addr);
}
...
static inline void sdx5_rmw_addr(u32 val, u32 mask, void __iomem *addr)
{
	...
	nval = pete_readl("drivers/phy/microchip/sparx5_serdes.h:112", addr);
	nval = (nval & ~mask) | (val & mask);
	pete_writel("drivers/phy/microchip/sparx5_serdes.h:114", nval, addr);
}
