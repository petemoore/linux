			    ...
			    u8 first, u8 second, u8 mask, u8 val)
{
	...
	orig = pete_readl("drivers/phy/rockchip/phy-rockchip-inno-dsidphy.c:290", inno->phy_base + reg);
	tmp = orig & ~mask;
	tmp |= val & mask;
	pete_writel("drivers/phy/rockchip/phy-rockchip-inno-dsidphy.c:293", tmp, inno->phy_base + reg);
}
