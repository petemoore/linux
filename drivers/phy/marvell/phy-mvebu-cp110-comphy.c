...
static int mvebu_comphy_ethernet_init_reset(struct mvebu_comphy_lane *lane)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:348", priv->base + MVEBU_COMPHY_SERDES_CFG0(lane->id));
	...
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:385", val, priv->base + MVEBU_COMPHY_SERDES_CFG0(lane->id));
	...
	/* reset */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:410", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val &= ~(MVEBU_COMPHY_SERDES_CFG1_RESET |
		 MVEBU_COMPHY_SERDES_CFG1_CORE_RESET |
		 MVEBU_COMPHY_SERDES_CFG1_RF_RESET);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:414", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));

	/* de-assert reset */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:417", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG1_RESET |
	       MVEBU_COMPHY_SERDES_CFG1_CORE_RESET;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:420", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	...
	/* refclk selection */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:431", priv->base + MVEBU_COMPHY_MISC_CTRL0(lane->id));
	val &= ~MVEBU_COMPHY_MISC_CTRL0_REFCLK_SEL;
	if (lane->submode == PHY_INTERFACE_MODE_10GBASER)
		val |= MVEBU_COMPHY_MISC_CTRL0_ICP_FORCE;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:435", val, priv->base + MVEBU_COMPHY_MISC_CTRL0(lane->id));

	/* power and pll selection */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:438", priv->base + MVEBU_COMPHY_PWRPLL_CTRL(lane->id));
	...
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:443", val, priv->base + MVEBU_COMPHY_PWRPLL_CTRL(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:445", priv->base + MVEBU_COMPHY_LOOPBACK(lane->id));
	val &= ~MVEBU_COMPHY_LOOPBACK_DBUS_WIDTH(0x7);
	val |= MVEBU_COMPHY_LOOPBACK_DBUS_WIDTH(0x1);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:448", val, priv->base + MVEBU_COMPHY_LOOPBACK(lane->id));
...
static int mvebu_comphy_init_plls(struct mvebu_comphy_lane *lane)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:459", priv->base + MVEBU_COMPHY_SERDES_CFG0(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG0_PU_PLL |
	       MVEBU_COMPHY_SERDES_CFG0_PU_RX |
	       MVEBU_COMPHY_SERDES_CFG0_PU_TX;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:463", val, priv->base + MVEBU_COMPHY_SERDES_CFG0(lane->id));
	...
	/* rx init */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:476", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG1_RX_INIT;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:478", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:487", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val &= ~MVEBU_COMPHY_SERDES_CFG1_RX_INIT;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:489", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
...
static int mvebu_comphy_set_mode_sgmii(struct phy *phy)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:505", priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));
	val &= ~MVEBU_COMPHY_RX_CTRL1_CLK8T_EN;
	val |= MVEBU_COMPHY_RX_CTRL1_RXCLK2X_SEL;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:508", val, priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:510", priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));
	val &= ~MVEBU_COMPHY_DTL_CTRL_DTL_FLOOP_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:512", val, priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:519", priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S0_TX_EMPH(0xf);
	val |= MVEBU_COMPHY_GEN1_S0_TX_EMPH(0x1);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:522", val, priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));
...
static int mvebu_comphy_set_mode_rxaui(struct phy *phy)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:538", priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));
	val |= MVEBU_COMPHY_RX_CTRL1_RXCLK2X_SEL |
	       MVEBU_COMPHY_RX_CTRL1_CLK8T_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:541", val, priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:543", priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));
	val |= MVEBU_COMPHY_DTL_CTRL_DTL_FLOOP_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:545", val, priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:547", priv->base + MVEBU_COMPHY_SERDES_CFG2(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG2_DFE_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:549", val, priv->base + MVEBU_COMPHY_SERDES_CFG2(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:551", priv->base + MVEBU_COMPHY_DFE_RES(lane->id));
	val |= MVEBU_COMPHY_DFE_RES_FORCE_GEN_TBL;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:553", val, priv->base + MVEBU_COMPHY_DFE_RES(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:555", priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S0_TX_EMPH(0xf);
	val |= MVEBU_COMPHY_GEN1_S0_TX_EMPH(0xd);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:558", val, priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:560", priv->base + MVEBU_COMPHY_GEN1_S1(lane->id));
	...
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:566", val, priv->base + MVEBU_COMPHY_GEN1_S1(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:568", priv->base + MVEBU_COMPHY_COEF(lane->id));
	val &= ~(MVEBU_COMPHY_COEF_DFE_EN | MVEBU_COMPHY_COEF_DFE_CTRL);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:570", val, priv->base + MVEBU_COMPHY_COEF(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:572", priv->base + MVEBU_COMPHY_GEN1_S4(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S4_DFE_RES(0x3);
	val |= MVEBU_COMPHY_GEN1_S4_DFE_RES(0x1);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:575", val, priv->base + MVEBU_COMPHY_GEN1_S4(lane->id));
...
static int mvebu_comphy_set_mode_10gbaser(struct phy *phy)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:591", priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));
	val |= MVEBU_COMPHY_RX_CTRL1_RXCLK2X_SEL |
	       MVEBU_COMPHY_RX_CTRL1_CLK8T_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:594", val, priv->base + MVEBU_COMPHY_RX_CTRL1(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:596", priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));
	val |= MVEBU_COMPHY_DTL_CTRL_DTL_FLOOP_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:598", val, priv->base + MVEBU_COMPHY_DTL_CTRL(lane->id));

	/* Speed divider */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:601", priv->base + MVEBU_COMPHY_SPEED_DIV(lane->id));
	val |= MVEBU_COMPHY_SPEED_DIV_TX_FORCE;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:603", val, priv->base + MVEBU_COMPHY_SPEED_DIV(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:605", priv->base + MVEBU_COMPHY_SERDES_CFG2(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG2_DFE_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:607", val, priv->base + MVEBU_COMPHY_SERDES_CFG2(lane->id));

	/* DFE resolution */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:610", priv->base + MVEBU_COMPHY_DFE_RES(lane->id));
	val |= MVEBU_COMPHY_DFE_RES_FORCE_GEN_TBL;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:612", val, priv->base + MVEBU_COMPHY_DFE_RES(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:614", priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));
	...
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:619", val, priv->base + MVEBU_COMPHY_GEN1_S0(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:621", priv->base + MVEBU_COMPHY_GEN1_S2(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S2_TX_EMPH(0xf);
	val |= MVEBU_COMPHY_GEN1_S2_TX_EMPH_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:624", val, priv->base + MVEBU_COMPHY_GEN1_S2(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:626", priv->base + MVEBU_COMPHY_TX_SLEW_RATE(lane->id));
	val |= MVEBU_COMPHY_TX_SLEW_RATE_EMPH(0x3) |
	       MVEBU_COMPHY_TX_SLEW_RATE_SLC(0x3f);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:629", val, priv->base + MVEBU_COMPHY_TX_SLEW_RATE(lane->id));

	/* Impedance calibration */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:632", priv->base + MVEBU_COMPHY_IMP_CAL(lane->id));
	val &= ~MVEBU_COMPHY_IMP_CAL_TX_EXT(0x1f);
	val |= MVEBU_COMPHY_IMP_CAL_TX_EXT(0xe) |
	       MVEBU_COMPHY_IMP_CAL_TX_EXT_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:636", val, priv->base + MVEBU_COMPHY_IMP_CAL(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:638", priv->base + MVEBU_COMPHY_GEN1_S5(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S5_ICP(0xf);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:640", val, priv->base + MVEBU_COMPHY_GEN1_S5(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:642", priv->base + MVEBU_COMPHY_GEN1_S1(lane->id));
	val &= ~(MVEBU_COMPHY_GEN1_S1_RX_MUL_PI(0x7) |
		 MVEBU_COMPHY_GEN1_S1_RX_MUL_PF(0x7) |
	...
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:652", val, priv->base + MVEBU_COMPHY_GEN1_S1(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:654", priv->base + MVEBU_COMPHY_COEF(lane->id));
	val &= ~(MVEBU_COMPHY_COEF_DFE_EN | MVEBU_COMPHY_COEF_DFE_CTRL);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:656", val, priv->base + MVEBU_COMPHY_COEF(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:658", priv->base + MVEBU_COMPHY_GEN1_S4(lane->id));
	val &= ~MVEBU_COMPHY_GEN1_S4_DFE_RES(0x3);
	val |= MVEBU_COMPHY_GEN1_S4_DFE_RES(0x1);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:661", val, priv->base + MVEBU_COMPHY_GEN1_S4(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:663", priv->base + MVEBU_COMPHY_GEN1_S3(lane->id));
	val |= MVEBU_COMPHY_GEN1_S3_FBCK_SEL;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:665", val, priv->base + MVEBU_COMPHY_GEN1_S3(lane->id));

	/* rx training timer */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:668", priv->base + MVEBU_COMPHY_TRAINING5(lane->id));
	val &= ~MVEBU_COMPHY_TRAINING5_RX_TIMER(0x3ff);
	val |= MVEBU_COMPHY_TRAINING5_RX_TIMER(0x13);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:671", val, priv->base + MVEBU_COMPHY_TRAINING5(lane->id));

	/* tx train peak to peak hold */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:674", priv->base + MVEBU_COMPHY_TRAINING0(lane->id));
	val |= MVEBU_COMPHY_TRAINING0_P2P_HOLD;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:676", val, priv->base + MVEBU_COMPHY_TRAINING0(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:678", priv->base + MVEBU_COMPHY_TX_PRESET(lane->id));
	val &= ~MVEBU_COMPHY_TX_PRESET_INDEX(0xf);
	val |= MVEBU_COMPHY_TX_PRESET_INDEX(0x2);	/* preset coeff */
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:681", val, priv->base + MVEBU_COMPHY_TX_PRESET(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:683", priv->base + MVEBU_COMPHY_FRAME_DETECT3(lane->id));
	val &= ~MVEBU_COMPHY_FRAME_DETECT3_LOST_TIMEOUT_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:685", val, priv->base + MVEBU_COMPHY_FRAME_DETECT3(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:687", priv->base + MVEBU_COMPHY_TX_TRAIN_PRESET(lane->id));
	val |= MVEBU_COMPHY_TX_TRAIN_PRESET_16B_AUTO_EN |
	       MVEBU_COMPHY_TX_TRAIN_PRESET_PRBS11;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:690", val, priv->base + MVEBU_COMPHY_TX_TRAIN_PRESET(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:692", priv->base + MVEBU_COMPHY_FRAME_DETECT0(lane->id));
	val &= ~MVEBU_COMPHY_FRAME_DETECT0_PATN(0x1ff);
	val |= MVEBU_COMPHY_FRAME_DETECT0_PATN(0x88);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:695", val, priv->base + MVEBU_COMPHY_FRAME_DETECT0(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:697", priv->base + MVEBU_COMPHY_DME(lane->id));
	val |= MVEBU_COMPHY_DME_ETH_MODE;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:699", val, priv->base + MVEBU_COMPHY_DME(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:701", priv->base + MVEBU_COMPHY_VDD_CAL0(lane->id));
	val |= MVEBU_COMPHY_VDD_CAL0_CONT_MODE;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:703", val, priv->base + MVEBU_COMPHY_VDD_CAL0(lane->id));

	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:705", priv->base + MVEBU_SP_CALIB(lane->id));
	val &= ~MVEBU_SP_CALIB_SAMPLER(0x3);
	val |= MVEBU_SP_CALIB_SAMPLER(0x3) |
	       MVEBU_SP_CALIB_SAMPLER_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:709", val, priv->base + MVEBU_SP_CALIB(lane->id));
	val &= ~MVEBU_SP_CALIB_SAMPLER_EN;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:711", val, priv->base + MVEBU_SP_CALIB(lane->id));

	/* External rx regulator */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:714", priv->base + MVEBU_COMPHY_EXT_SELV(lane->id));
	val &= ~MVEBU_COMPHY_EXT_SELV_RX_SAMPL(0x1f);
	val |= MVEBU_COMPHY_EXT_SELV_RX_SAMPL(0x1a);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:717", val, priv->base + MVEBU_COMPHY_EXT_SELV(lane->id));
...
static int mvebu_comphy_power_on_legacy(struct phy *phy)
{
	...
	/* digital reset */
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:759", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val |= MVEBU_COMPHY_SERDES_CFG1_RF_RESET;
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:761", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
...
static int mvebu_comphy_power_off_legacy(struct phy *phy)
{
	...
	val = pete_readl("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:880", priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
	val &= ~(MVEBU_COMPHY_SERDES_CFG1_RESET |
		 MVEBU_COMPHY_SERDES_CFG1_CORE_RESET |
		 MVEBU_COMPHY_SERDES_CFG1_RF_RESET);
	pete_writel("drivers/phy/marvell/phy-mvebu-cp110-comphy.c:884", val, priv->base + MVEBU_COMPHY_SERDES_CFG1(lane->id));
