...
static void a38x_set_conf(struct a38x_comphy_lane *lane, bool enable)
{
	...
	if (priv->conf) {
		...
		if (enable)
			...
			conf &= ~BIT(lane->port);
		pete_writel("drivers/phy/marvell/phy-armada38x-comphy.c:70", conf, priv->conf);
	}
}
				...
				unsigned int offset, u32 mask, u32 value)
{
	...
	pete_writel("drivers/phy/marvell/phy-armada38x-comphy.c:80", val | value, lane->base + offset);
}
