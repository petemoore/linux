				    ...
				    struct imx_pin *pin)
{
	...
	if (info->flags & SHARE_MUX_CONF_REG) {
		...
		reg = pete_readl("drivers/pinctrl/freescale/pinctrl-imx.c:185", ipctl->base + pin_reg->mux_reg);
		reg &= ~info->mux_mask;
		reg |= (pin_mmio->mux_mode << info->mux_shift);
		pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:188", reg, ipctl->base + pin_reg->mux_reg);
		dev_dbg(ipctl->dev, "write: offset 0x%x val 0x%x\n",
			pin_reg->mux_reg, reg);
	} else {
		pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:192", pin_mmio->mux_mode, ipctl->base + pin_reg->mux_reg);
		dev_dbg(ipctl->dev, "write: offset 0x%x val 0x%x\n",
	...
	if (pin_mmio->input_val >> 24 == 0xff) {
		 ...
		 */
		val = pete_readl("drivers/pinctrl/freescale/pinctrl-imx.c:220", ipctl->base + pin_mmio->input_reg);
		val &= ~mask;
		val |= select << shift;
		pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:223", val, ipctl->base + pin_mmio->input_reg);
		...
		if (ipctl->input_sel_base)
			pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:230", pin_mmio->input_val, ipctl->input_sel_base +
					pin_mmio->input_reg);
		else
			pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:233", pin_mmio->input_val, ipctl->base +
					pin_mmio->input_reg);
		dev_dbg(ipctl->dev,
				...
				unsigned long *config)
{
	...
	*config = pete_readl("drivers/pinctrl/freescale/pinctrl-imx.c:308", ipctl->base + pin_reg->conf_reg);
				...
				unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		if (info->flags & SHARE_MUX_CONF_REG) {
			u32 reg;
			reg = pete_readl("drivers/pinctrl/freescale/pinctrl-imx.c:349", ipctl->base + pin_reg->conf_reg);
			reg &= info->mux_mask;
			reg |= configs[i];
			pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:352", reg, ipctl->base + pin_reg->conf_reg);
			dev_dbg(ipctl->dev, "write: offset 0x%x val 0x%x\n",
				pin_reg->conf_reg, reg);
		} else {
			pete_writel("drivers/pinctrl/freescale/pinctrl-imx.c:356", configs[i], ipctl->base + pin_reg->conf_reg);
				   ...
				   struct seq_file *s, unsigned pin_id)
{
	...
	if (info->flags & IMX_USE_SCU) {
		...
		config = pete_readl("drivers/pinctrl/freescale/pinctrl-imx.c:404", ipctl->base + pin_reg->conf_reg);
	}
