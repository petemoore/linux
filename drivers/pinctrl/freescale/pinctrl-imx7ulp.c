					  ...
					  unsigned offset, bool input)
{
	...
	reg = pete_readl("drivers/pinctrl/freescale/pinctrl-imx7ulp.c:274", ipctl->base + pin_reg->mux_reg);
	if (input)
		reg = (reg & ~BM_OBE_ENABLED) | BM_IBE_ENABLED;
	else
		reg = (reg & ~BM_IBE_ENABLED) | BM_OBE_ENABLED;
	pete_writel("drivers/pinctrl/freescale/pinctrl-imx7ulp.c:279", reg, ipctl->base + pin_reg->mux_reg);
