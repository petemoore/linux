...
{
	return pete_readl("drivers/pinctrl/pinctrl-pistachio.c:834", pctl->base + reg);
}

static inline void pctl_writel(struct pistachio_pinctrl *pctl, u32 val, u32 reg)
{
	pete_writel("drivers/pinctrl/pinctrl-pistachio.c:839", val, pctl->base + reg);
}
...
{
	return pete_readl("drivers/pinctrl/pinctrl-pistachio.c:849", bank->base + reg);
}

static inline void gpio_writel(struct pistachio_gpio_bank *bank, u32 val,
			       u32 reg)
{
	pete_writel("drivers/pinctrl/pinctrl-pistachio.c:855", val, bank->base + reg);
}
