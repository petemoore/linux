			  ...
			  struct ns2_mux_log *mux_log)
{
	...
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:621", base_address + grp->mux.offset);
	val &= ~(mask << grp->mux.shift);
	val |= grp->mux.alt << grp->mux.shift;
	pete_writel("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:624", val, (base_address + grp->mux.offset));
			    ...
			    u16 enable)
{
	...
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:664", base_address + pin_data->pin_conf.offset);
	val &= ~(NS2_PIN_SRC_MASK << pin_data->pin_conf.input_en);

	if (!enable)
		val |= NS2_PIN_INPUT_EN_MASK << pin_data->pin_conf.input_en;

	pete_writel("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:670", val, (base_address + pin_data->pin_conf.offset));
	spin_unlock_irqrestore(&pinctrl->lock, flags);
...
static int ns2_pin_get_enable(struct pinctrl_dev *pctrldev, unsigned int pin)
{
	...
	spin_lock_irqsave(&pinctrl->lock, flags);
	enable = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:685", pinctrl->pinconf_base + pin_data->pin_conf.offset);
			    ...
			    u32 slew)
{
	...
	base_address = pinctrl->pinconf_base;
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:710", base_address + pin_data->pin_conf.offset);
	val &= ~(NS2_PIN_SRC_MASK << pin_data->pin_conf.src_shift);

	if (slew)
		val |= NS2_PIN_SRC_MASK << pin_data->pin_conf.src_shift;

	pete_writel("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:716", val, (base_address + pin_data->pin_conf.offset));
	spin_unlock_irqrestore(&pinctrl->lock, flags);
			    ...
			    u16 *slew)
{
	...
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:732", pinctrl->pinconf_base + pin_data->pin_conf.offset);
			    ...
			    bool pull_up, bool pull_down)
{
	...
	base_address = pinctrl->pinconf_base;
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:751", base_address + pin_data->pin_conf.offset);
	...
	pete_writel("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:758", val, (base_address + pin_data->pin_conf.offset));
	spin_unlock_irqrestore(&pinctrl->lock, flags);
			     ...
			     bool *pull_down)
{
	...
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:776", pinctrl->pinconf_base + pin_data->pin_conf.offset);
				...
				u32 strength)
{
	...
	base_address = pinctrl->pinconf_base;
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:804", base_address + pin_data->pin_conf.offset);
	val &= ~(NS2_PIN_DRIVE_STRENGTH_MASK << pin_data->pin_conf.drive_shift);
	val |= ((strength / 2) - 1) << pin_data->pin_conf.drive_shift;
	pete_writel("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:807", val, (base_address + pin_data->pin_conf.offset));
	spin_unlock_irqrestore(&pinctrl->lock, flags);
				 ...
				 u16 *strength)
{
	...
	spin_lock_irqsave(&pinctrl->lock, flags);
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-ns2-mux.c:824", pinctrl->pinconf_base + pin_data->pin_conf.offset);
