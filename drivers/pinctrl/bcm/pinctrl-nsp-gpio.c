			       ...
			       unsigned int reg, unsigned gpio, bool set)
{
	...
	val = pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:102", base_address + reg);
	if (set)
		val |= BIT(gpio);
	else
		val &= ~BIT(gpio);

	pete_writel("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:108", val, base_address + reg);
}
...
{
	if (address == IO_CTRL)
		return !!(pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:119", chip->io_ctrl + reg) & BIT(gpio));
	else
		return !!(pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:121", chip->base + reg) & BIT(gpio));
}
...
static irqreturn_t nsp_gpio_irq_handler(int irq, void *data)
{
	...
	int_status = pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:133", chip->base + NSP_CHIP_A_INT_STATUS);
	if (int_status & NSP_CHIP_A_GPIO_INT_BIT) {
		unsigned int event, level;

		/* Get level and edge interrupts */
		event = pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:138", chip->base + NSP_GPIO_EVENT_INT_MASK) &
			      pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:139", chip->base + NSP_GPIO_EVENT);
		level = pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:140", chip->base + NSP_GPIO_DATA_IN) ^
			      pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:141", chip->base + NSP_GPIO_INT_POLARITY);
		level &= pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:142", chip->base + NSP_GPIO_INT_MASK);
...
static void nsp_gpio_irq_ack(struct irq_data *d)
{
	...
	if (trigger_type & (IRQ_TYPE_EDGE_FALLING | IRQ_TYPE_EDGE_RISING))
		pete_writel("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:162", val, chip->base + NSP_GPIO_EVENT);
}
...
static int nsp_gpio_probe(struct platform_device *pdev)
{
	...
	if (irq > 0) {
		...
		val = pete_readl("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:664", chip->base + NSP_CHIP_A_INT_MASK);
		val = val | NSP_CHIP_A_GPIO_INT_BIT;
		pete_writel("drivers/pinctrl/bcm/pinctrl-nsp-gpio.c:666", val, (chip->base + NSP_CHIP_A_INT_MASK));
