					 ...
					 u32 pin, u32 func)
{
	...
	pete_writel("drivers/pinctrl/pinctrl-k210.c:506", val, &pdata->fpioa->pins[pin]);
}
...
{
	struct k210_fpioa_data *pdata = pinctrl_dev_get_drvdata(pctldev);
	u32 val = pete_readl("drivers/pinctrl/pinctrl-k210.c:514", &pdata->fpioa->pins[pin]);
	int drive;
	...
	case PIN_CONFIG_OUTPUT:
		...
		val = pete_readl("drivers/pinctrl/pinctrl-k210.c:558", &pdata->fpioa->pins[pin]);
	...
	pete_writel("drivers/pinctrl/pinctrl-k210.c:591", val, &pdata->fpioa->pins[pin]);
				  ...
				  struct seq_file *s, unsigned int pin)
{
	...
	seq_printf(s, "%#x", pete_readl("drivers/pinctrl/pinctrl-k210.c:621", &pdata->fpioa->pins[pin]));
}
