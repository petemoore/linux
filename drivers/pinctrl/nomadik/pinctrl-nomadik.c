				...
				unsigned offset, int gpio_mode)
{
	...
	afunc = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:296", nmk_chip->addr + NMK_GPIO_AFSLA) & ~BIT(offset);
	bfunc = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:297", nmk_chip->addr + NMK_GPIO_AFSLB) & ~BIT(offset);
	if (gpio_mode & NMK_GPIO_ALT_A)
		afunc |= BIT(offset);
	if (gpio_mode & NMK_GPIO_ALT_B)
		bfunc |= BIT(offset);
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:302", afunc, nmk_chip->addr + NMK_GPIO_AFSLA);
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:303", bfunc, nmk_chip->addr + NMK_GPIO_AFSLB);
}
				...
				unsigned offset, enum nmk_gpio_slpm mode)
{
	...
	slpm = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:311", nmk_chip->addr + NMK_GPIO_SLPC);
	if (mode == NMK_GPIO_SLPM_NOCHANGE)
		slpm |= BIT(offset);
	else
		slpm &= ~BIT(offset);
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:316", slpm, nmk_chip->addr + NMK_GPIO_SLPC);
}
				...
				unsigned offset, enum nmk_gpio_pull pull)
{
	...
	pdis = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:324", nmk_chip->addr + NMK_GPIO_PDIS);
	...
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:332", pdis, nmk_chip->addr + NMK_GPIO_PDIS);

	if (pull == NMK_GPIO_PULL_UP) {
		nmk_chip->pull_up |= BIT(offset);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:336", BIT(offset), nmk_chip->addr + NMK_GPIO_DATS);
	} else if (pull == NMK_GPIO_PULL_DOWN) {
		nmk_chip->pull_up &= ~BIT(offset);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:339", BIT(offset), nmk_chip->addr + NMK_GPIO_DATC);
	}
}
...
static void __nmk_gpio_make_input(struct nmk_gpio_chip *nmk_chip,
				  unsigned offset)
{
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:363", BIT(offset), nmk_chip->addr + NMK_GPIO_DIRC);
}
...
{
	if (val)
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:370", BIT(offset), nmk_chip->addr + NMK_GPIO_DATS);
	else
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:372", BIT(offset), nmk_chip->addr + NMK_GPIO_DATC);
}

static void __nmk_gpio_make_output(struct nmk_gpio_chip *nmk_chip,
				  unsigned offset, int val)
{
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:378", BIT(offset), nmk_chip->addr + NMK_GPIO_DIRS);
				     ...
				     bool glitch)
{
	...
	if (glitch && nmk_chip->set_ioforce) {
		...
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:393", rwimsc & ~bit, nmk_chip->addr + NMK_GPIO_RWIMSC);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:394", fwimsc & ~bit, nmk_chip->addr + NMK_GPIO_FWIMSC);
	...
	if (glitch && nmk_chip->set_ioforce) {
		...
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:404", rwimsc, nmk_chip->addr + NMK_GPIO_RWIMSC);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:405", fwimsc, nmk_chip->addr + NMK_GPIO_FWIMSC);
	}
}
...
static void nmk_write_masked(void __iomem *reg, u32 mask, u32 value)
{
	...
	val = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:443", reg);
	val = ((val & ~mask) | (value & mask));
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:445", val, reg);
}
	...
	unsigned offset, unsigned alt_num)
{
	...
	if (!alt_num) {
		for (i = 0 ; i < PRCM_IDX_GPIOCR_ALTC_MAX ; i++) {
			if (pin_desc->altcx[i].used == true) {
				...
				if (pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:489", npct->prcm_base + reg) & BIT(bit)) {
	...
	for (i = 0 ; i < PRCM_IDX_GPIOCR_ALTC_MAX ; i++) {
		...
		if (pin_desc->altcx[i].used == true) {
			reg = gpiocr_regs[pin_desc->altcx[i].reg_index];
			bit = pin_desc->altcx[i].control_bit;
			if (pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:518", npct->prcm_base + reg) & BIT(bit)) {
				nmk_write_masked(npct->prcm_base + reg, BIT(bit), 0);
				dev_dbg(npct->dev,
					"PRCM GPIOCR: pin %i: alternate-C%i has been disabled\n",
					offset, i+1);
...
static void nmk_gpio_glitch_slpm_init(unsigned int *slpm)
{
	...
	for (i = 0; i < NUM_BANKS; i++) {
		...
		slpm[i] = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:559", chip->addr + NMK_GPIO_SLPC);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:560", temp, chip->addr + NMK_GPIO_SLPC);
	}
}
...
static void nmk_gpio_glitch_slpm_restore(unsigned int *slpm)
{
	...
	for (i = 0; i < NUM_BANKS; i++) {
		...
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:574", slpm[i], chip->addr + NMK_GPIO_SLPC);
...
static int __maybe_unused nmk_prcm_gpiocr_get_mode(struct pinctrl_dev *pctldev, int gpio)
{
	...
	for (i = 0; i < PRCM_IDX_GPIOCR_ALTC_MAX; i++) {
		if (pin_desc->altcx[i].used == true) {
			reg = gpiocr_regs[pin_desc->altcx[i].reg_index];
			bit = pin_desc->altcx[i].control_bit;
			if (pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:605", npct->prcm_base + reg) & BIT(bit))
...
static void nmk_gpio_irq_ack(struct irq_data *d)
{
	...
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:620", BIT(d->hwirq), nmk_chip->addr + NMK_GPIO_IC);
				  ...
				  bool enable)
{
	...
	if (nmk_chip->edge_rising & BIT(offset)) {
		if (enable)
			...
			*rimscval &= ~BIT(offset);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:656", *rimscval, nmk_chip->addr + rimscreg);
	}
	if (nmk_chip->edge_falling & BIT(offset)) {
		if (enable)
			...
			*fimscval &= ~BIT(offset);
		pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:663", *fimscval, nmk_chip->addr + fimscreg);
	}
}
...
static void nmk_gpio_irq_handler(struct irq_desc *desc)
{
	...
	clk_enable(nmk_chip->clk);
	status = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:816", nmk_chip->addr + NMK_GPIO_IS);
	clk_disable(nmk_chip->clk);
...
static int nmk_gpio_get_dir(struct gpio_chip *chip, unsigned offset)
{
	...
	dir = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:838", nmk_chip->addr + NMK_GPIO_DIR) & BIT(offset);
...
static int nmk_gpio_make_input(struct gpio_chip *chip, unsigned offset)
{
	...
	pete_writel("drivers/pinctrl/nomadik/pinctrl-nomadik.c:854", BIT(offset), nmk_chip->addr + NMK_GPIO_DIRC);
...
static int nmk_gpio_get_input(struct gpio_chip *chip, unsigned offset)
{
	...
	value = !!(pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:868", nmk_chip->addr + NMK_GPIO_DAT) & BIT(offset));
...
static int nmk_gpio_get_mode(struct nmk_gpio_chip *nmk_chip, int offset)
{
	...
	afunc = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:908", nmk_chip->addr + NMK_GPIO_AFSLA) & BIT(offset);
	bfunc = pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:909", nmk_chip->addr + NMK_GPIO_AFSLB) & BIT(offset);
	...
	unsigned offset, unsigned gpio)
{
	...
	clk_enable(nmk_chip->clk);
	is_out = !!(pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:938", nmk_chip->addr + NMK_GPIO_DIR) & BIT(offset));
	pull = !(pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:939", nmk_chip->addr + NMK_GPIO_PDIS) & BIT(offset));
	data_out = !!(pete_readl("drivers/pinctrl/nomadik/pinctrl-nomadik.c:940", nmk_chip->addr + NMK_GPIO_DAT) & BIT(offset));
