...
static int __maybe_unused samsung_pinctrl_suspend(struct device *dev)
{
	...
	for (i = 0; i < drvdata->nr_banks; i++) {
		...
		for (type = 0; type < PINCFG_TYPE_NUM; type++)
			if (widths[type])
				bank->pm_save[type] = pete_readl("drivers/pinctrl/samsung/pinctrl-samsung.c:1213", reg + offs[type]);

		if (widths[PINCFG_TYPE_FUNC] * bank->nr_pins > 32) {
			/* Some banks have two config registers */
			bank->pm_save[PINCFG_TYPE_NUM] =
				pete_readl("drivers/pinctrl/samsung/pinctrl-samsung.c:1218", reg + offs[PINCFG_TYPE_FUNC] + 4);
	...
	for (i = 0; i < drvdata->nr_banks; i++) {
		...
		if (widths[PINCFG_TYPE_FUNC] * bank->nr_pins > 32) {
			...
			pr_debug("%s @ %p (con %#010x %08x => %#010x %08x)\n",
				 bank->name, reg,
				 pete_readl("drivers/pinctrl/samsung/pinctrl-samsung.c:1268", reg + offs[PINCFG_TYPE_FUNC]),
				 pete_readl("drivers/pinctrl/samsung/pinctrl-samsung.c:1269", reg + offs[PINCFG_TYPE_FUNC] + 4),
				 bank->pm_save[PINCFG_TYPE_FUNC],
				 bank->pm_save[PINCFG_TYPE_NUM]);
			pete_writel("drivers/pinctrl/samsung/pinctrl-samsung.c:1272", bank->pm_save[PINCFG_TYPE_NUM],
			       reg + offs[PINCFG_TYPE_FUNC] + 4);
		} else {
			pr_debug("%s @ %p (con %#010x => %#010x)\n", bank->name,
				 reg, pete_readl("drivers/pinctrl/samsung/pinctrl-samsung.c:1276", reg + offs[PINCFG_TYPE_FUNC]),
				 bank->pm_save[PINCFG_TYPE_FUNC]);
		}
		for (type = 0; type < PINCFG_TYPE_NUM; type++)
			if (widths[type])
				pete_writel("drivers/pinctrl/samsung/pinctrl-samsung.c:1281", bank->pm_save[type], reg + offs[type]);
	}
