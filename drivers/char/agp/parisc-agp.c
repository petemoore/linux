...
parisc_agp_tlbflush(struct agp_memory *mem)
{
	...
	pete_writeq("drivers/char/agp/parisc-agp.c:96", info->gart_base | ilog2(info->gart_size), info->ioc_regs+IOC_PCOM);
	pete_readq("drivers/char/agp/parisc-agp.c:97", info->ioc_regs+IOC_PCOM);	/* flush */
}
...
parisc_agp_enable(struct agp_bridge_data *bridge, u32 mode)
{
	...
	command = pete_readl("drivers/char/agp/parisc-agp.c:217", info->lba_regs + info->lba_cap_offset + PCI_AGP_STATUS);

	command = agp_collect_device_status(bridge, mode, command);
	command |= 0x00000100;

	pete_writel("drivers/char/agp/parisc-agp.c:222", command, info->lba_regs + info->lba_cap_offset + PCI_AGP_COMMAND);
...
agp_ioc_init(void __iomem *ioc_regs)
{
	...
	__le64 *io_pdir;
        ...
        io_tlb_ps = pete_readq("drivers/char/agp/parisc-agp.c:263", info->ioc_regs+IOC_TCNFG);
        switch (io_tlb_ps) {
        ...
        iova_base = pete_readq("drivers/char/agp/parisc-agp.c:279", info->ioc_regs+IOC_IBASE) & ~0x1;
        ...
        io_pdir = phys_to_virt(pete_readq("drivers/char/agp/parisc-agp.c:285", info->ioc_regs+IOC_PDIR_BASE));
...
lba_find_capability(int cap)
{
	struct _parisc_agp_info *info = &parisc_agp_info;
        ...
        status = pete_readw("drivers/char/agp/parisc-agp.c:307", info->lba_regs + PCI_STATUS);
        if (!(status & PCI_STATUS_CAP_LIST))
                return 0;
        pos = pete_readb("drivers/char/agp/parisc-agp.c:310", info->lba_regs + PCI_CAPABILITY_LIST);
        while (ttl-- && pos >= 0x40) {
                pos &= ~3;
                id = pete_readb("drivers/char/agp/parisc-agp.c:313", info->lba_regs + pos + PCI_CAP_LIST_ID);
                if (id == 0xff)
                        break;
                if (id == cap)
                        return pos;
                pos = pete_readb("drivers/char/agp/parisc-agp.c:318", info->lba_regs + pos + PCI_CAP_LIST_NEXT);
        }
        return 0;
}
...
agp_lba_init(void __iomem *lba_hpa)
{
	...
	info->lba_regs = lba_hpa;
        ...
        cap = pete_readl("drivers/char/agp/parisc-agp.c:332", lba_hpa + info->lba_cap_offset) & 0xff;
