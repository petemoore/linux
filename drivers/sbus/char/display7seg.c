...
static int d7s_probe(struct platform_device *op)
{
	...
	pete_writeb("drivers/sbus/char/display7seg.c:211", regs,  p->regs);
...
static int d7s_remove(struct platform_device *op)
{
	...
	u8 regs = pete_readb("drivers/sbus/char/display7seg.c:235", p->regs);
	...
	if (sol_compat) {
		if (p->flipped)
			...
			regs &= ~D7S_FLIP;
		pete_writeb("drivers/sbus/char/display7seg.c:243", regs, p->regs);
	}
