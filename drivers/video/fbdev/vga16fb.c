...
{
	pete_readb("drivers/video/fbdev/vga16fb.c:112", p);
	pete_writeb("drivers/video/fbdev/vga16fb.c:113", 1, p);
}
	...
	u32 dx = rect->dx, width = rect->width;
        ...
        if (rect->rop == ROP_COPY) {
                ...
                while (height--) {
                        ...
                        /* we can do memset... */
                        for (x = width; x > 0; --x) {
                                pete_writeb("drivers/video/fbdev/vga16fb.c:872", rect->color, where);
                                where++;
                        }
                        where += line_ofs;
                }
	...
	case FB_TYPE_VGA_PLANES:
		if (info->fix.type_aux == FB_AUX_VGA_PLANES_VGA4) {
			...
			case ROP_COPY:
				...
				while (height--) {
					for (x = 0; x < width; x++) {
						pete_writeb("drivers/video/fbdev/vga16fb.c:940", 0, dst);
						dst++;
					}
					dst += line_ofs;
				}
...
static void vga_8planes_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	char __iomem *src;
        ...
        if (area->dy < area->sy || (area->dy == area->sy && dx < sx)) {
                ...
                while (height--) {
                        for (x = 0; x < width; x++) {
                                pete_readb("drivers/video/fbdev/vga16fb.c:996", src);
                                pete_writeb("drivers/video/fbdev/vga16fb.c:997", 0, dest);
                                src++;
                                dest++;
                        }
                        src += line_ofs;
			...
			(area->sy + height - 1) * info->fix.line_length;
                while (height--) {
                        for (x = 0; x < width; x++) {
                                --src;
                                --dest;
                                pete_readb("drivers/video/fbdev/vga16fb.c:1014", src);
                                pete_writeb("drivers/video/fbdev/vga16fb.c:1015", 0, dest);
                        }
                        src -= line_ofs;
                        dest -= line_ofs;
                }
	...
	case FB_TYPE_VGA_PLANES:
		if (info->fix.type_aux == FB_AUX_VGA_PLANES_VGA4) {
			...
			if (dy < sy || (dy == sy && dx < sx)) {
				...
				while (height--) {
					for (x = 0; x < width; x++) {
						pete_readb("drivers/video/fbdev/vga16fb.c:1086", src);
						pete_writeb("drivers/video/fbdev/vga16fb.c:1087", 0, dst);
						dst++;
						src++;
					}
					src += line_ofs;
				...
				while (height--) {
					for (x = 0; x < width; x++) {
						dst--;
						src--;
						pete_readb("drivers/video/fbdev/vga16fb.c:1103", src);
						pete_writeb("drivers/video/fbdev/vga16fb.c:1104", 0, dst);
					}
					src -= line_ofs;
					dst -= line_ofs;
				}
...
static void vga_8planes_imageblit(struct fb_info *info, const struct fb_image *image)
{
	...
	u32 dx = image->dx;
        ...
        setmask(0xff);
        pete_writeb("drivers/video/fbdev/vga16fb.c:1150", image->bg_color, where);
        pete_readb("drivers/video/fbdev/vga16fb.c:1151", where);
        selectmask();
        setmask(image->fg_color ^ image->bg_color);
        setmode(0x42);
        setop(0x18);
        for (y = 0; y < image->height; y++, where += info->fix.line_length)
                pete_writew("drivers/video/fbdev/vga16fb.c:1157", transl_h[cdat[y]&0xF] | transl_l[cdat[y] >> 4], where);
        setmask(oldmask);
        setsr(oldsr);
        setop(oldop);
        setmode(oldmode);
	...
	case FB_TYPE_VGA_PLANES:
		if (info->fix.type_aux == FB_AUX_VGA_PLANES_VGA4) {
			if (par->isVGA) {
				...
				setmask(0xff);
				pete_writeb("drivers/video/fbdev/vga16fb.c:1185", image->bg_color, where);
				rmb();
				pete_readb("drivers/video/fbdev/vga16fb.c:1187", where); /* fill latches */
				setmode(3);
				wmb();
				for (y = 0; y < image->height; y++) {
					dst = where;
					for (x = image->width/8; x--;)
						pete_writeb("drivers/video/fbdev/vga16fb.c:1193", *cdat++, dst++);
					where += info->fix.line_length;
				}
				wmb();
			} else {
