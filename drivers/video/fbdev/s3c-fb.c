...
static void s3c_fb_release_win(struct s3c_fb *sfb, struct s3c_fb_win *win)
{
	...
	if (win->fbinfo) {
		if (sfb->variant.has_shadowcon) {
			data = pete_readl("drivers/video/fbdev/s3c-fb.c:1147", sfb->regs + SHADOWCON);
			data &= ~SHADOWCON_CHx_ENABLE(win->index);
			data &= ~SHADOWCON_CHx_LOCAL_ENABLE(win->index);
			pete_writel("drivers/video/fbdev/s3c-fb.c:1150", data, sfb->regs + SHADOWCON);
		}
...
static void s3c_fb_set_rgb_timing(struct s3c_fb *sfb)
{
	...
	if (sfb->variant.is_2443)
		data |= (1 << 5);
	pete_writel("drivers/video/fbdev/s3c-fb.c:1308", data, regs + VIDCON0);

	data = VIDTCON0_VBPD(vmode->upper_margin - 1) |
	       VIDTCON0_VFPD(vmode->lower_margin - 1) |
	       VIDTCON0_VSPW(vmode->vsync_len - 1);
	pete_writel("drivers/video/fbdev/s3c-fb.c:1313", data, regs + sfb->variant.vidtcon);

	data = VIDTCON1_HBPD(vmode->left_margin - 1) |
	       VIDTCON1_HFPD(vmode->right_margin - 1) |
	       VIDTCON1_HSPW(vmode->hsync_len - 1);
	pete_writel("drivers/video/fbdev/s3c-fb.c:1318", data, regs + sfb->variant.vidtcon + 4);

	data = VIDTCON2_LINEVAL(vmode->yres - 1) |
	       VIDTCON2_HOZVAL(vmode->xres - 1) |
	       VIDTCON2_LINEVAL_E(vmode->yres - 1) |
	       VIDTCON2_HOZVAL_E(vmode->xres - 1);
	pete_writel("drivers/video/fbdev/s3c-fb.c:1324", data, regs + sfb->variant.vidtcon + 8);
}
...
static void s3c_fb_clear_win(struct s3c_fb *sfb, int win)
{
	...
	pete_writel("drivers/video/fbdev/s3c-fb.c:1339", 0, regs + sfb->variant.wincon + (win * 4));
	pete_writel("drivers/video/fbdev/s3c-fb.c:1340", 0, regs + VIDOSD_A(win, sfb->variant));
	pete_writel("drivers/video/fbdev/s3c-fb.c:1341", 0, regs + VIDOSD_B(win, sfb->variant));
	pete_writel("drivers/video/fbdev/s3c-fb.c:1342", 0, regs + VIDOSD_C(win, sfb->variant));

	if (sfb->variant.has_shadowcon) {
		reg = pete_readl("drivers/video/fbdev/s3c-fb.c:1345", sfb->regs + SHADOWCON);
		reg &= ~(SHADOWCON_WINx_PROTECT(win) |
			SHADOWCON_CHx_ENABLE(win) |
			SHADOWCON_CHx_LOCAL_ENABLE(win));
		pete_writel("drivers/video/fbdev/s3c-fb.c:1349", reg, sfb->regs + SHADOWCON);
	}
}
...
static int s3c_fb_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/video/fbdev/s3c-fb.c:1438", pd->vidcon1, sfb->regs + VIDCON1);

	/* set video clock running at under-run */
	if (sfb->variant.has_fixvclk) {
		reg = pete_readl("drivers/video/fbdev/s3c-fb.c:1442", sfb->regs + VIDCON1);
		reg &= ~VIDCON1_VCLK_MASK;
		reg |= VIDCON1_VCLK_RUN;
		pete_writel("drivers/video/fbdev/s3c-fb.c:1445", reg, sfb->regs + VIDCON1);
	}
	...
	for (win = 0; win < (fbdrv->variant.nr_windows - 1); win++) {
		...
		regs += (win * 8);
		pete_writel("drivers/video/fbdev/s3c-fb.c:1458", 0xffffff, regs + WKEYCON0);
		pete_writel("drivers/video/fbdev/s3c-fb.c:1459", 0xffffff, regs + WKEYCON1);
	}
...
static int s3c_fb_resume(struct device *dev)
{
	...
	/* setup gpio and output polarity controls */
	pd->setup_gpio();
	pete_writel("drivers/video/fbdev/s3c-fb.c:1572", pd->vidcon1, sfb->regs + VIDCON1);

	/* set video clock running at under-run */
	if (sfb->variant.has_fixvclk) {
		reg = pete_readl("drivers/video/fbdev/s3c-fb.c:1576", sfb->regs + VIDCON1);
		reg &= ~VIDCON1_VCLK_MASK;
		reg |= VIDCON1_VCLK_RUN;
		pete_writel("drivers/video/fbdev/s3c-fb.c:1579", reg, sfb->regs + VIDCON1);
	}
	...
	for (win_no = 0; win_no < sfb->variant.nr_windows - 1; win_no++) {
		...
		shadow_protect_win(win, 1);
		regs += (win_no * 8);
		pete_writel("drivers/video/fbdev/s3c-fb.c:1594", 0xffffff, regs + WKEYCON0);
		pete_writel("drivers/video/fbdev/s3c-fb.c:1595", 0xffffff, regs + WKEYCON1);
		shadow_protect_win(win, 0);
	}
...
static int s3c_fb_runtime_resume(struct device *dev)
{
	...
	/* setup gpio and output polarity controls */
	pd->setup_gpio();
	pete_writel("drivers/video/fbdev/s3c-fb.c:1642", pd->vidcon1, sfb->regs + VIDCON1);
