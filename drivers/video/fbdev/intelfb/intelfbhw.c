			   ...
			   u8 *data)
{
	...
	for (i = height; i--; ) {
		for (j = 0; j < w; j++) {
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1971", 0x00, addr + j);
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1972", *(data++), addr + j+8);
		}
		if (mod) {
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1975", t_mask, addr + j);
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1976", *(data++) & d_mask, addr + j+8);
		}
		addr += 16;
	}
}
...
void intelfbhw_cursor_reset(struct intelfb_info *dinfo)
{
	...
	for (i = 64; i--; ) {
		for (j = 0; j < 8; j++) {
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1996", 0xff, addr + j+0);
			pete_writeb("drivers/video/fbdev/intelfb/intelfbhw.c:1997", 0x00, addr + j+8);
		}
		addr += 16;
	}
}
