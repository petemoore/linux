...
static int clps711x_fb_probe(struct platform_device *pdev)
{
	...
	if (info->fix.smem_start != (pete_readb("drivers/video/fbdev/clps711x-fb.c:291", cfb->base + CLPS711X_FBADDR) << 28))
		regmap_update_bits(cfb->syscon, SYSCON_OFFSET,
				   SYSCON1_LCDEN, 0);
	...
	if (!(val & SYSCON1_LCDEN)) {
		...
		pete_writeb("drivers/video/fbdev/clps711x-fb.c:301", info->fix.smem_start >> 28, cfb->base + CLPS711X_FBADDR);
		/* Clean FB memory */
