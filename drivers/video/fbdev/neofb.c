...
{
	pete_writel("drivers/video/fbdev/neofb.c:147", val, par->neo2200 + par->cursorOff + regindex);
}
...
static inline int neo2200_sync(struct fb_info *info)
{
	...
	while (pete_readl("drivers/video/fbdev/neofb.c:480", &par->neo2200->bltStat) & 1)
		cpu_relax();
	return 0;
}
				      ...
				      struct fb_var_screeninfo *var)
{
	...
	pete_writel("drivers/video/fbdev/neofb.c:543", bltMod << 16, &neo2200->bltStat);
	pete_writel("drivers/video/fbdev/neofb.c:544", (pitch << 16) | pitch, &neo2200->pitch);
}
...
neo2200_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
{
	...
	/* set blt control */
	pete_writel("drivers/video/fbdev/neofb.c:1372", NEO_BC3_FIFO_EN |
	...
	switch (info->var.bits_per_pixel) {
	case 8:
		pete_writel("drivers/video/fbdev/neofb.c:1380", rect->color, &par->neo2200->fgColor);
		break;
	case 16:
	case 24:
		pete_writel("drivers/video/fbdev/neofb.c:1384", ((u32 *) (info->pseudo_palette))[rect->color],
		       &par->neo2200->fgColor);
		break;
	}

	pete_writel("drivers/video/fbdev/neofb.c:1389", dst * ((info->var.bits_per_pixel + 7) >> 3),
	       &par->neo2200->dstStart);
	pete_writel("drivers/video/fbdev/neofb.c:1391", (rect->height << 16) | (rect->width & 0xffff),
	       &par->neo2200->xyExt);
}
...
neo2200_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	/* set blt control */
	pete_writel("drivers/video/fbdev/neofb.c:1420", bltCntl, &par->neo2200->bltCntl);

	pete_writel("drivers/video/fbdev/neofb.c:1422", src, &par->neo2200->srcStart);
	pete_writel("drivers/video/fbdev/neofb.c:1423", dst, &par->neo2200->dstStart);
	pete_writel("drivers/video/fbdev/neofb.c:1424", (area->height << 16) | (area->width & 0xffff),
	       &par->neo2200->xyExt);
}
...
neo2200_imageblit(struct fb_info *info, const struct fb_image *image)
{
	...
	switch (info->var.bits_per_pixel) {
	case 8:
		pete_writel("drivers/video/fbdev/neofb.c:1465", image->fg_color, &par->neo2200->fgColor);
		pete_writel("drivers/video/fbdev/neofb.c:1466", image->bg_color, &par->neo2200->bgColor);
		break;
	case 16:
	case 24:
		pete_writel("drivers/video/fbdev/neofb.c:1470", ((u32 *) (info->pseudo_palette))[image->fg_color],
		       &par->neo2200->fgColor);
		pete_writel("drivers/video/fbdev/neofb.c:1472", ((u32 *) (info->pseudo_palette))[image->bg_color],
		       &par->neo2200->bgColor);
		break;
	}

	pete_writel("drivers/video/fbdev/neofb.c:1477", NEO_BC0_SYS_TO_VID |
		NEO_BC3_SKIP_MAPPING | bltCntl_flags |
		// NEO_BC3_DST_XY_ADDR |
		0x0c0000, &par->neo2200->bltCntl);

	pete_writel("drivers/video/fbdev/neofb.c:1482", 0, &par->neo2200->srcStart);
//      par->neo2200->dstStart = (image->dy << 16) | (image->dx & 0xffff);
	pete_writel("drivers/video/fbdev/neofb.c:1484", ((image->dx & 0xffff) * (info->var.bits_per_pixel >> 3) +
		image->dy * info->fix.line_length), &par->neo2200->dstStart);
	pete_writel("drivers/video/fbdev/neofb.c:1486", (image->height << 16) | (image->width & 0xffff),
	       &par->neo2200->xyExt);
