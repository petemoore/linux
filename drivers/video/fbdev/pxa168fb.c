...
static int pxa168fb_probe(struct platform_device *pdev)
{
	 ...
	 */
	pete_writel("drivers/video/fbdev/pxa168fb.c:707", 0, fbi->reg_base + LCD_SPU_BLANKCOLOR);
	pete_writel("drivers/video/fbdev/pxa168fb.c:708", mi->io_pin_allocation_mode, fbi->reg_base + SPU_IOPAD_CONTROL);
	pete_writel("drivers/video/fbdev/pxa168fb.c:709", 0, fbi->reg_base + LCD_CFG_GRA_START_ADDR1);
	pete_writel("drivers/video/fbdev/pxa168fb.c:710", 0, fbi->reg_base + LCD_SPU_GRA_OVSA_HPXL_VLN);
	pete_writel("drivers/video/fbdev/pxa168fb.c:711", 0, fbi->reg_base + LCD_SPU_SRAM_PARA0);
	pete_writel("drivers/video/fbdev/pxa168fb.c:712", CFG_CSB_256x32(0x1)|CFG_CSB_256x24(0x1)|CFG_CSB_256x8(0x1),
	 ...
	 */
	pete_writel("drivers/video/fbdev/pxa168fb.c:737", GRA_FRAME_IRQ0_ENA(0x1), fbi->reg_base + SPU_IRQ_ENA);
...
static void pxa168fb_remove(struct platform_device *pdev)
{
	...
	data = pete_readl("drivers/video/fbdev/pxa168fb.c:776", fbi->reg_base + LCD_SPU_DMA_CTRL0);
	data &= ~CFG_GRA_ENA_MASK;
	pete_writel("drivers/video/fbdev/pxa168fb.c:778", data, fbi->reg_base + LCD_SPU_DMA_CTRL0);

	info = fbi->info;

	unregister_framebuffer(info);

	pete_writel("drivers/video/fbdev/pxa168fb.c:784", GRA_FRAME_IRQ0_ENA(0x0), fbi->reg_base + SPU_IRQ_ENA);
