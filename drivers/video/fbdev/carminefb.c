...
static void c_set_disp_reg(const struct carmine_fb *par,
		u32 offset, u32 val)
{
	pete_writel("drivers/video/fbdev/carminefb.c:143", val, par->display_reg + offset);
}

static u32 c_get_disp_reg(const struct carmine_fb *par,
		u32 offset)
{
	return pete_readl("drivers/video/fbdev/carminefb.c:149", par->display_reg + offset);
}

static void c_set_hw_reg(const struct carmine_hw *hw,
		u32 offset, u32 val)
{
	pete_writel("drivers/video/fbdev/carminefb.c:155", val, hw->v_regs + offset);
}

static u32 c_get_hw_reg(const struct carmine_hw *hw,
		u32 offset)
{
	return pete_readl("drivers/video/fbdev/carminefb.c:161", hw->v_regs + offset);
}
