	...
	if(ivideo->sisvga_engine == SIS_315_VGA) {
		...
		if((ivideo->chip >= XGI_40) && ivideo->modechanged) {
			...
			if(!((templ = MMIO_IN32(ivideo->mmio_vbase, 0x8240)) & (1 << 10))) {
				...
				pete_writel("drivers/video/fbdev/sis/sis_main.c:2910", 0x16800000 + 0x8240, ivideo->video_vbase + tempq);
				pete_writel("drivers/video/fbdev/sis/sis_main.c:2911", templ | (1 << 10), ivideo->video_vbase + tempq + 4);
				pete_writel("drivers/video/fbdev/sis/sis_main.c:2912", 0x168F0000, ivideo->video_vbase + tempq + 8);
				pete_writel("drivers/video/fbdev/sis/sis_main.c:2913", 0x168F0000, ivideo->video_vbase + tempq + 12);
			   ...
			   struct sis_video_info *ivideo)
{
	...
	if((pete_readb("drivers/video/fbdev/sis/sis_main.c:4096", rom_base) != 0x55) || (pete_readb("drivers/video/fbdev/sis/sis_main.c:4096", rom_base + 1) != 0xaa))
		return 0;

	romptr = (pete_readb("drivers/video/fbdev/sis/sis_main.c:4099", rom_base + 0x18) | (pete_readb("drivers/video/fbdev/sis/sis_main.c:4099", rom_base + 0x19) << 8));
	if(romptr > (0x10000 - 8))
		return 0;

	rom = rom_base + romptr;

	if((pete_readb("drivers/video/fbdev/sis/sis_main.c:4105", rom)     != 'P') || (pete_readb("drivers/video/fbdev/sis/sis_main.c:4105", rom + 1) != 'C') ||
	   (pete_readb("drivers/video/fbdev/sis/sis_main.c:4106", rom + 2) != 'I') || (pete_readb("drivers/video/fbdev/sis/sis_main.c:4106", rom + 3) != 'R'))
		return 0;

	if((pete_readb("drivers/video/fbdev/sis/sis_main.c:4109", rom + 4) | (pete_readb("drivers/video/fbdev/sis/sis_main.c:4109", rom + 5) << 8)) != ivideo->chip_vendor)
		return 0;

	if((pete_readb("drivers/video/fbdev/sis/sis_main.c:4112", rom + 6) | (pete_readb("drivers/video/fbdev/sis/sis_main.c:4112", rom + 7) << 8)) != ivideo->chip_id)
		return 0;
...
static int sisfb_post_300_buswidth(struct sis_video_info *ivideo)
{
	...
	for(i = 0; i < 2; i++) {
		temp = 0x1234;
		for(j = 0; j < 4; j++) {
			pete_writew("drivers/video/fbdev/sis/sis_main.c:4218", temp, FBAddress);
			if(pete_readw("drivers/video/fbdev/sis/sis_main.c:4219", FBAddress) == temp)
				break;
	...
	pete_writel("drivers/video/fbdev/sis/sis_main.c:4231", 0x01234567L, FBAddress);
	pete_writel("drivers/video/fbdev/sis/sis_main.c:4232", 0x456789ABL, (FBAddress + 4));
	pete_writel("drivers/video/fbdev/sis/sis_main.c:4233", 0x89ABCDEFL, (FBAddress + 8));
	pete_writel("drivers/video/fbdev/sis/sis_main.c:4234", 0xCDEF0123L, (FBAddress + 12));

	reg = SiS_GetReg(SISSR, 0x3b);
	if(reg & 0x01) {
		if(pete_readl("drivers/video/fbdev/sis/sis_main.c:4238", (FBAddress + 12)) == 0xCDEF0123L)
			return 4;	/* Channel A 128bit */
	}

	if(pete_readl("drivers/video/fbdev/sis/sis_main.c:4242", (FBAddress + 4)) == 0x456789ABL)
				 ...
				 int PseudoAdrPinCount, unsigned int mapsize)
{
	...
	for (k = 0; k < ARRAY_SIZE(SiS_DRAMType); k++) {
		...
		/* Write data */
		pete_writew("drivers/video/fbdev/sis/sis_main.c:4320", ((unsigned short)PhysicalAdrHigh),
				(FBAddr + BankNumHigh + PhysicalAdrHigh));
		pete_writew("drivers/video/fbdev/sis/sis_main.c:4322", ((unsigned short)BankNumMid),
				(FBAddr + BankNumMid  + PhysicalAdrHigh));
		pete_writew("drivers/video/fbdev/sis/sis_main.c:4324", ((unsigned short)PhysicalAdrHalfPage),
				(FBAddr + BankNumHigh + PhysicalAdrHalfPage));
		pete_writew("drivers/video/fbdev/sis/sis_main.c:4326", ((unsigned short)PhysicalAdrOtherPage),
				(FBAddr + BankNumHigh + PhysicalAdrOtherPage));

		/* Read data */
		if (pete_readw("drivers/video/fbdev/sis/sis_main.c:4330", FBAddr + BankNumHigh + PhysicalAdrHigh) == PhysicalAdrHigh)
			return 1;
	}
				 ...
				 unsigned int enda, unsigned int mapsize)
{
	...
	pete_writel("drivers/video/fbdev/sis/sis_main.c:4621", 0, ivideo->video_vbase);

	for(i = starta; i <= enda; i++) {
		pos = 1 << i;
		if(pos < mapsize)
			pete_writel("drivers/video/fbdev/sis/sis_main.c:4626", pos, ivideo->video_vbase + pos);
	}

	sisfb_post_xgi_delay(ivideo, 150);

	if(pete_readl("drivers/video/fbdev/sis/sis_main.c:4631", ivideo->video_vbase) != 0)
		return 0;

	for(i = starta; i <= enda; i++) {
		pos = 1 << i;
		if(pos < mapsize) {
			if(pete_readl("drivers/video/fbdev/sis/sis_main.c:4637", ivideo->video_vbase + pos) != pos)
				return 0;
		} else
			return 0;
	}
...
static int sisfb_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if(reg & 0x7f) {
		...
		if(tt) {
			ivideo->modeprechange = pete_readb("drivers/video/fbdev/sis/sis_main.c:6110", tt + 0x49);
			iounmap(tt);
		}
#endif
	}
