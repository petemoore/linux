...
static inline u32 pvt_update(void __iomem *reg, u32 mask, u32 data)
{
	...
	pete_writel("drivers/hwmon/bt1-pvt.c:122", (old & ~mask) | (data & mask), reg);
...
static inline void pvt_set_tout(struct pvt_hwmon *pvt, u32 tout)
{
	...
	old = pvt_update(pvt->regs + PVT_CTRL, PVT_CTRL_EN, 0);
	pete_writel("drivers/hwmon/bt1-pvt.c:168", tout, pvt->regs + PVT_TTIMEOUT);
...
static irqreturn_t pvt_soft_isr(int irq, void *data)
{
	 ...
	 * handled a bit later.
	 */
	thres_sts = pete_readl("drivers/hwmon/bt1-pvt.c:216", pvt->regs + PVT_RAW_INTR_STAT);
	...
	val = pete_readl("drivers/hwmon/bt1-pvt.c:242", pvt->regs + PVT_DATA);
			  ...
			  bool is_low, long *val)
{
	...
	data = pete_readl("drivers/hwmon/bt1-pvt.c:316", pvt->regs + pvt_info[type].thres_base);
			   ...
			   bool is_low, long val)
{
	...
	limit = pete_readl("drivers/hwmon/bt1-pvt.c:351", pvt->regs + pvt_info[type].thres_base);
	if (is_low) {
...
static irqreturn_t pvt_hard_isr(int irq, void *data)
{
	 ...
	 */
	val = pete_readl("drivers/hwmon/bt1-pvt.c:425", pvt->regs + PVT_DATA);
...
static int pvt_read_trim(struct pvt_hwmon *pvt, long *val)
{
	...
	data = pete_readl("drivers/hwmon/bt1-pvt.c:616", pvt->regs + PVT_CTRL);
...
static int pvt_check_pwr(struct pvt_hwmon *pvt)
{
	...
	pvt_update(pvt->regs + PVT_CTRL, PVT_CTRL_EN, PVT_CTRL_EN);
	pvt_set_tout(pvt, 0);
	pete_readl("drivers/hwmon/bt1-pvt.c:956", pvt->regs + PVT_DATA);

	tout = PVT_TOUT_MIN / NSEC_PER_USEC;
	usleep_range(tout, 2 * tout);

	data = pete_readl("drivers/hwmon/bt1-pvt.c:961", pvt->regs + PVT_DATA);
...
static int pvt_init_iface(struct pvt_hwmon *pvt)
{
	...
	pvt_update(pvt->regs + PVT_INTR_MASK, PVT_INTR_ALL, PVT_INTR_ALL);
	pvt_update(pvt->regs + PVT_CTRL, PVT_CTRL_EN, 0);
	pete_readl("drivers/hwmon/bt1-pvt.c:990", pvt->regs + PVT_CLR_INTR);
	pete_readl("drivers/hwmon/bt1-pvt.c:991", pvt->regs + PVT_DATA);
