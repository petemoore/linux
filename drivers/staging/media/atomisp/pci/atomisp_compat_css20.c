...
static void atomisp_css2_hw_store_8(hrt_address addr, uint8_t data)
{
	...
	pete_writeb("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:75", data, isp->base + (addr & 0x003FFFFF));
...
static void atomisp_css2_hw_store_16(hrt_address addr, uint16_t data)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	pete_writew("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:85", data, isp->base + (addr & 0x003FFFFF));
	spin_unlock_irqrestore(&mmio_lock, flags);
}
...
void atomisp_css2_hw_store_32(hrt_address addr, uint32_t data)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	pete_writel("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:95", data, isp->base + (addr & 0x003FFFFF));
	spin_unlock_irqrestore(&mmio_lock, flags);
}
...
static uint8_t atomisp_css2_hw_load_8(hrt_address addr)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	ret = pete_readb("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:106", isp->base + (addr & 0x003FFFFF));
	spin_unlock_irqrestore(&mmio_lock, flags);
	return ret;
}
...
static uint16_t atomisp_css2_hw_load_16(hrt_address addr)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	ret = pete_readw("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:118", isp->base + (addr & 0x003FFFFF));
	spin_unlock_irqrestore(&mmio_lock, flags);
	return ret;
}
...
static uint32_t atomisp_css2_hw_load_32(hrt_address addr)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	ret = pete_readl("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:130", isp->base + (addr & 0x003FFFFF));
	spin_unlock_irqrestore(&mmio_lock, flags);
	return ret;
}
...
static void atomisp_css2_hw_store(hrt_address addr, const void *from, uint32_t n)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	for (i = 0; i < n; i++, from++)
		pete_writeb("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:144", *(s8 *)from, isp->base + addr + i);
...
static void atomisp_css2_hw_load(hrt_address addr, void *to, uint32_t n)
{
	...
	spin_lock_irqsave(&mmio_lock, flags);
	for (i = 0; i < n; i++, to++)
		*(s8 *)to = pete_readb("drivers/staging/media/atomisp/pci/atomisp_compat_css20.c:158", isp->base + addr + i);
	spin_unlock_irqrestore(&mmio_lock, flags);
}
