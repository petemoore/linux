...
static irqreturn_t rkvdec_irq_handler(int irq, void *priv)
{
	...
	status = pete_readl("drivers/staging/media/rkvdec/rkvdec.c:959", rkvdec->regs + RKVDEC_REG_INTERRUPT);
	state = (status & RKVDEC_RDY_STA) ?
		VB2_BUF_STATE_DONE : VB2_BUF_STATE_ERROR;

	pete_writel("drivers/staging/media/rkvdec/rkvdec.c:963", 0, rkvdec->regs + RKVDEC_REG_INTERRUPT);
...
static void rkvdec_watchdog_func(struct work_struct *work)
{
	...
	if (ctx) {
		...
		pete_writel("drivers/staging/media/rkvdec/rkvdec.c:984", RKVDEC_IRQ_DIS, rkvdec->regs + RKVDEC_REG_INTERRUPT);
		pete_writel("drivers/staging/media/rkvdec/rkvdec.c:985", 0, rkvdec->regs + RKVDEC_REG_SYSCTRL);
		rkvdec_job_finish(ctx, VB2_BUF_STATE_ERROR);
	}
}
