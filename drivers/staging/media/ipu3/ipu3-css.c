...
static inline void writes(const void *mem, ssize_t count, void __iomem *addr)
{
	if (count >= 4) {
		...
		count /= 4;
		do {
			pete_writel("drivers/staging/media/ipu3/ipu3-css.c:183", *buf++, addr);
			addr += 4;
		} while (--count);
	}
}
			 ...
			 unsigned int freq)
{
	...
	pete_readl("drivers/staging/media/ipu3/ipu3-css.c:207", base + IMGU_REG_GP_BUSY);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:208", 0, base + IMGU_REG_GP_BUSY);
	...
	/* Reset the css */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:218", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:218", base + IMGU_REG_PM_CTRL) | IMGU_PM_CTRL_FORCE_RESET,
	...
	pm_ctrl = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:225", base + IMGU_REG_PM_CTRL);
	state = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:226", base + IMGU_REG_STATE);
	...
	if (state & IMGU_STATE_POWER_DOWN) {
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:233", IMGU_PM_CTRL_RACE_TO_HALT | IMGU_PM_CTRL_START,
		       base + IMGU_REG_PM_CTRL);
		...
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:242", IMGU_PM_CTRL_RACE_TO_HALT, base + IMGU_REG_PM_CTRL);
	}

	/* Set the busy bit */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:246", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:246", base + IMGU_REG_GP_BUSY) | 1, base + IMGU_REG_GP_BUSY);

	/* Set CSS clock frequency */
	pm_ctrl = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:249", base + IMGU_REG_PM_CTRL);
	val = pm_ctrl & ~(IMGU_PM_CTRL_CSS_PWRDN | IMGU_PM_CTRL_RST_AT_EOF);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:251", val, base + IMGU_REG_PM_CTRL);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:252", 0, base + IMGU_REG_GP_BUSY);
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:259", val, base + IMGU_REG_SYSTEM_REQ);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:260", 1, base + IMGU_REG_GP_BUSY);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:261", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:261", base + IMGU_REG_PM_CTRL) | IMGU_PM_CTRL_FORCE_HALT,
	       base + IMGU_REG_PM_CTRL);
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:269", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:269", base + IMGU_REG_PM_CTRL) | IMGU_PM_CTRL_START,
	       base + IMGU_REG_PM_CTRL);
	if (imgu_hw_wait(base, IMGU_REG_PM_CTRL, IMGU_PM_CTRL_START, 0)) {
		dev_err(dev, "failed to start CSS\n");
		goto fail;
	}
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:275", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:275", base + IMGU_REG_PM_CTRL) | IMGU_PM_CTRL_FORCE_UNHALT,
	       base + IMGU_REG_PM_CTRL);

	val = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:278", base + IMGU_REG_PM_CTRL);	/* get pm_ctrl */
	val &= ~(IMGU_PM_CTRL_CSS_PWRDN | IMGU_PM_CTRL_RST_AT_EOF);
	val |= pm_ctrl & (IMGU_PM_CTRL_CSS_PWRDN | IMGU_PM_CTRL_RST_AT_EOF);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:281", val, base + IMGU_REG_PM_CTRL);
...
void imgu_css_set_powerdown(struct device *dev, void __iomem *base)
{
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:304", 1, base + IMGU_REG_GP_HALT);
	if (imgu_hw_wait(base, IMGU_REG_STATE, IMGU_STATE_HALT_STS,
			 IMGU_STATE_HALT_STS))
		dev_warn(dev, "failed to halt css");

	/* de-assert the busy bit */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:310", 0, base + IMGU_REG_GP_BUSY);
}
...
static void imgu_css_hw_enable_irq(struct imgu_css *css)
{
	 ...
	 */
	val = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:324", base + IMGU_REG_SP_CTRL(0)) | IMGU_CTRL_IRQ_READY;
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:325", val, base + IMGU_REG_SP_CTRL(0));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:326", val | IMGU_CTRL_IRQ_CLEAR, base + IMGU_REG_SP_CTRL(0));

	/* Enable IRQs from the IMGU wrapper */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:329", IMGU_REG_INT_CSS_IRQ, base + IMGU_REG_INT_ENABLE);
	/* Clear */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:331", IMGU_REG_INT_CSS_IRQ, base + IMGU_REG_INT_STATUS);

	/* Enable IRQs from main IRQ controller */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:334", ~0, base + IMGU_REG_IRQCTRL_EDGE_NOT_PULSE(IMGU_IRQCTRL_MAIN));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:335", 0, base + IMGU_REG_IRQCTRL_MASK(IMGU_IRQCTRL_MAIN));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:336", IMGU_IRQCTRL_IRQ_MASK,
	       base + IMGU_REG_IRQCTRL_EDGE(IMGU_IRQCTRL_MAIN));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:338", IMGU_IRQCTRL_IRQ_MASK,
	       base + IMGU_REG_IRQCTRL_ENABLE(IMGU_IRQCTRL_MAIN));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:340", IMGU_IRQCTRL_IRQ_MASK,
	       base + IMGU_REG_IRQCTRL_CLEAR(IMGU_IRQCTRL_MAIN));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:342", IMGU_IRQCTRL_IRQ_MASK,
	       base + IMGU_REG_IRQCTRL_MASK(IMGU_IRQCTRL_MAIN));
	/* Wait for write complete */
	pete_readl("drivers/staging/media/ipu3/ipu3-css.c:345", base + IMGU_REG_IRQCTRL_ENABLE(IMGU_IRQCTRL_MAIN));

	/* Enable IRQs from SP0 and SP1 controllers */
	for (i = IMGU_IRQCTRL_SP0; i <= IMGU_IRQCTRL_SP1; i++) {
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:349", ~0, base + IMGU_REG_IRQCTRL_EDGE_NOT_PULSE(i));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:350", 0, base + IMGU_REG_IRQCTRL_MASK(i));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:351", IMGU_IRQCTRL_IRQ_MASK, base + IMGU_REG_IRQCTRL_EDGE(i));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:352", IMGU_IRQCTRL_IRQ_MASK,
		       base + IMGU_REG_IRQCTRL_ENABLE(i));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:354", IMGU_IRQCTRL_IRQ_MASK, base + IMGU_REG_IRQCTRL_CLEAR(i));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:355", IMGU_IRQCTRL_IRQ_MASK, base + IMGU_REG_IRQCTRL_MASK(i));
		/* Wait for write complete */
		pete_readl("drivers/staging/media/ipu3/ipu3-css.c:357", base + IMGU_REG_IRQCTRL_ENABLE(i));
	}
}
...
static int imgu_css_hw_init(struct imgu_css *css)
{
	...
	for (i = 0; i < IMGU_NUM_SP; i++) {
		...
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:421", css->binary[css->fw_sp[i]].daddr,
		       base + IMGU_REG_SP_ICACHE_ADDR(bi->type));
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:423", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:423", base + IMGU_REG_SP_CTRL(bi->type)) |
		       IMGU_CTRL_ICACHE_INV,
		       base + IMGU_REG_SP_CTRL(bi->type));
	}
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:427", css->binary[css->fw_bl].daddr, base + IMGU_REG_ISP_ICACHE_ADDR);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:428", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:428", base + IMGU_REG_ISP_CTRL) | IMGU_CTRL_ICACHE_INV,
	       base + IMGU_REG_ISP_CTRL);

	/* Check that IMGU hardware is ready */

	if (!(pete_readl("drivers/staging/media/ipu3/ipu3-css.c:433", base + IMGU_REG_SP_CTRL(0)) & IMGU_CTRL_IDLE)) {
		dev_err(dev, "SP is not idle\n");
		return -EIO;
	}
	if (!(pete_readl("drivers/staging/media/ipu3/ipu3-css.c:437", base + IMGU_REG_ISP_CTRL) & IMGU_CTRL_IDLE)) {
		dev_err(dev, "ISP is not idle\n");
		return -EIO;
	}

	for (i = 0; i < ARRAY_SIZE(stream_monitors); i++) {
		val = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:443", base + stream_monitors[i].reg);
	...
	for (i = 0; i < ARRAY_SIZE(imgu_css_gdc_lut[0]); i++) {
		...
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:459", val0 | (val1 << 16),
		       base + IMGU_REG_GDC_LUT_BASE + i * 8);
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:461", val2 | (val3 << 16),
...
static int imgu_css_hw_start_sp(struct imgu_css *css, int sp)
{
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:486", bi->info.sp.sp_entry, base + IMGU_REG_SP_START_ADDR(sp));

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:488", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:488", base + IMGU_REG_SP_CTRL(sp))
...
static int imgu_css_hw_start(struct imgu_css *css)
{
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:520", IMGU_TLB_INVALIDATE, base + IMGU_REG_TLB_INVALIDATE);

	/* Start bootloader */

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:524", IMGU_ABI_BL_SWSTATE_BUSY,
	       base + IMGU_REG_ISP_DMEM_BASE + bl->info.bl.sw_state);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:526", IMGU_NUM_SP,
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:546", bl->info.bl.bl_entry, base + IMGU_REG_ISP_START_ADDR);

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:548", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:548", base + IMGU_REG_ISP_CTRL)
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:564", css->xmem_sp_group_ptrs.daddr,
	       base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.per_frame_data);

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:567", IMGU_ABI_SP_SWSTATE_TERMINATED,
	       base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.sw_state);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:569", 1, base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.invalidate_tlb);

	if (imgu_css_hw_start_sp(css, 0))
		return -EIO;

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:574", 0, base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.isp_started);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:575", 0, base + IMGU_REG_SP_DMEM_BASE(0) +
		bi->info.sp.host_sp_queues_initialized);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:577", 0, base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.sleep_mode);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:578", 0, base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.invalidate_tlb);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:579", IMGU_ABI_SP_COMM_COMMAND_READY, base + IMGU_REG_SP_DMEM_BASE(0)
		+ bi->info.sp.host_sp_com + IMGU_ABI_SP_COMM_COMMAND);

	/* Enable all events for all queues */

	for (i = 0; i < IPU3_CSS_PIPE_ID_NUM; i++)
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:585", event_mask, base + IMGU_REG_SP_DMEM_BASE(0)
			+ bi->info.sp.host_sp_com
			+ IMGU_ABI_SP_COMM_EVENT_IRQ_MASK(i));
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:588", 1, base + IMGU_REG_SP_DMEM_BASE(0) +
		bi->info.sp.host_sp_queues_initialized);
	...
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:595", IMGU_ABI_SP_SWSTATE_TERMINATED,
	       base + IMGU_REG_SP_DMEM_BASE(1) + bi->info.sp.sw_state);

	if (imgu_css_hw_start_sp(css, 1))
		return -EIO;

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:601", IMGU_ABI_SP_COMM_COMMAND_READY, base + IMGU_REG_SP_DMEM_BASE(1)
		+ bi->info.sp.host_sp_com + IMGU_ABI_SP_COMM_COMMAND);
...
static void imgu_css_hw_stop(struct imgu_css *css)
{
	...
	/* Stop fw */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:613", IMGU_ABI_SP_COMM_COMMAND_TERMINATE,
	...
	if (pete_readl("drivers/staging/media/ipu3/ipu3-css.c:619", base + IMGU_REG_SP_DMEM_BASE(0) + bi->info.sp.sw_state) !=
...
static void imgu_css_hw_cleanup(struct imgu_css *css)
{
	...
	/* Clear the CSS busy signal */
	pete_readl("drivers/staging/media/ipu3/ipu3-css.c:634", base + IMGU_REG_GP_BUSY);
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:635", 0, base + IMGU_REG_GP_BUSY);
	...
	/* Reset the css */
	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:643", pete_readl("drivers/staging/media/ipu3/ipu3-css.c:643", base + IMGU_REG_PM_CTRL) | IMGU_PM_CTRL_FORCE_RESET,
	       base + IMGU_REG_PM_CTRL);
...
static u8 imgu_css_queue_pos(struct imgu_css *css, int queue, int thread)
{
	...
	return queue >= 0 ? pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1065", &q->host2sp_bufq_info[thread][queue].end) :
			    pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1066", &q->host2sp_evtq_info.end);
}
			       ...
			       int queue, int thread, u32 data)
{
	...
	if (queue >= 0) {
		size = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1081", &q->host2sp_bufq_info[thread][queue].size);
		start = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1082", &q->host2sp_bufq_info[thread][queue].start);
		end = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1083", &q->host2sp_bufq_info[thread][queue].end);
	} else {
		size = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1085", &q->host2sp_evtq_info.size);
		start = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1086", &q->host2sp_evtq_info.start);
		end = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1087", &q->host2sp_evtq_info.end);
	}
	...
	if (queue >= 0) {
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:1098", data, &q->host2sp_bufq[thread][queue][end]);
		pete_writeb("drivers/staging/media/ipu3/ipu3-css.c:1099", end2, &q->host2sp_bufq_info[thread][queue].end);
	} else {
		pete_writel("drivers/staging/media/ipu3/ipu3-css.c:1101", data, &q->host2sp_evtq[end]);
		pete_writeb("drivers/staging/media/ipu3/ipu3-css.c:1102", end2, &q->host2sp_evtq_info.end);
	}
...
static int imgu_css_dequeue_data(struct imgu_css *css, int queue, u32 *data)
{
	...
	if (queue >= 0) {
		size = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1119", &q->sp2host_bufq_info[queue].size);
		start = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1120", &q->sp2host_bufq_info[queue].start);
		end = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1121", &q->sp2host_bufq_info[queue].end);
	} else {
		size = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1123", &q->sp2host_evtq_info.size);
		start = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1124", &q->sp2host_evtq_info.start);
		end = pete_readb("drivers/staging/media/ipu3/ipu3-css.c:1125", &q->sp2host_evtq_info.end);
	}
	...
	if (queue >= 0) {
		*data = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:1137", &q->sp2host_bufq[queue][start]);
		pete_writeb("drivers/staging/media/ipu3/ipu3-css.c:1138", start2, &q->sp2host_bufq_info[queue].start);
	} else {
		int r;

		*data = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:1142", &q->sp2host_evtq[start]);
		pete_writeb("drivers/staging/media/ipu3/ipu3-css.c:1143", start2, &q->sp2host_evtq_info.start);
...
int imgu_css_irq_ack(struct imgu_css *css)
{
	...
	u32 imgu_status = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:2321", base + IMGU_REG_INT_STATUS);

	pete_writel("drivers/staging/media/ipu3/ipu3-css.c:2323", imgu_status, base + IMGU_REG_INT_STATUS);
	for (i = 0; i < IMGU_IRQCTRL_NUM; i++)
		irq_status[i] = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:2325", base + IMGU_REG_IRQCTRL_STATUS(i));

	for (i = 0; i < NUM_SWIRQS; i++) {
		if (irq_status[IMGU_IRQCTRL_SP0] & IMGU_IRQCTRL_IRQ_SW_PIN(i)) {
			/* SP SW interrupt */
			u32 cnt = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:2330", base + IMGU_REG_SP_DMEM_BASE(0) +
					bi->info.sp.output);
			u32 val = pete_readl("drivers/staging/media/ipu3/ipu3-css.c:2332", base + IMGU_REG_SP_DMEM_BASE(0) +
	...
	for (i = IMGU_IRQCTRL_NUM - 1; i >= 0; i--)
		if (irq_status[i]) {
			pete_writel("drivers/staging/media/ipu3/ipu3-css.c:2342", irq_status[i], base + IMGU_REG_IRQCTRL_CLEAR(i));
			/* Wait for write to complete */
			pete_readl("drivers/staging/media/ipu3/ipu3-css.c:2344", base + IMGU_REG_IRQCTRL_ENABLE(i));
		}
