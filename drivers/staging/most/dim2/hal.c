				   ...
				   u16 hw_buffer_size, u16 packet_length)
{
	...
	pete_writel("drivers/staging/most/dim2/hal.c:359", pete_readl("drivers/staging/most/dim2/hal.c:359", &g.dim2->ACMR0) | bit_mask(ch_addr), &g.dim2->ACMR0);
}

static void dim2_clear_channel(u8 ch_addr)
{
	/* mask interrupt for used channel, disable mlb_sys_int[0] interrupt */
	pete_writel("drivers/staging/most/dim2/hal.c:365", pete_readl("drivers/staging/most/dim2/hal.c:365", &g.dim2->ACMR0) & ~bit_mask(ch_addr), &g.dim2->ACMR0);
	...
	pete_writel("drivers/staging/most/dim2/hal.c:374", bit_mask(ch_addr), &g.dim2->ACSR0);
}
...
{
	/* disable MediaLB */
	pete_writel("drivers/staging/most/dim2/hal.c:518", false << MLBC0_MLBEN_BIT, &g.dim2->MLBC0);

	dim2_clear_ctram();

	/* disable mlb_int interrupt */
	pete_writel("drivers/staging/most/dim2/hal.c:523", 0, &g.dim2->MIEN);

	/* clear status for all dma channels */
	pete_writel("drivers/staging/most/dim2/hal.c:526", 0xFFFFFFFF, &g.dim2->ACSR0);
	pete_writel("drivers/staging/most/dim2/hal.c:527", 0xFFFFFFFF, &g.dim2->ACSR1);

	/* mask interrupts for all channels */
	pete_writel("drivers/staging/most/dim2/hal.c:530", 0, &g.dim2->ACMR0);
	pete_writel("drivers/staging/most/dim2/hal.c:531", 0, &g.dim2->ACMR1);
}
...
static void dim2_initialize(bool enable_6pin, u8 mlb_clock)
{
	...
	pete_writel("drivers/staging/most/dim2/hal.c:539", enable_6pin << MLBC0_MLBPEN_BIT |
	...
	pete_writel("drivers/staging/most/dim2/hal.c:546", 0xFFFFFFFF, &g.dim2->HCMR0);
	pete_writel("drivers/staging/most/dim2/hal.c:547", 0xFFFFFFFF, &g.dim2->HCMR1);

	/* enable HBI */
	pete_writel("drivers/staging/most/dim2/hal.c:550", bit_mask(HCTL_EN_BIT), &g.dim2->HCTL);

	/* configure DMA */
	pete_writel("drivers/staging/most/dim2/hal.c:553", ACTL_DMA_MODE_VAL_DMA_MODE_1 << ACTL_DMA_MODE_BIT |
...
static bool dim2_is_mlb_locked(void)
{
	...
	u32 const c1 = pete_readl("drivers/staging/most/dim2/hal.c:562", &g.dim2->MLBC1);
	u32 const nda_mask = (u32)MLBC1_NDA_MASK << MLBC1_NDA_SHIFT;

	pete_writel("drivers/staging/most/dim2/hal.c:565", c1 & nda_mask, &g.dim2->MLBC1);
	return (pete_readl("drivers/staging/most/dim2/hal.c:566", &g.dim2->MLBC1) & mask1) == 0 &&
	       (pete_readl("drivers/staging/most/dim2/hal.c:567", &g.dim2->MLBC0) & mask0) != 0;
}
...
static inline bool service_channel(u8 ch_addr, u8 idx)
{
	...
	/* clear channel status bit */
	pete_writel("drivers/staging/most/dim2/hal.c:590", bit_mask(ch_addr), &g.dim2->ACSR0);
...
{
	pete_writel("drivers/staging/most/dim2/hal.c:776", 0, &g.dim2->MS0);
	pete_writel("drivers/staging/most/dim2/hal.c:777", 0, &g.dim2->MS1);
}
		  ...
		  u16 max_buffer_size)
{
	...
	if (is_tx && !g.atx_dbr.ch_addr) {
		...
		pete_writel("drivers/staging/most/dim2/hal.c:824", bit_mask(20), &g.dim2->MIEN);
	}
...
u8 dim_destroy_channel(struct dim_channel *ch)
{
	...
	if (ch->addr == g.atx_dbr.ch_addr) {
		pete_writel("drivers/staging/most/dim2/hal.c:891", 0, &g.dim2->MIEN);
		g.atx_dbr.ch_addr = 0;
	}
