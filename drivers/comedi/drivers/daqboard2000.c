...
{
	pete_writew("drivers/comedi/drivers/daqboard2000.c:262", entry & 0x00ff, dev->mmio + DB2K_REG_ACQ_SCAN_LIST_FIFO);
	pete_writew("drivers/comedi/drivers/daqboard2000.c:263", (entry >> 8) & 0x00ff,
			  ...
			  struct comedi_insn *insn, unsigned long context)
{
	...
	status = pete_readw("drivers/comedi/drivers/daqboard2000.c:312", dev->mmio + DB2K_REG_ACQ_STATUS);
	if (status & context)
		return 0;
	return -EBUSY;
}
			     ...
			     struct comedi_insn *insn, unsigned int *data)
{
	...
	pete_writew("drivers/comedi/drivers/daqboard2000.c:326", DB2K_ACQ_CONTROL_RESET_SCAN_LIST_FIFO |
	...
	pete_writew("drivers/comedi/drivers/daqboard2000.c:337", 0, dev->mmio + DB2K_REG_ACQ_PACER_CLOCK_DIV_HIGH);
	...
	for (i = 0; i < insn->n; i++) {
		...
		pete_writew("drivers/comedi/drivers/daqboard2000.c:351", DB2K_ACQ_CONTROL_SEQ_START_SCAN_LIST,
		       dev->mmio + DB2K_REG_ACQ_CONTROL);
		...
		pete_writew("drivers/comedi/drivers/daqboard2000.c:359", DB2K_ACQ_CONTROL_ADC_PACER_ENABLE,
		       dev->mmio + DB2K_REG_ACQ_CONTROL);
		...
		data[i] = pete_readw("drivers/comedi/drivers/daqboard2000.c:373", dev->mmio + DB2K_REG_ACQ_RESULTS_FIFO);
		pete_writew("drivers/comedi/drivers/daqboard2000.c:374", DB2K_ACQ_CONTROL_ADC_PACER_DISABLE,
		       dev->mmio + DB2K_REG_ACQ_CONTROL);
		pete_writew("drivers/comedi/drivers/daqboard2000.c:376", DB2K_ACQ_CONTROL_SEQ_STOP_SCAN_LIST,
		       dev->mmio + DB2K_REG_ACQ_CONTROL);
	}
		       ...
		       struct comedi_insn *insn, unsigned long context)
{
	...
	status = pete_readw("drivers/comedi/drivers/daqboard2000.c:389", dev->mmio + DB2K_REG_DAC_STATUS);
			      ...
			      struct comedi_insn *insn, unsigned int *data)
{
	...
	for (i = 0; i < insn->n; i++) {
		...
		pete_writew("drivers/comedi/drivers/daqboard2000.c:406", val, dev->mmio + DB2K_REG_DAC_SETTING(chan));
...
static int db2k_wait_cpld_init(struct comedi_device *dev)
{
	...
	for (i = 0; i < 50; i++) {
		cpld = pete_readw("drivers/comedi/drivers/daqboard2000.c:471", dev->mmio + DB2K_REG_CPLD_STATUS);
...
static int db2k_wait_cpld_txready(struct comedi_device *dev)
{
	...
	for (i = 0; i < 100; i++) {
		if (pete_readw("drivers/comedi/drivers/daqboard2000.c:487", dev->mmio + DB2K_REG_CPLD_STATUS) &
...
static int db2k_write_cpld(struct comedi_device *dev, u16 data, bool new_cpld)
{
	...
	pete_writew("drivers/comedi/drivers/daqboard2000.c:507", data, dev->mmio + DB2K_REG_CPLD_WDATA);
	if (!(pete_readw("drivers/comedi/drivers/daqboard2000.c:508", dev->mmio + DB2K_REG_CPLD_STATUS) & DB2K_CPLD_STATUS_INIT))
		result = -EIO;
			      ...
			      size_t len, unsigned long context)
{
	...
	for (retry = 0; retry < 3; retry++) {
		...
		new_cpld = (pete_readw("drivers/comedi/drivers/daqboard2000.c:574", dev->mmio + DB2K_REG_CPLD_STATUS) &
...
static void db2k_adc_disarm(struct comedi_device *dev)
{
	...
	udelay(2);
	pete_writew("drivers/comedi/drivers/daqboard2000.c:602", DB2K_TRIG_CONTROL_TYPE_ANALOG | DB2K_TRIG_CONTROL_DISABLE,
	       dev->mmio + DB2K_REG_TRIG_CONTROL);
	udelay(2);
	pete_writew("drivers/comedi/drivers/daqboard2000.c:605", DB2K_TRIG_CONTROL_TYPE_TTL | DB2K_TRIG_CONTROL_DISABLE,
	       dev->mmio + DB2K_REG_TRIG_CONTROL);

	/* Stop the scan list FIFO from loading the configuration pipe */
	udelay(2);
	pete_writew("drivers/comedi/drivers/daqboard2000.c:610", DB2K_ACQ_CONTROL_SEQ_STOP_SCAN_LIST,
	       dev->mmio + DB2K_REG_ACQ_CONTROL);

	/* Stop the pacer clock */
	udelay(2);
	pete_writew("drivers/comedi/drivers/daqboard2000.c:615", DB2K_ACQ_CONTROL_ADC_PACER_DISABLE,
	       dev->mmio + DB2K_REG_ACQ_CONTROL);
...
static void db2k_activate_reference_dacs(struct comedi_device *dev)
{
	...
	pete_writew("drivers/comedi/drivers/daqboard2000.c:628", DB2K_REF_DACS_SET | DB2K_REF_DACS_SELECT_POS_REF,
	       dev->mmio + DB2K_REG_REF_DACS);
	for (timeout = 0; timeout < 20; timeout++) {
		val = pete_readw("drivers/comedi/drivers/daqboard2000.c:631", dev->mmio + DB2K_REG_DAC_STATUS);
	...
	/*  Set the - reference dac value in the FPGA */
	pete_writew("drivers/comedi/drivers/daqboard2000.c:638", DB2K_REF_DACS_SET | DB2K_REF_DACS_SELECT_NEG_REF,
	       dev->mmio + DB2K_REG_REF_DACS);
	for (timeout = 0; timeout < 20; timeout++) {
		val = pete_readw("drivers/comedi/drivers/daqboard2000.c:641", dev->mmio + DB2K_REG_DAC_STATUS);
		if ((val & DB2K_DAC_STATUS_REF_BUSY) == 0)
			break;
		udelay(2);
	}
...
{
	if (dir) {
		pete_writew("drivers/comedi/drivers/daqboard2000.c:672", data, dev->mmio + iobase + port * 2);
		return 0;
	}
	return pete_readw("drivers/comedi/drivers/daqboard2000.c:675", dev->mmio + iobase + port * 2);
}
