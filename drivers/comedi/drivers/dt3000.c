...
static void dt3k_send_cmd(struct comedi_device *dev, unsigned int cmd)
{
	...
	pete_writew("drivers/comedi/drivers/dt3000.c:232", cmd, dev->mmio + DPR_CMD_MBX);

	for (i = 0; i < DPR_CMD_TIMEOUT; i++) {
		status = pete_readw("drivers/comedi/drivers/dt3000.c:235", dev->mmio + DPR_CMD_MBX);
...
static unsigned int dt3k_readsingle(struct comedi_device *dev,
				    ...
				    unsigned int gain)
{
	pete_writew("drivers/comedi/drivers/dt3000.c:251", subsys, dev->mmio + DPR_SUBSYS);

	pete_writew("drivers/comedi/drivers/dt3000.c:253", chan, dev->mmio + DPR_PARAMS(0));
	pete_writew("drivers/comedi/drivers/dt3000.c:254", gain, dev->mmio + DPR_PARAMS(1));

	dt3k_send_cmd(dev, DPR_CMD_READSINGLE);

	return pete_readw("drivers/comedi/drivers/dt3000.c:258", dev->mmio + DPR_PARAMS(2));
}

static void dt3k_writesingle(struct comedi_device *dev, unsigned int subsys,
			     unsigned int chan, unsigned int data)
{
	pete_writew("drivers/comedi/drivers/dt3000.c:264", subsys, dev->mmio + DPR_SUBSYS);

	pete_writew("drivers/comedi/drivers/dt3000.c:266", chan, dev->mmio + DPR_PARAMS(0));
	pete_writew("drivers/comedi/drivers/dt3000.c:267", 0, dev->mmio + DPR_PARAMS(1));
	pete_writew("drivers/comedi/drivers/dt3000.c:268", data, dev->mmio + DPR_PARAMS(2));
			       ...
			       struct comedi_subdevice *s)
{
	...
	front = pete_readw("drivers/comedi/drivers/dt3000.c:283", dev->mmio + DPR_AD_BUF_FRONT);
	...
	for (i = 0; i < count; i++) {
		data = pete_readw("drivers/comedi/drivers/dt3000.c:291", dev->mmio + DPR_ADC_BUFFER + rear);
	...
	pete_writew("drivers/comedi/drivers/dt3000.c:299", rear, dev->mmio + DPR_AD_BUF_REAR);
}

static int dt3k_ai_cancel(struct comedi_device *dev,
			  struct comedi_subdevice *s)
{
	pete_writew("drivers/comedi/drivers/dt3000.c:305", DPR_SUBSYS_AI, dev->mmio + DPR_SUBSYS);
	dt3k_send_cmd(dev, DPR_CMD_STOP);

	pete_writew("drivers/comedi/drivers/dt3000.c:308", 0, dev->mmio + DPR_INT_MASK);
...
static irqreturn_t dt3k_interrupt(int irq, void *d)
{
	...
	status = pete_readw("drivers/comedi/drivers/dt3000.c:326", dev->mmio + DPR_INTR_FLAG);
...
static int dt3k_ai_cmd(struct comedi_device *dev, struct comedi_subdevice *s)
{
	...
	for (i = 0; i < cmd->chanlist_len; i++) {
		...
		pete_writew("drivers/comedi/drivers/dt3000.c:465", (range << 6) | chan, dev->mmio + DPR_ADC_BUFFER + i);
	}
	aref = CR_AREF(cmd->chanlist[0]);

	pete_writew("drivers/comedi/drivers/dt3000.c:469", cmd->scan_end_arg, dev->mmio + DPR_PARAMS(0));

	if (cmd->convert_src == TRIG_TIMER) {
		divider = dt3k_ns_to_timer(50, &cmd->convert_arg, cmd->flags);
		pete_writew("drivers/comedi/drivers/dt3000.c:473", (divider >> 16), dev->mmio + DPR_PARAMS(1));
		pete_writew("drivers/comedi/drivers/dt3000.c:474", (divider & 0xffff), dev->mmio + DPR_PARAMS(2));
	}

	if (cmd->scan_begin_src == TRIG_TIMER) {
		tscandiv = dt3k_ns_to_timer(100, &cmd->scan_begin_arg,
					    cmd->flags);
		pete_writew("drivers/comedi/drivers/dt3000.c:480", (tscandiv >> 16), dev->mmio + DPR_PARAMS(3));
		pete_writew("drivers/comedi/drivers/dt3000.c:481", (tscandiv & 0xffff), dev->mmio + DPR_PARAMS(4));
	}

	pete_writew("drivers/comedi/drivers/dt3000.c:484", DPR_PARAM5_AD_TRIG_INT_RETRIG, dev->mmio + DPR_PARAMS(5));
	pete_writew("drivers/comedi/drivers/dt3000.c:485", (aref == AREF_DIFF) ? DPR_PARAM6_AD_DIFF : 0,
	       dev->mmio + DPR_PARAMS(6));

	pete_writew("drivers/comedi/drivers/dt3000.c:488", DPR_AI_FIFO_DEPTH / 2, dev->mmio + DPR_PARAMS(7));

	pete_writew("drivers/comedi/drivers/dt3000.c:490", DPR_SUBSYS_AI, dev->mmio + DPR_SUBSYS);
	dt3k_send_cmd(dev, DPR_CMD_CONFIG);

	pete_writew("drivers/comedi/drivers/dt3000.c:493", DPR_INTR_ADFULL | DPR_INTR_ADSWERR | DPR_INTR_ADHWERR,
	       dev->mmio + DPR_INT_MASK);

	debug_n_ints = 0;

	pete_writew("drivers/comedi/drivers/dt3000.c:498", DPR_SUBSYS_AI, dev->mmio + DPR_SUBSYS);
...
{
	/* XXX */
	pete_writew("drivers/comedi/drivers/dt3000.c:543", DPR_SUBSYS_DOUT, dev->mmio + DPR_SUBSYS);

	pete_writew("drivers/comedi/drivers/dt3000.c:545", bits, dev->mmio + DPR_PARAMS(0));
			      ...
			      unsigned int *data)
{
	...
	for (i = 0; i < insn->n; i++) {
		pete_writew("drivers/comedi/drivers/dt3000.c:597", DPR_SUBSYS_MEM, dev->mmio + DPR_SUBSYS);
		pete_writew("drivers/comedi/drivers/dt3000.c:598", addr, dev->mmio + DPR_PARAMS(0));
		pete_writew("drivers/comedi/drivers/dt3000.c:599", 1, dev->mmio + DPR_PARAMS(1));

		dt3k_send_cmd(dev, DPR_CMD_READCODE);

		data[i] = pete_readw("drivers/comedi/drivers/dt3000.c:603", dev->mmio + DPR_PARAMS(2));
	}
