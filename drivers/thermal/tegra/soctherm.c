...
static void thermal_irq_enable(struct tegra_thermctl_zone *zn)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:641", zn->ts->regs + THERMCTL_INTR_ENABLE);
	r = REG_SET_MASK(r, zn->sg->thermctl_isr_mask, TH_INTR_UP_DN_EN);
	pete_writel("drivers/thermal/tegra/soctherm.c:643", r, zn->ts->regs + THERMCTL_INTR_ENABLE);
...
static void thermal_irq_disable(struct tegra_thermctl_zone *zn)
{
	...
	/* multiple zones could be handling and setting trips at once */
	mutex_lock(&zn->ts->thermctl_lock);
	r = pete_readl("drivers/thermal/tegra/soctherm.c:653", zn->ts->regs + THERMCTL_INTR_DISABLE);
	r = REG_SET_MASK(r, zn->sg->thermctl_isr_mask, 0);
	pete_writel("drivers/thermal/tegra/soctherm.c:655", r, zn->ts->regs + THERMCTL_INTR_DISABLE);
	mutex_unlock(&zn->ts->thermctl_lock);
}
...
static int tegra_thermctl_set_trips(struct thermal_zone_device *tz, int lo, int hi)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:666", zone->ts->regs + zone->sg->thermctl_lvl0_offset);
	r = REG_SET_MASK(r, THERMCTL_LVL0_CPU0_EN_MASK, 0);
	pete_writel("drivers/thermal/tegra/soctherm.c:668", r, zone->ts->regs + zone->sg->thermctl_lvl0_offset);
	...
	r = REG_SET_MASK(r, THERMCTL_LVL0_CPU0_EN_MASK, 1);
	pete_writel("drivers/thermal/tegra/soctherm.c:677", r, zone->ts->regs + zone->sg->thermctl_lvl0_offset);
...
static irqreturn_t soctherm_thermal_isr(int irq, void *dev_id)
{
	 ...
	 */
	r = pete_readl("drivers/thermal/tegra/soctherm.c:810", ts->regs + THERMCTL_INTR_STATUS);
	pete_writel("drivers/thermal/tegra/soctherm.c:811", r, ts->regs + THERMCTL_INTR_DISABLE);
...
static irqreturn_t soctherm_thermal_isr_thread(int irq, void *dev_id)
{
	...
	st = pete_readl("drivers/thermal/tegra/soctherm.c:839", ts->regs + THERMCTL_INTR_STATUS);
	...
	ex |= cp | gp | pl | me;
	if (ex) {
		pete_writel("drivers/thermal/tegra/soctherm.c:856", ex, ts->regs + THERMCTL_INTR_STATUS);
		st &= ~ex;
	...
	if (st) {
		...
		pete_writel("drivers/thermal/tegra/soctherm.c:891", st, ts->regs + THERMCTL_INTR_STATUS);
	}
				    ...
				    bool enable)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:916", ts->regs + OC_INTR_ENABLE);
	switch (alarm) {
	case THROTTLE_OC1:
	...
	pete_writel("drivers/thermal/tegra/soctherm.c:934", r, ts->regs + OC_INTR_ENABLE);
}
...
static irqreturn_t soctherm_edp_isr_thread(int irq, void *arg)
{
	...
	st = pete_readl("drivers/thermal/tegra/soctherm.c:1000", ts->regs + OC_INTR_STATUS);
	...
	if (ex) {
		pete_writel("drivers/thermal/tegra/soctherm.c:1011", st, ts->regs + OC_INTR_STATUS);
		st &= ~ex;
	...
	if (st) {
		...
		pete_writel("drivers/thermal/tegra/soctherm.c:1045", st, ts->regs + OC_INTR_STATUS);
	}
...
static irqreturn_t soctherm_edp_isr(int irq, void *arg)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1074", ts->regs + OC_INTR_STATUS);
	pete_writel("drivers/thermal/tegra/soctherm.c:1075", r, ts->regs + OC_INTR_DISABLE);
...
static int regs_show(struct seq_file *s, void *data)
{
	...
	for (i = 0; i < ts->soc->num_tsensors; i++) {
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1266", ts->regs + tsensors[i].base + SENSOR_CONFIG1);
		...
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1284", ts->regs + tsensors[i].base + SENSOR_STATUS1);
		...
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1290", ts->regs + tsensors[i].base + SENSOR_STATUS0);
		state = REG_GET_MASK(r, SENSOR_STATUS0_VALID_MASK);
		seq_printf(s, "Capture(%d/", state);
		state = REG_GET_MASK(r, SENSOR_STATUS0_CAPTURE_MASK);
		seq_printf(s, "%d) ", state);

		r = pete_readl("drivers/thermal/tegra/soctherm.c:1296", ts->regs + tsensors[i].base + SENSOR_CONFIG0);
		...
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1308", ts->regs + tsensors[i].base + SENSOR_CONFIG2);
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1315", ts->regs + SENSOR_PDIV);
	seq_printf(s, "PDIV: 0x%x\n", r);

	r = pete_readl("drivers/thermal/tegra/soctherm.c:1318", ts->regs + SENSOR_HOTSPOT_OFF);
	seq_printf(s, "HOTSPOT: 0x%x\n", r);

	seq_puts(s, "\n");
	seq_puts(s, "-----SOC_THERM-----\n");

	r = pete_readl("drivers/thermal/tegra/soctherm.c:1324", ts->regs + SENSOR_TEMP1);
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1329", ts->regs + SENSOR_TEMP2);
	...
	for (i = 0; i < ts->soc->num_ttgs; i++) {
		...
		for (level = 0; level < 4; level++) {
			...
			r = pete_readl("drivers/thermal/tegra/soctherm.c:1342", ts->regs + THERMCTL_LVL_REG(off, level));
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1393", ts->regs + THERMCTL_STATS_CTL);
	...
	for (level = 0; level < 4; level++) {
		...
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1402", ts->regs + THERMCTL_LVL_REG(off, level));
		seq_printf(s, "  Level_%d Up(%d) ", level, r);

		off = THERMCTL_LVL0_DN_STATS;
		r = pete_readl("drivers/thermal/tegra/soctherm.c:1406", ts->regs + THERMCTL_LVL_REG(off, level));
		seq_printf(s, "Dn(%d)\n", r);
	}

	r = pete_readl("drivers/thermal/tegra/soctherm.c:1410", ts->regs + THERMCTL_THERMTRIP_CTL);
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1421", ts->regs + THROT_GLOBAL_CFG);
	seq_puts(s, "\n");
	seq_printf(s, "GLOBAL THROTTLE CONFIG: 0x%08x\n", r);

	seq_puts(s, "---------------------------------------------------\n");
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1426", ts->regs + THROT_STATUS);
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1434", ts->regs + CPU_PSKIP_STATUS);
	if (ts->soc->use_ccroc) {
				    ...
				    unsigned long *cur_state)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1513", ts->regs + THROT_STATUS);
					 ...
					 enum soctherm_throttle_id throt)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1801", ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_CPU));
	...
	pete_writel("drivers/thermal/tegra/soctherm.c:1805", r, ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_CPU));

	/* bypass sequencer in soc_therm as it is programmed in ccroc */
	r = REG_SET_MASK(0, THROT_PSKIP_RAMP_SEQ_BYPASS_MODE_MASK, 1);
	pete_writel("drivers/thermal/tegra/soctherm.c:1809", r, ts->regs + THROT_PSKIP_RAMP(throt, THROTTLE_DEV_CPU));
}
			       ...
			       enum soctherm_throttle_id throt)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1834", ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_CPU));
	r = REG_SET_MASK(r, THROT_PSKIP_CTRL_ENABLE_MASK, 1);
	r = REG_SET_MASK(r, THROT_PSKIP_CTRL_DIVIDEND_MASK, dividend);
	r = REG_SET_MASK(r, THROT_PSKIP_CTRL_DIVISOR_MASK, 0xff);
	pete_writel("drivers/thermal/tegra/soctherm.c:1838", r, ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_CPU));

	r = pete_readl("drivers/thermal/tegra/soctherm.c:1840", ts->regs + THROT_PSKIP_RAMP(throt, THROTTLE_DEV_CPU));
	r = REG_SET_MASK(r, THROT_PSKIP_RAMP_DURATION_MASK, 0xff);
	r = REG_SET_MASK(r, THROT_PSKIP_RAMP_STEP_MASK, 0xf);
	pete_writel("drivers/thermal/tegra/soctherm.c:1843", r, ts->regs + THROT_PSKIP_RAMP(throt, THROTTLE_DEV_CPU));
}
					 ...
					 enum soctherm_throttle_id throt)
{
	...
	r = pete_readl("drivers/thermal/tegra/soctherm.c:1863", ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_GPU));
	r = REG_SET_MASK(r, THROT_PSKIP_CTRL_ENABLE_MASK, 1);
	r = REG_SET_MASK(r, THROT_PSKIP_CTRL_VECT_GPU_MASK, throt_vect);
	pete_writel("drivers/thermal/tegra/soctherm.c:1866", r, ts->regs + THROT_PSKIP_CTRL(throt, THROTTLE_DEV_GPU));
}
				      ...
				      enum soctherm_throttle_id throt)
{
	...
	pete_writel("drivers/thermal/tegra/soctherm.c:1882", r, ts->regs + ALARM_CFG(throt));
	pete_writel("drivers/thermal/tegra/soctherm.c:1883", oc->throt_period, ts->regs + ALARM_THROTTLE_PERIOD(throt));
	pete_writel("drivers/thermal/tegra/soctherm.c:1884", oc->alarm_cnt_thresh, ts->regs + ALARM_CNT_THRESHOLD(throt));
	pete_writel("drivers/thermal/tegra/soctherm.c:1885", oc->alarm_filter, ts->regs + ALARM_FILTER(throt));
				      ...
				      enum soctherm_throttle_id throt)
{
	...
	pete_writel("drivers/thermal/tegra/soctherm.c:1920", r, ts->regs + THROT_PRIORITY_CTRL(throt));

	r = REG_SET_MASK(0, THROT_DELAY_LITE_DELAY_MASK, 0);
	pete_writel("drivers/thermal/tegra/soctherm.c:1923", r, ts->regs + THROT_DELAY_CTRL(throt));

	r = pete_readl("drivers/thermal/tegra/soctherm.c:1925", ts->regs + THROT_PRIORITY_LOCK);
	r = REG_GET_MASK(r, THROT_PRIORITY_LOCK_PRIORITY_MASK);
	if (r >= stc.priority)
		return;
	r = REG_SET_MASK(0, THROT_PRIORITY_LOCK_PRIORITY_MASK,
			 stc.priority);
	pete_writel("drivers/thermal/tegra/soctherm.c:1931", r, ts->regs + THROT_PRIORITY_LOCK);
}
...
static void tegra_soctherm_throttle(struct device *dev)
{
	...
	if (ts->soc->use_ccroc) {
		...
		pete_writel("drivers/thermal/tegra/soctherm.c:1959", v, ts->regs + THROT_GLOBAL_CFG);

		v = pete_readl("drivers/thermal/tegra/soctherm.c:1961", ts->clk_regs + CAR_SUPER_CCLKG_DIVIDER);
		v = REG_SET_MASK(v, CDIVG_USE_THERM_CONTROLS_MASK, 1);
		pete_writel("drivers/thermal/tegra/soctherm.c:1963", v, ts->clk_regs + CAR_SUPER_CCLKG_DIVIDER);
	}

	/* initialize stats collection */
	v = STATS_CTL_CLR_DN | STATS_CTL_EN_DN |
	    STATS_CTL_CLR_UP | STATS_CTL_EN_UP;
	pete_writel("drivers/thermal/tegra/soctherm.c:1969", v, ts->regs + THERMCTL_STATS_CTL);
}
...
static void soctherm_init(struct platform_device *pdev)
{
	...
	pdiv = pete_readl("drivers/thermal/tegra/soctherm.c:2035", tegra->regs + SENSOR_PDIV);
	hotspot = pete_readl("drivers/thermal/tegra/soctherm.c:2036", tegra->regs + SENSOR_HOTSPOT_OFF);
	...
	pete_writel("drivers/thermal/tegra/soctherm.c:2047", pdiv, tegra->regs + SENSOR_PDIV);
	pete_writel("drivers/thermal/tegra/soctherm.c:2048", hotspot, tegra->regs + SENSOR_HOTSPOT_OFF);
