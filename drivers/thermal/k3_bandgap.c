			    ...
			    int *temp)
{
	 ...
	 * Errata workaround.
	 */
	s0 = pete_readl("drivers/thermal/k3_bandgap.c:126", bgp->base + devdata->stat_offset) &
		K3_VTM_TS_STAT_DTEMP_MASK;
	s1 = pete_readl("drivers/thermal/k3_bandgap.c:128", bgp->base + devdata->stat_offset) &
		K3_VTM_TS_STAT_DTEMP_MASK;
	s2 = pete_readl("drivers/thermal/k3_bandgap.c:130", bgp->base + devdata->stat_offset) &
		K3_VTM_TS_STAT_DTEMP_MASK;
...
static int k3_bandgap_probe(struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/thermal/k3_bandgap.c:190", bgp->base + K3_VTM_DEVINFO_PWR0_OFFSET);
	...
	for (id = 0; id < cnt; id++) {
		...
		val = pete_readl("drivers/thermal/k3_bandgap.c:208", data[id].bgp->base + data[id].ctrl_offset);
		val |= (K3_VTM_TMPSENS_CTRL_SOC |
			K3_VTM_TMPSENS_CTRL_CLRZ |
			K3_VTM_TMPSENS_CTRL_CLKON_REQ);
		val &= ~K3_VTM_TMPSENS_CTRL_CBIASSEL;
		pete_writel("drivers/thermal/k3_bandgap.c:213", val, data[id].bgp->base + data[id].ctrl_offset);
