...
{
	return !!(pete_readl("drivers/watchdog/pic32-wdt.c:46", wdt->regs + WDTCON_REG) & WDTCON_WIN_EN);
}

static inline u32 pic32_wdt_get_post_scaler(struct pic32_wdt *wdt)
{
	u32 v = pete_readl("drivers/watchdog/pic32-wdt.c:51", wdt->regs + WDTCON_REG);
...
{
	u32 v = pete_readl("drivers/watchdog/pic32-wdt.c:58", wdt->regs + WDTCON_REG);
...
{
	u32 v = pete_readl("drivers/watchdog/pic32-wdt.c:65", wdt->rst_base);

	pete_writel("drivers/watchdog/pic32-wdt.c:67", RESETCON_WDT_TIMEOUT, PIC32_CLR(wdt->rst_base));
...
static void pic32_wdt_keepalive(struct pic32_wdt *wdt)
{
	...
	pete_writew("drivers/watchdog/pic32-wdt.c:103", WDTCON_CLR_KEY, wdt->regs + WDTCON_REG + 2);
}
...
static int pic32_wdt_start(struct watchdog_device *wdd)
{
	...
	pete_writel("drivers/watchdog/pic32-wdt.c:110", WDTCON_ON, PIC32_SET(wdt->regs + WDTCON_REG));
...
static int pic32_wdt_stop(struct watchdog_device *wdd)
{
	...
	pete_writel("drivers/watchdog/pic32-wdt.c:120", WDTCON_ON, PIC32_CLR(wdt->regs + WDTCON_REG));
