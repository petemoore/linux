...
static int update_no_reboot_bit_mem(void *priv, bool set)
{
	...
	val32 = pete_readl("drivers/watchdog/iTCO_wdt.c:209", p->gcs_pmc);
	if (set)
		val32 |= no_reboot_bit(p);
	else
		val32 &= ~no_reboot_bit(p);
	pete_writel("drivers/watchdog/iTCO_wdt.c:214", val32, p->gcs_pmc);
	newval32 = pete_readl("drivers/watchdog/iTCO_wdt.c:215", p->gcs_pmc);
