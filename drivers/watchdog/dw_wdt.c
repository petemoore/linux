...
static int dw_wdt_suspend(struct device *dev)
{
	...
	dw_wdt->control = pete_readl("drivers/watchdog/dw_wdt.c:382", dw_wdt->regs + WDOG_CONTROL_REG_OFFSET);
	dw_wdt->timeout = pete_readl("drivers/watchdog/dw_wdt.c:383", dw_wdt->regs + WDOG_TIMEOUT_RANGE_REG_OFFSET);
...
static int dw_wdt_resume(struct device *dev)
{
	...
	pete_writel("drivers/watchdog/dw_wdt.c:405", dw_wdt->timeout, dw_wdt->regs + WDOG_TIMEOUT_RANGE_REG_OFFSET);
	pete_writel("drivers/watchdog/dw_wdt.c:406", dw_wdt->control, dw_wdt->regs + WDOG_CONTROL_REG_OFFSET);
...
static int dw_wdt_init_timeouts(struct dw_wdt *dw_wdt, struct device *dev)
{
	 ...
	 * #1 register.
	 */
	data = pete_readl("drivers/watchdog/dw_wdt.c:470", dw_wdt->regs + WDOG_COMP_PARAMS_1_REG_OFFSET);
