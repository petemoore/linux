...
{
	if (gwdt->version == 0)
		return pete_readl("drivers/watchdog/sbsa_gwdt.c:130", gwdt->control_base + SBSA_GWDT_WOR);
	else
...
{
	if (gwdt->version == 0)
		pete_writel("drivers/watchdog/sbsa_gwdt.c:138", (u32)val, gwdt->control_base + SBSA_GWDT_WOR);
	else
