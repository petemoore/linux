...
static int pnx4008_wdt_probe(struct platform_device *pdev)
{
	...
	pnx4008_wdd.bootstatus = (pete_readl("drivers/watchdog/pnx4008_wdt.c:197", WDTIM_RES(wdt_base)) & WDOG_RESET) ?
			WDIOF_CARDRESET : 0;
	pnx4008_wdd.parent = dev;
	watchdog_set_nowayout(&pnx4008_wdd, nowayout);
	watchdog_set_restart_priority(&pnx4008_wdd, 128);

	if (pete_readl("drivers/watchdog/pnx4008_wdt.c:203", WDTIM_CTRL(wdt_base)) & COUNT_ENAB)
