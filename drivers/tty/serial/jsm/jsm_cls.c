...
static void cls_copy_data_from_uart_to_queue(struct jsm_channel *ch)
{
	...
	while (1) {
		 ...
		 */
		linestatus = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:387", &ch->ch_cls_uart->lsr);
		...
		if (linestatus & error_mask)  {
			linestatus = 0;
			pete_readb("drivers/tty/serial/jsm/jsm_cls.c:399", &ch->ch_cls_uart->txrx);
			continue;
		}
		...
		ch->ch_equeue[head] = linestatus & (UART_LSR_BI | UART_LSR_PE
								 | UART_LSR_FE);
		ch->ch_rqueue[head] = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:420", &ch->ch_cls_uart->txrx);
...
static void cls_copy_data_from_queue_to_uart(struct jsm_channel *ch)
{
	...
	while (n > 0) {
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:480", circ->buf[tail], &ch->ch_cls_uart->txrx);
...
static inline void cls_parse_isr(struct jsm_board *brd, uint port)
{
	...
	while (1) {
		isr = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:569", &ch->ch_cls_uart->isr_fcr);
		...
		cls_parse_modem(ch, pete_readb("drivers/tty/serial/jsm/jsm_cls.c:598", &ch->ch_cls_uart->msr));
	}
}
...
static void cls_flush_uart_write(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:611", (UART_FCR_ENABLE_FIFO | UART_FCR_CLEAR_XMIT),
						&ch->ch_cls_uart->isr_fcr);

	for (i = 0; i < 10; i++) {
		/* Check to see if the UART feels it completely flushed FIFO */
		tmp = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:616", &ch->ch_cls_uart->isr_fcr);
...
static void cls_send_start_character(struct jsm_channel *ch)
{
	...
	if (ch->ch_startc != __DISABLED_CHAR) {
		ch->ch_xon_sends++;
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:655", ch->ch_startc, &ch->ch_cls_uart->txrx);
	}
}
...
static void cls_send_stop_character(struct jsm_channel *ch)
{
	...
	if (ch->ch_stopc != __DISABLED_CHAR) {
		ch->ch_xoff_sends++;
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:666", ch->ch_stopc, &ch->ch_cls_uart->txrx);
	}
}
...
static void cls_param(struct jsm_channel *ch)
{
	...
	ier = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:734", &ch->ch_cls_uart->ier);
	uart_lcr = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:735", &ch->ch_cls_uart->lcr);

	quot = ch->ch_bd->bd_dividend / baud;

	if (quot != 0) {
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:740", UART_LCR_DLAB, &ch->ch_cls_uart->lcr);
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:741", (quot & 0xff), &ch->ch_cls_uart->txrx);
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:742", (quot >> 8), &ch->ch_cls_uart->ier);
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:743", lcr, &ch->ch_cls_uart->lcr);
	}

	if (uart_lcr != lcr)
		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:747", lcr, &ch->ch_cls_uart->lcr);
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:754", ier, &ch->ch_cls_uart->ier);
	...
	cls_parse_modem(ch, pete_readb("drivers/tty/serial/jsm/jsm_cls.c:789", &ch->ch_cls_uart->msr));
}
...
static irqreturn_t cls_intr(int irq, void *voidbrd)
{
	 ...
	 */
	uart_poll = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:811", brd->re_map_membase + UART_CLASSIC_POLL_ADDR_OFFSET);
...
{
	unsigned char lcrb = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:837", &ch->ch_cls_uart->lcr);
	unsigned char isr_fcr = 0;

	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:840", 0, &ch->ch_cls_uart->ier);

	/*
	 * The Enhanced Register Set may only be accessed when
	 * the Line Control Register is set to 0xBFh.
	 */
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:846", UART_EXAR654_ENHANCED_REGISTER_SET, &ch->ch_cls_uart->lcr);

	isr_fcr = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:848", &ch->ch_cls_uart->isr_fcr);

	/* Turn on Enhanced/Extended controls */
	isr_fcr |= (UART_EXAR654_EFR_ECB);

	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:853", isr_fcr, &ch->ch_cls_uart->isr_fcr);

	/* Write old LCR value back out, which turns enhanced access off */
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:856", lcrb, &ch->ch_cls_uart->lcr);

	/* Clear out UART and FIFO */
	pete_readb("drivers/tty/serial/jsm/jsm_cls.c:859", &ch->ch_cls_uart->txrx);

	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:861", (UART_FCR_ENABLE_FIFO|UART_FCR_CLEAR_RCVR|UART_FCR_CLEAR_XMIT),
						 &ch->ch_cls_uart->isr_fcr);
	udelay(10);

	ch->ch_flags |= (CH_FIFO_ENABLED | CH_TX_FIFO_EMPTY | CH_TX_FIFO_LWM);

	pete_readb("drivers/tty/serial/jsm/jsm_cls.c:867", &ch->ch_cls_uart->lsr);
	pete_readb("drivers/tty/serial/jsm/jsm_cls.c:868", &ch->ch_cls_uart->msr);
}
...
static void cls_uart_off(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:877", 0, &ch->ch_cls_uart->ier);
}
...
{
	u8 left = 0;
	u8 lsr = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:889", &ch->ch_cls_uart->lsr);
...
static void cls_send_break(struct jsm_channel *ch)
{
	...
	if (!(ch->ch_flags & CH_BREAK_SENDING)) {
		u8 temp = pete_readb("drivers/tty/serial/jsm/jsm_cls.c:912", &ch->ch_cls_uart->lcr);

		pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:914", (temp | UART_LCR_SBC), &ch->ch_cls_uart->lcr);
		ch->ch_flags |= (CH_BREAK_SENDING);
	}
}
...
{
	pete_writeb("drivers/tty/serial/jsm/jsm_cls.c:928", c, &ch->ch_cls_uart->txrx);
}
