...
static void neo_copy_data_from_uart_to_queue(struct jsm_channel *ch)
{
	...
	if (!(ch->ch_flags & CH_FIFO_ENABLED))
		...
		total = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:308", &ch->ch_neo_uart->rfifo);
	...
	while (total > 0) {
		 ...
		 */
		linestatus = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:334", &ch->ch_neo_uart->lsr);
	...
	while (1) {
		 ...
		 */
		linestatus |= pete_readb("drivers/tty/serial/jsm/jsm_neo.c:398", &ch->ch_neo_uart->lsr);
...
static void neo_copy_data_from_queue_to_uart(struct jsm_channel *ch)
{
	 ...
	 */
	if (!(ch->ch_flags & CH_FIFO_ENABLED)) {
		u8 lsrbits = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:501", &ch->ch_neo_uart->lsr);

		ch->ch_cached_lsr |= lsrbits;
		if (ch->ch_cached_lsr & UART_LSR_THRE) {
			ch->ch_cached_lsr &= ~(UART_LSR_THRE);

			pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:507", circ->buf[circ->tail], &ch->ch_neo_uart->txrx);
...
static void neo_assert_modem_signals(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:611", ch->ch_mostat, &ch->ch_neo_uart->mcr);
...
static void neo_flush_uart_write(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:630", (UART_FCR_ENABLE_FIFO | UART_FCR_CLEAR_XMIT), &ch->ch_neo_uart->isr_fcr);

	for (i = 0; i < 10; i++) {

		/* Check to see if the UART feels it completely flushed the FIFO. */
		tmp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:635", &ch->ch_neo_uart->isr_fcr);
...
static void neo_flush_uart_read(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:662", (UART_FCR_ENABLE_FIFO | UART_FCR_CLEAR_RCVR), &ch->ch_neo_uart->isr_fcr);

	for (i = 0; i < 10; i++) {

		/* Check to see if the UART feels it completely flushed the FIFO. */
		tmp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:667", &ch->ch_neo_uart->isr_fcr);
		if (tmp & 2) {
			jsm_dbg(IOCTL, &ch->ch_bd->pci_dev,
...
static void neo_clear_break(struct jsm_channel *ch)
{
	...
	if (ch->ch_flags & CH_BREAK_SENDING) {
		u8 temp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:689", &ch->ch_neo_uart->lcr);
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:690", (temp & ~UART_LCR_SBC), &ch->ch_neo_uart->lcr);
...
static void neo_parse_isr(struct jsm_board *brd, u32 port)
{
	...
	while (1) {
		...
		isr = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:726", &ch->ch_neo_uart->isr_fcr);
		...
		if (isr & UART_17158_IIR_XONXOFF) {
			cause = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:759", &ch->ch_neo_uart->xoffchar1);
		...
		if (isr & UART_17158_IIR_HWFLOW_STATE_CHANGE) {
			 ...
			 */
			cause = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:798", &ch->ch_neo_uart->mcr);
		...
		neo_parse_modem(ch, pete_readb("drivers/tty/serial/jsm/jsm_neo.c:820", &ch->ch_neo_uart->msr));
...
static inline void neo_parse_lsr(struct jsm_board *brd, u32 port)
{
	...
	linestatus = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:841", &ch->ch_neo_uart->lsr);
...
static void neo_param(struct jsm_channel *ch)
{
	...
	ier = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1008", &ch->ch_neo_uart->ier);
	uart_lcr = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1009", &ch->ch_neo_uart->lcr);

	quot = ch->ch_bd->bd_dividend / baud;

	if (quot != 0) {
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1014", UART_LCR_DLAB, &ch->ch_neo_uart->lcr);
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1015", (quot & 0xff), &ch->ch_neo_uart->txrx);
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1016", (quot >> 8), &ch->ch_neo_uart->ier);
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1017", lcr, &ch->ch_neo_uart->lcr);
	}

	if (uart_lcr != lcr)
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1021", lcr, &ch->ch_neo_uart->lcr);
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1028", ier, &ch->ch_neo_uart->ier);
	 ...
	 */
	if (baud < 9600) {
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1062", 1, &ch->ch_neo_uart->rfifo);
		ch->ch_r_tlevel = 1;
	}
	...
	neo_parse_modem(ch, pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1069", &ch->ch_neo_uart->msr));
	return;
}
...
static irqreturn_t neo_intr(int irq, void *voidbrd)
{
	 ...
	 */
	uart_poll = pete_readl("drivers/tty/serial/jsm/jsm_neo.c:1099", brd->re_map_membase + UART_17158_POLL_ADDR_OFFSET);
...
{
	u8 tmp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1227", &ch->ch_neo_uart->ier);
	tmp &= ~(UART_IER_RDI);
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1229", tmp, &ch->ch_neo_uart->ier);
...
{
	u8 tmp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1243", &ch->ch_neo_uart->ier);
	tmp |= (UART_IER_RDI);
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1245", tmp, &ch->ch_neo_uart->ier);
...
static void neo_send_start_character(struct jsm_channel *ch)
{
	...
	if (ch->ch_startc != __DISABLED_CHAR) {
		ch->ch_xon_sends++;
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1258", ch->ch_startc, &ch->ch_neo_uart->txrx);
...
static void neo_send_stop_character(struct jsm_channel *ch)
{
	...
	if (ch->ch_stopc != __DISABLED_CHAR) {
		ch->ch_xoff_sends++;
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1272", ch->ch_stopc, &ch->ch_neo_uart->txrx);
...
{
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1284", 0, &ch->ch_neo_uart->ier);
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1285", 0, &ch->ch_neo_uart->efr);
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1286", UART_EFR_ECB, &ch->ch_neo_uart->efr);

	/* Clear out UART and FIFO */
	pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1289", &ch->ch_neo_uart->txrx);
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1290", (UART_FCR_ENABLE_FIFO|UART_FCR_CLEAR_RCVR|UART_FCR_CLEAR_XMIT), &ch->ch_neo_uart->isr_fcr);
	pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1291", &ch->ch_neo_uart->lsr);
	pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1292", &ch->ch_neo_uart->msr);

	ch->ch_flags |= CH_FIFO_ENABLED;

	/* Assert any signals we want up */
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1297", ch->ch_mostat, &ch->ch_neo_uart->mcr);
}
...
static void neo_uart_off(struct jsm_channel *ch)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1306", 0, &ch->ch_neo_uart->efr);

	/* Stop all interrupts from occurring. */
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1309", 0, &ch->ch_neo_uart->ier);
}

static u32 neo_get_uart_bytes_left(struct jsm_channel *ch)
{
	u8 left = 0;
	u8 lsr = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1315", &ch->ch_neo_uart->lsr);
...
static void neo_send_break(struct jsm_channel *ch)
{
	...
	if (!(ch->ch_flags & CH_BREAK_SENDING)) {
		u8 temp = pete_readb("drivers/tty/serial/jsm/jsm_neo.c:1342", &ch->ch_neo_uart->lcr);
		pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1343", (temp | UART_LCR_SBC), &ch->ch_neo_uart->lcr);
		ch->ch_flags |= (CH_BREAK_SENDING);
...
static void neo_send_immediate_char(struct jsm_channel *ch, unsigned char c)
{
	...
	pete_writeb("drivers/tty/serial/jsm/jsm_neo.c:1364", c, &ch->ch_neo_uart->txrx);
