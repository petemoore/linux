...
static void wr_reg_barrier(const struct uart_port *port, u32 reg, u32 val)
{
	switch (port->iotype) {
	case UPIO_MEM:
		pete_writeb("drivers/tty/serial/samsung_tty.c:2760", val, portaddr(port, reg));
		break;
	case UPIO_MEM32:
		pete_writel("drivers/tty/serial/samsung_tty.c:2763", val, portaddr(port, reg));
		break;
	}
}
...
{
	while (!(pete_readl("drivers/tty/serial/samsung_tty.c:2775", port->membase + S3C2410_UTRSTAT) & S3C2410_UTRSTAT_TXFE))
		;
}
...
static void samsung_early_busyuart_fifo(const struct uart_port *port)
{
	...
	while (pete_readl("drivers/tty/serial/samsung_tty.c:2783", port->membase + S3C2410_UFSTAT) & data->txfull_mask)
		;
}

static void samsung_early_putc(struct uart_port *port, unsigned char c)
{
	if (pete_readl("drivers/tty/serial/samsung_tty.c:2789", port->membase + S3C2410_UFCON) & S3C2410_UFCON_FIFOMODE)
