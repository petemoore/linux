...
static __inline__ void sunsab_tec_wait(struct uart_sunsab_port *up)
{
	...
	while ((pete_readb("drivers/tty/serial/sunsab.c:96", &up->regs->r.star) & SAB82532_STAR_TEC) && --timeout)
		udelay(1);
}
...
static __inline__ void sunsab_cec_wait(struct uart_sunsab_port *up)
{
	...
	while ((pete_readb("drivers/tty/serial/sunsab.c:104", &up->regs->r.star) & SAB82532_STAR_CEC) && --timeout)
		udelay(1);
}
	      ...
	      union sab82532_irq_status *stat)
{
	...
	if (stat->sreg.isr0 & SAB82532_ISR0_TCD) {
		count = pete_readb("drivers/tty/serial/sunsab.c:129", &up->regs->r.rbcl) & (SAB82532_RECV_FIFO_SIZE - 1);
		free_fifo++;
	}
	...
	if (stat->sreg.isr0 & SAB82532_ISR0_TIME) {
		sunsab_cec_wait(up);
		pete_writeb("drivers/tty/serial/sunsab.c:136", SAB82532_CMDR_RFRD, &up->regs->w.cmdr);
		return port;
	}
	...
	/* Read the FIFO. */
	for (i = 0; i < count; i++)
		buf[i] = pete_readb("drivers/tty/serial/sunsab.c:145", &up->regs->r.rfifo[i]);

	/* Issue Receive Message Complete command. */
	if (free_fifo) {
		sunsab_cec_wait(up);
		pete_writeb("drivers/tty/serial/sunsab.c:150", SAB82532_CMDR_RMC, &up->regs->w.cmdr);
	}
			   ...
			   union sab82532_irq_status *stat)
{
	...
	if (stat->sreg.isr1 & SAB82532_ISR1_ALLS) {
		...
		pete_writeb("drivers/tty/serial/sunsab.c:240", up->interrupt_mask1, &up->regs->w.imr1);
	...
	if (!(pete_readb("drivers/tty/serial/sunsab.c:249", &up->regs->r.star) & SAB82532_STAR_XFW))
		return;
	...
	if (uart_circ_empty(xmit) || uart_tx_stopped(&up->port)) {
		...
		pete_writeb("drivers/tty/serial/sunsab.c:257", up->interrupt_mask1, &up->regs->w.imr1);
		return;
	}

	up->interrupt_mask1 &= ~(SAB82532_IMR1_ALLS|SAB82532_IMR1_XPR);
	pete_writeb("drivers/tty/serial/sunsab.c:262", up->interrupt_mask1, &up->regs->w.imr1);
	clear_bit(SAB82532_ALLS, &up->irqflags);

	/* Stuff 32 bytes into Transmit FIFO. */
	clear_bit(SAB82532_XPR, &up->irqflags);
	for (i = 0; i < up->port.fifosize; i++) {
		pete_writeb("drivers/tty/serial/sunsab.c:268", xmit->buf[xmit->tail],
		       &up->regs->w.xfifo[i]);
...
static void sunsab_start_tx(struct uart_port *port)
{
	...
	/* Issue a Transmit Frame command.  */
	sunsab_cec_wait(up);
	pete_writeb("drivers/tty/serial/sunsab.c:463", SAB82532_CMDR_XF, &up->regs->w.cmdr);
}
...
static void sunsab_send_xchar(struct uart_port *port, char ch)
{
	...
	sunsab_tec_wait(up);
	pete_writeb("drivers/tty/serial/sunsab.c:479", ch, &up->regs->w.tic);
...
static void sunsab_stop_rx(struct uart_port *port)
{
	...
	pete_writeb("drivers/tty/serial/sunsab.c:491", up->interrupt_mask1, &up->regs->w.imr0);
}
...
static int sunsab_startup(struct uart_port *port)
{
	 ...
	 */
	pete_writeb("drivers/tty/serial/sunsab.c:541", SAB82532_CMDR_RRES, &up->regs->w.cmdr);
	sunsab_cec_wait(up);
	pete_writeb("drivers/tty/serial/sunsab.c:543", SAB82532_CMDR_XRES, &up->regs->w.cmdr);

	/*
	 * Clear the interrupt registers.
	 */
	(void) pete_readb("drivers/tty/serial/sunsab.c:548", &up->regs->r.isr0);
	(void) pete_readb("drivers/tty/serial/sunsab.c:549", &up->regs->r.isr1);

	/*
	 * Now, initialize the UART 
	 */
	pete_writeb("drivers/tty/serial/sunsab.c:554", 0, &up->regs->w.ccr0);				/* power-down */
	pete_writeb("drivers/tty/serial/sunsab.c:555", SAB82532_CCR0_MCE | SAB82532_CCR0_SC_NRZ |
	       SAB82532_CCR0_SM_ASYNC, &up->regs->w.ccr0);
	pete_writeb("drivers/tty/serial/sunsab.c:557", SAB82532_CCR1_ODS | SAB82532_CCR1_BCR | 7, &up->regs->w.ccr1);
	pete_writeb("drivers/tty/serial/sunsab.c:558", SAB82532_CCR2_BDF | SAB82532_CCR2_SSEL |
	       SAB82532_CCR2_TOE, &up->regs->w.ccr2);
	pete_writeb("drivers/tty/serial/sunsab.c:560", 0, &up->regs->w.ccr3);
	pete_writeb("drivers/tty/serial/sunsab.c:561", SAB82532_CCR4_MCK4 | SAB82532_CCR4_EBRG, &up->regs->w.ccr4);
	up->cached_mode = (SAB82532_MODE_RTS | SAB82532_MODE_FCTS |
			   SAB82532_MODE_RAC);
	pete_writeb("drivers/tty/serial/sunsab.c:564", up->cached_mode, &up->regs->w.mode);
	pete_writeb("drivers/tty/serial/sunsab.c:565", SAB82532_RFC_DPS|SAB82532_RFC_RFTH_32, &up->regs->w.rfc);
	
	tmp = pete_readb("drivers/tty/serial/sunsab.c:567", &up->regs->rw.ccr0);
	tmp |= SAB82532_CCR0_PU;	/* power-up */
	pete_writeb("drivers/tty/serial/sunsab.c:569", tmp, &up->regs->rw.ccr0);
	...
	up->interrupt_mask0 = (SAB82532_IMR0_PERR | SAB82532_IMR0_FERR |
			       SAB82532_IMR0_PLLA);
	pete_writeb("drivers/tty/serial/sunsab.c:576", up->interrupt_mask0, &up->regs->w.imr0);
	up->interrupt_mask1 = (SAB82532_IMR1_BRKT | SAB82532_IMR1_ALLS |
			       SAB82532_IMR1_XOFF | SAB82532_IMR1_TIN |
			       SAB82532_IMR1_CSC | SAB82532_IMR1_XON |
			       SAB82532_IMR1_XPR);
	pete_writeb("drivers/tty/serial/sunsab.c:581", up->interrupt_mask1, &up->regs->w.imr1);
	set_bit(SAB82532_ALLS, &up->irqflags);
	set_bit(SAB82532_XPR, &up->irqflags);
...
static void sunsab_shutdown(struct uart_port *port)
{
	...
	pete_writeb("drivers/tty/serial/sunsab.c:601", up->interrupt_mask0, &up->regs->w.imr0);
	up->interrupt_mask1 = 0xff;
	pete_writeb("drivers/tty/serial/sunsab.c:603", up->interrupt_mask1, &up->regs->w.imr1);

	/* Disable break condition */
	up->cached_dafo = pete_readb("drivers/tty/serial/sunsab.c:606", &up->regs->rw.dafo);
	up->cached_dafo &= ~SAB82532_DAFO_XBRK;
	pete_writeb("drivers/tty/serial/sunsab.c:608", up->cached_dafo, &up->regs->rw.dafo);

	/* Disable Receiver */	
	up->cached_mode &= ~SAB82532_MODE_RAC;
	pete_writeb("drivers/tty/serial/sunsab.c:612", up->cached_mode, &up->regs->rw.mode);
...
#if 0
	/* Power Down */	
	tmp = pete_readb("drivers/tty/serial/sunsab.c:626", &up->regs->rw.ccr0);
	tmp &= ~SAB82532_CCR0_PU;
	pete_writeb("drivers/tty/serial/sunsab.c:628", tmp, &up->regs->rw.ccr0);
#endif
...
static void sunsab_console_putchar(struct uart_port *port, unsigned char c)
{
	...
	sunsab_tec_wait(up);
	pete_writeb("drivers/tty/serial/sunsab.c:850", c, &up->regs->w.tic);
}
...
static int sunsab_console_setup(struct console *con, char *options)
{
	...
	pete_writeb("drivers/tty/serial/sunsab.c:924", up->interrupt_mask0, &up->regs->w.imr0);
	up->interrupt_mask1 = SAB82532_IMR1_BRKT | SAB82532_IMR1_ALLS |
				SAB82532_IMR1_XOFF | SAB82532_IMR1_TIN |
				SAB82532_IMR1_CSC | SAB82532_IMR1_XON |
				SAB82532_IMR1_XPR;
	pete_writeb("drivers/tty/serial/sunsab.c:929", up->interrupt_mask1, &up->regs->w.imr1);
				     ...
				     int line)
{
	...
	pete_writeb("drivers/tty/serial/sunsab.c:981", SAB82532_IPC_IC_ACT_LOW, &up->regs->w.ipc);

	up->port.ops = &sunsab_pops;
	up->port.type = PORT_SUNSAB;
	up->port.uartclk = SAB_BASE_BAUD;

	up->type = pete_readb("drivers/tty/serial/sunsab.c:987", &up->regs->r.vstr) & 0x0f;
	pete_writeb("drivers/tty/serial/sunsab.c:988", ~((1 << 1) | (1 << 2) | (1 << 4)), &up->regs->w.pcr);
	pete_writeb("drivers/tty/serial/sunsab.c:989", 0xff, &up->regs->w.pim);
	...
	pete_writeb("drivers/tty/serial/sunsab.c:1000", up->cached_pvr, &up->regs->w.pvr);
	up->cached_mode = pete_readb("drivers/tty/serial/sunsab.c:1001", &up->regs->rw.mode);
	up->cached_mode |= SAB82532_MODE_FRTS;
	pete_writeb("drivers/tty/serial/sunsab.c:1003", up->cached_mode, &up->regs->rw.mode);
	up->cached_mode |= SAB82532_MODE_RTS;
	pete_writeb("drivers/tty/serial/sunsab.c:1005", up->cached_mode, &up->regs->rw.mode);
