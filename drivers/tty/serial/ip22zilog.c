...
ack_tx_int:
	pete_writeb("drivers/tty/serial/ip22zilog.c:420", RES_Tx_P, &channel->control);
	ZSDELAY();
	ZS_WSYNC(channel);
}
...
static irqreturn_t ip22zilog_interrupt(int irq, void *dev_id)
{
	...
	while (up) {
		...
		if (r3 & (CHAEXT | CHATxIP | CHARxIP)) {
			pete_writeb("drivers/tty/serial/ip22zilog.c:440", RES_H_IUS, &channel->control);
			ZSDELAY();
			ZS_WSYNC(channel);
		...
		if (r3 & (CHBEXT | CHBTxIP | CHBRxIP)) {
			pete_writeb("drivers/tty/serial/ip22zilog.c:463", RES_H_IUS, &channel->control);
			ZSDELAY();
			ZS_WSYNC(channel);
...
static __inline__ unsigned char ip22zilog_read_channel_status(struct uart_port *port)
{
	...
	status = pete_readb("drivers/tty/serial/ip22zilog.c:494", &channel->control);
	ZSDELAY();
...
static void ip22zilog_start_tx(struct uart_port *port)
{
	...
	status = pete_readb("drivers/tty/serial/ip22zilog.c:585", &channel->control);
	ZSDELAY();
	 ...
	 */
	if (port->x_char) {
		pete_writeb("drivers/tty/serial/ip22zilog.c:596", port->x_char, &channel->data);
		ZSDELAY();
		ZS_WSYNC(channel);
		...
		if (uart_circ_empty(xmit))
			return;
		pete_writeb("drivers/tty/serial/ip22zilog.c:607", xmit->buf[xmit->tail], &channel->data);
		ZSDELAY();
		ZS_WSYNC(channel);
...
static void __ip22zilog_startup(struct uart_ip22zilog_port *up)
{
	...
	up->prev_status = pete_readb("drivers/tty/serial/ip22zilog.c:720", &channel->control);
...
static void ip22zilog_put_char(struct uart_port *port, unsigned char ch)
{
	 ...
	 */
	do {
		unsigned char val = pete_readb("drivers/tty/serial/ip22zilog.c:1000", &channel->control);
		if (val & Tx_BUF_EMP) {
			ZSDELAY();
			break;
		}
	...
	pete_writeb("drivers/tty/serial/ip22zilog.c:1008", ch, &channel->data);
	ZSDELAY();
	ZS_WSYNC(channel);
}
