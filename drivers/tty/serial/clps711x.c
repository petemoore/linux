...
static irqreturn_t uart_clps711x_int_rx(int irq, void *dev_id)
{
	...
	for (;;) {
		...
		ch = pete_readw("drivers/tty/serial/clps711x.c:106", port->membase + UARTDR_OFFSET);
...
static irqreturn_t uart_clps711x_int_tx(int irq, void *dev_id)
{
	...
	if (port->x_char) {
		pete_writew("drivers/tty/serial/clps711x.c:152", port->x_char, port->membase + UARTDR_OFFSET);
		port->icount.tx++;
		port->x_char = 0;
		return IRQ_HANDLED;
	}
	...
	while (!uart_circ_empty(xmit)) {
		...
		pete_writew("drivers/tty/serial/clps711x.c:169", xmit->buf[xmit->tail], port->membase + UARTDR_OFFSET);
