...
static void sprd_putc(struct uart_port *port, unsigned char c)
{
	...
	while (timeout-- &&
	       !(pete_readl("drivers/tty/serial/sprd_serial.c:1040", port->membase + SPRD_LSR) & SPRD_LSR_TX_OVER))
		cpu_relax();

	pete_writeb("drivers/tty/serial/sprd_serial.c:1043", c, port->membase + SPRD_TXD);
}
