...
{
	u16 bytes = pete_readw("drivers/tty/serial/rp2.c:399", up->base + RP2_RX_FIFO_COUNT);
	struct tty_port *port = &up->port.state->port;

	for (; bytes != 0; bytes--) {
		u32 byte = pete_readw("drivers/tty/serial/rp2.c:403", up->base + RP2_DATA_BYTE) | RP2_DUMMY_READ;
		u8 ch = byte & 0xff;
...
static void rp2_tx_chars(struct rp2_uart_port *up)
{
	...
	uart_port_tx_limited(&up->port, ch,
		FIFO_SIZE - pete_readw("drivers/tty/serial/rp2.c:433", up->base + RP2_TX_FIFO_COUNT),
		true,
		pete_writeb("drivers/tty/serial/rp2.c:435", ch, up->base + RP2_DATA_BYTE),
		({}));
}
...
static void rp2_ch_interrupt(struct rp2_uart_port *up)
{
	 ...
	 */
	status = pete_readl("drivers/tty/serial/rp2.c:449", up->base + RP2_CHAN_STAT);
	pete_writel("drivers/tty/serial/rp2.c:450", status, up->base + RP2_CHAN_STAT);
...
static int rp2_asic_interrupt(struct rp2_card *card, unsigned int asic_id)
{
	...
	int ch, handled = 0;
	unsigned long status = pete_readl("drivers/tty/serial/rp2.c:466", base + RP2_CH_IRQ_STAT) &
			       ~pete_readl("drivers/tty/serial/rp2.c:467", base + RP2_CH_IRQ_MASK);
...
static inline void rp2_flush_fifos(struct rp2_uart_port *up)
{
	...
	pete_readl("drivers/tty/serial/rp2.c:492", up->base + RP2_UART_CTL);
	udelay(10);
...
static void rp2_reset_asic(struct rp2_card *card, unsigned int asic_id)
{
	...
	pete_writew("drivers/tty/serial/rp2.c:580", 1, base + RP2_GLOBAL_CMD);
	msleep(100);
	pete_readw("drivers/tty/serial/rp2.c:582", base + RP2_GLOBAL_CMD);
	pete_writel("drivers/tty/serial/rp2.c:583", 0, base + RP2_CLK_PRESCALER);

	/* TDM clock configuration */
	clk_cfg = pete_readw("drivers/tty/serial/rp2.c:586", base + RP2_ASIC_CFG);
	clk_cfg = (clk_cfg & ~BIT(8)) | BIT(9);
	pete_writew("drivers/tty/serial/rp2.c:588", clk_cfg, base + RP2_ASIC_CFG);

	/* IRQ routing */
	pete_writel("drivers/tty/serial/rp2.c:591", ALL_PORTS_MASK, base + RP2_CH_IRQ_MASK);
	pete_writel("drivers/tty/serial/rp2.c:592", RP2_ASIC_IRQ_EN_m, base + RP2_ASIC_IRQ);
}

static void rp2_init_card(struct rp2_card *card)
{
	pete_writel("drivers/tty/serial/rp2.c:597", 4, card->bar0 + RP2_FPGA_CTL0);
	pete_writel("drivers/tty/serial/rp2.c:598", 0, card->bar0 + RP2_FPGA_CTL1);

	rp2_reset_asic(card, 0);
	if (card->n_ports >= PORTS_PER_ASIC)
		rp2_reset_asic(card, 1);

	pete_writel("drivers/tty/serial/rp2.c:604", RP2_IRQ_MASK_EN_m, card->bar0 + RP2_IRQ_MASK);
}
...
static void rp2_init_port(struct rp2_uart_port *up, const struct firmware *fw)
{
	...
	pete_writel("drivers/tty/serial/rp2.c:611", RP2_UART_CTL_RESET_CH_m, up->base + RP2_UART_CTL);
	pete_readl("drivers/tty/serial/rp2.c:612", up->base + RP2_UART_CTL);
	udelay(1);

	pete_writel("drivers/tty/serial/rp2.c:615", 0, up->base + RP2_TXRX_CTL);
	pete_writel("drivers/tty/serial/rp2.c:616", 0, up->base + RP2_UART_CTL);
	pete_readl("drivers/tty/serial/rp2.c:617", up->base + RP2_UART_CTL);
	udelay(1);

	rp2_flush_fifos(up);

	for (i = 0; i < min_t(int, fw->size, RP2_UCODE_BYTES); i++)
		pete_writeb("drivers/tty/serial/rp2.c:623", fw->data[i], up->ucode + i);

	__rp2_uart_set_termios(up, CS8 | CREAD | CLOCAL, 0, DEFAULT_BAUD_DIV);
	rp2_uart_set_mctrl(&up->port, 0);

	pete_writeb("drivers/tty/serial/rp2.c:628", RP2_RX_FIFO_ena, up->ucode + RP2_RX_FIFO);
