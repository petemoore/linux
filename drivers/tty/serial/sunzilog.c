...
ack_tx_int:
	pete_writeb("drivers/tty/serial/sunzilog.c:519", RES_Tx_P, &channel->control);
	ZSDELAY();
	ZS_WSYNC(channel);
}
...
static irqreturn_t sunzilog_interrupt(int irq, void *dev_id)
{
	...
	while (up) {
		...
		if (r3 & (CHAEXT | CHATxIP | CHARxIP)) {
			pete_writeb("drivers/tty/serial/sunzilog.c:540", RES_H_IUS, &channel->control);
			ZSDELAY();
			ZS_WSYNC(channel);
		...
		if (r3 & (CHBEXT | CHBTxIP | CHBRxIP)) {
			pete_writeb("drivers/tty/serial/sunzilog.c:563", RES_H_IUS, &channel->control);
			ZSDELAY();
			ZS_WSYNC(channel);
...
static __inline__ unsigned char sunzilog_read_channel_status(struct uart_port *port)
{
	...
	status = pete_readb("drivers/tty/serial/sunzilog.c:594", &channel->control);
	ZSDELAY();
...
static void sunzilog_start_tx(struct uart_port *port)
{
	...
	status = pete_readb("drivers/tty/serial/sunzilog.c:685", &channel->control);
	ZSDELAY();
	 ...
	 */
	if (port->x_char) {
		pete_writeb("drivers/tty/serial/sunzilog.c:696", port->x_char, &channel->data);
		ZSDELAY();
		ZS_WSYNC(channel);
		...
		if (uart_circ_empty(xmit))
			return;
		pete_writeb("drivers/tty/serial/sunzilog.c:707", xmit->buf[xmit->tail], &channel->data);
		ZSDELAY();
		ZS_WSYNC(channel);
...
static void __sunzilog_startup(struct uart_sunzilog_port *up)
{
	...
	channel = ZILOG_CHANNEL_FROM_PORT(&up->port);
	up->prev_status = pete_readb("drivers/tty/serial/sunzilog.c:785", &channel->control);
...
static int sunzilog_get_poll_char(struct uart_port *port)
{
	...
	r1 = read_zsreg(channel, R1);
	if (r1 & (PAR_ERR | Rx_OVR | CRC_ERR)) {
		pete_writeb("drivers/tty/serial/sunzilog.c:1010", ERR_RES, &channel->control);
		ZSDELAY();
		ZS_WSYNC(channel);
	}

	ch = pete_readb("drivers/tty/serial/sunzilog.c:1015", &channel->control);
	ZSDELAY();
	...
	ch = pete_readb("drivers/tty/serial/sunzilog.c:1027", &channel->data);
	ZSDELAY();
...
static void __maybe_unused sunzilog_putchar(struct uart_port *port, unsigned char ch)
{
	 ...
	 */
	do {
		unsigned char val = pete_readb("drivers/tty/serial/sunzilog.c:1135", &channel->control);
		if (val & Tx_BUF_EMP) {
			ZSDELAY();
			break;
		}
	...
	pete_writeb("drivers/tty/serial/sunzilog.c:1143", ch, &channel->data);
	ZSDELAY();
	ZS_WSYNC(channel);
}
