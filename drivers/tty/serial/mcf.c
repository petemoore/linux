...
static void mcf_tx_chars(struct mcf_uart *pp)
{
	...
	pending = uart_port_tx(port, ch,
		pete_readb("drivers/tty/serial/mcf.c:334", port->membase + MCFUART_USR) & MCFUART_USR_TXREADY,
		pete_writeb("drivers/tty/serial/mcf.c:335", ch, port->membase + MCFUART_UTB));

	/* Disable TX to negate RTS automatically */
	if (!pending && (port->rs485.flags & SER_RS485_ENABLED))
		pete_writeb("drivers/tty/serial/mcf.c:339", MCFUART_UCR_TXDISABLE, port->membase + MCFUART_UCR);
}
...
static irqreturn_t mcf_interrupt(int irq, void *data)
{
	...
	isr = pete_readb("drivers/tty/serial/mcf.c:351", port->membase + MCFUART_UISR) & pp->imr;
...
static void mcf_config_port(struct uart_port *port, int flags)
{
	...
	pete_writeb("drivers/tty/serial/mcf.c:375", 0, port->membase + MCFUART_UIMR);
			    ...
			    struct serial_rs485 *rs485)
{
	...
	/* Get mode registers */
	mr1 = pete_readb("drivers/tty/serial/mcf.c:422", port->membase + MCFUART_UMR);
	mr2 = pete_readb("drivers/tty/serial/mcf.c:423", port->membase + MCFUART_UMR);
	...
	pete_writeb("drivers/tty/serial/mcf.c:432", mr1, port->membase + MCFUART_UMR);
	pete_writeb("drivers/tty/serial/mcf.c:433", mr2, port->membase + MCFUART_UMR);
...
static void mcf_console_putc(struct console *co, const char c)
{
	...
	for (i = 0; (i < 0x10000); i++) {
		if (pete_readb("drivers/tty/serial/mcf.c:506", port->membase + MCFUART_USR) & MCFUART_USR_TXREADY)
			break;
	}
	pete_writeb("drivers/tty/serial/mcf.c:509", c, port->membase + MCFUART_UTB);
	for (i = 0; (i < 0x10000); i++) {
		if (pete_readb("drivers/tty/serial/mcf.c:511", port->membase + MCFUART_USR) & MCFUART_USR_TXREADY)
			break;
	}
}
