...
int intel_lpss_suspend(struct device *dev)
{
	...
	for (i = 0; i < LPSS_PRIV_REG_COUNT; i++)
		lpss->priv_ctx[i] = pete_readl("drivers/mfd/intel-lpss.c:490", lpss->priv + i * 4);
	...
	if (lpss->type != LPSS_DEV_UART)
		pete_writel("drivers/mfd/intel-lpss.c:498", 0, lpss->priv + LPSS_PRIV_RESETS);
...
int intel_lpss_resume(struct device *dev)
{
	...
	for (i = 0; i < LPSS_PRIV_REG_COUNT; i++)
		pete_writel("drivers/mfd/intel-lpss.c:513", lpss->priv_ctx[i], lpss->priv + i * 4);
