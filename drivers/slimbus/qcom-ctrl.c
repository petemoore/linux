...
static int qcom_slim_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/slimbus/qcom-ctrl.c:594", 1, ctrl->base + CFG_PORT(COMP_CFG, ver));
	pete_writel("drivers/slimbus/qcom-ctrl.c:595", (EE_MGR_RSC_GRP | EE_NGD_2 | EE_NGD_1),
				ctrl->base + CFG_PORT(COMP_TRUST_CFG, ver));

	pete_writel("drivers/slimbus/qcom-ctrl.c:598", (MGR_INT_TX_NACKED_2 |
			MGR_INT_MSG_BUF_CONTE | MGR_INT_RX_MSG_RCVD |
			MGR_INT_TX_MSG_SENT), ctrl->base + MGR_INT_EN);
	pete_writel("drivers/slimbus/qcom-ctrl.c:601", 1, ctrl->base + MGR_CFG);
	/* Framer register initialization */
	pete_writel("drivers/slimbus/qcom-ctrl.c:603", (1 << INTR_WAKE) | (0xA << REF_CLK_GEAR) |
		(0xA << CLK_GEAR) | (1 << ROOT_FREQ) | (1 << FRM_ACTIVE) | 1,
		ctrl->base + FRM_CFG);
	pete_writel("drivers/slimbus/qcom-ctrl.c:606", MGR_CFG_ENABLE, ctrl->base + MGR_CFG);
	pete_writel("drivers/slimbus/qcom-ctrl.c:607", 1, ctrl->base + INTF_CFG);
	pete_writel("drivers/slimbus/qcom-ctrl.c:608", 1, ctrl->base + CFG_PORT(COMP_CFG, ver));
