...
static void iosapic_wr_irt_entry(struct vector_info *vi, u32 dp0, u32 dp1)
{
	...
	dp0 = pete_readl("drivers/parisc/iosapic.c:537", isp->addr+IOSAPIC_REG_WINDOW);

	iosapic_write(isp->addr, IOSAPIC_IRDT_ENTRY_HI(vi->irqline), dp1);

	/* Read the window register to flush the writes down to HW  */
	dp1 = pete_readl("drivers/parisc/iosapic.c:542", isp->addr+IOSAPIC_REG_WINDOW);
}
...
#ifdef DEBUG_IOSAPIC_IRT
{
	...
	for ( ; t < vi->eoi_addr; t++)
		printk(" %x", pete_readl("drivers/parisc/iosapic.c:623", t));
	printk("\n");
}
