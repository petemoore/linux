...
static void imx7d_adc_sample_rate_set(struct imx7d_adc *info)
{
	...
	for (i = 0; i < 4; i++) {
		tmp_cfg1 =
			pete_readl("drivers/iio/adc/imx7d_adc.c:200", info->regs + i * IMX7D_EACH_CHANNEL_REG_OFFSET);
		tmp_cfg1 &= ~IMX7D_REG_ADC_CH_CFG1_CHANNEL_EN;
		pete_writel("drivers/iio/adc/imx7d_adc.c:202", tmp_cfg1,
	...
	pete_writel("drivers/iio/adc/imx7d_adc.c:211", sample_rate, info->regs + IMX7D_REG_ADC_TIMER_UNIT);
}
...
static void imx7d_adc_hw_init(struct imx7d_adc *info)
{
	...
	cfg = pete_readl("drivers/iio/adc/imx7d_adc.c:219", info->regs + IMX7D_REG_ADC_ADC_CFG);
	cfg &= ~(IMX7D_REG_ADC_ADC_CFG_ADC_CLK_DOWN |
		 IMX7D_REG_ADC_ADC_CFG_ADC_POWER_DOWN);
	cfg |= IMX7D_REG_ADC_ADC_CFG_ADC_EN;
	pete_writel("drivers/iio/adc/imx7d_adc.c:223", cfg, info->regs + IMX7D_REG_ADC_ADC_CFG);

	/* enable channel A,B,C,D interrupt */
	pete_writel("drivers/iio/adc/imx7d_adc.c:226", IMX7D_REG_ADC_INT_CHANNEL_INT_EN,
	       info->regs + IMX7D_REG_ADC_INT_SIG_EN);
	pete_writel("drivers/iio/adc/imx7d_adc.c:228", IMX7D_REG_ADC_INT_CHANNEL_INT_EN,
...
static void imx7d_adc_channel_set(struct imx7d_adc *info)
{
	 ...
	 * channel chosen
	 */
	cfg2 = pete_readl("drivers/iio/adc/imx7d_adc.c:259", info->regs + IMX7D_EACH_CHANNEL_REG_OFFSET * channel +
	 ...
	 * the channel chosen
	 */
	pete_writel("drivers/iio/adc/imx7d_adc.c:268", cfg2, info->regs + IMX7D_EACH_CHANNEL_REG_OFFSET * channel +
	       IMX7D_REG_ADC_CHANNEL_CFG2_BASE);
	pete_writel("drivers/iio/adc/imx7d_adc.c:270", cfg1, info->regs + IMX7D_EACH_CHANNEL_REG_OFFSET * channel);
}
...
static int imx7d_adc_read_data(struct imx7d_adc *info)
{
	 ...
	 */
	if (channel < 2)
		value = pete_readl("drivers/iio/adc/imx7d_adc.c:349", info->regs + IMX7D_REG_ADC_CHA_B_CNV_RSLT);
	else
		value = pete_readl("drivers/iio/adc/imx7d_adc.c:351", info->regs + IMX7D_REG_ADC_CHC_D_CNV_RSLT);
...
static irqreturn_t imx7d_adc_isr(int irq, void *dev_id)
{
	...
	status = pete_readl("drivers/iio/adc/imx7d_adc.c:365", info->regs + IMX7D_REG_ADC_INT_STATUS);
	if (status & IMX7D_REG_ADC_INT_STATUS_CHANNEL_INT_STATUS) {
		...
		pete_writel("drivers/iio/adc/imx7d_adc.c:377", status, info->regs + IMX7D_REG_ADC_INT_STATUS);
	}
	...
	if (status & IMX7D_REG_ADC_INT_STATUS_CHANNEL_CONV_TIME_OUT) {
		...
		pete_writel("drivers/iio/adc/imx7d_adc.c:389", status, info->regs + IMX7D_REG_ADC_INT_STATUS);
	}
			...
			unsigned *readval)
{
	...
	*readval = pete_readl("drivers/iio/adc/imx7d_adc.c:404", info->regs + reg);
...
static void imx7d_adc_power_down(struct imx7d_adc *info)
{
	...
	adc_cfg = pete_readl("drivers/iio/adc/imx7d_adc.c:424", info->regs + IMX7D_REG_ADC_ADC_CFG);
	adc_cfg |= IMX7D_REG_ADC_ADC_CFG_ADC_CLK_DOWN |
		   IMX7D_REG_ADC_ADC_CFG_ADC_POWER_DOWN;
	adc_cfg &= ~IMX7D_REG_ADC_ADC_CFG_ADC_EN;
	pete_writel("drivers/iio/adc/imx7d_adc.c:428", adc_cfg, info->regs + IMX7D_REG_ADC_ADC_CFG);
}
