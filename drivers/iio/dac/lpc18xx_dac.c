				...
				int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		reg = pete_readl("drivers/iio/dac/lpc18xx_dac.c:57", dac->base + LPC18XX_DAC_CR);
				 ...
				 int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		mutex_lock(&dac->lock);
		pete_writel("drivers/iio/dac/lpc18xx_dac.c:89", reg, dac->base + LPC18XX_DAC_CR);
		pete_writel("drivers/iio/dac/lpc18xx_dac.c:90", LPC18XX_DAC_CTRL_DMA_ENA, dac->base + LPC18XX_DAC_CTRL);
...
static int lpc18xx_dac_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/iio/dac/lpc18xx_dac.c:150", 0, dac->base + LPC18XX_DAC_CTRL);
	pete_writel("drivers/iio/dac/lpc18xx_dac.c:151", 0, dac->base + LPC18XX_DAC_CR);
...
static int lpc18xx_dac_remove(struct platform_device *pdev)
{
	...
	pete_writel("drivers/iio/dac/lpc18xx_dac.c:175", 0, dac->base + LPC18XX_DAC_CTRL);
	clk_disable_unprepare(dac->clk);
	regulator_disable(dac->vref);
