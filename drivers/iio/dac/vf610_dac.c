...
static void vf610_dac_init(struct vf610_dac *info)
{
	...
	val = VF610_DAC_DACEN | VF610_DAC_DACRFS |
		VF610_DAC_LPEN;
	pete_writel("drivers/iio/dac/vf610_dac.c:50", val, info->regs + VF610_DACx_STATCTRL);
}
...
static void vf610_dac_exit(struct vf610_dac *info)
{
	...
	val = pete_readl("drivers/iio/dac/vf610_dac.c:57", info->regs + VF610_DACx_STATCTRL);
	val &= ~VF610_DAC_DACEN;
	pete_writel("drivers/iio/dac/vf610_dac.c:59", val, info->regs + VF610_DACx_STATCTRL);
}
				...
				unsigned int mode)
{
	...
	info->conv_mode = mode;
	val = pete_readl("drivers/iio/dac/vf610_dac.c:71", info->regs + VF610_DACx_STATCTRL);
	if (mode)
		val |= VF610_DAC_LPEN;
	else
		val &= ~VF610_DAC_LPEN;
	pete_writel("drivers/iio/dac/vf610_dac.c:76", val, info->regs + VF610_DACx_STATCTRL);
			...
			long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		*val = VF610_DAC_DAT0(pete_readl("drivers/iio/dac/vf610_dac.c:126", info->regs));
		return IIO_VAL_INT;
			...
			long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		mutex_lock(&info->lock);
		pete_writel("drivers/iio/dac/vf610_dac.c:153", VF610_DAC_DAT0(val), info->regs);
		mutex_unlock(&info->lock);
		return 0;
