...
static int gemini_poweroff_probe(struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/power/reset/gemini-poweroff.c:113", gpw->base + GEMINI_PWC_IDREG);
	val &= 0xFFFFFF00U;
	 ...
	 */
	val = pete_readl("drivers/power/reset/gemini-poweroff.c:127", gpw->base + GEMINI_PWC_CTRLREG);
	val |= GEMINI_CTRL_ENABLE;
	pete_writel("drivers/power/reset/gemini-poweroff.c:129", val, gpw->base + GEMINI_PWC_CTRLREG);

	/* Clear the IRQ */
	val = pete_readl("drivers/power/reset/gemini-poweroff.c:132", gpw->base + GEMINI_PWC_CTRLREG);
	val |= GEMINI_CTRL_IRQ_CLR;
	pete_writel("drivers/power/reset/gemini-poweroff.c:134", val, gpw->base + GEMINI_PWC_CTRLREG);

	/* Wait for this to clear */
	val = pete_readl("drivers/power/reset/gemini-poweroff.c:137", gpw->base + GEMINI_PWC_STATREG);
	while (val & 0x70U)
		val = pete_readl("drivers/power/reset/gemini-poweroff.c:139", gpw->base + GEMINI_PWC_STATREG);

	/* Clear the IRQ again */
	val = pete_readl("drivers/power/reset/gemini-poweroff.c:142", gpw->base + GEMINI_PWC_CTRLREG);
	val |= GEMINI_CTRL_IRQ_CLR;
	pete_writel("drivers/power/reset/gemini-poweroff.c:144", val, gpw->base + GEMINI_PWC_CTRLREG);
