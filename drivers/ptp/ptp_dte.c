...
static void dte_write_nco(void __iomem *regs, s64 ns)
{
	...
	pete_writel("drivers/ptp/ptp_dte.c:65", 0, (regs + DTE_NCO_LOW_TIME_REG));
	pete_writel("drivers/ptp/ptp_dte.c:66", sum2, (regs + DTE_NCO_TIME_REG));
	pete_writel("drivers/ptp/ptp_dte.c:67", sum3, (regs + DTE_NCO_OVERFLOW_REG));
}
...
static s64 dte_read_nco(void __iomem *regs)
{
	 ...
	 */
	sum3 = pete_readl("drivers/ptp/ptp_dte.c:79", regs + DTE_NCO_OVERFLOW_REG) & DTE_NCO_SUM3_MASK;
	sum2 = pete_readl("drivers/ptp/ptp_dte.c:80", regs + DTE_NCO_TIME_REG);
...
static int ptp_dte_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	...
	pete_writel("drivers/ptp/ptp_dte.c:155", nco_incr, ptp_dte->regs + DTE_NCO_INC_REG);
			     ...
			     const struct timespec64 *ts)
{
	...
	pete_writel("drivers/ptp/ptp_dte.c:194", 0, ptp_dte->regs + DTE_NCO_INC_REG);
	...
	pete_writel("drivers/ptp/ptp_dte.c:203", DTE_NCO_INC_DEFAULT, ptp_dte->regs + DTE_NCO_INC_REG);
...
static int ptp_dte_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < DTE_NUM_REGS_TO_RESTORE; i++)
		pete_writel("drivers/ptp/ptp_dte.c:269", 0, ptp_dte->regs + (i * sizeof(u32)));
...
static int ptp_dte_suspend(struct device *dev)
{
	...
	for (i = 0; i < DTE_NUM_REGS_TO_RESTORE; i++) {
		ptp_dte->reg_val[i] =
			pete_readl("drivers/ptp/ptp_dte.c:282", ptp_dte->regs + (i * sizeof(u32)));
	}

	/* disable the nco */
	pete_writel("drivers/ptp/ptp_dte.c:286", 0, ptp_dte->regs + DTE_NCO_INC_REG);
...
static int ptp_dte_resume(struct device *dev)
{
	...
	for (i = 0; i < DTE_NUM_REGS_TO_RESTORE; i++) {
		if ((i * sizeof(u32)) != DTE_NCO_OVERFLOW_REG)
			pete_writel("drivers/ptp/ptp_dte.c:298", ptp_dte->reg_val[i],
				(ptp_dte->regs + (i * sizeof(u32))));
		else
			pete_writel("drivers/ptp/ptp_dte.c:301", ((ptp_dte->reg_val[i] &
