					    ...
					    unsigned char signal_voltage)
{
	...
	if (params->pad_ctrl.pad_type == SOC_PAD_FIXED_1_8V) {
		pete_writel("drivers/mmc/host/sdhci-xenon-phy.c:317", ARMADA_3700_SOC_PAD_1_8V, params->pad_ctrl.reg);
	} else if (params->pad_ctrl.pad_type == SOC_PAD_SD) {
		if (signal_voltage == MMC_SIGNAL_VOLTAGE_180)
			pete_writel("drivers/mmc/host/sdhci-xenon-phy.c:320", ARMADA_3700_SOC_PAD_1_8V, params->pad_ctrl.reg);
		else if (signal_voltage == MMC_SIGNAL_VOLTAGE_330)
			pete_writel("drivers/mmc/host/sdhci-xenon-phy.c:322", ARMADA_3700_SOC_PAD_3_3V, params->pad_ctrl.reg);
	}
}
