...
{
	pete_writew("drivers/mmc/host/sdricoh_cs.c:107", value, host->iobase + reg);
...
static inline unsigned int sdricoh_readb(struct sdricoh_host *host,
					 unsigned int reg)
{
	unsigned int value = pete_readb("drivers/mmc/host/sdricoh_cs.c:114", host->iobase + reg);
			    ...
			    struct pcmcia_device *pcmcia_dev)
{
	...
	/* check version? */
	if (pete_readl("drivers/mmc/host/sdricoh_cs.c:399", iobase + R104_VERSION) != 0x4000) {
