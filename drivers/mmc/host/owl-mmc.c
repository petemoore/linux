...
static void owl_mmc_update_reg(void __iomem *reg, unsigned int val, bool state)
{
	...
	regval = pete_readl("drivers/mmc/host/owl-mmc.c:125", reg);
	...
	pete_writel("drivers/mmc/host/owl-mmc.c:132", regval, reg);
}
...
static irqreturn_t owl_irq_handler(int irq, void *devid)
{
	...
	state = pete_readl("drivers/mmc/host/owl-mmc.c:142", owl_host->base + OWL_REG_SD_STATE);
	if (state & OWL_SD_STATE_TEI) {
		state = pete_readl("drivers/mmc/host/owl-mmc.c:144", owl_host->base + OWL_REG_SD_STATE);
		state |= OWL_SD_STATE_TEI;
		pete_writel("drivers/mmc/host/owl-mmc.c:146", state, owl_host->base + OWL_REG_SD_STATE);
			     ...
			     struct mmc_data *data)
{
	...
	mode |= (pete_readl("drivers/mmc/host/owl-mmc.c:223", owl_host->base + OWL_REG_SD_CTL) & (0xff << 16));

	/* Start to send corresponding command type */
	pete_writel("drivers/mmc/host/owl-mmc.c:226", cmd->arg, owl_host->base + OWL_REG_SD_ARG);
	pete_writel("drivers/mmc/host/owl-mmc.c:227", cmd->opcode, owl_host->base + OWL_REG_SD_CMD);
	...
	/* Start transfer */
	pete_writel("drivers/mmc/host/owl-mmc.c:240", mode, owl_host->base + OWL_REG_SD_CTL);
	...
	state = pete_readl("drivers/mmc/host/owl-mmc.c:254", owl_host->base + OWL_REG_SD_STATE);
	if (mmc_resp_type(cmd) & MMC_RSP_PRESENT) {
		...
		if (mmc_resp_type(cmd) & MMC_RSP_136) {
			cmd->resp[3] = pete_readl("drivers/mmc/host/owl-mmc.c:271", owl_host->base + OWL_REG_SD_RSPBUF0);
			cmd->resp[2] = pete_readl("drivers/mmc/host/owl-mmc.c:272", owl_host->base + OWL_REG_SD_RSPBUF1);
			cmd->resp[1] = pete_readl("drivers/mmc/host/owl-mmc.c:273", owl_host->base + OWL_REG_SD_RSPBUF2);
			cmd->resp[0] = pete_readl("drivers/mmc/host/owl-mmc.c:274", owl_host->base + OWL_REG_SD_RSPBUF3);
		} else {
			resp[0] = pete_readl("drivers/mmc/host/owl-mmc.c:276", owl_host->base + OWL_REG_SD_RSPBUF0);
			resp[1] = pete_readl("drivers/mmc/host/owl-mmc.c:277", owl_host->base + OWL_REG_SD_RSPBUF1);
				...
				struct mmc_data *data)
{
	...
	owl_mmc_update_reg(owl_host->base + OWL_REG_SD_EN, OWL_SD_EN_BSEL,
			   true);
	pete_writel("drivers/mmc/host/owl-mmc.c:300", data->blocks, owl_host->base + OWL_REG_SD_BLK_NUM);
	pete_writel("drivers/mmc/host/owl-mmc.c:301", data->blksz, owl_host->base + OWL_REG_SD_BLK_SIZE);
	total = data->blksz * data->blocks;

	if (total < 512)
		pete_writel("drivers/mmc/host/owl-mmc.c:305", total, owl_host->base + OWL_REG_SD_BUF_SIZE);
	else
		pete_writel("drivers/mmc/host/owl-mmc.c:307", 512, owl_host->base + OWL_REG_SD_BUF_SIZE);
				...
				unsigned int rate)
{
	...
	reg = pete_readl("drivers/mmc/host/owl-mmc.c:395", owl_host->base + OWL_REG_SD_CTL);
	reg &= ~OWL_SD_CTL_DELAY_MSK;

	/* Set RDELAY and WDELAY based on the clock */
	if (rate <= 1000000) {
		pete_writel("drivers/mmc/host/owl-mmc.c:400", reg | OWL_SD_CTL_RDELAY(OWL_SD_DELAY_LOW_CLK) |
		       OWL_SD_CTL_WDELAY(OWL_SD_DELAY_LOW_CLK),
		       owl_host->base + OWL_REG_SD_CTL);
	} else if ((rate > 1000000) && (rate <= 26000000)) {
		pete_writel("drivers/mmc/host/owl-mmc.c:404", reg | OWL_SD_CTL_RDELAY(OWL_SD_DELAY_MID_CLK) |
		       OWL_SD_CTL_WDELAY(OWL_SD_DELAY_MID_CLK),
		       owl_host->base + OWL_REG_SD_CTL);
	} else if ((rate > 26000000) && (rate <= 52000000) && !owl_host->ddr_50) {
		pete_writel("drivers/mmc/host/owl-mmc.c:408", reg | OWL_SD_CTL_RDELAY(OWL_SD_DELAY_HIGH_CLK) |
		       OWL_SD_CTL_WDELAY(OWL_SD_DELAY_HIGH_CLK),
		       owl_host->base + OWL_REG_SD_CTL);
	/* DDR50 mode has special delay chain */
	} else if ((rate > 26000000) && (rate <= 52000000) && owl_host->ddr_50) {
		pete_writel("drivers/mmc/host/owl-mmc.c:413", reg | OWL_SD_CTL_RDELAY(OWL_SD_RDELAY_DDR50) |
				  ...
				  struct mmc_ios *ios)
{
	...
	reg = pete_readl("drivers/mmc/host/owl-mmc.c:441", owl_host->base + OWL_REG_SD_EN);
	reg &= ~0x03;
	...
	pete_writel("drivers/mmc/host/owl-mmc.c:454", reg, owl_host->base + OWL_REG_SD_EN);
}
...
static void owl_mmc_power_on(struct owl_mmc_host *owl_host)
{
	...
	/* Send init clk */
	mode = (pete_readl("drivers/mmc/host/owl-mmc.c:475", owl_host->base + OWL_REG_SD_CTL) & (0xff << 16));
	mode |= OWL_SD_CTL_TS | OWL_SD_CTL_TCN(5) | OWL_SD_CTL_TM(8);
	pete_writel("drivers/mmc/host/owl-mmc.c:477", mode, owl_host->base + OWL_REG_SD_CTL);
...
static void owl_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		pete_writel("drivers/mmc/host/owl-mmc.c:496", OWL_SD_ENABLE | OWL_SD_EN_RESE,
