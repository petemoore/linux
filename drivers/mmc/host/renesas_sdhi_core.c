...
{
	return pete_readl("drivers/mmc/host/renesas_sdhi_core.c:335", priv->scc_ctl + (addr << host->bus_shift));
}
...
static inline void sd_scc_write32(struct tmio_mmc_host *host,
				  ...
				  int addr, u32 val)
{
	pete_writel("drivers/mmc/host/renesas_sdhi_core.c:342", val, priv->scc_ctl + (addr << host->bus_shift));
}
