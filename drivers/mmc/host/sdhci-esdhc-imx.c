...
static inline void esdhc_clrset_le(struct sdhci_host *host, u32 mask, u32 val, int reg)
{
	...
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:405", ((pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:405", base) & ~(mask << shift)) | (val << shift)), base);
}
...
static void esdhc_dump_debug_regs(struct sdhci_host *host)
{
	...
	for (i = 0; i < 7; i++) {
		...
		ESDHC_IMX_DUMP("%s:  0x%04x\n", debug_status[i],
			pete_readw("drivers/mmc/host/sdhci-esdhc-imx.c:429", host->ioaddr + ESDHC_DEBUG_SEL_AND_STATUS_REG));
	}
...
static inline void usdhc_auto_tuning_mode_sel_and_en(struct sdhci_host *host)
{
	...
	buswidth = USDHC_GET_BUSWIDTH(pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:455", host->ioaddr + SDHCI_HOST_CONTROL));
	...
	reg = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:487", host->ioaddr + ESDHC_MIX_CTRL);
	reg |= ESDHC_MIX_CTRL_AUTO_TUNE_EN;
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:489", reg, host->ioaddr + ESDHC_MIX_CTRL);
}
...
static u32 esdhc_readl_le(struct sdhci_host *host, int reg)
{
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct pltfm_imx_data *imx_data = sdhci_pltfm_priv(pltfm_host);
	u32 val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:496", host->ioaddr + reg);
	...
	if (unlikely(reg == SDHCI_CAPABILITIES_1)) {
		if (esdhc_is_usdhc(imx_data)) {
			if (imx_data->socdata->flags & ESDHC_FLAG_HAVE_CAP1)
				val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:529", host->ioaddr + SDHCI_CAPABILITIES) & 0xFFFF;
			else
	...
	if (unlikely(reg == SDHCI_INT_STATUS)) {
		...
		if ((imx_data->multiblock_status == WAIT_FOR_INT) &&
			...
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:569", SDHCI_INT_RESPONSE, host->ioaddr +
						   SDHCI_INT_STATUS);
...
static void esdhc_writel_le(struct sdhci_host *host, u32 val, int reg)
{
	...
	if (unlikely(reg == SDHCI_INT_ENABLE || reg == SDHCI_SIGNAL_ENABLE ||
		...
		if ((val & SDHCI_INT_CARD_INT) && !esdhc_is_usdhc(imx_data)) {
			 ...
			 */
			data = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:595", host->ioaddr + SDHCI_HOST_CONTROL);
			data &= ~ESDHC_CTRL_D3CD;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:597", data, host->ioaddr + SDHCI_HOST_CONTROL);
			data |= ESDHC_CTRL_D3CD;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:599", data, host->ioaddr + SDHCI_HOST_CONTROL);
		}
	...
	if (unlikely((imx_data->socdata->flags & ESDHC_FLAG_MULTIBLK_NO_INT)
			...
			u32 v;
			v = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:612", host->ioaddr + ESDHC_VENDOR_SPEC);
			v &= ~ESDHC_VENDOR_SPEC_SDIO_QUIRK;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:614", v, host->ioaddr + ESDHC_VENDOR_SPEC);
			...
			if (imx_data->multiblock_status == MULTIBLK_IN_PROCESS)
			{
				...
				pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:621", data, host->ioaddr + SDHCI_TRANSFER_MODE);
				imx_data->multiblock_status = WAIT_FOR_INT;
			}
	}

	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:626", val, host->ioaddr + reg);
}
...
static u16 esdhc_readw_le(struct sdhci_host *host, int reg)
{
	...
	if (unlikely(reg == SDHCI_HOST_CONTROL2)) {
		val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:648", host->ioaddr + ESDHC_VENDOR_SPEC);
		if (val & ESDHC_VENDOR_SPEC_VSELECT)
			ret |= SDHCI_CTRL_VDD_180;

		if (esdhc_is_usdhc(imx_data)) {
			if (imx_data->socdata->flags & ESDHC_FLAG_MAN_TUNING)
				val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:654", host->ioaddr + ESDHC_MIX_CTRL);
			else if (imx_data->socdata->flags & ESDHC_FLAG_STD_TUNING)
				/* the std tuning bits is in ACMD12_ERR for imx6sl */
				val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:657", host->ioaddr + SDHCI_AUTO_CMD_STATUS);
		}
	...
	if (unlikely(reg == SDHCI_TRANSFER_MODE)) {
		if (esdhc_is_usdhc(imx_data)) {
			u32 m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:672", host->ioaddr + ESDHC_MIX_CTRL);
			...
			ret = pete_readw("drivers/mmc/host/sdhci-esdhc-imx.c:680", host->ioaddr + SDHCI_TRANSFER_MODE);
		}

		return ret;
	}

	return pete_readw("drivers/mmc/host/sdhci-esdhc-imx.c:686", host->ioaddr + reg);
}
...
static void esdhc_writew_le(struct sdhci_host *host, u16 val, int reg)
{
	...
	case SDHCI_CLOCK_CONTROL:
		new_val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:697", host->ioaddr + ESDHC_VENDOR_SPEC);
		if (val & SDHCI_CLOCK_CARD_EN)
			new_val |= ESDHC_VENDOR_SPEC_FRC_SDCLK_ON;
		else
			new_val &= ~ESDHC_VENDOR_SPEC_FRC_SDCLK_ON;
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:702", new_val, host->ioaddr + ESDHC_VENDOR_SPEC);
		if (!(new_val & ESDHC_VENDOR_SPEC_FRC_SDCLK_ON))
			esdhc_wait_for_card_clock_gate_off(host);
		return;
	case SDHCI_HOST_CONTROL2:
		new_val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:707", host->ioaddr + ESDHC_VENDOR_SPEC);
		if (val & SDHCI_CTRL_VDD_180)
			new_val |= ESDHC_VENDOR_SPEC_VSELECT;
		else
			new_val &= ~ESDHC_VENDOR_SPEC_VSELECT;
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:712", new_val, host->ioaddr + ESDHC_VENDOR_SPEC);
		if (imx_data->socdata->flags & ESDHC_FLAG_STD_TUNING) {
			u32 v = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:714", host->ioaddr + SDHCI_AUTO_CMD_STATUS);
			u32 m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:715", host->ioaddr + ESDHC_MIX_CTRL);
			...
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:730", v, host->ioaddr + SDHCI_AUTO_CMD_STATUS);
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:731", m, host->ioaddr + ESDHC_MIX_CTRL);
		}
		return;
	case SDHCI_TRANSFER_MODE:
		if ((imx_data->socdata->flags & ESDHC_FLAG_MULTIBLK_NO_INT)
			...
			u32 v;
			v = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:740", host->ioaddr + ESDHC_VENDOR_SPEC);
			v |= ESDHC_VENDOR_SPEC_SDIO_QUIRK;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:742", v, host->ioaddr + ESDHC_VENDOR_SPEC);
		}

		if (esdhc_is_usdhc(imx_data)) {
			u32 wml;
			u32 m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:747", host->ioaddr + ESDHC_MIX_CTRL);
			/* Swap AC23 bit */
			...
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:754", m, host->ioaddr + ESDHC_MIX_CTRL);

			/* Set watermark levels for PIO access to maximum value
			 * (128 words) to accommodate full 512 bytes buffer.
			 * For DMA access restore the levels to default value.
			 */
			m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:760", host->ioaddr + ESDHC_WTMK_LVL);
			...
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:783", m, host->ioaddr + ESDHC_WTMK_LVL);
		} else {
			/*
	...
	case SDHCI_COMMAND:
		...
		if (esdhc_is_usdhc(imx_data))
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:801", val << 16,
			       host->ioaddr + SDHCI_TRANSFER_MODE);
		else
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:804", val << 16 | imx_data->scratchpad,
			       host->ioaddr + SDHCI_TRANSFER_MODE);
		return;
...
static u8 esdhc_readb_le(struct sdhci_host *host, int reg)
{
	...
	case SDHCI_HOST_CONTROL:
		val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:821", host->ioaddr + reg);
	...
	return pete_readb("drivers/mmc/host/sdhci-esdhc-imx.c:830", host->ioaddr + reg);
}
...
static void esdhc_writeb_le(struct sdhci_host *host, u8 val, int reg)
{
	...
	case SDHCI_SOFTWARE_RESET:
		if (val & SDHCI_RESET_DATA)
			new_val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:870", host->ioaddr + SDHCI_HOST_CONTROL);
		break;
	}
	...
	if (reg == SDHCI_SOFTWARE_RESET) {
		if (val & SDHCI_RESET_ALL) {
			...
			if (esdhc_is_usdhc(imx_data)) {
				 ...
				 */
				new_val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:895", host->ioaddr + ESDHC_MIX_CTRL);
				pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:896", new_val & ESDHC_MIX_CTRL_TUNING_MASK,
					 ...
					 unsigned int clock)
{
	...
	if (esdhc_is_usdhc(imx_data)) {
		val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:939", host->ioaddr + ESDHC_VENDOR_SPEC);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:940", val & ~ESDHC_VENDOR_SPEC_FRC_SDCLK_ON,
	...
	if (is_imx53_esdhc(imx_data)) {
		 ...
		 */
		val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:956", host->ioaddr + ESDHC_DLL_CTRL);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:957", val | BIT(10), host->ioaddr + ESDHC_DLL_CTRL);
		temp = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:958", host->ioaddr + ESDHC_DLL_CTRL);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:959", val, host->ioaddr + ESDHC_DLL_CTRL);
		if (temp & BIT(10))
			pre_div = 2;
	}
	...
	if (esdhc_is_usdhc(imx_data)) {
		val = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1005", host->ioaddr + ESDHC_VENDOR_SPEC);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1006", val | ESDHC_VENDOR_SPEC_FRC_SDCLK_ON,
			host->ioaddr + ESDHC_VENDOR_SPEC);
	}
...
static unsigned int esdhc_pltfm_get_ro(struct sdhci_host *host)
{
	...
	case ESDHC_WP_CONTROLLER:
		return !(pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1022", host->ioaddr + SDHCI_PRESENT_STATE) &
			       SDHCI_WRITE_PROTECT);
	case ESDHC_WP_NONE:
		break;
	}
...
static void esdhc_reset_tuning(struct sdhci_host *host)
{
	...
	if (esdhc_is_usdhc(imx_data)) {
		ctrl = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1060", host->ioaddr + ESDHC_MIX_CTRL);
		ctrl &= ~ESDHC_MIX_CTRL_AUTO_TUNE_EN;
		if (imx_data->socdata->flags & ESDHC_FLAG_MAN_TUNING) {
			ctrl &= ~ESDHC_MIX_CTRL_SMPCLK_SEL;
			ctrl &= ~ESDHC_MIX_CTRL_FBCLK_SEL;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1065", ctrl, host->ioaddr + ESDHC_MIX_CTRL);
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1066", 0, host->ioaddr + ESDHC_TUNE_CTRL_STATUS);
		} else if (imx_data->socdata->flags & ESDHC_FLAG_STD_TUNING) {
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1068", ctrl, host->ioaddr + ESDHC_MIX_CTRL);
			ctrl = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1069", host->ioaddr + SDHCI_AUTO_CMD_STATUS);
			ctrl &= ~ESDHC_MIX_CTRL_SMPCLK_SEL;
			ctrl &= ~ESDHC_MIX_CTRL_EXE_TUNE;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1072", ctrl, host->ioaddr + SDHCI_AUTO_CMD_STATUS);
			 ...
			 */
			ctrl = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1084", host->ioaddr + SDHCI_INT_STATUS);
			ctrl |= SDHCI_INT_DATA_AVAIL;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1086", ctrl, host->ioaddr + SDHCI_INT_STATUS);
		}
	}
}
...
static void esdhc_prepare_tuning(struct sdhci_host *host, u32 val)
{
	...
	reg = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1143", host->ioaddr + ESDHC_MIX_CTRL);
	reg |= ESDHC_MIX_CTRL_EXE_TUNE | ESDHC_MIX_CTRL_SMPCLK_SEL |
			ESDHC_MIX_CTRL_FBCLK_SEL;
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1146", reg, host->ioaddr + ESDHC_MIX_CTRL);
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1147", val << 8, host->ioaddr + ESDHC_TUNE_CTRL_STATUS);
	dev_dbg(mmc_dev(host->mmc),
		"tuning with delay 0x%x ESDHC_TUNE_CTRL_STATUS 0x%x\n",
			val, pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1150", host->ioaddr + ESDHC_TUNE_CTRL_STATUS));
}
...
static void esdhc_post_tuning(struct sdhci_host *host)
{
	...
	reg = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1157", host->ioaddr + ESDHC_MIX_CTRL);
	reg &= ~ESDHC_MIX_CTRL_EXE_TUNE;
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1159", reg, host->ioaddr + ESDHC_MIX_CTRL);
}
...
static void esdhc_hs400_enhanced_strobe(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1203", host->ioaddr + ESDHC_MIX_CTRL);
	if (ios->enhanced_strobe)
		m |= ESDHC_MIX_CTRL_HS400_ES_EN;
	else
		m &= ~ESDHC_MIX_CTRL_HS400_ES_EN;
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1208", m, host->ioaddr + ESDHC_MIX_CTRL);
}
...
static void esdhc_set_strobe_dll(struct sdhci_host *host)
{
	...
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1261", pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1261", host->ioaddr + ESDHC_VENDOR_SPEC) &
		~ESDHC_VENDOR_SPEC_FRC_SDCLK_ON,
		host->ioaddr + ESDHC_VENDOR_SPEC);
	esdhc_wait_for_card_clock_gate_off(host);

	/* force a reset on strobe dll */
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1267", ESDHC_STROBE_DLL_CTRL_RESET,
		host->ioaddr + ESDHC_STROBE_DLL_CTRL);
	/* clear the reset bit on strobe dll before any setting */
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1270", 0, host->ioaddr + ESDHC_STROBE_DLL_CTRL);
	...
	pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1283", v, host->ioaddr + ESDHC_STROBE_DLL_CTRL);
...
static void esdhc_set_uhs_signaling(struct sdhci_host *host, unsigned timing)
{
	...
	m = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1301", host->ioaddr + ESDHC_MIX_CTRL);
	...
	case MMC_TIMING_MMC_HS200:
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1312", m, host->ioaddr + ESDHC_MIX_CTRL);
		break;
	case MMC_TIMING_UHS_DDR50:
	case MMC_TIMING_MMC_DDR52:
		m |= ESDHC_MIX_CTRL_DDREN;
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1317", m, host->ioaddr + ESDHC_MIX_CTRL);
		imx_data->is_ddr = 1;
		if (boarddata->delay_line) {
			...
			if (is_imx53_esdhc(imx_data))
				v <<= 1;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1326", v, host->ioaddr + ESDHC_DLL_CTRL);
		}
		break;
	case MMC_TIMING_MMC_HS400:
		m |= ESDHC_MIX_CTRL_DDREN | ESDHC_MIX_CTRL_HS400_EN;
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1331", m, host->ioaddr + ESDHC_MIX_CTRL);
		imx_data->is_ddr = 1;
...
static void sdhci_esdhc_imx_hwinit(struct sdhci_host *host)
{
	...
	if (esdhc_is_usdhc(imx_data)) {
		 ...
		 */
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1427", ESDHC_WTMK_DEFAULT_VAL, host->ioaddr + ESDHC_WTMK_LVL);
		 ...
		 */
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1440", pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1440", host->ioaddr + SDHCI_HOST_CONTROL)
		...
		if (!(imx_data->socdata->flags & ESDHC_FLAG_SKIP_ERR004536)) {
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1449", pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1449", host->ioaddr + 0x6c) & ~BIT(7),
				host->ioaddr + 0x6c);
		}

		/* disable DLL_CTRL delay line settings */
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1454", 0x0, host->ioaddr + ESDHC_DLL_CTRL);
		...
		if (imx_data->socdata->flags & ESDHC_FLAG_CQHCI) {
			tmp = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1466", host->ioaddr + ESDHC_VEND_SPEC2);
			tmp |= ESDHC_VEND_SPEC2_EN_BUSY_IRQ;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1468", tmp, host->ioaddr + ESDHC_VEND_SPEC2);

			host->quirks &= ~SDHCI_QUIRK_NO_BUSY_IRQ;
		}

		if (imx_data->socdata->flags & ESDHC_FLAG_STD_TUNING) {
			tmp = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1474", host->ioaddr + ESDHC_TUNING_CTRL);
			...
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1506", tmp, host->ioaddr + ESDHC_TUNING_CTRL);
			 ...
			 */
			tmp = pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1513", host->ioaddr + ESDHC_TUNING_CTRL);
			tmp &= ~ESDHC_STD_TUNING_EN;
			pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1515", tmp, host->ioaddr + ESDHC_TUNING_CTRL);
		}
...
static int sdhci_esdhc_imx_probe(struct platform_device *pdev)
{
	...
	if (esdhc_is_usdhc(imx_data)) {
		...
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1721", 0x0, host->ioaddr + ESDHC_MIX_CTRL);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1722", 0x0, host->ioaddr + SDHCI_AUTO_CMD_STATUS);
		pete_writel("drivers/mmc/host/sdhci-esdhc-imx.c:1723", 0x0, host->ioaddr + ESDHC_TUNE_CTRL_STATUS);
...
static void sdhci_esdhc_imx_remove(struct platform_device *pdev)
{
	...
	dead = (pete_readl("drivers/mmc/host/sdhci-esdhc-imx.c:1820", host->ioaddr + SDHCI_INT_STATUS) == 0xffffffff);
