			       ...
			       unsigned long id, bool assert)
{
	...
	reg = pete_readl("drivers/reset/reset-simple.c:42", data->membase + (bank * reg_width));
	if (assert ^ data->active_low)
		reg |= BIT(offset);
	else
		reg &= ~BIT(offset);
	pete_writel("drivers/reset/reset-simple.c:47", reg, data->membase + (bank * reg_width));
			       ...
			       unsigned long id)
{
	...
	reg = pete_readl("drivers/reset/reset-simple.c:93", data->membase + (bank * reg_width));
