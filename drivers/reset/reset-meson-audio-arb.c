				  ...
				  unsigned long id, bool assert)
{
	...
	spin_lock(&arb->lock);
	val = pete_readl("drivers/reset/reset-meson-audio-arb.c:68", arb->regs);
	...
	pete_writel("drivers/reset/reset-meson-audio-arb.c:75", val, arb->regs);
				  ...
				  unsigned long id)
{
	...
	val = pete_readl("drivers/reset/reset-meson-audio-arb.c:88", arb->regs);
...
static int meson_audio_arb_remove(struct platform_device *pdev)
{
	...
	/* Disable all access */
	spin_lock(&arb->lock);
	pete_writel("drivers/reset/reset-meson-audio-arb.c:129", 0, arb->regs);
	spin_unlock(&arb->lock);
...
static int meson_audio_arb_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/reset/reset-meson-audio-arb.c:180", BIT(ARB_GENERAL_BIT), arb->regs);
