...
static int mv_otg_probe(struct platform_device *pdev)
{
	...
	mvotg->op_regs =
		(struct mv_otg_regs __iomem *) ((unsigned long) mvotg->cap_regs
			+ (pete_readl("drivers/usb/phy/phy-mv-usb.c:752", mvotg->cap_regs) & CAPLENGTH_MASK));
...
static int mv_otg_resume(struct platform_device *pdev)
{
	...
	if (!mvotg->clock_gating) {
		...
		otgsc = pete_readl("drivers/usb/phy/phy-mv-usb.c:855", &mvotg->op_regs->otgsc);
		otgsc |= mvotg->irq_en;
		pete_writel("drivers/usb/phy/phy-mv-usb.c:857", otgsc, &mvotg->op_regs->otgsc);
