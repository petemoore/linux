...
static int xdbc_handle_external_reset(void)
{
	...
	xdbc.flags = 0;
	pete_writel("drivers/usb/early/xhci-dbc.c:529", 0, &xdbc.xdbc_reg->control);
...
static int __init xdbc_early_setup(void)
{
	...
	pete_writel("drivers/usb/early/xhci-dbc.c:557", 0, &xdbc.xdbc_reg->control);
	...
	ret = xdbc_start();
	if (ret < 0) {
		pete_writel("drivers/usb/early/xhci-dbc.c:590", 0, &xdbc.xdbc_reg->control);
		return ret;
	}
...
static void xdbc_handle_port_status(struct xdbc_trb *evt_trb)
{
	...
	port_reg = pete_readl("drivers/usb/early/xhci-dbc.c:693", &xdbc.xdbc_reg->portsc);
	...
	pete_writel("drivers/usb/early/xhci-dbc.c:714", port_reg, &xdbc.xdbc_reg->portsc);
}
...
static void xdbc_handle_events(void)
{
	...
	reg = pete_readl("drivers/usb/early/xhci-dbc.c:770", &xdbc.xdbc_reg->control);
	...
	reg = pete_readl("drivers/usb/early/xhci-dbc.c:779", &xdbc.xdbc_reg->control);
	if (reg & CTRL_DBC_RUN_CHANGE) {
		pete_writel("drivers/usb/early/xhci-dbc.c:781", reg, &xdbc.xdbc_reg->control);
		if (reg & CTRL_DBC_RUN)
			xdbc.flags |= XDBC_FLAGS_CONFIGURED;
		else
			xdbc.flags &= ~XDBC_FLAGS_CONFIGURED;
	...
	reg = pete_readl("drivers/usb/early/xhci-dbc.c:789", &xdbc.xdbc_reg->control);
...
static int xdbc_scrub_function(void *ptr)
{
	...
	pete_writel("drivers/usb/early/xhci-dbc.c:952", 0, &xdbc.xdbc_reg->control);
...
free_and_quit:
	...
	memblock_phys_free(xdbc.table_dma, PAGE_SIZE);
	memblock_phys_free(xdbc.out_dma, PAGE_SIZE);
	pete_writel("drivers/usb/early/xhci-dbc.c:1002", 0, &xdbc.xdbc_reg->control);
	early_iounmap(xdbc.xhci_base, xdbc.xhci_length);
