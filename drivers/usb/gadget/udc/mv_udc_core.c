...
static int mv_ep_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	if (ep->queue.next == &req->queue) {
		...
		pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:834", pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:834", &req->tail->dtd_next),
	...
	/* Enable EP */
out:
	epctrlx = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:843", &udc->op_regs->epctrlx[ep->ep_num]);
	if (ep_dir(ep) == EP_DIR_IN)
		epctrlx |= EPCTRL_TX_ENABLE;
	else
		epctrlx |= EPCTRL_RX_ENABLE;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:848", epctrlx, &udc->op_regs->epctrlx[ep->ep_num]);
	ep->stopped = stopped;
...
static void ep_set_stall(struct mv_udc *udc, u8 ep_num, u8 direction, int stall)
{
	...
	epctrlx = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:859", &udc->op_regs->epctrlx[ep_num]);
	...
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:875", epctrlx, &udc->op_regs->epctrlx[ep_num]);
}
...
static int ep_is_stall(struct mv_udc *udc, u8 ep_num, u8 direction)
{
	...
	epctrlx = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:882", &udc->op_regs->epctrlx[ep_num]);
...
static void udc_stop(struct mv_udc *udc)
{
	...
	/* Disable interrupts */
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:974", &udc->op_regs->usbintr);
	tmp &= ~(USBINTR_INT_EN | USBINTR_ERR_INT_EN |
		USBINTR_PORT_CHANGE_DETECT_EN | USBINTR_RESET_EN);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:977", tmp, &udc->op_regs->usbintr);

	udc->stopped = 1;

	/* Reset the Run the bit in the command register to stop VUSB */
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:982", &udc->op_regs->usbcmd);
	tmp &= ~USBCMD_RUN_STOP;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:984", tmp, &udc->op_regs->usbcmd);
}
...
static void udc_start(struct mv_udc *udc)
{
	...
	/* Enable interrupts */
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:995", usbintr, &udc->op_regs->usbintr);

	udc->stopped = 0;

	/* Set the Run bit in the command register */
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1000", USBCMD_RUN_STOP, &udc->op_regs->usbcmd);
}
...
static int udc_reset(struct mv_udc *udc)
{
	...
	/* Stop the controller */
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1009", &udc->op_regs->usbcmd);
	tmp &= ~USBCMD_RUN_STOP;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1011", tmp, &udc->op_regs->usbcmd);

	/* Reset the controller to get default values */
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1014", USBCMD_CTRL_RESET, &udc->op_regs->usbcmd);

	/* wait for reset to complete */
	loops = LOOPS(RESET_TIMEOUT);
	while (pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1018", &udc->op_regs->usbcmd) & USBCMD_CTRL_RESET) {
		if (loops == 0) {
			dev_err(&udc->dev->dev,
	...
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1029", &udc->op_regs->usbmode);
	tmp |= USBMODE_CTRL_MODE_DEVICE;

	/* turn setup lockout off, require setup tripwire in usbcmd */
	tmp |= USBMODE_SETUP_LOCK_OFF;

	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1035", tmp, &udc->op_regs->usbmode);

	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1037", 0x0, &udc->op_regs->epsetupstat);

	/* Configure the Endpoint List Address */
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1040", udc->ep_dqh_dma & USB_EP_LIST_ADDRESS_MASK,
		&udc->op_regs->eplistaddr);

	portsc = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1043", &udc->op_regs->portsc[0]);
	if (pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1044", &udc->cap_regs->hcsparams) & HCSPARAMS_PPC)
	...
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1052", portsc, &udc->op_regs->portsc[0]);

	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1054", &udc->op_regs->epctrlx[0]);
	tmp &= ~(EPCTRL_TX_EP_STALL | EPCTRL_RX_EP_STALL);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1056", tmp, &udc->op_regs->epctrlx[0]);
...
static int mv_udc_get_frame(struct usb_gadget *gadget)
{
	...
	retval = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1122", &udc->op_regs->frindex) & USB_FRINDEX_MASKS;
...
static int mv_udc_wakeup(struct usb_gadget *gadget)
{
	...
	portsc = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1137", &udc->op_regs->portsc);
	/* not suspended? */
	if (!(portsc & PORTSCX_PORT_SUSPEND))
		return 0;
	/* trigger force resume */
	portsc |= PORTSCX_PORT_FORCE_RESUME;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1143", portsc, &udc->op_regs->portsc[0]);
	return 0;
}
...
static void mv_set_ptc(struct mv_udc *udc, u32 mode)
{
	...
	portsc = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1417", &udc->op_regs->portsc[0]);
	portsc |= mode << 16;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1419", portsc, &udc->op_regs->portsc[0]);
}
	...
	struct mv_ep *ep0, struct mv_req *req)
{
	...
	if (udc->usb_state == USB_STATE_ADDRESS) {
		...
		pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1729", new_addr << USB_DEVICE_ADDRESS_BIT_SHIFT,
...
static void get_setup_data(struct mv_udc *udc, u8 ep_num, u8 *buffer_ptr)
{
	...
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1766", (1 << ep_num), &udc->op_regs->epsetupstat);

	/* while a hazard exists when setup package arrives */
	do {
		/* Set Setup Tripwire */
		temp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1771", &udc->op_regs->usbcmd);
		pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1772", temp | USBCMD_SETUP_TRIPWIRE_SET, &udc->op_regs->usbcmd);

		/* Copy the setup packet to local buffer */
		memcpy(buffer_ptr, (u8 *) dqh->setup_buffer, 8);
	} while (!(pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1776", &udc->op_regs->usbcmd) & USBCMD_SETUP_TRIPWIRE_SET));

	/* Clear Setup Tripwire */
	temp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1779", &udc->op_regs->usbcmd);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1780", temp & ~USBCMD_SETUP_TRIPWIRE_SET, &udc->op_regs->usbcmd);
}
...
static void irq_process_tr_complete(struct mv_udc *udc)
{
	...
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1797", &udc->op_regs->epsetupstat);
	...
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1815", &udc->op_regs->epcomplete);

	if (!tmp)
		return;

	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1820", tmp, &udc->op_regs->epcomplete);
...
static void irq_process_reset(struct mv_udc *udc)
{
	...
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1866", &udc->op_regs->deviceaddr);
	tmp &= ~(USB_DEVICE_ADDRESS_MASK);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1868", tmp, &udc->op_regs->deviceaddr);

	/* Clear all the setup token semaphores */
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1871", &udc->op_regs->epsetupstat);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1872", tmp, &udc->op_regs->epsetupstat);

	/* Clear all the endpoint complete status bits */
	tmp = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1875", &udc->op_regs->epcomplete);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1876", tmp, &udc->op_regs->epcomplete);

	/* wait until all endptprime bits cleared */
	loops = LOOPS(PRIME_TIMEOUT);
	while (pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1880", &udc->op_regs->epprime) & 0xFFFFFFFF) {
		if (loops == 0) {
			dev_err(&udc->dev->dev,
				"Timeout for ENDPTPRIME = 0x%x\n",
				pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1884", &udc->op_regs->epprime));
			break;
		}
		loops--;
		udelay(LOOPS_USEC);
	...
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:1892", (u32)~0, &udc->op_regs->epflush);

	if (pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1894", &udc->op_regs->portsc[0]) & PORTSCX_PORT_RESET) {
		...
		dev_info(&udc->dev->dev, "USB reset portsc 0x%x\n",
			pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1901", &udc->op_regs->portsc));
...
static void irq_process_port_change(struct mv_udc *udc)
{
	...
	portsc = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:1953", &udc->op_regs->portsc[0]);
...
static irqreturn_t mv_udc_irq(int irq, void *dev)
{
	...
	status = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:2009", &udc->op_regs->usbsts);
	intr = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:2010", &udc->op_regs->usbintr);
	status &= intr;
	...
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:2019", status, &udc->op_regs->usbsts);
...
static int mv_udc_probe(struct platform_device *pdev)
{
	...
	udc->op_regs =
		...
		+ (pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:2178", &udc->cap_regs->caplength_hciversion)
			& CAPLENGTH_MASK));
	udc->max_eps = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:2180", &udc->cap_regs->dccparams) & DCCPARAMS_DEN_MASK;
	 ...
	 */
	udc_stop(udc);
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:2187", 0xFFFFFFFF, &udc->op_regs->usbsts);
...
static void mv_udc_shutdown(struct platform_device *pdev)
{
	...
	mv_udc_enable(udc);
	mode = pete_readl("drivers/usb/gadget/udc/mv_udc_core.c:2404", &udc->op_regs->usbmode);
	mode &= ~3;
	pete_writel("drivers/usb/gadget/udc/mv_udc_core.c:2406", mode, &udc->op_regs->usbmode);
	mv_udc_disable(udc);
}
