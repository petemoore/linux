...
static void udc_handle_dma_ep(struct lpc32xx_udc *udc, struct lpc32xx_ep *ep)
{
	...
	/* Disable DMA */
	udc_ep_dma_disable(udc, ep->hwep_num);
	pete_writel("drivers/usb/gadget/udc/lpc32xx_udc.c:2004", (1 << ep->hwep_num), USBD_EOTINTCLR(udc->udp_baseaddr));
	pete_writel("drivers/usb/gadget/udc/lpc32xx_udc.c:2005", (1 << ep->hwep_num), USBD_NDDRTINTCLR(udc->udp_baseaddr));

	/* System error? */
	if (pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2008", USBD_SYSERRTINTST(udc->udp_baseaddr)) &
	    (1 << ep->hwep_num)) {
		pete_writel("drivers/usb/gadget/udc/lpc32xx_udc.c:2010", (1 << ep->hwep_num),
...
static irqreturn_t lpc32xx_usb_lp_irq(int irq, void *_udc)
{
	...
	devstat = pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2734", USBD_DEVINTST(udc->udp_baseaddr));

	devstat &= ~USBD_EP_FAST;
	pete_writel("drivers/usb/gadget/udc/lpc32xx_udc.c:2737", devstat, USBD_DEVINTCLR(udc->udp_baseaddr));
...
static irqreturn_t lpc32xx_usb_hp_irq(int irq, void *_udc)
{
	...
	/* Read the device status register */
	pete_writel("drivers/usb/gadget/udc/lpc32xx_udc.c:2773", USBD_EP_FAST, USBD_DEVINTCLR(udc->udp_baseaddr));

	/* Endpoints */
	tmp = pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2776", USBD_EPINTST(udc->udp_baseaddr));
...
static irqreturn_t lpc32xx_usb_devdma_irq(int irq, void *_udc)
{
	...
	tmp = pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2815", USBD_EOTINTST(udc->udp_baseaddr)) |
		(pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2816", USBD_EPDMAST(udc->udp_baseaddr)) &
		 pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2817", USBD_NDDRTINTST(udc->udp_baseaddr))) |
		pete_readl("drivers/usb/gadget/udc/lpc32xx_udc.c:2818", USBD_SYSERRTINTST(udc->udp_baseaddr));
	for (i = 1; i < NUM_ENDPOINTS; i++) {
		if (tmp & (1 << udc->ep[i].hwep_num))
