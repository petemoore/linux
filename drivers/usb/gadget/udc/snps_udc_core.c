		...
		struct usb_gadget_driver *driver)
{
	...
	/* clear SD */
	tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:1948", &dev->regs->ctl);
	tmp = tmp & AMD_CLEAR_BIT(UDC_DEVCTL_SD);
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:1950", tmp, &dev->regs->ctl);
...
static int amd5536_udc_stop(struct usb_gadget *g)
{
	...
	/* set SD */
	tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:1989", &dev->regs->ctl);
	tmp |= AMD_BIT(UDC_DEVCTL_SD);
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:1991", tmp, &dev->regs->ctl);
...
static void udc_process_cnak_queue(struct udc *dev)
{
	...
	for (tmp = 0; tmp < UDC_EPIN_NUM_USED; tmp++) {
		if (cnak_pending & (1 << tmp)) {
			...
			/* clear NAK by writing CNAK */
			reg = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2008", &dev->ep[tmp].regs->ctl);
			reg |= AMD_BIT(UDC_EPCTL_CNAK);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2010", reg, &dev->ep[tmp].regs->ctl);
			dev->ep[tmp].naking = 0;
	...
	if (cnak_pending & (1 << UDC_EP0OUT_IX)) {
		DBG(dev, "CNAK pending for ep%d\n", UDC_EP0OUT_IX);
		/* clear NAK by writing CNAK */
		reg = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2019", &dev->ep[UDC_EP0OUT_IX].regs->ctl);
		reg |= AMD_BIT(UDC_EPCTL_CNAK);
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2021", reg, &dev->ep[UDC_EP0OUT_IX].regs->ctl);
		dev->ep[UDC_EP0OUT_IX].naking = 0;
...
static irqreturn_t udc_data_out_isr(struct udc *dev, int ep_ix)
{
	...
	tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2069", &ep->regs->sts);
	if (use_dma) {
		/* BNA event ? */
		if (tmp & AMD_BIT(UDC_EPSTS_BNA)) {
			DBG(dev, "BNA ep%dout occurred - DESPTR = %x\n",
					ep->num, pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2074", &ep->regs->desptr));
			/* clear BNA */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2076", tmp | AMD_BIT(UDC_EPSTS_BNA), &ep->regs->sts);
	...
	if (tmp & AMD_BIT(UDC_EPSTS_HE)) {
		...
		/* clear HE */
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2090", tmp | AMD_BIT(UDC_EPSTS_HE), &ep->regs->sts);
		ret_val = IRQ_HANDLED;
		goto finished;
	}
			...
			if (!list_empty(&ep->queue) && !ep->halted) {
				...
				if (req->dma_going == 0) {
					...
					if (prep_dma(ep, req, GFP_ATOMIC) != 0)
						goto finished;
					/* write desc pointer */
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2207", req->td_phys,
						&ep->regs->desptr);
					req->dma_going = 1;
					/* enable DMA */
					udc_set_rde(dev);
				...
				if (ep->bna_dummy_req) {
					/* write desc pointer */
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2220", ep->bna_dummy_req->td_phys,
						&ep->regs->desptr);
					ep->bna_occurred = 0;
				}
	...
	if (cnak_pending) {
		...
		if (pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2259", &dev->regs->sts) & AMD_BIT(UDC_DEVSTS_RXFIFO_EMPTY))
			udc_process_cnak_queue(dev);
	}

	/* clear OUT bits in ep status */
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2264", UDC_EPSTS_OUT_CLEAR, &ep->regs->sts);
finished:
	return ret_val;
}
...
static irqreturn_t udc_data_in_isr(struct udc *dev, int ep_ix)
{
	...
	epsts = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2282", &ep->regs->sts);
	if (use_dma) {
		/* BNA ? */
		if (epsts & AMD_BIT(UDC_EPSTS_BNA)) {
			dev_err(dev->dev,
				...
				ep->num,
				(unsigned long) pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2289", &ep->regs->desptr));

			/* clear BNA */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2292", epsts, &ep->regs->sts);
			ret_val = IRQ_HANDLED;
			goto finished;
		}
	}
	...
	if (epsts & AMD_BIT(UDC_EPSTS_HE)) {
		dev_err(dev->dev,
			...
			ep->num, (unsigned long) pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2301", &ep->regs->desptr));

		/* clear HE */
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2304", epsts | AMD_BIT(UDC_EPSTS_HE), &ep->regs->sts);
		ret_val = IRQ_HANDLED;
		goto finished;
	}
	...
	if (epsts & AMD_BIT(UDC_EPSTS_TDC)) {
		...
		if (!ep->cancel_transfer && !list_empty(&ep->queue)) {
			...
			if (req->req.actual == req->req.length) {
				...
				if (list_empty(&ep->queue)) {
					/* disable interrupt */
					tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2336", &dev->regs->ep_irqmsk);
					tmp |= AMD_BIT(ep->num);
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2338", tmp, &dev->regs->ep_irqmsk);
				}
			}
		}
		ep->cancel_transfer = 0;
		...
		if (!list_empty(&ep->queue)) {
			...
			/* DMA */
				...
				if (req->td_data) {
					...
					/* write desc pointer */
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2389", req->td_phys, &ep->regs->desptr);
					...
					/* set poll demand bit */
					tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2399", &ep->regs->ctl);
					tmp |= AMD_BIT(UDC_EPCTL_P);
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2401", tmp, &ep->regs->ctl);
				}
			}

		} else if (!use_dma && ep->in) {
			/* disable interrupt */
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2407", 
				&dev->regs->ep_irqmsk);
			tmp |= AMD_BIT(ep->num);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2410", tmp,
				&dev->regs->ep_irqmsk);
		}
	}
	/* clear status bits */
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2415", epsts, &ep->regs->sts);
...
__acquires(dev->lock)
{
	...
	/* clear irq */
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2438", AMD_BIT(UDC_EPINT_OUT_EP0), &dev->regs->ep_irqsts);

	tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2440", &dev->ep[UDC_EP0OUT_IX].regs->sts);
	/* check BNA and clear if set */
	if (tmp & AMD_BIT(UDC_EPSTS_BNA)) {
		VDBG(dev, "ep0: BNA set\n");
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2444", AMD_BIT(UDC_EPSTS_BNA),
	...
	if (tmp == UDC_EPSTS_OUT_SETUP) {
		...
		/* set NAK for EP0_IN */
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2463", &dev->ep[UDC_EP0IN_IX].regs->ctl);
		tmp |= AMD_BIT(UDC_EPCTL_SNAK);
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2465", tmp, &dev->ep[UDC_EP0IN_IX].regs->ctl);
		dev->ep[UDC_EP0IN_IX].naking = 1;
		/* get setup data */
		if (use_dma) {

			/* clear OUT bits in ep status */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2471", UDC_EPSTS_OUT_CLEAR,
				&dev->ep[UDC_EP0OUT_IX].regs->sts);
		...
		if ((setup_data.request.bRequestType & USB_DIR_IN) != 0) {
			...
			if (ep->bna_dummy_req) {
				/* write desc pointer */
				pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2500", ep->bna_dummy_req->td_phys,
					&dev->ep[UDC_EP0OUT_IX].regs->desptr);
				ep->bna_occurred = 0;
			}
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2545", &dev->ep[UDC_EP0IN_IX].regs->ctl);
		/* ep0 in returns data (not zlp) on IN phase */
		if (setup_supported >= 0 && setup_supported <
				UDC_EP0IN_MAXPACKET) {
			/* clear NAK by writing CNAK in EP0_IN */
			tmp |= AMD_BIT(UDC_EPCTL_CNAK);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2551", tmp, &dev->ep[UDC_EP0IN_IX].regs->ctl);
			dev->ep[UDC_EP0IN_IX].naking = 0;
			UDC_QUEUE_CNAK(&dev->ep[UDC_EP0IN_IX], UDC_EP0IN_IX);
		...
		/* if unsupported request then stall */
			...
			tmp |= AMD_BIT(UDC_EPCTL_S);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2558", tmp, &dev->ep[UDC_EP0IN_IX].regs->ctl);
		} else
		...
		if (!set) {
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2565", &dev->ep[UDC_EP0OUT_IX].regs->ctl);
			tmp |= AMD_BIT(UDC_EPCTL_CNAK);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2567", tmp, &dev->ep[UDC_EP0OUT_IX].regs->ctl);
			dev->ep[UDC_EP0OUT_IX].naking = 0;
			UDC_QUEUE_CNAK(&dev->ep[UDC_EP0OUT_IX], UDC_EP0OUT_IX);
		}
		...
		if (!use_dma) {
			/* clear OUT bits in ep status */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2574", UDC_EPSTS_OUT_CLEAR,
				&dev->ep[UDC_EP0OUT_IX].regs->sts);
		}
	...
	/* data packet 0 bytes */
		...
		/* clear OUT bits in ep status */
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2581", UDC_EPSTS_OUT_CLEAR, &dev->ep[UDC_EP0OUT_IX].regs->sts);
		...
		if (use_dma) {
			...
			if (list_empty(&dev->ep[UDC_EP0OUT_IX].queue)) {
				...
				pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2603", dev->ep[UDC_EP0OUT_IX].td_phys,
					&dev->ep[UDC_EP0OUT_IX].regs->desptr);
				/* enable RDE */
				udc_ep0_set_rde(dev);
			}
			...
			/* received number bytes */
			count = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2611", &dev->ep[UDC_EP0OUT_IX].regs->sts);
			...
			if (count != 0) {
				...
				/* dummy read confirm */
				pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2621", &dev->ep[UDC_EP0OUT_IX].regs->confirm);
				ret_val = IRQ_HANDLED;
			}
		}
	}
	...
	if (cnak_pending) {
		/* CNAk processing when rxfifo empty only */
		if (pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2630", &dev->regs->sts) & AMD_BIT(UDC_DEVSTS_RXFIFO_EMPTY))
			udc_process_cnak_queue(dev);
	}
...
static irqreturn_t udc_control_in_isr(struct udc *dev)
{
	...
	/* clear irq */
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2650", AMD_BIT(UDC_EPINT_IN_EP0), &dev->regs->ep_irqsts);

	tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2652", &dev->ep[UDC_EP0IN_IX].regs->sts);
	/* DMA completion */
	if (tmp & AMD_BIT(UDC_EPSTS_TDC)) {
		...
		/* clear TDC bit */
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2659", AMD_BIT(UDC_EPSTS_TDC),
	...
	/* status reg has IN bit set ? */
		...
		if (ep->dma) {
			/* clear IN bit */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2668", AMD_BIT(UDC_EPSTS_IN),
				&dev->ep[UDC_EP0IN_IX].regs->sts);
		}
		if (dev->stall_ep0in) {
			DBG(dev, "stall ep0in\n");
			/* halt ep0in */
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2674", &ep->regs->ctl);
			tmp |= AMD_BIT(UDC_EPCTL_S);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2676", tmp, &ep->regs->ctl);
		} else {
			if (!list_empty(&ep->queue)) {
				/* next request */
				req = list_entry(ep->queue.next,
				...
				if (ep->dma) {
					/* write desc pointer */
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2685", req->td_phys, &ep->regs->desptr);
					/* set HOST READY */
					req->td_data->status =
						AMD_ADDBITS(
						req->td_data->status,
					...
					/* set poll demand bit */
					tmp =
					pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2695", &dev->ep[UDC_EP0IN_IX].regs->ctl);
					tmp |= AMD_BIT(UDC_EPCTL_P);
					pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2697", tmp,
		...
		if (!ep->dma) {
			/* clear IN bit */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2729", AMD_BIT(UDC_EPSTS_IN),
				&dev->ep[UDC_EP0IN_IX].regs->sts);
		}
	}
...
__acquires(dev->lock)
{
	...
	if (dev_irq & AMD_BIT(UDC_DEVINT_SC)) {
		...
		/* read config value */
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2755", &dev->regs->sts);
		...
		for (i = 0; i < UDC_EP_NUM; i++) {
			...
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2781", &dev->csr->ne[udc_csr_epix]);
			/* ep cfg */
			tmp = AMD_ADDBITS(tmp, ep->dev->cur_config,
						UDC_CSR_NE_CFG);
			/* write reg */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2786", tmp, &dev->csr->ne[udc_csr_epix]);

			/* clear stall bits */
			ep->halted = 0;
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2790", &ep->regs->ctl);
			tmp = tmp & AMD_CLEAR_BIT(UDC_EPCTL_S);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2792", tmp, &ep->regs->ctl);
		}
	...
	if (dev_irq & AMD_BIT(UDC_DEVINT_SI)) {
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2805", &dev->regs->sts);
		...
		for (i = 0; i < UDC_EP_NUM; i++) {
			...
			/* UDC CSR reg */
			/* set ep values */
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2836", &dev->csr->ne[udc_csr_epix]);
			/* ep interface */
			...
			tmp = AMD_ADDBITS(tmp, ep->dev->cur_alt,
						UDC_CSR_NE_ALT);
			/* write reg */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2845", tmp, &dev->csr->ne[udc_csr_epix]);

			/* clear stall bits */
			ep->halted = 0;
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2849", &ep->regs->ctl);
			tmp = tmp & AMD_CLEAR_BIT(UDC_EPCTL_S);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2851", tmp, &ep->regs->ctl);
		}
	...
	if (dev_irq & AMD_BIT(UDC_DEVINT_UR)) {
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2887", &dev->regs->sts);
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2900", &dev->regs->cfg);
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2901", tmp | AMD_BIT(UDC_DEVCFG_DMARST), &dev->regs->cfg);
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2902", tmp, &dev->regs->cfg);
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2911", &dev->regs->irqmsk);
		tmp &= AMD_UNMASK_BIT(UDC_DEVINT_US);
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2913", tmp, &dev->regs->irqmsk);
	...
	if (dev_irq & AMD_BIT(UDC_DEVINT_SVC)) {
		...
		tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2952", &dev->regs->sts);
		if (!(tmp & AMD_BIT(UDC_DEVSTS_SESSVLD))) {
			/* disable suspend interrupt */
			tmp = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2955", &dev->regs->irqmsk);
			tmp |= AMD_BIT(UDC_DEVINT_US);
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2957", tmp, &dev->regs->irqmsk);
...
irqreturn_t udc_irq(int irq, void *pdev)
{
	...
	/* check for ep irq */
	reg = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:2980", &dev->regs->ep_irqsts);
	if (reg) {
		...
		for (i = 1; i < UDC_EP_NUM; i++) {
			...
			/* clear irq status */
			pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:2997", ep_irq, &dev->regs->ep_irqsts);
	...
	/* check for dev irq */
	reg = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:3010", &dev->regs->irqsts);
	if (reg) {
		/* clear irq */
		pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:3013", reg, &dev->regs->irqsts);
...
int udc_probe(struct udc *dev)
{
	...
	/* set SD */
	reg = pete_readl("drivers/usb/gadget/udc/snps_udc_core.c:3176", &dev->regs->ctl);
	reg |= AMD_BIT(UDC_DEVCTL_SD);
	pete_writel("drivers/usb/gadget/udc/snps_udc_core.c:3178", reg, &dev->regs->ctl);
