...
static int goku_set_halt(struct usb_ep *_ep, int value)
{
	...
	if (!list_empty(&ep->queue))
		retval = -EAGAIN;
			...
			&& (pete_readl("drivers/usb/gadget/udc/goku_udc.c:911", &ep->dev->regs->DataSet)
					& DATASET_AB(ep->num)))
		retval = -EAGAIN;
	else if (!value)
		goku_clear_halt(ep);
		...
		pete_readl("drivers/usb/gadget/udc/goku_udc.c:920", ep->reg_status);
	}
	spin_unlock_irqrestore(&ep->dev->lock, flags);
	return retval;
}
...
static int goku_fifo_status(struct usb_ep *_ep)
{
	...
	regs = ep->dev->regs;
	size = pete_readl("drivers/usb/gadget/udc/goku_udc.c:942", &regs->EPxSizeLA[ep->num]) & DATASIZE;
	size += pete_readl("drivers/usb/gadget/udc/goku_udc.c:943", &regs->EPxSizeLB[ep->num]) & DATASIZE;
...
static void goku_fifo_flush(struct usb_ep *_ep)
{
	...
	regs = ep->dev->regs;
	size = pete_readl("drivers/usb/gadget/udc/goku_udc.c:966", &regs->EPxSizeLA[ep->num]);
	size &= DATASIZE;
...
static int udc_proc_read(struct seq_file *m, void *v)
{
	...
	/* basic device status */
	tmp = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1148", &regs->power_detect);
	...
	dump_intmask(m, "int_status", pete_readl("drivers/usb/gadget/udc/goku_udc.c:1164", &regs->int_status));
	dump_intmask(m, "int_enable", pete_readl("drivers/usb/gadget/udc/goku_udc.c:1165", &regs->int_enable));
	...
	seq_printf(m, "\nirqs %lu\ndataset %02x single.bcs %02x.%02x state %x addr %u\n",
		   dev->irqs, pete_readl("drivers/usb/gadget/udc/goku_udc.c:1172", &regs->DataSet),
		   pete_readl("drivers/usb/gadget/udc/goku_udc.c:1173", &regs->EPxSingle), pete_readl("drivers/usb/gadget/udc/goku_udc.c:1173", &regs->EPxBCS),
		   pete_readl("drivers/usb/gadget/udc/goku_udc.c:1174", &regs->UsbState),
		   pete_readl("drivers/usb/gadget/udc/goku_udc.c:1175", &regs->address));
	if (seq_has_overflowed(m))
		goto done;

	tmp = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1179", &regs->dma_master);
	...
	for (i = 0; i < 4; i++) {
		...
		tmp = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1205", ep->reg_status);
		...
		list_for_each_entry(req, &ep->queue, queue) {
			if (ep->dma && req->queue.prev == &ep->queue) {
				if (i == UDC_MSTRD_ENDPOINT)
					tmp = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1229", &regs->in_dma_current);
				else
					tmp = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1231", &regs->out_dma_current);
				tmp -= req->req.dma;
				tmp++;
			} else
				tmp = req->req.actual;
...
static void udc_reset(struct goku_udc *dev)
{
	...
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1299", 0, &regs->power_detect);
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1300", 0, &regs->int_enable);
	pete_readl("drivers/usb/gadget/udc/goku_udc.c:1301", &regs->int_enable);
	dev->int_enable = 0;
	 ...
	 */
	udelay(250);
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1308", PW_RESETB, &regs->power_detect);
	pete_readl("drivers/usb/gadget/udc/goku_udc.c:1309", &regs->int_enable);
}
...
static void ep0_start(struct goku_udc *dev)
{
	...
	udc_reset(dev);
	udc_reinit (dev);
	//pete_writel("drivers/usb/gadget/udc/goku_udc.c:1321", MST_EOPB_ENA | MST_TIMEOUT_ENA, &regs->dma_master);

	/* hw handles set_address, set_feature, get_status; maybe more */
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1324",    G_REQMODE_SET_INTF | G_REQMODE_GET_INTF
	...
	for (i = 0; i < DESC_LEN; i++)
		pete_writel("drivers/usb/gadget/udc/goku_udc.c:1335", 0, &regs->descriptors[i]);
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1336", 0, &regs->UsbReady);

	/* expect ep0 requests when the host drops reset */
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1339", PW_RESETB | PW_PULLUP, &regs->power_detect);
	dev->int_enable = INT_DEVWIDE | INT_EP0;
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1341", dev->int_enable, &dev->regs->int_enable);
	pete_readl("drivers/usb/gadget/udc/goku_udc.c:1342", &regs->int_enable);
...
static void udc_enable(struct goku_udc *dev)
{
	...
	if (pete_readl("drivers/usb/gadget/udc/goku_udc.c:1350", &dev->regs->power_detect) & PW_DETECT)
		ep0_start(dev);
	else {
		DBG(dev, "%s\n", __func__);
		dev->int_enable = INT_PWRDETECT;
		pete_writel("drivers/usb/gadget/udc/goku_udc.c:1355", dev->int_enable, &dev->regs->int_enable);
	}
}
...
static void ep0_setup(struct goku_udc *dev)
{
	...
	ctrl.bRequestType = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1426", &regs->bRequestType);
	ctrl.bRequest = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1427", &regs->bRequest);
	ctrl.wValue  = cpu_to_le16((pete_readl("drivers/usb/gadget/udc/goku_udc.c:1428", &regs->wValueH)  << 8)
					| pete_readl("drivers/usb/gadget/udc/goku_udc.c:1429", &regs->wValueL));
	ctrl.wIndex  = cpu_to_le16((pete_readl("drivers/usb/gadget/udc/goku_udc.c:1430", &regs->wIndexH)  << 8)
					| pete_readl("drivers/usb/gadget/udc/goku_udc.c:1431", &regs->wIndexL));
	ctrl.wLength = cpu_to_le16((pete_readl("drivers/usb/gadget/udc/goku_udc.c:1432", &regs->wLengthH) << 8)
					| pete_readl("drivers/usb/gadget/udc/goku_udc.c:1433", &regs->wLengthL));
	pete_writel("drivers/usb/gadget/udc/goku_udc.c:1434", 0, &regs->SetupRecv);
	...
	if (likely(ctrl.bRequestType & USB_DIR_IN)) {
		...
		pete_writel("drivers/usb/gadget/udc/goku_udc.c:1442", ICONTROL_STATUSNAK, &dev->regs->IntControl);
	} else {
		dev->ep[0].is_in = 0;
...
succeed:
				...
				pete_writel("drivers/usb/gadget/udc/goku_udc.c:1475", ~(1<<0), &regs->EOP);
				dev->ep[0].stopped = 1;
				dev->ep0state = EP0_STATUS;
				return;
...
#define ACK(irqbit) { \
		stat &= ~irqbit; \
		pete_writel("drivers/usb/gadget/udc/goku_udc.c:1531", ~irqbit, &regs->int_status); \
		handled = 1; \
		}
...
rescan:
	stat = pete_readl("drivers/usb/gadget/udc/goku_udc.c:1546", &regs->int_status) & dev->int_enable;
        if (!stat)
		goto done;
	dev->irqs++;
	...
	if (unlikely(stat & INT_DEVWIDE)) {
		...
		if (stat & INT_PWRDETECT) {
			pete_writel("drivers/usb/gadget/udc/goku_udc.c:1563", ~stat, &regs->int_status);
			if (pete_readl("drivers/usb/gadget/udc/goku_udc.c:1564", &dev->regs->power_detect) & PW_DETECT) {
				VDBG(dev, "connect\n");
				ep0_start(dev);
			} else {
				DBG(dev, "disconnect\n");
				...
				pete_writel("drivers/usb/gadget/udc/goku_udc.c:1573", dev->int_enable, &dev->regs->int_enable);
			}
			stat = 0;
			handled = 1;
			goto done;
		...
		if (stat & INT_SUSPEND) {
			ACK(INT_SUSPEND);
			if (pete_readl("drivers/usb/gadget/udc/goku_udc.c:1581", &regs->ep_status[0]) & EPxSTATUS_SUSPEND) {
				switch (dev->ep0state) {
				case EP0_DISCONNECT:
				case EP0_SUSPEND:
					goto pm_next;
...
pm_next:
	...
	if (stat & INT_SETUP) {
		...
		if (dev->ep0state == EP0_IN) {
			...
			ep->irqs++;
			nuke(ep, 0);
			pete_writel("drivers/usb/gadget/udc/goku_udc.c:1638", ~(1<<0), &regs->EOP);
			dev->ep0state = EP0_STATUS;
		}
	}
	...
	(void)pete_readl("drivers/usb/gadget/udc/goku_udc.c:1689", &regs->int_enable);
	spin_unlock(&dev->lock);
	if (stat)
		DBG(dev, "unhandled irq status: %05x (%05x, %05x)\n", stat,
				pete_readl("drivers/usb/gadget/udc/goku_udc.c:1693", &regs->int_status), dev->int_enable);
