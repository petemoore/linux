...
void ast_vhub_ep0_handle_setup(struct ast_vhub_ep *ep)
{
 ...
 stall:
	EPDBG(ep, "stalling\n");
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:163", VHUB_EP0_CTRL_STALL, ep->ep0.ctlstat);
	ep->ep0.state = ep0_state_stall;
	ep->ep0.dir_in = false;
	return;
 ...
 complete:
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:170", VHUB_EP0_TX_BUFF_RDY, ep->ep0.ctlstat);
				 ...
				 struct ast_vhub_req *req)
{
	...
	if (req->last_desc >= 0) {
		...
		ep->ep0.state = ep0_state_status;
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:193", VHUB_EP0_RX_BUFF_RDY, ep->ep0.ctlstat);
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:222", reg, ep->ep0.ctlstat);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:223", reg | VHUB_EP0_TX_BUFF_RDY, ep->ep0.ctlstat);
...
static void ast_vhub_ep0_rx_prime(struct ast_vhub_ep *ep)
{
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:232", VHUB_EP0_RX_BUFF_RDY, ep->ep0.ctlstat);
}
				    ...
				    unsigned int len)
{
	...
	if (len < ep->ep.maxpacket || len == remain) {
		ep->ep0.state = ep0_state_status;
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:267", VHUB_EP0_TX_BUFF_RDY, ep->ep0.ctlstat);
...
void ast_vhub_ep0_handle_ack(struct ast_vhub_ep *ep, bool in_ack)
{
	...
	/* Read EP0 status */
	stat = pete_readl("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:282", ep->ep0.ctlstat);
	...
	if (stall) {
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:353", VHUB_EP0_CTRL_STALL, ep->ep0.ctlstat);
		ep->ep0.state = ep0_state_stall;
	} else
			      ...
			      gfp_t gfp_flags)
{
	...
	if (ep->ep0.dir_in) {
		...
		ep->ep0.state = ep0_state_status;
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:426", VHUB_EP0_TX_BUFF_RDY, ep->ep0.ctlstat);
		ast_vhub_done(ep, req, 0);
	} else {
...
static int ast_vhub_ep0_dequeue(struct usb_ep* u_ep, struct usb_request *u_req)
{
	...
	if (req && u_req == &req->req) {
		...
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/ep0.c:462", VHUB_EP0_CTRL_STALL, ep->ep0.ctlstat);
		ep->ep0.state = ep0_state_status;
		ep->ep0.dir_in = false;
		rc = 0;
	}
