...
static irqreturn_t ast_vhub_irq(int irq, void *data)
{
	...
	istat = pete_readl("drivers/usb/gadget/udc/aspeed-vhub/core.c:106", vhub->regs + AST_VHUB_ISR);
	if (!istat)
		goto bail;
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:109", istat, vhub->regs + AST_VHUB_ISR);
	iret = IRQ_HANDLED;

	UDCVDBG(vhub, "irq status=%08x, ep_acks=%08x ep_nacks=%08x\n",
	       istat,
	       pete_readl("drivers/usb/gadget/udc/aspeed-vhub/core.c:114", vhub->regs + AST_VHUB_EP_ACK_ISR),
	       pete_readl("drivers/usb/gadget/udc/aspeed-vhub/core.c:115", vhub->regs + AST_VHUB_EP_NACK_ISR));

	/* Handle generic EPs first */
	if (istat & VHUB_IRQ_EP_POOL_ACK_STALL) {
		u32 ep_acks = pete_readl("drivers/usb/gadget/udc/aspeed-vhub/core.c:119", vhub->regs + AST_VHUB_EP_ACK_ISR);
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:120", ep_acks, vhub->regs + AST_VHUB_EP_ACK_ISR);
...
void ast_vhub_init_hw(struct ast_vhub *vhub)
{
	...
	*/
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:198", ctrl, vhub->regs + AST_VHUB_CTRL);
	udelay(1);

	/* Set descriptor ring size */
	if (AST_VHUB_DESCS_COUNT == 256) {
		ctrl |= VHUB_CTRL_LONG_DESC;
		pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:204", ctrl, vhub->regs + AST_VHUB_CTRL);
	} else {
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:211", VHUB_SW_RESET_ROOT_HUB |
	       VHUB_SW_RESET_DMA_CONTROLLER |
	       VHUB_SW_RESET_EP_POOL |
	       port_mask, vhub->regs + AST_VHUB_SW_RESET);
	udelay(1);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:216", 0, vhub->regs + AST_VHUB_SW_RESET);

	/* Disable and cleanup EP ACK/NACK interrupts */
	epn_mask = GENMASK(vhub->max_epns - 1, 0);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:220", 0, vhub->regs + AST_VHUB_EP_ACK_IER);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:221", 0, vhub->regs + AST_VHUB_EP_NACK_IER);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:222", epn_mask, vhub->regs + AST_VHUB_EP_ACK_ISR);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:223", epn_mask, vhub->regs + AST_VHUB_EP_NACK_ISR);

	/* Default settings for EP0, enable HW hub EP1 */
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:226", 0, vhub->regs + AST_VHUB_EP0_CTRL);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:227", VHUB_EP1_CTRL_RESET_TOGGLE |
	       VHUB_EP1_CTRL_ENABLE,
	       vhub->regs + AST_VHUB_EP1_CTRL);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:230", 0, vhub->regs + AST_VHUB_EP1_STS_CHG);

	/* Configure EP0 DMA buffer */
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:233", vhub->ep0.buf_dma, vhub->regs + AST_VHUB_EP0_DATA);

	/* Clear address */
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:236", 0, vhub->regs + AST_VHUB_CONF);
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:243", ctrl, vhub->regs + AST_VHUB_CTRL);

	/* Enable some interrupts */
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:246", VHUB_IRQ_HUB_EP0_IN_ACK_STALL |
...
static void ast_vhub_remove(struct platform_device *pdev)
{
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:272", 0, vhub->regs + AST_VHUB_IER);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:273", VHUB_IRQ_ACK_ALL, vhub->regs + AST_VHUB_ISR);

	/* Pull device, leave PHY enabled */
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:276", VHUB_CTRL_PHY_CLK |
	       VHUB_CTRL_PHY_RESET_DIS,
...
static int ast_vhub_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:357", 0, vhub->regs + AST_VHUB_IER);
	pete_writel("drivers/usb/gadget/udc/aspeed-vhub/core.c:358", VHUB_IRQ_ACK_ALL, vhub->regs + AST_VHUB_ISR);
