...
static int cdnsp_update_port_id(struct cdnsp_device *pdev, u32 port_id)
{
	...
	if (port_id == pdev->usb2_port.port_num)
		...
		pete_writel("drivers/usb/cdns3/cdnsp-ring.c:782", PORT_U1_TIMEOUT(1) | PORT_U2_TIMEOUT(1),
...
new_event:
	portsc = pete_readl("drivers/usb/cdns3/cdnsp-ring.c:812", &port_regs->portsc);
	pete_writel("drivers/usb/cdns3/cdnsp-ring.c:813", cdnsp_port_state_to_neutral(portsc) |
	...
	if ((portsc & PORT_PLC)) {
		if (!(pdev->cdnsp_state & CDNSP_WAKEUP_PENDING)  &&
			...
			cmd_regs = pete_readl("drivers/usb/cdns3/cdnsp-ring.c:825", &pdev->op_regs->command);
			if (!(cmd_regs & CMD_R_S))
				goto cleanup;
	...
	if (pete_readl("drivers/usb/cdns3/cdnsp-ring.c:884", &port_regs->portsc) & PORT_CHANGE_BITS)
		goto new_event;
...
irqreturn_t cdnsp_irq_handler(int irq, void *priv)
{
	...
	status = pete_readl("drivers/usb/cdns3/cdnsp-ring.c:1575", &pdev->op_regs->status);
	...
	pete_writel("drivers/usb/cdns3/cdnsp-ring.c:1585", status | STS_EINT, &pdev->op_regs->status);
	irq_pending = pete_readl("drivers/usb/cdns3/cdnsp-ring.c:1586", &pdev->ir_set->irq_pending);
	irq_pending |= IMAN_IP;
	pete_writel("drivers/usb/cdns3/cdnsp-ring.c:1588", irq_pending, &pdev->ir_set->irq_pending);
