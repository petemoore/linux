...
static void xhci_disable_port(struct xhci_hcd *xhci, struct xhci_port *port)
{
	...
	portsc = pete_readl("drivers/usb/host/xhci-hub.c:600", port->addr);
	portsc = xhci_port_state_to_neutral(portsc);

	/* Write 1 to disable the port */
	pete_writel("drivers/usb/host/xhci-hub.c:604", portsc | PORT_PE, port->addr);

	portsc = pete_readl("drivers/usb/host/xhci-hub.c:606", port->addr);
		...
		u16 wIndex, __le32 __iomem *addr, u32 port_status)
{
	...
	pete_writel("drivers/usb/host/xhci-hub.c:655", port_status | status, addr);
	port_status = pete_readl("drivers/usb/host/xhci-hub.c:656", addr);
	...
	__must_hold(&xhci->lock)
{
	...
	hcd = port->rhub->hcd;
	temp = pete_readl("drivers/usb/host/xhci-hub.c:684", port->addr);
	...
	if (on) {
		/* Power on */
		pete_writel("drivers/usb/host/xhci-hub.c:693", temp | PORT_POWER, port->addr);
		pete_readl("drivers/usb/host/xhci-hub.c:694", port->addr);
	} else {
		/* Power off */
		pete_writel("drivers/usb/host/xhci-hub.c:697", temp & ~PORT_POWER, port->addr);
	}
	...
	u16 test_mode, u16 wIndex)
{
	...
	temp = pete_readl("drivers/usb/host/xhci-hub.c:717", port->addr + PORTPMSC);
	temp |= test_mode << PORT_TEST_MODE_SHIFT;
	pete_writel("drivers/usb/host/xhci-hub.c:719", temp, port->addr + PORTPMSC);
			 ...
			 u32 link_state)
{
	...
	portsc = pete_readl("drivers/usb/host/xhci-hub.c:793", port->addr);
	temp = xhci_port_state_to_neutral(portsc);
	temp &= ~PORT_PLS_MASK;
	temp |= PORT_LINK_STROBE | link_state;
	pete_writel("drivers/usb/host/xhci-hub.c:797", temp, port->addr);
				      ...
				      struct xhci_port *port, u16 wake_mask)
{
	...
	temp = pete_readl("drivers/usb/host/xhci-hub.c:809", port->addr);
	temp = xhci_port_state_to_neutral(temp);
	...
	pete_writel("drivers/usb/host/xhci-hub.c:827", temp, port->addr);
}
			     ...
			     u32 port_bit)
{
	...
	temp = pete_readl("drivers/usb/host/xhci-hub.c:836", port->addr);
	if (temp & port_bit) {
		temp = xhci_port_state_to_neutral(temp);
		temp |= port_bit;
		pete_writel("drivers/usb/host/xhci-hub.c:840", temp, port->addr);
	}
}
					     ...
					     unsigned long *flags)
{
	...
	/* Has resume been signalled for USB_RESUME_TIME yet? */
		...
		if (time_left) {
			...
			int port_status = pete_readl("drivers/usb/host/xhci-hub.c:997", port->addr);
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case GetPortStatus:
		...
		temp = pete_readl("drivers/usb/host/xhci-hub.c:1259", port->addr);
		if (temp == ~(u32)0) {
			xhci_hc_died(xhci);
			retval = -ENODEV;
			break;
		...
		if (wValue == 0x02) {
			...
			port_li = pete_readl("drivers/usb/host/xhci-hub.c:1284", port->addr + PORTLI);
	...
	case SetPortFeature:
		...
		port = ports[portnum1 - 1];
		wIndex--;
		temp = pete_readl("drivers/usb/host/xhci-hub.c:1305", port->addr);
		if (temp == ~(u32)0) {
			xhci_hc_died(xhci);
			retval = -ENODEV;
			break;
		...
		case USB_PORT_FEAT_SUSPEND:
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1315", port->addr);
			 ...
			 */
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1327", port->addr);
			...
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1352", port->addr);
			bus_state->suspended_ports |= 1 << wIndex;
			break;
		case USB_PORT_FEAT_LINK_STATE:
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1356", port->addr);
			/* Disable port */
			if (link_state == USB_SS_PORT_LS_SS_DISABLED) {
				...
				temp |= PORT_CSC | PORT_PEC | PORT_WRC |
					...
					PORT_CEC;
				pete_writel("drivers/usb/host/xhci-hub.c:1369", temp | PORT_PE, port->addr);
				temp = pete_readl("drivers/usb/host/xhci-hub.c:1370", port->addr);
				break;
			}
			...
			if (link_state == USB_SS_PORT_LS_RX_DETECT) {
				...
				temp = pete_readl("drivers/usb/host/xhci-hub.c:1379", port->addr);
				break;
			}
			...
			if (link_state == USB_SS_PORT_LS_COMP_MOD) {
				...
				temp = pete_readl("drivers/usb/host/xhci-hub.c:1412", port->addr);
				break;
			}
			...
			if (link_state == USB_SS_PORT_LS_U0) {
				...
				if (!wait_for_completion_timeout(&port->u3exit_done,
					...
					xhci_dbg(xhci, "missing U0 port change event for port %d-%d\n",
						 hcd->self.busnum, portnum1);
				spin_lock_irqsave(&xhci->lock, flags);
				temp = pete_readl("drivers/usb/host/xhci-hub.c:1460", port->addr);
				break;
			}
			...
			if (link_state == USB_SS_PORT_LS_U3) {
				...
				while (retries--) {
					...
					temp = pete_readl("drivers/usb/host/xhci-hub.c:1480", port->addr);
					if ((temp & PORT_PLS_MASK) == XDEV_U3)
						break;
				}
				spin_lock_irqsave(&xhci->lock, flags);
				temp = pete_readl("drivers/usb/host/xhci-hub.c:1485", port->addr);
				bus_state->suspended_ports |= 1 << wIndex;
			}
			break;
		...
		case USB_PORT_FEAT_RESET:
			...
			pete_writel("drivers/usb/host/xhci-hub.c:1500", temp, port->addr);

			temp = pete_readl("drivers/usb/host/xhci-hub.c:1502", port->addr);
			xhci_dbg(xhci, "set port reset, actual port %d-%d status  = 0x%x\n",
				 hcd->self.busnum, portnum1, temp);
			break;
		case USB_PORT_FEAT_REMOTE_WAKE_MASK:
			xhci_set_remote_wake_mask(xhci, port, wake_mask);
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1508", port->addr);
			xhci_dbg(xhci, "set port remote wake mask, actual port %d-%d status  = 0x%x\n",
				 hcd->self.busnum, portnum1, temp);
			break;
		case USB_PORT_FEAT_BH_PORT_RESET:
			temp |= PORT_WR;
			pete_writel("drivers/usb/host/xhci-hub.c:1514", temp, port->addr);
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1515", port->addr);
			break;
		case USB_PORT_FEAT_U1_TIMEOUT:
			if (hcd->speed < HCD_USB3)
				goto error;
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1520", port->addr + PORTPMSC);
			temp &= ~PORT_U1_TIMEOUT_MASK;
			temp |= PORT_U1_TIMEOUT(timeout);
			pete_writel("drivers/usb/host/xhci-hub.c:1523", temp, port->addr + PORTPMSC);
			break;
		case USB_PORT_FEAT_U2_TIMEOUT:
			if (hcd->speed < HCD_USB3)
				goto error;
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1528", port->addr + PORTPMSC);
			temp &= ~PORT_U2_TIMEOUT_MASK;
			temp |= PORT_U2_TIMEOUT(timeout);
			pete_writel("drivers/usb/host/xhci-hub.c:1531", temp, port->addr + PORTPMSC);
			break;
		...
		temp = pete_readl("drivers/usb/host/xhci-hub.c:1547", port->addr);
		break;
	case ClearPortFeature:
		...
		wIndex--;
		temp = pete_readl("drivers/usb/host/xhci-hub.c:1556", port->addr);
		if (temp == ~(u32)0) {
			xhci_hc_died(xhci);
			retval = -ENODEV;
			break;
		...
		switch (wValue) {
		case USB_PORT_FEAT_SUSPEND:
			temp = pete_readl("drivers/usb/host/xhci-hub.c:1566", port->addr);
...
int xhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < max_ports; i++) {
		temp = pete_readl("drivers/usb/host/xhci-hub.c:1684", ports[i]->addr);
		if (temp == ~(u32)0) {
			xhci_hc_died(xhci);
			retval = -ENODEV;
			break;
...
int xhci_bus_suspend(struct usb_hcd *hcd)
{
	...
	while (port_index--) {
		...
		int retries = 10;
retry:
		t1 = pete_readl("drivers/usb/host/xhci-hub.c:1753", ports[port_index]->addr);
	...
	while (port_index--) {
		...
		pete_writel("drivers/usb/host/xhci-hub.c:1834", portsc_buf[port_index], ports[port_index]->addr);
	}
...
static bool xhci_port_missing_cas_quirk(struct xhci_port *port)
{
	...
	portsc = pete_readl("drivers/usb/host/xhci-hub.c:1855", port->addr);
	...
	portsc |= PORT_WR;
	pete_writel("drivers/usb/host/xhci-hub.c:1868", portsc, port->addr);
	/* flush write */
	pete_readl("drivers/usb/host/xhci-hub.c:1870", port->addr);
	return true;
}
...
int xhci_bus_resume(struct usb_hcd *hcd)
{
	...
	/* delay the irqs */
	temp = pete_readl("drivers/usb/host/xhci-hub.c:1902", &xhci->op_regs->command);
	temp &= ~CMD_EIE;
	pete_writel("drivers/usb/host/xhci-hub.c:1904", temp, &xhci->op_regs->command);
	...
	port_index = max_ports;
	while (port_index--) {
		portsc = pete_readl("drivers/usb/host/xhci-hub.c:1914", ports[port_index]->addr);
		...
		portsc &= ~(PORT_RWC_BITS | PORT_CEC | PORT_WAKE_BITS);
		pete_writel("drivers/usb/host/xhci-hub.c:1944", portsc, ports[port_index]->addr);
	}
	...
	(void) pete_readl("drivers/usb/host/xhci-hub.c:1977", &xhci->op_regs->command);

	bus_state->next_statechange = jiffies + msecs_to_jiffies(5);
	/* re-enable irqs */
	temp = pete_readl("drivers/usb/host/xhci-hub.c:1981", &xhci->op_regs->command);
	temp |= CMD_EIE;
	pete_writel("drivers/usb/host/xhci-hub.c:1983", temp, &xhci->op_regs->command);
	temp = pete_readl("drivers/usb/host/xhci-hub.c:1984", &xhci->op_regs->command);
