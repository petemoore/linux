...
static int fsl_ehci_drv_probe(struct platform_device *pdev)
{
	...
	if (pdata->has_fsl_erratum_a007792) {
		...
		pete_writel("drivers/usb/host/ehci-fsl.c:135", PORT_PTS_UTMI, hcd->regs + FSL_SOC_USB_PORTSC1);
	}
...
static int ehci_start_port_reset(struct usb_hcd *hcd, unsigned port)
{
	...
	status = pete_readl("drivers/usb/host/ehci-fsl.c:654", &ehci->regs->port_status[port]);
	if (!(status & PORT_CONNECT))
		return -ENODEV;

	/* hub_wq will finish the reset later */
	if (ehci_is_TDI(ehci)) {
		pete_writel("drivers/usb/host/ehci-fsl.c:660", PORT_RESET |
		       (status & ~(PORT_CSC | PORT_PEC | PORT_OCC)),
		       &ehci->regs->port_status[port]);
	} else {
		pete_writel("drivers/usb/host/ehci-fsl.c:664", PORT_RESET, &ehci->regs->port_status[port]);
	}
