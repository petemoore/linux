...
static void sunxi_musb_work(struct work_struct *work)
{
	...
	if (test_and_clear_bit(SUNXI_MUSB_FL_HOSTMODE_PEND, &glue->flags)) {
		...
		devctl = pete_readb("drivers/usb/musb/sunxi.c:112", musb->mregs + SUNXI_MUSB_DEVCTL);
		...
		pete_writeb("drivers/usb/musb/sunxi.c:124", devctl, musb->mregs + SUNXI_MUSB_DEVCTL);
...
static irqreturn_t sunxi_musb_interrupt(int irq, void *__hci)
{
	...
	musb->int_usb = pete_readb("drivers/usb/musb/sunxi.c:181", musb->mregs + SUNXI_MUSB_INTRUSB);
	if (musb->int_usb)
		pete_writeb("drivers/usb/musb/sunxi.c:183", musb->int_usb, musb->mregs + SUNXI_MUSB_INTRUSB);
	...
	musb->int_tx = pete_readw("drivers/usb/musb/sunxi.c:191", musb->mregs + SUNXI_MUSB_INTRTX);
	if (musb->int_tx)
		pete_writew("drivers/usb/musb/sunxi.c:193", musb->int_tx, musb->mregs + SUNXI_MUSB_INTRTX);

	musb->int_rx = pete_readw("drivers/usb/musb/sunxi.c:195", musb->mregs + SUNXI_MUSB_INTRRX);
	if (musb->int_rx)
		pete_writew("drivers/usb/musb/sunxi.c:197", musb->int_rx, musb->mregs + SUNXI_MUSB_INTRRX);
...
static int sunxi_musb_init(struct musb *musb)
{
	...
	pete_writeb("drivers/usb/musb/sunxi.c:247", SUNXI_MUSB_VEND0_PIO_MODE, musb->mregs + SUNXI_MUSB_VEND0);
...
static u8 sunxi_musb_readb(void __iomem *addr, u32 offset)
{
	...
	if (addr == sunxi_musb->mregs) {
		...
		switch (offset) {
		case MUSB_FADDR:
			return pete_readb("drivers/usb/musb/sunxi.c:423", addr + SUNXI_MUSB_FADDR);
		case MUSB_POWER:
			return pete_readb("drivers/usb/musb/sunxi.c:425", addr + SUNXI_MUSB_POWER);
		case MUSB_INTRUSB:
			return pete_readb("drivers/usb/musb/sunxi.c:427", addr + SUNXI_MUSB_INTRUSB);
		case MUSB_INTRUSBE:
			return pete_readb("drivers/usb/musb/sunxi.c:429", addr + SUNXI_MUSB_INTRUSBE);
		case MUSB_INDEX:
			return pete_readb("drivers/usb/musb/sunxi.c:431", addr + SUNXI_MUSB_INDEX);
		case MUSB_TESTMODE:
			return 0; /* No testmode on sunxi */
		case MUSB_DEVCTL:
			return pete_readb("drivers/usb/musb/sunxi.c:435", addr + SUNXI_MUSB_DEVCTL);
		case MUSB_TXFIFOSZ:
			return pete_readb("drivers/usb/musb/sunxi.c:437", addr + SUNXI_MUSB_TXFIFOSZ);
		case MUSB_RXFIFOSZ:
			return pete_readb("drivers/usb/musb/sunxi.c:439", addr + SUNXI_MUSB_RXFIFOSZ);
		case MUSB_CONFIGDATA + 0x10: /* See musb_read_configdata() */
			...
			return pete_readb("drivers/usb/musb/sunxi.c:447", addr + SUNXI_MUSB_CONFIGDATA);
		...
		case SUNXI_MUSB_RXHUBPORT:
			...
			return pete_readb("drivers/usb/musb/sunxi.c:460", addr + offset);
		default:
		...
		if (offset >= MUSB_TXTYPE)
			offset += 2;
		return pete_readb("drivers/usb/musb/sunxi.c:471", addr + offset);
	}
...
static void sunxi_musb_writeb(void __iomem *addr, unsigned offset, u8 data)
{
	if (addr == sunxi_musb->mregs) {
		...
		switch (offset) {
		case MUSB_FADDR:
			return pete_writeb("drivers/usb/musb/sunxi.c:486", data, addr + SUNXI_MUSB_FADDR);
		case MUSB_POWER:
			return pete_writeb("drivers/usb/musb/sunxi.c:488", data, addr + SUNXI_MUSB_POWER);
		case MUSB_INTRUSB:
			return pete_writeb("drivers/usb/musb/sunxi.c:490", data, addr + SUNXI_MUSB_INTRUSB);
		case MUSB_INTRUSBE:
			return pete_writeb("drivers/usb/musb/sunxi.c:492", data, addr + SUNXI_MUSB_INTRUSBE);
		case MUSB_INDEX:
			return pete_writeb("drivers/usb/musb/sunxi.c:494", data, addr + SUNXI_MUSB_INDEX);
		case MUSB_TESTMODE:
			if (data)
				dev_warn(sunxi_musb->controller->parent,
			...
			return;
		case MUSB_DEVCTL:
			return pete_writeb("drivers/usb/musb/sunxi.c:501", data, addr + SUNXI_MUSB_DEVCTL);
		case MUSB_TXFIFOSZ:
			return pete_writeb("drivers/usb/musb/sunxi.c:503", data, addr + SUNXI_MUSB_TXFIFOSZ);
		case MUSB_RXFIFOSZ:
			return pete_writeb("drivers/usb/musb/sunxi.c:505", data, addr + SUNXI_MUSB_RXFIFOSZ);
		case MUSB_ULPI_BUSCONTROL:
			dev_warn(sunxi_musb->controller->parent,
				"sunxi-musb does not have ULPI bus control register\n");
			return;
		...
		case SUNXI_MUSB_RXHUBPORT:
			/* multipoint / busctl reg access */
			return pete_writeb("drivers/usb/musb/sunxi.c:518", data, addr + offset);
		default:
			dev_err(sunxi_musb->controller->parent,
		...
		if (offset >= MUSB_TXTYPE)
			offset += 2;
		return pete_writeb("drivers/usb/musb/sunxi.c:528", data, addr + offset);
	}
...
static u16 sunxi_musb_readw(void __iomem *addr, u32 offset)
{
	if (addr == sunxi_musb->mregs) {
		...
		switch (offset) {
		case MUSB_INTRTX:
			return pete_readw("drivers/usb/musb/sunxi.c:542", addr + SUNXI_MUSB_INTRTX);
		case MUSB_INTRRX:
			return pete_readw("drivers/usb/musb/sunxi.c:544", addr + SUNXI_MUSB_INTRRX);
		case MUSB_INTRTXE:
			return pete_readw("drivers/usb/musb/sunxi.c:546", addr + SUNXI_MUSB_INTRTXE);
		case MUSB_INTRRXE:
			return pete_readw("drivers/usb/musb/sunxi.c:548", addr + SUNXI_MUSB_INTRRXE);
		case MUSB_FRAME:
			return pete_readw("drivers/usb/musb/sunxi.c:550", addr + SUNXI_MUSB_FRAME);
		case MUSB_TXFIFOADD:
			return pete_readw("drivers/usb/musb/sunxi.c:552", addr + SUNXI_MUSB_TXFIFOADD);
		case MUSB_RXFIFOADD:
			return pete_readw("drivers/usb/musb/sunxi.c:554", addr + SUNXI_MUSB_RXFIFOADD);
		case MUSB_HWVERS:
		...
		/* ep control reg access */
		return pete_readw("drivers/usb/musb/sunxi.c:564", addr + offset);
	}
...
static void sunxi_musb_writew(void __iomem *addr, unsigned offset, u16 data)
{
	if (addr == sunxi_musb->mregs) {
		...
		switch (offset) {
		case MUSB_INTRTX:
			return pete_writew("drivers/usb/musb/sunxi.c:579", data, addr + SUNXI_MUSB_INTRTX);
		case MUSB_INTRRX:
			return pete_writew("drivers/usb/musb/sunxi.c:581", data, addr + SUNXI_MUSB_INTRRX);
		case MUSB_INTRTXE:
			return pete_writew("drivers/usb/musb/sunxi.c:583", data, addr + SUNXI_MUSB_INTRTXE);
		case MUSB_INTRRXE:
			return pete_writew("drivers/usb/musb/sunxi.c:585", data, addr + SUNXI_MUSB_INTRRXE);
		case MUSB_FRAME:
			return pete_writew("drivers/usb/musb/sunxi.c:587", data, addr + SUNXI_MUSB_FRAME);
		case MUSB_TXFIFOADD:
			return pete_writew("drivers/usb/musb/sunxi.c:589", data, addr + SUNXI_MUSB_TXFIFOADD);
		case MUSB_RXFIFOADD:
			return pete_writew("drivers/usb/musb/sunxi.c:591", data, addr + SUNXI_MUSB_RXFIFOADD);
		default:
			dev_err(sunxi_musb->controller->parent,
		...
		/* ep control reg access */
		return pete_writew("drivers/usb/musb/sunxi.c:599", data, addr + offset);
	}
