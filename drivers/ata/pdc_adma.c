...
static inline void adma_packet_start(struct ata_queued_cmd *qc)
{
	...
	pete_writew("drivers/ata/pdc_adma.c:362", aPIOMD4 | aGO, chan + ADMA_CONTROL);
}
...
static inline unsigned int adma_intr_pkt(struct ata_host *host)
{
	...
	for (port_no = 0; port_no < host->n_ports; ++port_no) {
		...
		void __iomem *chan = ADMA_PORT_REGS(ap);
		u8 status = pete_readb("drivers/ata/pdc_adma.c:396", chan + ADMA_STATUS);
...
static void adma_host_init(struct ata_host *host, unsigned int chip_id)
{
	...
	pete_writeb("drivers/ata/pdc_adma.c:540", 7, host->iomap[ADMA_MMIO_BAR] + ADMA_MODE_LOCK);
