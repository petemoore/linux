...
static inline void qs_packet_start(struct ata_queued_cmd *qc)
{
	...
	pete_writeb("drivers/ata/sata_qstor.c:302", QS_CTR0_CLER, chan + QS_CCT_CTR0);
	wmb();                             /* flush PRDs and pkt to memory */
	pete_writel("drivers/ata/sata_qstor.c:304", QS_CCF_RUN_PKT, chan + QS_CCT_CFF);
	pete_readl("drivers/ata/sata_qstor.c:305", chan + QS_CCT_CFF);          /* flush */
}
...
static inline unsigned int qs_intr_pkt(struct ata_host *host)
{
	...
	do {
		u32 sff0 = pete_readl("drivers/ata/sata_qstor.c:357", mmio_base + QS_HST_SFF);
		u32 sff1 = pete_readl("drivers/ata/sata_qstor.c:358", mmio_base + QS_HST_SFF + 4);
...
static int qs_port_start(struct ata_port *ap)
{
	...
	qs_enter_reg_mode(ap);
	addr = (u64)pp->pkt_dma;
	pete_writel("drivers/ata/sata_qstor.c:475", (u32) addr,        chan + QS_CCF_CPBA);
	pete_writel("drivers/ata/sata_qstor.c:476", (u32)(addr >> 32), chan + QS_CCF_CPBA + 4);
	return 0;
}
...
static void qs_host_stop(struct ata_host *host)
{
	...
	pete_writeb("drivers/ata/sata_qstor.c:484", 0, mmio_base + QS_HCT_CTRL); /* disable host interrupts */
	pete_writeb("drivers/ata/sata_qstor.c:485", QS_CNFG3_GSRST, mmio_base + QS_HCF_CNFG3); /* global reset */
}
...
static void qs_host_init(struct ata_host *host, unsigned int chip_id)
{
	...
	pete_writeb("drivers/ata/sata_qstor.c:493", 0, mmio_base + QS_HCT_CTRL); /* disable host interrupts */
	pete_writeb("drivers/ata/sata_qstor.c:494", QS_CNFG3_GSRST, mmio_base + QS_HCF_CNFG3); /* global reset */

	/* reset each channel in turn */
	for (port_no = 0; port_no < host->n_ports; ++port_no) {
		u8 __iomem *chan = mmio_base + (port_no * 0x4000);
		pete_writeb("drivers/ata/sata_qstor.c:499", QS_CTR1_RDEV|QS_CTR1_RCHN, chan + QS_CCT_CTR1);
		pete_writeb("drivers/ata/sata_qstor.c:500", QS_CTR0_REG, chan + QS_CCT_CTR0);
		pete_readb("drivers/ata/sata_qstor.c:501", chan + QS_CCT_CTR0);        /* flush */
	}
	pete_writeb("drivers/ata/sata_qstor.c:503", QS_SERD3_PHY_ENA, mmio_base + QS_HVS_SERD3); /* enable phy */

	for (port_no = 0; port_no < host->n_ports; ++port_no) {
		u8 __iomem *chan = mmio_base + (port_no * 0x4000);
		/* set FIFO depths to same settings as Windows driver */
		pete_writew("drivers/ata/sata_qstor.c:508", 32, chan + QS_CFC_HUFT);
		pete_writew("drivers/ata/sata_qstor.c:509", 32, chan + QS_CFC_HDFT);
		pete_writew("drivers/ata/sata_qstor.c:510", 10, chan + QS_CFC_DUFT);
		pete_writew("drivers/ata/sata_qstor.c:511",  8, chan + QS_CFC_DDFT);
		/* set CPB size in bytes, as a power of two */
		pete_writeb("drivers/ata/sata_qstor.c:513", QS_CPB_ORDER,    chan + QS_CCF_CSEP);
	}
	pete_writeb("drivers/ata/sata_qstor.c:515", 1, mmio_base + QS_HCT_CTRL); /* enable host interrupts */
}
...
{
	u32 bus_info = pete_readl("drivers/ata/sata_qstor.c:530", mmio_base + QS_HID_HPHY);
