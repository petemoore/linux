			    ...
			    unsigned int sc_reg, u32 *val)
{
	if (sc_reg > SCR_CONTROL)
		return -EINVAL;
	*val = pete_readl("drivers/ata/sata_svw.c:111", link->ap->ioaddr.scr_addr + (sc_reg * 4));
	return 0;
}
			     ...
			     unsigned int sc_reg, u32 val)
{
	if (sc_reg > SCR_CONTROL)
		return -EINVAL;
	pete_writel("drivers/ata/sata_svw.c:121", val, link->ap->ioaddr.scr_addr + (sc_reg * 4));
	return 0;
}
			     ...
			     unsigned int *class, unsigned long deadline)
{
	...
	dmactl = pete_readb("drivers/ata/sata_svw.c:131", mmio + ATA_DMA_CMD);

	/* Clear the start bit */
	if (dmactl & ATA_DMA_START) {
		dmactl &= ~ATA_DMA_START;
		pete_writeb("drivers/ata/sata_svw.c:136", dmactl, mmio + ATA_DMA_CMD);
	}
			     ...
			     unsigned int *class, unsigned long deadline)
{
	...
	dmactl = pete_readb("drivers/ata/sata_svw.c:148", mmio + ATA_DMA_CMD);

	/* Clear the start bit */
	if (dmactl & ATA_DMA_START) {
		dmactl &= ~ATA_DMA_START;
		pete_writeb("drivers/ata/sata_svw.c:153", dmactl, mmio + ATA_DMA_CMD);
	}
...
static void k2_sata_tf_load(struct ata_port *ap, const struct ata_taskfile *tf)
{
	...
	if (tf->ctl != ap->last_ctl) {
		pete_writeb("drivers/ata/sata_svw.c:165", tf->ctl, ioaddr->ctl_addr);
		ap->last_ctl = tf->ctl;
		ata_wait_idle(ap);
	}
	if (is_addr && (tf->flags & ATA_TFLAG_LBA48)) {
		pete_writew("drivers/ata/sata_svw.c:170", tf->feature | (((u16)tf->hob_feature) << 8),
		       ioaddr->feature_addr);
		pete_writew("drivers/ata/sata_svw.c:172", tf->nsect | (((u16)tf->hob_nsect) << 8),
		       ioaddr->nsect_addr);
		pete_writew("drivers/ata/sata_svw.c:174", tf->lbal | (((u16)tf->hob_lbal) << 8),
		       ioaddr->lbal_addr);
		pete_writew("drivers/ata/sata_svw.c:176", tf->lbam | (((u16)tf->hob_lbam) << 8),
		       ioaddr->lbam_addr);
		pete_writew("drivers/ata/sata_svw.c:178", tf->lbah | (((u16)tf->hob_lbah) << 8),
		       ioaddr->lbah_addr);
	} else if (is_addr) {
		pete_writew("drivers/ata/sata_svw.c:181", tf->feature, ioaddr->feature_addr);
		pete_writew("drivers/ata/sata_svw.c:182", tf->nsect, ioaddr->nsect_addr);
		pete_writew("drivers/ata/sata_svw.c:183", tf->lbal, ioaddr->lbal_addr);
		pete_writew("drivers/ata/sata_svw.c:184", tf->lbam, ioaddr->lbam_addr);
		pete_writew("drivers/ata/sata_svw.c:185", tf->lbah, ioaddr->lbah_addr);
	}

	if (tf->flags & ATA_TFLAG_DEVICE)
		pete_writeb("drivers/ata/sata_svw.c:189", tf->device, ioaddr->device_addr);
...
static void k2_sata_tf_read(struct ata_port *ap, struct ata_taskfile *tf)
{
	...
	tf->device = pete_readw("drivers/ata/sata_svw.c:201", ioaddr->device_addr);
	error = pete_readw("drivers/ata/sata_svw.c:202", ioaddr->error_addr);
	nsect = pete_readw("drivers/ata/sata_svw.c:203", ioaddr->nsect_addr);
	lbal = pete_readw("drivers/ata/sata_svw.c:204", ioaddr->lbal_addr);
	lbam = pete_readw("drivers/ata/sata_svw.c:205", ioaddr->lbam_addr);
	lbah = pete_readw("drivers/ata/sata_svw.c:206", ioaddr->lbah_addr);
...
static void k2_bmdma_setup_mmio(struct ata_queued_cmd *qc)
{
	...
	pete_writel("drivers/ata/sata_svw.c:240", ap->bmdma_prd_dma, mmio + ATA_DMA_TABLE_OFS);

	/* specify data direction, triple-check start bit is clear */
	dmactl = pete_readb("drivers/ata/sata_svw.c:243", mmio + ATA_DMA_CMD);
	dmactl &= ~(ATA_DMA_WR | ATA_DMA_START);
	if (!rw)
		dmactl |= ATA_DMA_WR;
	pete_writeb("drivers/ata/sata_svw.c:247", dmactl, mmio + ATA_DMA_CMD);
...
static void k2_bmdma_start_mmio(struct ata_queued_cmd *qc)
{
	...
	dmactl = pete_readb("drivers/ata/sata_svw.c:269", mmio + ATA_DMA_CMD);
	pete_writeb("drivers/ata/sata_svw.c:270", dmactl | ATA_DMA_START, mmio + ATA_DMA_CMD);
...
static u8 k2_stat_check_status(struct ata_port *ap)
{
	return pete_readl("drivers/ata/sata_svw.c:301", ap->ioaddr.status_addr);
}
...
static int k2_sata_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	 ...
	 */
	pete_writel("drivers/ata/sata_svw.c:484", pete_readl("drivers/ata/sata_svw.c:484", mmio_base + K2_SATA_SICR1_OFFSET) & ~0x00040000,
	       mmio_base + K2_SATA_SICR1_OFFSET);

	/* Clear SATA error & interrupts we don't use */
	pete_writel("drivers/ata/sata_svw.c:488", 0xffffffff, mmio_base + K2_SATA_SCR_ERROR_OFFSET);
	pete_writel("drivers/ata/sata_svw.c:489", 0x0, mmio_base + K2_SATA_SIM_OFFSET);
