...
static void ahci_ceva_setup(struct ahci_host_priv *hpriv)
{
	...
	/* Set AHCI Enable */
	tmp = pete_readl("drivers/ata/ahci_ceva.c:130", mmio + HOST_CTL);
	tmp |= HOST_AHCI_EN;
	pete_writel("drivers/ata/ahci_ceva.c:132", tmp, mmio + HOST_CTL);

	for (i = 0; i < NR_PORTS; i++) {
		/* TPSS TPRS scalars, CISE and Port Addr */
		tmp = PCFG_TPSS_VAL | PCFG_TPRS_VAL | (PCFG_PAD_VAL + i);
		pete_writel("drivers/ata/ahci_ceva.c:137", tmp, mmio + AHCI_VEND_PCFG);
		...
		pete_writel("drivers/ata/ahci_ceva.c:147", tmp, mmio + AHCI_VEND_PAXIC);

		/* Set AXI cache control register if CCi is enabled */
		if (cevapriv->is_cci_enabled) {
			tmp = pete_readl("drivers/ata/ahci_ceva.c:151", mmio + AHCI_VEND_AXICC);
			tmp |= AXICC_ARCA_VAL | AXICC_ARCF_VAL |
				AXICC_ARCH_VAL | AXICC_ARCP_VAL |
				AXICC_AWCFD_VAL | AXICC_AWCD_VAL |
				AXICC_AWCF_VAL;
			pete_writel("drivers/ata/ahci_ceva.c:156", tmp, mmio + AHCI_VEND_AXICC);
		}

		/* Port Phy Cfg register enables */
		tmp = PPCFG_TTA | PPCFG_PSS_EN | PPCFG_ESDF_EN;
		pete_writel("drivers/ata/ahci_ceva.c:161", tmp, mmio + AHCI_VEND_PPCFG);

		/* Phy Control OOB timing parameters COMINIT */
		pete_writel("drivers/ata/ahci_ceva.c:164", cevapriv->pp2c[i], mmio + AHCI_VEND_PP2C);

		/* Phy Control OOB timing parameters COMWAKE */
		pete_writel("drivers/ata/ahci_ceva.c:167", cevapriv->pp3c[i], mmio + AHCI_VEND_PP3C);

		/* Phy Control Burst timing setting */
		pete_writel("drivers/ata/ahci_ceva.c:170", cevapriv->pp4c[i], mmio + AHCI_VEND_PP4C);

		/* Rate Change Timer and Retry Interval Timer setting */
		pete_writel("drivers/ata/ahci_ceva.c:173", cevapriv->pp5c[i], mmio + AHCI_VEND_PP5C);

		/* Rx Watermark setting  */
		tmp = rx_watermark | PTC_RSVD;
		pete_writel("drivers/ata/ahci_ceva.c:177", tmp, mmio + AHCI_VEND_PTC);
		...
		pete_writel("drivers/ata/ahci_ceva.c:183", tmp, mmio + PORT_SCR_CTL + PORT_BASE + PORT_OFFSET * i);
	}
}
