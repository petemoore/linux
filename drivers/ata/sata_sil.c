...
static int sil_set_mode(struct ata_link *link, struct ata_device **r_failed)
{
	...
	tmp = pete_readl("drivers/ata/sata_sil.c:368", addr);
	tmp &= ~((1<<5) | (1<<4) | (1<<1) | (1<<0));
	tmp |= dev_mode[0];
	tmp |= (dev_mode[1] << 4);
	pete_writel("drivers/ata/sata_sil.c:372", tmp, addr);
	pete_readl("drivers/ata/sata_sil.c:373", addr);	/* flush */
	return 0;
}
...
static int sil_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
{
	...
	if (mmio) {
		*val = pete_readl("drivers/ata/sata_sil.c:402", mmio);
		return 0;
	}
	return -EINVAL;
}
...
static int sil_scr_write(struct ata_link *link, unsigned int sc_reg, u32 val)
{
	...
	if (mmio) {
		pete_writel("drivers/ata/sata_sil.c:413", val, mmio);
		return 0;
	}
	return -EINVAL;
}
...
static irqreturn_t sil_interrupt(int irq, void *dev_instance)
{
	...
	for (i = 0; i < host->n_ports; i++) {
		...
		u32 bmdma2 = pete_readl("drivers/ata/sata_sil.c:517", mmio_base + sil_port[ap->port_no].bmdma2);
...
static void sil_freeze(struct ata_port *ap)
{
	...
	pete_writel("drivers/ata/sata_sil.c:542", 0, mmio_base + sil_port[ap->port_no].sien);

	/* plug IRQ */
	tmp = pete_readl("drivers/ata/sata_sil.c:545", mmio_base + SIL_SYSCFG);
	tmp |= SIL_MASK_IDE0_INT << ap->port_no;
	pete_writel("drivers/ata/sata_sil.c:547", tmp, mmio_base + SIL_SYSCFG);
	pete_readl("drivers/ata/sata_sil.c:548", mmio_base + SIL_SYSCFG);	/* flush */
...
static void sil_thaw(struct ata_port *ap)
{
	...
	if (!(ap->flags & SIL_FLAG_NO_SATA_IRQ))
		pete_writel("drivers/ata/sata_sil.c:575", SIL_SIEN_N, mmio_base + sil_port[ap->port_no].sien);

	/* turn on IRQ */
	tmp = pete_readl("drivers/ata/sata_sil.c:578", mmio_base + SIL_SYSCFG);
	tmp &= ~(SIL_MASK_IDE0_INT << ap->port_no);
	pete_writel("drivers/ata/sata_sil.c:580", tmp, mmio_base + SIL_SYSCFG);
}
...
static void sil_init_controller(struct ata_host *host)
{
	...
	if (cls) {
		...
		for (i = 0; i < host->n_ports; i++)
			pete_writew("drivers/ata/sata_sil.c:664", cls << 8 | cls,
	...
	if (host->ports[0]->flags & SIL_FLAG_RERR_ON_DMA_ACT) {
		...
		for (i = 0, cnt = 0; i < host->n_ports; i++) {
			tmp = pete_readl("drivers/ata/sata_sil.c:675", mmio_base + sil_port[i].sfis_cfg);
			if ((tmp & 0x3) != 0x01)
				continue;
			if (!cnt)
				dev_info(&pdev->dev,
					 "Applying R_ERR on DMA activate FIS errata fix\n");
			pete_writel("drivers/ata/sata_sil.c:681", tmp & ~0x3, mmio_base + sil_port[i].sfis_cfg);
			cnt++;
		}
	}
	...
	if (host->n_ports == 4) {
		...
		tmp = pete_readl("drivers/ata/sata_sil.c:688", mmio_base + sil_port[2].bmdma);
		if ((tmp & SIL_INTR_STEERING) == 0)
			pete_writel("drivers/ata/sata_sil.c:690", tmp | SIL_INTR_STEERING,
