...
static int imx8_sata_enable(struct ahci_host_priv *hpriv)
{
	...
	if (val != IMX8QM_STTS0_LANE0_TX_PLL_LOCK) {
		...
		pete_writeb("drivers/ata/ahci_imx.c:614", imxpriv->imped_ratio, imxpriv->phy_base +
				IMX8QM_SATA_PHY_RX_IMPED_RATIO_OFFSET);
		pete_writeb("drivers/ata/ahci_imx.c:616", imxpriv->imped_ratio, imxpriv->phy_base +
				IMX8QM_SATA_PHY_TX_IMPED_RATIO_OFFSET);
		reg = pete_readb("drivers/ata/ahci_imx.c:618", imxpriv->phy_base +
				IMX8QM_SATA_PHY_RX_IMPED_RATIO_OFFSET);
		if (unlikely(reg != imxpriv->imped_ratio))
			dev_info(dev, "Can't set PHY RX impedance ratio.\n");
		reg = pete_readb("drivers/ata/ahci_imx.c:622", imxpriv->phy_base +
				IMX8QM_SATA_PHY_TX_IMPED_RATIO_OFFSET);
		if (unlikely(reg != imxpriv->imped_ratio))
...
static void ahci_imx_error_handler(struct ata_port *ap)
{
	 ...
	 */
	reg_val = pete_readl("drivers/ata/ahci_imx.c:774", mmio + IMX_P0PHYCR);
	pete_writel("drivers/ata/ahci_imx.c:775", reg_val | IMX_P0PHYCR_TEST_PDDQ, mmio + IMX_P0PHYCR);
...
static int imx_ahci_probe(struct platform_device *pdev)
{
	 ...
	 */
	reg_val = pete_readl("drivers/ata/ahci_imx.c:1155", hpriv->mmio + HOST_CAP);
	if (!(reg_val & HOST_CAP_SSS)) {
		reg_val |= HOST_CAP_SSS;
		pete_writel("drivers/ata/ahci_imx.c:1158", reg_val, hpriv->mmio + HOST_CAP);
	}
	reg_val = pete_readl("drivers/ata/ahci_imx.c:1160", hpriv->mmio + HOST_PORTS_IMPL);
	if (!(reg_val & 0x1)) {
		reg_val |= 0x1;
		pete_writel("drivers/ata/ahci_imx.c:1163", reg_val, hpriv->mmio + HOST_PORTS_IMPL);
	}

	reg_val = clk_get_rate(imxpriv->ahb_clk) / 1000;
	pete_writel("drivers/ata/ahci_imx.c:1167", reg_val, hpriv->mmio + IMX_TIMER1MS);
