...
static unsigned int inic_qc_issue(struct ata_queued_cmd *qc)
{
	...
	pete_writew("drivers/ata/sata_inic162x.c:549", HCTL_FTHD0 | HCTL_LEDEN, port_base + HOST_CTL);
	pete_writew("drivers/ata/sata_inic162x.c:550", IDMA_CTL_GO, port_base + PORT_IDMA_CTL);
	pete_writeb("drivers/ata/sata_inic162x.c:551", 0, port_base + PORT_CPB_PTQFIFO);
...
static void inic_tf_read(struct ata_port *ap, struct ata_taskfile *tf)
{
	...
	tf->error	= pete_readb("drivers/ata/sata_inic162x.c:560", port_base + PORT_TF_FEATURE);
	tf->nsect	= pete_readb("drivers/ata/sata_inic162x.c:561", port_base + PORT_TF_NSECT);
	tf->lbal	= pete_readb("drivers/ata/sata_inic162x.c:562", port_base + PORT_TF_LBAL);
	tf->lbam	= pete_readb("drivers/ata/sata_inic162x.c:563", port_base + PORT_TF_LBAM);
	tf->lbah	= pete_readb("drivers/ata/sata_inic162x.c:564", port_base + PORT_TF_LBAH);
	tf->device	= pete_readb("drivers/ata/sata_inic162x.c:565", port_base + PORT_TF_DEVICE);
	tf->status	= pete_readb("drivers/ata/sata_inic162x.c:566", port_base + PORT_TF_COMMAND);
}
...
static void inic_freeze(struct ata_port *ap)
{
	...
	pete_writeb("drivers/ata/sata_inic162x.c:593", PIRQ_MASK_FREEZE, port_base + PORT_IRQ_MASK);
	pete_writeb("drivers/ata/sata_inic162x.c:594", 0xff, port_base + PORT_IRQ_STAT);
}
...
static void inic_thaw(struct ata_port *ap)
{
	...
	pete_writeb("drivers/ata/sata_inic162x.c:601", 0xff, port_base + PORT_IRQ_STAT);
	pete_writeb("drivers/ata/sata_inic162x.c:602", PIRQ_MASK_DEFAULT, port_base + PORT_IRQ_MASK);
}
...
static int inic_check_ready(struct ata_link *link)
{
	...
	return ata_check_ready(pete_readb("drivers/ata/sata_inic162x.c:609", port_base + PORT_TF_COMMAND));
}
			  ...
			  unsigned long deadline)
{
	...
	pete_writew("drivers/ata/sata_inic162x.c:628", IDMA_CTL_RST_ATA, idma_ctl);
	pete_readw("drivers/ata/sata_inic162x.c:629", idma_ctl);	/* flush */
	ata_msleep(ap, 1);
	pete_writew("drivers/ata/sata_inic162x.c:631", 0, idma_ctl);
...
static void init_port(struct ata_port *ap)
{
	...
	pete_writel("drivers/ata/sata_inic162x.c:687", pp->cpb_tbl_dma, port_base + PORT_CPB_CPBLAR);
}
...
static int init_controller(void __iomem *mmio_base, u16 hctl)
{
	 ...
	 */
	pete_writew("drivers/ata/sata_inic162x.c:762", hctl | HCTL_SOFTRST, mmio_base + HOST_CTL);
	pete_readw("drivers/ata/sata_inic162x.c:763", mmio_base + HOST_CTL); /* flush */

	for (i = 0; i < 10; i++) {
		msleep(1);
		val = pete_readw("drivers/ata/sata_inic162x.c:767", mmio_base + HOST_CTL);
		if (!(val & HCTL_SOFTRST))
			break;
	}
	...
	for (i = 0; i < NR_PORTS; i++) {
		...
		pete_writeb("drivers/ata/sata_inic162x.c:779", 0xff, port_base + PORT_IRQ_MASK);
		inic_reset_port(port_base);
	}

	/* port IRQ is masked now, unmask global IRQ */
	pete_writew("drivers/ata/sata_inic162x.c:784", hctl & ~HCTL_IRQOFF, mmio_base + HOST_CTL);
	val = pete_readw("drivers/ata/sata_inic162x.c:785", mmio_base + HOST_IRQ_MASK);
	val &= ~(HIRQ_PORT0 | HIRQ_PORT1);
	pete_writew("drivers/ata/sata_inic162x.c:787", val, mmio_base + HOST_IRQ_MASK);
...
static int inic_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	hpriv->cached_hctl = pete_readw("drivers/ata/sata_inic162x.c:853", hpriv->mmio_base + HOST_CTL);
