...
static void ftide010_set_dmamode(struct ata_port *ap, struct ata_device *adev)
{
	...
	clkreg = pete_readb("drivers/ata/pata_ftide010.c:160", ftide->base + FTIDE010_CLK_MOD);
	clkreg &= ~udma_en_mask;
	clkreg &= ~f66m_en_mask;
	...
	if (speed & XFER_UDMA_0) {
		...
		pete_writeb("drivers/ata/pata_ftide010.c:186", clkreg, ftide->base + FTIDE010_CLK_MOD);
		pete_writeb("drivers/ata/pata_ftide010.c:187", timreg, ftide->base + FTIDE010_UDMA_TIMING0 + devno);
	} else {
		...
		pete_writeb("drivers/ata/pata_ftide010.c:205", clkreg, ftide->base + FTIDE010_CLK_MOD);
		pete_writeb("drivers/ata/pata_ftide010.c:206", timreg, ftide->base + FTIDE010_MWDMA_TIMING);
	}
...
static void ftide010_set_piomode(struct ata_port *ap, struct ata_device *adev)
{
	...
	dev_dbg(ftide->dev, "set PIO mode %02x, index %d\n",
		adev->pio_mode, pio);
	pete_writeb("drivers/ata/pata_ftide010.c:226", pio_active_time[pio] << 4 | pio_recovery_time[pio],
...
static int pata_ftide010_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "device ID %08x, irq %d, reg %pR\n",
		 pete_readl("drivers/ata/pata_ftide010.c:524", ftide->base + FTIDE010_IDE_DEVICE_ID), irq, res);
