...
static void pdc_freeze(struct ata_port *ap)
{
	...
	tmp = pete_readl("drivers/ata/sata_promise.c:699", ata_mmio + PDC_CTLSTAT);
	tmp |= PDC_IRQ_DISABLE;
	tmp &= ~PDC_DMA_ENABLE;
	pete_writel("drivers/ata/sata_promise.c:702", tmp, ata_mmio + PDC_CTLSTAT);
	pete_readl("drivers/ata/sata_promise.c:703", ata_mmio + PDC_CTLSTAT); /* flush */
}
...
static void pdc_sata_freeze(struct ata_port *ap)
{
	 ...
	 */
	hotplug_status = pete_readl("drivers/ata/sata_promise.c:721", host_mmio + hotplug_offset);
	hotplug_status |= 0x11 << (ata_no + 16);
	pete_writel("drivers/ata/sata_promise.c:723", hotplug_status, host_mmio + hotplug_offset);
	pete_readl("drivers/ata/sata_promise.c:724", host_mmio + hotplug_offset); /* flush */
...
static void pdc_thaw(struct ata_port *ap)
{
	...
	/* clear IRQ */
	pete_readl("drivers/ata/sata_promise.c:735", ata_mmio + PDC_COMMAND);

	/* turn IRQ back on */
	tmp = pete_readl("drivers/ata/sata_promise.c:738", ata_mmio + PDC_CTLSTAT);
	tmp &= ~PDC_IRQ_DISABLE;
	pete_writel("drivers/ata/sata_promise.c:740", tmp, ata_mmio + PDC_CTLSTAT);
	pete_readl("drivers/ata/sata_promise.c:741", ata_mmio + PDC_CTLSTAT); /* flush */
}
...
static void pdc_sata_thaw(struct ata_port *ap)
{
	 ...
	 */
	hotplug_status = pete_readl("drivers/ata/sata_promise.c:757", host_mmio + hotplug_offset);
	hotplug_status |= 0x11 << ata_no;
	hotplug_status &= ~(0x11 << (ata_no + 16));
	pete_writel("drivers/ata/sata_promise.c:760", hotplug_status, host_mmio + hotplug_offset);
	pete_readl("drivers/ata/sata_promise.c:761", host_mmio + hotplug_offset); /* flush */
}
...
static void pdc_hard_reset_port(struct ata_port *ap)
{
	...
	tmp = pete_readb("drivers/ata/sata_promise.c:790", pcictl_b1_mmio);
	tmp &= ~(0x10 << ata_no);
	pete_writeb("drivers/ata/sata_promise.c:792", tmp, pcictl_b1_mmio);
	pete_readb("drivers/ata/sata_promise.c:793", pcictl_b1_mmio); /* flush */
	udelay(100);
	tmp |= (0x10 << ata_no);
	pete_writeb("drivers/ata/sata_promise.c:796", tmp, pcictl_b1_mmio);
	pete_readb("drivers/ata/sata_promise.c:797", pcictl_b1_mmio); /* flush */
				  ...
				  struct ata_queued_cmd *qc)
{
	...
	port_status = pete_readl("drivers/ata/sata_promise.c:881", ata_mmio + PDC_GLOBAL_CTL);
...
static void pdc_irq_clear(struct ata_port *ap)
{
	...
	pete_readl("drivers/ata/sata_promise.c:908", ata_mmio + PDC_COMMAND);
}
...
static irqreturn_t pdc_interrupt(int irq, void *dev_instance)
{
	...
	if (host->ports[0]->flags & PDC_FLAG_GEN_II) {
		...
		hotplug_status = pete_readl("drivers/ata/sata_promise.c:933", host_mmio + hotplug_offset);
		if (hotplug_status & 0xff)
			pete_writel("drivers/ata/sata_promise.c:935", hotplug_status | 0xff, host_mmio + hotplug_offset);
		hotplug_status &= 0xff;	/* clear uninteresting bits */
	} else
		hotplug_status = 0;

	/* reading should also clear interrupts */
	mask = pete_readl("drivers/ata/sata_promise.c:941", host_mmio + PDC_INT_SEQMASK);
	...
	pete_writel("drivers/ata/sata_promise.c:950", mask, host_mmio + PDC_INT_SEQMASK);
...
static void pdc_packet_start(struct ata_queued_cmd *qc)
{
	...
	pete_writel("drivers/ata/sata_promise.c:995", 0x00000001, host_mmio + (seq * 4));
	pete_readl("drivers/ata/sata_promise.c:996", host_mmio + (seq * 4));	/* flush */

	pp->pkt[2] = seq;
	wmb();			/* flush PRD, pkt writes */
	pete_writel("drivers/ata/sata_promise.c:1000", pp->pkt_dma, ata_mmio + PDC_PKT_SUBMIT);
	pete_readl("drivers/ata/sata_promise.c:1001", ata_mmio + PDC_PKT_SUBMIT); /* flush */
}
...
static void pdc_host_init(struct ata_host *host)
{
	...
	tmp = pete_readl("drivers/ata/sata_promise.c:1112", host_mmio + PDC_FLASH_CTL);
	tmp |= 0x02000;	/* bit 13 (enable bmr burst) */
	if (!is_gen2)
		tmp |= 0x10000;	/* bit 16 (fifo threshold at 8 dw) */
	pete_writel("drivers/ata/sata_promise.c:1116", tmp, host_mmio + PDC_FLASH_CTL);

	/* clear plug/unplug flags for all ports */
	tmp = pete_readl("drivers/ata/sata_promise.c:1119", host_mmio + hotplug_offset);
	pete_writel("drivers/ata/sata_promise.c:1120", tmp | 0xff, host_mmio + hotplug_offset);

	tmp = pete_readl("drivers/ata/sata_promise.c:1122", host_mmio + hotplug_offset);
	if (is_gen2)	/* unmask plug/unplug ints */
		pete_writel("drivers/ata/sata_promise.c:1124", tmp & ~0xff0000, host_mmio + hotplug_offset);
	else		/* mask plug/unplug ints */
		pete_writel("drivers/ata/sata_promise.c:1126", tmp | 0xff0000, host_mmio + hotplug_offset);
	...
	tmp = pete_readl("drivers/ata/sata_promise.c:1133", host_mmio + PDC_TBG_MODE);
	tmp &= ~0x30000; /* clear bit 17, 16*/
	tmp |= 0x10000;  /* set bit 17:16 = 0:1 */
	pete_writel("drivers/ata/sata_promise.c:1136", tmp, host_mmio + PDC_TBG_MODE);

	pete_readl("drivers/ata/sata_promise.c:1138", host_mmio + PDC_TBG_MODE);	/* flush */
	msleep(10);

	/* adjust slew rate control register. */
	tmp = pete_readl("drivers/ata/sata_promise.c:1142", host_mmio + PDC_SLEW_CTL);
	tmp &= 0xFFFFF03F; /* clear bit 11 ~ 6 */
	tmp  |= 0x00000900; /* set bit 11-9 = 100b , bit 8-6 = 100 */
	pete_writel("drivers/ata/sata_promise.c:1145", tmp, host_mmio + PDC_SLEW_CTL);
}
			    ...
			    const struct pci_device_id *ent)
{
	...
	if (pi->flags & PDC_FLAG_SATA_PATA) {
		u8 tmp = pete_readb("drivers/ata/sata_promise.c:1181", host_mmio + PDC_FLASH_CTL + 1);
		if (!(tmp & 0x80))
			ppi[n_ports++] = pi + 1;
	}
