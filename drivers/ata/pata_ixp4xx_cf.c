					  ...
					  unsigned char *buf, unsigned int buflen, int rw)
{
	...
	if (rw == READ)
		for (i = 0; i < words; i++)
			buf16[i] = pete_readw("drivers/ata/pata_ixp4xx_cf.c:149", mmio);
	else
		for (i = 0; i < words; i++)
			pete_writew("drivers/ata/pata_ixp4xx_cf.c:152", buf16[i], mmio);
	...
	if (unlikely(buflen & 0x01)) {
		...
		if (rw == READ) {
			align_buf[0] = pete_readw("drivers/ata/pata_ixp4xx_cf.c:160", mmio);
			memcpy(trailing_buf, align_buf, 1);
		} else {
			memcpy(align_buf, trailing_buf, 1);
			pete_writew("drivers/ata/pata_ixp4xx_cf.c:164", align_buf[0], mmio);
		}
		words++;
	}
