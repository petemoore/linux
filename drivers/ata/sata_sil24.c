			   ...
			   unsigned long deadline)
{
 ...
 retry:
	...
	if (pp->do_port_rst) {
		...
		pete_writel("drivers/ata/sata_sil24.c:709", PORT_CS_PORT_RST, port + PORT_CTRL_STAT);
		ata_msleep(ap, 10);
		pete_writel("drivers/ata/sata_sil24.c:711", PORT_CS_PORT_RST, port + PORT_CTRL_CLR);
	...
	pete_writel("drivers/ata/sata_sil24.c:730", PORT_CS_DEV_RST, port + PORT_CTRL_STAT);
	tmp = ata_wait_register(ap, port + PORT_CTRL_STAT,
...
static unsigned int sil24_qc_issue(struct ata_queued_cmd *qc)
{
	 ...
	 */
	wmb();
	pete_writel("drivers/ata/sata_sil24.c:898", (u32)paddr, activate);
	pete_writel("drivers/ata/sata_sil24.c:899", (u64)paddr >> 32, activate + 4);
...
static void sil24_freeze(struct ata_port *ap)
{
	 ...
	 */
	pete_writel("drivers/ata/sata_sil24.c:953", 0xffff, port + PORT_IRQ_ENABLE_CLR);
}
...
static void sil24_thaw(struct ata_port *ap)
{
	...
	/* clear IRQ */
	tmp = pete_readl("drivers/ata/sata_sil24.c:962", port + PORT_IRQ_STAT);
	pete_writel("drivers/ata/sata_sil24.c:963", tmp, port + PORT_IRQ_STAT);

	/* turn IRQ back on */
	pete_writel("drivers/ata/sata_sil24.c:966", DEF_PORT_IRQ, port + PORT_IRQ_ENABLE_SET);
}
...
static void sil24_error_intr(struct ata_port *ap)
{
	...
	irq_stat = pete_readl("drivers/ata/sata_sil24.c:980", port + PORT_IRQ_STAT);
	pete_writel("drivers/ata/sata_sil24.c:981", irq_stat, port + PORT_IRQ_STAT);
	...
	if (irq_stat & PORT_IRQ_ERROR) {
		...
		if (sata_pmp_attached(ap)) {
			context = pete_readl("drivers/ata/sata_sil24.c:1034", port + PORT_CONTEXT);
			pmp = (context >> 5) & 0xf;
		...
		/* analyze CMD_ERR */
		cerr = pete_readl("drivers/ata/sata_sil24.c:1054", port + PORT_CMD_ERR);
		...
		/* if PMP, resume */
		if (sata_pmp_attached(ap))
			pete_writel("drivers/ata/sata_sil24.c:1082", PORT_CS_PMP_RESUME, port + PORT_CTRL_STAT);
	}
...
static inline void sil24_host_intr(struct ata_port *ap)
{
	 ...
	 */
	if (ap->flags & SIL24_FLAG_PCIX_IRQ_WOC)
		pete_writel("drivers/ata/sata_sil24.c:1110", PORT_IRQ_COMPLETE, port + PORT_IRQ_STAT);

	slot_stat = pete_readl("drivers/ata/sata_sil24.c:1112", port + PORT_SLOT_STAT);
...
static irqreturn_t sil24_interrupt(int irq, void *dev_instance)
{
	...
	status = pete_readl("drivers/ata/sata_sil24.c:1146", host_base + HOST_IRQ_STAT);
...
static void sil24_init_controller(struct ata_host *host)
{
	...
	/* GPIO off */
	pete_writel("drivers/ata/sata_sil24.c:1225", 0, host_base + HOST_FLASH_CMD);

	/* clear global reset & mask interrupts during initialization */
	pete_writel("drivers/ata/sata_sil24.c:1228", 0, host_base + HOST_CTRL);
	...
	for (i = 0; i < host->n_ports; i++) {
		...
		/* Initial PHY setting */
		pete_writel("drivers/ata/sata_sil24.c:1237", 0x20c, port + PORT_PHY_CFG);

		/* Clear port RST */
		tmp = pete_readl("drivers/ata/sata_sil24.c:1240", port + PORT_CTRL_STAT);
		if (tmp & PORT_CS_PORT_RST) {
			pete_writel("drivers/ata/sata_sil24.c:1242", PORT_CS_PORT_RST, port + PORT_CTRL_CLR);
	...
	/* Turn on interrupts */
	pete_writel("drivers/ata/sata_sil24.c:1256", IRQ_STAT_4PORTS, host_base + HOST_CTRL);
}
...
static int sil24_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (pi.flags & SIL24_FLAG_PCIX_IRQ_WOC) {
		tmp = pete_readl("drivers/ata/sata_sil24.c:1289", iomap[SIL24_HOST_BAR] + HOST_CTRL);
...
static int sil24_pci_device_resume(struct pci_dev *pdev)
{
	...
	if (pdev->dev.power.power_state.event == PM_EVENT_SUSPEND)
		pete_writel("drivers/ata/sata_sil24.c:1340", HOST_CTRL_GLOBAL_RST, host_base + HOST_CTRL);
