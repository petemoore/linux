...
static void jmb38x_ms_remove(struct pci_dev *dev)
{
	...
	for (cnt = 0; cnt < jm->host_cnt; ++cnt) {
		...
		pete_writel("drivers/memstick/host/jmb38x_ms.c:984", 0, host->addr + INT_SIGNAL_ENABLE);
		pete_writel("drivers/memstick/host/jmb38x_ms.c:985", 0, host->addr + INT_STATUS_ENABLE);
