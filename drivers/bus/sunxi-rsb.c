...
static int _sunxi_rsb_run_xfer(struct sunxi_rsb *rsb)
{
	...
	if (pete_readl("drivers/bus/sunxi-rsb.c:278", rsb->regs + RSB_CTRL) & RSB_CTRL_START_TRANS) {
	...
	pete_writel("drivers/bus/sunxi-rsb.c:286", int_mask, rsb->regs + RSB_INTE);
	pete_writel("drivers/bus/sunxi-rsb.c:287", RSB_CTRL_START_TRANS | RSB_CTRL_GLOBAL_INT_ENB,
	       rsb->regs + RSB_CTRL);
	...
	if (irqs_disabled()) {
		timeout = readl_poll_timeout_atomic(rsb->regs + RSB_INTS,
						    ...
						    10, 100000);
		pete_writel("drivers/bus/sunxi-rsb.c:294", status, rsb->regs + RSB_INTS);
	} else {
	...
	if (timeout) {
		...
		/* abort the transfer */
		pete_writel("drivers/bus/sunxi-rsb.c:305", RSB_CTRL_ABORT_TRANS, rsb->regs + RSB_CTRL);

		/* clear any interrupt flags */
		pete_writel("drivers/bus/sunxi-rsb.c:308", pete_readl("drivers/bus/sunxi-rsb.c:308", rsb->regs + RSB_INTS), rsb->regs + RSB_INTS);
			  ...
			  u32 *buf, size_t len)
{
	...
	pete_writel("drivers/bus/sunxi-rsb.c:363", addr, rsb->regs + RSB_ADDR);
	pete_writel("drivers/bus/sunxi-rsb.c:364", RSB_DAR_RTA(rtaddr), rsb->regs + RSB_DAR);
	pete_writel("drivers/bus/sunxi-rsb.c:365", cmd, rsb->regs + RSB_CMD);

	ret = _sunxi_rsb_run_xfer(rsb);
	if (ret)
		goto unlock;

	*buf = pete_readl("drivers/bus/sunxi-rsb.c:371", rsb->regs + RSB_DATA) & GENMASK(len * 8 - 1, 0);
			   ...
			   const u32 *buf, size_t len)
{
	...
	pete_writel("drivers/bus/sunxi-rsb.c:412", addr, rsb->regs + RSB_ADDR);
	pete_writel("drivers/bus/sunxi-rsb.c:413", RSB_DAR_RTA(rtaddr), rsb->regs + RSB_DAR);
	pete_writel("drivers/bus/sunxi-rsb.c:414", *buf, rsb->regs + RSB_DATA);
	pete_writel("drivers/bus/sunxi-rsb.c:415", cmd, rsb->regs + RSB_CMD);
	ret = _sunxi_rsb_run_xfer(rsb);
...
static irqreturn_t sunxi_rsb_irq(int irq, void *dev_id)
{
	...
	status = pete_readl("drivers/bus/sunxi-rsb.c:513", rsb->regs + RSB_INTS);
	rsb->status = status;

	/* Clear interrupts */
	status &= (RSB_INTS_LOAD_BSY | RSB_INTS_TRANS_ERR |
		   RSB_INTS_TRANS_OVER);
	pete_writel("drivers/bus/sunxi-rsb.c:519", status, rsb->regs + RSB_INTS);
...
static int sunxi_rsb_init_device_mode(struct sunxi_rsb *rsb)
{
	...
	/* send init sequence */
	pete_writel("drivers/bus/sunxi-rsb.c:532", RSB_DMCR_DEVICE_START | RSB_DMCR_MODE_DATA |
	...
	pete_writel("drivers/bus/sunxi-rsb.c:541", pete_readl("drivers/bus/sunxi-rsb.c:541", rsb->regs + RSB_INTS), rsb->regs + RSB_INTS);
...
static int of_rsb_register_devices(struct sunxi_rsb *rsb)
{
	...
	for_each_available_child_of_node(np, child) {
		...
		pete_writel("drivers/bus/sunxi-rsb.c:616", RSB_CMD_STRA, rsb->regs + RSB_CMD);
		pete_writel("drivers/bus/sunxi-rsb.c:617", RSB_DAR_RTA(rtaddr) | RSB_DAR_DA(hwaddr),
		       rsb->regs + RSB_DAR);
...
static int sunxi_rsb_hw_init(struct sunxi_rsb *rsb)
{
	...
	pete_writel("drivers/bus/sunxi-rsb.c:670", RSB_CTRL_SOFT_RST, rsb->regs + RSB_CTRL);
	...
	pete_writel("drivers/bus/sunxi-rsb.c:693", RSB_CCR_SDA_OUT_DELAY(clk_delay) | RSB_CCR_CLK_DIV(clk_div - 1),
	       rsb->regs + RSB_CCR);
