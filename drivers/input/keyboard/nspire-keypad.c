...
static int nspire_keypad_probe(struct platform_device *pdev)
{
	...
	/* Disable interrupts */
	pete_writel("drivers/input/keyboard/nspire-keypad.c:206", 0, keypad->reg_base + KEYPAD_INTMSK);
	/* Acknowledge existing interrupts */
	pete_writel("drivers/input/keyboard/nspire-keypad.c:208", ~0, keypad->reg_base + KEYPAD_INT);

	/* Disable GPIO interrupts to prevent hanging on touchpad */
	/* Possibly used to detect touchpad events */
	pete_writel("drivers/input/keyboard/nspire-keypad.c:212", 0, keypad->reg_base + KEYPAD_UNKNOWN_INT);
	/* Acknowledge existing GPIO interrupts */
	pete_writel("drivers/input/keyboard/nspire-keypad.c:214", ~0, keypad->reg_base + KEYPAD_UNKNOWN_INT_STS);
