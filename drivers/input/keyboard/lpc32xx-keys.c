...
static int lpc32xx_kscan_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/input/keyboard/lpc32xx-keys.c:233", kscandat->deb_clks, LPC32XX_KS_DEB(kscandat->kscan_base));
	pete_writel("drivers/input/keyboard/lpc32xx-keys.c:234", kscandat->scan_delay, LPC32XX_KS_SCAN_CTL(kscandat->kscan_base));
	pete_writel("drivers/input/keyboard/lpc32xx-keys.c:235", LPC32XX_KSCAN_FTST_USE32K_CLK,
	       LPC32XX_KS_FAST_TST(kscandat->kscan_base));
	pete_writel("drivers/input/keyboard/lpc32xx-keys.c:237", kscandat->matrix_sz,
	       LPC32XX_KS_MATRIX_DIM(kscandat->kscan_base));
	pete_writel("drivers/input/keyboard/lpc32xx-keys.c:239", 1, LPC32XX_KS_IRQ(kscandat->kscan_base));
	clk_disable_unprepare(kscandat->clk);
...
static int lpc32xx_kscan_suspend(struct device *dev)
{
	...
	if (input_device_enabled(input)) {
		...
		pete_writel("drivers/input/keyboard/lpc32xx-keys.c:270", 1, LPC32XX_KS_IRQ(kscandat->kscan_base));
		clk_disable_unprepare(kscandat->clk);
	}
...
static int lpc32xx_kscan_resume(struct device *dev)
{
	...
	if (input_device_enabled(input)) {
		...
		if (retval == 0)
			pete_writel("drivers/input/keyboard/lpc32xx-keys.c:291", 1, LPC32XX_KS_IRQ(kscandat->kscan_base));
	}
