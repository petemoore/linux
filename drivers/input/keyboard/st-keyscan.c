...
static irqreturn_t keyscan_isr(int irq, void *dev_id)
{
	...
	state = pete_readl("drivers/input/keyboard/st-keyscan.c:48", keypad->base + KEYSCAN_MATRIX_STATE_OFF) & 0xffff;
...
static int keyscan_start(struct st_keyscan *keypad)
{
	...
	pete_writel("drivers/input/keyboard/st-keyscan.c:69", keypad->debounce_us * (clk_get_rate(keypad->clk) / 1000000),
	       keypad->base + KEYSCAN_DEBOUNCE_TIME_OFF);

	pete_writel("drivers/input/keyboard/st-keyscan.c:72", ((keypad->n_cols - 1) << KEYSCAN_MATRIX_DIM_X_SHIFT) |
	       ((keypad->n_rows - 1) << KEYSCAN_MATRIX_DIM_Y_SHIFT),
	       keypad->base + KEYSCAN_MATRIX_DIM_OFF);

	pete_writel("drivers/input/keyboard/st-keyscan.c:76", KEYSCAN_CONFIG_ENABLE, keypad->base + KEYSCAN_CONFIG_OFF);
...
{
	pete_writel("drivers/input/keyboard/st-keyscan.c:83", 0, keypad->base + KEYSCAN_CONFIG_OFF);
