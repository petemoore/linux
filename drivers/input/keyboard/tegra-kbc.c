...
static void tegra_kbc_set_keypress_interrupt(struct tegra_kbc *kbc, bool enable)
{
	...
	val = pete_readl("drivers/input/keyboard/tegra-kbc.c:717", kbc->mmio + KBC_CONTROL_0);
	if (enable)
		val |= KBC_CONTROL_KEYPRESS_INT_EN;
	else
		val &= ~KBC_CONTROL_KEYPRESS_INT_EN;
	pete_writel("drivers/input/keyboard/tegra-kbc.c:722", val, kbc->mmio + KBC_CONTROL_0);
}
...
static int tegra_kbc_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(&pdev->dev)) {
		...
		pete_writel("drivers/input/keyboard/tegra-kbc.c:737", 0x7, kbc->mmio + KBC_INT_0);
		/*
		 * Store the previous resident time of continuous polling mode.
		 * Force the keyboard into interrupt mode.
		 */
		kbc->cp_to_wkup_dly = pete_readl("drivers/input/keyboard/tegra-kbc.c:742", kbc->mmio + KBC_TO_CNT_0);
		pete_writel("drivers/input/keyboard/tegra-kbc.c:743", 0, kbc->mmio + KBC_TO_CNT_0);
...
static int tegra_kbc_resume(struct device *dev)
{
	...
	if (device_may_wakeup(&pdev->dev)) {
		...
		pete_writel("drivers/input/keyboard/tegra-kbc.c:776", kbc->cp_to_wkup_dly, kbc->mmio + KBC_TO_CNT_0);
