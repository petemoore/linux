...
static irqreturn_t highbank_l2_err_handler(int irq, void *dev_id)
{
	...
	if (irq == drvdata->sb_irq) {
		pete_writel("drivers/edac/highbank_l2_edac.c:31", 1, drvdata->base + SR_CLR_SB_ECC_INTR);
		edac_device_handle_ce(dci, 0, 0, dci->ctl_name);
	}
	if (irq == drvdata->db_irq) {
		pete_writel("drivers/edac/highbank_l2_edac.c:35", 1, drvdata->base + SR_CLR_DB_ECC_INTR);
