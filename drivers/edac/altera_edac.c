...
altr_check_ocram_deps_init(struct altr_edac_device_dev *device)
{
	...
	pete_writel("drivers/edac/altera_edac.c:1207", ALTR_A10_ECC_SERRINTEN, (base + ALTR_A10_ECC_ERRINTENS_OFST));
	/* Ensure all writes complete */
	wmb();
...
static int altr_l2_check_deps(struct altr_edac_device_dev *device)
{
	...
	if ((pete_readl("drivers/edac/altera_edac.c:1282", base) & prv->ecc_enable_mask) ==
	     prv->ecc_enable_mask)
		return 0;
...
static irqreturn_t altr_edac_a10_ecc_irq_portb(int irq, void *dev_id)
{
	...
	if (irq == ad->sb_irq) {
		pete_writel("drivers/edac/altera_edac.c:1660", priv->ce_clear_mask,
		       base + ALTR_A10_ECC_INTSTAT_OFST);
		edac_device_handle_ce(ad->edac_dev, 0, 0, ad->edac_dev_name);
		return IRQ_HANDLED;
	} else if (irq == ad->db_irq) {
		pete_writel("drivers/edac/altera_edac.c:1665", priv->ue_clear_mask,
		       base + ALTR_A10_ECC_INTSTAT_OFST);
			  ...
			  size_t count, loff_t *ppos)
{
	...
	local_irq_save(flags);
	if (trig_type == ALTR_UE_TRIGGER_CHAR)
		pete_writel("drivers/edac/altera_edac.c:1762", priv->ue_set_mask, set_addr);
	else
		pete_writel("drivers/edac/altera_edac.c:1764", priv->ce_set_mask, set_addr);
			   ...
			   size_t count, loff_t *ppos)
{
	...
	local_irq_save(flags);
	if (trig_type == ALTR_UE_TRIGGER_CHAR) {
		pete_writel("drivers/edac/altera_edac.c:1794", priv->ue_set_mask, set_addr);
	} else {
		/* Setup read/write of 4 bytes */
		pete_writel("drivers/edac/altera_edac.c:1797", ECC_WORD_WRITE, drvdata->base + ECC_BLK_DBYTECTRL_OFST);
		/* Setup Address to 0 */
		pete_writel("drivers/edac/altera_edac.c:1799", 0, drvdata->base + ECC_BLK_ADDRESS_OFST);
		/* Setup accctrl to read & ecc & data override */
		pete_writel("drivers/edac/altera_edac.c:1801", ECC_READ_EDOVR, drvdata->base + ECC_BLK_ACCCTRL_OFST);
		/* Kick it. */
		pete_writel("drivers/edac/altera_edac.c:1803", ECC_XACT_KICK, drvdata->base + ECC_BLK_STARTACC_OFST);
		/* Setup write for single bit change */
		pete_writel("drivers/edac/altera_edac.c:1805", pete_readl("drivers/edac/altera_edac.c:1805", drvdata->base + ECC_BLK_RDATA0_OFST) ^ 0x1,
		       drvdata->base + ECC_BLK_WDATA0_OFST);
		pete_writel("drivers/edac/altera_edac.c:1807", pete_readl("drivers/edac/altera_edac.c:1807", drvdata->base + ECC_BLK_RDATA1_OFST),
		       drvdata->base + ECC_BLK_WDATA1_OFST);
		pete_writel("drivers/edac/altera_edac.c:1809", pete_readl("drivers/edac/altera_edac.c:1809", drvdata->base + ECC_BLK_RDATA2_OFST),
		       drvdata->base + ECC_BLK_WDATA2_OFST);
		pete_writel("drivers/edac/altera_edac.c:1811", pete_readl("drivers/edac/altera_edac.c:1811", drvdata->base + ECC_BLK_RDATA3_OFST),
		       drvdata->base + ECC_BLK_WDATA3_OFST);

		/* Copy Read ECC to Write ECC */
		pete_writel("drivers/edac/altera_edac.c:1815", pete_readl("drivers/edac/altera_edac.c:1815", drvdata->base + ECC_BLK_RECC0_OFST),
		       drvdata->base + ECC_BLK_WECC0_OFST);
		pete_writel("drivers/edac/altera_edac.c:1817", pete_readl("drivers/edac/altera_edac.c:1817", drvdata->base + ECC_BLK_RECC1_OFST),
		       drvdata->base + ECC_BLK_WECC1_OFST);
		/* Setup accctrl to write & ecc override & data override */
		pete_writel("drivers/edac/altera_edac.c:1820", ECC_WRITE_EDOVR, drvdata->base + ECC_BLK_ACCCTRL_OFST);
		/* Kick it. */
		pete_writel("drivers/edac/altera_edac.c:1822", ECC_XACT_KICK, drvdata->base + ECC_BLK_STARTACC_OFST);
		/* Setup accctrl to read & ecc overwrite & data overwrite */
		pete_writel("drivers/edac/altera_edac.c:1824", ECC_READ_EDOVR, drvdata->base + ECC_BLK_ACCCTRL_OFST);
		/* Kick it. */
		pete_writel("drivers/edac/altera_edac.c:1826", ECC_XACT_KICK, drvdata->base + ECC_BLK_STARTACC_OFST);
	}
				  ...
				  unsigned long event, void *ptr)
{
	...
	if (dberror & S10_DBE_IRQ_MASK) {
		...
		list_for_each(position, &edac->a10_ecc_devices) {
			...
			pete_writel("drivers/edac/altera_edac.c:2097", ALTR_A10_ECC_DERRPENA,
			       ed->base + ALTR_A10_ECC_INTSTAT_OFST);
			err_addr = pete_readl("drivers/edac/altera_edac.c:2099", ed->base + ALTR_S10_DERR_ADDRA_OFST);
