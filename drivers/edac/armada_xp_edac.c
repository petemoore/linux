...
static int axp_mc_probe(struct platform_device *pdev)
{
	...
	config = pete_readl("drivers/edac/armada_xp_edac.c:298", base + SDRAM_CONFIG_REG);
	...
	pete_writel("drivers/edac/armada_xp_edac.c:335", 1 << SDRAM_ERR_CTRL_THR_OFFSET, drvdata->base + SDRAM_ERR_CTRL_REG);

	/* clear cause registers */
	pete_writel("drivers/edac/armada_xp_edac.c:338", ~(SDRAM_ERR_CAUSE_DBE_MASK | SDRAM_ERR_CAUSE_SBE_MASK), drvdata->base + SDRAM_ERR_CAUSE_ERR_REG);
	pete_writel("drivers/edac/armada_xp_edac.c:339", ~(SDRAM_ERR_CAUSE_DBE_MASK | SDRAM_ERR_CAUSE_SBE_MASK), drvdata->base + SDRAM_ERR_CAUSE_MSG_REG);

	/* clear counter registers */
	pete_writel("drivers/edac/armada_xp_edac.c:342", 0, drvdata->base + SDRAM_ERR_SBE_COUNT_REG);
	pete_writel("drivers/edac/armada_xp_edac.c:343", 0, drvdata->base + SDRAM_ERR_DBE_COUNT_REG);
...
static void aurora_l2_inject(struct aurora_l2_drvdata *drvdata)
{
	...
	pete_writel("drivers/edac/armada_xp_edac.c:394", 0, drvdata->base + AURORA_ERR_INJECT_CTL_REG);
	pete_writel("drivers/edac/armada_xp_edac.c:395", drvdata->inject_mask, drvdata->base + AURORA_ERR_INJECT_MASK_REG);
	pete_writel("drivers/edac/armada_xp_edac.c:396", drvdata->inject_addr | drvdata->inject_ctl, drvdata->base + AURORA_ERR_INJECT_CTL_REG);
}
#endif
...
static void aurora_l2_check(struct edac_device_ctl_info *dci)
{
	...
	cnt = pete_readl("drivers/edac/armada_xp_edac.c:409", drvdata->base + AURORA_ERR_CNT_REG);
	attr_cap = pete_readl("drivers/edac/armada_xp_edac.c:410", drvdata->base + AURORA_ERR_ATTR_CAP_REG);
	addr_cap = pete_readl("drivers/edac/armada_xp_edac.c:411", drvdata->base + AURORA_ERR_ADDR_CAP_REG);
	way_cap = pete_readl("drivers/edac/armada_xp_edac.c:412", drvdata->base + AURORA_ERR_WAY_CAP_REG);

	cnt_ce = (cnt & AURORA_ERR_CNT_CE_MASK) >> AURORA_ERR_CNT_CE_OFFSET;
	cnt_ue = (cnt & AURORA_ERR_CNT_UE_MASK) >> AURORA_ERR_CNT_UE_OFFSET;
	/* clear error counter registers */
	if (cnt_ce || cnt_ue)
		pete_writel("drivers/edac/armada_xp_edac.c:418", AURORA_ERR_CNT_CLR, drvdata->base + AURORA_ERR_CNT_REG);
	...
	pete_writel("drivers/edac/armada_xp_edac.c:467", AURORA_ERR_ATTR_CAP_VALID, drvdata->base + AURORA_ERR_ATTR_CAP_REG);
	if (err) {
...
static int aurora_l2_probe(struct platform_device *pdev)
{
	...
	l2x0_aux_ctrl = pete_readl("drivers/edac/armada_xp_edac.c:519", base + L2X0_AUX_CTRL);
	...
	/* clear registers */
	pete_writel("drivers/edac/armada_xp_edac.c:542", AURORA_ERR_CNT_CLR, drvdata->base + AURORA_ERR_CNT_REG);
	pete_writel("drivers/edac/armada_xp_edac.c:543", AURORA_ERR_ATTR_CAP_VALID, drvdata->base + AURORA_ERR_ATTR_CAP_REG);
