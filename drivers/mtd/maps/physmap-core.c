					...
					unsigned long ofs)
{
	...
	word = pete_readw("drivers/mtd/maps/physmap-core.c:154", map->virt + (ofs & win_mask(info->win_order)));
	mw.x[0] = word;
	return mw;
}
				     ...
				     unsigned long ofs)
{
	...
	word = mw.x[0];
	pete_writew("drivers/mtd/maps/physmap-core.c:193", word, map->virt + (ofs & win_mask(info->win_order)));
}
