	...
	unsigned int u32_count, int is_write, struct omap_nand_info *info)
{
	...
	if (pete_readl("drivers/mtd/nand/raw/omap2.c:214", info->reg.gpmc_prefetch_control))
		return -EBUSY;

	/* Set the amount of bytes to be prefetched */
	pete_writel("drivers/mtd/nand/raw/omap2.c:218", u32_count, info->reg.gpmc_prefetch_config2);
	...
	pete_writel("drivers/mtd/nand/raw/omap2.c:226", val, info->reg.gpmc_prefetch_config1);

	/*  Start the prefetch engine */
	pete_writel("drivers/mtd/nand/raw/omap2.c:229", 0x1, info->reg.gpmc_prefetch_control);
...
static int omap_prefetch_reset(int cs, struct omap_nand_info *info)
{
	...
	config1 = pete_readl("drivers/mtd/nand/raw/omap2.c:242", info->reg.gpmc_prefetch_config1);
	if (((config1 >> PREFETCH_CONFIG1_CS_SHIFT) & CS_MASK) != cs)
		return -EINVAL;

	/* Stop the PFPW engine */
	pete_writel("drivers/mtd/nand/raw/omap2.c:247", 0x0, info->reg.gpmc_prefetch_control);

	/* Reset/disable the PFPW engine */
	pete_writel("drivers/mtd/nand/raw/omap2.c:250", 0x0, info->reg.gpmc_prefetch_config1);
				   ...
				   unsigned int len, bool force_8bit)
{
	...
	if (ret) {
		...
		do {
			r_count = pete_readl("drivers/mtd/nand/raw/omap2.c:283", info->reg.gpmc_prefetch_status);
				    ...
				    bool force_8bit)
{
	...
	if (len % 2 != 0) {
		pete_writeb("drivers/mtd/nand/raw/omap2.c:319", *(u8 *)buf, info->fifo);
		p = (u16 *)(buf + 1);
		len--;
	}
	...
	if (ret) {
		...
		while (len) {
			w_count = pete_readl("drivers/mtd/nand/raw/omap2.c:332", info->reg.gpmc_prefetch_status);
		...
		do {
			cpu_relax();
			val = pete_readl("drivers/mtd/nand/raw/omap2.c:344", info->reg.gpmc_prefetch_status);
					 ...
					 int is_write)
{
	...
	do {
		cpu_relax();
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:422", info->reg.gpmc_prefetch_status);
		val = PREFETCH_STATUS_COUNT(val);
	} while (val && (tim++ < limit));
...
static irqreturn_t omap_nand_irq(int this_irq, void *dev)
{
	...
	bytes = pete_readl("drivers/mtd/nand/raw/omap2.c:492", info->reg.gpmc_prefetch_status);
					...
					bool force_8bit)
{
	...
	limit = (loops_per_jiffy *  msecs_to_jiffies(OMAP_NAND_TIMEOUT_MS));
	do {
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:612", info->reg.gpmc_prefetch_status);
		val = PREFETCH_STATUS_COUNT(val);
		cpu_relax();
	} while (val && (tim++ < limit));
			      ...
			      u_char *ecc_code)
{
	...
	val = pete_readl("drivers/mtd/nand/raw/omap2.c:831", info->reg.gpmc_ecc_config);
	if (((val >> ECC_CONFIG_CS_SHIFT) & CS_MASK) != info->gpmc_cs)
		return -EINVAL;

	/* read ecc result */
	val = pete_readl("drivers/mtd/nand/raw/omap2.c:836", info->reg.gpmc_ecc1_result);
...
static void omap_enable_hwecc(struct nand_chip *chip, int mode)
{
	...
	val = ECCCLEAR | ECC1;
	pete_writel("drivers/mtd/nand/raw/omap2.c:858", val, info->reg.gpmc_ecc_control);

	/* program ecc and result sizes */
	val = ((((info->nand.ecc.size >> 1) - 1) << ECCSIZE1_SHIFT) |
			 ECC1RESULTSIZE);
	pete_writel("drivers/mtd/nand/raw/omap2.c:863", val, info->reg.gpmc_ecc_size_config);

	switch (mode) {
	case NAND_ECC_READ:
	case NAND_ECC_WRITE:
		pete_writel("drivers/mtd/nand/raw/omap2.c:868", ECCCLEAR | ECC1, info->reg.gpmc_ecc_control);
		break;
	case NAND_ECC_READSYN:
		pete_writel("drivers/mtd/nand/raw/omap2.c:871", ECCCLEAR, info->reg.gpmc_ecc_control);
		break;
	default:
	...
	pete_writel("drivers/mtd/nand/raw/omap2.c:881", val, info->reg.gpmc_ecc_config);
}
						 ...
						 int mode)
{
	...
	pete_writel("drivers/mtd/nand/raw/omap2.c:964", ECC1, info->reg.gpmc_ecc_control);

	/* Configure ecc size for BCH */
	val = (ecc_size1 << ECCSIZE1_SHIFT) | (ecc_size0 << ECCSIZE0_SHIFT);
	pete_writel("drivers/mtd/nand/raw/omap2.c:968", val, info->reg.gpmc_ecc_size_config);
	...
	/* Clear ecc and enable bits */
	pete_writel("drivers/mtd/nand/raw/omap2.c:984", ECCCLEAR | ECC1, info->reg.gpmc_ecc_control);
}
				   ...
				   const u_char *dat, u_char *ecc_calc, int i)
{
	...
	case OMAP_ECC_BCH8_CODE_HW_DETECTION_SW:
	case OMAP_ECC_BCH8_CODE_HW:
		bch_val1 = pete_readl("drivers/mtd/nand/raw/omap2.c:1016", gpmc_regs->gpmc_bch_result0[i]);
		bch_val2 = pete_readl("drivers/mtd/nand/raw/omap2.c:1017", gpmc_regs->gpmc_bch_result1[i]);
		bch_val3 = pete_readl("drivers/mtd/nand/raw/omap2.c:1018", gpmc_regs->gpmc_bch_result2[i]);
		bch_val4 = pete_readl("drivers/mtd/nand/raw/omap2.c:1019", gpmc_regs->gpmc_bch_result3[i]);
	...
	case OMAP_ECC_BCH4_CODE_HW_DETECTION_SW:
	case OMAP_ECC_BCH4_CODE_HW:
		bch_val1 = pete_readl("drivers/mtd/nand/raw/omap2.c:1036", gpmc_regs->gpmc_bch_result0[i]);
		bch_val2 = pete_readl("drivers/mtd/nand/raw/omap2.c:1037", gpmc_regs->gpmc_bch_result1[i]);
		...
		*ecc_code++ = ((bch_val1 & 0xF) << 4);
		break;
	case OMAP_ECC_BCH16_CODE_HW:
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1048", gpmc_regs->gpmc_bch_result6[i]);
		ecc_code[0]  = ((val >>  8) & 0xFF);
		ecc_code[1]  = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1051", gpmc_regs->gpmc_bch_result5[i]);
		ecc_code[2]  = ((val >> 24) & 0xFF);
		ecc_code[3]  = ((val >> 16) & 0xFF);
		ecc_code[4]  = ((val >>  8) & 0xFF);
		ecc_code[5]  = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1056", gpmc_regs->gpmc_bch_result4[i]);
		ecc_code[6]  = ((val >> 24) & 0xFF);
		ecc_code[7]  = ((val >> 16) & 0xFF);
		ecc_code[8]  = ((val >>  8) & 0xFF);
		ecc_code[9]  = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1061", gpmc_regs->gpmc_bch_result3[i]);
		ecc_code[10] = ((val >> 24) & 0xFF);
		ecc_code[11] = ((val >> 16) & 0xFF);
		ecc_code[12] = ((val >>  8) & 0xFF);
		ecc_code[13] = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1066", gpmc_regs->gpmc_bch_result2[i]);
		ecc_code[14] = ((val >> 24) & 0xFF);
		ecc_code[15] = ((val >> 16) & 0xFF);
		ecc_code[16] = ((val >>  8) & 0xFF);
		ecc_code[17] = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1071", gpmc_regs->gpmc_bch_result1[i]);
		ecc_code[18] = ((val >> 24) & 0xFF);
		ecc_code[19] = ((val >> 16) & 0xFF);
		ecc_code[20] = ((val >>  8) & 0xFF);
		ecc_code[21] = ((val >>  0) & 0xFF);
		val = pete_readl("drivers/mtd/nand/raw/omap2.c:1076", gpmc_regs->gpmc_bch_result0[i]);
					...
					const u_char *dat, u_char *ecc_calc)
{
	...
	nsectors = ((pete_readl("drivers/mtd/nand/raw/omap2.c:1151", info->reg.gpmc_ecc_config) >> 4) & 0x7) + 1;
