...
static int rk_nfc_hw_ecc_setup(struct nand_chip *chip, u32 strength)
{
	...
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:253", reg, nfc->regs + nfc->cfg->bchctl_off);
...
static void rk_nfc_select_chip(struct nand_chip *chip, int cs)
{
	...
	if (cs < 0) {
		...
		val &= ~FMCTL_CE_SEL_M;
		pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:273", val, nfc->regs + NFC_FMCTL);
		return;
	}
	...
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:284", val, nfc->regs + NFC_FMCTL);
	...
	if (nfc->cur_timing != rknand->timing) {
		pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:291", rknand->timing, nfc->regs + NFC_FMWAIT);
...
static void rk_nfc_write_buf(struct rk_nfc *nfc, const u8 *buf, int len)
{
	...
	for (i = 0; i < len; i++)
		pete_writeb("drivers/mtd/nand/raw/rockchip-nand-controller.c:328", buf[i], nfc->regs + nfc->band_offset + BANK_DATA);
}
		      ...
		      const struct nand_subop *subop)
{
	...
	for (i = 0; i < subop->ninstrs; i++) {
		...
		case NAND_OP_CMD_INSTR:
			pete_writeb("drivers/mtd/nand/raw/rockchip-nand-controller.c:347", instr->ctx.cmd.opcode,
		...
		case NAND_OP_ADDR_INSTR:
			...
			for (j = 0; j < 8 && j + start < remaining; j++)
				pete_writeb("drivers/mtd/nand/raw/rockchip-nand-controller.c:356", instr->ctx.addr.addrs[j + start],
			      ...
			      dma_addr_t dma_data, dma_addr_t dma_oob)
{
	...
	if (nfc->cfg->type == NFC_V6 || nfc->cfg->type == NFC_V8) {
		...
		pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:483", bch_reg, nfc->regs + nfc->cfg->bchctl_off);
	}

	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:486", dma_reg, nfc->regs + nfc->cfg->dma_cfg_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:487", (u32)dma_data, nfc->regs + nfc->cfg->dma_data_buf_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:488", (u32)dma_oob, nfc->regs + nfc->cfg->dma_oob_buf_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:489", fl_reg, nfc->regs + nfc->cfg->flctl_off);
	fl_reg |= FLCTL_XFER_ST;
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:491", fl_reg, nfc->regs + nfc->cfg->flctl_off);
}
				   ...
				   int oob_on, int page)
{
	...
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:664", INT_DMA, nfc->regs + nfc->cfg->int_en_off);
				  ...
				  int page)
{
	...
	reinit_completion(&nfc->done);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:792", INT_DMA, nfc->regs + nfc->cfg->int_en_off);
...
{
	/* Disable flash wp. */
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:878", FMCTL_WP, nfc->regs + NFC_FMCTL);
	/* Config default timing 40ns at 150 Mhz NFC clock. */
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:880", 0x1081, nfc->regs + NFC_FMWAIT);
	nfc->cur_timing = 0x1081;
	/* Disable randomizer and DMA. */
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:883", 0, nfc->regs + nfc->cfg->randmz_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:884", 0, nfc->regs + nfc->cfg->dma_cfg_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:885", FLCTL_RST, nfc->regs + nfc->cfg->flctl_off);
}
...
static irqreturn_t rk_nfc_irq(int irq, void *id)
{
	...
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:899", sta, nfc->regs + nfc->cfg->int_clr_off);
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:900", ~sta & ien, nfc->regs + nfc->cfg->int_en_off);
...
static int rk_nfc_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/mtd/nand/raw/rockchip-nand-controller.c:1412", 0, nfc->regs + nfc->cfg->int_en_off);
