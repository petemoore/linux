				   ...
				   unsigned int ctrl)
{
	...
	if (ctrl & NAND_CTRL_CHANGE) {
		...
		pete_writeb("drivers/mtd/nand/raw/sharpsl.c:69", (pete_readb("drivers/mtd/nand/raw/sharpsl.c:69", sharpsl->io + FLASHCTL) & ~0x17) | bits, sharpsl->io + FLASHCTL);
	}

	if (cmd != NAND_CMD_NONE)
		pete_writeb("drivers/mtd/nand/raw/sharpsl.c:73", cmd, chip->legacy.IO_ADDR_W);
}

static int sharpsl_nand_dev_ready(struct nand_chip *chip)
{
	struct sharpsl_nand *sharpsl = mtd_to_sharpsl(nand_to_mtd(chip));
	return !((pete_readb("drivers/mtd/nand/raw/sharpsl.c:79", sharpsl->io + FLASHCTL) & FLRYBY) == 0);
}

static void sharpsl_nand_enable_hwecc(struct nand_chip *chip, int mode)
{
	struct sharpsl_nand *sharpsl = mtd_to_sharpsl(nand_to_mtd(chip));
	pete_writeb("drivers/mtd/nand/raw/sharpsl.c:85", 0, sharpsl->io + ECCCLRR);
}
...
{
	struct sharpsl_nand *sharpsl = mtd_to_sharpsl(nand_to_mtd(chip));
	ecc_code[0] = ~pete_readb("drivers/mtd/nand/raw/sharpsl.c:92", sharpsl->io + ECCLPUB);
	ecc_code[1] = ~pete_readb("drivers/mtd/nand/raw/sharpsl.c:93", sharpsl->io + ECCLPLB);
	ecc_code[2] = (~pete_readb("drivers/mtd/nand/raw/sharpsl.c:94", sharpsl->io + ECCCP) << 2) | 0x03;
	return pete_readb("drivers/mtd/nand/raw/sharpsl.c:95", sharpsl->io + ECCCNTR) != 0;
}
...
static int sharpsl_nand_probe(struct platform_device *pdev)
{
	 ...
	 * PXA initialize
	 */
	pete_writeb("drivers/mtd/nand/raw/sharpsl.c:171", pete_readb("drivers/mtd/nand/raw/sharpsl.c:171", sharpsl->io + FLASHCTL) | FLWP, sharpsl->io + FLASHCTL);
