...
static irqreturn_t lpc3xxx_nand_irq(int irq, void *data)
{
	...
	sr = pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:312", MLC_IRQ_SR(host->io_base));
...
static int lpc32xx_waitfunc_nand(struct nand_chip *chip)
{
	...
	if (pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:326", MLC_ISR(host->io_base)) & MLCISR_NAND_READY)
		goto exit;

	wait_for_completion(&host->comp_nand);

	while (!(pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:331", MLC_ISR(host->io_base)) & MLCISR_NAND_READY)) {
...
static int lpc32xx_waitfunc_controller(struct nand_chip *chip)
{
	...
	if (pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:346", MLC_ISR(host->io_base)) & MLCISR_CONTROLLER_READY)
		goto exit;

	wait_for_completion(&host->comp_controller);

	while (!(pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:351", MLC_ISR(host->io_base)) &
			     ...
			     int oob_required, int page)
{
	...
	for (i = 0; i < host->mlcsubpages; i++) {
		...
		pete_writeb("drivers/mtd/nand/raw/lpc32xx_mlc.c:460", 0x00, MLC_ECC_AUTO_DEC_REG(host->io_base));

		/* Wait for Controller Ready */
		lpc32xx_waitfunc_controller(chip);

		/* Check ECC Error status */
		mlc_isr = pete_readl("drivers/mtd/nand/raw/lpc32xx_mlc.c:466", MLC_ISR(host->io_base));
		...
		if (use_dma) {
			...
			for (j = 0; j < (512 >> 2); j++) {
				*((uint32_t *)(buf)) =
					pete_readl("drivers/mtd/nand/raw/lpc32xx_mlc.c:483", MLC_BUFF(host->io_base));
				buf += 4;
			}
		}
		for (j = 0; j < (16 >> 2); j++) {
			*((uint32_t *)(oobbuf)) =
				pete_readl("drivers/mtd/nand/raw/lpc32xx_mlc.c:489", MLC_BUFF(host->io_base));
			oobbuf += 4;
		}
	}
				       ...
				       int page)
{
	...
	for (i = 0; i < host->mlcsubpages; i++) {
		/* Start Encode */
		pete_writeb("drivers/mtd/nand/raw/lpc32xx_mlc.c:520", 0x00, MLC_ECC_ENC_REG(host->io_base));
		...
		if (use_dma) {
			...
			for (j = 0; j < (512 >> 2); j++) {
				pete_writel("drivers/mtd/nand/raw/lpc32xx_mlc.c:530", *((uint32_t *)(buf)),
				       MLC_BUFF(host->io_base));
				buf += 4;
			}
		}
		pete_writel("drivers/mtd/nand/raw/lpc32xx_mlc.c:535", *((uint32_t *)(oobbuf)), MLC_BUFF(host->io_base));
		oobbuf += 4;
		pete_writew("drivers/mtd/nand/raw/lpc32xx_mlc.c:537", *((uint16_t *)(oobbuf)), MLC_BUFF(host->io_base));
		oobbuf += 12;

		/* Auto Encode w/ Bit 8 = 0 (see LPC MLC Controller manual) */
		pete_writeb("drivers/mtd/nand/raw/lpc32xx_mlc.c:541", 0x00, MLC_ECC_AUTO_ENC_REG(host->io_base));
...
static int lpc32xx_nand_probe(struct platform_device *pdev)
{
	...
	pete_readb("drivers/mtd/nand/raw/lpc32xx_mlc.c:773", MLC_IRQ_SR(host->io_base));
