				   ...
				   const struct nand_op_instr *instr)
{
	...
	case NAND_OP_CMD_INSTR:
		pete_writeb("drivers/mtd/nand/raw/ingenic/ingenic_nand_drv.c:267", instr->ctx.cmd.opcode,
		       cs->base + nfc->soc_info->cmd_offset);
		return 0;
	case NAND_OP_ADDR_INSTR:
		for (i = 0; i < instr->ctx.addr.naddrs; i++)
			pete_writeb("drivers/mtd/nand/raw/ingenic/ingenic_nand_drv.c:272", instr->ctx.addr.addrs[i],
