...
static int marvell_nfc_wait_ndrun(struct nand_chip *chip)
{
	...
	if (ret) {
		...
		writel_relaxed(pete_readl("drivers/mtd/nand/raw/marvell_nand.c:584", nfc->regs + NDCR) & ~NDCR_ND_RUN,
			       nfc->regs + NDCR);
		return ret;
	}
...
static int marvell_nfc_prepare_cmd(struct nand_chip *chip)
{
	...
	ndcr = readl_relaxed(nfc->regs + NDCR);
	writel_relaxed(pete_readl("drivers/mtd/nand/raw/marvell_nand.c:621", nfc->regs + NDSR), nfc->regs + NDSR);
				 ...
				 struct marvell_nfc_op *nfc_op)
{
	...
	pete_writel("drivers/mtd/nand/raw/marvell_nand.c:653", nfc_op->ndcb[2], nfc->regs + NDCB0);
	...
	if (nfc_op->ndcb[0] & NDCB0_LEN_OVRD ||
		...
		if (!WARN_ON_ONCE(!nfc->caps->is_nfcv2))
			pete_writel("drivers/mtd/nand/raw/marvell_nand.c:662", nfc_op->ndcb[3], nfc->regs + NDCB0);
	}
}
					      ...
					      const struct nand_subop *subop)
{
	...
	if (!reading) {
		...
		writel_relaxed(pete_readl("drivers/mtd/nand/raw/marvell_nand.c:1887", nfc->regs + NDCR) & ~NDCR_ND_RUN,
			       nfc->regs + NDCR);
	}
					 ...
					 const struct nand_subop *subop)
{
	...
	if (subop->instrs[0].type == NAND_OP_DATA_OUT_INSTR) {
		...
		writel_relaxed(pete_readl("drivers/mtd/nand/raw/marvell_nand.c:1959", nfc->regs + NDCR) & ~NDCR_ND_RUN,
			       nfc->regs + NDCR);
	}
