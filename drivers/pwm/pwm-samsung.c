				      ...
				      struct pwm_device *pwm)
{
	...
	tcon = pete_readl("drivers/pwm/pwm-samsung.c:126", chip->base + REG_TCON);
	tcon |= TCON_MANUALUPDATE(tcon_chan);
	pete_writel("drivers/pwm/pwm-samsung.c:128", tcon, chip->base + REG_TCON);

	tcon &= ~TCON_MANUALUPDATE(tcon_chan);
	pete_writel("drivers/pwm/pwm-samsung.c:131", tcon, chip->base + REG_TCON);
}
				    ...
				    unsigned int channel, u8 divisor)
{
	...
	reg = pete_readl("drivers/pwm/pwm-samsung.c:146", pwm->base + REG_TCFG1);
	reg &= ~(TCFG1_MUX_MASK << shift);
	reg |= bits << shift;
	pete_writel("drivers/pwm/pwm-samsung.c:149", reg, pwm->base + REG_TCFG1);
...
static int pwm_samsung_is_tdiv(struct samsung_pwm_chip *chip, unsigned int chan)
{
	...
	reg = pete_readl("drivers/pwm/pwm-samsung.c:159", chip->base + REG_TCFG1);
					      ...
					      unsigned int chan)
{
	...
	reg = pete_readl("drivers/pwm/pwm-samsung.c:174", chip->base + REG_TCFG0);
	if (chan >= 2)
...
static int pwm_samsung_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	tcon = pete_readl("drivers/pwm/pwm-samsung.c:263", our_chip->base + REG_TCON);

	tcon &= ~TCON_START(tcon_chan);
	tcon |= TCON_MANUALUPDATE(tcon_chan);
	pete_writel("drivers/pwm/pwm-samsung.c:267", tcon, our_chip->base + REG_TCON);

	tcon &= ~TCON_MANUALUPDATE(tcon_chan);
	tcon |= TCON_START(tcon_chan) | TCON_AUTORELOAD(tcon_chan);
	pete_writel("drivers/pwm/pwm-samsung.c:271", tcon, our_chip->base + REG_TCON);
...
static void pwm_samsung_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	tcon = pete_readl("drivers/pwm/pwm-samsung.c:289", our_chip->base + REG_TCON);
	tcon &= ~TCON_AUTORELOAD(tcon_chan);
	pete_writel("drivers/pwm/pwm-samsung.c:291", tcon, our_chip->base + REG_TCON);

	/*
	 * In case the PWM is at 100% duty cycle, force a manual
	 * update to prevent the signal from staying high.
	 */
	if (pete_readl("drivers/pwm/pwm-samsung.c:297", our_chip->base + REG_TCMPB(pwm->hwpwm)) == (u32)-1U)
				...
				int duty_ns, int period_ns, bool force_period)
{
	...
	tcnt = pete_readl("drivers/pwm/pwm-samsung.c:324", our_chip->base + REG_TCNTB(pwm->hwpwm));
	oldtcmp = pete_readl("drivers/pwm/pwm-samsung.c:325", our_chip->base + REG_TCMPB(pwm->hwpwm));
	...
	pete_writel("drivers/pwm/pwm-samsung.c:370", tcnt, our_chip->base + REG_TCNTB(pwm->hwpwm));
	pete_writel("drivers/pwm/pwm-samsung.c:371", tcmp, our_chip->base + REG_TCMPB(pwm->hwpwm));
				   ...
				   unsigned int channel, bool invert)
{
	...
	tcon = pete_readl("drivers/pwm/pwm-samsung.c:405", chip->base + REG_TCON);
	...
	pete_writel("drivers/pwm/pwm-samsung.c:415", tcon, chip->base + REG_TCON);
