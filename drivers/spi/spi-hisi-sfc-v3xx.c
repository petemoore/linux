...
static irqreturn_t hisi_sfc_v3xx_isr(int irq, void *data)
{
	...
	reg = pete_readl("drivers/spi/spi-hisi-sfc-v3xx.c:382", host->regbase + HISI_SFC_V3XX_INT_STAT);
	if (!reg)
		return IRQ_NONE;
...
static int hisi_sfc_v3xx_probe(struct platform_device *pdev)
{
	 ...
	 * for the OS driver.
	 */
	glb_config = pete_readl("drivers/spi/spi-hisi-sfc-v3xx.c:489", host->regbase + HISI_SFC_V3XX_GLB_CFG);
	if (glb_config & HISI_SFC_V3XX_GLB_CFG_CS0_ADDR_MODE)
		host->address_mode = 4;
	else
		host->address_mode = 3;

	version = pete_readl("drivers/spi/spi-hisi-sfc-v3xx.c:495", host->regbase + HISI_SFC_V3XX_VERSION);
