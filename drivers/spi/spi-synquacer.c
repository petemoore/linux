...
static void synquacer_spi_set_cs(struct spi_device *spi, bool enable)
{
	...
	val = pete_readl("drivers/spi/spi-synquacer.c:488", sspi->regs + SYNQUACER_HSSPI_REG_DMSTART);
	...
	pete_writel("drivers/spi/spi-synquacer.c:496", val, sspi->regs + SYNQUACER_HSSPI_REG_DMSTART);
}
					    ...
					    bool enable)
{
	...
	do {
		val = pete_readl("drivers/spi/spi-synquacer.c:508", sspi->regs + SYNQUACER_HSSPI_REG_MCTRL) &
...
static int synquacer_spi_enable(struct spi_master *master)
{
	...
	/* Disable module */
	pete_writel("drivers/spi/spi-synquacer.c:527", 0, sspi->regs + SYNQUACER_HSSPI_REG_MCTRL);
	status = synquacer_spi_wait_status_update(sspi, false);
	if (status < 0)
		return status;

	pete_writel("drivers/spi/spi-synquacer.c:532", 0, sspi->regs + SYNQUACER_HSSPI_REG_TXE);
	pete_writel("drivers/spi/spi-synquacer.c:533", 0, sspi->regs + SYNQUACER_HSSPI_REG_RXE);
	pete_writel("drivers/spi/spi-synquacer.c:534", ~0, sspi->regs + SYNQUACER_HSSPI_REG_TXC);
	pete_writel("drivers/spi/spi-synquacer.c:535", ~0, sspi->regs + SYNQUACER_HSSPI_REG_RXC);
	pete_writel("drivers/spi/spi-synquacer.c:536", ~0, sspi->regs + SYNQUACER_HSSPI_REG_FAULTC);

	val = pete_readl("drivers/spi/spi-synquacer.c:538", sspi->regs + SYNQUACER_HSSPI_REG_DMCFG);
	val &= ~SYNQUACER_HSSPI_DMCFG_SSDC;
	val &= ~SYNQUACER_HSSPI_DMCFG_MSTARTEN;
	pete_writel("drivers/spi/spi-synquacer.c:541", val, sspi->regs + SYNQUACER_HSSPI_REG_DMCFG);

	val = pete_readl("drivers/spi/spi-synquacer.c:543", sspi->regs + SYNQUACER_HSSPI_REG_MCTRL);
	...
	/* Enable module */
	pete_writel("drivers/spi/spi-synquacer.c:554", val, sspi->regs + SYNQUACER_HSSPI_REG_MCTRL);
...
static irqreturn_t sq_spi_rx_handler(int irq, void *priv)
{
	...
	val = pete_readl("drivers/spi/spi-synquacer.c:567", sspi->regs + SYNQUACER_HSSPI_REG_RXF);
	if ((val & SYNQUACER_HSSPI_RXF_SLAVE_RELEASED) ||
	    (val & SYNQUACER_HSSPI_RXF_FIFO_MORE_THAN_THRESHOLD)) {
		read_fifo(sspi);

		if (sspi->rx_words == 0) {
			pete_writel("drivers/spi/spi-synquacer.c:573", 0, sspi->regs + SYNQUACER_HSSPI_REG_RXE);
...
static irqreturn_t sq_spi_tx_handler(int irq, void *priv)
{
	...
	val = pete_readl("drivers/spi/spi-synquacer.c:587", sspi->regs + SYNQUACER_HSSPI_REG_TXF);
	if (val & SYNQUACER_HSSPI_TXF_FIFO_EMPTY) {
		if (sspi->tx_words == 0) {
			pete_writel("drivers/spi/spi-synquacer.c:590", 0, sspi->regs + SYNQUACER_HSSPI_REG_TXE);
			complete(&sspi->transfer_done);
		} else {
			write_fifo(sspi);
		}
