...
{
	return pete_readl("drivers/spi/spi-tegra210-quad.c:235", tqspi->base + offset);
}

static inline void tegra_qspi_writel(struct tegra_qspi *tqspi, u32 value, unsigned long offset)
{
	pete_writel("drivers/spi/spi-tegra210-quad.c:240", value, tqspi->base + offset);

	/* read back register to make sure that register writes completed */
	if (offset != QSPI_TX_FIFO)
		pete_readl("drivers/spi/spi-tegra210-quad.c:244", tqspi->base + QSPI_COMMAND1);
}
