...
{
	return pete_readl("drivers/spi/spi-hisi-kunpeng.c:183", hs->regs + HISI_SPI_SR) & SR_BUSY;
}

static u32 hisi_spi_rx_not_empty(struct hisi_spi *hs)
{
	return pete_readl("drivers/spi/spi-hisi-kunpeng.c:188", hs->regs + HISI_SPI_SR) & SR_RXNE;
}

static u32 hisi_spi_tx_not_full(struct hisi_spi *hs)
{
	return pete_readl("drivers/spi/spi-hisi-kunpeng.c:193", hs->regs + HISI_SPI_SR) & SR_TXNF;
}
...
static void hisi_spi_flush_fifo(struct hisi_spi *hs)
{
	...
	do {
		while (hisi_spi_rx_not_empty(hs))
			pete_readl("drivers/spi/spi-hisi-kunpeng.c:202", hs->regs + HISI_SPI_DOUT);
...
{
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:209", 0, hs->regs + HISI_SPI_ENR);
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:210", IMR_MASK, hs->regs + HISI_SPI_IMR);
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:211", ICR_MASK, hs->regs + HISI_SPI_ICR);
}
...
static void hisi_spi_reader(struct hisi_spi *hs)
{
	...
	while (hisi_spi_rx_not_empty(hs) && max--) {
		rxw = pete_readl("drivers/spi/spi-hisi-kunpeng.c:230", hs->regs + HISI_SPI_DOUT);
...
static void hisi_spi_writer(struct hisi_spi *hs)
{
	...
	while (hisi_spi_tx_not_full(hs) && max--) {
		...
		pete_writel("drivers/spi/spi-hisi-kunpeng.c:271", txw, hs->regs + HISI_SPI_DIN);
		--hs->tx_len;
	}
}
...
static void hisi_spi_hw_init(struct hisi_spi *hs)
{
	...
	/* FIFO default config */
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:328", FIELD_PREP(FIFOC_TX_MASK, HISI_SPI_TX_64_OR_LESS) |
...
static irqreturn_t hisi_spi_irq(int irq, void *dev_id)
{
	...
	u32 irq_status = pete_readl("drivers/spi/spi-hisi-kunpeng.c:339", hs->regs + HISI_SPI_ISR) & ISR_MASK;
		...
		struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:387", cr, hs->regs + HISI_SPI_CR);
	...
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:404", ~(u32)IMR_MASK, hs->regs + HISI_SPI_IMR);
	pete_writel("drivers/spi/spi-hisi-kunpeng.c:405", 1, hs->regs + HISI_SPI_ENR);
...
static int hisi_spi_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "hw version:0x%x max-freq:%u kHz\n",
		pete_readl("drivers/spi/spi-hisi-kunpeng.c:522", hs->regs + HISI_SPI_VERSION),
