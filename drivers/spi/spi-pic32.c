...
static int pic32_spi_set_word_size(struct pic32_spi *pic32s, u8 bits_per_word)
{
	...
	pic32s->fifo_n_elm = DIV_ROUND_UP(pic32s->fifo_n_byte,
					  bits_per_word / 8);
	/* set word size */
	v = pete_readl("drivers/spi/spi-pic32.c:421", &pic32s->regs->ctrl);
	v &= ~(CTRL_BPW_MASK << CTRL_BPW_SHIFT);
	v |= buswidth << CTRL_BPW_SHIFT;
	pete_writel("drivers/spi/spi-pic32.c:424", v, &pic32s->regs->ctrl);
				     ...
				     struct spi_message *msg)
{
	...
	if (pic32s->mode != spi->mode) {
		val = pete_readl("drivers/spi/spi-pic32.c:463", &pic32s->regs->ctrl);
		/* active low */
		...
		/* rx at end of tx */
		val |= CTRL_SMP;
		pete_writel("drivers/spi/spi-pic32.c:477", val, &pic32s->regs->ctrl);
...
static void pic32_spi_hw_init(struct pic32_spi *pic32s)
{
	...
	ctrl = pete_readl("drivers/spi/spi-pic32.c:675", &pic32s->regs->ctrl);
	...
	pete_writel("drivers/spi/spi-pic32.c:700", ctrl, &pic32s->regs->ctrl);

	/* enable error reporting */
	ctrl = CTRL2_TX_UR_EN | CTRL2_RX_OV_EN | CTRL2_FRM_ERR_EN;
	pete_writel("drivers/spi/spi-pic32.c:704", ctrl, &pic32s->regs->ctrl2_set);
}
