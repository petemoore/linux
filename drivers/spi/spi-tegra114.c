...
static inline u32 tegra_spi_readl(struct tegra_spi_data *tspi,
		unsigned long reg)
{
	return pete_readl("drivers/spi/spi-tegra114.c:229", tspi->base + reg);
}

static inline void tegra_spi_writel(struct tegra_spi_data *tspi,
		u32 val, unsigned long reg)
{
	pete_writel("drivers/spi/spi-tegra114.c:235", val, tspi->base + reg);

	/* Read back register to make sure that register writes completed */
	if (reg != SPI_TX_FIFO)
		pete_readl("drivers/spi/spi-tegra114.c:239", tspi->base + SPI_COMMAND1);
}
