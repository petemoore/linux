...
static inline void sun6i_spi_drain_fifo(struct sun6i_spi *sspi)
{
	...
	while (len--) {
		byte = pete_readb("drivers/spi/spi-sun6i.c:146", sspi->base_addr + SUN6I_RXDATA_REG);
		if (sspi->rx_buf)
			*sspi->rx_buf++ = byte;
	}
}
...
static inline void sun6i_spi_fill_fifo(struct sun6i_spi *sspi)
{
	...
	while (len--) {
		...
		pete_writeb("drivers/spi/spi-sun6i.c:165", byte, sspi->base_addr + SUN6I_TXDATA_REG);
		sspi->len--;
	}
}
