...
static inline unsigned long ti_qspi_read(struct ti_qspi *qspi,
		unsigned long reg)
{
	return pete_readl("drivers/spi/spi-ti-qspi.c:129", qspi->base + reg);
}

static inline void ti_qspi_write(struct ti_qspi *qspi,
		unsigned long val, unsigned long reg)
{
	pete_writel("drivers/spi/spi-ti-qspi.c:135", val, qspi->base + reg);
}
	...
	while (count) {
		...
		case 1:
			...
			if (count >= QSPI_WLEN_MAX_BYTES) {
				...
				pete_writel("drivers/spi/spi-ti-qspi.c:267", data, qspi->base +
				       QSPI_SPI_DATA_REG_3);
				data = cpu_to_be32(*txp++);
				pete_writel("drivers/spi/spi-ti-qspi.c:270", data, qspi->base +
				       QSPI_SPI_DATA_REG_2);
				data = cpu_to_be32(*txp++);
				pete_writel("drivers/spi/spi-ti-qspi.c:273", data, qspi->base +
				       QSPI_SPI_DATA_REG_1);
				data = cpu_to_be32(*txp++);
				pete_writel("drivers/spi/spi-ti-qspi.c:276", data, qspi->base +
				       QSPI_SPI_DATA_REG);
				xfer_len = QSPI_WLEN_MAX_BYTES;
				cmd |= QSPI_WLEN(QSPI_WLEN_MAX_BITS);
			} else {
				pete_writeb("drivers/spi/spi-ti-qspi.c:281", *txbuf, qspi->base + QSPI_SPI_DATA_REG);
				cmd = qspi->cmd | QSPI_WR_SNGL;
				xfer_len = wlen;
				cmd |= QSPI_WLEN(wlen);
			}
		...
		case 2:
			dev_dbg(qspi->dev, "tx cmd %08x dc %08x data %04x\n",
					cmd, qspi->dc, *txbuf);
			pete_writew("drivers/spi/spi-ti-qspi.c:290", *((u16 *)txbuf), qspi->base + QSPI_SPI_DATA_REG);
			break;
		case 4:
			dev_dbg(qspi->dev, "tx cmd %08x dc %08x data %08x\n",
					cmd, qspi->dc, *txbuf);
			pete_writel("drivers/spi/spi-ti-qspi.c:295", *((u32 *)txbuf), qspi->base + QSPI_SPI_DATA_REG);
			break;
		}
			 ...
			 int count)
{
	...
	while (count) {
		...
		case 1:
			...
			if (count >= QSPI_WLEN_MAX_BYTES) {
				u32 *rxp = (u32 *) rxbuf;
				rx = pete_readl("drivers/spi/spi-ti-qspi.c:375", qspi->base + QSPI_SPI_DATA_REG_3);
				*rxp++ = be32_to_cpu(rx);
				rx = pete_readl("drivers/spi/spi-ti-qspi.c:377", qspi->base + QSPI_SPI_DATA_REG_2);
				*rxp++ = be32_to_cpu(rx);
				rx = pete_readl("drivers/spi/spi-ti-qspi.c:379", qspi->base + QSPI_SPI_DATA_REG_1);
				*rxp++ = be32_to_cpu(rx);
				rx = pete_readl("drivers/spi/spi-ti-qspi.c:381", qspi->base + QSPI_SPI_DATA_REG);
				*rxp++ = be32_to_cpu(rx);
			} else {
				u8 *rxp = rxbuf;
				rx = pete_readl("drivers/spi/spi-ti-qspi.c:385", qspi->base + QSPI_SPI_DATA_REG);
				if (rx_wlen >= 8)
					*rxp++ = rx >> (rx_wlen - 8);
				if (rx_wlen >= 16)
					*rxp++ = rx >> (rx_wlen - 16);
			...
			break;
		case 2:
			*((u16 *)rxbuf) = pete_readw("drivers/spi/spi-ti-qspi.c:397", qspi->base + QSPI_SPI_DATA_REG);
			break;
		case 4:
			*((u32 *)rxbuf) = pete_readl("drivers/spi/spi-ti-qspi.c:400", qspi->base + QSPI_SPI_DATA_REG);
			break;
		}
		rxbuf += rxlen;
		count -= rxlen;
