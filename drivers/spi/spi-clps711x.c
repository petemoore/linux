				     ...
				     struct spi_transfer *xfer)
{
	...
	pete_writel("drivers/spi/spi-clps711x.c:64", data | SYNCIO_FRMLEN(hw->bpw) | SYNCIO_TXFRMEN, hw->syncio);
...
static irqreturn_t spi_clps711x_isr(int irq, void *dev_id)
{
	...
	/* Handle RX */
	data = pete_readb("drivers/spi/spi-clps711x.c:76", hw->syncio);
	if (hw->rx_buf)
		*hw->rx_buf++ = data;
	...
	if (--hw->len > 0) {
		data = hw->tx_buf ? *hw->tx_buf++ : 0;
		pete_writel("drivers/spi/spi-clps711x.c:83", data | SYNCIO_FRMLEN(hw->bpw) | SYNCIO_TXFRMEN,
		       hw->syncio);
	} else
...
static int spi_clps711x_probe(struct platform_device *pdev)
{
	...
	pete_readl("drivers/spi/spi-clps711x.c:138", hw->syncio);
