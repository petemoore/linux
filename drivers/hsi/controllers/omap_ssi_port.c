...
static int ssi_start_dma(struct hsi_msg *msg, int lch)
{
	...
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:270", omap_ssi->sys + SSI_GDD_MPU_IRQ_ENABLE_REG);
	tmp |= SSI_GDD_LCH(lch);
	writel_relaxed(tmp, omap_ssi->sys + SSI_GDD_MPU_IRQ_ENABLE_REG);
	spin_unlock_bh(&omap_ssi->lock);
	pete_writew("drivers/hsi/controllers/omap_ssi_port.c:274", ccr, gdd + SSI_GDD_CCR_REG(lch));
...
static int ssi_start_pio(struct hsi_msg *msg)
{
	...
	val |= pete_readl("drivers/hsi/controllers/omap_ssi_port.c:305", omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:306", val, omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
	pm_runtime_put_autosuspend(omap_port->pdev);
	msg->actual_len = 0;
	msg->status = HSI_STATUS_PROCEEDING;
...
static int ssi_async_break(struct hsi_msg *msg)
{
	...
	if (msg->ttype == HSI_MSG_WRITE) {
		...
		pete_writel("drivers/hsi/controllers/omap_ssi_port.c:347", 1, omap_port->sst_base + SSI_SST_BREAK_REG);
		...
		tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:356", omap_ssi->sys +
					SSI_MPU_ENABLE_REG(port->num, 0));
		pete_writel("drivers/hsi/controllers/omap_ssi_port.c:358", tmp | SSI_BREAKDETECTED,
...
static int ssi_setup(struct hsi_client *cl)
{
	...
	/* Flush posted write */
	val = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:475", ssr + SSI_SSR_MODE_REG);
	/* TX */
...
static int ssi_flush(struct hsi_client *cl)
{
	...
	/* Flush all errors */
	err = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:549", ssr + SSI_SSR_ERROR_REG);
	...
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:558", 0xff, omap_ssi->sys + SSI_GDD_MPU_IRQ_STATUS_REG);
...
static void start_tx_work(struct work_struct *work)
{
	...
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:588", SSI_WAKE(0), omap_ssi->sys + SSI_SET_WAKE_REG(port->num));
}
...
static int ssi_stop_tx(struct hsi_client *cl)
{
	...
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:625", SSI_WAKE(0), omap_ssi->sys + SSI_CLEAR_WAKE_REG(port->num));
	spin_unlock_bh(&omap_port->wk_lock);
...
static void ssi_cleanup_queues(struct hsi_client *cl)
{
	...
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:706", omap_port->sst_base + SSI_SST_BUFSTATE_REG);
	tmp &= ~txbufstate;
	writel_relaxed(tmp, omap_port->sst_base + SSI_SST_BUFSTATE_REG);
	/* Cleanup read buffers */
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:710", omap_port->ssr_base + SSI_SSR_BUFSTATE_REG);
	tmp &= ~rxbufstate;
	writel_relaxed(tmp, omap_port->ssr_base + SSI_SSR_BUFSTATE_REG);
	/* Disarm and ack pending interrupts */
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:714", omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
	tmp &= ~status;
...
static void ssi_cleanup_gdd(struct hsi_controller *ssi, struct hsi_client *cl)
{
	...
	tmp &= ~val;
	writel_relaxed(tmp, omap_ssi->sys + SSI_GDD_MPU_IRQ_ENABLE_REG);
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:750", val, omap_ssi->sys + SSI_GDD_MPU_IRQ_STATUS_REG);
}

static int ssi_set_port_mode(struct omap_ssi_port *omap_port, u32 mode)
{
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:755", mode, omap_port->sst_base + SSI_SST_MODE_REG);
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:756", mode, omap_port->ssr_base + SSI_SSR_MODE_REG);
	/* OCP barrier */
	mode = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:758", omap_port->ssr_base + SSI_SSR_MODE_REG);
...
static void ssi_error(struct hsi_port *port)
{
	...
	/* ACK error */
	err = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:811", omap_port->ssr_base + SSI_SSR_ERROR_REG);
	...
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:827", omap_ssi->sys + SSI_GDD_MPU_IRQ_ENABLE_REG);
	tmp &= ~val;
	writel_relaxed(tmp, omap_ssi->sys + SSI_GDD_MPU_IRQ_ENABLE_REG);
	spin_unlock(&omap_ssi->lock);
	/* Cancel all PIO read transfers */
	spin_lock(&omap_port->lock);
	tmp = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:833", omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
...
static void ssi_break_complete(struct hsi_port *port)
{
	...
	spin_lock(&omap_port->lock);
	val = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:869", omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
	...
	pete_writel("drivers/hsi/controllers/omap_ssi_port.c:873", SSI_BREAKDETECTED,
			omap_ssi->sys + SSI_MPU_STATUS_REG(port->num, 0));
	spin_unlock(&omap_port->lock);
...
static void ssi_pio_complete(struct hsi_port *port, struct list_head *queue)
{
	...
	if (msg->status == HSI_STATUS_PROCEEDING) {
		...
		if (msg->ttype == HSI_MSG_WRITE)
			pete_writel("drivers/hsi/controllers/omap_ssi_port.c:910", *buf, omap_port->sst_base +
					SSI_SST_BUFFER_CH_REG(msg->channel));
		 else
			*buf = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:913", omap_port->ssr_base +
					SSI_SSR_BUFFER_CH_REG(msg->channel));
		...
		if ((msg->status == HSI_STATUS_PROCEEDING) ||
			...
			pete_writel("drivers/hsi/controllers/omap_ssi_port.c:927", val, omap_ssi->sys +
					SSI_MPU_STATUS_REG(port->num, 0));
			spin_unlock_bh(&omap_port->lock);
	...
	reg = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:936", omap_ssi->sys + SSI_MPU_ENABLE_REG(port->num, 0));
	if (msg->ttype == HSI_MSG_WRITE) {
...
static irqreturn_t ssi_pio_thread(int irq, void *ssi_port)
{
	...
	do {
		status_reg = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:964", sys + SSI_MPU_STATUS_REG(port->num, 0));
		status_reg &= pete_readl("drivers/hsi/controllers/omap_ssi_port.c:965", sys + SSI_MPU_ENABLE_REG(port->num, 0));
		...
		status_reg = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:978", sys + SSI_MPU_STATUS_REG(port->num, 0));
		status_reg &= pete_readl("drivers/hsi/controllers/omap_ssi_port.c:979", sys + SSI_MPU_ENABLE_REG(port->num, 0));
...
static irqreturn_t ssi_wake_thread(int irq __maybe_unused, void *ssi_port)
{
	...
	if (ssi_wakein(port)) {
		...
		if (omap_port->wktest) { /* FIXME: HACK ! To be removed */
			pete_writel("drivers/hsi/controllers/omap_ssi_port.c:1009", SSI_WAKE(0),
		...
		if (omap_port->wktest) { /* FIXME: HACK ! To be removed */
			pete_writel("drivers/hsi/controllers/omap_ssi_port.c:1016", SSI_WAKE(0),
...
static int ssi_save_port_ctx(struct omap_ssi_port *omap_port)
{
	...
	omap_port->sys_mpu_enable = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:1282", omap_ssi->sys +
					SSI_MPU_ENABLE_REG(port->num, 0));
...
static int ssi_restore_port_mode(struct omap_ssi_port *omap_port)
{
	...
	/* OCP barrier */
	mode = pete_readl("drivers/hsi/controllers/omap_ssi_port.c:1322", omap_port->ssr_base + SSI_SSR_MODE_REG);
