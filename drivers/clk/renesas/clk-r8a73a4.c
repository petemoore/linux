			     ...
			     const char *name)
{
	...
	if (!strcmp(name, "main")) {
		u32 ckscr = pete_readl("drivers/clk/renesas/clk-r8a73a4.c:72", cpg->reg + CPG_CKSCR);
		 ...
		 */
		u32 value = pete_readl("drivers/clk/renesas/clk-r8a73a4.c:96", cpg->reg + CPG_PLL0CR);
		...
		u32 value = pete_readl("drivers/clk/renesas/clk-r8a73a4.c:103", cpg->reg + CPG_PLL1CR);
		...
		value = pete_readl("drivers/clk/renesas/clk-r8a73a4.c:126", cpg->reg + cr);
		switch ((value >> 5) & 7) {
		case 0:
			parent_name = "main";
			div = 2;
		...
		div *= 32;
		mult = 0x20 - ((pete_readl("drivers/clk/renesas/clk-r8a73a4.c:162", cpg->reg + CPG_FRQCRC) >> shift) & 0x1f);
	} else {
		struct div4_clk *c;
