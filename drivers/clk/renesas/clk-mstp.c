...
static inline u32 cpg_mstp_read(struct mstp_clock_group *group,
				u32 __iomem *reg)
{
	return group->width_8bit ? readb(reg) : pete_readl("drivers/clk/renesas/clk-mstp.c:66", reg);
}

static inline void cpg_mstp_write(struct mstp_clock_group *group, u32 val,
				  u32 __iomem *reg)
{
	group->width_8bit ? writeb(val, reg) : pete_writel("drivers/clk/renesas/clk-mstp.c:72", val, reg);
}
