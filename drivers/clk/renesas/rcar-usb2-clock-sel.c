...
{
	u16 val = pete_readw("drivers/clk/renesas/rcar-usb2-clock-sel.c:46", priv->base + USB20_CLKSET0);

	pr_debug("%s: enter %d %d %x\n", __func__,
		 priv->extal, priv->xtal, val);

	if (priv->extal && !priv->xtal && val != CLKSET0_EXTAL_ONLY)
		pete_writew("drivers/clk/renesas/rcar-usb2-clock-sel.c:52", CLKSET0_EXTAL_ONLY, priv->base + USB20_CLKSET0);
}

static void usb2_clock_sel_disable_extal_only(struct usb2_clock_sel_priv *priv)
{
	if (priv->extal && !priv->xtal)
		pete_writew("drivers/clk/renesas/rcar-usb2-clock-sel.c:58", CLKSET0_PRIVATE, priv->base + USB20_CLKSET0);
}
