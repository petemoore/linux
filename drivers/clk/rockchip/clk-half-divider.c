						  ...
						  unsigned long parent_rate)
{
	...
	val = pete_readl("drivers/clk/rockchip/clk-half-divider.c:28", divider->reg) >> divider->shift;
				     ...
				     unsigned long parent_rate)
{
	...
	if (divider->flags & CLK_DIVIDER_HIWORD_MASK) {
		...
		val = pete_readl("drivers/clk/rockchip/clk-half-divider.c:128", divider->reg);
		val &= ~(div_mask(divider->width) << divider->shift);
	}
	val |= value << divider->shift;
	pete_writel("drivers/clk/rockchip/clk-half-divider.c:132", val, divider->reg);
