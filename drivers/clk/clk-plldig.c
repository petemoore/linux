...
static int plldig_enable(struct clk_hw *hw)
{
	...
	val = pete_readl("drivers/clk/clk-plldig.c:70", data->regs + PLLDIG_REG_PLLFM);
	/*
	 * Use Bypass mode with PLL off by default, the frequency overshoot
	 * detector output was disable. SSCG Bypass mode should be enable.
	 */
	val |= PLLDIG_SSCGBYP_ENABLE;
	pete_writel("drivers/clk/clk-plldig.c:76", val, data->regs + PLLDIG_REG_PLLFM);
...
static void plldig_disable(struct clk_hw *hw)
{
	...
	val = pete_readl("drivers/clk/clk-plldig.c:86", data->regs + PLLDIG_REG_PLLFM);

	val &= ~PLLDIG_SSCGBYP_ENABLE;
	val |= FIELD_PREP(PLLDIG_SSCGBYP_ENABLE, 0x0);

	pete_writel("drivers/clk/clk-plldig.c:91", val, data->regs + PLLDIG_REG_PLLFM);
}
...
static int plldig_is_enabled(struct clk_hw *hw)
{
	...
	return pete_readl("drivers/clk/clk-plldig.c:98", data->regs + PLLDIG_REG_PLLFM) &
					...
					unsigned long parent_rate)
{
	...
	val = pete_readl("drivers/clk/clk-plldig.c:108", data->regs + PLLDIG_REG_PLLDV);
		...
		unsigned long parent_rate)
{
	...
	val = pete_readl("drivers/clk/clk-plldig.c:161", data->regs + PLLDIG_REG_PLLDV);
	val &= ~PLLDIG_RFDPHI1_MASK;
	val |= FIELD_PREP(PLLDIG_RFDPHI1_MASK, rfdphi1);
	pete_writel("drivers/clk/clk-plldig.c:164", val, data->regs + PLLDIG_REG_PLLDV);
...
static int plldig_init(struct clk_hw *hw)
{
	...
	pete_writel("drivers/clk/clk-plldig.c:210", val, data->regs + PLLDIG_REG_PLLDV);

	/* Enable fractional divider */
	if (fracdiv) {
		val = FIELD_PREP(PLLDIG_FRAC_MASK, fracdiv);
		val |= PLLDIG_FDEN;
		pete_writel("drivers/clk/clk-plldig.c:216", val, data->regs + PLLDIG_REG_PLLFD);
	}
