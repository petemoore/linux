...
static int lpc18xx_ccu_gate_endisable(struct clk_hw *hw, bool enable)
{
	 ...
	 */
	val = pete_readl("drivers/clk/nxp/clk-lpc18xx-ccu.c:143", gate->reg);
	...
	if (enable) {
		...
		pete_writel("drivers/clk/nxp/clk-lpc18xx-ccu.c:156", val, gate->reg);

		val &= ~LPC18XX_CCU_RUN;
	}

	pete_writel("drivers/clk/nxp/clk-lpc18xx-ccu.c:161", val, gate->reg);
