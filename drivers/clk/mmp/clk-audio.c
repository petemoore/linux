					   ...
					   unsigned long parent_rate)
{
	...
	aud_pll_ctrl0 = pete_readl("drivers/clk/mmp/clk-audio.c:125", priv->mmio_base + SSPA_AUD_PLL_CTRL0);
	...
	aud_pll_ctrl1 = pete_readl("drivers/clk/mmp/clk-audio.c:133", priv->mmio_base + SSPA_AUD_PLL_CTRL1);
			      ...
			      unsigned long parent_rate)
{
	...
	for (prediv = 0; prediv < ARRAY_SIZE(predivs); prediv++) {
		...
		for (postdiv = 0; postdiv < ARRAY_SIZE(postdivs); postdiv++) {
			...
			val |= SSPA_AUD_PLL_CTRL0_DIV_FBCCLK(predivs[prediv].fbcclk);
			val |= SSPA_AUD_PLL_CTRL0_DIV_MCLK(predivs[prediv].mclk);
			pete_writel("drivers/clk/mmp/clk-audio.c:216", val, priv->mmio_base + SSPA_AUD_PLL_CTRL0);

			val = SSPA_AUD_PLL_CTRL1_CLK_SEL_AUDIO_PLL;
			val |= SSPA_AUD_PLL_CTRL1_DIV_OCLK_PATTERN(postdivs[postdiv].pattern);
			pete_writel("drivers/clk/mmp/clk-audio.c:220", val, priv->mmio_base + SSPA_AUD_PLL_CTRL1);
