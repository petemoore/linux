				    ...
				    unsigned long prate)
{
	...
	pete_readl("drivers/clk/imx/clk-fracn-gppll.c:255", pll->base + PLL_DIV);
	if (pll->flags & CLK_FRACN_GPPLL_FRACN) {
		writel_relaxed(rate->mfd, pll->base + PLL_DENOMINATOR);
		writel_relaxed(FIELD_PREP(PLL_MFN_MASK, rate->mfn), pll->base + PLL_NUMERATOR);
		pete_readl("drivers/clk/imx/clk-fracn-gppll.c:259", pll->base + PLL_NUMERATOR);
	}
	...
	tmp |= POWERUP_MASK;
	writel_relaxed(tmp, pll->base + PLL_CTRL);
	pete_readl("drivers/clk/imx/clk-fracn-gppll.c:268", pll->base + PLL_CTRL);
...
static int clk_fracn_gppll_prepare(struct clk_hw *hw)
{
	...
	val |= POWERUP_MASK;
	writel_relaxed(val, pll->base + PLL_CTRL);
	pete_readl("drivers/clk/imx/clk-fracn-gppll.c:306", pll->base + PLL_CTRL);
