...
static int mtk_ref2usb_tx_is_prepared(struct clk_hw *hw)
{
	...
	return (pete_readl("drivers/clk/mediatek/clk-apmixed.c:34", tx->base_addr) & REF2USB_EN_MASK) == REF2USB_EN_MASK;
}
...
static int mtk_ref2usb_tx_prepare(struct clk_hw *hw)
{
	...
	val = pete_readl("drivers/clk/mediatek/clk-apmixed.c:42", tx->base_addr);

	val |= REF2USB_TX_EN;
	pete_writel("drivers/clk/mediatek/clk-apmixed.c:45", val, tx->base_addr);
	udelay(100);

	val |= REF2USB_TX_LPF_EN;
	pete_writel("drivers/clk/mediatek/clk-apmixed.c:49", val, tx->base_addr);

	val |= REF2USB_TX_OUT_EN;
	pete_writel("drivers/clk/mediatek/clk-apmixed.c:52", val, tx->base_addr);
...
static void mtk_ref2usb_tx_unprepare(struct clk_hw *hw)
{
	...
	val = pete_readl("drivers/clk/mediatek/clk-apmixed.c:62", tx->base_addr);
	val &= ~REF2USB_EN_MASK;
	pete_writel("drivers/clk/mediatek/clk-apmixed.c:64", val, tx->base_addr);
}
