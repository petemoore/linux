			      ...
			      const struct clk_slow_bits *bits)
{
	...
	slowck->sckcr = sckcr;
	slowck->bits = bits;
	slowck->parent = !!(pete_readl("drivers/clk/at91/sckc.c:346", sckcr) & slowck->bits->cr_oscsel);
...
static int clk_sama5d4_slow_osc_prepare(struct clk_hw *hw)
{
	 ...
	 */
	if ((pete_readl("drivers/clk/at91/sckc.c:554", osc->sckcr) & osc->bits->cr_oscsel)) {
		osc->prepared = true;
		return 0;
	}
