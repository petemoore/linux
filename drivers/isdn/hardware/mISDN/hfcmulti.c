...
hfcmulti_resync(struct hfc_multi *locked, struct hfc_multi *newmaster, int rm)
{
	...
	list_for_each_entry_safe(hc, next, &HFClist, list) {
		if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
			...
			pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:914", plx_acc_32);
			pv &= ~PLX_SYNC_O_EN;
			pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:916", pv, plx_acc_32);
	...
	if (newmaster) {
		...
		plx_acc_32 = hc->plx_membase + PLX_GPIOC;
		pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:936", plx_acc_32);
		pv |= PLX_SYNC_O_EN;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:938", pv, plx_acc_32);
...
release_io_hfcmulti(struct hfc_multi *hc)
{
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip) && hc->plx_membase) {
		...
		plx_acc_32 = hc->plx_membase + PLX_GPIOC;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1033", PLX_GPIOC_INIT, plx_acc_32);
		pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1034", plx_acc_32);
		/* Termination off */
		pv &= ~PLX_TERM_ON;
		/* Disconnect the PCM */
		pv |= PLX_SLAVE_EN_N;
		...
		pv &= ~PLX_DSP_RES_N;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1043", pv, plx_acc_32);
		if (debug & DEBUG_HFCMULTI_INIT)
...
init_chip(struct hfc_multi *hc)
{
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		...
		spin_lock_irqsave(&plx_lock, plx_flags);
		plx_acc_32 = hc->plx_membase + PLX_GPIOC;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1166", PLX_GPIOC_INIT, plx_acc_32);
		pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1167", plx_acc_32);
		...
		/* Put the DSP in Reset */
		pv &= ~PLX_DSP_RES_N;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1176", pv, plx_acc_32);
		spin_unlock_irqrestore(&plx_lock, plx_flags);
		if (debug & DEBUG_HFCMULTI_INIT)
		...
		if (plx_count >= 3) {
			...
			pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1202", plx_acc_32);
			pv &= ~PLX_TERM_ON;
			pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1204", pv, plx_acc_32);
			spin_unlock_irqrestore(&plx_lock, plx_flags);
			if (debug & DEBUG_HFCMULTI_INIT)
				printk(KERN_DEBUG
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		spin_lock_irqsave(&plx_lock, plx_flags);
		plx_acc_32 = hc->plx_membase + PLX_GPIOC;
		pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1264", plx_acc_32);
		/* Connect PCM */
		...
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1279", pv, plx_acc_32);
		spin_unlock_irqrestore(&plx_lock, plx_flags);
	}
	...
	if (val2 >= val + 8) { /* 1 ms */
		...
		if (test_bit(HFC_CHIP_PCM_SLAVE, &hc->chip)) {
			...
			if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
				spin_lock_irqsave(&plx_lock, plx_flags);
				plx_acc_32 = hc->plx_membase + PLX_GPIOC;
				pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1382", plx_acc_32);
				pv |= PLX_MASTER_EN | PLX_SLAVE_EN_N;
				pv |= PLX_SYNC_O_EN;
				pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1385", pv, plx_acc_32);
				spin_unlock_irqrestore(&plx_lock, plx_flags);
				if (debug & DEBUG_HFCMULTI_INIT)
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		...
		spin_lock_irqsave(&plx_lock, plx_flags);
		plx_acc_32 = hc->plx_membase + PLX_GPIOC;
		pv = pete_readl("drivers/isdn/hardware/mISDN/hfcmulti.c:1418", plx_acc_32);
		pv |=  PLX_DSP_RES_N;
		pete_writel("drivers/isdn/hardware/mISDN/hfcmulti.c:1420", pv, plx_acc_32);
		spin_unlock_irqrestore(&plx_lock, plx_flags);
		if (debug & DEBUG_HFCMULTI_INIT)
...
hfcmulti_interrupt(int intno, void *dev_id)
{
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		...
		wval = pete_readw("drivers/isdn/hardware/mISDN/hfcmulti.c:2682", plx_acc);
...
init_card(struct hfc_multi *hc)
{
	...
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		spin_lock_irqsave(&plx_lock, plx_flags);
		plx_acc = hc->plx_membase + PLX_INTCSR;
		pete_writew("drivers/isdn/hardware/mISDN/hfcmulti.c:4246", (PLX_INTCSR_PCIINT_ENABLE | PLX_INTCSR_LINTI1_ENABLE),
...
error:
	if (test_bit(HFC_CHIP_PLXSD, &hc->chip)) {
		spin_lock_irqsave(&plx_lock, plx_flags);
		plx_acc = hc->plx_membase + PLX_INTCSR;
		pete_writew("drivers/isdn/hardware/mISDN/hfcmulti.c:4295", 0x00, plx_acc); /*disable IRQs*/
		spin_unlock_irqrestore(&plx_lock, plx_flags);
	}
