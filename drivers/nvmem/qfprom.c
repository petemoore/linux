				      ...
				      struct qfprom_touched_values *old)
{
	...
	old->timer_val = pete_readl("drivers/nvmem/qfprom.c:227", priv->qfpconf + QFPROM_BLOW_TIMER_OFFSET);
	old->accel_val = pete_readl("drivers/nvmem/qfprom.c:228", priv->qfpconf + QFPROM_ACCEL_OFFSET);
	pete_writel("drivers/nvmem/qfprom.c:229", priv->soc_data->qfprom_blow_timer_value,
	       priv->qfpconf + QFPROM_BLOW_TIMER_OFFSET);
	pete_writel("drivers/nvmem/qfprom.c:231", priv->soc_data->accel_value,
			    ...
			    size_t bytes)
{
	...
	for (i = 0; i < words; i++)
		pete_writel("drivers/nvmem/qfprom.c:303", value[i], priv->qfpraw + reg + (i * 4));
			...
			unsigned int reg, void *_val, size_t bytes)
{
	...
	while (words--)
		*val++ = pete_readb("drivers/nvmem/qfprom.c:332", base + reg + i++);
...
static int qfprom_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		version = pete_readl("drivers/nvmem/qfprom.c:412", priv->qfpsecurity + QFPROM_VERSION_OFFSET);
