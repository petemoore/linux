				     ...
				     u32 mask, u32 set)
{
	...
	data = pete_readl("drivers/nvmem/meson-mx-efuse.c:55", efuse->base + reg);
	data &= ~mask;
	data |= (set & mask);

	pete_writel("drivers/nvmem/meson-mx-efuse.c:59", data, efuse->base + reg);
}
				    ...
				    unsigned int addr, u32 *value)
{
	 ...
	 */
	pete_readl("drivers/nvmem/meson-mx-efuse.c:118", efuse->base + MESON_MX_EFUSE_CNTL1);
	...
	*value = pete_readl("drivers/nvmem/meson-mx-efuse.c:130", efuse->base + MESON_MX_EFUSE_CNTL2);
