...
static void lpss_uart_setup(struct lpss_private_data *pdata)
{
	...
	val = pete_readl("drivers/acpi/acpi_lpss.c:135", pdata->mmio_base + offset);
	pete_writel("drivers/acpi/acpi_lpss.c:136", val | LPSS_TX_INT_MASK, pdata->mmio_base + offset);

	val = pete_readl("drivers/acpi/acpi_lpss.c:138", pdata->mmio_base + LPSS_UART_CPR);
	if (!(val & LPSS_UART_CPR_AFCE)) {
		offset = pdata->dev_desc->prv_offset + LPSS_GENERAL;
		val = pete_readl("drivers/acpi/acpi_lpss.c:141", pdata->mmio_base + offset);
		val |= LPSS_GENERAL_UART_RTS_OVRD;
		pete_writel("drivers/acpi/acpi_lpss.c:143", val, pdata->mmio_base + offset);
	}
}
...
static void lpss_deassert_reset(struct lpss_private_data *pdata)
{
	...
	val = pete_readl("drivers/acpi/acpi_lpss.c:153", pdata->mmio_base + offset);
	val |= LPSS_RESETS_RESET_APB | LPSS_RESETS_RESET_FUNC;
	pete_writel("drivers/acpi/acpi_lpss.c:155", val, pdata->mmio_base + offset);
}
				 ...
				 struct lpss_private_data *pdata)
{
	...
	if (dev_desc->flags & LPSS_CLK_DIVIDER) {
		...
		if (!pete_readl("drivers/acpi/acpi_lpss.c:462", prv_base))
			pete_writel("drivers/acpi/acpi_lpss.c:463", LPSS_CLK_DIVIDER_DEF_MASK, prv_base);
...
{
	return pete_readl("drivers/acpi/acpi_lpss.c:732", pdata->mmio_base + pdata->dev_desc->prv_offset + reg);
}

static void __lpss_reg_write(u32 val, struct lpss_private_data *pdata,
			     unsigned int reg)
{
	pete_writel("drivers/acpi/acpi_lpss.c:738", val, pdata->mmio_base + pdata->dev_desc->prv_offset + reg);
}
