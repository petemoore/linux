...
{
	u32 list_count = pete_readl("drivers/scsi/hptiop.c:1194", &hba->u.mvfrey.mu->inbound_conf_ctl);
	char *p;
	dma_addr_t phy;
...
static void hptiop_disable_intr_itl(struct hptiop_hba *hba)
{
	...
	int_mask = pete_readl("drivers/scsi/hptiop.c:1522", &hba->u.itl.iop->outbound_intmask);
	pete_writel("drivers/scsi/hptiop.c:1523", int_mask |
		IOPMU_OUTBOUND_INT_MSG0 | IOPMU_OUTBOUND_INT_POSTQUEUE,
		&hba->u.itl.iop->outbound_intmask);
	pete_readl("drivers/scsi/hptiop.c:1526", &hba->u.itl.iop->outbound_intmask);
}

static void hptiop_disable_intr_mv(struct hptiop_hba *hba)
{
	pete_writel("drivers/scsi/hptiop.c:1531", 0, &hba->u.mv.regs->outbound_intmask);
	pete_readl("drivers/scsi/hptiop.c:1532", &hba->u.mv.regs->outbound_intmask);
}

static void hptiop_disable_intr_mvfrey(struct hptiop_hba *hba)
{
	pete_writel("drivers/scsi/hptiop.c:1537", 0, &(hba->u.mvfrey.mu->f0_doorbell_enable));
	pete_readl("drivers/scsi/hptiop.c:1538", &(hba->u.mvfrey.mu->f0_doorbell_enable));
	pete_writel("drivers/scsi/hptiop.c:1539", 0, &(hba->u.mvfrey.mu->isr_enable));
	pete_readl("drivers/scsi/hptiop.c:1540", &(hba->u.mvfrey.mu->isr_enable));
	pete_writel("drivers/scsi/hptiop.c:1541", 0, &(hba->u.mvfrey.mu->pcie_f0_int_enable));
	pete_readl("drivers/scsi/hptiop.c:1542", &(hba->u.mvfrey.mu->pcie_f0_int_enable));
}
