...
megasas_dump_reg_set(void __iomem *reg_set)
{
	...
	for (i = 0; i < (sz / sizeof(u32)); i++)
		printk("%08x: %08x\n", (i * 4), pete_readl("drivers/scsi/megaraid/megaraid_sas_base.c:2990", &reg[i]));
}
...
megasas_dump_sys_regs(void __iomem *reg_set, char *buf)
{
	...
	for (i = 0; i < sz / sizeof(u32); i++) {
		bytes_wrote += scnprintf(loc + bytes_wrote,
					 ...
					 "%08x: %08x\n", (i * 4),
					 pete_readl("drivers/scsi/megaraid/megaraid_sas_base.c:3050", &reg[i]));
	}
	return bytes_wrote;
}
	...
	while (fw_state != MFI_STATE_READY) {
		...
		case MFI_STATE_WAIT_HANDSHAKE:
			...
			if ((instance->pdev->device ==
				...
				(instance->adapter_type != MFI_SERIES))
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4154", 
				  MFI_INIT_CLEAR_HANDSHAKE|MFI_INIT_HOTPLUG,
				  &instance->reg_set->doorbell);
			else
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4158", 
				    MFI_INIT_CLEAR_HANDSHAKE|MFI_INIT_HOTPLUG,
					&instance->reg_set->inbound_doorbell);
		...
		case MFI_STATE_BOOT_MESSAGE_PENDING:
			if ((instance->pdev->device ==
				...
				(instance->pdev->device ==
				 PCI_DEVICE_ID_LSI_SAS0071SKINNY) ||
				(instance->adapter_type != MFI_SERIES))
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4171", MFI_INIT_HOTPLUG,
				       &instance->reg_set->doorbell);
			else
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4174", MFI_INIT_HOTPLUG,
					&instance->reg_set->inbound_doorbell);
		...
		case MFI_STATE_OPERATIONAL:
			...
			if ((instance->pdev->device ==
				...
				PCI_DEVICE_ID_LSI_SAS0071SKINNY)  ||
				(instance->adapter_type != MFI_SERIES)) {
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4190", MFI_RESET_FLAGS,
					&instance->reg_set->doorbell);
				...
				pete_writel("drivers/scsi/megaraid/megaraid_sas_base.c:4206", MFI_RESET_FLAGS,
					&instance->reg_set->inbound_doorbell);
