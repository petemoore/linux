	...
	struct CommandList *c, int reply_queue)
{
	...
	case CMD_IOACCEL1:
		...
		pete_writel("drivers/scsi/hpsa.c:1167", c->busaddr, h->vaddr + SA5_REQUEST_PORT_OFFSET);
		break;
	case CMD_IOACCEL2:
	...
	case IOACCEL2_TMF:
		...
		pete_writel("drivers/scsi/hpsa.c:1175", c->busaddr, h->vaddr + IOACCEL2_INBOUND_POSTQ_32);
		break;
	default:
...
static void controller_lockup_detected(struct ctlr_info *h)
{
	...
	spin_lock_irqsave(&h->lock, flags);
	lockup_detected = pete_readl("drivers/scsi/hpsa.c:8274", h->vaddr + SA5_SCRATCHPAD_OFFSET);
	if (!lockup_detected) {
	...
	if (lockup_detected == 0xffff0000) {
		...
		pete_writel("drivers/scsi/hpsa.c:8288", DOORBELL_GENERATE_CHKPT, h->vaddr + SA5_DOORBELL);
	}
	pci_disable_device(h->pdev);
...
static int detect_controller_lockup(struct ctlr_info *h)
{
	...
	spin_lock_irqsave(&h->lock, flags);
	heartbeat = pete_readl("drivers/scsi/hpsa.c:8317", &h->cfgtable->HeartBeat);
	spin_unlock_irqrestore(&h->lock, flags);
...
static void hpsa_ack_ctlr_events(struct ctlr_info *h)
{
	...
	if ((h->transMethod & (CFGTBL_Trans_io_accel1
		...
		dev_warn(&h->pdev->dev,
			...
			h->events, event_type);
		pete_writel("drivers/scsi/hpsa.c:8437", h->events, &(h->cfgtable->clear_event_notify));
		/* Set the "clear event notify field update" bit 6 */
		pete_writel("drivers/scsi/hpsa.c:8439", DOORBELL_CLEAR_EVENTS, h->vaddr + SA5_DOORBELL);
		/* Wait until ctlr clears 'clear event notify field', bit 6 */
		hpsa_wait_for_clear_event_notify_ack(h);
		scsi_unblock_requests(h->scsi_host);
	} else {
		/* Acknowledge controller notification events. */
		pete_writel("drivers/scsi/hpsa.c:8445", h->events, &(h->cfgtable->clear_event_notify));
		pete_writel("drivers/scsi/hpsa.c:8446", DOORBELL_CLEAR_EVENTS, h->vaddr + SA5_DOORBELL);
		hpsa_wait_for_clear_event_notify_ack(h);
	}
	return;
}
...
static int hpsa_ctlr_needs_rescan(struct ctlr_info *h)
{
	...
	h->events = pete_readl("drivers/scsi/hpsa.c:8467", &(h->cfgtable->event_notify));
...
#define HPSA_IOACCEL2_HEADER_SZ 4
	 ...
	 * perform the superfluous pete_readl("drivers/scsi/hpsa.c:9224", ) after each command submission.
	 */
	...
	for (i = 0; i < 8; i++)
		pete_writel("drivers/scsi/hpsa.c:9237", bft[i], &h->transtable->BlockFetch[i]);

	/* size of controller ring buffer */
	pete_writel("drivers/scsi/hpsa.c:9240", h->max_commands, &h->transtable->RepQSize);
	pete_writel("drivers/scsi/hpsa.c:9241", h->nreply_queues, &h->transtable->RepQCount);
	pete_writel("drivers/scsi/hpsa.c:9242", 0, &h->transtable->RepQCtrAddrLow32);
	pete_writel("drivers/scsi/hpsa.c:9243", 0, &h->transtable->RepQCtrAddrHigh32);

	for (i = 0; i < h->nreply_queues; i++) {
		pete_writel("drivers/scsi/hpsa.c:9246", 0, &h->transtable->RepQAddr[i].upper);
		pete_writel("drivers/scsi/hpsa.c:9247", h->reply_queue[i].busaddr,
			&h->transtable->RepQAddr[i].lower);
	}

	pete_writel("drivers/scsi/hpsa.c:9251", 0, &h->cfgtable->HostWrite.command_pool_addr_hi);
	pete_writel("drivers/scsi/hpsa.c:9252", transMethod, &(h->cfgtable->HostWrite.TransportRequest));
	/*
	 * enable outbound interrupt coalescing in accelerator mode;
	 */
	if (trans_support & CFGTBL_Trans_io_accel1) {
		access = SA5_ioaccel_mode1_access;
		pete_writel("drivers/scsi/hpsa.c:9258", 10, &h->cfgtable->HostWrite.CoalIntDelay);
		pete_writel("drivers/scsi/hpsa.c:9259", 4, &h->cfgtable->HostWrite.CoalIntCount);
	} else
		if (trans_support & CFGTBL_Trans_io_accel2)
			access = SA5_ioaccel_mode2_access;
	pete_writel("drivers/scsi/hpsa.c:9263", CFGTBL_ChangeReq, h->vaddr + SA5_DOORBELL);
	if (hpsa_wait_for_mode_change_ack(h)) {
		dev_err(&h->pdev->dev,
			"performant mode problem - doorbell timeout\n");
		return -ENODEV;
	}
	register_value = pete_readl("drivers/scsi/hpsa.c:9269", &(h->cfgtable->TransportActive));
	...
	if (trans_support & CFGTBL_Trans_io_accel1) {
		...
		for (i = 0; i < h->nreply_queues; i++) {
			pete_writel("drivers/scsi/hpsa.c:9286", i, h->vaddr + IOACCEL_MODE1_REPLY_QUEUE_INDEX);
			h->reply_queue[i].current_entry =
				pete_readl("drivers/scsi/hpsa.c:9288", h->vaddr + IOACCEL_MODE1_PRODUCER_INDEX);
		}
		...
		bft2_offset = pete_readl("drivers/scsi/hpsa.c:9331", &h->cfgtable->io_accel_request_size_offset);
		...
		for (i = 0; i < ARRAY_SIZE(bft2); i++)
			pete_writel("drivers/scsi/hpsa.c:9341", bft2[i], &h->ioaccel2_bft2_regs[i]);
	}
	pete_writel("drivers/scsi/hpsa.c:9343", CFGTBL_ChangeReq, h->vaddr + SA5_DOORBELL);
	if (hpsa_wait_for_mode_change_ack(h)) {
		dev_err(&h->pdev->dev,
...
{
	h->ioaccel_maxsg =
		pete_readl("drivers/scsi/hpsa.c:9371", &(h->cfgtable->io_accel_max_embedded_sg_count));
...
static int hpsa_alloc_ioaccel2_cmd_and_bft(struct ctlr_info *h)
{
	...
	h->ioaccel_maxsg =
		pete_readl("drivers/scsi/hpsa.c:9428", &(h->cfgtable->io_accel_max_embedded_sg_count));
...
static int hpsa_put_ctlr_into_performant_mode(struct ctlr_info *h)
{
	...
	trans_support = pete_readl("drivers/scsi/hpsa.c:9483", &(h->cfgtable->TransportSupport));
