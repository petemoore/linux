		       ...
		       loff_t *ppos)
{
	...
	if (acc_range == SINGLE_WORD) {
		offset_run = offset;
		u32val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3651", mem_mapped_bar + offset_run);
...
baracc_browse:
	...
	while (index > 0) {
		u32val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3670", mem_mapped_bar + offset_run);
		len += scnprintf(pbuffer+len, LPFC_PCI_BAR_RD_BUF_SIZE-len,
				"%08x ", u32val);
			...
			size_t nbytes, loff_t *ppos)
{
	...
	if (idiag.cmd.opcode == LPFC_IDIAG_CMD_BARACC_RD) {
		...
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_BARACC_WR) {
			pete_writel("drivers/scsi/lpfc/lpfc_debugfs.c:3835", value, mem_mapped_bar + offset);
			pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3836", mem_mapped_bar + offset);
		}
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_BARACC_ST) {
			u32val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3839", mem_mapped_bar + offset);
			u32val |= value;
			pete_writel("drivers/scsi/lpfc/lpfc_debugfs.c:3841", u32val, mem_mapped_bar + offset);
			pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3842", mem_mapped_bar + offset);
		}
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_BARACC_CL) {
			u32val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3845", mem_mapped_bar + offset);
			u32val &= ~value;
			pete_writel("drivers/scsi/lpfc/lpfc_debugfs.c:3847", u32val, mem_mapped_bar + offset);
			pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:3848", mem_mapped_bar + offset);
		}
	} else
		/* All other opecodes are illegal for now */
		goto error_out;
			   ...
			   int len, uint32_t drbregid)
{
	...
	case LPFC_DRB_EQ:
		len += scnprintf(pbuffer + len, LPFC_DRB_ACC_BUF_SIZE-len,
				"EQ-DRB-REG: 0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4582", phba->sli4_hba.EQDBregaddr));
		break;
	case LPFC_DRB_CQ:
		len += scnprintf(pbuffer + len, LPFC_DRB_ACC_BUF_SIZE - len,
				"CQ-DRB-REG: 0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4587", phba->sli4_hba.CQDBregaddr));
		break;
	case LPFC_DRB_MQ:
		len += scnprintf(pbuffer+len, LPFC_DRB_ACC_BUF_SIZE-len,
				"MQ-DRB-REG:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4592", phba->sli4_hba.MQDBregaddr));
		break;
	case LPFC_DRB_WQ:
		len += scnprintf(pbuffer+len, LPFC_DRB_ACC_BUF_SIZE-len,
				"WQ-DRB-REG:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4597", phba->sli4_hba.WQDBregaddr));
		break;
	case LPFC_DRB_RQ:
		len += scnprintf(pbuffer+len, LPFC_DRB_ACC_BUF_SIZE-len,
				"RQ-DRB-REG:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4602", phba->sli4_hba.RQDBregaddr));
		break;
	default:
		break;
	}
			...
			size_t nbytes, loff_t *ppos)
{
	...
	if (idiag.cmd.opcode == LPFC_IDIAG_CMD_DRBACC_WR ||
		...
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_DRBACC_ST) {
			reg_val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4748", drb_reg);
			reg_val |= value;
		}
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_DRBACC_CL) {
			reg_val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4752", drb_reg);
			reg_val &= ~value;
		}
		pete_writel("drivers/scsi/lpfc/lpfc_debugfs.c:4755", reg_val, drb_reg);
		pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4756", drb_reg); /* flush */
	}
	return nbytes;
			   ...
			   int len, uint32_t ctlregid)
{
	...
	case LPFC_CTL_PORT_SEM:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"Port SemReg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4792", phba->sli4_hba.conf_regs_memmap_p +
				      LPFC_CTL_PORT_SEM_OFFSET));
		break;
	case LPFC_CTL_PORT_STA:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"Port StaReg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4798", phba->sli4_hba.conf_regs_memmap_p +
				      LPFC_CTL_PORT_STA_OFFSET));
		break;
	case LPFC_CTL_PORT_CTL:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"Port CtlReg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4804", phba->sli4_hba.conf_regs_memmap_p +
				      LPFC_CTL_PORT_CTL_OFFSET));
		break;
	case LPFC_CTL_PORT_ER1:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"Port Er1Reg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4810", phba->sli4_hba.conf_regs_memmap_p +
				      LPFC_CTL_PORT_ER1_OFFSET));
		break;
	case LPFC_CTL_PORT_ER2:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"Port Er2Reg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4816", phba->sli4_hba.conf_regs_memmap_p +
				      LPFC_CTL_PORT_ER2_OFFSET));
		break;
	case LPFC_CTL_PDEV_CTL:
		len += scnprintf(pbuffer+len, LPFC_CTL_ACC_BUF_SIZE-len,
				"PDev CtlReg:   0x%08x\n",
				pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4822", phba->sli4_hba.conf_regs_memmap_p +
			...
			size_t nbytes, loff_t *ppos)
{
	...
	if (idiag.cmd.opcode == LPFC_IDIAG_CMD_CTLACC_WR ||
		...
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_CTLACC_ST) {
			reg_val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4972", ctl_reg);
			reg_val |= value;
		}
		if (idiag.cmd.opcode == LPFC_IDIAG_CMD_CTLACC_CL) {
			reg_val = pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4976", ctl_reg);
			reg_val &= ~value;
		}
		pete_writel("drivers/scsi/lpfc/lpfc_debugfs.c:4979", reg_val, ctl_reg);
		pete_readl("drivers/scsi/lpfc/lpfc_debugfs.c:4980", ctl_reg); /* flush */
	}
	return nbytes;
