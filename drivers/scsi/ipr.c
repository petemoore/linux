					  ...
					  u32 clr_ints)
{
	...
	if (ioa_cfg->sis64)
		pete_writeq("drivers/scsi/ipr.c:743", ~0, ioa_cfg->regs.set_interrupt_mask_reg);
	else
		pete_writel("drivers/scsi/ipr.c:745", ~0, ioa_cfg->regs.set_interrupt_mask_reg);

	/* Clear any pending interrupts */
	if (ioa_cfg->sis64)
		pete_writel("drivers/scsi/ipr.c:749", ~0, ioa_cfg->regs.clr_interrupt_reg);
	pete_writel("drivers/scsi/ipr.c:750", clr_ints, ioa_cfg->regs.clr_interrupt_reg32);
	pete_readl("drivers/scsi/ipr.c:751", ioa_cfg->regs.sense_interrupt_reg);
}
...
static void ipr_send_command(struct ipr_cmnd *ipr_cmd)
{
	...
	if (ioa_cfg->sis64) {
		...
		if (ipr_cmd->dma_use_sg * sizeof(struct ipr_ioadl64_desc) > 128 )
			send_dma_addr |= 0x4;
		pete_writeq("drivers/scsi/ipr.c:907", send_dma_addr, ioa_cfg->regs.ioarrin_reg);
	} else
		pete_writel("drivers/scsi/ipr.c:909", send_dma_addr, ioa_cfg->regs.ioarrin_reg);
}
...
static int ipr_wait_iodbg_ack(struct ipr_ioa_cfg *ioa_cfg, int max_delay)
{
	...
	while (delay < max_delay) {
		pcii_reg = pete_readl("drivers/scsi/ipr.c:2733", ioa_cfg->regs.sense_interrupt_reg);
					   ...
					   __be32 *dest, u32 length_in_words)
{
	...
	for (i = 0; i < length_in_words; i++) {
		pete_writel("drivers/scsi/ipr.c:2766", start_addr+(i*4), ioa_cfg->regs.dump_addr_reg);
		*dest = cpu_to_be32(pete_readl("drivers/scsi/ipr.c:2767", ioa_cfg->regs.dump_data_reg));
		dest++;
	}
				      ...
				      __be32 *dest, u32 length_in_words)
{
	...
	pete_writel("drivers/scsi/ipr.c:2796", (IPR_UPROCI_RESET_ALERT | IPR_UPROCI_IO_DEBUG_ALERT),
	...
	pete_writel("drivers/scsi/ipr.c:2808", IPR_PCII_IO_DEBUG_ACKNOWLEDGE,
	       ioa_cfg->regs.clr_interrupt_reg);

	/* Write Mailbox with starting address */
	pete_writel("drivers/scsi/ipr.c:2812", start_addr, ioa_cfg->ioa_mailbox);

	/* Signal address valid - clear IOA Reset alert */
	pete_writel("drivers/scsi/ipr.c:2815", IPR_UPROCI_RESET_ALERT,
	...
	for (i = 0; i < length_in_words; i++) {
		...
		*dest = cpu_to_be32(pete_readl("drivers/scsi/ipr.c:2828", ioa_cfg->ioa_mailbox));
		dest++;

		/* For all but the last word of data, signal data received */
		if (i < (length_in_words - 1)) {
			/* Signal dump data received - Clear IO debug Ack */
			pete_writel("drivers/scsi/ipr.c:2834", IPR_PCII_IO_DEBUG_ACKNOWLEDGE,
			       ioa_cfg->regs.clr_interrupt_reg);
		}
	}

	/* Signal end of block transfer. Set reset alert then clear IO debug ack */
	pete_writel("drivers/scsi/ipr.c:2840", IPR_UPROCI_RESET_ALERT,
	       ioa_cfg->regs.set_uproc_interrupt_reg32);

	pete_writel("drivers/scsi/ipr.c:2843", IPR_UPROCI_IO_DEBUG_ALERT,
	       ioa_cfg->regs.clr_uproc_interrupt_reg32);

	/* Signal dump data received - Clear IO debug Ack */
	pete_writel("drivers/scsi/ipr.c:2847", IPR_PCII_IO_DEBUG_ACKNOWLEDGE,
	       ioa_cfg->regs.clr_interrupt_reg);

	/* Wait for IOA to signal LDUMP exit - IOA reset alert will be cleared */
	while (delay < IPR_LDUMP_MAX_SHORT_ACK_DELAY_IN_USEC) {
		temp_pcii_reg =
		    pete_readl("drivers/scsi/ipr.c:2853", ioa_cfg->regs.sense_uproc_interrupt_reg32);
...
static void ipr_get_ioa_dump(struct ipr_ioa_cfg *ioa_cfg, struct ipr_dump *dump)
{
	...
	start_addr = pete_readl("drivers/scsi/ipr.c:3075", ioa_cfg->ioa_mailbox);
...
static int ipr_cancel_op(struct scsi_cmnd *scsi_cmd)
{
	 ...
	 */
	pete_readl("drivers/scsi/ipr.c:5217", ioa_cfg->regs.sense_interrupt_reg);
					      ...
					      u32 int_reg)
{
	...
	int_mask_reg = pete_readl("drivers/scsi/ipr.c:5331", ioa_cfg->regs.sense_interrupt_mask_reg32);
	...
	if ((int_reg & IPR_PCII_OPER_INTERRUPTS) == 0) {
		if (ioa_cfg->sis64) {
			int_mask_reg = pete_readl("drivers/scsi/ipr.c:5339", ioa_cfg->regs.sense_interrupt_mask_reg);
			int_reg = pete_readl("drivers/scsi/ipr.c:5340", ioa_cfg->regs.sense_interrupt_reg) & ~int_mask_reg;
			if (int_reg & IPR_PCII_IPL_STAGE_CHANGE) {

				/* clear stage change */
				pete_writel("drivers/scsi/ipr.c:5344", IPR_PCII_IPL_STAGE_CHANGE, ioa_cfg->regs.clr_interrupt_reg);
				int_reg = pete_readl("drivers/scsi/ipr.c:5345", ioa_cfg->regs.sense_interrupt_reg) & ~int_mask_reg;
	...
	if (int_reg & IPR_PCII_IOA_TRANS_TO_OPER) {
		/* Mask the interrupt */
		pete_writel("drivers/scsi/ipr.c:5358", IPR_PCII_IOA_TRANS_TO_OPER, ioa_cfg->regs.set_interrupt_mask_reg);
		int_reg = pete_readl("drivers/scsi/ipr.c:5359", ioa_cfg->regs.sense_interrupt_reg);
		...
		if (ioa_cfg->clear_isr) {
			...
			pete_writel("drivers/scsi/ipr.c:5369", IPR_PCII_HRRQ_UPDATED, ioa_cfg->regs.clr_interrupt_reg32);
			int_reg = pete_readl("drivers/scsi/ipr.c:5370", ioa_cfg->regs.sense_interrupt_reg32);
			return IRQ_NONE;
		}
	} else {
...
static irqreturn_t ipr_isr(int irq, void *devp)
{
	...
	while (1) {
		if (ipr_process_hrrq(hrrq, -1, &doneq)) {
			...
			num_hrrq = 0;
			do {
				pete_writel("drivers/scsi/ipr.c:5525", IPR_PCII_HRRQ_UPDATED,
				     ioa_cfg->regs.clr_interrupt_reg32);
				int_reg = pete_readl("drivers/scsi/ipr.c:5527", ioa_cfg->regs.sense_interrupt_reg32);
			} while (int_reg & IPR_PCII_HRRQ_UPDATED &&
				num_hrrq++ < IPR_MAX_HRRQ_RETRIES);

		} else if (rc == IRQ_NONE && irq_none == 0) {
			int_reg = pete_readl("drivers/scsi/ipr.c:5532", ioa_cfg->regs.sense_interrupt_reg32);
			irq_none++;
...
static int ipr_reset_next_stage(struct ipr_cmnd *ipr_cmd)
{
	...
	feedback = pete_readl("drivers/scsi/ipr.c:7618", ioa_cfg->regs.init_feedback_reg);
	...
	if (stage == IPR_IPL_INIT_STAGE_UNKNOWN) {
		pete_writel("drivers/scsi/ipr.c:7633", IPR_PCII_IPL_STAGE_CHANGE, ioa_cfg->regs.set_interrupt_mask_reg);
		int_reg = pete_readl("drivers/scsi/ipr.c:7634", ioa_cfg->regs.sense_interrupt_mask_reg);
		stage_time = ioa_cfg->transop_timeout;
		ipr_cmd->job_step = ipr_ioafp_identify_hrrq;
	} else if (stage == IPR_IPL_INIT_STAGE_TRANSOP) {
		int_reg = pete_readl("drivers/scsi/ipr.c:7638", ioa_cfg->regs.sense_interrupt_reg32);
		if (int_reg & IPR_PCII_IOA_TRANS_TO_OPER) {
			ipr_cmd->job_step = ipr_ioafp_identify_hrrq;
			maskval = IPR_PCII_IPL_STAGE_CHANGE;
			maskval = (maskval << 32) | IPR_PCII_IOA_TRANS_TO_OPER;
			pete_writeq("drivers/scsi/ipr.c:7643", maskval, ioa_cfg->regs.set_interrupt_mask_reg);
			int_reg = pete_readl("drivers/scsi/ipr.c:7644", ioa_cfg->regs.sense_interrupt_mask_reg);
			return IPR_RC_JOB_CONTINUE;
		}
	}
...
static int ipr_reset_enable_ioa(struct ipr_cmnd *ipr_cmd)
{
	...
	if (ioa_cfg->sis64) {
		...
		pete_writel("drivers/scsi/ipr.c:7687", IPR_ENDIAN_SWAP_KEY, ioa_cfg->regs.endian_swap_reg);
		int_reg = pete_readl("drivers/scsi/ipr.c:7688", ioa_cfg->regs.endian_swap_reg);
	}

	int_reg = pete_readl("drivers/scsi/ipr.c:7691", ioa_cfg->regs.sense_interrupt_reg32);

	if (int_reg & IPR_PCII_IOA_TRANS_TO_OPER) {
		pete_writel("drivers/scsi/ipr.c:7694", (IPR_PCII_ERROR_INTERRUPTS | IPR_PCII_HRRQ_UPDATED),
		       ioa_cfg->regs.clr_interrupt_mask_reg32);
		int_reg = pete_readl("drivers/scsi/ipr.c:7696", ioa_cfg->regs.sense_interrupt_mask_reg);
		return IPR_RC_JOB_CONTINUE;
	}

	/* Enable destructive diagnostics on IOA */
	pete_writel("drivers/scsi/ipr.c:7701", ioa_cfg->doorbell, ioa_cfg->regs.set_uproc_interrupt_reg32);

	if (ioa_cfg->sis64) {
		maskval = IPR_PCII_IPL_STAGE_CHANGE;
		maskval = (maskval << 32) | IPR_PCII_OPER_INTERRUPTS;
		pete_writeq("drivers/scsi/ipr.c:7706", maskval, ioa_cfg->regs.clr_interrupt_mask_reg);
	} else
		pete_writel("drivers/scsi/ipr.c:7708", IPR_PCII_OPER_INTERRUPTS, ioa_cfg->regs.clr_interrupt_mask_reg32);

	int_reg = pete_readl("drivers/scsi/ipr.c:7710", ioa_cfg->regs.sense_interrupt_mask_reg);
...
static void ipr_get_unit_check_buffer(struct ipr_ioa_cfg *ioa_cfg)
{
	...
	mailbox = pete_readl("drivers/scsi/ipr.c:7788", ioa_cfg->ioa_mailbox);
...
static int ipr_dump_mailbox_wait(struct ipr_cmnd *ipr_cmd)
{
	...
	if (!ioa_cfg->sis64 || !ipr_cmd->u.time_left ||
	    (pete_readl("drivers/scsi/ipr.c:7869", ioa_cfg->regs.sense_interrupt_reg) &
...
static int ipr_reset_restore_cfg_space(struct ipr_cmnd *ipr_cmd)
{
	...
	if (ioa_cfg->sis64) {
		/* Set the adapter to the correct endian mode. */
		pete_writel("drivers/scsi/ipr.c:7923", IPR_ENDIAN_SWAP_KEY, ioa_cfg->regs.endian_swap_reg);
		pete_readl("drivers/scsi/ipr.c:7924", ioa_cfg->regs.endian_swap_reg);
	}
...
static int ipr_reset_start_bist(struct ipr_cmnd *ipr_cmd)
{
	...
	if (ioa_cfg->ipr_chip->bist_method == IPR_MMIO)
		pete_writel("drivers/scsi/ipr.c:7992", IPR_UPROCI_SIS64_START_BIST,
		       ioa_cfg->regs.set_uproc_interrupt_reg32);
	else
...
static int ipr_reset_allowed(struct ipr_ioa_cfg *ioa_cfg)
{
	...
	temp_reg = pete_readl("drivers/scsi/ipr.c:8139", ioa_cfg->regs.sense_interrupt_reg);
...
static int ipr_reset_alert(struct ipr_cmnd *ipr_cmd)
{
	...
	if ((rc == PCIBIOS_SUCCESSFUL) && (cmd_reg & PCI_COMMAND_MEMORY)) {
		ipr_mask_and_clear_interrupts(ioa_cfg, ~0);
		pete_writel("drivers/scsi/ipr.c:8197", IPR_UPROCI_RESET_ALERT, ioa_cfg->regs.set_uproc_interrupt_reg32);
...
static int ipr_test_msi(struct ipr_ioa_cfg *ioa_cfg, struct pci_dev *pdev)
{
	...
	ipr_mask_and_clear_interrupts(ioa_cfg, ~IPR_PCII_IOA_TRANS_TO_OPER);
	pete_writel("drivers/scsi/ipr.c:9352", IPR_PCII_IO_DEBUG_ACKNOWLEDGE, ioa_cfg->regs.clr_interrupt_mask_reg32);
	pete_readl("drivers/scsi/ipr.c:9353", ioa_cfg->regs.sense_interrupt_mask_reg);
	spin_unlock_irqrestore(ioa_cfg->host->host_lock, lock_flags);
	...
	pete_writel("drivers/scsi/ipr.c:9363", IPR_PCII_IO_DEBUG_ACKNOWLEDGE, ioa_cfg->regs.sense_interrupt_reg32);
	pete_readl("drivers/scsi/ipr.c:9364", ioa_cfg->regs.sense_interrupt_reg);
			 ...
			 const struct pci_device_id *dev_id)
{
	...
	interrupts = pete_readl("drivers/scsi/ipr.c:9508", ioa_cfg->regs.sense_interrupt_reg);
	ipr_wait_for_pci_err_recovery(ioa_cfg);
	 ...
	 */
	mask = pete_readl("drivers/scsi/ipr.c:9591", ioa_cfg->regs.sense_interrupt_mask_reg32);
	interrupts = pete_readl("drivers/scsi/ipr.c:9592", ioa_cfg->regs.sense_interrupt_reg32);
	uproc = pete_readl("drivers/scsi/ipr.c:9593", ioa_cfg->regs.sense_uproc_interrupt_reg32);
