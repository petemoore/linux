...
static int do_s2io_add_mac(struct s2io_nic *sp, u64 addr, int off)
{
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5128", RMAC_ADDR_DATA0_MEM_ADDR(addr),
	       &bar0->rmac_addr_data0_mem);

	val64 =	RMAC_ADDR_CMD_MEM_WE | RMAC_ADDR_CMD_MEM_STROBE_NEW_CMD |
		RMAC_ADDR_CMD_MEM_OFFSET(off);
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5133", val64, &bar0->rmac_addr_cmd_mem);
...
static u64 do_s2io_read_unicast_mc(struct s2io_nic *sp, int offset)
{
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5177", val64, &bar0->rmac_addr_cmd_mem);
	...
	tmp64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5186", &bar0->rmac_addr_data0_mem);
			       ...
			       struct ethtool_regs *regs, void *space)
{
	...
	for (i = 0; i < regs->len; i += 8) {
		reg = pete_readq("drivers/net/ethernet/neterion/s2io.c:5374", sp->bar0 + i);
...
static void s2io_set_led(struct s2io_nic *sp, bool on)
{
	...
	if ((sp->device_type == XFRAME_II_DEVICE) ||
	    ((subid & 0xFF) >= 0x07)) {
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5390", &bar0->gpio_control);
		if (on)
			val64 |= GPIO_CTRL_GPIO_0;
		else
			val64 &= ~GPIO_CTRL_GPIO_0;

		pete_writeq("drivers/net/ethernet/neterion/s2io.c:5396", val64, &bar0->gpio_control);
	} else {
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5398", &bar0->adapter_control);
		if (on)
			val64 |= ADAPTER_LED_ON;
		else
			val64 &= ~ADAPTER_LED_ON;

		pete_writeq("drivers/net/ethernet/neterion/s2io.c:5404", val64, &bar0->adapter_control);
	}
				...
				enum ethtool_phys_id_state state)
{
	...
	if ((sp->device_type == XFRAME_I_DEVICE) && ((subid & 0xFF) < 0x07)) {
		u64 val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5429", &bar0->adapter_control);
	...
	case ETHTOOL_ID_ACTIVE:
		sp->adapt_ctrl_org = pete_readq("drivers/net/ethernet/neterion/s2io.c:5438", &bar0->gpio_control);
	...
	case ETHTOOL_ID_INACTIVE:
		if (CARDS_WITH_FAULTY_LINK_INDICATORS(sp->device_type, subid))
			pete_writeq("drivers/net/ethernet/neterion/s2io.c:5451", sp->adapt_ctrl_org, &bar0->gpio_control);
	}
				       ...
				       struct ethtool_pauseparam *ep)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5503", &bar0->rmac_pause_cfg);
				      ...
				      struct ethtool_pauseparam *ep)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5529", &bar0->rmac_pause_cfg);
	if (ep->tx_pause)
	...
	if (ep->rx_pause)
		...
		val64 &= ~RMAC_PAUSE_RX_ENABLE;
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5538", val64, &bar0->rmac_pause_cfg);
	return 0;
}
...
static int read_eeprom(struct s2io_nic *sp, int off, u64 *data)
{
	...
	if (sp->device_type == XFRAME_I_DEVICE) {
		...
		while (exit_cnt < 5) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5574", &bar0->i2c_control);
	...
	if (sp->device_type == XFRAME_II_DEVICE) {
		...
		SPECIAL_REG_WRITE(val64, &bar0->spi_control, LF);
		while (exit_cnt < 5) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5593", &bar0->spi_control);
			if (val64 & SPI_CONTROL_NACK) {
				ret = 1;
				break;
			} else if (val64 & SPI_CONTROL_DONE) {
				*data = pete_readq("drivers/net/ethernet/neterion/s2io.c:5598", &bar0->spi_data);
				*data &= 0xffffff;
				ret = 0;
				break;
			}
...
static int write_eeprom(struct s2io_nic *sp, int off, u64 data, int cnt)
{
	...
	if (sp->device_type == XFRAME_I_DEVICE) {
		...
		while (exit_cnt < 5) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5640", &bar0->i2c_control);
			if (I2C_CONTROL_CNTL_END(val64)) {
	...
	if (sp->device_type == XFRAME_II_DEVICE) {
		...
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:5653", SPI_DATA_WRITE(data, (cnt << 3)), &bar0->spi_data);
		...
		SPECIAL_REG_WRITE(val64, &bar0->spi_control, LF);
		while (exit_cnt < 5) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5662", &bar0->spi_control);
			if (val64 & SPI_CONTROL_NACK) {
				ret = 1;
				break;
			} else if (val64 & SPI_CONTROL_DONE) {
...
static int s2io_register_test(struct s2io_nic *sp, uint64_t *data)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5852", &bar0->pif_rd_swapper_fb);
	if (val64 != 0x123456789abcdefULL) {
		fail = 1;
		DBG_PRINT(INFO_DBG, "Read Test level %d fails\n", 1);
	}

	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5858", &bar0->rmac_pause_cfg);
	if (val64 != 0xc000ffff00000000ULL) {
		fail = 1;
		DBG_PRINT(INFO_DBG, "Read Test level %d fails\n", 2);
	}

	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5864", &bar0->rx_queue_cfg);
	if (sp->device_type == XFRAME_II_DEVICE)
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5874", &bar0->xgxs_efifo_cfg);
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5881", val64, &bar0->xmsi_data);
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5882", &bar0->xmsi_data);
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:5889", val64, &bar0->xmsi_data);
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:5890", &bar0->xmsi_data);
...
static int s2io_link_test(struct s2io_nic *sp, uint64_t *data)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6056", &bar0->adapter_status);
	if (!(LINK_IS_UP(val64)))
		*data = 1;
	else
		*data = 0;
...
static int s2io_rldram_test(struct s2io_nic *sp, uint64_t *data)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6084", &bar0->adapter_control);
	val64 &= ~ADAPTER_ECC_EN;
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:6086", val64, &bar0->adapter_control);

	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6088", &bar0->mc_rldram_test_ctrl);
	val64 |= MC_RLDRAM_TEST_MODE;
	SPECIAL_REG_WRITE(val64, &bar0->mc_rldram_test_ctrl, LF);

	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6092", &bar0->mc_rldram_mrs);
	...
	while (iteration < 2) {
		...
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6103", val64, &bar0->mc_rldram_test_d0);

		val64 = 0xaaaa5a5555550000ULL;
		if (iteration == 1)
			val64 ^= 0xFFFFFFFFFFFF0000ULL;
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6108", val64, &bar0->mc_rldram_test_d1);

		val64 = 0x55aaaaaaaa5a0000ULL;
		if (iteration == 1)
			val64 ^= 0xFFFFFFFFFFFF0000ULL;
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6113", val64, &bar0->mc_rldram_test_d2);

		val64 = (u64) (0x0000003ffffe0100ULL);
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6116", val64, &bar0->mc_rldram_test_add);
		...
		for (cnt = 0; cnt < 5; cnt++) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6124", &bar0->mc_rldram_test_ctrl);
		...
		for (cnt = 0; cnt < 5; cnt++) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6137", &bar0->mc_rldram_test_ctrl);
			if (val64 & MC_RLDRAM_TEST_DONE)
				break;
			msleep(500);
		}
		...
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6146", &bar0->mc_rldram_test_ctrl);
...
static int s2io_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	if (netif_running(dev)) {
		...
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6654", vBIT(val64, 2, 14), &bar0->rmac_max_pyld_len);
	}
...
static void s2io_set_link(struct work_struct *work)
{
	...
	val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6694", &bar0->adapter_status);
	if (LINK_IS_UP(val64)) {
		if (!(pete_readq("drivers/net/ethernet/neterion/s2io.c:6696", &bar0->adapter_control) & ADAPTER_CNTL_EN)) {
			if (verify_xena_quiescence(nic)) {
				val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6698", &bar0->adapter_control);
				val64 |= ADAPTER_CNTL_EN;
				pete_writeq("drivers/net/ethernet/neterion/s2io.c:6700", val64, &bar0->adapter_control);
				if (CARDS_WITH_FAULTY_LINK_INDICATORS(
					    nic->device_type, subid)) {
					val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6703", &bar0->gpio_control);
					val64 |= GPIO_CTRL_GPIO_0;
					pete_writeq("drivers/net/ethernet/neterion/s2io.c:6705", val64, &bar0->gpio_control);
					val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6706", &bar0->gpio_control);
				} else {
					val64 |= ADAPTER_LED_ON;
					pete_writeq("drivers/net/ethernet/neterion/s2io.c:6709", val64, &bar0->adapter_control);
				}
		...
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6719", &bar0->adapter_control);
		val64 |= ADAPTER_LED_ON;
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6721", val64, &bar0->adapter_control);
		s2io_link(nic, LINK_UP);
	} else {
		if (CARDS_WITH_FAULTY_LINK_INDICATORS(nic->device_type,
						      subid)) {
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6726", &bar0->gpio_control);
			val64 &= ~GPIO_CTRL_GPIO_0;
			pete_writeq("drivers/net/ethernet/neterion/s2io.c:6728", val64, &bar0->gpio_control);
			val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6729", &bar0->gpio_control);
		}
		/* turn off LED */
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:6732", &bar0->adapter_control);
		val64 = val64 & (~ADAPTER_LED_ON);
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:6734", val64, &bar0->adapter_control);
...
static void do_s2io_card_down(struct s2io_nic *sp, int do_io)
{
	...
	while (do_io) {
		...
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:7058", &bar0->adapter_status);
...
static int rts_ds_steer(struct s2io_nic *nic, u8 ds_codepoint, u8 ring)
{
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:7614", val64, &bar0->rts_ds_mem_data);

	val64 = RTS_DS_MEM_CTRL_WE |
		RTS_DS_MEM_CTRL_STROBE_NEW_CMD |
		RTS_DS_MEM_CTRL_OFFSET(ds_codepoint);

	pete_writeq("drivers/net/ethernet/neterion/s2io.c:7620", val64, &bar0->rts_ds_mem_ctrl);
...
s2io_init_nic(struct pci_dev *pdev, const struct pci_device_id *pre)
{
	...
	pete_writeq("drivers/net/ethernet/neterion/s2io.c:7934", val64, &bar0->rmac_addr_cmd_mem);
	wait_for_cmd_complete(&bar0->rmac_addr_cmd_mem,
			      RMAC_ADDR_CMD_MEM_STROBE_CMD_EXECUTING,
			      S2IO_BIT_RESET, true);
	tmp64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:7938", &bar0->rmac_addr_data0_mem);
	mac_down = (u32)tmp64;
	mac_up = (u32) (tmp64 >> 32);
	...
	subid = sp->pdev->subsystem_device;
	if ((subid & 0xFF) >= 0x07) {
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:8000", &bar0->gpio_control);
		val64 |= 0x0000800000000000ULL;
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:8002", val64, &bar0->gpio_control);
		val64 = 0x0411040400000000ULL;
		pete_writeq("drivers/net/ethernet/neterion/s2io.c:8004", val64, (void __iomem *)bar0 + 0x2700);
		val64 = pete_readq("drivers/net/ethernet/neterion/s2io.c:8005", &bar0->gpio_control);
	}
