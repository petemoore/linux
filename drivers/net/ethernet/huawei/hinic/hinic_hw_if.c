			  ...
			  enum hinic_msix_state flag)
{
	...
	mask_bits = pete_readl("drivers/net/ethernet/huawei/hinic/hinic_hw_if.c:143", hwif->intr_regs_base + offset);
	mask_bits &= ~HINIC_PCI_MSIX_ENTRY_CTRL_MASKBIT;

	if (flag)
		mask_bits |= HINIC_PCI_MSIX_ENTRY_CTRL_MASKBIT;

	pete_writel("drivers/net/ethernet/huawei/hinic/hinic_hw_if.c:149", mask_bits, hwif->intr_regs_base + offset);
}
