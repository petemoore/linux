			    ...
			    struct netlink_ext_ack *extack)
{
	...
	if (netif_running(dev)) {
		...
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:4774", np->rx_buf_sz, base + NvRegOffloadConfig);
		setup_hw_rings(dev, NV_SETUP_RX_RING | NV_SETUP_TX_RING);
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:4776", ((np->rx_ring_size-1) << NVREG_RINGSZ_RXSHIFT) + ((np->tx_ring_size-1) << NVREG_RINGSZ_TXSHIFT),
			base + NvRegRingSizes);
		pci_push(base);
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:4779", NVREG_TXRXCTL_KICK|np->txrxctl_bits, get_hwbase(dev) + NvRegTxRxControl);
		pci_push(base);
...
static void nv_vlan_mode(struct net_device *dev, netdev_features_t features)
{
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:4955", np->txrxctl_bits, get_hwbase(dev) + NvRegTxRxControl);
...
static int nv_set_features(struct net_device *dev, netdev_features_t features)
{
	...
	if (changed & NETIF_F_RXCSUM) {
		...
		if (netif_running(dev))
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:4982", np->txrxctl_bits, base + NvRegTxRxControl);
...
static int nv_register_test(struct net_device *dev)
{
	...
	do {
		orig_read = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5053", base + nv_registers_test[i].reg);

		/* xor with mask to toggle bits */
		orig_read ^= nv_registers_test[i].mask;

		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5058", orig_read, base + nv_registers_test[i].reg);

		new_read = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5060", base + nv_registers_test[i].reg);
		...
		orig_read ^= nv_registers_test[i].mask;
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5067", orig_read, base + nv_registers_test[i].reg);
...
static int nv_interrupt_test(struct net_device *dev)
{
	...
	if (netif_running(dev)) {
		/* free current irq */
		nv_free_irq(dev);
		save_poll_interval = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5085", base+NvRegPollingInterval);
	}
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5099", NVREG_POLL_DEFAULT_CPU, base + NvRegPollingInterval);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5100", NVREG_UNKSETUP6_VAL, base + NvRegUnknownSetupReg6);
	...
	if (!(np->msi_flags & NV_MSI_X_ENABLED))
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5116", NVREG_IRQSTAT_MASK, base + NvRegIrqStatus);
	else
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5118", NVREG_IRQSTAT_MASK, base + NvRegMSIXIrqStatus);
	...
	if (netif_running(dev)) {
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5127", save_poll_interval, base + NvRegPollingInterval);
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5128", NVREG_UNKSETUP6_VAL, base + NvRegUnknownSetupReg6);
...
static int nv_loopback_test(struct net_device *dev)
{
	...
	if (netif_running(dev)) {
		nv_disable_irq(dev);
		filter_flags = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5153", base + NvRegPacketFilterFlags);
		misc1_flags = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5154", base + NvRegMisc1);
	} else {
		nv_txrx_reset(dev);
	}
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5164", NVREG_MISC1_FORCE, base + NvRegMisc1);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5165", NVREG_PFF_ALWAYS | NVREG_PFF_LOOPBACK, base + NvRegPacketFilterFlags);

	/* reinit nic view of the rx queue */
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5168", np->rx_buf_sz, base + NvRegOffloadConfig);
	setup_hw_rings(dev, NV_SETUP_RX_RING | NV_SETUP_TX_RING);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5170", ((np->rx_ring_size-1) << NVREG_RINGSZ_RXSHIFT) + ((np->tx_ring_size-1) << NVREG_RINGSZ_TXSHIFT),
		base + NvRegRingSizes);
	pci_push(base);
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5204", NVREG_TXRXCTL_KICK|np->txrxctl_bits, get_hwbase(dev) + NvRegTxRxControl);
 ...
 out:
	...
	if (netif_running(dev)) {
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5255", misc1_flags, base + NvRegMisc1);
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5256", filter_flags, base + NvRegPacketFilterFlags);
		nv_enable_irq(dev);
	}
...
static void nv_self_test(struct net_device *dev, struct ethtool_test *test, u64 *buffer)
{
	...
	if (test->flags & ETH_TEST_FL_OFFLINE) {
		if (netif_running(dev)) {
			...
			if (!(np->msi_flags & NV_MSI_X_ENABLED))
				pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5286", NVREG_IRQSTAT_MASK, base + NvRegIrqStatus);
			else
				pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5288", NVREG_IRQSTAT_MASK, base + NvRegMSIXIrqStatus);
			/* stop engines */
			nv_stop_rxtx(dev);
			nv_txrx_reset(dev);
			/* drain rx queue */
		...
		if (netif_running(dev)) {
			...
			/* reinit nic view of the rx queue */
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5327", np->rx_buf_sz, base + NvRegOffloadConfig);
			setup_hw_rings(dev, NV_SETUP_RX_RING | NV_SETUP_TX_RING);
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5329", ((np->rx_ring_size-1) << NVREG_RINGSZ_RXSHIFT) + ((np->tx_ring_size-1) << NVREG_RINGSZ_TXSHIFT),
				base + NvRegRingSizes);
			pci_push(base);
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5332", NVREG_TXRXCTL_KICK|np->txrxctl_bits, get_hwbase(dev) + NvRegTxRxControl);
			pci_push(base);
			/* restart rx engine */
			nv_start_rxtx(dev);
...
static int nv_mgmt_acquire_sema(struct net_device *dev)
{
	...
	for (i = 0; i < 10; i++) {
		mgmt_sema = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5385", base + NvRegTransmitterControl) & NVREG_XMITCTL_MGMT_SEMA_MASK;
	...
	for (i = 0; i < 2; i++) {
		tx_ctrl = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5395", base + NvRegTransmitterControl);
		tx_ctrl |= NVREG_XMITCTL_HOST_SEMA_ACQ;
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5397", tx_ctrl, base + NvRegTransmitterControl);

		/* verify that semaphore was acquired */
		tx_ctrl = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5400", base + NvRegTransmitterControl);
...
static void nv_mgmt_release_sema(struct net_device *dev)
{
	...
	if (np->driver_data & DEV_HAS_MGMT_UNIT) {
		if (np->mgmt_sema) {
			tx_ctrl = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5420", base + NvRegTransmitterControl);
			tx_ctrl &= ~NVREG_XMITCTL_HOST_SEMA_ACQ;
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5422", tx_ctrl, base + NvRegTransmitterControl);
		}
	}
}
...
static int nv_mgmt_get_version(struct net_device *dev)
{
	struct fe_priv *np = netdev_priv(dev);
	u8 __iomem *base = get_hwbase(dev);
	u32 data_ready = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5432", base + NvRegTransmitterControl);
	u32 data_ready2 = 0;
	unsigned long start;
	int ready = 0;

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5437", NVREG_MGMTUNITGETVERSION, base + NvRegMgmtUnitGetVersion);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5438", data_ready ^ NVREG_XMITCTL_DATA_START, base + NvRegTransmitterControl);
	start = jiffies;
	while (time_before(jiffies, start + 5*HZ)) {
		data_ready2 = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5441", base + NvRegTransmitterControl);
	...
	np->mgmt_version = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5452", base + NvRegMgmtUnitVersion) & NVREG_MGMTUNITVERSION;
...
static int nv_open(struct net_device *dev)
{
	...
	if (np->driver_data & DEV_HAS_POWER_CNTRL)
		nv_mac_reset(dev);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5473", NVREG_MCASTADDRA_FORCE, base + NvRegMulticastAddrA);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5474", 0, base + NvRegMulticastAddrB);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5475", NVREG_MCASTMASKA_NONE, base + NvRegMulticastMaskA);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5476", NVREG_MCASTMASKB_NONE, base + NvRegMulticastMaskB);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5477", 0, base + NvRegPacketFilterFlags);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5479", 0, base + NvRegTransmitterControl);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5480", 0, base + NvRegReceiverControl);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5482", 0, base + NvRegAdapterControl);

	if (np->pause_flags & NV_PAUSEFRAME_TX_CAPABLE)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5485", NVREG_TX_PAUSEFRAME_DISABLE,  base + NvRegTxPauseFrame);

	/* initialize descriptor rings */
	set_bufsize(dev);
	oom = nv_init_ring(dev);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5491", 0, base + NvRegLinkSpeed);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5492", pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5492", base + NvRegTransmitPoll) & NVREG_TRANSMITPOLL_MAC_ADDR_REV, base + NvRegTransmitPoll);
	nv_txrx_reset(dev);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5494", 0, base + NvRegUnknownSetupReg6);

	np->in_shutdown = 0;

	/* give hw rings */
	setup_hw_rings(dev, NV_SETUP_RX_RING | NV_SETUP_TX_RING);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5500", ((np->rx_ring_size-1) << NVREG_RINGSZ_RXSHIFT) + ((np->tx_ring_size-1) << NVREG_RINGSZ_TXSHIFT),
		base + NvRegRingSizes);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5503", np->linkspeed, base + NvRegLinkSpeed);
	if (np->desc_ver == DESC_VER_1)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5505", NVREG_TX_WM_DESC1_DEFAULT, base + NvRegTxWatermark);
	else
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5507", NVREG_TX_WM_DESC2_3_DEFAULT, base + NvRegTxWatermark);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5508", np->txrxctl_bits, base + NvRegTxRxControl);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5509", np->vlanctl_bits, base + NvRegVlanControl);
	pci_push(base);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5511", NVREG_TXRXCTL_BIT1|np->txrxctl_bits, base + NvRegTxRxControl);
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5518", 0, base + NvRegMIIMask);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5519", NVREG_IRQSTAT_MASK, base + NvRegIrqStatus);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5520", NVREG_MIISTAT_MASK_ALL, base + NvRegMIIStatus);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5522", NVREG_MISC1_FORCE | NVREG_MISC1_HD, base + NvRegMisc1);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5523", pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5523", base + NvRegTransmitterStatus), base + NvRegTransmitterStatus);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5524", NVREG_PFF_ALWAYS, base + NvRegPacketFilterFlags);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5525", np->rx_buf_sz, base + NvRegOffloadConfig);

	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5527", pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5527", base + NvRegReceiverStatus), base + NvRegReceiverStatus);

	get_random_bytes(&low, sizeof(low));
	low &= NVREG_SLOTTIME_MASK;
	if (np->desc_ver == DESC_VER_1) {
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5532", low|NVREG_SLOTTIME_DEFAULT, base + NvRegSlotTime);
	} else {
		if (!(np->driver_data & DEV_HAS_GEAR_MODE)) {
			/* setup legacy backoff */
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5536", NVREG_SLOTTIME_LEGBF_ENABLED|NVREG_SLOTTIME_10_100_FULL|low, base + NvRegSlotTime);
		} else {
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5538", NVREG_SLOTTIME_10_100_FULL, base + NvRegSlotTime);
			nv_gear_backoff_reseed(dev);
		}
	}
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5542", NVREG_TX_DEFERRAL_DEFAULT, base + NvRegTxDeferral);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5543", NVREG_RX_DEFERRAL_DEFAULT, base + NvRegRxDeferral);
	if (poll_interval == -1) {
		if (optimization_mode == NV_OPTIMIZATION_MODE_THROUGHPUT)
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5546", NVREG_POLL_DEFAULT_THROUGHPUT, base + NvRegPollingInterval);
		else
			pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5548", NVREG_POLL_DEFAULT_CPU, base + NvRegPollingInterval);
	} else
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5550", poll_interval & 0xFFFF, base + NvRegPollingInterval);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5551", NVREG_UNKSETUP6_VAL, base + NvRegUnknownSetupReg6);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5552", (np->phyaddr << NVREG_ADAPTCTL_PHYSHIFT)|NVREG_ADAPTCTL_PHYVALID|NVREG_ADAPTCTL_RUNNING,
			base + NvRegAdapterControl);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5554", NVREG_MIISPEED_BIT8|NVREG_MIIDELAY, base + NvRegMIISpeed);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5555", NVREG_MII_LINKCHANGE, base + NvRegMIIMask);
	if (np->wolenabled)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5557", NVREG_WAKEUPFLAGS_ENABLE , base + NvRegWakeUpFlags);

	i = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5559", base + NvRegPowerState);
	if ((i & NVREG_POWERSTATE_POWEREDUP) == 0)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5561", NVREG_POWERSTATE_POWEREDUP|i, base + NvRegPowerState);

	pci_push(base);
	udelay(10);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5565", pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5565", base + NvRegPowerState) | NVREG_POWERSTATE_VALID, base + NvRegPowerState);

	nv_disable_hw_interrupts(dev, np->irqmask);
	pci_push(base);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5569", NVREG_MIISTAT_MASK_ALL, base + NvRegMIIStatus);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5570", NVREG_IRQSTAT_MASK, base + NvRegIrqStatus);
	pci_push(base);
	...
	spin_lock_irq(&np->lock);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5580", NVREG_MCASTADDRA_FORCE, base + NvRegMulticastAddrA);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5581", 0, base + NvRegMulticastAddrB);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5582", NVREG_MCASTMASKA_NONE, base + NvRegMulticastMaskA);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5583", NVREG_MCASTMASKB_NONE, base + NvRegMulticastMaskB);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5584", NVREG_PFF_ALWAYS|NVREG_PFF_MYADDR, base + NvRegPacketFilterFlags);
	/* One manual link speed update: Interrupts are enabled, future link
	 * speed changes cause interrupts and are handled by nv_link_irq().
	 */
	pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5588", base + NvRegMIIStatus);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5589", NVREG_MIISTAT_MASK_ALL, base + NvRegMIIStatus);
...
static int nv_close(struct net_device *dev)
{
	...
	if (np->wolenabled || !phy_power_down) {
		nv_txrx_gate(dev, false);
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5661", NVREG_PFF_ALWAYS|NVREG_PFF_MYADDR, base + NvRegPacketFilterFlags);
		nv_start_rx(dev);
	} else {
		/* power down phy */
		mii_rw(dev, np->phyaddr, MII_BMCR,
...
static int nv_probe(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	base = get_hwbase(dev);
	np->orig_mac[0] = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5887", base + NvRegMacAddrA);
	np->orig_mac[1] = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5888", base + NvRegMacAddrB);

	/* check the workaround bit for correct mac address order */
	txreg = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5891", base + NvRegTransmitPoll);
	if (id->driver_data & DEV_HAS_CORRECT_MACADDR) {
		...
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5924", txreg|NVREG_TRANSMITPOLL_MAC_ADDR_REV, base + NvRegTransmitPoll);
		dev_dbg(&pci_dev->dev,
	...
	/* disable WOL */
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5949", 0, base + NvRegWakeUpFlags);
	np->wolenabled = 0;
	...
	if (id->driver_data & DEV_HAS_POWER_CNTRL) {
		...
		powerstate = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:5956", base + NvRegPowerState2);
		powerstate &= ~NVREG_POWERSTATE2_POWERUP_MASK;
		if ((id->driver_data & DEV_NEED_LOW_POWER_FIX) &&
		    pci_dev->revision >= 0xA3)
			powerstate |= NVREG_POWERSTATE2_POWERUP_REV_A3;
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:5961", powerstate, base + NvRegPowerState2);
	}
	...
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6017", 0, base + NvRegMIIMask);
	phystate = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6018", base + NvRegAdapterControl);
	if (phystate & NVREG_ADAPTCTL_RUNNING) {
		phystate_orig = 1;
		phystate &= ~NVREG_ADAPTCTL_RUNNING;
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6022", phystate, base + NvRegAdapterControl);
	}
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6024", NVREG_MIISTAT_MASK_ALL, base + NvRegMIIStatus);

	if (id->driver_data & DEV_HAS_MGMT_UNIT) {
		/* management unit running on the mac? */
		if ((pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6028", base + NvRegTransmitterControl) & NVREG_XMITCTL_MGMT_ST) &&
		    (pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6029", base + NvRegTransmitterControl) & NVREG_XMITCTL_SYNC_PHY_INIT) &&
		    nv_mgmt_acquire_sema(dev) &&
		    nv_mgmt_get_version(dev)) {
			np->mac_in_use = 1;
			if (np->mgmt_version > 0)
				np->mac_in_use = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6034", base + NvRegMgmtUnitControl) & NVREG_MGMTUNITCONTROL_INUSE;
			/* management unit setup the phy already? */
			if (np->mac_in_use &&
			    ((pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6037", base + NvRegTransmitterControl) & NVREG_XMITCTL_SYNC_MASK) ==
...
out_error:
	...
	if (phystate_orig)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6143", phystate|NVREG_ADAPTCTL_RUNNING, base + NvRegAdapterControl);
out_freering:
	free_rings(dev);
out_unmap:
...
static void nv_restore_mac_addr(struct pci_dev *pci_dev)
{
	 ...
	 */
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6191", np->orig_mac[0], base + NvRegMacAddrA);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6192", np->orig_mac[1], base + NvRegMacAddrB);
	pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6193", pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6193", base + NvRegTransmitPoll) & ~NVREG_TRANSMITPOLL_MAC_ADDR_REV,
	       base + NvRegTransmitPoll);
}
...
static int nv_suspend(struct device *device)
{
	...
	for (i = 0; i <= np->register_size/sizeof(u32); i++)
		np->saved_config_space[i] = pete_readl("drivers/net/ethernet/nvidia/forcedeth.c:6237", base + i*sizeof(u32));
...
static int nv_resume(struct device *device)
{
	...
	for (i = 0; i <= np->register_size/sizeof(u32); i++)
		pete_writel("drivers/net/ethernet/nvidia/forcedeth.c:6252", np->saved_config_space[i], base+i*sizeof(u32));
