...
static int amd8111e_set_coalesce(struct net_device *dev, enum coal_mode cmod)
{
	...
	switch(cmod)
	{
		case RX_INTR_COAL :
			...
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:382", VAL0|STINTEN, mmio+INTEN0);
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:383", (u32)DLY_INT_A_R0 | (event_count << 16) |
		...
		case TX_INTR_COAL:
			...
			timeout = timeout * DELAY_TIMER_CONV;
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:396", VAL0 | STINTEN, mmio + INTEN0);
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:397", (u32)DLY_INT_B_T0 | (event_count << 16) |
				timeout, mmio + DLY_INT_B);
			break;

		case DISABLE_COAL:
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:402", 0, mmio + STVAL);
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:403", STINTEN, mmio + INTEN0);
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:404", 0, mmio + DLY_INT_B);
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:405", 0, mmio + DLY_INT_A);
			break;
		 case ENABLE_COAL:
		       /* Start the timer */
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:409", (u32)SOFT_TIMER_FREQ, mmio + STVAL); /* 0.5 sec */
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:410", VAL0 | STINTEN, mmio + INTEN0);
			break;
		default:
			break;
...
static int amd8111e_restart(struct net_device *dev)
{
	...
	/* stop the chip */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:428", RUN, mmio + CMD0);

	if (amd8111e_init_ring(dev))
		return -ENOMEM;

	/* enable the port manager and set auto negotiation always */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:434", (u32)VAL1 | EN_PMGR, mmio + CMD3);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:435", (u32)XPHYANE | XPHYRST, mmio + CTRL2);

	amd8111e_set_ext_phy(dev);

	/* set control registers */
	reg_val = pete_readl("drivers/net/ethernet/amd/amd8111e.c:440", mmio + CTRL1);
	reg_val &= ~XMTSP_MASK;
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:442", reg_val | XMTSP_128 | CACHE_ALIGN, mmio + CTRL1);

	/* enable interrupt */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:445", APINT5EN | APINT4EN | APINT3EN | APINT2EN | APINT1EN |
		APINT0EN | MIIPDTINTEN | MCCIINTEN | MCCINTEN | MREINTEN |
		SPNDINTEN | MPINTEN | SINTEN | STINTEN, mmio + INTEN0);

	pete_writel("drivers/net/ethernet/amd/amd8111e.c:449", VAL3 | LCINTEN | VAL1 | TINTEN0 | VAL0 | RINTEN0, mmio + INTEN0);

	/* initialize tx and rx ring base addresses */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:452", (u32)lp->tx_ring_dma_addr, mmio + XMT_RING_BASE_ADDR0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:453", (u32)lp->rx_ring_dma_addr, mmio + RCV_RING_BASE_ADDR0);

	pete_writew("drivers/net/ethernet/amd/amd8111e.c:455", (u32)NUM_TX_RING_DR, mmio + XMT_RING_LEN0);
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:456", (u16)NUM_RX_RING_DR, mmio + RCV_RING_LEN0);

	/* set default IPG to 96 */
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:459", (u32)DEFAULT_IPG, mmio + IPG);
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:460", (u32)(DEFAULT_IPG-IFS1_DELTA), mmio + IFS1);

	if (lp->options & OPTION_JUMBO_ENABLE) {
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:463", (u32)VAL2|JUMBO, mmio + CMD3);
		/* Reset REX_UFLO */
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:465", REX_UFLO, mmio + CMD2);
		/* Should not set REX_UFLO for jumbo frames */
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:467", VAL0 | APAD_XMT | REX_RTRY, mmio + CMD2);
	} else {
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:469", VAL0 | APAD_XMT | REX_RTRY | REX_UFLO, mmio + CMD2);
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:470", (u32)JUMBO, mmio + CMD3);
	}

#if AMD8111E_VLAN_TAG_USED
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:474", (u32)VAL2 | VSIZE | VL_TAG_DEL, mmio + CMD3);
#endif
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:476", VAL0 | APAD_XMT | REX_RTRY, mmio + CMD2);

	/* Setting the MAC address to the device */
	for (i = 0; i < ETH_ALEN; i++)
		pete_writeb("drivers/net/ethernet/amd/amd8111e.c:480", dev->dev_addr[i], mmio + PADR + i);
	...
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:489", VAL2 | RDMD0, mmio + CMD0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:490", VAL0 | INTREN | RUN, mmio + CMD0);

	/* To avoid PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:493", mmio+CMD0);
	return 0;
}
...
static void amd8111e_init_hw_default(struct amd8111e_priv *lp)
{
	...
	/* stop the chip */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:506", RUN, mmio + CMD0);

	/* AUTOPOLL0 Register *//*TBD default value is 8100 in FPS */
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:509",  0x8100 | lp->ext_phy_addr, mmio + AUTOPOLL0);

	/* Clear RCV_RING_BASE_ADDR */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:512", 0, mmio + RCV_RING_BASE_ADDR0);

	/* Clear XMT_RING_BASE_ADDR */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:515", 0, mmio + XMT_RING_BASE_ADDR0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:516", 0, mmio + XMT_RING_BASE_ADDR1);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:517", 0, mmio + XMT_RING_BASE_ADDR2);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:518", 0, mmio + XMT_RING_BASE_ADDR3);

	/* Clear CMD0  */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:521", CMD0_CLEAR, mmio + CMD0);

	/* Clear CMD2 */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:524", CMD2_CLEAR, mmio + CMD2);

	/* Clear CMD7 */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:527", CMD7_CLEAR, mmio + CMD7);

	/* Clear DLY_INT_A and DLY_INT_B */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:530", 0x0, mmio + DLY_INT_A);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:531", 0x0, mmio + DLY_INT_B);

	/* Clear FLOW_CONTROL */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:534", 0x0, mmio + FLOW_CONTROL);

	/* Clear INT0  write 1 to clear register */
	reg_val = pete_readl("drivers/net/ethernet/amd/amd8111e.c:537", mmio + INT0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:538", reg_val, mmio + INT0);

	/* Clear STVAL */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:541", 0x0, mmio + STVAL);

	/* Clear INTEN0 */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:544", INTEN0_CLEAR, mmio + INTEN0);

	/* Clear LADRF */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:547", 0x0, mmio + LADRF);

	/* Set SRAM_SIZE & SRAM_BOUNDARY registers  */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:550", 0x80010, mmio + SRAM_SIZE);

	/* Clear RCV_RING0_LEN */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:553", 0x0, mmio + RCV_RING_LEN0);

	/* Clear XMT_RING0/1/2/3_LEN */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:556", 0x0, mmio +  XMT_RING_LEN0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:557", 0x0, mmio +  XMT_RING_LEN1);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:558", 0x0, mmio +  XMT_RING_LEN2);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:559", 0x0, mmio +  XMT_RING_LEN3);

	/* Clear XMT_RING_LIMIT */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:562", 0x0, mmio + XMT_RING_LIMIT);

	/* Clear MIB */
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:565", MIB_CLEAR, mmio + MIB_ADDR);

	/* Clear LARF */
	amd8111e_writeq(*(u64 *)logic_filter, mmio + LADRF);

	/* SRAM_SIZE register */
	reg_val = pete_readl("drivers/net/ethernet/amd/amd8111e.c:571", mmio + SRAM_SIZE);

	if (lp->options & OPTION_JUMBO_ENABLE)
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:574", VAL2 | JUMBO, mmio + CMD3);
#if AMD8111E_VLAN_TAG_USED
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:576", VAL2 | VSIZE | VL_TAG_DEL, mmio + CMD3);
#endif
	/* Set default value to CTRL1 Register */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:579", CTRL1_DEFAULT, mmio + CTRL1);

	/* To avoid PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:582", mmio + CMD2);
...
static void amd8111e_disable_interrupt(struct amd8111e_priv *lp)
{
	...
	/* Disable interrupt */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:594", INTREN, lp->mmio + CMD0);

	/* Clear INT0 */
	intr0 = pete_readl("drivers/net/ethernet/amd/amd8111e.c:597", lp->mmio + INT0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:598", intr0, lp->mmio + INT0);

	/* To avoid PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:601", lp->mmio + INT0);
...
{
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:608", RUN, lp->mmio + CMD0);

	/* To avoid PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:611", lp->mmio + CMD0);
}
...
err_next_pkt:
	...
	if (num_rx_pkt < budget && napi_complete_done(napi, num_rx_pkt)) {
		...
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:783", VAL0|RINTEN0, mmio + INTEN0);
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:784", VAL2 | RDMD0, mmio + CMD0);
...
static int amd8111e_link_change(struct net_device *dev)
{
	...
	status0 = pete_readl("drivers/net/ethernet/amd/amd8111e.c:798", lp->mmio + STAT0);
...
static int amd8111e_read_mib(void __iomem *mmio, u8 MIB_COUNTER)
{
	...
	pete_writew("drivers/net/ethernet/amd/amd8111e.c:841", MIB_RD_CMD | MIB_COUNTER, mmio + MIB_ADDR);
	do {
		status = pete_readw("drivers/net/ethernet/amd/amd8111e.c:843", mmio + MIB_ADDR);
		udelay(2);	/* controller takes MAX 2 us to get mib data */
	}
	while (--repeat && (status & MIB_CMD_ACTIVE));

	data = pete_readl("drivers/net/ethernet/amd/amd8111e.c:848", mmio + MIB_DATA);
	return data;
}
...
static struct net_device_stats *amd8111e_get_stats(struct net_device *dev)
{
	...
	/* pete_writew("drivers/net/ethernet/amd/amd8111e.c:942", MIB_CLEAR, mmio + MIB_ADDR);*/
...
static irqreturn_t amd8111e_interrupt(int irq, void *dev_id)
{
	...
	/* disabling interrupt */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1092", INTREN, mmio + CMD0);

	/* Read interrupt status */
	intr0 = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1095", mmio + INT0);
	intren0 = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1096", mmio + INTEN0);
	...
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1106", intr0, mmio + INT0);

	/* Check if Receive Interrupt has occurred. */
	if (intr0 & RINT0) {
		if (napi_schedule_prep(&lp->napi)) {
			/* Disable receive interrupts */
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:1112", RINTEN0, mmio + INTEN0);
			/* Schedule a polling routine */
			__napi_schedule(&lp->napi);
		} else if (intren0 & RINTEN0) {
			netdev_dbg(dev, "************Driver bug! interrupt while in poll\n");
			/* Fix by disable receive interrupts */
			pete_writel("drivers/net/ethernet/amd/amd8111e.c:1118", RINTEN0, mmio + INTEN0);
		}
	}
...
err_no_interrupt:
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1135", VAL0 | INTREN, mmio + CMD0);
				       ...
				       struct net_device *dev)
{
	...
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1284", VAL1 | TDMD0, lp->mmio + CMD0);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1285", VAL2 | RDMD0, lp->mmio + CMD0);
...
static void amd8111e_read_regs(struct amd8111e_priv *lp, u32 *buf)
{
	...
	buf[0] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1298", mmio + XMT_RING_BASE_ADDR0);
	buf[1] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1299", mmio + XMT_RING_LEN0);
	buf[2] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1300", mmio + RCV_RING_BASE_ADDR0);
	buf[3] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1301", mmio + RCV_RING_LEN0);
	buf[4] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1302", mmio + CMD0);
	buf[5] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1303", mmio + CMD2);
	buf[6] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1304", mmio + CMD3);
	buf[7] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1305", mmio + CMD7);
	buf[8] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1306", mmio + INT0);
	buf[9] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1307", mmio + INTEN0);
	buf[10] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1308", mmio + LADRF);
	buf[11] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1309", mmio + LADRF+4);
	buf[12] = pete_readl("drivers/net/ethernet/amd/amd8111e.c:1310", mmio + STAT0);
}
...
static void amd8111e_set_multicast_list(struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:1325", VAL2 | PROM, lp->mmio + CMD2);
		return;
	}
	else
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:1329", PROM, lp->mmio + CMD2);
	...
	if (netdev_mc_empty(dev)) {
		...
		pete_writel("drivers/net/ethernet/amd/amd8111e.c:1344", PROM, lp->mmio + CMD2);
		return;
	}
	...
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:1357", lp->mmio + CMD2);
...
static int amd8111e_set_mac_address(struct net_device *dev, void *p)
{
	...
	/* Setting the MAC address to the device */
	for (i = 0; i < ETH_ALEN; i++)
		pete_writeb("drivers/net/ethernet/amd/amd8111e.c:1506", dev->dev_addr[i], lp->mmio + PADR + i);
...
static int amd8111e_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	/* stop the chip */
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1532", RUN, lp->mmio + CMD0);
...
{
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1545", VAL1 | MPPLBA, lp->mmio + CMD3);
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1546", VAL0 | MPEN_SW, lp->mmio + CMD7);

	/* To eliminate PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:1549", lp->mmio + CMD7);
	return 0;
}
...
static int amd8111e_enable_link_change(struct amd8111e_priv *lp)
{
	...
	pete_writel("drivers/net/ethernet/amd/amd8111e.c:1557", VAL0 | LCMODE_SW, lp->mmio + CMD7);

	/* To eliminate PCI posting bug */
	pete_readl("drivers/net/ethernet/amd/amd8111e.c:1560", lp->mmio + CMD7);
	return 0;
}
...
static void amd8111e_config_ipg(struct timer_list *t)
{
	...
	if (ipg_data->ipg_state == CSTATE) {
		...
		pete_writew("drivers/net/ethernet/amd/amd8111e.c:1694", (u32)tmp_ipg, mmio + IPG);
		pete_writew("drivers/net/ethernet/amd/amd8111e.c:1695", (u32)(tmp_ipg - IFS1_DELTA), mmio + IFS1);
	}
				  ...
				  const struct pci_device_id *ent)
{
	...
	for (i = 0; i < ETH_ALEN; i++)
		addr[i] = pete_readb("drivers/net/ethernet/amd/amd8111e.c:1812", lp->mmio + PADR + i);
	...
	chip_version = (pete_readl("drivers/net/ethernet/amd/amd8111e.c:1863", lp->mmio + CHIPID) & 0xf0000000) >> 28;
