			     ...
			     const struct pci_device_id *ent)
{
	...
	for (i = 0; i < 6; i++)
		addr[i] = pete_readb("drivers/net/ethernet/adaptec/starfire.c:700", base + EEPROMCtrl + 20 - i);
	...
	if (debug > 4)
		for (i = 0; i < 0x20; i++)
			printk("%2.2x%s",
			       (unsigned int)pete_readb("drivers/net/ethernet/adaptec/starfire.c:707", base + EEPROMCtrl + i),
			       i % 16 != 15 ? " " : "\n");
#endif

	/* Issue soft reset */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:712", MiiSoftReset, base + TxMode);
	udelay(1000);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:714", 0, base + TxMode);

	/* Reset the chip to erase previous misconfiguration. */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:717", 1, base + PCIDeviceConfig);
	boguscnt = 1000;
	while (--boguscnt > 0) {
		udelay(10);
		if ((pete_readl("drivers/net/ethernet/adaptec/starfire.c:721", base + PCIDeviceConfig) & 1) == 0)
			break;
	}
	if (boguscnt == 0)
...
static int mdio_read(struct net_device *dev, int phy_id, int location)
{
	...
	do {
		result = pete_readl("drivers/net/ethernet/adaptec/starfire.c:841", mdio_addr);
...
static void mdio_write(struct net_device *dev, int phy_id, int location, int value)
{
	struct netdev_private *np = netdev_priv(dev);
	void __iomem *mdio_addr = np->base + MIICtrl + (phy_id<<7) + (location<<2);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:855", value, mdio_addr);
...
static int netdev_open(struct net_device *dev)
{
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:878", 0, ioaddr + GenCtrl);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:879", 1, ioaddr + PCIDeviceConfig);
	if (debug > 1)
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:913", (np->rx_buf_sz << RxBufferLenShift) |
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:922", RxChecksumIgnore |
	...
	/* Set Tx descriptor */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:929", (2 << TxHiPriFIFOThreshShift) |
	       (0 << TxPadLenShift) |
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:936",  (np->queue_mem_dma >> 16) >> 16, ioaddr + RxDescQHiAddr);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:937",  (np->queue_mem_dma >> 16) >> 16, ioaddr + TxRingHiAddr);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:938",  (np->queue_mem_dma >> 16) >> 16, ioaddr + CompletionHiAddr);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:939", np->rx_ring_dma, ioaddr + RxDescQAddr);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:940", np->tx_ring_dma, ioaddr + TxRingPtr);

	pete_writel("drivers/net/ethernet/adaptec/starfire.c:942", np->tx_done_q_dma, ioaddr + TxCompletionAddr);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:943", np->rx_done_q_dma |
	       RxComplType |
	...
	for (i = 0; i < 6; i++)
		pete_writeb("drivers/net/ethernet/adaptec/starfire.c:953", dev->dev_addr[i], ioaddr + TxStationAddr + 5 - i);
	/* The first entry is special because it bypasses the VLAN filter.
	   Don't use it. */
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:956", 0, ioaddr + PerfFilterTable);
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:957", 0, ioaddr + PerfFilterTable + 4);
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:958", 0, ioaddr + PerfFilterTable + 8);
	for (i = 1; i < 16; i++) {
		const __be16 *eaddrs = (const __be16 *)dev->dev_addr;
		void __iomem *setup_frm = ioaddr + PerfFilterTable + i * 16;
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:962", be16_to_cpu(eaddrs[2]), setup_frm); setup_frm += 4;
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:963", be16_to_cpu(eaddrs[1]), setup_frm); setup_frm += 4;
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:964", be16_to_cpu(eaddrs[0]), setup_frm); setup_frm += 8;
	}

	/* Initialize other registers. */
	/* Configure the PCI bus bursts and FIFO thresholds. */
	np->tx_mode = TxFlowEnable|RxFlowEnable|PadEnable;	/* modified when link is up. */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:970", MiiSoftReset | np->tx_mode, ioaddr + TxMode);
	udelay(1000);
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:972", np->tx_mode, ioaddr + TxMode);
	np->tx_threshold = 4;
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:974", np->tx_threshold, ioaddr + TxThreshold);

	pete_writel("drivers/net/ethernet/adaptec/starfire.c:976", np->intr_timer_ctrl, ioaddr + IntrTimerCtrl);
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:990", 0x0f00ff00, ioaddr + GPIOCtrl);

	/* Set the interrupt mask */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:993", IntrRxDone | IntrRxEmpty | IntrDMAErr |
	       IntrTxDMADone | IntrStatsMax | IntrLinkChange |
	       IntrRxGFPDead | IntrNoTxCsum | IntrTxBadID,
	       ioaddr + IntrEnable);
	/* Enable PCI interrupts. */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:998", 0x00800000 | pete_readl("drivers/net/ethernet/adaptec/starfire.c:998", ioaddr + PCIDeviceConfig),
	       ioaddr + PCIDeviceConfig);

#ifdef VLAN_SUPPORT
	/* Set VLAN type to 802.1q */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1003", ETH_P_8021Q, ioaddr + VlanType);
#endif /* VLAN_SUPPORT */
	...
	for (i = 0; i < rx_size; i++)
		pete_writel("drivers/net/ethernet/adaptec/starfire.c:1037", be32_to_cpup(&fw_rx_data[i]), ioaddr + RxGfpMem + i * 4);
	for (i = 0; i < tx_size; i++)
		pete_writel("drivers/net/ethernet/adaptec/starfire.c:1039", be32_to_cpup(&fw_tx_data[i]), ioaddr + TxGfpMem + i * 4);
	if (enable_hw_cksum)
		/* Enable the Rx and Tx units, and the Rx/Tx frame processors. */
		pete_writel("drivers/net/ethernet/adaptec/starfire.c:1042", TxEnable|TxGFPEnable|RxEnable|RxGFPEnable, ioaddr + GenCtrl);
	else
		/* Enable the Rx and Tx units only. */
		pete_writel("drivers/net/ethernet/adaptec/starfire.c:1045", TxEnable|RxEnable, ioaddr + GenCtrl);
...
static void tx_timeout(struct net_device *dev, unsigned int txqueue)
{
	...
	printk(KERN_WARNING "%s: Transmit timed out, status %#8.8x, "
	       "resetting...\n", dev->name, (int) pete_readl("drivers/net/ethernet/adaptec/starfire.c:1104", ioaddr + IntrStatus));
...
static void init_ring(struct net_device *dev)
{
	...
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:1155", i - 1, np->base + RxDescQIdx);
...
static netdev_tx_t start_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1269", entry * (sizeof(starfire_tx_desc) / 8), np->base + TxProducerIdx);
...
static irqreturn_t intr_handler(int irq, void *dev_instance)
{
	...
	do {
		u32 intr_status = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1312", ioaddr + IntrClear);
		...
		if (intr_status & (IntrRxDone | IntrRxEmpty)) {
			...
			if (likely(napi_schedule_prep(&np->napi))) {
				...
				enable = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1328", ioaddr + IntrEnable);
				enable &= ~(IntrRxDone | IntrRxEmpty);
				pete_writel("drivers/net/ethernet/adaptec/starfire.c:1330", enable, ioaddr + IntrEnable);
				/* flush PCI posting buffers */
				pete_readl("drivers/net/ethernet/adaptec/starfire.c:1332", ioaddr + IntrEnable);
			} else {
				/* Paranoia check */
				enable = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1335", ioaddr + IntrEnable);
				if (enable & (IntrRxDone | IntrRxEmpty)) {
					printk(KERN_INFO
					       "%s: interrupt while in poll!\n",
					       dev->name);
					enable &= ~(IntrRxDone | IntrRxEmpty);
					pete_writel("drivers/net/ethernet/adaptec/starfire.c:1341", enable, ioaddr + IntrEnable);
				}
			}
		}
		...
		consumer = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1349", ioaddr + TxConsumerIdx);
		if (debug > 3)
		...
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:1388", np->tx_done, ioaddr + CompletionQConsumerIdx + 2);
	...
	if (debug > 4)
		printk(KERN_DEBUG "%s: exiting interrupt, status=%#8.8x.\n",
		       dev->name, (int) pete_readl("drivers/net/ethernet/adaptec/starfire.c:1419", ioaddr + IntrStatus));
...
static int __netdev_rx(struct net_device *dev, int *quota)
{
	...
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:1542", np->rx_done, np->base + CompletionQConsumerIdx);
...
static int netdev_poll(struct napi_struct *napi, int budget)
{
	...
	do {
		pete_writel("drivers/net/ethernet/adaptec/starfire.c:1561", IntrRxDone | IntrRxEmpty, ioaddr + IntrClear);

		if (__netdev_rx(dev, &quota))
			goto out;

		intr_status = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1566", ioaddr + IntrStatus);
	} while (intr_status & (IntrRxDone | IntrRxEmpty));

	napi_complete(napi);
	intr_status = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1570", ioaddr + IntrEnable);
	intr_status |= IntrRxDone | IntrRxEmpty;
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1572", intr_status, ioaddr + IntrEnable);
...
static void refill_rx_ring(struct net_device *dev)
{
	...
	if (entry >= 0)
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:1612", entry, np->base + RxDescQIdx);
}
...
static void netdev_media_change(struct net_device *dev)
{
	...
	if (reg1 & BMSR_LSTATUS) {
		...
		if (np->tx_mode != new_tx_mode) {
			...
			pete_writel("drivers/net/ethernet/adaptec/starfire.c:1672", np->tx_mode | MiiSoftReset, ioaddr + TxMode);
			udelay(1000);
			pete_writel("drivers/net/ethernet/adaptec/starfire.c:1674", np->tx_mode, ioaddr + TxMode);
		}
		...
		if (np->intr_timer_ctrl != new_intr_timer_ctrl) {
			...
			pete_writel("drivers/net/ethernet/adaptec/starfire.c:1682", new_intr_timer_ctrl, ioaddr + IntrTimerCtrl);
		}
	} else {
		netif_carrier_off(dev);
...
static void netdev_error(struct net_device *dev, int intr_status)
{
	...
	if (intr_status & IntrTxDataLow) {
		if (np->tx_threshold <= PKT_BUF_SZ / 16) {
			pete_writel("drivers/net/ethernet/adaptec/starfire.c:1698", ++np->tx_threshold, np->base + TxThreshold);
...
static struct net_device_stats *get_stats(struct net_device *dev)
{
	...
	dev->stats.tx_bytes = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1724", ioaddr + 0x57010);
	dev->stats.rx_bytes = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1725", ioaddr + 0x57044);
	dev->stats.tx_packets = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1726", ioaddr + 0x57000);
	dev->stats.tx_aborted_errors =
		pete_readl("drivers/net/ethernet/adaptec/starfire.c:1728", ioaddr + 0x57024) + pete_readl("drivers/net/ethernet/adaptec/starfire.c:1728", ioaddr + 0x57028);
	dev->stats.tx_window_errors = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1729", ioaddr + 0x57018);
	dev->stats.collisions =
		pete_readl("drivers/net/ethernet/adaptec/starfire.c:1731", ioaddr + 0x57004) + pete_readl("drivers/net/ethernet/adaptec/starfire.c:1731", ioaddr + 0x57008);

	/* The chip only need report frame silently dropped. */
	dev->stats.rx_dropped += pete_readw("drivers/net/ethernet/adaptec/starfire.c:1734", ioaddr + RxDMAStatus);
	pete_writew("drivers/net/ethernet/adaptec/starfire.c:1735", 0, ioaddr + RxDMAStatus);
	dev->stats.rx_crc_errors = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1736", ioaddr + 0x5703C);
	dev->stats.rx_frame_errors = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1737", ioaddr + 0x57040);
	dev->stats.rx_length_errors = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1738", ioaddr + 0x57058);
	dev->stats.rx_missed_errors = pete_readl("drivers/net/ethernet/adaptec/starfire.c:1739", ioaddr + 0x5707C);
...
static u32 set_vlan_mode(struct netdev_private *np)
{
	...
	for_each_set_bit(vid, np->active_vlans, VLAN_N_VID) {
		if (vlan_count == 32)
			break;
		pete_writew("drivers/net/ethernet/adaptec/starfire.c:1755", vid, filter_addr);
		filter_addr += 16;
		vlan_count++;
	}
	if (vlan_count == 32) {
		...
		while (vlan_count < 32) {
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1762", 0, filter_addr);
			filter_addr += 16;
			vlan_count++;
		}
	}
...
static void set_rx_mode(struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {	/* Set promiscuous. */
		...
		netdev_for_each_mc_addr(ha, dev) {
			...
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1795", be16_to_cpu(eaddrs[2]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1796", be16_to_cpu(eaddrs[1]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1797", be16_to_cpu(eaddrs[0]), filter_addr); filter_addr += 8;
		}
		eaddrs = (const __be16 *)dev->dev_addr;
		i = netdev_mc_count(dev) + 2;
		while (i++ < 16) {
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1802", be16_to_cpu(eaddrs[0]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1803", be16_to_cpu(eaddrs[1]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1804", be16_to_cpu(eaddrs[2]), filter_addr); filter_addr += 8;
		}
		...
		eaddrs = (const __be16 *)dev->dev_addr;
		for (i = 2; i < 16; i++) {
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1826", be16_to_cpu(eaddrs[0]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1827", be16_to_cpu(eaddrs[1]), filter_addr); filter_addr += 4;
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1828", be16_to_cpu(eaddrs[2]), filter_addr); filter_addr += 8;
		}
		for (filter_addr = ioaddr + HashTable, i = 0; i < 32; filter_addr+= 16, i++)
			pete_writew("drivers/net/ethernet/adaptec/starfire.c:1831", mc_filter[i], filter_addr);
		rx_mode |= AcceptBroadcast|PerfectFilter|HashFilter;
	}
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1834", rx_mode, ioaddr + RxFilterMode);
}
...
static int netdev_close(struct net_device *dev)
{
	...
	if (debug > 1) {
		printk(KERN_DEBUG "%s: Shutting down ethercard, Intr status %#8.8x.\n",
			   dev->name, (int) pete_readl("drivers/net/ethernet/adaptec/starfire.c:1937", ioaddr + IntrStatus));
	...
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1944", 0, ioaddr + IntrEnable);

	/* Stop the chip's Tx and Rx processes. */
	pete_writel("drivers/net/ethernet/adaptec/starfire.c:1947", 0, ioaddr + GenCtrl);
	pete_readl("drivers/net/ethernet/adaptec/starfire.c:1948", ioaddr + GenCtrl);
