...
static int xgmac_probe(struct platform_device *pdev)
{
	...
	uid = pete_readl("drivers/net/ethernet/calxeda/xgmac.c:1732", priv->base + XGMAC_VERSION);
	netdev_info(ndev, "h/w version is 0x%x\n", uid);

	/* Figure out how many valid mac address filter registers we have */
	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1736", 1, priv->base + XGMAC_ADDR_HIGH(31));
	if (pete_readl("drivers/net/ethernet/calxeda/xgmac.c:1737", priv->base + XGMAC_ADDR_HIGH(31)) == 1)
		priv->max_macs = 31;
	else
		priv->max_macs = 7;

	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1742", 0, priv->base + XGMAC_DMA_INTR_ENA);
	...
	if (pete_readl("drivers/net/ethernet/calxeda/xgmac.c:1778", priv->base + XGMAC_DMA_HW_FEATURE) & DMA_HW_FEAT_TXCOESEL)
...
static void xgmac_pmt(void __iomem *ioaddr, unsigned long mode)
{
	...
	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1857", pmt, ioaddr + XGMAC_PMT);
}
...
static int xgmac_suspend(struct device *dev)
{
	...
	napi_disable(&priv->napi);
	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1871", 0, priv->base + XGMAC_DMA_INTR_ENA);

	if (device_may_wakeup(priv->device)) {
		/* Stop TX/RX DMA Only */
		value = pete_readl("drivers/net/ethernet/calxeda/xgmac.c:1875", priv->base + XGMAC_DMA_CONTROL);
		value &= ~(DMA_CONTROL_ST | DMA_CONTROL_SR);
		pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1877", value, priv->base + XGMAC_DMA_CONTROL);
...
static int xgmac_resume(struct device *dev)
{
	...
	xgmac_mac_enable(ioaddr);
	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1899", DMA_INTR_DEFAULT_MASK, ioaddr + XGMAC_DMA_STATUS);
	pete_writel("drivers/net/ethernet/calxeda/xgmac.c:1900", DMA_INTR_DEFAULT_MASK, ioaddr + XGMAC_DMA_INTR_ENA);
