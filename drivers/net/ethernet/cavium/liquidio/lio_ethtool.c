...
static int lio_reset_queues(struct net_device *netdev, uint32_t num_qs)
{
	...
	if (queue_count_update) {
		...
		for (i = 0; i < oct->num_oqs; i++)
			pete_writel("drivers/net/ethernet/cavium/liquidio/lio_ethtool.c:1216", oct->droq[i]->max_count,
		   ...
		   struct ethtool_coalesce *intr_coal)
{
	...
	case OCTEON_CN23XX_PF_VID: {
		...
		for (q_no = 0; q_no < oct->num_iqs; q_no++) {
			...
			val = pete_readq("drivers/net/ethernet/cavium/liquidio/lio_ethtool.c:2403", inst_cnt_reg);
			/*clear wmark and count.dont want to write count back*/
			val = (val & 0xFFFF000000000000ULL) |
			      ((u64)(iq_intr_pkt - 1)
			       << CN23XX_PKT_IN_DONE_WMARK_BIT_POS);
			pete_writeq("drivers/net/ethernet/cavium/liquidio/lio_ethtool.c:2408", val, inst_cnt_reg);
			/*consider setting resend bit*/
		}
