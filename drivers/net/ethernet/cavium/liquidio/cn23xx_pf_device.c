...
static int cn23xx_setup_pf_mbox(struct octeon_device *oct)
{
	...
	for (i = 0; i < oct->sriov_info.max_vfs; i++) {
		...
		pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:753", OCTEON_PFVFSIG, mbox->mbox_read_reg);
	}
...
static u64 cn23xx_pf_msix_interrupt_handler(void *dev)
{
	...
	pkts_sent = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:956", droq->pkts_sent_reg);
...
static void cn23xx_handle_pf_mbox_intr(struct octeon_device *oct)
{
	...
	mbox_int_val = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:988", oct->mbox[0]->mbox_int_reg);

	for (i = 0; i < oct->sriov_info.num_vfs_alloced; i++) {
		q_no = i * oct->sriov_info.rings_per_vf;

		if (mbox_int_val & BIT_ULL(q_no)) {
			pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:994", BIT_ULL(q_no),
...
static irqreturn_t cn23xx_interrupt_handler(void *dev)
{
	...
	dev_dbg(&oct->pci_dev->dev, "In %s octeon_dev @ %p\n", __func__, oct);
	intr64 = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1012", cn23xx->intr_sum_reg64);
	...
	pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1035", intr64, cn23xx->intr_sum_reg64);
...
static u32 cn23xx_update_read_index(struct octeon_instr_queue *iq)
{
	u32 new_idx;
	u32 last_done;
	u32 pkt_in_done = pete_readl("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1085", iq->inst_cnt_reg);
...
static void cn23xx_enable_pf_interrupt(struct octeon_device *oct, u8 intr_flag)
{
	...
	if (intr_flag == OCTEON_ALL_INTR) {
		pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1109", cn23xx->intr_mask64, cn23xx->intr_enb_reg64);
	} else if (intr_flag & OCTEON_OUTPUT_INTR) {
		intr_val = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1111", cn23xx->intr_enb_reg64);
		intr_val |= CN23XX_INTR_PKT_DATA;
		pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1113", intr_val, cn23xx->intr_enb_reg64);
	} else if ((intr_flag & OCTEON_MBOX_INTR) &&
		   (oct->sriov_info.max_vfs > 0)) {
		if (oct->rev_id >= OCTEON_CN23XX_REV_1_1) {
			intr_val = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1117", cn23xx->intr_enb_reg64);
			intr_val |= CN23XX_INTR_VF_MBOX;
			pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1119", intr_val, cn23xx->intr_enb_reg64);
		}
	}
}
...
static void cn23xx_disable_pf_interrupt(struct octeon_device *oct, u8 intr_flag)
{
	...
	/* Disable Interrupts */
	if (intr_flag == OCTEON_ALL_INTR) {
		pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1131", 0, cn23xx->intr_enb_reg64);
	} else if (intr_flag & OCTEON_OUTPUT_INTR) {
		intr_val = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1133", cn23xx->intr_enb_reg64);
		intr_val &= ~CN23XX_INTR_PKT_DATA;
		pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1135", intr_val, cn23xx->intr_enb_reg64);
	} else if ((intr_flag & OCTEON_MBOX_INTR) &&
		   (oct->sriov_info.max_vfs > 0)) {
		if (oct->rev_id >= OCTEON_CN23XX_REV_1_1) {
			intr_val = pete_readq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1139", cn23xx->intr_enb_reg64);
			intr_val &= ~CN23XX_INTR_VF_MBOX;
			pete_writeq("drivers/net/ethernet/cavium/liquidio/cn23xx_pf_device.c:1141", intr_val, cn23xx->intr_enb_reg64);
		}
	}
}
