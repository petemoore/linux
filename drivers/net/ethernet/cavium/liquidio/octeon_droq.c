...
int octeon_retry_droq_refill(struct octeon_droq *droq)
{
	...
	pkts_credit = pete_readl("drivers/net/ethernet/cavium/liquidio/octeon_droq.c:506", droq->pkts_credit_reg);
	...
	if (desc_refilled) {
		 ...
		 */
		wmb();
		pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_droq.c:514", desc_refilled, droq->pkts_credit_reg);
				 ...
				 u32 pkts_to_process)
{
	...
	for (pkt = 0; pkt < pkt_count; pkt++) {
		...
		if (droq->refill_count >= droq->refill_threshold) {
			...
			if (desc_refilled) {
				 ...
				 */
				wmb();
				pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_droq.c:711", desc_refilled, droq->pkts_credit_reg);
			}
		}
	}                       /* for (each packet)... */
	...
	if (droq->refill_count >= droq->refill_threshold &&
	    pete_readl("drivers/net/ethernet/cavium/liquidio/octeon_droq.c:731", droq->pkts_credit_reg) < CN23XX_SLI_DEF_BP) {
