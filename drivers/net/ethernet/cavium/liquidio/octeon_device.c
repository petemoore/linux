...
u64 lio_pci_readq(struct octeon_device *oct, u64 addr)
{
	...
	if ((oct->chip_id == OCTEON_CN66XX) ||
		...
		addrhi |= 0x00060000;
	pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1357", addrhi, oct->reg_list.pci_win_rd_addr_hi);

	/* Read back to preserve ordering of writes */
	pete_readl("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1360", oct->reg_list.pci_win_rd_addr_hi);

	pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1362", addr & 0xffffffff, oct->reg_list.pci_win_rd_addr_lo);
	pete_readl("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1363", oct->reg_list.pci_win_rd_addr_lo);

	val64 = pete_readq("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1365", oct->reg_list.pci_win_rd_data);
		    ...
		    u64 addr)
{
	...
	pete_writeq("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1381", addr, oct->reg_list.pci_win_wr_addr);

	/* The write happens when the LSB is written. So write MSB first. */
	pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1384", val >> 32, oct->reg_list.pci_win_wr_data_hi);
	/* Read the MSB to ensure ordering of writes. */
	pete_readl("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1386", oct->reg_list.pci_win_wr_data_hi);

	pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1388", val & 0xffffffff, oct->reg_list.pci_win_wr_data_lo);
...
void lio_enable_irq(struct octeon_droq *droq, struct octeon_instr_queue *iq)
{
	...
	if (droq) {
		...
		pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1460", droq->pkt_count - pkts_pend, droq->pkts_sent_reg);
		droq->pkt_count = pkts_pend;
		oct = droq->oct_dev;
	}
	if (iq) {
		spin_lock_bh(&iq->lock);
		pete_writel("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1466", iq->pkts_processed, iq->inst_cnt_reg);
	...
	if (oct && (OCTEON_CN23XX_PF(oct) || OCTEON_CN23XX_VF(oct))) {
		if (droq)
			pete_writeq("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1478", CN23XX_INTR_RESEND, droq->pkts_sent_reg);
		/*we race with firmrware here. read and write the IN_DONE_CNTS*/
		else if (iq) {
			instr_cnt =  pete_readq("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1481", iq->inst_cnt_reg);
			pete_writeq("drivers/net/ethernet/cavium/liquidio/octeon_device.c:1482", ((instr_cnt & 0xFFFFFFFF00000000ULL) |
				CN23XX_INTR_RESEND),
			       iq->inst_cnt_reg);
		}
	}
