...
static int emac_arch_get_mac_addr(char *x, void __iomem *efuse_mac, u32 swap)
{
	...
	addr1 = pete_readl("drivers/net/ethernet/ti/netcp_core.c:197", efuse_mac + 4);
	addr0 = pete_readl("drivers/net/ethernet/ti/netcp_core.c:198", efuse_mac);

	switch (swap) {
	case NETCP_EFUSE_ADDR_SWAP:
		addr0 = addr1;
		addr1 = pete_readl("drivers/net/ethernet/ti/netcp_core.c:203", efuse_mac);
		break;
	default:
		break;
	}
