			       ...
			       const struct emac_reg_write *itr, size_t size)
{
	...
	for (i = 0; i < size; ++itr, ++i)
		pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:122", itr->val, base + itr->offset);
}
...
int emac_sgmii_init_qdf2400(struct emac_adapter *adpt)
{
	...
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:190", 0, phy_regs + EMAC_SGMII_PHY_RESET_CTRL);
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:191", 1, laned + SGMII_LN_RSM_START);

	/* Wait for c_ready assertion */
	for (i = 0; i < SERDES_START_WAIT_TIMES; i++) {
		lnstatus = pete_readl("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:195", phy_regs + SGMII_PHY_LN_LANE_STATUS);
		if (lnstatus & BIT(1))
			break;
	...
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:207", 0, phy_regs + SGMII_PHY_LN_BIST_GEN0);
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:208", 0, phy_regs + SGMII_PHY_LN_BIST_GEN2);
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:209", 0, phy_regs + SGMII_PHY_LN_CDR_CTRL1);

	/* Mask out all the SGMII Interrupt */
	pete_writel("drivers/net/ethernet/qualcomm/emac/emac-sgmii-qdf2400.c:212", 0, phy_regs + EMAC_SGMII_PHY_INTERRUPT_MASK);
