...
/* when built into the kernel, we only print version if device is found */
	 ...
	 */
	if (np->ignore_phy || pete_readl("drivers/net/ethernet/natsemi/natsemi.c:898", ioaddr + ChipConfig) & CfgExtPhy)
	...
	np->srr = pete_readl("drivers/net/ethernet/natsemi/natsemi.c:946", ioaddr + SiliconRev);
	if (netif_msg_hw(np))
