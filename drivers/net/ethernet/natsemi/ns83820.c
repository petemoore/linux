...
static void ns83820_getmac(struct ns83820 *dev, struct net_device *ndev)
{
	...
	for (i=0; i<3; i++) {
		 ...
		 */
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1663", i*2, dev->base + RFCR);
		data = pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1664", dev->base + RFDR);
...
static void ns83820_set_multicast(struct net_device *ndev)
{
	...
	spin_lock_irq(&dev->misc_lock);
	val = (pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1691", rfcr) & and_mask) | or_mask;
	/* Ramit : RFCR Write Fix doc says RFEN must be 0 modify other bits */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1693", val & ~RFCR_RFEN, rfcr);
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1694", val, rfcr);
	spin_unlock_irq(&dev->misc_lock);
}
...
static void ns83820_run_bist(struct net_device *ndev, const char *name, u32 enable, u32 done, u32 fail)
{
	...
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1710", enable, dev->base + PTSCR);
	for (;;) {
		loops++;
		status = pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1713", dev->base + PTSCR);
		if (!(status & enable))
			break;
		if (status & done)
			break;
...
static void ns83820_mii_write_bit(struct ns83820 *dev, int bit)
{
	...
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1742", dev->MEAR_cache, dev->base + MEAR);
	pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1743", dev->base + MEAR);
	...
	/* set the output bit */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1753", dev->MEAR_cache, dev->base + MEAR);
	pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1754", dev->base + MEAR);
	...
	dev->MEAR_cache |= MEAR_MDC;
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1761", dev->MEAR_cache, dev->base + MEAR);
	pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1762", dev->base + MEAR);
...
static int ns83820_mii_read_bit(struct ns83820 *dev)
{
	...
	dev->MEAR_cache &= ~MEAR_MDC;
	dev->MEAR_cache &= ~MEAR_MDDIR;
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1775", dev->MEAR_cache, dev->base + MEAR);
	pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1776", dev->base + MEAR);

	/* Wait.  Max clock rate is 2.5MHz, this way we come in under 1MHz */
	udelay(1);

	/* drive MDC high causing the data bit to be latched */
	bit = (pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1782", dev->base + MEAR) & MEAR_MDIO) ? 1 : 0;
	dev->MEAR_cache |= MEAR_MDC;
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1784", dev->MEAR_cache, dev->base + MEAR);
			    ...
			    const struct pci_device_id *id)
{
	...
	printk("%s: ns83820.c: 0x22c: %08x, subsystem: %04x:%04x\n",
		ndev->name, le32_to_cpu(pete_readl("drivers/net/ethernet/natsemi/ns83820.c:1984", dev->base + 0x22c)),
	...
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:1995", PTSCR_RBIST_RST, dev->base + PTSCR);
	...
	dev->CFG_cache = pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2003", dev->base + CFG);
	...
	if (dev->CFG_cache & CFG_TBI_EN) {
		printk(KERN_INFO "%s: enabling optical transceiver\n",
			ndev->name);
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2039", pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2039", dev->base + GPIOR) | 0x3e8, dev->base + GPIOR);

		/* setup auto negotiation feature advertisement */
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2042", pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2042", dev->base + TANAR)
		       | TANAR_HALF_DUP | TANAR_FULL_DUP,
		       dev->base + TANAR);

		/* start auto negotiation */
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2047", TBICR_MR_AN_ENABLE | TBICR_MR_RESTART_AN,
		       dev->base + TBICR);
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2049", TBICR_MR_AN_ENABLE, dev->base + TBICR);
		dev->linkstate = LINK_AUTONEGOTIATE;

		dev->CFG_cache |= CFG_MODE_1000;
	}

	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2055", dev->CFG_cache, dev->base + CFG);
	dprintk("CFG: %08x\n", dev->CFG_cache);

	if (reset_phy) {
		printk(KERN_INFO "%s: resetting phy\n", ndev->name);
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2060", dev->CFG_cache | CFG_PHY_RST, dev->base + CFG);
		msleep(10);
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2062", dev->CFG_cache, dev->base + CFG);
	}

#if 0	/* Huh?  This sets the PCI latency register.  Should be done via
	 * the PCI layer.  FIXME.
	 */
	if (pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2068", dev->base + SRR))
		pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2069", pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2069", dev->base+0x20c) | 0xfe00, dev->base + 0x20c);
#endif
	...
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2079", TXCFG_CSI | TXCFG_HBI | TXCFG_ATP | TXCFG_MXDMA512
		| ((1600 / 32) * 0x100),
		dev->base + TXCFG);

	/* Flush the interrupt holdoff timer */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2084", 0x000, dev->base + IHR);
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2085", 0x100, dev->base + IHR);
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2086", 0x000, dev->base + IHR);
	...
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2094", RXCFG_AEP | RXCFG_ARP | RXCFG_AIRL | RXCFG_RX_FD
		| RXCFG_STRIPCRC
		//| RXCFG_ALP
		| (RXCFG_MXDMA512) | 0, dev->base + RXCFG);

	/* Disable priority queueing */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2100", 0, dev->base + PQCR);
...
#endif
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2120", VRCR_INIT_VALUE, dev->base + VRCR);
...
#define VTCR_INIT_VALUE VTCR_PPCHK
#endif
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2131", VTCR_INIT_VALUE, dev->base + VTCR);

	/* Ramit : Enable async and sync pause frames */
	/* pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2134", 0, dev->base + PCR); */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2135", (PCR_PS_MCAST | PCR_PS_DA | PCR_PSEN | PCR_FFLO_4K |
		PCR_FFHI_8K | PCR_STLO_4 | PCR_STHI_8 | PCR_PAUSE_CNT),
		dev->base + PCR);

	/* Disable Wake On Lan */
	pete_writel("drivers/net/ethernet/natsemi/ns83820.c:2140", 0, dev->base + WCSR);
	...
	printk(KERN_INFO "%s: ns83820 v" VERSION ": DP83820 v%u.%u: %pM io=0x%08lx irq=%d f=%s\n",
		ndev->name,
		(unsigned)pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2163", dev->base + SRR) >> 8,
		(unsigned)pete_readl("drivers/net/ethernet/natsemi/ns83820.c:2164", dev->base + SRR) & 0xff,
