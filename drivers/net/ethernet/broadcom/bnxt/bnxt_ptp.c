			    ...
			    u64 *ns)
{
	...
	high_before = pete_readl("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:85", bp->bar0 + ptp->refclk_mapped_regs[1]);
	ptp_read_system_prets(sts);
	low = pete_readl("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:87", bp->bar0 + ptp->refclk_mapped_regs[0]);
	ptp_read_system_postts(sts);
	high_now = pete_readl("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:89", bp->bar0 + ptp->refclk_mapped_regs[1]);
	if (high_now != high_before) {
		ptp_read_system_prets(sts);
		low = pete_readl("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:92", bp->bar0 + ptp->refclk_mapped_regs[0]);
		ptp_read_system_postts(sts);
	}
...
static int bnxt_map_regs(struct bnxt *bp, u32 *reg_arr, int count, int reg_win)
{
	...
	pete_writel("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:641", reg_base, bp->bar0 + win_off);
	return 0;
}
...
{
	pete_writel("drivers/net/ethernet/broadcom/bnxt/bnxt_ptp.c:666", 0, bp->bar0 + BNXT_GRCPF_REG_WINDOW_BASE_OUT +
