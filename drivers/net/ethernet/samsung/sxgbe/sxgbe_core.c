				     ...
				     unsigned int reg_n)
{
	...
	low_word = (addr[3] << 24) | (addr[2] << 16) |
		   (addr[1] << 8) | (addr[0]);
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:97", high_word, ioaddr + SXGBE_CORE_ADD_HIGHOFFSET(reg_n));
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:98", low_word, ioaddr + SXGBE_CORE_ADD_LOWOFFSET(reg_n));
}
				     ...
				     unsigned int reg_n)
{
	...
	high_word = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:106", ioaddr + SXGBE_CORE_ADD_HIGHOFFSET(reg_n));
	low_word = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:107", ioaddr + SXGBE_CORE_ADD_LOWOFFSET(reg_n));
...
static void sxgbe_enable_tx(void __iomem *ioaddr, bool enable)
{
	...
	tx_config = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:122", ioaddr + SXGBE_CORE_TX_CONFIG_REG);
	tx_config &= ~SXGBE_TX_ENABLE;

	if (enable)
		tx_config |= SXGBE_TX_ENABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:127", tx_config, ioaddr + SXGBE_CORE_TX_CONFIG_REG);
}
...
static void sxgbe_enable_rx(void __iomem *ioaddr, bool enable)
{
	...
	rx_config = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:134", ioaddr + SXGBE_CORE_RX_CONFIG_REG);
	rx_config &= ~SXGBE_RX_ENABLE;

	if (enable)
		rx_config |= SXGBE_RX_ENABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:139", rx_config, ioaddr + SXGBE_CORE_RX_CONFIG_REG);
}

static int sxgbe_get_controller_version(void __iomem *ioaddr)
{
	return pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:144", ioaddr + SXGBE_CORE_VERSION_REG);
}
...
{
	return pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:151", ioaddr + (SXGBE_CORE_HW_FEA_REG(feature_index)));
}

static void sxgbe_core_set_speed(void __iomem *ioaddr, unsigned char speed)
{
	u32 tx_cfg = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:156", ioaddr + SXGBE_CORE_TX_CONFIG_REG);
	...
	/* set the speed */
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:163", tx_cfg, ioaddr + SXGBE_CORE_TX_CONFIG_REG);
}
...
static void sxgbe_core_enable_rxqueue(void __iomem *ioaddr, int queue_num)
{
	...
	reg_val = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:170", ioaddr + SXGBE_CORE_RX_CTL0_REG);
	reg_val &= ~(SXGBE_CORE_RXQ_ENABLE_MASK << queue_num);
	reg_val |= SXGBE_CORE_RXQ_ENABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:173", reg_val, ioaddr + SXGBE_CORE_RX_CTL0_REG);
}
...
static void sxgbe_core_disable_rxqueue(void __iomem *ioaddr, int queue_num)
{
	...
	reg_val = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:180", ioaddr + SXGBE_CORE_RX_CTL0_REG);
	reg_val &= ~(SXGBE_CORE_RXQ_ENABLE_MASK << queue_num);
	reg_val |= SXGBE_CORE_RXQ_DISABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:183", reg_val, ioaddr + SXGBE_CORE_RX_CTL0_REG);
}
...
static void  sxgbe_set_eee_mode(void __iomem *ioaddr)
{
	 ...
	 * transmitted.
	 */
	ctrl = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:195", ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
	ctrl |= LPI_CTRL_STATUS_LPIEN | LPI_CTRL_STATUS_TXA;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:197", ctrl, ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
}
...
static void  sxgbe_reset_eee_mode(void __iomem *ioaddr)
{
	...
	ctrl = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:204", ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
	ctrl &= ~(LPI_CTRL_STATUS_LPIEN | LPI_CTRL_STATUS_TXA);
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:206", ctrl, ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
}
...
static void  sxgbe_set_eee_pls(void __iomem *ioaddr, const int link)
{
	...
	ctrl = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:213", ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
	...
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:221", ctrl, ioaddr + SXGBE_CORE_LPI_CTRL_STATUS);
}
				 ...
				 const int ls, const int tw)
{
	 ...
	 */
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:236", value, ioaddr + SXGBE_CORE_LPI_TIMER_CTRL);
}
...
static void sxgbe_enable_rx_csum(void __iomem *ioaddr)
{
	...
	ctrl = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:243", ioaddr + SXGBE_CORE_RX_CONFIG_REG);
	ctrl |= SXGBE_RX_CSUMOFFLOAD_ENABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:245", ctrl, ioaddr + SXGBE_CORE_RX_CONFIG_REG);
}
...
static void sxgbe_disable_rx_csum(void __iomem *ioaddr)
{
	...
	ctrl = pete_readl("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:252", ioaddr + SXGBE_CORE_RX_CONFIG_REG);
	ctrl &= ~SXGBE_RX_CSUMOFFLOAD_ENABLE;
	pete_writel("drivers/net/ethernet/samsung/sxgbe/sxgbe_core.c:254", ctrl, ioaddr + SXGBE_CORE_RX_CONFIG_REG);
}
