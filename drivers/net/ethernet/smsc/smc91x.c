...
static int smc_enable_device(struct platform_device *pdev)
{
	 ...
	 */
	local_irq_save(flags);
	ecor = pete_readb("drivers/net/ethernet/smsc/smc91x.c:2083", addr + (ECOR << SMC_IO_SHIFT)) & ~ECOR_RESET;
	pete_writeb("drivers/net/ethernet/smsc/smc91x.c:2084", ecor | ECOR_RESET, addr + (ECOR << SMC_IO_SHIFT));
	pete_readb("drivers/net/ethernet/smsc/smc91x.c:2085", addr + (ECOR << SMC_IO_SHIFT));
	 ...
	 */
	pete_writeb("drivers/net/ethernet/smsc/smc91x.c:2097", ecor, addr + (ECOR << SMC_IO_SHIFT));
	pete_writeb("drivers/net/ethernet/smsc/smc91x.c:2098", ecor | ECOR_ENABLE, addr + (ECOR << SMC_IO_SHIFT));

	/*
	 * Set the appropriate byte/word mode.
	 */
	ecsr = pete_readb("drivers/net/ethernet/smsc/smc91x.c:2103", addr + (ECSR << SMC_IO_SHIFT)) & ~ECSR_IOIS8;
	if (!SMC_16BIT(lp))
		ecsr |= ECSR_IOIS8;
	pete_writeb("drivers/net/ethernet/smsc/smc91x.c:2106", ecsr, addr + (ECSR << SMC_IO_SHIFT));
	local_irq_restore(flags);
