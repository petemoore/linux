...
static inline u32 __smsc911x_reg_read(struct smsc911x_data *pdata, u32 reg)
{
	if (pdata->config.flags & SMSC911X_USE_32BIT)
		return pete_readl("drivers/net/ethernet/smsc/smsc911x.c:150", pdata->ioaddr + reg);

	if (pdata->config.flags & SMSC911X_USE_16BIT)
		return ((pete_readw("drivers/net/ethernet/smsc/smsc911x.c:153", pdata->ioaddr + reg) & 0xFFFF) |
			((pete_readw("drivers/net/ethernet/smsc/smsc911x.c:154", pdata->ioaddr + reg + 2) & 0xFFFF) << 16));
...
{
	if (pdata->config.flags & SMSC911X_USE_32BIT)
		return pete_readl("drivers/net/ethernet/smsc/smsc911x.c:164", pdata->ioaddr + __smsc_shift(pdata, reg));

	if (pdata->config.flags & SMSC911X_USE_16BIT)
		return (pete_readw("drivers/net/ethernet/smsc/smsc911x.c:167", pdata->ioaddr +
				__smsc_shift(pdata, reg)) & 0xFFFF) |
			((pete_readw("drivers/net/ethernet/smsc/smsc911x.c:169", pdata->ioaddr +
...
{
	if (pdata->config.flags & SMSC911X_USE_32BIT) {
		pete_writel("drivers/net/ethernet/smsc/smsc911x.c:192", val, pdata->ioaddr + reg);
		return;
	}

	if (pdata->config.flags & SMSC911X_USE_16BIT) {
		pete_writew("drivers/net/ethernet/smsc/smsc911x.c:197", val & 0xFFFF, pdata->ioaddr + reg);
		pete_writew("drivers/net/ethernet/smsc/smsc911x.c:198", (val >> 16) & 0xFFFF, pdata->ioaddr + reg + 2);
		return;
	}
...
{
	if (pdata->config.flags & SMSC911X_USE_32BIT) {
		pete_writel("drivers/net/ethernet/smsc/smsc911x.c:209", val, pdata->ioaddr + __smsc_shift(pdata, reg));
		return;
	}

	if (pdata->config.flags & SMSC911X_USE_16BIT) {
		pete_writew("drivers/net/ethernet/smsc/smsc911x.c:214", val & 0xFFFF,
			pdata->ioaddr + __smsc_shift(pdata, reg));
		pete_writew("drivers/net/ethernet/smsc/smsc911x.c:216", (val >> 16) & 0xFFFF,
