...
#define eeprom_delay()	pete_readb("drivers/net/ethernet/realtek/8139cp.c:1658", ee_addr)
...
{
	pete_writeb("drivers/net/ethernet/realtek/8139cp.c:1698", 0, ee_addr);
	eeprom_delay ();
}
			 ...
			 int addr_len)
{
	...
	eeprom_cmd_start(ee_addr);
	for (i = 0; i < 20000; i++)
		if (pete_readb("drivers/net/ethernet/realtek/8139cp.c:1753", ee_addr) & EE_DATA_READ)
			break;
	eeprom_cmd_end(ee_addr);
