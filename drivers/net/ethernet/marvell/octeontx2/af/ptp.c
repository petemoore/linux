...
static enum hrtimer_restart ptp_reset_thresh(struct hrtimer *hrtimer)
{
	 ...
	 */
	ptp_clock_hi = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:129", ptp->reg_base + PTP_CLOCK_HI);
	if (ptp_clock_hi > 500000000) {
		period_ns = ktime_set(0, (NSEC_PER_SEC + 100 - ptp_clock_hi));
	} else {
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:133", 500000000, ptp->reg_base + PTP_PPS_THRESH_HI);
...
static u64 read_ptp_tstmp_sec_nsec(struct ptp *ptp)
{
	...
	sec = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:158", ptp->reg_base + PTP_CLOCK_SEC) & 0xFFFFFFFFUL;
	nsec = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:159", ptp->reg_base + PTP_CLOCK_HI);
	sec1 = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:160", ptp->reg_base + PTP_CLOCK_SEC) & 0xFFFFFFFFUL;
	/* check nsec rollover */
	if (sec1 > sec) {
		nsec = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:163", ptp->reg_base + PTP_CLOCK_HI);
		sec = sec1;
	}
...
{
	return pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:173", ptp->reg_base + PTP_CLOCK_HI);
}
...
static void ptp_atomic_update(struct ptp *ptp, u64 timestamp)
{
	...
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:257", timestamp, ptp->reg_base + PTP_NANO_TIMESTAMP);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:258", 0, ptp->reg_base + PTP_FRNS_TIMESTAMP);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:259", timestamp / NSEC_PER_SEC,
	...
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:264", nxt_rollover_set, ptp->reg_base + PTP_NXT_ROLLOVER_SET);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:265", curr_rollover_set, ptp->reg_base + PTP_CURR_ROLLOVER_SET);

	/* Now, initiate atomic update */
	regval = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:268", ptp->reg_base + PTP_CLOCK_CFG);
	regval &= ~PTP_CLOCK_CFG_ATOMIC_OP_MASK;
	regval |= (ATOMIC_SET << 26);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:271", regval, ptp->reg_base + PTP_CLOCK_CFG);
}
...
static void ptp_atomic_adjtime(struct ptp *ptp, s64 delta)
{
	...
	if (!atomic_inc_dec) {
		ptp_clock_hi = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:289", ptp->reg_base + PTP_CLOCK_HI);
		if (neg_adj) {
		...
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:300", delta, ptp->reg_base + PTP_NANO_TIMESTAMP);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:301", 0, ptp->reg_base + PTP_FRNS_TIMESTAMP);

		/* initiate atomic inc/dec */
		regval = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:304", ptp->reg_base + PTP_CLOCK_CFG);
		regval &= ~PTP_CLOCK_CFG_ATOMIC_OP_MASK;
		regval |= neg_adj ? (ATOMIC_DEC << 26) : (ATOMIC_INC << 26);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:307", regval, ptp->reg_base + PTP_CLOCK_CFG);
	}
}
...
static int ptp_adjfine(struct ptp *ptp, long scaled_ppm)
{
	...
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:353", comp, ptp->reg_base + PTP_CLOCK_COMP);
...
void ptp_start(struct rvu *rvu, u64 sclk, u32 ext_clk_freq, u32 extts)
{
	...
	if (is_tstmp_atomic_update_supported(rvu)) {
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:388", 0, ptp->reg_base + PTP_NANO_TIMESTAMP);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:389", 0, ptp->reg_base + PTP_FRNS_TIMESTAMP);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:390", 0, ptp->reg_base + PTP_SEC_TIMESTAMP);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:391", 0, ptp->reg_base + PTP_CURR_ROLLOVER_SET);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:392", 0x3b9aca00, ptp->reg_base + PTP_NXT_ROLLOVER_SET);
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:393", 0x3b9aca00, ptp->reg_base + PTP_SEC_ROLLOVER);
	}

	/* Enable PTP clock */
	clock_cfg = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:397", ptp->reg_base + PTP_CLOCK_CFG);
	...
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:415", clock_cfg, ptp->reg_base + PTP_CLOCK_CFG);
	clock_cfg = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:416", ptp->reg_base + PTP_CLOCK_CFG);
	clock_cfg &= ~PTP_CLOCK_CFG_ATOMIC_OP_MASK;
	clock_cfg |= (ATOMIC_SET << 26);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:419", clock_cfg, ptp->reg_base + PTP_CLOCK_CFG);

	/* Set 50% duty cycle for 1Hz output */
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:422", 0x1dcd650000000000, ptp->reg_base + PTP_PPS_HI_INCR);
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:423", 0x1dcd650000000000, ptp->reg_base + PTP_PPS_LO_INCR);
	if (cn10k_ptp_errata(ptp)) {
		...
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:433", (0x1dcd6500ULL - ptp->clock_period) << 32,
	...
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:443", clock_comp, ptp->reg_base + PTP_CLOCK_COMP);
}
...
static int ptp_get_tstmp(struct ptp *ptp, u64 *clk)
{
	...
	if (is_ptp_dev_cn10ka(ptp) || is_ptp_dev_cnf10ka(ptp)) {
		timestamp = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:451", ptp->reg_base + PTP_TIMESTAMP);
		*clk = (timestamp >> 32) * NSEC_PER_SEC + (timestamp & 0xFFFFFFFF);
	} else {
		*clk = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:454", ptp->reg_base + PTP_TIMESTAMP);
	}
...
{
	if (!cn10k_ptp_errata(ptp))
		pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:463", thresh, ptp->reg_base + PTP_PPS_THRESH_HI);
...
static int ptp_extts_on(struct ptp *ptp, int on)
{
	...
	if (cn10k_ptp_errata(ptp)) {
		if (on) {
			ptp_clock_hi = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:474", ptp->reg_base + PTP_CLOCK_HI);
...
static void ptp_remove(struct pci_dev *pdev)
{
	...
	/* Disable PTP clock */
	clock_cfg = pete_readq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:552", ptp->reg_base + PTP_CLOCK_CFG);
	clock_cfg &= ~PTP_CLOCK_CFG_PTP_EN;
	pete_writeq("drivers/net/ethernet/marvell/octeontx2/af/ptp.c:554", clock_cfg, ptp->reg_base + PTP_CLOCK_CFG);
	kfree(ptp);
}
