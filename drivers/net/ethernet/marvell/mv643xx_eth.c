...
{
	return pete_readl("drivers/net/ethernet/marvell/mv643xx_eth.c:421", mp->shared->base + offset);
}

static inline u32 rdlp(struct mv643xx_eth_private *mp, int offset)
{
	return pete_readl("drivers/net/ethernet/marvell/mv643xx_eth.c:426", mp->base + offset);
}

static inline void wrl(struct mv643xx_eth_private *mp, int offset, u32 data)
{
	pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:431", data, mp->shared->base + offset);
}

static inline void wrlp(struct mv643xx_eth_private *mp, int offset, u32 data)
{
	pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:436", data, mp->base + offset);
}
			      ...
			      const struct mbus_dram_target_info *dram)
{
	...
	for (i = 0; i < 6; i++) {
		pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2635", 0, base + WINDOW_BASE(i));
		pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2636", 0, base + WINDOW_SIZE(i));
		if (i < 4)
			pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2638", 0, base + WINDOW_REMAP_HIGH(i));
	}
	...
	for (i = 0; i < dram->num_cs; i++) {
		...
		pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2647", (cs->base & 0xffff0000) |
			(cs->mbus_attr << 8) |
			dram->mbus_dram_target_id, base + WINDOW_BASE(i));
		pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2650", (cs->size - 1) & 0xffff0000, base + WINDOW_SIZE(i));

		win_enable &= ~(1 << i);
		win_protect |= 3 << (2 * i);
	}

	pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2656", win_enable, base + WINDOW_BAR_ENABLE);
...
static void infer_hw_params(struct mv643xx_eth_shared_private *msp)
{
	 ...
	 */
	pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2667", 0x02000000, msp->base + 0x0400 + SDMA_CONFIG);
	if (pete_readl("drivers/net/ethernet/marvell/mv643xx_eth.c:2668", msp->base + 0x0400 + SDMA_CONFIG) & 0x02000000)
	 ...
	 * the new place.
	 */
	pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2678", 1, msp->base + 0x0400 + TX_BW_MTU_MOVED);
	if (pete_readl("drivers/net/ethernet/marvell/mv643xx_eth.c:2679", msp->base + 0x0400 + TX_BW_MTU_MOVED) & 1) {
		msp->tx_bw_control = TX_BW_CONTROL_NEW_LAYOUT;
	} else {
		pete_writel("drivers/net/ethernet/marvell/mv643xx_eth.c:2682", 7, msp->base + 0x0400 + TX_BW_RATE);
		if (pete_readl("drivers/net/ethernet/marvell/mv643xx_eth.c:2683", msp->base + 0x0400 + TX_BW_RATE) & 7)
