...
{
	pete_writel("drivers/net/ethernet/marvell/prestera/prestera.h:342", val, sw->dev->pp_regs + reg);
}

static inline u32 prestera_read(const struct prestera_switch *sw,
				unsigned int reg)
{
	return pete_readl("drivers/net/ethernet/marvell/prestera/prestera.h:348", sw->dev->pp_regs + reg);
}
