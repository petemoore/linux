...
static irqreturn_t netxen_intr(int irq, void *data)
{
	...
	status = pete_readl("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2281", adapter->isr_int_vec);
	...
	if (NX_IS_REVISION_P3(adapter->ahw.revision_id)) {
		...
		status = pete_readl("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2288", adapter->crb_int_state_reg);
		...
		our_int = pete_readl("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2295", adapter->crb_int_state_reg);
		...
		/* claim interrupt */
		pete_writel("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2302", (our_int & 0xffffffff), adapter->crb_int_state_reg);

		/* clear interrupt */
		netxen_nic_disable_int(sds_ring);
	}

	pete_writel("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2308", 0xffffffff, adapter->tgt_status_reg);
	/* read twice to ensure write is flushed */
	pete_readl("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2310", adapter->isr_int_vec);
	pete_readl("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2311", adapter->isr_int_vec);
...
static irqreturn_t netxen_msi_intr(int irq, void *data)
{
	...
	/* clear interrupt */
	pete_writel("drivers/net/ethernet/qlogic/netxen/netxen_nic_main.c:2324", 0xffffffff, adapter->tgt_status_reg);
