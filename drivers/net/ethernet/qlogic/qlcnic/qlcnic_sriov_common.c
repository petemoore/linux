				     ...
				     u32 *hdr, u32 *pay, u32 size)
{
	...
	for (i = 2, j = 0; j < hdr_size; i++, j++)
		*(hdr++) = pete_readl("drivers/net/ethernet/qlogic/qlcnic/qlcnic_sriov_common.c:915", QLCNIC_MBX_FW(ahw, i));
	for (; j < max; i++, j++)
		*(pay++) = pete_readl("drivers/net/ethernet/qlogic/qlcnic/qlcnic_sriov_common.c:917", QLCNIC_MBX_FW(ahw, i));
}
					  ...
					  struct qlcnic_vf_info *vf)
{
	...
	for (i = 2; i < 6; i++)
		ptr[i - 2] = pete_readl("drivers/net/ethernet/qlogic/qlcnic/qlcnic_sriov_common.c:1272", QLCNIC_MBX_FW(vf->adapter->ahw, i));
