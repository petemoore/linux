...
#define BYTE_REG_BITS_ON(x,p)       do { pete_writeb("drivers/net/ethernet/via/via-velocity.h:34", pete_readb("drivers/net/ethernet/via/via-velocity.h:34", (p))|(x),(p));} while (0)
#define WORD_REG_BITS_ON(x,p)       do { pete_writew("drivers/net/ethernet/via/via-velocity.h:35", pete_readw("drivers/net/ethernet/via/via-velocity.h:35", (p))|(x),(p));} while (0)
#define DWORD_REG_BITS_ON(x,p)      do { pete_writel("drivers/net/ethernet/via/via-velocity.h:36", pete_readl("drivers/net/ethernet/via/via-velocity.h:36", (p))|(x),(p));} while (0)

#define BYTE_REG_BITS_IS_ON(x,p)    (pete_readb("drivers/net/ethernet/via/via-velocity.h:38", (p)) & (x))
#define WORD_REG_BITS_IS_ON(x,p)    (pete_readw("drivers/net/ethernet/via/via-velocity.h:39", (p)) & (x))
#define DWORD_REG_BITS_IS_ON(x,p)   (pete_readl("drivers/net/ethernet/via/via-velocity.h:40", (p)) & (x))

#define BYTE_REG_BITS_OFF(x,p)      do { pete_writeb("drivers/net/ethernet/via/via-velocity.h:42", pete_readb("drivers/net/ethernet/via/via-velocity.h:42", (p)) & (~(x)),(p));} while (0)
#define WORD_REG_BITS_OFF(x,p)      do { pete_writew("drivers/net/ethernet/via/via-velocity.h:43", pete_readw("drivers/net/ethernet/via/via-velocity.h:43", (p)) & (~(x)),(p));} while (0)
#define DWORD_REG_BITS_OFF(x,p)     do { pete_writel("drivers/net/ethernet/via/via-velocity.h:44", pete_readl("drivers/net/ethernet/via/via-velocity.h:44", (p)) & (~(x)),(p));} while (0)

#define BYTE_REG_BITS_SET(x,m,p)    do { pete_writeb("drivers/net/ethernet/via/via-velocity.h:46",  (pete_readb("drivers/net/ethernet/via/via-velocity.h:46", (p)) & (~(m))) |(x),(p));} while (0)
#define WORD_REG_BITS_SET(x,m,p)    do { pete_writew("drivers/net/ethernet/via/via-velocity.h:47",  (pete_readw("drivers/net/ethernet/via/via-velocity.h:47", (p)) & (~(m))) |(x),(p));} while (0)
#define DWORD_REG_BITS_SET(x,m,p)   do { pete_writel("drivers/net/ethernet/via/via-velocity.h:48",  (pete_readl("drivers/net/ethernet/via/via-velocity.h:48", (p)) & (~(m)))|(x),(p));}  while (0)
 ...
 *	way but generates offsets for readl/pete_writel("drivers/net/ethernet/via/via-velocity.h:964", ) calls
 */
...
#define mac_read_isr(regs)  		pete_readl("drivers/net/ethernet/via/via-velocity.h:1147", &((regs)->ISR))
#define mac_write_isr(regs, x)  	pete_writel("drivers/net/ethernet/via/via-velocity.h:1148", (x),&((regs)->ISR))
#define mac_clear_isr(regs) 		pete_writel("drivers/net/ethernet/via/via-velocity.h:1149", 0xffffffffL,&((regs)->ISR))

#define mac_write_int_mask(mask, regs) 	pete_writel("drivers/net/ethernet/via/via-velocity.h:1151", (mask),&((regs)->IMR));
#define mac_disable_int(regs)       	pete_writel("drivers/net/ethernet/via/via-velocity.h:1152", CR0_GINTMSK1,&((regs)->CR0Clr))
#define mac_enable_int(regs)    	pete_writel("drivers/net/ethernet/via/via-velocity.h:1153", CR0_GINTMSK1,&((regs)->CR0Set))
...
#define mac_rx_queue_run(regs) {\
	pete_writeb("drivers/net/ethernet/via/via-velocity.h:1164", TRDCSR_RUN, &((regs)->RDCSRSet));\
}

#define mac_rx_queue_wake(regs) {\
	pete_writeb("drivers/net/ethernet/via/via-velocity.h:1168", TRDCSR_WAK, &((regs)->RDCSRSet));\
}

#define mac_tx_queue_run(regs, n) {\
	pete_writew("drivers/net/ethernet/via/via-velocity.h:1172", TRDCSR_RUN<<((n)*4),&((regs)->TDCSRSet));\
}

#define mac_tx_queue_wake(regs, n) {\
	pete_writew("drivers/net/ethernet/via/via-velocity.h:1176", TRDCSR_WAK<<(n*4),&((regs)->TDCSRSet));\
}
...
static inline void velocity_update_hw_mibs(struct velocity_info *vptr)
{
	...
	for (i = 0; i < HW_MIB_SIZE; i++) {
		tmp = pete_readl("drivers/net/ethernet/via/via-velocity.h:1499", &(vptr->mac_regs->MIBData)) & 0x00FFFFFFUL;
...
static inline void init_flow_control_register(struct velocity_info *vptr)
{
	...
	pete_writel("drivers/net/ethernet/via/via-velocity.h:1517", (CR0_XONEN | CR0_XHITH1 | CR0_XLTH1 | CR0_XLTH0), &regs->CR0Set);
	pete_writel("drivers/net/ethernet/via/via-velocity.h:1518", (CR0_FDXTFCEN | CR0_FDXRFCEN | CR0_HDXFCEN | CR0_XHITH0), &regs->CR0Clr);

	/* Set TxPauseTimer to 0xFFFF */
	pete_writew("drivers/net/ethernet/via/via-velocity.h:1521", 0xFFFF, &regs->tx_pause_timer);

	/* Initialize RBRDU to Rx buffer count. */
	pete_writew("drivers/net/ethernet/via/via-velocity.h:1524", vptr->options.numrx, &regs->RBRDU);
}
