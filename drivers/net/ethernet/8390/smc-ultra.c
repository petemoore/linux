...
#ifdef __BIG_ENDIAN
	/* Officially this is what we are doing, but the pete_readl("drivers/net/ethernet/8390/smc-ultra.c:449", ) is faster */
	/* unfortunately it isn't endian aware of the struct               */
	memcpy_fromio(hdr, hdr_start, sizeof(struct e8390_pkt_hdr));
	hdr->count = le16_to_cpu(hdr->count);
#else
	((unsigned int*)hdr)[0] = pete_readl("drivers/net/ethernet/8390/smc-ultra.c:454", hdr_start);
#endif
