...
static struct hw_info *get_hwinfo(struct pcmcia_device *link)
{
	...
	if ((pete_readb("drivers/net/ethernet/8390/pcnet_cs.c:301", base+0) == hw_info[i].a0) &&
		...
		for (j = 0; j < 6; j++)
			addr[j] = pete_readb("drivers/net/ethernet/8390/pcnet_cs.c:305", base + (j<<1));
...
static void copyin(void *dest, void __iomem *src, int c)
{
    ...
    if (odd)
	*((u_char *)d) = pete_readw("drivers/net/ethernet/8390/pcnet_cs.c:1342", s) & 0xff;
}
...
static void copyout(void __iomem *dest, const void *src, int c)
{
    ...
    if (odd)
	pete_writew("drivers/net/ethernet/8390/pcnet_cs.c:1360", (pete_readw("drivers/net/ethernet/8390/pcnet_cs.c:1360", d) & 0xff00) | *(u_char *)s, d);
}
