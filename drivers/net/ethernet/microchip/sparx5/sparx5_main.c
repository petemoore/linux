...
static int sparx5_init_ram(struct sparx5 *s5)
{
	...
	for (jdx = 0; jdx < 10; jdx++) {
		...
		for (idx = 0; idx < ARRAY_SIZE(spx5_ram_cfg); idx++) {
			...
			if (jdx == 0) {
				pete_writel("drivers/net/ethernet/microchip/sparx5/sparx5_main.c:364", cfg->init_val, cfg->init_reg);
			} else {
				value = pete_readl("drivers/net/ethernet/microchip/sparx5/sparx5_main.c:366", cfg->init_reg);
