...
{
	u32 csr6 = pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:57", ioaddr + DMA_CONTROL);
	...
	pete_writel("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:66", csr6, ioaddr + DMA_CONTROL);
}
				   ...
				   void __iomem *ioaddr, u32 *reg_space)
{
	...
	for (i = 0; i < NUM_DWMAC100_DMA_REGS; i++)
		reg_space[DMA_BUS_MODE / 4 + i] =
			pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:76", ioaddr + DMA_BUS_MODE + i * 4);

	reg_space[DMA_CUR_TX_BUF_ADDR / 4] =
		pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:79", ioaddr + DMA_CUR_TX_BUF_ADDR);
	reg_space[DMA_CUR_RX_BUF_ADDR / 4] =
		pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:81", ioaddr + DMA_CUR_RX_BUF_ADDR);
}
...
static void dwmac100_dma_diagnostic_fr(struct stmmac_extra_stats *x,
				       void __iomem *ioaddr)
{
	u32 csr8 = pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac100_dma.c:88", ioaddr + DMA_MISSED_FRAME_CTR);
