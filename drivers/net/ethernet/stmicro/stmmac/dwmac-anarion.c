...
{
	return pete_readl("drivers/net/ethernet/stmicro/stmmac/dwmac-anarion.c:29", gmac->ctl_block + reg);
};

static void gmac_write_reg(struct anarion_gmac *gmac, uint8_t reg, uint32_t val)
{
	pete_writel("drivers/net/ethernet/stmicro/stmmac/dwmac-anarion.c:34", val, gmac->ctl_block + reg);
}
