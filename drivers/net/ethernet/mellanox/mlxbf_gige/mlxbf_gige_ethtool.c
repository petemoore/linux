					 ...
					 u64 *data)
{
	...
	*data++ = (priv->stats.rx_din_dropped_pkts +
		   pete_readq("drivers/net/ethernet/mellanox/mlxbf_gige/mlxbf_gige_ethtool.c:111", priv->base + MLXBF_GIGE_RX_DIN_DROP_COUNTER));
	*data++ = priv->stats.tx_fifo_full;
	*data++ = (priv->stats.rx_filter_passed_pkts +
		   pete_readq("drivers/net/ethernet/mellanox/mlxbf_gige/mlxbf_gige_ethtool.c:114", priv->base + MLXBF_GIGE_RX_PASS_COUNTER_ALL));
	*data++ = (priv->stats.rx_filter_discard_pkts +
		   pete_readq("drivers/net/ethernet/mellanox/mlxbf_gige/mlxbf_gige_ethtool.c:116", priv->base + MLXBF_GIGE_RX_DISC_COUNTER_ALL));
}
