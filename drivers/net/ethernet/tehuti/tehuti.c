...
bdx_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
...
#ifdef BDX_MSI
	if ((pete_readl("drivers/net/ethernet/tehuti/tehuti.c:1953", nic->regs + FPGA_VER) & 0xFFF) >= 378) {
	...
	/************** priv ****************/
		...
		if ((pete_readl("drivers/net/ethernet/tehuti/tehuti.c:1999", nic->regs + FPGA_VER) & 0xFFF) == 308) {
