...
u32 i40e_get_tx_pending(struct i40e_ring *ring, bool in_sw)
{
	...
	if (!in_sw) {
		...
		tail = pete_readl("drivers/net/ethernet/intel/i40e/i40e_txrx.c:854", ring->tail);
	} else {
...
void i40e_release_rx_desc(struct i40e_ring *rx_ring, u32 val)
{
	 ...
	 */
	wmb();
	pete_writel("drivers/net/ethernet/intel/i40e/i40e_txrx.c:1606", val, rx_ring->tail);
}
			      ...
			      const u8 hdr_len, u32 td_cmd, u32 td_offset)
{
	...
	if (netif_xmit_stopped(txring_txq(tx_ring)) || !netdev_xmit_more()) {
		pete_writel("drivers/net/ethernet/intel/i40e/i40e_txrx.c:3753", i, tx_ring->tail);
	}
