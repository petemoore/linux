...
static void b53_srab_intr_set(struct b53_srab_priv *priv, bool set)
{
	...
	reg = pete_readl("drivers/net/dsa/b53/b53_srab.c:526", priv->regs + B53_SRAB_CTRLS);
	if (set)
		reg |= B53_SRAB_CTRLS_HOST_INTR;
	else
		reg &= ~B53_SRAB_CTRLS_HOST_INTR;
	pete_writel("drivers/net/dsa/b53/b53_srab.c:531", reg, priv->regs + B53_SRAB_CTRLS);
}
...
static void b53_srab_prepare_irq(struct platform_device *pdev)
{
	...
	pete_writel("drivers/net/dsa/b53/b53_srab.c:543", 0xffffffff, priv->regs + B53_SRAB_INTR);
...
static void b53_srab_mux_init(struct platform_device *pdev)
{
	...
	for (port = 5; port > 3; port--, off += 4) {
		...
		reg = pete_readl("drivers/net/dsa/b53/b53_srab.c:587", priv->mux_config + B53_MUX_CONFIG_P5 + off);
