			  ...
			  u16 rxin)
{
	...
	len = pete_readw("drivers/net/wan/hd64572.c:201", &desc->len);
...
static inline int sca_rx_done(port_t *port, int budget)
{
	...
	while (received < budget) {
		...
		stat = pete_readb("drivers/net/wan/hd64572.c:248", &desc->stat);
		if (!(stat & ST_RX_EOM))
...
static inline void sca_tx_done(port_t *port)
{
	...
	while (1) {
		...
		u8 stat = pete_readb("drivers/net/wan/hd64572.c:295", &desc->stat);
		...
		if (stat & ST_TX_UNDRRUN) {
			...
			dev->stats.tx_bytes += pete_readw("drivers/net/wan/hd64572.c:304", &desc->len);
		}
		pete_writeb("drivers/net/wan/hd64572.c:306", 0, &desc->stat);	/* Free descriptor */
		count++;
...
static void sca_dump_rings(struct net_device *dev)
{
	...
	for (cnt = 0; cnt < port->card->rx_ring_buffers; cnt++)
		pr_cont(" %02X", pete_readb("drivers/net/wan/hd64572.c:531", &(desc_address(port, cnt, 0)->stat)));
	pr_cont("\n");
	...
	for (cnt = 0; cnt < port->card->tx_ring_buffers; cnt++)
		pr_cont(" %02X", pete_readb("drivers/net/wan/hd64572.c:542", &(desc_address(port, cnt, 1)->stat)));
	pr_cont("\n");
...
static netdev_tx_t sca_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	BUG_ON(pete_readb("drivers/net/wan/hd64572.c:574", &desc->stat)); /* previous xmit should stop queue */
	...
	pete_writew("drivers/net/wan/hd64572.c:586", len, &desc->len);
	pete_writeb("drivers/net/wan/hd64572.c:587", ST_TX_EOM, &desc->stat);
	...
	desc = desc_address(port, port->txin + 1, 1);
	if (pete_readb("drivers/net/wan/hd64572.c:596", &desc->stat)) /* allow 1 packet gap */
		netif_stop_queue(dev);
...
static u32 sca_detect_ram(card_t *card, u8 __iomem *rambase, u32 ramsize)
{
	...
	do {
		i -= 4;
		pete_writel("drivers/net/wan/hd64572.c:612", i ^ 0x12345678, rambase + i);
	} while (i > 0);

	for (i = 0; i < ramsize ; i += 4) {
		if (pete_readl("drivers/net/wan/hd64572.c:616", rambase + i) != (i ^ 0x12345678))
			break;
	}
