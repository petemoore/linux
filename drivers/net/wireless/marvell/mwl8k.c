...
static void mwl8k_tx_poll(struct tasklet_struct *t)
{
	...
	if (limit) {
		pete_writel("drivers/net/wireless/marvell/mwl8k.c:4662", ~MWL8K_A2H_INT_TX_DONE,
...
static void mwl8k_rx_poll(struct tasklet_struct *t)
{
	...
	if (limit) {
		pete_writel("drivers/net/wireless/marvell/mwl8k.c:4680", ~MWL8K_A2H_INT_RX_READY,
		       priv->regs + MWL8K_HIU_A2H_INTERRUPT_STATUS);
	} else {
		tasklet_schedule(&priv->poll_rx_task);
	}
