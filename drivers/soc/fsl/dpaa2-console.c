...
{
	u32 last_byte = pete_readl("drivers/soc/fsl/dpaa2-console.c:65", &cd->hdr->last_byte);
...
static u64 get_mc_fw_base_address(void)
{
	...
	mcfwbase  = pete_readl("drivers/soc/fsl/dpaa2-console.c:81", mcfbaregs + MCFBAHR_OFFSET) &
			  MC_FW_ADDR_MASK_HIGH;
	mcfwbase <<= 32;
	mcfwbase |= pete_readl("drivers/soc/fsl/dpaa2-console.c:84", mcfbaregs + MCFBALR_OFFSET) & MC_FW_ADDR_MASK_LOW;
	iounmap(mcfbaregs);
				      ...
				      u32 offset_delta)
{
	...
	read_magic = pete_readl("drivers/soc/fsl/dpaa2-console.c:132", &cd->hdr->magic_word);
	last_byte =  pete_readl("drivers/soc/fsl/dpaa2-console.c:133", &cd->hdr->last_byte);
	buf_start =  pete_readl("drivers/soc/fsl/dpaa2-console.c:134", &cd->hdr->buf_start);
	buf_length = pete_readl("drivers/soc/fsl/dpaa2-console.c:135", &cd->hdr->buf_length);
