...
void k3_ringacc_ring_reset_dma(struct k3_ring *ring, u32 occ)
{
	...
	if (occ) {
		...
		while (db_ring_cnt != 0) {
			...
			pete_writel("drivers/soc/ti/k3-ringacc.c:553", db_ring_cnt_cur, &ring->rt->db);
				     ...
				     enum k3_ringacc_proxy_access_mode mode)
{
	...
	pete_writel("drivers/soc/ti/k3-ringacc.c:942", val, &ring->proxy->control);
	return 0;
}
...
static int k3_dmaring_fwd_pop(struct k3_ring *ring, void *elem)
{
	...
	ring->state.occ--;
	pete_writel("drivers/soc/ti/k3-ringacc.c:1121", -1, &ring->rt->db);
...
static int k3_dmaring_reverse_pop(struct k3_ring *ring, void *elem)
{
	...
	if (ring->state.occ) {
		...
		ring->state.occ--;
		pete_writel("drivers/soc/ti/k3-ringacc.c:1141", -1 & K3_DMARING_RT_DB_ENTRY_MASK, &ring->rt->db);
	} else if (ring->state.tdown_complete) {
		dma_addr_t *value = elem;

		*value = CPPI5_TDCM_MARKER;
		pete_writel("drivers/soc/ti/k3-ringacc.c:1146", K3_DMARING_RT_DB_TDOWN_ACK, &ring->rt->db);
...
static int k3_ringacc_ring_push_mem(struct k3_ring *ring, void *elem)
{
	...
	ring->state.free--;
	pete_writel("drivers/soc/ti/k3-ringacc.c:1170", 1, &ring->rt->db);
...
static int k3_ringacc_ring_pop_mem(struct k3_ring *ring, void *elem)
{
	...
	ring->state.rindex = (ring->state.rindex + 1) % ring->size;
	ring->state.occ--;
	pete_writel("drivers/soc/ti/k3-ringacc.c:1188", -1, &ring->rt->db);
			   ...
			   struct k3_ringacc *ringacc)
{
	...
	ringacc->num_proxies = pete_readl("drivers/soc/ti/k3-ringacc.c:1406", &ringacc->proxy_gcfg->config) &
	...
	dev_info(dev, "RA Proxy rev. %08x, num_proxies:%u\n",
		 pete_readl("drivers/soc/ti/k3-ringacc.c:1441", &ringacc->proxy_gcfg->revision), ringacc->num_proxies);
