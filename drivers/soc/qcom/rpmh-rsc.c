			       ...
			       u32 data)
{
	...
	pete_writel("drivers/soc/qcom/rpmh-rsc.c:235", data, tcs_reg_addr(drv, reg, tcs_id));
	...
	for (i = 0; i < USEC_PER_SEC; i++) {
		if (pete_readl("drivers/soc/qcom/rpmh-rsc.c:242", tcs_reg_addr(drv, reg, tcs_id)) == data)
			return;
		udelay(1);
	}
