...
static void qcom_smp2p_notify_in(struct qcom_smp2p *smp2p)
{
	...
	list_for_each_entry(entry, &smp2p->inbound, node) {
		...
		val = pete_readl("drivers/soc/qcom/smp2p.c:249", entry->value);
...
static int smp2p_update_bits(void *data, u32 mask, u32 value)
{
	...
	val = orig = pete_readl("drivers/soc/qcom/smp2p.c:401", entry->value);
	val &= ~mask;
	val |= value;
	pete_writel("drivers/soc/qcom/smp2p.c:404", val, entry->value);
