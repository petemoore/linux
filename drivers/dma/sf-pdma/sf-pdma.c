...
static int sf_pdma_alloc_chan_resources(struct dma_chan *dchan)
{
	...
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:125", PDMA_CLAIM_MASK, regs->ctrl);
...
static void sf_pdma_disable_request(struct sf_pdma_chan *chan)
{
	...
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:134", pete_readl("drivers/dma/sf-pdma/sf-pdma.c:134", regs->ctrl) & ~PDMA_RUN_MASK, regs->ctrl);
}
				   ...
				   dma_cookie_t cookie)
{
	...
	if (cookie == tx->cookie) {
		residue = pete_readq("drivers/dma/sf-pdma/sf-pdma.c:176", regs->residue);
	} else {
...
static void sf_pdma_enable_request(struct sf_pdma_chan *chan)
{
	...
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:235", v, regs->ctrl);
}
...
static void sf_pdma_xfer_desc(struct sf_pdma_chan *chan)
{
	...
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:261", desc->xfer_type, regs->xfer_type);
	pete_writeq("drivers/dma/sf-pdma/sf-pdma.c:262", desc->xfer_size, regs->xfer_size);
	pete_writeq("drivers/dma/sf-pdma/sf-pdma.c:263", desc->dst_addr, regs->dst_addr);
	pete_writeq("drivers/dma/sf-pdma/sf-pdma.c:264", desc->src_addr, regs->src_addr);
...
static irqreturn_t sf_pdma_done_isr(int irq, void *dev_id)
{
	...
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:348", (pete_readl("drivers/dma/sf-pdma/sf-pdma.c:348", regs->ctrl)) & ~PDMA_DONE_STATUS_MASK, regs->ctrl);
	residue = pete_readq("drivers/dma/sf-pdma/sf-pdma.c:349", regs->residue);
...
static irqreturn_t sf_pdma_err_isr(int irq, void *dev_id)
{
	...
	spin_lock(&chan->lock);
	pete_writel("drivers/dma/sf-pdma/sf-pdma.c:375", (pete_readl("drivers/dma/sf-pdma/sf-pdma.c:375", regs->ctrl)) & ~PDMA_ERR_STATUS_MASK, regs->ctrl);
...
static void sf_pdma_setup_chans(struct sf_pdma *pdma)
{
	...
	for (i = 0; i < pdma->n_chans; i++) {
		...
		pete_writel("drivers/dma/sf-pdma/sf-pdma.c:486", PDMA_CLEAR_CTRL, chan->regs.ctrl);
