...
static inline void sun6i_dma_dump_com_regs(struct sun6i_dma_dev *sdev)
{
	dev_dbg(sdev->slave.dev, "Common register:\n"
		...
		"\tpend1(%04x): 0x%08x\n"
		"\tstats(%04x): 0x%08x\n",
		DMA_IRQ_EN(0), pete_readl("drivers/dma/sun6i-dma.c:244", sdev->base + DMA_IRQ_EN(0)),
		DMA_IRQ_EN(1), pete_readl("drivers/dma/sun6i-dma.c:245", sdev->base + DMA_IRQ_EN(1)),
		DMA_IRQ_STAT(0), pete_readl("drivers/dma/sun6i-dma.c:246", sdev->base + DMA_IRQ_STAT(0)),
		DMA_IRQ_STAT(1), pete_readl("drivers/dma/sun6i-dma.c:247", sdev->base + DMA_IRQ_STAT(1)),
		DMA_STAT, pete_readl("drivers/dma/sun6i-dma.c:248", sdev->base + DMA_STAT));
}
					    ...
					    struct sun6i_pchan *pchan)
{
	dev_dbg(sdev->slave.dev, "Chan %d reg:\n"
		...
		pchan->idx,
		DMA_CHAN_ENABLE,
		pete_readl("drivers/dma/sun6i-dma.c:265", pchan->base + DMA_CHAN_ENABLE),
		DMA_CHAN_PAUSE,
		pete_readl("drivers/dma/sun6i-dma.c:267", pchan->base + DMA_CHAN_PAUSE),
		DMA_CHAN_LLI_ADDR,
		pete_readl("drivers/dma/sun6i-dma.c:269", pchan->base + DMA_CHAN_LLI_ADDR),
		DMA_CHAN_CUR_CFG,
		pete_readl("drivers/dma/sun6i-dma.c:271", pchan->base + DMA_CHAN_CUR_CFG),
		DMA_CHAN_CUR_SRC,
		pete_readl("drivers/dma/sun6i-dma.c:273", pchan->base + DMA_CHAN_CUR_SRC),
		DMA_CHAN_CUR_DST,
		pete_readl("drivers/dma/sun6i-dma.c:275", pchan->base + DMA_CHAN_CUR_DST),
		DMA_CHAN_CUR_CNT,
		pete_readl("drivers/dma/sun6i-dma.c:277", pchan->base + DMA_CHAN_CUR_CNT),
		DMA_CHAN_CUR_PARA,
		pete_readl("drivers/dma/sun6i-dma.c:279", pchan->base + DMA_CHAN_CUR_PARA));
}
...
{
	pete_writel("drivers/dma/sun6i-dma.c:305", SUN8I_DMA_GATE_ENABLE, sdev->base + SUN8I_DMA_GATE);
}

static void sun6i_enable_clock_autogate_h3(struct sun6i_dma_dev *sdev)
{
	pete_writel("drivers/dma/sun6i-dma.c:310", SUNXI_H3_DMA_GATE_ENABLE, sdev->base + SUNXI_H3_DMA_GATE);
}
...
static size_t sun6i_get_chan_size(struct sun6i_pchan *pchan)
{
	...
	pos = pete_readl("drivers/dma/sun6i-dma.c:356", pchan->base + DMA_CHAN_LLI_ADDR);
	bytes = pete_readl("drivers/dma/sun6i-dma.c:357", pchan->base + DMA_CHAN_CUR_CNT);
...
static int sun6i_dma_start_desc(struct sun6i_vchan *vchan)
{
	...
	irq_val = pete_readl("drivers/dma/sun6i-dma.c:462", sdev->base + DMA_IRQ_EN(irq_reg));
	irq_val &= ~((DMA_IRQ_HALF | DMA_IRQ_PKG | DMA_IRQ_QUEUE) <<
			(irq_offset * DMA_IRQ_CHAN_WIDTH));
	irq_val |= vchan->irq_type << (irq_offset * DMA_IRQ_CHAN_WIDTH);
	pete_writel("drivers/dma/sun6i-dma.c:466", irq_val, sdev->base + DMA_IRQ_EN(irq_reg));

	pete_writel("drivers/dma/sun6i-dma.c:468", pchan->desc->p_lli, pchan->base + DMA_CHAN_LLI_ADDR);
	pete_writel("drivers/dma/sun6i-dma.c:469", DMA_CHAN_ENABLE_START, pchan->base + DMA_CHAN_ENABLE);
...
static irqreturn_t sun6i_dma_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < sdev->num_pchans / DMA_IRQ_CHAN_NR; i++) {
		status = pete_readl("drivers/dma/sun6i-dma.c:551", sdev->base + DMA_IRQ_STAT(i));
		if (!status)
			continue;
		...
		pete_writel("drivers/dma/sun6i-dma.c:558", status, sdev->base + DMA_IRQ_STAT(i));
...
static int sun6i_dma_pause(struct dma_chan *chan)
{
	...
	if (pchan) {
		pete_writel("drivers/dma/sun6i-dma.c:873", DMA_CHAN_PAUSE_PAUSE,
...
static int sun6i_dma_resume(struct dma_chan *chan)
{
	...
	if (pchan) {
		pete_writel("drivers/dma/sun6i-dma.c:896", DMA_CHAN_PAUSE_RESUME,
		       pchan->base + DMA_CHAN_PAUSE);
...
static int sun6i_dma_terminate_all(struct dma_chan *chan)
{
	...
	if (pchan) {
		pete_writel("drivers/dma/sun6i-dma.c:936", DMA_CHAN_ENABLE_STOP, pchan->base + DMA_CHAN_ENABLE);
		pete_writel("drivers/dma/sun6i-dma.c:937", DMA_CHAN_PAUSE_RESUME, pchan->base + DMA_CHAN_PAUSE);
...
static inline void sun6i_kill_tasklet(struct sun6i_dma_dev *sdev)
{
	...
	pete_writel("drivers/dma/sun6i-dma.c:1054", 0, sdev->base + DMA_IRQ_EN(0));
	pete_writel("drivers/dma/sun6i-dma.c:1055", 0, sdev->base + DMA_IRQ_EN(1));
