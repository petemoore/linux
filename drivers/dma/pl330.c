...
static bool _until_dmac_idle(struct pl330_thread *thrd)
{
	...
	do {
		...
		if (!(pete_readl("drivers/dma/pl330.c:878", regs + DBGSTATUS) & DBG_BUSY))
			break;
		...
		u8 insn[], bool as_manager)
{
	...
	pete_writel("drivers/dma/pl330.c:907", val, regs + DBGINST0);

	val = le32_to_cpu(*((__le32 *)&insn[2]));
	pete_writel("drivers/dma/pl330.c:910", val, regs + DBGINST1);

	/* Get going */
	pete_writel("drivers/dma/pl330.c:913", 0, regs + DBGCMD);
}
...
static inline u32 _state(struct pl330_thread *thrd)
{
	...
	if (is_manager(thrd))
		val = pete_readl("drivers/dma/pl330.c:922", regs + DS) & 0xf;
	else
		val = pete_readl("drivers/dma/pl330.c:924", regs + CS(thrd->id)) & 0xf;
...
static void _stop(struct pl330_thread *thrd)
{
	...
	u32 inten = pete_readl("drivers/dma/pl330.c:978", regs + INTEN);
	...
	/* clear the event */
	if (inten & (1 << thrd->ev))
		pete_writel("drivers/dma/pl330.c:995", 1 << thrd->ev, regs + INTCLR);
	/* Stop generating interrupts for SEV */
	pete_writel("drivers/dma/pl330.c:997", inten & ~(1 << thrd->ev), regs + INTEN);
}
...
static bool _trigger(struct pl330_thread *thrd)
{
	...
	pete_writel("drivers/dma/pl330.c:1049", pete_readl("drivers/dma/pl330.c:1049", regs + INTEN) | (1 << thrd->ev), regs + INTEN);
...
static void pl330_dotask(struct tasklet_struct *t)
{
	...
	for (i = 0; i < pl330->pcfg.num_chan; i++) {
		...
		if (pl330->dmac_tbd.reset_chan & (1 << i)) {
			...
			if (pete_readl("drivers/dma/pl330.c:1614", regs + FSC) & (1 << thrd->id))
				err = PL330_ERR_FAIL;
			else
				err = PL330_ERR_ABORT;
...
static int pl330_update(struct pl330_dmac *pl330)
{
	...
	val = pete_readl("drivers/dma/pl330.c:1651", regs + FSM) & 0x1;
	if (val)
		pl330->dmac_tbd.reset_mngr = true;
	else
		pl330->dmac_tbd.reset_mngr = false;

	val = pete_readl("drivers/dma/pl330.c:1657", regs + FSC) & ((1 << pl330->pcfg.num_chan) - 1);
			...
			if (val & (1 << i)) {
				dev_info(pl330->ddma.dev,
					"Reset Channel-%d\t CS-%x FTC-%x\n",
						i, pete_readl("drivers/dma/pl330.c:1665", regs + CS(i)),
						pete_readl("drivers/dma/pl330.c:1666", regs + FTC(i)));
	...
	val = pete_readl("drivers/dma/pl330.c:1674", regs + ES);
	...
	for (ev = 0; ev < pl330->pcfg.num_events; ev++) {
		if (val & (1 << ev)) { /* Event occurred */
			...
			u32 inten = pete_readl("drivers/dma/pl330.c:1687", regs + INTEN);
			int active;

			/* Clear the event */
			if (inten & (1 << ev))
				pete_writel("drivers/dma/pl330.c:1692", 1 << ev, regs + INTCLR);
...
static void read_dmac_config(struct pl330_dmac *pl330)
{
	...
	val = pete_readl("drivers/dma/pl330.c:1827", regs + CRD) >> CRD_DATA_WIDTH_SHIFT;
	val &= CRD_DATA_WIDTH_MASK;
	pl330->pcfg.data_bus_width = 8 * (1 << val);

	val = pete_readl("drivers/dma/pl330.c:1831", regs + CRD) >> CRD_DATA_BUFF_SHIFT;
	val &= CRD_DATA_BUFF_MASK;
	pl330->pcfg.data_buf_dep = val + 1;

	val = pete_readl("drivers/dma/pl330.c:1835", regs + CR0) >> CR0_NUM_CHANS_SHIFT;
	val &= CR0_NUM_CHANS_MASK;
	val += 1;
	pl330->pcfg.num_chan = val;

	val = pete_readl("drivers/dma/pl330.c:1840", regs + CR0);
	if (val & CR0_PERIPH_REQ_SET) {
		val = (val >> CR0_NUM_PERIPH_SHIFT) & CR0_NUM_PERIPH_MASK;
		val += 1;
		pl330->pcfg.num_peri = val;
		pl330->pcfg.peri_ns = pete_readl("drivers/dma/pl330.c:1845", regs + CR4);
	} else {
		pl330->pcfg.num_peri = 0;
	}

	val = pete_readl("drivers/dma/pl330.c:1850", regs + CR0);
	if (val & CR0_BOOT_MAN_NS)
		pl330->pcfg.mode |= DMAC_MODE_NS;
	else
		pl330->pcfg.mode &= ~DMAC_MODE_NS;

	val = pete_readl("drivers/dma/pl330.c:1856", regs + CR0) >> CR0_NUM_EVENTS_SHIFT;
	val &= CR0_NUM_EVENTS_MASK;
	val += 1;
	pl330->pcfg.num_events = val;

	pl330->pcfg.irq_ns = pete_readl("drivers/dma/pl330.c:1861", regs + CR3);
}
					   ...
					   struct dma_pl330_desc *desc)
{
	...
	if (desc->rqcfg.src_inc) {
		val = pete_readl("drivers/dma/pl330.c:2390", regs + SA(thrd->id));
		addr = desc->px.src_addr;
	} else {
		val = pete_readl("drivers/dma/pl330.c:2393", regs + DA(thrd->id));
