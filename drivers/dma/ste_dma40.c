...
static void d40_phy_lli_load(struct d40_chan *chan, struct d40_desc *desc)
{
	...
	pete_writel("drivers/dma/ste_dma40.c:837", lli_src->reg_cfg, base + D40_CHAN_REG_SSCFG);
	pete_writel("drivers/dma/ste_dma40.c:838", lli_src->reg_elt, base + D40_CHAN_REG_SSELT);
	pete_writel("drivers/dma/ste_dma40.c:839", lli_src->reg_ptr, base + D40_CHAN_REG_SSPTR);
	pete_writel("drivers/dma/ste_dma40.c:840", lli_src->reg_lnk, base + D40_CHAN_REG_SSLNK);

	pete_writel("drivers/dma/ste_dma40.c:842", lli_dst->reg_cfg, base + D40_CHAN_REG_SDCFG);
	pete_writel("drivers/dma/ste_dma40.c:843", lli_dst->reg_elt, base + D40_CHAN_REG_SDELT);
	pete_writel("drivers/dma/ste_dma40.c:844", lli_dst->reg_ptr, base + D40_CHAN_REG_SDPTR);
	pete_writel("drivers/dma/ste_dma40.c:845", lli_dst->reg_lnk, base + D40_CHAN_REG_SDLNK);
}
				     ...
				     enum d40_command command)
{
	...
	if (command == D40_DMA_SUSPEND_REQ) {
		status = (pete_readl("drivers/dma/ste_dma40.c:1087", active_reg) &
	...
	pete_writel("drivers/dma/ste_dma40.c:1096", wmask | (command << D40_CHAN_POS(d40c->phy_chan->num)),
	       active_reg);

	if (command == D40_DMA_SUSPEND_REQ) {

		for (i = 0 ; i < D40_SUSPEND_MAX_IT; i++) {
			status = (pete_readl("drivers/dma/ste_dma40.c:1102", active_reg) &
				  D40_CHAN_POS_MASK(d40c->phy_chan->num)) >>
				D40_CHAN_POS(d40c->phy_chan->num);
				   ...
				   int reg)
{
	...
	case D40_DEACTIVATE_EVENTLINE:
		...
		pete_writel("drivers/dma/ste_dma40.c:1192", (D40_DEACTIVATE_EVENTLINE << D40_EVENTLINE_POS(event))
		       | ~D40_EVENTLINE_MASK(event), addr);
		break;

	case D40_SUSPEND_REQ_EVENTLINE:
		status = (pete_readl("drivers/dma/ste_dma40.c:1197", addr) & D40_EVENTLINE_MASK(event)) >>
		...
		pete_writel("drivers/dma/ste_dma40.c:1204", (D40_SUSPEND_REQ_EVENTLINE << D40_EVENTLINE_POS(event))
		       | ~D40_EVENTLINE_MASK(event), addr);

		for (tries = 0 ; tries < D40_SUSPEND_MAX_IT; tries++) {

			status = (pete_readl("drivers/dma/ste_dma40.c:1209", addr) & D40_EVENTLINE_MASK(event)) >>
				  D40_EVENTLINE_POS(event);
	 ...
	 */
		tries = 100;
		while (--tries) {
			pete_writel("drivers/dma/ste_dma40.c:1239", (D40_ACTIVATE_EVENTLINE <<
				D40_EVENTLINE_POS(event)) |
				~D40_EVENTLINE_MASK(event), addr);

			if (pete_readl("drivers/dma/ste_dma40.c:1243", addr) & D40_EVENTLINE_MASK(event))
				break;
		}
...
static u32 d40_chan_has_events(struct d40_chan *d40c)
{
	...
	val = pete_readl("drivers/dma/ste_dma40.c:1284", chanbase + D40_CHAN_REG_SSLNK);
	val |= pete_readl("drivers/dma/ste_dma40.c:1285", chanbase + D40_CHAN_REG_SDLNK);
...
__d40_execute_command_log(struct d40_chan *d40c, enum d40_command command)
{
	...
	case D40_DMA_SUSPEND_REQ:
		...
		active_status = (pete_readl("drivers/dma/ste_dma40.c:1310", active_reg) &
				 D40_CHAN_POS_MASK(d40c->phy_chan->num)) >>
				 D40_CHAN_POS(d40c->phy_chan->num);
...
static void d40_config_write(struct d40_chan *d40c)
{
	...
	var = ((u32)(chan_is_logical(d40c)) + 1) <<
		D40_CHAN_POS(d40c->phy_chan->num);
	pete_writel("drivers/dma/ste_dma40.c:1383", var, d40c->base->virtbase + D40_DREG_PRMSE + addr_base);

	/* Setup operational mode option register */
	var = d40_get_prmo(d40c) << D40_CHAN_POS(d40c->phy_chan->num);

	pete_writel("drivers/dma/ste_dma40.c:1388", var, d40c->base->virtbase + D40_DREG_PRMOE + addr_base);
	...
	if (chan_is_logical(d40c)) {
		...
		pete_writel("drivers/dma/ste_dma40.c:1396", d40c->src_def_cfg, chanbase + D40_CHAN_REG_SSCFG);
		pete_writel("drivers/dma/ste_dma40.c:1397", d40c->dst_def_cfg, chanbase + D40_CHAN_REG_SDCFG);

		/* Set LIDX for lcla */
		pete_writel("drivers/dma/ste_dma40.c:1400", lidx, chanbase + D40_CHAN_REG_SSELT);
		pete_writel("drivers/dma/ste_dma40.c:1401", lidx, chanbase + D40_CHAN_REG_SDELT);

		/* Clear LNK which will be used by d40_chan_has_events() */
		pete_writel("drivers/dma/ste_dma40.c:1404", 0, chanbase + D40_CHAN_REG_SSLNK);
		pete_writel("drivers/dma/ste_dma40.c:1405", 0, chanbase + D40_CHAN_REG_SDLNK);
	}
}
...
static u32 d40_residue(struct d40_chan *d40c)
{
	...
	if (chan_is_logical(d40c))
		num_elt = (pete_readl("drivers/dma/ste_dma40.c:1414", &d40c->lcpa->lcsp2) & D40_MEM_LCSP2_ECNT_MASK)
			>> D40_MEM_LCSP2_ECNT_POS;
	else {
		u32 val = pete_readl("drivers/dma/ste_dma40.c:1417", chan_base(d40c) + D40_CHAN_REG_SDELT);
...
static bool d40_tx_is_linked(struct d40_chan *d40c)
{
	...
	if (chan_is_logical(d40c))
		is_link = pete_readl("drivers/dma/ste_dma40.c:1430", &d40c->lcpa->lcsp3) &  D40_MEM_LCSP3_DLOS_MASK;
	else
		is_link = pete_readl("drivers/dma/ste_dma40.c:1432", chan_base(d40c) + D40_CHAN_REG_SDLNK)
...
static irqreturn_t d40_handle_interrupt(int irq, void *data)
{
	...
	for (i = 0; i < il_size; i++)
		regs[i] = pete_readl("drivers/dma/ste_dma40.c:1680", base->virtbase + il[i].src);
	...
	for (;;) {
		...
		/* ACK interrupt */
		pete_writel("drivers/dma/ste_dma40.c:1708", BIT(idx), base->virtbase + il[row].clr);
...
static bool d40_is_paused(struct d40_chan *d40c)
{
	...
	if (chan_is_physical(d40c)) {
		...
		status = (pete_readl("drivers/dma/ste_dma40.c:2090", active_reg) &
			  D40_CHAN_POS_MASK(d40c->phy_chan->num)) >>
			D40_CHAN_POS(d40c->phy_chan->num);
		if (status == D40_DMA_SUSPENDED || status == D40_DMA_STOP)
			is_paused = true;
	...
	if (d40c->dma_cfg.dir == DMA_MEM_TO_DEV ||
	    d40c->dma_cfg.dir == DMA_MEM_TO_MEM) {
		status = pete_readl("drivers/dma/ste_dma40.c:2100", chanbase + D40_CHAN_REG_SDLNK);
	} else if (d40c->dma_cfg.dir == DMA_DEV_TO_MEM) {
		status = pete_readl("drivers/dma/ste_dma40.c:2102", chanbase + D40_CHAN_REG_SSLNK);
	} else {
		chan_err(d40c, "Unknown direction\n");
		goto unlock;
	}
...
static void __d40_set_prio_rt(struct d40_chan *d40c, int dev_type, bool src)
{
	...
	pete_writel("drivers/dma/ste_dma40.c:2345", bit, d40c->base->virtbase + prioreg + group * 4);
	pete_writel("drivers/dma/ste_dma40.c:2346", bit, d40c->base->virtbase + rtreg + group * 4);
}
...
static int __init d40_phy_res_init(struct d40_base *base)
{
	...
	val[0] = pete_readl("drivers/dma/ste_dma40.c:3051", base->virtbase + D40_DREG_PRSME);
	val[1] = pete_readl("drivers/dma/ste_dma40.c:3052", base->virtbase + D40_DREG_PRSMO);
	...
	val[0] = pete_readl("drivers/dma/ste_dma40.c:3102", base->virtbase + D40_DREG_PRTYP);
	 ...
	 */
	pete_writel("drivers/dma/ste_dma40.c:3121", D40_DREG_GCC_ENABLE_ALL, base->virtbase + D40_DREG_GCC);
				     ...
				     struct d40_base **retbase)
{
	...
	for (pid = 0, i = 0; i < 4; i++)
		pid |= (pete_readl("drivers/dma/ste_dma40.c:3163", virtbase + SZ_4K - 0x20 + 4 * i)
			& 255) << (i * 8);
	for (cid = 0, i = 0; i < 4; i++)
		cid |= (pete_readl("drivers/dma/ste_dma40.c:3166", virtbase + SZ_4K - 0x10 + 4 * i)
			& 255) << (i * 8);
	...
	if (plat_data->num_of_phy_chans)
		...
		num_phy_chans = 4 * (pete_readl("drivers/dma/ste_dma40.c:3198", virtbase + D40_DREG_ICFG) & 0x7) + 4;
...
static void __init d40_hw_init(struct d40_base *base)
{
	...
	for (i = 0; i < reg_size; i++)
		pete_writel("drivers/dma/ste_dma40.c:3326", dma_init_reg[i].val,
	...
	pete_writel("drivers/dma/ste_dma40.c:3352", prmseo[1], base->virtbase + D40_DREG_PRMSE);
	pete_writel("drivers/dma/ste_dma40.c:3353", prmseo[0], base->virtbase + D40_DREG_PRMSO);
	pete_writel("drivers/dma/ste_dma40.c:3354", activeo[1], base->virtbase + D40_DREG_ACTIVE);
	pete_writel("drivers/dma/ste_dma40.c:3355", activeo[0], base->virtbase + D40_DREG_ACTIVO);

	/* Write which interrupt to enable */
	pete_writel("drivers/dma/ste_dma40.c:3358", pcmis, base->virtbase + base->gen_dmac.interrupt_en);

	/* Write which interrupt to clear */
	pete_writel("drivers/dma/ste_dma40.c:3361", pcicr, base->virtbase + base->gen_dmac.interrupt_clear);
...
static int __init d40_lcla_allocate(struct d40_base *base)
{
	...
	pete_writel("drivers/dma/ste_dma40.c:3443", virt_to_phys(base->lcla_pool.base),
...
static int __init d40_probe(struct platform_device *pdev)
{
	...
	val = pete_readl("drivers/dma/ste_dma40.c:3549", base->virtbase + D40_DREG_LCPA);
	if (base->phy_lcpa != val && val != 0) {
		dev_warn(dev,
			 "[%s] Mismatch LCPA dma 0x%x, def %08x\n",
			 __func__, val, (u32)base->phy_lcpa);
	} else
		pete_writel("drivers/dma/ste_dma40.c:3555", base->phy_lcpa, base->virtbase + D40_DREG_LCPA);
	...
	if (base->plat_data->use_esram_lcla) {
		...
		pete_writel("drivers/dma/ste_dma40.c:3580", res->start, base->virtbase + D40_DREG_LCLA);
