...
{
	pete_writel("drivers/dma/tegra210-adma.c:170", val, tdma->base_addr + tdma->cdata->global_reg_offset + reg);
}

static inline u32 tdma_read(struct tegra_adma *tdma, u32 reg)
{
	return pete_readl("drivers/dma/tegra210-adma.c:175", tdma->base_addr + tdma->cdata->global_reg_offset + reg);
}

static inline void tdma_ch_write(struct tegra_adma_chan *tdc, u32 reg, u32 val)
{
	pete_writel("drivers/dma/tegra210-adma.c:180", val, tdc->chan_addr + reg);
}

static inline u32 tdma_ch_read(struct tegra_adma_chan *tdc, u32 reg)
{
	return pete_readl("drivers/dma/tegra210-adma.c:185", tdc->chan_addr + reg);
}
