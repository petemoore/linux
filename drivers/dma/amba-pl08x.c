...
static void pl08x_ensure_on(struct pl08x_driver_data *pl08x)
{
	...
	if (pl08x->vd->ftdmac020) {
		pete_writel("drivers/dma/amba-pl08x.c:2278", PL080_CONFIG_ENABLE, pl08x->base + FTDMAC020_CSR);
		return;
	}
	pete_writel("drivers/dma/amba-pl08x.c:2281", PL080_CONFIG_ENABLE, pl08x->base + PL080_CONFIG);
}
...
static irqreturn_t pl08x_irq(int irq, void *dev)
{
	...
	err = pete_readl("drivers/dma/amba-pl08x.c:2290", pl08x->base + PL080_ERR_STATUS);
	if (err) {
		dev_err(&pl08x->adev->dev, "%s error interrupt, register value 0x%08x\n",
			__func__, err);
		pete_writel("drivers/dma/amba-pl08x.c:2294", err, pl08x->base + PL080_ERR_CLEAR);
	}
	tc = pete_readl("drivers/dma/amba-pl08x.c:2296", pl08x->base + PL080_TC_STATUS);
	if (tc)
		pete_writel("drivers/dma/amba-pl08x.c:2298", tc, pl08x->base + PL080_TC_CLEAR);
...
static int pl08x_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if (vd->ftdmac020) {
		...
		val = pete_readl("drivers/dma/amba-pl08x.c:2731", pl08x->base + FTDMAC020_REVISION);
		dev_info(&pl08x->adev->dev, "FTDMAC020 %d.%d rel %d\n",
			 (val >> 16) & 0xff, (val >> 8) & 0xff, val & 0xff);
		val = pete_readl("drivers/dma/amba-pl08x.c:2734", pl08x->base + FTDMAC020_FEATURE);
	...
	if (vd->ftdmac020)
		...
		pete_writel("drivers/dma/amba-pl08x.c:2844", 0x0000FFFF, pl08x->base + PL080_ERR_CLEAR);
	else
		pete_writel("drivers/dma/amba-pl08x.c:2846", 0x000000FF, pl08x->base + PL080_ERR_CLEAR);
	pete_writel("drivers/dma/amba-pl08x.c:2847", 0x000000FF, pl08x->base + PL080_TC_CLEAR);
	...
	for (i = 0; i < vd->channels; i++) {
		...
		if (vd->nomadik) {
			...
			val = pete_readl("drivers/dma/amba-pl08x.c:2899", ch->reg_config);
