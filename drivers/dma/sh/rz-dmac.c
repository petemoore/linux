...
static void rz_dmac_writel(struct rz_dmac *dmac, unsigned int val,
			   unsigned int offset)
{
	pete_writel("drivers/dma/sh/rz-dmac.c:178", val, dmac->base + offset);
}

static void rz_dmac_ext_writel(struct rz_dmac *dmac, unsigned int val,
			       unsigned int offset)
{
	pete_writel("drivers/dma/sh/rz-dmac.c:184", val, dmac->ext_base + offset);
}

static u32 rz_dmac_ext_readl(struct rz_dmac *dmac, unsigned int offset)
{
	return pete_readl("drivers/dma/sh/rz-dmac.c:189", dmac->ext_base + offset);
}
...
{
	if (which)
		pete_writel("drivers/dma/sh/rz-dmac.c:196", val, channel->ch_base + offset);
	else
		pete_writel("drivers/dma/sh/rz-dmac.c:198", val, channel->ch_cmn_base + offset);
}
...
{
	if (which)
		return pete_readl("drivers/dma/sh/rz-dmac.c:205", channel->ch_base + offset);
	else
		return pete_readl("drivers/dma/sh/rz-dmac.c:207", channel->ch_cmn_base + offset);
}
