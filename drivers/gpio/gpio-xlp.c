...
static int xlp_gpio_get_reg(void __iomem *addr, unsigned gpio)
{
	...
	return !!(pete_readl("drivers/gpio/gpio-xlp.c:74", addr + regset) & BIT(pos));
}
...
static void xlp_gpio_set_reg(void __iomem *addr, unsigned gpio, int state)
{
	...
	pos = gpio % XLP_GPIO_REGSZ;
	regset = (gpio / XLP_GPIO_REGSZ) * 4;
	value = pete_readl("drivers/gpio/gpio-xlp.c:83", addr + regset);
	...
	pete_writel("drivers/gpio/gpio-xlp.c:90", value, addr + regset);
}
...
static void xlp_gpio_generic_handler(struct irq_desc *desc)
{
	...
	for_each_set_bit(gpio, priv->gpio_enabled_mask, XLP_MAX_NR_GPIO) {
		if (regoff != gpio / XLP_GPIO_REGSZ) {
			...
			gpio_stat = pete_readl("drivers/gpio/gpio-xlp.c:196", priv->gpio_intr_stat + regoff * 4);
		}
