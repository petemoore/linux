...
static void altera_gpio_irq_unmask(struct irq_data *d)
{
	...
	intmask = pete_readl("drivers/gpio/gpio-altera.c:47", mm_gc->regs + ALTERA_GPIO_IRQ_MASK);
	/* Set ALTERA_GPIO_IRQ_MASK bit to unmask */
	intmask |= BIT(irqd_to_hwirq(d));
	pete_writel("drivers/gpio/gpio-altera.c:50", intmask, mm_gc->regs + ALTERA_GPIO_IRQ_MASK);
...
static void altera_gpio_irq_mask(struct irq_data *d)
{
	...
	raw_spin_lock_irqsave(&altera_gc->gpio_lock, flags);
	intmask = pete_readl("drivers/gpio/gpio-altera.c:65", mm_gc->regs + ALTERA_GPIO_IRQ_MASK);
	/* Clear ALTERA_GPIO_IRQ_MASK bit to mask */
	intmask &= ~BIT(irqd_to_hwirq(d));
	pete_writel("drivers/gpio/gpio-altera.c:68", intmask, mm_gc->regs + ALTERA_GPIO_IRQ_MASK);
	raw_spin_unlock_irqrestore(&altera_gc->gpio_lock, flags);
