...
{
	pete_writel("drivers/gpio/gpio-rockchip.c:67", (val & 0xffff) | 0xffff0000, reg);
	pete_writel("drivers/gpio/gpio-rockchip.c:68", (val >> 16) | 0xffff0000, reg + 0x4);
}

static inline u32 gpio_readl_v2(void __iomem *reg)
{
	return pete_readl("drivers/gpio/gpio-rockchip.c:73", reg + 0x4) << 16 | pete_readl("drivers/gpio/gpio-rockchip.c:73", reg);
}
					...
					u32 value, unsigned int offset)
{
	...
	if (bank->gpio_type == GPIO_TYPE_V2)
		...
		pete_writel("drivers/gpio/gpio-rockchip.c:84", value, reg);
}
				      ...
				      unsigned int offset)
{
	...
	if (bank->gpio_type == GPIO_TYPE_V2)
		...
		value = pete_readl("drivers/gpio/gpio-rockchip.c:96", reg);
					    ...
					    unsigned int offset)
{
	...
	if (bank->gpio_type == GPIO_TYPE_V2) {
		if (value)
			...
			data = BIT(bit % 16 + 16);
		pete_writel("drivers/gpio/gpio-rockchip.c:113", data, bit >= 16 ? reg + 0x4 : reg);
	} else {
		data = pete_readl("drivers/gpio/gpio-rockchip.c:115", reg);
		data &= ~BIT(bit);
		if (value)
			data |= BIT(bit);
		pete_writel("drivers/gpio/gpio-rockchip.c:119", data, reg);
	}
}
					  ...
					  u32 bit, unsigned int offset)
{
	...
	if (bank->gpio_type == GPIO_TYPE_V2) {
		data = pete_readl("drivers/gpio/gpio-rockchip.c:130", bit >= 16 ? reg + 0x4 : reg);
		data >>= bit % 16;
	} else {
		data = pete_readl("drivers/gpio/gpio-rockchip.c:133", reg);
		data >>= bit;
	}
...
static int rockchip_gpio_get(struct gpio_chip *gc, unsigned int offset)
{
	...
	data = pete_readl("drivers/gpio/gpio-rockchip.c:189", bank->reg_base + bank->gpio_regs->ext_port);
	data >>= offset;
	data &= 1;
				      ...
				      unsigned int debounce)
{
	...
	if (debounce) {
		if (div_debounce_support) {
			...
			cur_div_reg = pete_readl("drivers/gpio/gpio-rockchip.c:226", bank->reg_base +
					    reg->dbclk_div_con);
			if (cur_div_reg < div_reg)
				pete_writel("drivers/gpio/gpio-rockchip.c:229", div_reg, bank->reg_base +
				       reg->dbclk_div_con);
		...
		if (bank->toggle_edge_mode & BIT(irq)) {
			...
			do {
				...
				if (data & BIT(irq))
					...
					polarity |= BIT(irq);
				pete_writel("drivers/gpio/gpio-rockchip.c:365", polarity,
				       bank->reg_base +
				       bank->gpio_regs->int_polarity);
...
static int rockchip_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	if (type == IRQ_TYPE_EDGE_BOTH) {
		if (bank->gpio_type == GPIO_TYPE_V2) {
			 ...
			 * low otherwise high.
			 */
			data = pete_readl("drivers/gpio/gpio-rockchip.c:424", bank->reg_base + bank->gpio_regs->ext_port);
			if (data & mask)
				polarity &= ~mask;
			else
				polarity |= mask;
...
static int rockchip_get_bank_data(struct rockchip_pin_bank *bank)
{
	...
	id = pete_readl("drivers/gpio/gpio-rockchip.c:662", bank->reg_base + gpio_regs_v2.version_id);
