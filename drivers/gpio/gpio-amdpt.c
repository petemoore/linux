...
static int pt_gpio_request(struct gpio_chip *gc, unsigned offset)
{
	...
	using_pins = pete_readl("drivers/gpio/gpio-amdpt.c:41", pt_gpio->reg_base + PT_SYNC_REG);
	...
	pete_writel("drivers/gpio/gpio-amdpt.c:49", using_pins | BIT(offset), pt_gpio->reg_base + PT_SYNC_REG);
...
static void pt_gpio_free(struct gpio_chip *gc, unsigned offset)
{
	...
	using_pins = pete_readl("drivers/gpio/gpio-amdpt.c:64", pt_gpio->reg_base + PT_SYNC_REG);
	using_pins &= ~BIT(offset);
	pete_writel("drivers/gpio/gpio-amdpt.c:66", using_pins, pt_gpio->reg_base + PT_SYNC_REG);
...
static int pt_gpio_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/gpio/gpio-amdpt.c:118", 0, pt_gpio->reg_base + PT_SYNC_REG);
	pete_writel("drivers/gpio/gpio-amdpt.c:119", 0, pt_gpio->reg_base + PT_CLOCKRATE_REG);
