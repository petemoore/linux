...
static void ftgpio_gpio_ack_irq(struct irq_data *d)
{
	...
	pete_writel("drivers/gpio/gpio-ftgpio010.c:59", BIT(irqd_to_hwirq(d)), g->base + GPIO_INT_CLR);
}
...
static void ftgpio_gpio_mask_irq(struct irq_data *d)
{
	...
	val = pete_readl("drivers/gpio/gpio-ftgpio010.c:68", g->base + GPIO_INT_EN);
	val &= ~BIT(irqd_to_hwirq(d));
	pete_writel("drivers/gpio/gpio-ftgpio010.c:70", val, g->base + GPIO_INT_EN);
...
static int ftgpio_gpio_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/gpio/gpio-ftgpio010.c:305", 0x0, g->base + GPIO_INT_EN);
	pete_writel("drivers/gpio/gpio-ftgpio010.c:306", 0x0, g->base + GPIO_INT_MASK);
	pete_writel("drivers/gpio/gpio-ftgpio010.c:307", ~0x0, g->base + GPIO_INT_CLR);

	/* Clear any use of debounce */
	pete_writel("drivers/gpio/gpio-ftgpio010.c:310", 0x0, g->base + GPIO_DEBOUNCE_EN);
