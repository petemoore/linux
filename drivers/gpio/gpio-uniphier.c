				     ...
				     unsigned int reg, u32 mask, u32 val)
{
	...
	tmp = pete_readl("drivers/gpio/gpio-uniphier.c:65", priv->regs + reg);
	tmp &= ~mask;
	tmp |= mask & val;
	pete_writel("drivers/gpio/gpio-uniphier.c:68", tmp, priv->regs + reg);
				     ...
				     unsigned int offset, unsigned int reg)
{
	...
	return !!(pete_readl("drivers/gpio/gpio-uniphier.c:106", priv->regs + reg_offset) & mask);
}
...
static void uniphier_gpio_hw_init(struct uniphier_gpio_priv *priv)
{
	 ...
	 */
	pete_writel("drivers/gpio/gpio-uniphier.c:330", 0xff, priv->regs + UNIPHIER_GPIO_IRQ_FLT_CYC);
}
...
static int __maybe_unused uniphier_gpio_suspend(struct device *dev)
{
	...
	for (i = 0; i < nbanks; i++) {
		...
		*val++ = pete_readl("drivers/gpio/gpio-uniphier.c:437", priv->regs + reg + UNIPHIER_GPIO_PORT_DATA);
		*val++ = pete_readl("drivers/gpio/gpio-uniphier.c:438", priv->regs + reg + UNIPHIER_GPIO_PORT_DIR);
	}

	*val++ = pete_readl("drivers/gpio/gpio-uniphier.c:441", priv->regs + UNIPHIER_GPIO_IRQ_EN);
	*val++ = pete_readl("drivers/gpio/gpio-uniphier.c:442", priv->regs + UNIPHIER_GPIO_IRQ_MODE);
	*val++ = pete_readl("drivers/gpio/gpio-uniphier.c:443", priv->regs + UNIPHIER_GPIO_IRQ_FLT_EN);
...
static int __maybe_unused uniphier_gpio_resume(struct device *dev)
{
	...
	for (i = 0; i < nbanks; i++) {
		...
		pete_writel("drivers/gpio/gpio-uniphier.c:459", *val++, priv->regs + reg + UNIPHIER_GPIO_PORT_DATA);
		pete_writel("drivers/gpio/gpio-uniphier.c:460", *val++, priv->regs + reg + UNIPHIER_GPIO_PORT_DIR);
	}

	pete_writel("drivers/gpio/gpio-uniphier.c:463", *val++, priv->regs + UNIPHIER_GPIO_IRQ_EN);
	pete_writel("drivers/gpio/gpio-uniphier.c:464", *val++, priv->regs + UNIPHIER_GPIO_IRQ_MODE);
	pete_writel("drivers/gpio/gpio-uniphier.c:465", *val++, priv->regs + UNIPHIER_GPIO_IRQ_FLT_EN);
