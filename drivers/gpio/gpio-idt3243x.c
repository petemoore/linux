...
static void idt_gpio_unmask(struct irq_data *d)
{
	...
	ctrl->mask_cache &= ~BIT(d->hwirq);
	pete_writel("drivers/gpio/gpio-idt3243x.c:109", ctrl->mask_cache, ctrl->pic + IDT_PIC_IRQ_MASK);
...
static int idt_gpio_irq_init_hw(struct gpio_chip *gc)
{
	...
	pete_writel("drivers/gpio/gpio-idt3243x.c:120", ctrl->mask_cache, ctrl->pic + IDT_PIC_IRQ_MASK);
