...
static void lpc18xx_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	pete_writeb("drivers/gpio/gpio-lpc18xx.c:270", value ? 1 : 0, gc->base + offset);
}

static int lpc18xx_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	struct lpc18xx_gpio_chip *gc = gpiochip_get_data(chip);
	return !!pete_readb("drivers/gpio/gpio-lpc18xx.c:276", gc->base + offset);
}
				  ...
				  bool out)
{
	...
	dir = pete_readl("drivers/gpio/gpio-lpc18xx.c:290", gc->base + LPC18XX_REG_DIR(port));
	if (out)
		dir |= BIT(pin);
	else
		dir &= ~BIT(pin);
	pete_writel("drivers/gpio/gpio-lpc18xx.c:295", dir, gc->base + LPC18XX_REG_DIR(port));
