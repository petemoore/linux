...
int hfi1_pcie_ddinit(struct hfi1_devdata *dd, struct pci_dev *pdev)
{
	...
	dd->revision = pete_readq("drivers/infiniband/hw/hfi1/pcie.c:123", dd->kregbase1 + CCE_REVISION);
	if (dd->revision == ~(u64)0) {
		dd_dev_err(dd, "Cannot read chip CSRs\n");
		goto nomem;
	}

	rcv_array_count = pete_readq("drivers/infiniband/hw/hfi1/pcie.c:129", dd->kregbase1 + RCV_ARRAY_CNT);
