...
{
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:2471", cq->cq_uk.cq_id, cq->cq_uk.cq_ack_db);
}
...
static inline void irdma_get_cqp_reg_info(struct irdma_sc_cqp *cqp, u32 *val,
					  u32 *tail, u32 *error)
{
	*val = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:2735", cqp->dev->hw_regs[IRDMA_CQPTAIL]);
				    ...
				    u32 count)
{
	...
	while (i++ < count) {
		...
		if (error) {
			error = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:2755", cqp->dev->hw_regs[IRDMA_CQPERRCODES]);
			ibdev_dbg(to_ibdev(cqp->dev),
		      ...
		      struct irdma_cqp_init_info *info)
{
	...
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3129", 0, cqp->dev->hw_regs[IRDMA_CQPTAIL]);
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3130", 0, cqp->dev->hw_regs[IRDMA_CQPDB]);
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3131", 0, cqp->dev->hw_regs[IRDMA_CCQPSTATUS]);
...
int irdma_sc_cqp_create(struct irdma_sc_cqp *cqp, u16 *maj_err, u16 *min_err)
{
	...
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3210", p1, cqp->dev->hw_regs[IRDMA_CCQPHIGH]);
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3211", p2, cqp->dev->hw_regs[IRDMA_CCQPLOW]);
	...
	do {
		...
		udelay(cqp->dev->hw_attrs.max_sleep_count);
		val = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:3219", cqp->dev->hw_regs[IRDMA_CCQPSTATUS]);
	} while (!val);
...
err:
	...
	cqp->sdbuf.va = NULL;
	err_code = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:3234", cqp->dev->hw_regs[IRDMA_CQPERRCODES]);
...
{
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3246", IRDMA_RING_CURRENT_HEAD(cqp->sq_ring), cqp->dev->cqp_db);
...
int irdma_sc_cqp_destroy(struct irdma_sc_cqp *cqp)
{
	...
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3296", 0, cqp->dev->hw_regs[IRDMA_CCQPHIGH]);
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3297", 0, cqp->dev->hw_regs[IRDMA_CCQPLOW]);
	do {
		if (cnt++ > cqp->dev->hw_attrs.max_done_count) {
			ret_code = -ETIMEDOUT;
			break;
		...
		udelay(cqp->dev->hw_attrs.max_sleep_count);
		val = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:3304", cqp->dev->hw_regs[IRDMA_CCQPSTATUS]);
...
void irdma_sc_ccq_arm(struct irdma_sc_cq *ccq)
{
	...
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3337", ccq->cq_uk.cq_id, ccq->dev->cq_arm_db);
}
			      ...
			      struct irdma_ccq_cqe_info *info)
{
	...
	if (info->error) {
		...
		error = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:3376", cqp->dev->hw_regs[IRDMA_CQPERRCODES]);
		ibdev_dbg(to_ibdev(cqp->dev),
			  "CQP: CQPERRCODES error_code[x%08X]\n", error);
	}
				...
				bool post_sq)
{
	...
	dev = aeq->dev;
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:3977", 0, dev->hw_regs[IRDMA_PFINT_AEQCTL]);
...
{
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:4158", count, dev->hw_regs[IRDMA_AEQALLOC]);
}
...
int irdma_cfg_fpm_val(struct irdma_sc_dev *dev, u32 qp_count)
{
	...
	if (ret_code) {
		ibdev_dbg(to_ibdev(dev),
			  ...
			  pete_readl("drivers/infiniband/hw/irdma/ctrl.c:4958", dev->hw_regs[IRDMA_CQPERRCODES]));
		return ret_code;
	}
...
void irdma_cfg_aeq(struct irdma_sc_dev *dev, u32 idx, bool enable)
{
	...
	pete_writel("drivers/infiniband/hw/irdma/ctrl.c:5308", reg_val, dev->hw_regs[IRDMA_PFINT_AEQCTL]);
}
...
static int irdma_wait_pe_ready(struct irdma_sc_dev *dev)
{
	...
	do {
		statuscpu0 = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:5339", dev->hw_regs[IRDMA_GLPE_CPUSTATUS0]);
		statuscpu1 = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:5340", dev->hw_regs[IRDMA_GLPE_CPUSTATUS1]);
		statuscpu2 = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:5341", dev->hw_regs[IRDMA_GLPE_CPUSTATUS2]);
		      ...
		      struct irdma_device_init_info *info)
{
	...
	val = pete_readl("drivers/infiniband/hw/irdma/ctrl.c:5418", dev->hw_regs[IRDMA_GLPCI_LBARCTRL]);
