...
{
	return pete_readl("drivers/perf/arm_dmc620_pmu.c:267", dmc620_pmu->base + DMC620_PMU_COUNTERn_OFFSET(idx) + reg);
}
...
{
	pete_writel("drivers/perf/arm_dmc620_pmu.c:274", val, dmc620_pmu->base + DMC620_PMU_COUNTERn_OFFSET(idx) + reg);
}
...
static irqreturn_t dmc620_pmu_handle_irq(int irq_num, void *data)
{
	...
	list_for_each_entry_rcu(dmc620_pmu, &irq->pmus_node, pmus_node) {
		...
		status = pete_readl("drivers/perf/arm_dmc620_pmu.c:390", dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLKDIV2);
		status |= (pete_readl("drivers/perf/arm_dmc620_pmu.c:391", dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLK) <<
		...
		if (status) {
			...
			if (status & DMC620_PMU_OVERFLOW_STATUS_CLKDIV2_MASK)
				pete_writel("drivers/perf/arm_dmc620_pmu.c:404", 0, dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLKDIV2);

			if ((status >> DMC620_PMU_CLKDIV2_MAX_COUNTERS) &
				DMC620_PMU_OVERFLOW_STATUS_CLK_MASK)
				pete_writel("drivers/perf/arm_dmc620_pmu.c:408", 0, dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLK);
		}
...
static int dmc620_pmu_device_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/perf/arm_dmc620_pmu.c:697", 0, dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLKDIV2);
	pete_writel("drivers/perf/arm_dmc620_pmu.c:698", 0, dmc620_pmu->base + DMC620_PMU_OVERFLOW_STATUS_CLK);
