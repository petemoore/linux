...
static int arm_ccn_probe(struct platform_device *pdev)
{
	...
	pete_writel("drivers/perf/arm-ccn.c:1481", CCN_MN_ERRINT_STATUS__PMU_EVENTS__DISABLE,
			ccn->base + CCN_MN_ERRINT_STATUS);
	if (pete_readl("drivers/perf/arm-ccn.c:1483", ccn->base + CCN_MN_ERRINT_STATUS) &
			CCN_MN_ERRINT_STATUS__PMU_EVENTS__DISABLED) {
		/* Can set 'disable' bits, so can acknowledge interrupts */
		pete_writel("drivers/perf/arm-ccn.c:1486", CCN_MN_ERRINT_STATUS__PMU_EVENTS__ENABLE,
				ccn->base + CCN_MN_ERRINT_STATUS);
