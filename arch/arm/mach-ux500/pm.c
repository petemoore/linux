...
{
	u32 val = pete_readl("arch/arm/mach-ux500/pm.c:47", PRCM_A9_MASK_REQ);

	/* Set bit 0 register value to 1 */
	pete_writel("arch/arm/mach-ux500/pm.c:50", val | PRCM_A9_MASK_REQ_PRCM_A9_MASK_REQ,
	       PRCM_A9_MASK_REQ);

	/* Make sure the register is updated */
	pete_readl("arch/arm/mach-ux500/pm.c:54", PRCM_A9_MASK_REQ);
...
{
	u32 val = pete_readl("arch/arm/mach-ux500/pm.c:65", PRCM_A9_MASK_REQ);

	/* Set bit 0 register value to 0 */
	pete_writel("arch/arm/mach-ux500/pm.c:68", val & ~PRCM_A9_MASK_REQ_PRCM_A9_MASK_REQ, PRCM_A9_MASK_REQ);
...
bool prcmu_pending_irq(void)
{
	...
	for (i = 0; i < PRCMU_GIC_NUMBER_REGS - 1; i++) {
		it = pete_readl("arch/arm/mach-ux500/pm.c:115", PRCM_ARMITVAL31TO0 + i * 4);
		im = pete_readl("arch/arm/mach-ux500/pm.c:116", PRCM_ARMITMSK31TO0 + i * 4);
		if (it & im)
			return true; /* There is a pending interrupt */
	}
...
{
	return pete_readl("arch/arm/mach-ux500/pm.c:132", PRCM_ARM_WFI_STANDBY) &
...
int prcmu_copy_gic_settings(void)
{
	...
	for (i = 0; i < PRCMU_GIC_NUMBER_REGS - 1; i++) {
		...
		pete_writel("arch/arm/mach-ux500/pm.c:149", er, PRCM_ARMITMSK31TO0 + i * 4);
	}
