...
static void mvebu_armada_pm_enter(void __iomem *sdram_reg, u32 srcmd)
{
	...
	reg = pete_readl("arch/arm/mach-mvebu/pm-board.c:32", gpio_ctrl);
	for (i = 0; i < ARMADA_PIC_NR_GPIOS; i++)
		reg &= ~BIT(pic_raw_gpios[i]);
	reg |= BIT(pic_raw_gpios[0]);
	pete_writel("arch/arm/mach-mvebu/pm-board.c:36", reg, gpio_ctrl);

	/* Prepare writing 111 to the GPIOs */
	ackcmd = pete_readl("arch/arm/mach-mvebu/pm-board.c:39", gpio_ctrl);
	for (i = 0; i < ARMADA_PIC_NR_GPIOS; i++)
