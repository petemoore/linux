...
{
	pcic_timer_dummy = pete_readl("arch/sparc/kernel/pcic.c:676", pcic0.pcic_regs+PCI_SYS_LIMIT);
}
...
static unsigned int pcic_cycles_offset(void)
{
	...
	value = pete_readl("arch/sparc/kernel/pcic.c:687", pcic0.pcic_regs + PCI_SYS_COUNTER);
...
void __init pci_time_init(void)
{
	...
	v = pete_readb("arch/sparc/kernel/pcic.c:722", pcic->pcic_regs+PCI_COUNTER_IRQ);
...
static void watchdog_reset() {
	pete_writeb("arch/sparc/kernel/pcic.c:739", 0, pcic->pcic_regs+PCI_SYS_STATUS);
}
#endif
...
static void pcic_mask_irq(struct irq_data *data)
{
	...
	local_irq_save(flags);
	pete_writel("arch/sparc/kernel/pcic.c:776", mask, pcic0.pcic_regs+PCI_SYS_INT_TARGET_MASK_SET);
	local_irq_restore(flags);
}
...
static void pcic_unmask_irq(struct irq_data *data)
{
	...
	mask = (unsigned long)data->chip_data;
	local_irq_save(flags);
	pete_writel("arch/sparc/kernel/pcic.c:786", mask, pcic0.pcic_regs+PCI_SYS_INT_TARGET_MASK_CLEAR);
	local_irq_restore(flags);
}
