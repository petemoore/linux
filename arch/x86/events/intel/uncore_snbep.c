				       ...
				       unsigned int device)
{
	if (!snr_uncore_mmio_map(box, box_ctl, mem_offset, device))
		pete_writel("arch/x86/events/intel/uncore_snbep.c:5122", IVBEP_PMON_BOX_CTL_INT, box->io_addr);
}
...
static void snr_uncore_mmio_disable_box(struct intel_uncore_box *box)
{
	...
	config = pete_readl("arch/x86/events/intel/uncore_snbep.c:5139", box->io_addr);
	config |= SNBEP_PMON_BOX_CTL_FRZ;
	pete_writel("arch/x86/events/intel/uncore_snbep.c:5141", config, box->io_addr);
}
...
static void snr_uncore_mmio_enable_box(struct intel_uncore_box *box)
{
	...
	config = pete_readl("arch/x86/events/intel/uncore_snbep.c:5151", box->io_addr);
	config &= ~SNBEP_PMON_BOX_CTL_FRZ;
	pete_writel("arch/x86/events/intel/uncore_snbep.c:5153", config, box->io_addr);
}
					   ...
					   struct perf_event *event)
{
	...
	pete_writel("arch/x86/events/intel/uncore_snbep.c:5167", hwc->config | SNBEP_PMON_CTL_EN,
					    ...
					    struct perf_event *event)
{
	...
	pete_writel("arch/x86/events/intel/uncore_snbep.c:5182", hwc->config, box->io_addr + hwc->config_base);
}
					 ...
					 struct perf_event *event)
{
	...
	if (uncore_pmc_fixed(hwc->idx))
		pete_writel("arch/x86/events/intel/uncore_snbep.c:6064", SNBEP_PMON_CTL_EN, box->io_addr + hwc->config_base);
	else
		pete_writel("arch/x86/events/intel/uncore_snbep.c:6066", hwc->config, box->io_addr + hwc->config_base);
}
