...
static void uv_init_hubless_pch_io(int offset, int mask, int data)
{
	...
	int readd = pete_readl("arch/x86/platform/uv/uv_nmi.c:315", addr);
	...
	if (mask) {			/* OR in new data */
		...
		pete_writel("arch/x86/platform/uv/uv_nmi.c:322", writed, addr);
	} else if (readd & data) {	/* clear status bit */
		nmi_debug("UV:PCH: %p = %x\n", addr, data);
		pete_writel("arch/x86/platform/uv/uv_nmi.c:325", data, addr);
	}

	(void)pete_readl("arch/x86/platform/uv/uv_nmi.c:328", addr);		/* flush write data */
}
