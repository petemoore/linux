...
static void __init nettel_smc91x_setmac(unsigned int ioaddr, unsigned int flashaddr)
{
	...
	pete_writew("arch/m68k/coldfire/nettel.c:109", 1, NETTEL_SMC0_ADDR + SMC91xx_BANKSELECT);
	pete_writew("arch/m68k/coldfire/nettel.c:110", macp[0], ioaddr + SMC91xx_BASEMAC);
	pete_writew("arch/m68k/coldfire/nettel.c:111", macp[1], ioaddr + SMC91xx_BASEMAC + 2);
	pete_writew("arch/m68k/coldfire/nettel.c:112", macp[2], ioaddr + SMC91xx_BASEMAC + 4);
}
...
{
	pete_writew("arch/m68k/coldfire/nettel.c:125", 0x00ec, MCFSIM_PADDR);
	mcf_setppdata(0, 0x0080);
	pete_writew("arch/m68k/coldfire/nettel.c:127", 1, NETTEL_SMC0_ADDR + SMC91xx_BANKSELECT);
	pete_writew("arch/m68k/coldfire/nettel.c:128", 0x0067, NETTEL_SMC0_ADDR + SMC91xx_BASEADDR);
	mcf_setppdata(0x0080, 0);

	/* Set correct chip select timing for SMC9196 accesses */
	pete_writew("arch/m68k/coldfire/nettel.c:132", 0x1180, MCFSIM_CSCR3);
