...
static void intc_irq_mask(struct irq_data *d)
{
	...
	if ((irq >= MCFINT_VECBASE) && (irq <= MCFINT_VECMAX)) {
		...
		pete_writel("arch/m68k/coldfire/intc-5272.c:89", v, intc_irqmap[irq].icr);
	}
}
...
static void intc_irq_unmask(struct irq_data *d)
{
	...
	if ((irq >= MCFINT_VECBASE) && (irq <= MCFINT_VECMAX)) {
		...
		pete_writel("arch/m68k/coldfire/intc-5272.c:101", v, intc_irqmap[irq].icr);
	}
}
...
static void intc_irq_ack(struct irq_data *d)
{
	...
	if ((irq >= MCFINT_VECBASE) && (irq <= MCFINT_VECMAX)) {
		...
		if (intc_irqmap[irq].ack) {
			u32 v;
			v = pete_readl("arch/m68k/coldfire/intc-5272.c:114", intc_irqmap[irq].icr);
			v &= (0x7 << intc_irqmap[irq].index);
			v |= (0x8 << intc_irqmap[irq].index);
			pete_writel("arch/m68k/coldfire/intc-5272.c:117", v, intc_irqmap[irq].icr);
		}
	}
}
...
static int intc_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	if ((irq >= MCFINT_VECBASE) && (irq <= MCFINT_VECMAX)) {
		...
		if (intc_irqmap[irq].ack) {
			u32 v;
			v = pete_readl("arch/m68k/coldfire/intc-5272.c:130", MCFSIM_PITR);
			if (type == IRQ_TYPE_EDGE_FALLING)
				v &= ~(0x1 << (32 - irq));
			else
				v |= (0x1 << (32 - irq));
			pete_writel("arch/m68k/coldfire/intc-5272.c:135", v, MCFSIM_PITR);
		}
	}
	return 0;
}
...
void __init init_IRQ(void)
{
	...
	pete_writel("arch/m68k/coldfire/intc-5272.c:166", 0x88888888, MCFSIM_ICR1);
	pete_writel("arch/m68k/coldfire/intc-5272.c:167", 0x88888888, MCFSIM_ICR2);
	pete_writel("arch/m68k/coldfire/intc-5272.c:168", 0x88888888, MCFSIM_ICR3);
	pete_writel("arch/m68k/coldfire/intc-5272.c:169", 0x88888888, MCFSIM_ICR4);
