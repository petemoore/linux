				   ...
				   unsigned short gain, bool on)
{
	...
	if (!(chip->mixer.src_mask & (1 << id)))
		return -EINVAL;
	oldval = val = pete_readl("sound/pci/lola/lola_mixer.c:217", &chip->mixer.array->src_gain_enable);
	if (on)
		val |= (1 << id);
	else
		val &= ~(1 << id);
	...
	if ((val == oldval) &&
	    (gain == pete_readw("sound/pci/lola/lola_mixer.c:224", &chip->mixer.array->src_gain[id])))
		return 0;

	dev_dbg(chip->card->dev,
		"lola_mixer_set_src_gain (id=%d, gain=%d) enable=%x\n",
			id, gain, val);
	pete_writew("sound/pci/lola/lola_mixer.c:230", gain, &chip->mixer.array->src_gain[id]);
	pete_writel("sound/pci/lola/lola_mixer.c:231", val, &chip->mixer.array->src_gain_enable);
				    ...
				    unsigned short *gains)
{
	...
	for (i = 0; i < LOLA_MIXER_DIM; i++) {
		if (mask & (1 << i)) {
			pete_writew("sound/pci/lola/lola_mixer.c:248", *gains, &chip->mixer.array->src_gain[i]);
			gains++;
		}
	}
	pete_writel("sound/pci/lola/lola_mixer.c:252", mask, &chip->mixer.array->src_gain_enable);
	lola_codec_flush(chip);
				       ...
				       unsigned short gain, bool on)
{
	...
	if (!(chip->mixer.src_mask & (1 << src)) ||
		...
		return -EINVAL;
	if (on)
		pete_writew("sound/pci/lola/lola_mixer.c:280", gain, &chip->mixer.array->dest_mix_gain[dest][src]);
	val = pete_readl("sound/pci/lola/lola_mixer.c:281", &chip->mixer.array->dest_mix_gain_enable[dest]);
	if (on)
		val |= (1 << src);
	else
		val &= ~(1 << src);
	pete_writel("sound/pci/lola/lola_mixer.c:286", val, &chip->mixer.array->dest_mix_gain_enable[dest]);
	lola_codec_flush(chip);
				     ...
				     unsigned int mask, unsigned short *gains)
{
	...
	for (i = 0; i < LOLA_MIXER_DIM; i++) {
		if (mask & (1 << i)) {
			pete_writew("sound/pci/lola/lola_mixer.c:303", *gains, &chip->mixer.array->dest_mix_gain[id][i]);
			gains++;
		}
	}
	pete_writel("sound/pci/lola/lola_mixer.c:307", mask, &chip->mixer.array->dest_mix_gain_enable[id]);
	lola_codec_flush(chip);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	mask = pete_readl("sound/pci/lola/lola_mixer.c:681", &chip->mixer.array->src_gain_enable);
	for (i = 0; i < count; i++) {
		...
		if (!(chip->mixer.src_mask & (1 << idx)))
			return -EINVAL;
		if (mask & (1 << idx))
			val = pete_readw("sound/pci/lola/lola_mixer.c:688", &chip->mixer.array->src_gain[idx]) + 1;
		else
			val = 0;
			      ...
			      struct snd_ctl_elem_value *ucontrol)
{
	...
	mask = pete_readl("sound/pci/lola/lola_mixer.c:764", &chip->mixer.array->dest_mix_gain_enable[dst]);
	for (i = 0; i < src_num; i++) {
		...
		if (!(chip->mixer.src_mask & (1 << src)))
			return -EINVAL;
		if (mask & (1 << dst))
			val = pete_readw("sound/pci/lola/lola_mixer.c:771", &chip->mixer.array->dest_mix_gain[dst][src]) + 1;
		else
			val = 0;
		ucontrol->value.integer.value[i] = val;
	}
