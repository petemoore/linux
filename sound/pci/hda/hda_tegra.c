...
static void hda_tegra_init(struct hda_tegra *hda)
{
	...
	/* Enable PCI access */
	v = pete_readl("sound/pci/hda/hda_tegra.c:105", hda->regs + HDA_IPFS_CONFIG);
	v |= HDA_IPFS_EN_FPCI;
	pete_writel("sound/pci/hda/hda_tegra.c:107", v, hda->regs + HDA_IPFS_CONFIG);

	/* Enable MEM/IO space and bus master */
	v = pete_readl("sound/pci/hda/hda_tegra.c:110", hda->regs + HDA_CFG_CMD);
	v &= ~HDA_DISABLE_INTR;
	v |= HDA_ENABLE_MEM_SPACE | HDA_ENABLE_IO_SPACE |
		HDA_ENABLE_BUS_MASTER | HDA_ENABLE_SERR;
	pete_writel("sound/pci/hda/hda_tegra.c:114", v, hda->regs + HDA_CFG_CMD);

	pete_writel("sound/pci/hda/hda_tegra.c:116", HDA_BAR0_INIT_PROGRAM, hda->regs + HDA_CFG_BAR0);
	pete_writel("sound/pci/hda/hda_tegra.c:117", HDA_BAR0_FINAL_PROGRAM, hda->regs + HDA_CFG_BAR0);
	pete_writel("sound/pci/hda/hda_tegra.c:118", HDA_FPCI_BAR0_START, hda->regs + HDA_IPFS_FPCI_BAR0);

	v = pete_readl("sound/pci/hda/hda_tegra.c:120", hda->regs + HDA_IPFS_INTR_MASK);
	v |= HDA_IPFS_EN_INTR;
	pete_writel("sound/pci/hda/hda_tegra.c:122", v, hda->regs + HDA_IPFS_INTR_MASK);
}
...
static int hda_tegra_first_init(struct azx *chip, struct platform_device *pdev)
{
	...
	if (of_device_is_compatible(np, "nvidia,tegra194-hda")) {
		...
		val = pete_readl("sound/pci/hda/hda_tegra.c:305", hda->regs + FPCI_DBG_CFG_2) & ~FPCI_GCAP_NSDO_MASK;
		val |= (TEGRA194_NUM_SDO_LINES >> 1) << FPCI_GCAP_NSDO_SHIFT;
		pete_writel("sound/pci/hda/hda_tegra.c:307", val, hda->regs + FPCI_DBG_CFG_2);
	}
