				  ...
				  unsigned int *val)
{
	...
	tmp = pete_readl("sound/soc/codecs/jz4725b.c:560", icdc->base + ICDC_RGADW_OFFSET);
	tmp = (tmp & ~ICDC_RGADW_RGADDR_MASK)
	    | (reg << ICDC_RGADW_RGADDR_OFFSET);
	pete_writel("sound/soc/codecs/jz4725b.c:563", tmp, icdc->base + ICDC_RGADW_OFFSET);

	/* wait 6+ cycles */
	for (i = 0; i < 6; i++)
		*val = pete_readl("sound/soc/codecs/jz4725b.c:567", icdc->base + ICDC_RGDATA_OFFSET) &
				   ...
				   unsigned int val)
{
	...
	pete_writel("sound/soc/codecs/jz4725b.c:583", ICDC_RGADW_RGWR | (reg << ICDC_RGADW_RGADDR_OFFSET) | val,
