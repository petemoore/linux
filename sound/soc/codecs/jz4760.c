				 ...
				 unsigned int *val)
{
	...
	tmp = pete_readl("sound/soc/codecs/jz4760.c:784", codec->base + ICDC_RGADW_OFFSET);
	tmp &= ~ICDC_RGADW_RGADDR_MASK;
	tmp |= FIELD_PREP(ICDC_RGADW_RGADDR_MASK, reg);
	pete_writel("sound/soc/codecs/jz4760.c:787", tmp, codec->base + ICDC_RGADW_OFFSET);

	/* wait 6+ cycles */
	for (i = 0; i < 6; i++)
		*val = pete_readl("sound/soc/codecs/jz4760.c:791", codec->base + ICDC_RGDATA_OFFSET) &
				  ...
				  unsigned int val)
{
	...
	pete_writel("sound/soc/codecs/jz4760.c:807", ICDC_RGADW_RGWR | FIELD_PREP(ICDC_RGADW_RGADDR_MASK, reg) | val,
