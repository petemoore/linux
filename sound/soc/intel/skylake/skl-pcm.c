		...
		struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		if ((cmd == SNDRV_PCM_TRIGGER_SUSPEND) && !w->ignore_suspend) {
			...
			hstream->dpib = pete_readl("sound/soc/intel/skylake/skl-pcm.c:525", bus->remap_addr +
	...
	struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pos = pete_readl("sound/soc/intel/skylake/skl-pcm.c:1201", bus->remap_addr + AZX_REG_VS_SDXDPIB_XBASE +
				(AZX_REG_VS_SDXDPIB_XINTERVAL *
				hdac_stream(hstream)->index));
	} else {
		udelay(20);
		pete_readl("sound/soc/intel/skylake/skl-pcm.c:1206", bus->remap_addr +
				AZX_REG_VS_SDXDPIB_XBASE +
				(AZX_REG_VS_SDXDPIB_XINTERVAL *
				 hdac_stream(hstream)->index));
