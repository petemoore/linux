...
static inline void kmb_i2s_enable_dma(struct kmb_i2s_info *kmb_i2s, u32 stream)
{
	...
	dma_reg = pete_readl("sound/soc/intel/keembay/kmb_platform.c:421", kmb_i2s->i2s_base + I2S_DMACR);
	...
	pete_writel("sound/soc/intel/keembay/kmb_platform.c:428", dma_reg, kmb_i2s->i2s_base + I2S_DMACR);
}
...
static inline void kmb_i2s_disable_dma(struct kmb_i2s_info *kmb_i2s, u32 stream)
{
	...
	dma_reg = pete_readl("sound/soc/intel/keembay/kmb_platform.c:435", kmb_i2s->i2s_base + I2S_DMACR);
	/* Disable DMA handshake for stream */
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		dma_reg &= ~I2S_DMAEN_TXBLOCK;
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:439", 1, kmb_i2s->i2s_base + I2S_RTXDMA);
	} else {
		dma_reg &= ~I2S_DMAEN_RXBLOCK;
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:442", 1, kmb_i2s->i2s_base + I2S_RRXDMA);
	}
	pete_writel("sound/soc/intel/keembay/kmb_platform.c:444", dma_reg, kmb_i2s->i2s_base + I2S_DMACR);
}
			  ...
			  struct snd_pcm_substream *substream)
{
	...
	pete_writel("sound/soc/intel/keembay/kmb_platform.c:453", 1, kmb_i2s->i2s_base + IER);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:456", 1, kmb_i2s->i2s_base + ITER);
	else
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:458", 1, kmb_i2s->i2s_base + IRER);
	...
	if (kmb_i2s->clock_provider)
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:467", 1, kmb_i2s->i2s_base + CER);
	else
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:469", 0, kmb_i2s->i2s_base + CER);
}
			 ...
			 struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:479", 0, kmb_i2s->i2s_base + ITER);
	else
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:481", 0, kmb_i2s->i2s_base + IRER);

	kmb_i2s_irq_trigger(kmb_i2s, substream->stream, 8, false);

	if (!kmb_i2s->active) {
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:486", 0, kmb_i2s->i2s_base + CER);
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:487", 0, kmb_i2s->i2s_base + IER);
	}
}
...
static int kmb_set_dai_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_BP_FP:
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:507", CLOCK_PROVIDER_MODE, kmb_i2s->pss_base + I2S_GEN_CFG_0);
...
static void kmb_i2s_config(struct kmb_i2s_info *kmb_i2s, int stream)
{
	...
	for (ch_reg = 0; ch_reg < config->chan_nr / 2; ch_reg++) {
		if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
			pete_writel("sound/soc/intel/keembay/kmb_platform.c:561", kmb_i2s->xfer_resolution,
			       kmb_i2s->i2s_base + TCR(ch_reg));

			pete_writel("sound/soc/intel/keembay/kmb_platform.c:564", kmb_i2s->fifo_th - 1,
			       kmb_i2s->i2s_base + TFCR(ch_reg));

			pete_writel("sound/soc/intel/keembay/kmb_platform.c:567", 1, kmb_i2s->i2s_base + TER(ch_reg));
		} else {
			pete_writel("sound/soc/intel/keembay/kmb_platform.c:569", kmb_i2s->xfer_resolution,
			       kmb_i2s->i2s_base + RCR(ch_reg));

			pete_writel("sound/soc/intel/keembay/kmb_platform.c:572", kmb_i2s->fifo_th - 1,
			       kmb_i2s->i2s_base + RFCR(ch_reg));

			pete_writel("sound/soc/intel/keembay/kmb_platform.c:575", 1, kmb_i2s->i2s_base + RER(ch_reg));
		}
	}
}
			     ...
			     struct snd_soc_dai *cpu_dai)
{
	...
	case 4:
		...
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:635", write_val, kmb_i2s->pss_base + I2S_GEN_CFG_0);
		break;
	case 2:
		/*
		...
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:649", write_val, kmb_i2s->pss_base + I2S_GEN_CFG_0);
		break;
	default:
	...
	pete_writel("sound/soc/intel/keembay/kmb_platform.c:658", kmb_i2s->ccr, kmb_i2s->i2s_base + CCR);
			   ...
			   struct snd_soc_dai *cpu_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:683", 1, kmb_i2s->i2s_base + TXFFR);
	else
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:685", 1, kmb_i2s->i2s_base + RXFFR);
			   ...
			   struct snd_soc_dai *cpu_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:718", 0, kmb_i2s->i2s_base + ITER);
	else
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:720", 0, kmb_i2s->i2s_base + IRER);
	...
	if (!kmb_i2s->active) {
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:728", 0, kmb_i2s->i2s_base + CER);
		pete_writel("sound/soc/intel/keembay/kmb_platform.c:729", 0, kmb_i2s->i2s_base + IER);
	}
...
static int kmb_plat_dai_probe(struct platform_device *pdev)
{
	...
	comp1_reg = pete_readl("sound/soc/intel/keembay/kmb_platform.c:879", kmb_i2s->i2s_base + I2S_COMP_PARAM_1);
