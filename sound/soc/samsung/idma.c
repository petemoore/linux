...
static irqreturn_t iis_irq(int irqno, void *dev_id)
{
	...
	iisahb  = pete_readl("sound/soc/samsung/idma.c:262", idma.regs + I2SAHB);

	val = (iisahb & AHB_LVL0INT) ? AHB_CLRLVL0INT : 0;

	if (val) {
		iisahb |= val;
		pete_writel("sound/soc/samsung/idma.c:268", iisahb, idma.regs + I2SAHB);

		addr = pete_readl("sound/soc/samsung/idma.c:270", idma.regs + I2SLVL0ADDR) - idma.lp_tx_addr;
		addr += prtd->periodsz;
		addr %= (u32)(prtd->end - prtd->start);
		addr += idma.lp_tx_addr;

		pete_writel("sound/soc/samsung/idma.c:275", addr, idma.regs + I2SLVL0ADDR);
