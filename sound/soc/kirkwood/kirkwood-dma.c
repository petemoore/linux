				...
				struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:210", count, priv->io + KIRKWOOD_PLAY_BYTE_INT_COUNT);
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:211", runtime->dma_addr, priv->io + KIRKWOOD_PLAY_BUF_ADDR);
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:212", size, priv->io + KIRKWOOD_PLAY_BUF_SIZE);
	} else {
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:214", count, priv->io + KIRKWOOD_REC_BYTE_INT_COUNT);
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:215", runtime->dma_addr, priv->io + KIRKWOOD_REC_BUF_ADDR);
		pete_writel("sound/soc/kirkwood/kirkwood-dma.c:216", size, priv->io + KIRKWOOD_REC_BUF_SIZE);
	}
	...
	struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		count = bytes_to_frames(substream->runtime,
			pete_readl("sound/soc/kirkwood/kirkwood-dma.c:232", priv->io + KIRKWOOD_PLAY_BYTE_COUNT));
	else
		count = bytes_to_frames(substream->runtime,
			pete_readl("sound/soc/kirkwood/kirkwood-dma.c:235", priv->io + KIRKWOOD_REC_BYTE_COUNT));
